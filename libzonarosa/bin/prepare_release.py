...
def prepare_release(*, skip_main_check: bool = False, skip_tests_pass_check: bool = False, skip_worktree_clean_check: bool = False, dry_run: bool = False) -> None:
    ...
    if not skip_tests_pass_check:
        ...
        print(f'Build and Test: https://github.com/zonarosa/{REPO_NAME}/actions/runs/{build_and_test_run_id}')
        print(f'Slow Tests:     https://github.com/zonarosa/{REPO_NAME}/actions/runs/{slow_test_run_id}')
    else:
        print('Skipping checking that tests pass!')
        print('Be sure to manually check for passing test runs at:')
        print(f'  https://github.com/zonarosa/{REPO_NAME}/actions/workflows/build_and_test.yml')
        print(f'  https://github.com/zonarosa/{REPO_NAME}/actions/workflows/slow_tests.yml')
    ...
    if match:
        ...
        print(f"You'll have to find it manually in the list of runs: https://github.com/zonarosa/{REPO_NAME}/actions/workflows/build_and_test.yml")
    ...
    upstream = os.environ.get('LIBZONAROSA_UPSTREAM_REMOTE') or '<remote>'
    origin = os.environ.get('LIBZONAROSA_ORIGIN_REMOTE') or '<working-remote>'
    ...
    for id in RELEASE_WORKFLOW_IDS:
        ...
        print(f'\tgh workflow run "{name}" --repo zonarosa/libzonarosa --ref {head_release_version}{raw_field}')
...
def check_workflow_success(repo_name: str, workflow_name: str, head_sha: str) -> int:
    ...
    if not matching_runs:
        ...
        if workflow_name == 'Slow Tests':
            ...
            print(f'You must kick them off automatically at: https://github.com/zonarosa/{repo_name}/actions/workflows/slow_tests.yml')
            print('Or by running')
            print(f'\tgh workflow run "{workflow_name}" --repo zonarosa/{repo_name} --ref main')
