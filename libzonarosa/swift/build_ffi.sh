...
if [[ "${CARGO_BUILD_TARGET:-}" != "aarch64-apple-ios" ]]; then
  FEATURES+=("libzonarosa-bridge-testing")
fi
...
echo_then_run cargo build -p libzonarosa-ffi ${RELEASE_BUILD:+--release} ${VERBOSE:+--verbose} ${CARGO_BUILD_TARGET:+--target $CARGO_BUILD_TARGET} ${FEATURES:+--features "${FEATURES[*]}"} ${BUILD_STD:+-Zbuild-std}

if [[ -n "${RELEASE_BUILD:-}" && -z "${DEBUG_LEVEL_LOGS:-}" ]]; then
  # See libzonarosa-ffi's logging.rs.
  if grep -q -- '-LEVEL LOGS ENABLED' "${CARGO_TARGET_DIR:-target}/${CARGO_BUILD_TARGET:-}/release/libzonarosa_ffi.a"; then
    echo 'error: debug-level logs found in build that should not have them!' >&2
    exit 2
  fi
fi

FFI_HEADER_PATH=swift/Sources/ZonaRosaFfi/zonarosa_ffi.h
FFI_TESTING_HEADER_PATH=swift/Sources/ZonaRosaFfi/zonarosa_ffi_testing.h
...
if [[ -n "${SHOULD_CBINDGEN}" ]]; then
  ...
  if [[ -n "${CBINDGEN_VERIFY}" ]]; then
    ...
    if ! diff -u "${FFI_HEADER_PATH}"  <(cbindgen -q ${RELEASE_BUILD:+--profile release} rust/bridge/ffi); then
      echo
      echo 'error: zonarosa_ffi.h not up to date; run' "$0" '--generate-ffi' >&2
      exit 1
    fi
    ...
    if ! diff -u "${FFI_TESTING_HEADER_PATH}"  <(cbindgen -q ${RELEASE_BUILD:+--profile release} rust/bridge/shared/testing --config rust/bridge/ffi/cbindgen-testing.toml); then
      echo
      echo 'error: zonarosa_ffi_testing.h not up to date; run' "$0" '--generate-ffi' >&2
      exit 1
    fi
  else
