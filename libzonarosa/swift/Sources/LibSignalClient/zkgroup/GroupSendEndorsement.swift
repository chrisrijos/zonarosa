...
public class GroupSendEndorsement: ByteArray, @unchecked Sendable {
    public required init(contents: Data) throws {
        try super.init(contents, checkValid: zonarosa_group_send_endorsement_check_valid_contents)
    }

    init(unchecked contents: Data) {
            ...
            return slices.withUnsafeBufferPointer { slices in
                failOnError {
                    try invokeFnReturningVariableLengthSerialized {
                        zonarosa_group_send_endorsement_combine(
                            $0,
        ...
        return failOnError {
            try withUnsafeBorrowedBuffer { endorsement in
                try toRemove.withUnsafeBorrowedBuffer { toRemove in
                    try invokeFnReturningVariableLengthSerialized {
                        zonarosa_group_send_endorsement_remove($0, endorsement, toRemove)
                    }
                }
            }
        }
    ...
    public class Token: ByteArray, @unchecked Sendable {
        public required init(contents: Data) throws {
            try super.init(contents, checkValid: zonarosa_group_send_token_check_valid_contents)
        }
        ...
        public func toFullToken(expiration: Date) -> GroupSendFullToken {
            return failOnError {
                try withUnsafeBorrowedBuffer { token in
                    try invokeFnReturningVariableLengthSerialized {
                        zonarosa_group_send_token_to_full_token($0, token, UInt64(expiration.timeIntervalSince1970))
                    }
                }
            }
        }
        ...
        return failOnError {
            try withUnsafeBorrowedBuffer { endorsement in
                try groupParams.withUnsafePointerToSerialized { groupParams in
                    try invokeFnReturningVariableLengthSerialized {
                        zonarosa_group_send_endorsement_to_token($0, endorsement, groupParams)
                    }
                }
            }
        }
