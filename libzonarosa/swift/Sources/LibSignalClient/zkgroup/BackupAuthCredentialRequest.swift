...
public class BackupAuthCredentialRequest: ByteArray, @unchecked Sendable {
    public required init(contents: Data) throws {
        try super.init(contents, checkValid: zonarosa_backup_auth_credential_request_check_valid_contents)
    }
        ...
        return failOnError {
            try withAllBorrowed(self, params, randomness) { contents, params, randomness in
                try invokeFnReturningVariableLengthSerialized {
                    zonarosa_backup_auth_credential_request_issue_deterministic(
                        $0,
                        contents,
