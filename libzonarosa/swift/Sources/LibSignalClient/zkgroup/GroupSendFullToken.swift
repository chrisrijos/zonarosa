...
public class GroupSendFullToken: ByteArray, @unchecked Sendable {
    public required init(contents: Data) throws {
        try super.init(contents, checkValid: zonarosa_group_send_full_token_check_valid_contents)
    }
        ...
        let expirationSinceEpoch: UInt64 = failOnError {
            try withUnsafeBorrowedBuffer { response in
                try invokeFnReturningInteger {
                    zonarosa_group_send_full_token_get_expiration($0, response)
                }
            }
        }
    ...
    ///
    /// - Throws: ``ZonaRosaError/verificationFailed(_:)`` if the token is invalid
    public func verify(
        userIds: [ServiceId],
        now: Date = Date(),
        ...
        try withUnsafeBorrowedBuffer { token in
            try ServiceId.concatenatedFixedWidthBinary(userIds).withUnsafeBorrowedBuffer { userIds in
                try keyPair.withUnsafeBorrowedBuffer { keyPair in
                    try checkError(
                        zonarosa_group_send_full_token_verify(token, userIds, UInt64(now.timeIntervalSince1970), keyPair)
                    )
                }
            }
        }
