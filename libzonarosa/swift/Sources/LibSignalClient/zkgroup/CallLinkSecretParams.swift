...
public class CallLinkSecretParams: ByteArray, @unchecked Sendable {
        ...
        return failOnError {
            try rootKey.withUnsafeBorrowedBuffer { rootKey in
                try invokeFnReturningVariableLengthSerialized {
                    zonarosa_call_link_secret_params_derive_from_root_key($0, rootKey)
                }
            }
        }
    }
    ...
    public required init(contents: Data) throws {
        try super.init(contents, checkValid: zonarosa_call_link_secret_params_check_valid_contents)
    }
    ...
    public func getPublicParams() -> CallLinkPublicParams {
        return failOnError {
            try withUnsafeBorrowedBuffer { contents in
                try invokeFnReturningVariableLengthSerialized {
                    zonarosa_call_link_secret_params_get_public_params($0, contents)
                }
            }
        }
    }
    ...
    public func decrypt(_ ciphertext: UuidCiphertext) throws -> Aci {
        return try withAllBorrowed(self, .fixed(ciphertext)) { contents, ciphertext in
            try invokeFnReturningServiceId {
                zonarosa_call_link_secret_params_decrypt_user_id($0, contents, ciphertext)
            }
        }
    }
}
