...
public class CreateCallLinkCredential: ByteArray, @unchecked Sendable {
    public required init(contents: Data) throws {
        try super.init(contents, checkValid: zonarosa_create_call_link_credential_check_valid_contents)
    }
                            ...
                            try callLinkParams.withUnsafeBorrowedBuffer { callLinkParams in
                                try randomness.withUnsafePointerToBytes { randomness in
                                    try invokeFnReturningVariableLengthSerialized {
                                        zonarosa_create_call_link_credential_present_deterministic(
                                            $0,
                                            contents,
                                            roomId,
                                            userId,
