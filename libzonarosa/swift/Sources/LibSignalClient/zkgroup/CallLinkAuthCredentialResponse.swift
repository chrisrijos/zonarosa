...
public class CallLinkAuthCredentialResponse: ByteArray, @unchecked Sendable {
    public required init(contents: Data) throws {
        try super.init(contents, checkValid: zonarosa_call_link_auth_credential_response_check_valid_contents)
    }
        ...
        return failOnError {
            try withAllBorrowed(userId, params, randomness) { userId, params, randomness in
                try invokeFnReturningVariableLengthSerialized {
                    zonarosa_call_link_auth_credential_response_issue_deterministic(
                        $0,
                        userId,
    ...
    public func receive(
        ...
        return try withAllBorrowed(self, userId, params) { contents, userId, params in
            try invokeFnReturningVariableLengthSerialized {
                zonarosa_call_link_auth_credential_response_receive(
                    $0,
                    contents,
                    userId,
                    UInt64(redemptionTime.timeIntervalSince1970),
