...
public class CreateCallLinkCredentialRequest: ByteArray, @unchecked Sendable {
    public required init(contents: Data) throws {
        try super.init(contents, checkValid: zonarosa_create_call_link_credential_request_check_valid_contents)
    }
                    ...
                    try params.withUnsafeBorrowedBuffer { params in
                        try randomness.withUnsafePointerToBytes { randomness in
                            try invokeFnReturningVariableLengthSerialized {
                                zonarosa_create_call_link_credential_request_issue_deterministic(
                                    $0,
                                    contents,
                                    userId,
