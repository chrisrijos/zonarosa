...
public class BackupAuthCredential: ByteArray, @unchecked Sendable {
    public required init(contents: Data) throws {
        try super.init(contents, checkValid: zonarosa_backup_auth_credential_check_valid_contents)
    }
        ...
        return failOnError {
            try withAllBorrowed(self, serverParams, randomness) { contents, serverParams, randomness in
                try invokeFnReturningVariableLengthSerialized {
                    zonarosa_backup_auth_credential_present_deterministic($0, contents, serverParams, randomness)
                }
            }
        }
    }
        ...
        return failOnError {
            try withUnsafeBorrowedBuffer { contents in
                try invokeFnReturningFixedLengthArray {
                    zonarosa_backup_auth_credential_get_backup_id($0, contents)
                }
            }
        }
    }
        ...
        return failOnError {
            let rawValue = try withUnsafeBorrowedBuffer { contents in
                try invokeFnReturningInteger {
                    zonarosa_backup_auth_credential_get_backup_level($0, contents)
                }
            }
            guard let backupLevel = BackupLevel(rawValue: rawValue) else {
                throw ZonaRosaError.internalError("Invalid BackupLevel \(rawValue)")
            }
            return backupLevel
        }
    }
    ...
    public var type: BackupCredentialType {
        return failOnError {
            let rawValue = try withUnsafeBorrowedBuffer { contents in
                try invokeFnReturningInteger {
                    zonarosa_backup_auth_credential_get_type($0, contents)
                }
            }
            guard let type = BackupCredentialType(rawValue: rawValue) else {
                throw ZonaRosaError.internalError("Invalid BackupCredentialType \(rawValue)")
            }
            return type
        }
    }
