...
public enum KeyTransparency {
    ...
    public class Client {
        ...
        /// - Throws:
        ///   - ``ZonaRosaError/keyTransparencyError`` for errors related to key transparency logic, which
        ...
        ///     different result.
        ///   - ``ZonaRosaError/keyTransparencyVerificationFailed`` when it fails to
        ...
        ///     recommended).
        ///   - Other ``ZonaRosaError``s for networking issues. These can be manually
        ///     retried, but some may indicate a possible bug in libzonarosa.
        ///
        ...
        public func search(
            ...
            let bytes = try await self.asyncContext.invokeAsyncFunction { promise, tokioContext in
                try! withAllBorrowed(
                    ...
                    zonarosa_key_transparency_search(
                        promise,
                        tokioContext.const(),
        ...
        ///     search operation results.
        /// - Throws:
        ///   - ``ZonaRosaErrorrkeyTransparencyError`` for errors related to key transparency logic, which
        ///     includes missing required fields in the serialized data. Retrying the search without
        ///     changing any of the arguments (including the state of the store) is unlikely to yield a
        ///     different result.
        ///   - ``ZonaRosaError/keyTransparencyVerificationFailed`` when it fails to
        ...
        public func monitor(
            ...
            let bytes = try await self.asyncContext.invokeAsyncFunction { promise, tokioContext in
                try! withAllBorrowed(
                    ...
                    zonarosa_key_transparency_monitor(
                        promise,
                        tokioContext.const(),
                        self.environment.rawValue,
                        chatHandle.const(),
        ...
        internal func getDistinguished(
            ...
            let bytes = try await self.asyncContext.invokeAsyncFunction { promise, tokioContext in
                try! withAllBorrowed(self.chatConnection, distinguished) { chatHandle, distinguishedBytes in
                    zonarosa_key_transparency_distinguished(
                        promise,
                        tokioContext.const(),
                        self.environment.rawValue,
                        chatHandle.const(),
