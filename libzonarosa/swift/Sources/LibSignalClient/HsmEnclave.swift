...
///
/// HsmEnclaveClient provides bindings to interact with ZonaRosa's HSM-backed enclave.
///
...
///
public class HsmEnclaveClient: NativeHandleOwner<ZonaRosaMutPointerHsmEnclaveClient> {
        ...
        let handle = try publicKey.withUnsafeBorrowedBuffer { publicKeyBuffer in
            try codeHashBytes.withUnsafeBorrowedBuffer { codeHashBuffer in
                try invokeFnReturningValueByPointer(.init()) {
                    zonarosa_hsm_enclave_client_new($0, publicKeyBuffer, codeHashBuffer)
                }
            }
        }
    ...
    override internal class func destroyNativeHandle(
        _ handle: NonNull<ZonaRosaMutPointerHsmEnclaveClient>
    ) -> ZonaRosaFfiErrorRef? {
        return zonarosa_hsm_enclave_client_destroy(handle.pointer)
    }
        ...
        return withNativeHandle { nativeHandle in
            failOnError {
                try invokeFnReturningData {
                    zonarosa_hsm_enclave_client_initial_request($0, nativeHandle.const())
                }
            }
        }
    }
    ...
    public func completeHandshake<Bytes: ContiguousBytes>(_ handshakeResponse: Bytes) throws {
        try withNativeHandle { nativeHandle in
            try handshakeResponse.withUnsafeBorrowedBuffer { buffer in
                try checkError(zonarosa_hsm_enclave_client_complete_handshake(nativeHandle, buffer))
            }
        }
    }
    ...
    public func establishedSend<Bytes: ContiguousBytes>(_ plaintextToSend: Bytes) throws -> Data {
        return try withNativeHandle { nativeHandle in
            try plaintextToSend.withUnsafeBorrowedBuffer { buffer in
                try invokeFnReturningData {
                    zonarosa_hsm_enclave_client_established_send($0, nativeHandle, buffer)
                }
            }
        }
    }
    ...
    public func establishedRecv<Bytes: ContiguousBytes>(_ receivedCiphertext: Bytes) throws -> Data {
        return try withNativeHandle { nativeHandle in
            try receivedCiphertext.withUnsafeBorrowedBuffer { buffer in
                try invokeFnReturningData {
                    zonarosa_hsm_enclave_client_established_recv($0, nativeHandle, buffer)
                }
            }
        }
    }
...
extension ZonaRosaMutPointerHsmEnclaveClient: ZonaRosaMutPointer {
    public typealias ConstPointer = ZonaRosaConstPointerHsmEnclaveClient
...
extension ZonaRosaConstPointerHsmEnclaveClient: ZonaRosaConstPointer {
    public func toOpaque() -> OpaquePointer? {
        self.raw
    }
}
