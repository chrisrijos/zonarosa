...
/// A dummy StoreContext usable with InMemoryZonaRosaProtocolStore.
...
open class InMemoryZonaRosaProtocolStore: IdentityKeyStore, PreKeyStore, SignedPreKeyStore, KyberPreKeyStore,
    SessionStore, SenderKeyStore
{
    ...
    open func loadPreKey(id: UInt32, context: StoreContext) throws -> PreKeyRecord {
        if let record = prekeyMap[id] {
            ...
            throw ZonaRosaError.invalidKeyIdentifier("no prekey with this identifier")
        }
    }
    ...
    open func loadSignedPreKey(id: UInt32, context: StoreContext) throws -> SignedPreKeyRecord {
        if let record = signedPrekeyMap[id] {
            ...
            throw ZonaRosaError.invalidKeyIdentifier("no signed prekey with this identifier")
        }
    }
    ...
    open func loadKyberPreKey(id: UInt32, context: StoreContext) throws -> KyberPreKeyRecord {
        if let record = kyberPrekeyMap[id] {
            ...
            throw ZonaRosaError.invalidKeyIdentifier("no kyber prekey with this identifier")
        }
    }
    ...
    open func markKyberPreKeyUsed(id: UInt32, signedPreKeyId: UInt32, baseKey: PublicKey, context: StoreContext) throws
    {
        ...
        if baseKeysSeen[bothKeyIds, default: []].contains(baseKey) {
            throw ZonaRosaError.invalidMessage("reused base key")
        }
