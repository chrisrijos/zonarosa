...
///
/// - Throws:
///  - `ZonaRosaError.ioError`: If an IO error on the input occurs.
///  - `ZonaRosaError.invalidMediaInput` If the input could not be parsed because it was invalid.
///  - `ZonaRosaError.unsupportedMediaInput` If the input could not be parsed because it's unsupported in some way.
public func sanitizeMp4(input: ZonaRosaInputStream, len: UInt64) throws -> SanitizedMetadata {
    return try withInputStream(input) { ffiInput in
        try invokeFnReturningNativeHandle {
            zonarosa_mp4_sanitizer_sanitize($0, ffiInput, len)
        }
    }
}
...
///  - `ZonaRosaError.invalidMediaInput` If the input could not be parsed because it was invalid.
///  - `ZonaRosaError.unsupportedMediaInput` If the input could not be parsed because it's unsupported in some way.
public func sanitizeWebp(input: ZonaRosaInputStream) throws {
    try withInputStream(input) { ffiInput in
        try checkError(zonarosa_webp_sanitizer_sanitize(ffiInput))
    }
}
...
public class SanitizedMetadata: ClonableHandleOwner<OpaquePointer?> {
    override internal class func cloneNativeHandle(
        ...
        return zonarosa_sanitized_metadata_clone(&newHandle, currentHandle)
    }

    override internal class func destroyNativeHandle(_ handle: OpaquePointer) -> ZonaRosaFfiErrorRef? {
        return zonarosa_sanitized_metadata_destroy(handle)
    }
        ...
        let metadata = withNativeHandle { nativeHandle in
            failOnError {
                try invokeFnReturningData {
                    zonarosa_sanitized_metadata_get_metadata($0, nativeHandle)
                }
            }
        }
        ...
        return withNativeHandle { nativeHandle in
            failOnError {
                try invokeFnReturningInteger {
                    zonarosa_sanitized_metadata_get_data_offset($0, nativeHandle)
                }
            }
        }
    }
    ...
    public var dataLen: UInt64 {
        return withNativeHandle { nativeHandle in
            failOnError {
                try invokeFnReturningInteger {
                    zonarosa_sanitized_metadata_get_data_len($0, nativeHandle)
                }
            }
        }
    }
