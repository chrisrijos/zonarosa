        ...
        return failOnError {
            let chatHandle = try fakeChatConnection.withNativeHandle { connectionHandle in
                try invokeFnReturningValueByPointer(.init()) {
                    zonarosa_testing_fake_chat_connection_take_authenticated_chat(
                        $0,
            ...
            let fakeRemoteHandle = try fakeChatConnection.withNativeHandle { connectionHandle in
                try invokeFnReturningValueByPointer(.init()) {
                    zonarosa_testing_fake_chat_connection_take_remote(
                        $0,
                        connectionHandle.const()
                    )
                }
...
extension UnauthenticatedChatConnection {
    internal static func fakeConnect(
        ...
        return failOnError {
            let chatHandle = try fakeChatConnection.withNativeHandle { connectionHandle in
                try invokeFnReturningValueByPointer(.init()) {
                    zonarosa_testing_fake_chat_connection_take_unauthenticated_chat(
                        $0,
                        connectionHandle.const()
                    )
                }
    ...
    internal static func fakeConnect(
        ...
        return failOnError {
            let connectionHandle = try fakeChatConnection.withNativeHandle { connectionHandle in
                try invokeFnReturningValueByPointer(.init()) {
                    zonarosa_testing_fake_chat_connection_take_provisioning_chat(
                        $0,
                        connectionHandle.const()
                    )
                }
...
internal class FakeChatRemote: NativeHandleOwner<ZonaRosaMutPointerFakeChatRemoteEnd> {
    private let tokioAsyncContext: TokioAsyncContext

    required init(owned: NonNull<ZonaRosaMutPointerFakeChatRemoteEnd>) {
        fatalError("must not be invoked directly")
    }

    init(
        handle: NonNull<ZonaRosaMutPointerFakeChatRemoteEnd>,
        tokioAsyncContext: TokioAsyncContext
    ) {
    ...
    func injectServerRequest(_ requestBytes: Data) {
        withNativeHandle { handle in
            requestBytes.withUnsafeBorrowedBuffer { requestBytes in
                failOnError(
                    zonarosa_testing_fake_chat_remote_end_send_raw_server_request(
                        handle.const(),
                        requestBytes
                    )
                )
    ...
    func getNextIncomingRequest() async throws -> (ChatRequest.InternalRequest, UInt64) {
        while true {
            let request = try await self.tokioAsyncContext.invokeAsyncFunction { promise, asyncContext in
                withNativeHandle { handle in
                    zonarosa_testing_fake_chat_remote_end_receive_incoming_request(
                        promise,
                        asyncContext.const(),
                        handle.const()
                    )
    ...
    func sendResponse(requestId: UInt64, _ response: ChatResponse) throws {
        ...
        try self.withNativeHandle { nativeHandle in
            try fakeResponse.withNativeHandle { response in
                try checkError(
                    zonarosa_testing_fake_chat_remote_end_send_server_response(nativeHandle.const(), response.const())
                )
            }
        }
    }
    ...
    func injectServerResponse(_ responseBytes: Data) {
        withNativeHandle { handle in
            responseBytes.withUnsafeBorrowedBuffer { responseBytes in
                failOnError(
                    zonarosa_testing_fake_chat_remote_end_send_raw_server_response(
                        handle.const(),
                        responseBytes
                    )
                )
    ...
    func injectConnectionInterrupted() {
        withNativeHandle { handle in
            failOnError(
                zonarosa_testing_fake_chat_remote_end_inject_connection_interrupted(
                    handle.const()
                )
            )
        }
    ...
    override class func destroyNativeHandle(
        _ handle: NonNull<ZonaRosaMutPointerFakeChatRemoteEnd>
    ) -> ZonaRosaFfiErrorRef? {
        zonarosa_fake_chat_remote_end_destroy(handle.pointer)
    }
}

internal class FakeChatServer: NativeHandleOwner<ZonaRosaMutPointerFakeChatServer>, @unchecked Sendable {
    ...
    internal init(asyncContext: TokioAsyncContext) {
        ...
        let pointer = failOnError {
            try invokeFnReturningValueByPointer(.init()) {
                zonarosa_testing_fake_chat_server_create($0)
            }
        }
        super.init(owned: NonNull(pointer)!)
    }

    internal required init(owned: NonNull<ZonaRosaMutPointerFakeChatServer>) {
        fatalError("cannot be invoked directly")
    }

    override class func destroyNativeHandle(
        _ nativeHandle: NonNull<ZonaRosaMutPointerFakeChatServer>
    ) -> ZonaRosaFfiErrorRef? {
        zonarosa_fake_chat_server_destroy(nativeHandle.pointer)
    }
    ...
    internal func getNextRemote() async throws -> FakeChatRemote {
        let remote = try await self.asyncContext.invokeAsyncFunction { promise, asyncContext in
            self.withNativeHandle { nativeHandle in
                zonarosa_testing_fake_chat_server_get_next_remote(promise, asyncContext.const(), nativeHandle.const())
            }
        }
...
internal class FakeChatResponse: NativeHandleOwner<ZonaRosaMutPointerFakeChatResponse> {
            ...
            try response.message.withCString { message in
                try response.headers.map { (key: String, value: String) in
                    ...
                    try response.body.withUnsafeBorrowedBuffer { body in
                        try invokeFnReturningValueByPointer(.init()) {
                            zonarosa_testing_fake_chat_response_create(
                                $0,
                                requestId,
                                response.status,
                                message,
                                headers,
                                ZonaRosaOptionalBorrowedSliceOfc_uchar(present: true, value: body)
                            )
                        }
                    }
                }
    ...
    internal required init(owned: NonNull<ZonaRosaMutPointerFakeChatResponse>) {
        fatalError("cannot be invoked directly")
    }

    override class func destroyNativeHandle(
        _ nativeHandle: NonNull<ZonaRosaMutPointerFakeChatResponse>
    ) -> ZonaRosaFfiErrorRef? {
        zonarosa_fake_chat_response_destroy(nativeHandle.pointer)
    }
}

private class FakeChatConnection: NativeHandleOwner<ZonaRosaMutPointerFakeChatConnection> {
    static func create(
        tokioAsyncContext: TokioAsyncContext,
        listener: any ChatConnectionListener,
        alerts: [String]
        ...
        let connection: FakeChatConnection = try withUnsafePointer(to: &listenerStruct) { listener in
            try tokioAsyncContext.withNativeHandle { asyncContext in
                try invokeFnReturningNativeHandle {
                    zonarosa_testing_fake_chat_connection_create_provisioning(
                        $0,
                        asyncContext.const(),
                        ZonaRosaConstPointerFfiProvisioningListenerStruct(raw: listener),
                    )
                }
            }
        }
    ...
    private static func internalCreate(
        ...
        _ listenerStruct: inout ZonaRosaFfiChatListenerStruct,
        _ alerts: [String]
    ) throws -> FakeChatConnection {
        let connection: FakeChatConnection = try withUnsafePointer(to: &listenerStruct) { listener in
            try tokioAsyncContext.withNativeHandle { asyncContext in
                try invokeFnReturningNativeHandle {
                    zonarosa_testing_fake_chat_connection_create(
                        $0,
                        asyncContext.const(),
                        ZonaRosaConstPointerFfiChatListenerStruct(raw: listener),
    ...
    override class func destroyNativeHandle(
        _ handle: NonNull<ZonaRosaMutPointerFakeChatConnection>
    ) -> ZonaRosaFfiErrorRef? {
        zonarosa_fake_chat_connection_destroy(handle.pointer)
    }
}

extension ZonaRosaMutPointerFakeChatConnection: ZonaRosaMutPointer {
    public typealias ConstPointer = ZonaRosaConstPointerFakeChatConnection
...
extension ZonaRosaConstPointerFakeChatConnection: ZonaRosaConstPointer {
    public func toOpaque() -> OpaquePointer? {
        self.raw
    }
}

extension ZonaRosaMutPointerFakeChatRemoteEnd: ZonaRosaMutPointer {
    public typealias ConstPointer = ZonaRosaConstPointerFakeChatRemoteEnd
...
extension ZonaRosaConstPointerFakeChatRemoteEnd: ZonaRosaConstPointer {
    public func toOpaque() -> OpaquePointer? {
        self.raw
    }
}

extension ZonaRosaMutPointerFakeChatServer: ZonaRosaMutPointer {
    public typealias ConstPointer = ZonaRosaConstPointerFakeChatServer
...
extension ZonaRosaConstPointerFakeChatServer: ZonaRosaConstPointer {
    public func toOpaque() -> OpaquePointer? {
        self.raw
    }
}

extension ZonaRosaMutPointerFakeChatResponse: ZonaRosaMutPointer {
    public typealias ConstPointer = ZonaRosaConstPointerFakeChatResponse
...
extension ZonaRosaConstPointerFakeChatResponse: ZonaRosaConstPointer {
    public func toOpaque() -> OpaquePointer? {
        self.raw
    }
}

extension ZonaRosaCPromiseOptionalPairOfMutPointerHttpRequestu64: PromiseStruct {
    typealias Result = ZonaRosaOptionalPairOfMutPointerHttpRequestu64
}

extension ZonaRosaCPromiseMutPointerFakeChatRemoteEnd: PromiseStruct {
    typealias Result = ZonaRosaMutPointerFakeChatRemoteEnd
}
