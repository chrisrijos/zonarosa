...
internal func invokeFnReturningValueByPointer<Value>(
    _ initial: Value,
    fn: (UnsafeMutablePointer<Value>?) -> ZonaRosaFfiErrorRef?
) throws -> Value {
    var output = initial
...
internal func invokeFnReturningString(
    fn: (UnsafeMutablePointer<UnsafePointer<CChar>?>?) -> ZonaRosaFfiErrorRef?
) throws -> String {
...
internal func invokeFnReturningOptionalString(
    ...
    zonarosa_free_string(output)
    return result
}

internal func invokeFnReturningSomeBytestringArray<Element>(
    fn: (UnsafeMutablePointer<ZonaRosaBytestringArray>?) -> ZonaRosaFfiErrorRef?,
    ...
    zonarosa_free_bytestring_array(array)
    return result
}

internal func invokeFnReturningStringArray(
    fn: (UnsafeMutablePointer<ZonaRosaStringArray>?) -> ZonaRosaFfiErrorRef?
) throws -> [String] {
...
internal func invokeFnReturningOptionalArray(
    fn: (UnsafeMutablePointer<ZonaRosaOwnedBuffer>?) -> ZonaRosaFfiErrorRef?
) throws -> Data? {
...
internal func invokeFnReturningData(fn: (UnsafeMutablePointer<ZonaRosaOwnedBuffer>?) -> ZonaRosaFfiErrorRef?) throws -> Data
{
    try invokeFnReturningOptionalArray(fn: fn) ?? Data()
}

internal func invokeFnReturningFixedLengthArray<ResultAsTuple>(
    fn: (UnsafeMutablePointer<ResultAsTuple>) -> ZonaRosaFfiErrorRef?
) throws -> Data {
...
internal func invokeFnReturningSerialized<Result: ByteArray, SerializedResult>(
    fn: (UnsafeMutablePointer<SerializedResult>) -> ZonaRosaFfiErrorRef?
) throws -> Result {
...
internal func invokeFnReturningUuid(fn: (UnsafeMutablePointer<ZonaRosaUuid>?) -> ZonaRosaFfiErrorRef?) throws -> UUID {
    return UUID(uuid: try invokeFnReturningValueByPointer(.init(), fn: fn).bytes)
}

internal func invokeFnReturningOptionalUuid(
    fn: (UnsafeMutablePointer<ZonaRosaOptionalUuid>?) -> ZonaRosaFfiErrorRef?
) throws -> UUID? {
    let output = try invokeFnReturningValueByPointer(.init(), fn: fn)
...
internal func invokeFnReturningServiceId<Id: ServiceId>(
    fn: (UnsafeMutablePointer<ServiceIdStorage>?) -> ZonaRosaFfiErrorRef?
) throws -> Id {
...
internal func invokeFnReturningBool(fn: (UnsafeMutablePointer<Bool>?) -> ZonaRosaFfiErrorRef?) throws -> Bool {
...
internal func invokeFnReturningNativeHandle<Owner: NativeHandleOwner<PointerType>, PointerType>(
    fn: (UnsafeMutablePointer<PointerType>?) -> ZonaRosaFfiErrorRef?
) throws -> Owner {
...
extension ContiguousBytes {
    func withUnsafeBorrowedBuffer<Result>(_ body: (ZonaRosaBorrowedBuffer) throws -> Result) rethrows -> Result {
        try withUnsafeBytes {
            try body(ZonaRosaBorrowedBuffer($0))
        }
    }
}
...
>(
    of this: T?,
    _ body: (ZonaRosaOptionalBorrowedSliceOfc_uchar) throws -> R
) rethrows -> R {
    switch this {
    case .none:
        let empty = ZonaRosaOptionalBorrowedSliceOfc_uchar()
        return try body(empty)
    case .some(let wrapped):
        return try wrapped.withUnsafeBorrowedBuffer { buffer in
            let slice = ZonaRosaOptionalBorrowedSliceOfc_uchar(present: true, value: buffer)
...
extension Sequence where Self.Element == String {
    func withUnsafeBorrowedBytestringArray<Result>(
        _ body: (ZonaRosaBorrowedBytestringArray) throws -> Result
    ) rethrows -> Result {
            ...
            try lengths.withUnsafeBufferPointer { lengths in
                try body(
                    ZonaRosaBorrowedBytestringArray(
                        bytes: bytes,
                        lengths: ZonaRosaBorrowedSliceOfusize(base: lengths.baseAddress, length: lengths.count)
                    )
                )
            }
        }
...
extension ZonaRosaBorrowedBuffer {
...
extension ZonaRosaBorrowedMutableBuffer {
...
extension Data {
    internal init(consuming buffer: ZonaRosaOwnedBuffer) {
        if let base = buffer.base {
            self.init(
                bytesNoCopy: base,
                count: buffer.length,
                deallocator: .custom { base, length in
                    zonarosa_free_buffer(base, length)
                }
            )
        } else {
            self.init()
...
internal func fillRandom(_ buffer: UnsafeMutableRawBufferPointer) throws {
    ...
    guard result == errSecSuccess else {
        throw ZonaRosaError.internalError("SecRandomCopyBytes failed (error code \(result))")
    }
    #else
...
internal func rethrowCallbackErrors<Store, Result>(
    ...
    } catch ZonaRosaError.callbackError(_) where context.error != nil {
        throw context.error!
    }
}
    ...
    public func toHex() -> String {
        ...
        hex.withUnsafeMutableBytes { hex in
            self.withUnsafeBorrowedBuffer { input in
                failOnError(
                    zonarosa_hex_encode(
                        ZonaRosaBorrowedMutableBuffer(hex),
                        input
                    )
                )
            }
...
extension [String: String] {
    internal func withBridgedStringMap<Result>(
        _ callback: (ZonaRosaMutPointerBridgedStringMap) throws -> Result
    ) rethrows -> Result {
        var map = ZonaRosaMutPointerBridgedStringMap()
        failOnError(zonarosa_bridged_string_map_new(&map, UInt32(clamping: self.count)))
        defer { zonarosa_bridged_string_map_destroy(map) }

        for (key, value) in self {
            failOnError(zonarosa_bridged_string_map_insert(map, key, value))
        }
...
extension ZonaRosaMutPointerBridgedStringMap: ZonaRosaMutPointer {
    public typealias ConstPointer = ZonaRosaConstPointerBridgedStringMap
...
extension ZonaRosaConstPointerBridgedStringMap: ZonaRosaConstPointer {
    public func toOpaque() -> OpaquePointer? {
        self.raw
    }
}
