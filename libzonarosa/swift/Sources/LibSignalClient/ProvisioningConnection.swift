...
>, @unchecked Sendable
{
    ...
    internal init(
        ...
        let nativeHandle = try await tokioAsyncContext.invokeAsyncFunction { promise, tokioAsyncContext in
            connectionManager.withNativeHandle { connectionManager in
                zonarosa_provisioning_chat_connection_connect(
                    promise,
    ...
    override internal class func destroyNativeHandle(
        _ handle: NonNull<ZonaRosaMutPointerProvisioningChatConnection>
    ) -> ZonaRosaFfiErrorRef? {
        return zonarosa_provisioning_chat_connection_destroy(handle.pointer)
    }

    internal required init(owned handle: NonNull<ZonaRosaMutPointerProvisioningChatConnection>) {
        fatalError("should not be called directly for a ProvisioningConnection")
    }

    internal init(
        fakeHandle handle: NonNull<ZonaRosaMutPointerProvisioningChatConnection>,
        tokioAsyncContext: TokioAsyncContext
    ) {
        self.tokioAsyncContext = tokioAsyncContext
    ...
    public func start(listener: any ProvisioningConnectionListener) {
        withNativeHandle { connectionHandle in
            ...
            withUnsafePointer(to: &listenerStruct) {
                failOnError(
                    zonarosa_provisioning_chat_connection_init_listener(
                        connectionHandle.const(),
                        ZonaRosaConstPointerFfiProvisioningListenerStruct(raw: $0)
                    )
                )
            }
        }
    ...
    public func disconnect() async throws {
        _ = try await self.tokioAsyncContext.invokeAsyncFunction { promise, tokioAsyncContext in
            withNativeHandle { chatConnection in
                zonarosa_provisioning_chat_connection_disconnect(
                    promise,
                    tokioAsyncContext.const(),
        ...
        withNativeHandle { chatConnection in
            failOnError {
                try invokeFnReturningNativeHandle {
                    zonarosa_provisioning_chat_connection_info($0, chatConnection.const())
                }
            }
        }
    }
...
extension ZonaRosaMutPointerProvisioningChatConnection: ZonaRosaMutPointer {
    public typealias ConstPointer = ZonaRosaConstPointerProvisioningChatConnection
...
extension ZonaRosaConstPointerProvisioningChatConnection: ZonaRosaConstPointer {
    public func toOpaque() -> OpaquePointer? {
        self.raw
    }
}
...
internal class ProvisioningListenerBridge {
    private class AckHandleOwner: NativeHandleOwner<ZonaRosaMutPointerServerMessageAck> {
        override class func destroyNativeHandle(
            _ handle: NonNull<ZonaRosaMutPointerServerMessageAck>
        ) -> ZonaRosaFfiErrorRef? {
            zonarosa_server_message_ack_destroy(handle.pointer)
        }
    }
    ...
    func makeListenerStruct() -> ZonaRosaFfiProvisioningListenerStruct {
        let receivedAddress: ZonaRosaFfiProvisioningListenerReceivedAddress = { rawCtx, rawAddress, ackHandle in
            defer { zonarosa_free_string(rawAddress) }
            ...
            bridge.listener.provisioningConnection(connection, didReceiveAddress: address) {
                _ = ackHandleOwner.withNativeHandle { ackHandle in zonarosa_server_message_ack_send(ackHandle.const()) }
            }
            return 0
        }

        let receivedEnvelope: ZonaRosaFfiProvisioningListenerReceivedEnvelope = { rawCtx, envelope, ackHandle in
            let bridge = Unmanaged<ProvisioningListenerBridge>.fromOpaque(rawCtx!).takeUnretainedValue()
        ...
        let connectionInterrupted: ZonaRosaFfiProvisioningListenerConnectionInterrupted = { rawCtx, maybeError in
            let bridge = Unmanaged<ProvisioningListenerBridge>.fromOpaque(rawCtx!).takeUnretainedValue()
