...
public class Aes256GcmSiv: NativeHandleOwner<ZonaRosaMutPointerAes256GcmSiv> {
    public convenience init<Bytes: ContiguousBytes>(key bytes: Bytes) throws {
        let handle = try bytes.withUnsafeBorrowedBuffer { bytes in
            try invokeFnReturningValueByPointer(.init()) {
                zonarosa_aes256_gcm_siv_new($0, bytes)
            }
        }
    ...
    override internal class func destroyNativeHandle(
        _ handle: NonNull<ZonaRosaMutPointerAes256GcmSiv>
    ) -> ZonaRosaFfiErrorRef? {
        return zonarosa_aes256_gcm_siv_destroy(handle.pointer)
    }
                ...
                try nonce.withUnsafeBorrowedBuffer { nonceBuffer in
                    try associatedData.withUnsafeBorrowedBuffer { adBuffer in
                        try invokeFnReturningData {
                            zonarosa_aes256_gcm_siv_encrypt(
                                $0,
                                nativeHandle.const(),
                                messageBuffer,
                                nonceBuffer,
        ...
        try withNativeHandle { nativeHandle in
            try message.withUnsafeBorrowedBuffer { messageBuffer in
                try nonce.withUnsafeBorrowedBuffer { nonceBuffer in
                    try associatedData.withUnsafeBorrowedBuffer { adBuffer in
                        try invokeFnReturningData {
                            zonarosa_aes256_gcm_siv_decrypt(
                                $0,
                                nativeHandle.const(),
                                messageBuffer,
                                nonceBuffer,
...
extension ZonaRosaMutPointerAes256GcmSiv: ZonaRosaMutPointer {
    public typealias ConstPointer = ZonaRosaConstPointerAes256GcmSiv
...
extension ZonaRosaConstPointerAes256GcmSiv: ZonaRosaConstPointer {
    public func toOpaque() -> OpaquePointer? {
        self.raw
    }
}
