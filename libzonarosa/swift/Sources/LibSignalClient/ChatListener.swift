...
internal class ChatListenerBridge {
    ...
    func makeListenerStruct() -> ZonaRosaFfiChatListenerStruct {
        let receivedIncomingMessage: ZonaRosaFfiChatListenerReceivedIncomingMessage = {
            rawCtx,
            envelope,
            timestamp,
            ackHandle in
        ...
        let receivedQueueEmpty: ZonaRosaFfiChatListenerReceivedQueueEmpty = { rawCtx in
            let bridge = Unmanaged<ChatListenerBridge>.fromOpaque(rawCtx!).takeUnretainedValue()
            guard let chatConnection = bridge.chatConnection else {
                // The client no longer listening is not an error.
                return 0
        ...
        let receivedAlerts: ZonaRosaFfiChatListenerReceivedAlerts = { rawCtx, alerts in
            let bridge = Unmanaged<ChatListenerBridge>.fromOpaque(rawCtx!).takeUnretainedValue()
        ...
        let connectionInterrupted: ZonaRosaFfiChatListenerConnectionInterrupted = { rawCtx, maybeError in
            let bridge = Unmanaged<ChatListenerBridge>.fromOpaque(rawCtx!).takeUnretainedValue()
...
extension ZonaRosaMutPointerServerMessageAck: ZonaRosaMutPointer {
    public typealias ConstPointer = ZonaRosaConstPointerServerMessageAck
    ...
    public func const() -> ZonaRosaConstPointerServerMessageAck {
        ZonaRosaConstPointerServerMessageAck(raw: self.raw)
    }
}

extension ZonaRosaConstPointerServerMessageAck: ZonaRosaConstPointer {
    public func toOpaque() -> OpaquePointer? {
        self.raw
    }
}
...
internal class UnauthConnectionEventsListenerBridge {
    ...
    /// or the ChatListenerBridge object used to construct it (`self`) will be leaked.
    func makeListenerStruct() -> ZonaRosaFfiChatListenerStruct {
        let receivedIncomingMessage: ZonaRosaFfiChatListenerReceivedIncomingMessage = { _, _, _, _ in
        ...
        let receivedQueueEmpty: ZonaRosaFfiChatListenerReceivedQueueEmpty = { _ in
            // Not used in the unauth chat listener
            LoggerBridge.shared?.logger.log(
                level: .error,
                file: #fileID,
        ...
        let receivedAlerts: ZonaRosaFfiChatListenerReceivedAlerts = { _, alerts in
            // Not used in the unauth chat listener
