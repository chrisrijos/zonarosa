...
public struct Aes256GcmEncryptedData: Sendable {
    ...
    public init(concatenated: Data) throws {
        guard concatenated.count >= Self.nonceLength + Self.authenticationTagLength else {
            throw ZonaRosaError.invalidMessage("concatenated AES-256-GCM ciphertext too short")
        }
    ...
    public func decrypt(
        ...
        guard try cipher.verifyTag(self.authenticationTag) else {
            throw ZonaRosaError.invalidMessage("failed to decrypt")
        }
        return plaintext
    }
...
public class Aes256GcmEncryption: NativeHandleOwner<ZonaRosaMutPointerAes256GcmEncryption> {
            ...
            try nonce.withUnsafeBorrowedBuffer { nonceBuffer in
                try associatedData.withUnsafeBorrowedBuffer { adBuffer in
                    try invokeFnReturningValueByPointer(.init()) {
                        zonarosa_aes256_gcm_encryption_new(
                            $0,
                            keyBuffer,
                            nonceBuffer,
                            adBuffer
    ...
    override internal class func destroyNativeHandle(
        _ handle: NonNull<ZonaRosaMutPointerAes256GcmEncryption>
    ) -> ZonaRosaFfiErrorRef? {
        return zonarosa_aes256_gcm_encryption_destroy(handle.pointer)
    }
    ...
    public func encrypt(_ message: inout Data) throws {
        try withNativeHandle { nativeHandle in
            try message.withUnsafeMutableBytes { messageBytes in
                try checkError(
                    zonarosa_aes256_gcm_encryption_update(
                        nativeHandle,
                        ZonaRosaBorrowedMutableBuffer(messageBytes),
                        0,
    ...
    public func computeTag() throws -> Data {
        return try withNativeHandle { nativeHandle in
            try invokeFnReturningData {
                zonarosa_aes256_gcm_encryption_compute_tag($0, nativeHandle)
            }
        }
    }
}

extension ZonaRosaMutPointerAes256GcmEncryption: ZonaRosaMutPointer {
...
public class Aes256GcmDecryption: NativeHandleOwner<ZonaRosaMutPointerAes256GcmDecryption> {
    /// Initializes the cipher with the given inputs.
    ///
    /// The associated data is not included in the plaintext or tag; instead, it's expected to match
    ...
    public convenience init(
        ...
        let handle = try key.withUnsafeBorrowedBuffer { keyBuffer in
            try nonce.withUnsafeBorrowedBuffer { nonceBuffer in
                try associatedData.withUnsafeBorrowedBuffer { adBuffer in
                    try invokeFnReturningValueByPointer(.init()) {
                        zonarosa_aes256_gcm_decryption_new(
                            $0,
                            keyBuffer,
                            nonceBuffer,
                            adBuffer
    ...
    override internal class func destroyNativeHandle(
        _ handle: NonNull<ZonaRosaMutPointerAes256GcmDecryption>
    ) -> ZonaRosaFfiErrorRef? {
        return zonarosa_aes256_gcm_decryption_destroy(handle.pointer)
    }
    ...
    public func decrypt(_ message: inout Data) throws {
        try withNativeHandle { nativeHandle in
            try message.withUnsafeMutableBytes { messageBytes in
                try checkError(
                    zonarosa_aes256_gcm_decryption_update(
                        nativeHandle,
                        ZonaRosaBorrowedMutableBuffer(messageBytes),
                        0,
                        UInt32(messageBytes.count)
        ...
        return try withNativeHandle { nativeHandle in
            try tag.withUnsafeBorrowedBuffer { tagBuffer in
                try invokeFnReturningBool {
                    zonarosa_aes256_gcm_decryption_verify_tag($0, nativeHandle, tagBuffer)
                }
            }
        }
    }
...
extension ZonaRosaMutPointerAes256GcmDecryption: ZonaRosaMutPointer {
    public typealias ConstPointer = OpaquePointer?
