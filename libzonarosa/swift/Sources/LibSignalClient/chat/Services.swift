...
public struct UnauthServiceSelectorHelper<Api>: UnauthServiceSelector {
    ...
    /// An escape hatch in case libzonarosa adds a new service protocol but forgets to add a selector
    /// for it.
    ///
    /// ## Usage
    ///
    ...
    ///     static var oopsNewService: Self { .workaroundBecauseLibzonarosaForgotToExposeASelector() }
    /// }
    /// ```
    ///
    /// (We could just make `init` public as well, but this way it's more obvious that the
    /// workaround is meant to be temporary.)
    public static func workaroundBecauseLibzonarosaForgotToExposeASelector() -> Self { .init() }
}
