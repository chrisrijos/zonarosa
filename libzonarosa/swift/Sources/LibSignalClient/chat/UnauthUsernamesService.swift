...
public protocol UnauthUsernamesService: Sendable {
    ...
    /// ``ZonaRosaError/usernameLinkInvalidEntropyDataLength(_:)`` if the entropy is invalid, and
    /// ``ZonaRosaError/usernameLinkInvalid(_:)`` if the data fetched from the service could not be
...
extension UnauthenticatedChatConnection: UnauthUsernamesService {
    public func lookUpUsernameHash(_ hash: Data) async throws -> Aci? {
        let rawResponse: ZonaRosaOptionalUuid = try await self.tokioAsyncContext
            .invokeAsyncFunction { promise, tokioAsyncContext in
                withNativeHandle { chatService in
                    hash.withUnsafeBorrowedBuffer { hash in
                        zonarosa_unauthenticated_chat_connection_look_up_username_hash(
                            promise,
    ...
    public func lookUpUsernameLink(_ uuid: UUID, entropy: Data) async throws -> Username? {
        var rawResponse: ZonaRosaOptionalPairOfc_charu832 = try await self.tokioAsyncContext
            .invokeAsyncFunction { promise, tokioAsyncContext in
                withNativeHandle { chatService in
                    try! withAllBorrowed(uuid, entropy) { uuid, entropy in
                        zonarosa_unauthenticated_chat_connection_look_up_username_link(
                            promise,
                            tokioAsyncContext.const(),
                            chatService.const(),
                            uuid,
        ...
        defer { zonarosa_free_string(rawResponse.first) }
