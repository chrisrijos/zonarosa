...
/// Client for the ZonaRosa registration service.
///
/// This wraps a ``Net`` to provide a reliable registration service client.
///
public class RegistrationService: NativeHandleOwner<ZonaRosaMutPointerRegistrationService>, @unchecked Sendable {
    private let asyncContext: TokioAsyncContext

    override internal class func destroyNativeHandle(
        _ nativeHandle: NonNull<ZonaRosaMutPointerRegistrationService>
    ) -> ZonaRosaFfiErrorRef? {
        zonarosa_registration_service_destroy(nativeHandle.pointer)
    }

    internal init(owned: NonNull<ZonaRosaMutPointerRegistrationService>, asyncContext: TokioAsyncContext) {
        self.asyncContext = asyncContext
        super.init(owned: owned)
    }

    required init(owned: NonNull<ZonaRosaMutPointerRegistrationService>) {
    ...
    ///   - ``ZonaRosaError/rateLimitedError(retryAfter:message:)`` if the server requests a retry later.
    ///   - ``ZonaRosaError/possibleCaptiveNetwork(_:)`` if the server's TLS response
    ///     suggests a captive network.
    ///   - Some other `ZonaRosaError` if the request can't be completed.
    public static func createSession(
        ...
        let connectChatBridge = ZonaRosaFfiConnectChatBridgeStruct.forManager(net.connectionManager)

        let service: ZonaRosaMutPointerRegistrationService =
            try await net.asyncContext.invokeAsyncFunction { promise, tokioContext in
                ZonaRosaFfiRegistrationCreateSessionRequest.withNativeStruct(
                    e164: e164,
                    pushToken: pushToken,
                    mcc: mcc,
                    mnc: mnc
                    ...
                    withUnsafePointer(to: connectChatBridge) { connectChatBridge in
                        zonarosa_registration_service_create_session(
                            promise,
                            tokioContext.const(),
                            request,
        ...
        let service: ZonaRosaMutPointerRegistrationService =
            try await net.asyncContext.invokeAsyncFunction { promise, tokioContext in
                withUnsafePointer(
                    ...
                    zonarosa_registration_service_resume_session(
                        promise,
                        tokioContext.const(),
                        sessionId,
                        number,
    ...
    public func requestPushChallenge(apnPushToken: String) async throws {
        let _: Bool = try await self.asyncContext.invokeAsyncFunction { promise, asyncContext in
            self.withNativeHandle {
                zonarosa_registration_service_request_push_challenge(
                    promise,
                    asyncContext.const(),
                    $0.const(),
                    apnPushToken
    ...
    public func submitPushChallenge(pushChallenge: String) async throws {
        let _: Bool = try await self.asyncContext.invokeAsyncFunction { promise, asyncContext in
            self.withNativeHandle {
                zonarosa_registration_service_submit_push_challenge(
                    promise,
                    asyncContext.const(),
                    $0.const(),
                    pushChallenge
    ...
    /// Note that "quality weighting" can be left out; the ZonaRosa server will always consider the
    ...
    public func requestVerificationCode(
        ...
        let _: Bool = try await self.asyncContext.invokeAsyncFunction { promise, asyncContext in
            languages.withUnsafeBorrowedBytestringArray { languages in
                self.withNativeHandle {
                    zonarosa_registration_service_request_verification_code(
                        promise,
                        asyncContext.const(),
                        $0.const(),
    ...
    public func submitVerificationCode(code: String) async throws {
        let _: Bool = try await self.asyncContext.invokeAsyncFunction { promise, asyncContext in
            self.withNativeHandle {
                zonarosa_registration_service_submit_verification_code(
                    promise,
                    asyncContext.const(),
                    $0.const(),
                    code
    ...
    public func checkSvr2Credentials(svrTokens: [String]) async throws -> [String: Svr2CredentialsResult] {
        var result = try await self.asyncContext.invokeAsyncFunction { promise, asyncContext in
            svrTokens.withUnsafeBorrowedBytestringArray { svrTokens in
                self.withNativeHandle {
                    zonarosa_registration_service_check_svr2_credentials(
                        promise,
                        asyncContext.const(),
                        $0.const(),
                        svrTokens
    ...
    public var sessionId: String {
        return failOnError {
            try invokeFnReturningString { out in
                self.withNativeHandle {
                    zonarosa_registration_service_session_id(out, $0.const())
                }
            }
        }
    }
        ...
        return failOnError {
            try invokeFnReturningNativeHandle { out in
                self.withNativeHandle { nativeHandle in
                    zonarosa_registration_service_registration_session(out, nativeHandle.const())
                }
            }
        }
    }
            ...
            self.withNativeHandle { registrationService in
                request.withNativeHandle { request in
                    accountAttributes.withNativeHandle { accountAttributes in
                        zonarosa_registration_service_register_account(
                            promise,
                            asyncContext.const(),
    ...
    public class func reregisterAccount(
        ...
        var connectChatBridge = ZonaRosaFfiConnectChatBridgeStruct.forManager(net.connectionManager)
                ...
                accountAttributes.withNativeHandle { accountAttributes in
                    withUnsafePointer(to: &connectChatBridge) { connectChatBridge in
                        e164.withCString { e164 in
                            zonarosa_registration_service_reregister_account(
                                promise,
                                asyncContext.const(),
                                ZonaRosaConstPointerFfiConnectChatBridgeStruct(raw: connectChatBridge),
                                e164,
                                request.const(),
                                accountAttributes.const()
                            )
...
private class RegisterAccountRequst: NativeHandleOwner<ZonaRosaMutPointerRegisterAccountRequest> {
    override internal class func destroyNativeHandle(
        _ nativeHandle: NonNull<ZonaRosaMutPointerRegisterAccountRequest>
    ) -> ZonaRosaFfiErrorRef? {
        zonarosa_register_account_request_destroy(nativeHandle.pointer)
    }
    ...
    fileprivate static func create(
        ...
        let request: Self = failOnError {
            try invokeFnReturningNativeHandle {
                zonarosa_register_account_request_create($0)
            }
        }
        failOnError {
            try request.withNativeHandle { native in
                try checkError(
                    accountPassword.withCString {
                        zonarosa_register_account_request_set_account_password(native.const(), $0)
                    }
                )
                if let apnPushToken {
                    try checkError(
                        apnPushToken.withCString {
                            zonarosa_register_account_request_set_apn_push_token(native.const(), $0)
                        }
                    )
                }
                if skipDeviceTransfer {
                    try checkError(zonarosa_register_account_request_set_skip_device_transfer(native.const()))
                }

                try checkError(
                    aciIdentityKey.withNativeHandle {
                        zonarosa_register_account_request_set_identity_public_key(
                            native.const(),
                ...
                try checkError(
                    aciSignedPreKey.withNativeStruct {
                        zonarosa_register_account_request_set_identity_signed_pre_key(
                            native.const(),
                            ServiceIdKind.aci.rawValue,
                            $0
                        )
                ...
                try checkError(
                    aciPqSignedPreKey.withNativeStruct {
                        zonarosa_register_account_request_set_identity_pq_last_resort_pre_key(
                            native.const(),
                            ServiceIdKind.aci.rawValue,
                            $0
                        )
...
extension ZonaRosaFfiConnectChatBridgeStruct {
    /// Constructs a ``ZonaRosaFfiConnectChatBridgeStruct`` that uses the given
    ...
    fileprivate static func forManager(_ connectionManager: ConnectionManager) -> Self {
        return ZonaRosaFfiConnectChatBridgeStruct(
...
internal func invokeFnReturningCheckSvr2CredentialsResponse(
    fn: (_ out: UnsafeMutablePointer<ZonaRosaFfiCheckSvr2CredentialsResponse>?) -> ZonaRosaFfiErrorRef?
    ...
    let entries = try invokeFnReturningSomeBytestringArray(
        fn: { out in
            ...
            var wrapper = ZonaRosaFfiCheckSvr2CredentialsResponse()
            let err = fn(&wrapper)
        ...
        transform: { view in
            ...
            let value =
                switch ZonaRosaSvr2CredentialsResult(valueByte) {
                case ZonaRosaSvr2CredentialsResultInvalid:
                    Svr2CredentialsResult.invalid
                case ZonaRosaSvr2CredentialsResultMatch:
                    Svr2CredentialsResult.match
                case ZonaRosaSvr2CredentialsResultNoMatch:
...
extension ZonaRosaMutPointerRegistrationService: ZonaRosaMutPointer {
    public typealias ConstPointer = ZonaRosaConstPointerRegistrationService
    ...
    public func const() -> Self.ConstPointer {
        ZonaRosaConstPointerRegistrationService(raw: self.raw)
    }
}

extension ZonaRosaConstPointerRegistrationService: ZonaRosaConstPointer {
    public func toOpaque() -> OpaquePointer? {
        self.raw
    }
}
