...
public class ProtocolAddress: ClonableHandleOwner<ZonaRosaMutPointerProtocolAddress>, @unchecked Sendable {
    /// Creates an address in the ZonaRosa protocol.
    /// - Parameters:
    ...
    ///
    /// - Throws: ``ZonaRosaError#invalidProtocolAddress(name:deviceId:message:)`` if the address is not valid.
    public convenience init(name: String, deviceId: UInt32) throws {
        let handle = try invokeFnReturningValueByPointer(.init()) {
            zonarosa_address_new($0, name, deviceId)
        }
    ...
    override internal class func cloneNativeHandle(
        _ newHandle: inout ZonaRosaMutPointerProtocolAddress,
        currentHandle: ZonaRosaConstPointerProtocolAddress
    ) -> ZonaRosaFfiErrorRef? {
        return zonarosa_address_clone(&newHandle, currentHandle)
    }

    override internal class func destroyNativeHandle(
        _ handle: NonNull<ZonaRosaMutPointerProtocolAddress>
    ) -> ZonaRosaFfiErrorRef? {
        return zonarosa_address_destroy(handle.pointer)
    }
        ...
        return withNativeHandle { nativeHandle in
            failOnError {
                try invokeFnReturningString {
                    zonarosa_address_get_name($0, nativeHandle.const())
                }
            }
        }
    }
        ...
        return withNativeHandle { nativeHandle in
            failOnError {
                try invokeFnReturningInteger {
                    zonarosa_address_get_device_id($0, nativeHandle.const())
                }
            }
        }
    }
...
extension ZonaRosaMutPointerProtocolAddress: ZonaRosaMutPointer {
    public typealias ConstPointer = ZonaRosaConstPointerProtocolAddress
...
extension ZonaRosaConstPointerProtocolAddress: ZonaRosaConstPointer {
    public func toOpaque() -> OpaquePointer? {
        self.raw
    }
}
