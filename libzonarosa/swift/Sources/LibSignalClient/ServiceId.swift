...
internal typealias ServiceIdStorage = ZonaRosaServiceIdFixedWidthBinaryBytes
...
/// Typed representation of a ZonaRosa service ID, which can be one of various types.
///
...
public class ServiceId: @unchecked Sendable {
        ...
        failOnError {
            try withUnsafePointer(to: self.storage) { ptr in
                try invokeFnReturningString {
                    zonarosa_service_id_service_id_string($0, ptr)
                }
            }
        }
    }
    ...
    public var logString: String {
        failOnError {
            try withUnsafePointer(to: self.storage) { ptr in
                try invokeFnReturningString {
                    zonarosa_service_id_service_id_log($0, ptr)
                }
            }
        }
    }
        ...
        return failOnError {
            try withUnsafePointer(to: self.storage) { ptr in
                try invokeFnReturningData {
                    zonarosa_service_id_service_id_binary($0, ptr)
                }
            }
        }
    }
    ...
    public static func parseFrom(serviceIdString s: String) throws -> Self {
        let result = try invokeFnReturningServiceId {
            zonarosa_service_id_parse_from_service_id_string($0, s)
        }
    ...
    >(serviceIdBinary sourceBytes: Bytes) throws -> Self {
        let result = try sourceBytes.withUnsafeBorrowedBuffer { buffer in
            try invokeFnReturningServiceId {
                zonarosa_service_id_parse_from_service_id_binary($0, buffer)
            }
        }
        return try result.downcast(to: Self.self)
    }
