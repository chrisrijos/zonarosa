...
///
/// SgxClient provides bindings to interact with a ZonaRosa SGX service
///
...
///
public class SgxClient: NativeHandleOwner<ZonaRosaMutPointerSgxClientState> {
    override internal class func destroyNativeHandle(
        _ handle: NonNull<ZonaRosaMutPointerSgxClientState>
    ) -> ZonaRosaFfiErrorRef? {
        return zonarosa_sgx_client_state_destroy(handle.pointer)
    }
        ...
        return withNativeHandle { nativeHandle in
            failOnError {
                try invokeFnReturningData {
                    zonarosa_sgx_client_state_initial_request($0, nativeHandle.const())
                }
            }
        }
    }
    ...
    public func completeHandshake<Bytes: ContiguousBytes>(_ handshakeResponse: Bytes) throws {
        try withNativeHandle { nativeHandle in
            try handshakeResponse.withUnsafeBorrowedBuffer { buffer in
                try checkError(zonarosa_sgx_client_state_complete_handshake(nativeHandle, buffer))
            }
        }
    }
    ...
    public func establishedSend<Bytes: ContiguousBytes>(_ plaintextToSend: Bytes) throws -> Data {
        return try withNativeHandle { nativeHandle in
            try plaintextToSend.withUnsafeBorrowedBuffer { buffer in
                try invokeFnReturningData {
                    zonarosa_sgx_client_state_established_send($0, nativeHandle, buffer)
                }
            }
        }
    }
    ...
    public func establishedRecv<Bytes: ContiguousBytes>(_ receivedCiphertext: Bytes) throws -> Data {
        return try withNativeHandle { nativeHandle in
            try receivedCiphertext.withUnsafeBorrowedBuffer { buffer in
                try invokeFnReturningData {
                    zonarosa_sgx_client_state_established_recv($0, nativeHandle, buffer)
                }
            }
        }
    }
...
extension ZonaRosaMutPointerSgxClientState: ZonaRosaMutPointer {
    public typealias ConstPointer = ZonaRosaConstPointerSgxClientState
...
extension ZonaRosaConstPointerSgxClientState: ZonaRosaConstPointer {
    public func toOpaque() -> OpaquePointer? {
        self.raw
    }
}
