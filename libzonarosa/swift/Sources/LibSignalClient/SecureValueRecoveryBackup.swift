...
///
/// This service provides forward secrecy for ZonaRosa backups using SVR-B. Forward secrecy ensures
...
public class SvrB {
        ...
        backupKey.withUnsafePointer { backupKey in
            failOnError {
                try invokeFnReturningData {
                    zonarosa_secure_value_recovery_for_backups_create_new_backup_chain(
                        $0,
    ...
    ///   secret data.
    /// - Throws:
    ///   - ``ZonaRosaError/invalidArgument(_:)`` if `previousSecretData` is malformed. There's no
    ...
    ///     recommended).
    ///   - Other ``ZonaRosaError``s for networking and attestation issues. These can be manually
    ///     retried, but some may indicate a possible bug in libzonarosa or in the enclave.
    public func store(
        backupKey: BackupKey,
            ...
            net.connectionManager.withNativeHandle { connectionManager in
                backupKey.withUnsafePointer { backupKey in
                    previousSecretData.withBorrowed { previousSecretData in
                        zonarosa_secure_value_recovery_for_backups_store_backup(
                            promise,
                            runtime.const(),
                            backupKey,
                            previousSecretData,
    ...
    /// - Throws:
    ///   - ``ZonaRosaError/invalidArgument(_:)`` if the backup metadata is malformed. In this case
    ///     the user's data is **not recoverable**.
    ///   - ``ZonaRosaError/svrRestoreFailed(triesRemaining:message:)`` if restoration fails. This
    ///     should never happen but if it does the user's data is **not recoverable**.
    ///   - ``ZonaRosaError/svrDataMissing(_:)`` if the backup data is not found on the server,
            ...
            net.connectionManager.withNativeHandle { connectionManager in
                backupKey.withUnsafePointer { keyBuffer in
                    metadata.withUnsafeBorrowedBuffer { metadataBuffer in
                        zonarosa_secure_value_recovery_for_backups_restore_backup_from_server(
                            promise,
                            runtime.const(),
                            keyBuffer,
                            metadataBuffer,
    ...
    public func remove() async throws {
        let _: Bool = try await self.net.asyncContext.invokeAsyncFunction {
            ...
            runtime in
            net.connectionManager.withNativeHandle { connectionManager in
                zonarosa_secure_value_recovery_for_backups_remove_backup(
                    promise,
                    runtime.const(),
                    connectionManager.const(),
                    self.auth.username,
...
extension SvrB {
    ...
    public class StoreBackupResponse: NativeHandleOwner<ZonaRosaMutPointerBackupStoreResponse> {
        override internal class func destroyNativeHandle(
            _ handle: NonNull<ZonaRosaMutPointerBackupStoreResponse>
        ) -> ZonaRosaFfiErrorRef? {
            zonarosa_backup_store_response_destroy(handle.pointer)
        }
            ...
            withNativeHandle { nativeHandle in
                failOnError {
                    let data = try invokeFnReturningFixedLengthArray {
                        zonarosa_backup_store_response_get_forward_secrecy_token($0, nativeHandle.const())
                    }
        ...
        public var metadata: Data {
            withNativeHandle { nativeHandle in
                failOnError {
                    try invokeFnReturningData {
                        zonarosa_backup_store_response_get_opaque_metadata($0, nativeHandle.const())
                    }
                }
            }
        }
        ...
        public var nextBackupSecretData: Data {
            withNativeHandle { nativeHandle in
                failOnError {
                    try invokeFnReturningData {
                        zonarosa_backup_store_response_get_next_backup_secret_data($0, nativeHandle.const())
                    }
                }
            }
        }
    ...
    ///
    /// - SeeAlso: ``BackupForwardSecrecyToken``
    public class RestoreBackupResponse: NativeHandleOwner<ZonaRosaMutPointerBackupRestoreResponse> {
        override internal class func destroyNativeHandle(
            _ handle: NonNull<ZonaRosaMutPointerBackupRestoreResponse>
        ) -> ZonaRosaFfiErrorRef? {
            zonarosa_backup_restore_response_destroy(handle.pointer)
        }
        ...
        public var forwardSecrecyToken: BackupForwardSecrecyToken {
            withNativeHandle { nativeHandle in
                failOnError {
                    let data = try invokeFnReturningFixedLengthArray {
                        zonarosa_backup_restore_response_get_forward_secrecy_token($0, nativeHandle.const())
                    }
                    return try BackupForwardSecrecyToken(contents: data)
                }
            }
        ...
        public var nextBackupSecretData: Data {
            withNativeHandle { nativeHandle in
                failOnError {
                    try invokeFnReturningData {
                        zonarosa_backup_restore_response_get_next_backup_secret_data($0, nativeHandle.const())
                    }
                }
            }
        }
...
// MARK: - ZonaRosaMutPointer Conformances

extension ZonaRosaMutPointerBackupStoreResponse: ZonaRosaMutPointer {
    public typealias ConstPointer = ZonaRosaConstPointerBackupStoreResponse
...
extension ZonaRosaConstPointerBackupStoreResponse: ZonaRosaConstPointer {
    public func toOpaque() -> OpaquePointer? {
        self.raw
    }
}

extension ZonaRosaMutPointerBackupRestoreResponse: ZonaRosaMutPointer {
    public typealias ConstPointer = ZonaRosaConstPointerBackupRestoreResponse
...
extension ZonaRosaConstPointerBackupRestoreResponse: ZonaRosaConstPointer {
    public func toOpaque() -> OpaquePointer? {
        self.raw
    }
}
