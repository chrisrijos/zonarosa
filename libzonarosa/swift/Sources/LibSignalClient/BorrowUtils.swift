...
extension ByteArray: BorrowForFfi {
    typealias Borrowed = ZonaRosaBorrowedBuffer
    func withBorrowed<Result>(_ callback: (Borrowed) throws -> Result) rethrows -> Result {
...
extension Randomness: BorrowForFfi {
    typealias Borrowed = UnsafePointer<ZonaRosaRandomnessBytes>
    func withBorrowed<Result>(_ callback: (Borrowed) throws -> Result) rethrows -> Result {
...
extension Data: BorrowForFfi {
    typealias Borrowed = ZonaRosaBorrowedBuffer
    func withBorrowed<Result>(_ callback: (ZonaRosaBorrowedBuffer) throws -> Result) rethrows -> Result {
        return try self.withUnsafeBorrowedBuffer(callback)
    }
}

extension Optional: BorrowForFfi where Wrapped: BorrowForFfi, Wrapped.Borrowed == ZonaRosaBorrowedBuffer {
    typealias Borrowed = ZonaRosaOptionalBorrowedSliceOfc_uchar
    func withBorrowed<Result>(_ callback: (ZonaRosaOptionalBorrowedSliceOfc_uchar) throws -> Result) throws -> Result {
        guard let self else {
...
extension UUID: BorrowForFfi {
    typealias Borrowed = ZonaRosaUuid
    func withBorrowed<Result>(_ callback: (Borrowed) throws -> Result) rethrows -> Result {
        return try callback(ZonaRosaUuid(bytes: self.uuid))
    }
}
...
extension ZonaRosaBorrowedSliceOfConstPointerSessionRecord: FfiBorrowedSlice {
    init(_ buffer: UnsafeBufferPointer<ZonaRosaConstPointerSessionRecord>) {
        self.init(base: buffer.baseAddress, length: buffer.count)
    }
}

extension ZonaRosaBorrowedSliceOfConstPointerProtocolAddress: FfiBorrowedSlice {
    init(_ buffer: UnsafeBufferPointer<ZonaRosaConstPointerProtocolAddress>) {
        self.init(base: buffer.baseAddress, length: buffer.count)
    }
}

extension ZonaRosaBorrowedSliceOfConstPointerPublicKey: FfiBorrowedSlice {
    init(_ buffer: UnsafeBufferPointer<ZonaRosaConstPointerPublicKey>) {
        self.init(base: buffer.baseAddress, length: buffer.count)
    }
}
