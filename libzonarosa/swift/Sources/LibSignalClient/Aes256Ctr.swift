...
public class Aes256Ctr32: NativeHandleOwner<ZonaRosaMutPointerAes256Ctr32> {
    ...
    public convenience init<KeyBytes, NonceBytes>(
        ...
        let handle = try key.withUnsafeBorrowedBuffer { keyBuffer in
            try nonce.withUnsafeBytes { nonceBytes in
                ...
                var nonceBufferWithoutCounter = ZonaRosaBorrowedBuffer(nonceBytes)
                nonceBufferWithoutCounter.length -= 4
                return try invokeFnReturningValueByPointer(.init()) {
                    zonarosa_aes256_ctr32_new(
                        $0,
                        keyBuffer,
    ...
    override internal class func destroyNativeHandle(
        _ handle: NonNull<ZonaRosaMutPointerAes256Ctr32>
    ) -> ZonaRosaFfiErrorRef? {
        return zonarosa_aes256_ctr32_destroy(handle.pointer)
    }
...
extension ZonaRosaMutPointerAes256Ctr32: ZonaRosaMutPointer {
