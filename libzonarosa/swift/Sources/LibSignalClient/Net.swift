...
/// Entry point for interacting with ZonaRosa remote services.
public class Net {
    /// The ZonaRosa environment to use when connecting to remote services.
    ///
    ...
    public enum Environment: UInt8, Sendable {
        ...
        /// ZonaRosa's staging environment.
        case staging = 0

        /// ZonaRosa's production environment.
        case production = 1
    }
    ...
    /// The "scheme" for ZonaRosa TLS proxies. See ``Net/setProxy(scheme:host:port:username:password:)``.
    public static let zonarosaTlsProxyScheme = "io.zonarosa.tls"

    /// Creates a new `Net` instance that enables interacting with services in the given ZonaRosa environment.
    ///
    ...
    ///
    /// To specify a ZonaRosa transparent TLS proxy, use ``Net/zonarosaTlsProxyScheme``,
    ...
    ///   (e.g. ZonaRosa TLS proxies don't support authentication)
    ...
    /// Sets the ZonaRosa TLS proxy host to be used for all new connections (until overridden).
    ///
    /// Sets a domain name and port to be used to proxy all new outgoing connections, using a ZonaRosa
    ...
    public func setProxy(host: String, port: UInt16?) throws {
        ...
        try self.connectionManager.setProxy(
            scheme: Net.zonarosaTlsProxyScheme,
            host: host,
            port: port,
            username: username,
            password: nil
    ...
    ///
    /// If CC is enabled, *new* connections and services may try additional routes to the ZonaRosa servers.
    /// Existing connections and services will continue with the setting they were created with.
    /// (In particular, changing this setting will not affect any existing ``ChatConnection``s.)
    ///
    ...
    /// Updates the remote configuration settings used by libzonarosa with the specified build variant.
    ///
    ...
    /// - Remove any platform-specific prefixes from keys (e.g., `"ios.libzonarosa."`).
    ...
    ///
    /// - Parameters:
    ///   - remoteConfig: A dictionary containing preprocessed libzonarosa configuration keys and their associated values
    ...
    /// Updates the remote configuration settings used by libzonarosa using Production build variant.
    ///
    /// This is a backwards-compatible overload that defaults to Production.
    ///
    /// - Parameter remoteConfig: A dictionary containing preprocessed libzonarosa configuration keys and their associated values
    @available(
        *,
        deprecated,
    ...
    /// Notifies libzonarosa that the network has changed.
    ///
    ...
    public func networkDidChange() throws {
        try self.connectionManager.withNativeHandle { connectionManager in
            try checkError(zonarosa_connection_manager_on_network_change(connectionManager.const()))
        }
    }

    /// Get the SVR-B (Secure Value Recovery for Backups) service for this network instance.
    ///
    /// SVR-B provides forward secrecy for ZonaRosa backups, ensuring that even if the user's
    ...
    ///
    /// - Throws: On error, throws a ``ZonaRosaError``. Expected error cases are
    ...
    ///   `ZonaRosaError.rateLimitedError` with the amount of time to wait before trying again.
    ///
    /// ## Example:
    ///
    /// ```swift
    ...
    public func preconnectChat() async throws {
        _ = try await self.asyncContext.invokeAsyncFunction { promise, asyncContext in
            self.connectionManager.withNativeHandle { connectionManager in
                zonarosa_authenticated_chat_connection_preconnect(
                    promise,
                    asyncContext.const(),
                    connectionManager.const()
                )
    ...
    /// ``ZonaRosaError`` if one cannot be established, or if the connection
    ...
    ///     the ZonaRosa server will always consider the list to be in priority order.
    ///
    /// - Throws: ``ZonaRosaError/appExpired(_:)`` if the current app version is too old (as judged by
    ///   the server).
    /// - Throws: ``ZonaRosaError/rateLimitedError(retryAfter:message:)`` if the server
    ///   response indicates the request should be tried again after some time.
    /// - Throws: ``ZonaRosaError/deviceDeregistered(_:)`` if the server response
    ///   indicates the device is no longer registered.
    /// - Throws: ``ZonaRosaError/possibleCaptiveNetwork(_:)`` if the server's TLS response
...
extension Auth {
    ...
    internal init(username: String, enclaveSecret: String) throws {
        let otp = try invokeFnReturningString {
            zonarosa_testing_create_otp_from_base64($0, username, enclaveSecret)
        }
...
internal class ConnectionManager: NativeHandleOwner<ZonaRosaMutPointerConnectionManager> {
    private class ProxyConfig: NativeHandleOwner<ZonaRosaMutPointerConnectionProxyConfig> {
        override class func destroyNativeHandle(
            _ handle: NonNull<ZonaRosaMutPointerConnectionProxyConfig>
        ) -> ZonaRosaFfiErrorRef? {
            zonarosa_connection_proxy_config_destroy(handle.pointer)
        }
    }
        ...
        let handle = remoteConfig.withBridgedStringMap { remoteConfig in
            failOnError {
                try invokeFnReturningValueByPointer(.init()) {
                    zonarosa_connection_manager_new($0, env.rawValue, userAgent, remoteConfig, buildVariant.rawValue)
                }
            }
        }
        ...
        do {
            let proxyConfig: ProxyConfig = try username.withCString { username in
                try password.withCString { password in
                    try invokeFnReturningNativeHandle {
                        zonarosa_connection_proxy_config_new($0, scheme, host, port, username, password)
                    }
                }
            }
            ...
            try proxyConfig.withNativeHandle { proxyConfig in
                try self.withNativeHandle {
                    try checkError(zonarosa_connection_manager_set_proxy($0.const(), proxyConfig.const()))
                }
            }
        } catch {
    ...
    internal func setInvalidProxy() {
        self.withNativeHandle {
            failOnError(zonarosa_connection_manager_set_invalid_proxy($0.const()))
        }
    }

    internal func clearProxy() {
        self.withNativeHandle {
            failOnError(zonarosa_connection_manager_clear_proxy($0.const()))
        }
    }

    internal func setCensorshipCircumventionEnabled(_ enabled: Bool) {
        self.withNativeHandle {
            failOnError(zonarosa_connection_manager_set_censorship_circumvention_enabled($0.const(), enabled))
        }
    }
    ...
    internal func setRemoteConfig(_ remoteConfig: [String: String], buildVariant: Net.BuildVariant) {
        remoteConfig.withBridgedStringMap { remoteConfig in
            self.withNativeHandle {
                failOnError(
                    zonarosa_connection_manager_set_remote_config($0.const(), remoteConfig, buildVariant.rawValue)
                )
            }
        }
    }
    ...
    override internal class func destroyNativeHandle(
        _ handle: NonNull<ZonaRosaMutPointerConnectionManager>
    ) -> ZonaRosaFfiErrorRef? {
        zonarosa_connection_manager_destroy(handle.pointer)
    }
}

extension ZonaRosaMutPointerConnectionManager: ZonaRosaMutPointer {
    public typealias ConstPointer = ZonaRosaConstPointerConnectionManager
...
extension ZonaRosaConstPointerConnectionManager: ZonaRosaConstPointer {
    public func toOpaque() -> OpaquePointer? {
        self.raw
    }
}

extension ZonaRosaMutPointerConnectionProxyConfig: ZonaRosaMutPointer {
    public typealias ConstPointer = ZonaRosaConstPointerConnectionProxyConfig
...
extension ZonaRosaConstPointerConnectionProxyConfig: ZonaRosaConstPointer {
    public func toOpaque() -> OpaquePointer? {
        self.raw
    }
}
