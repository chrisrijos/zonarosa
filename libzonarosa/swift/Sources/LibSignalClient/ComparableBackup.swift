...
public class ComparableBackup: NativeHandleOwner<ZonaRosaMutPointerComparableBackup> {
    ...
    public convenience init(purpose: MessageBackupPurpose, length: UInt64, stream: ZonaRosaInputStream) throws {
        let handle = try withInputStream(stream) { stream in
            try invokeFnReturningValueByPointer(.init()) {
                zonarosa_comparable_backup_read_unencrypted($0, stream, length, purpose.rawValue)
            }
        }
        ...
        let fields = failOnError {
            try self.withNativeHandle { result in
                try invokeFnReturningStringArray {
                    zonarosa_comparable_backup_get_unknown_fields($0, result.const())
                }
            }
        }
        ...
        return failOnError {
            try self.withNativeHandle { result in
                try invokeFnReturningString {
                    zonarosa_comparable_backup_get_comparable_string($0, result.const())
                }
            }
        }
    }
    ...
    override internal class func destroyNativeHandle(
        _ handle: NonNull<ZonaRosaMutPointerComparableBackup>
    ) -> ZonaRosaFfiErrorRef? {
        zonarosa_comparable_backup_destroy(handle.pointer)
    }
}

extension ZonaRosaMutPointerComparableBackup: ZonaRosaMutPointer {
    public typealias ConstPointer = ZonaRosaConstPointerComparableBackup
...
extension ZonaRosaConstPointerComparableBackup: ZonaRosaConstPointer {
    public func toOpaque() -> OpaquePointer? {
        self.raw
    }
}
