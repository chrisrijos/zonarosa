...
public class UnidentifiedSenderMessageContent: NativeHandleOwner<ZonaRosaMutPointerUnidentifiedSenderMessageContent> {
    public struct ContentHint: RawRepresentable, Hashable, Sendable {
        ...
        internal init(_ knownType: ZonaRosaContentHint) {
            self.init(rawValue: UInt32(knownType.rawValue))
        }

        public static var `default`: Self {
            return Self(ZonaRosaContentHintDefault)
        }

        public static var resendable: Self {
            return Self(ZonaRosaContentHintResendable)
        }

        public static var implicit: Self {
            return Self(ZonaRosaContentHintImplicit)
        }
    }
    ...
    public convenience init<Bytes: ContiguousBytes>(
        ...
        let result = try bytes.withUnsafeBorrowedBuffer { bytes in
            try invokeFnReturningValueByPointer(.init()) {
                zonarosa_unidentified_sender_message_content_deserialize($0, bytes)
            }
        }
    ...
    public convenience init<Bytes: ContiguousBytes>(
        ...
        let result = try sealedSenderMessage.withUnsafeBorrowedBuffer { messageBuffer in
            try withIdentityKeyStore(identityStore, context) { ffiIdentityStore in
                try invokeFnReturningValueByPointer(.init()) {
                    zonarosa_sealed_session_cipher_decrypt_to_usmc(
                        $0,
                        messageBuffer,
                        ffiIdentityStore
                    )
    ...
    public convenience init<GroupIdBytes: ContiguousBytes>(
        ...
        let result = try withAllBorrowed(message, sender, .bytes(groupId)) {
            ...
            groupIdBuffer in
            try invokeFnReturningValueByPointer(.init()) {
                zonarosa_unidentified_sender_message_content_new(
                    $0,
                    messageHandle.const(),
                    senderHandle.const(),
    ...
    public convenience init<MessageBytes: ContiguousBytes, GroupIdBytes: ContiguousBytes>(
        ...
        let result = try withAllBorrowed(.bytes(message), sender, .bytes(groupId)) {
            ...
            groupIdBuffer in
            try invokeFnReturningValueByPointer(.init()) {
                zonarosa_unidentified_sender_message_content_new_from_content_and_type(
                    $0,
                    messageBuffer,
                    type.rawValue,
                    senderHandle.const(),
    ...
    override internal class func destroyNativeHandle(
        _ handle: NonNull<ZonaRosaMutPointerUnidentifiedSenderMessageContent>
    ) -> ZonaRosaFfiErrorRef? {
        return zonarosa_unidentified_sender_message_content_destroy(handle.pointer)
    }
        ...
        return withNativeHandle { nativeHandle in
            failOnError {
                try invokeFnReturningNativeHandle {
                    zonarosa_unidentified_sender_message_content_get_sender_cert($0, nativeHandle.const())
                }
            }
        }
    }
        ...
        let rawType = withNativeHandle { nativeHandle in
            failOnError {
                try invokeFnReturningInteger {
                    zonarosa_unidentified_sender_message_content_get_msg_type($0, nativeHandle.const())
                }
            }
        }
        ...
        return withNativeHandle { nativeHandle in
            failOnError {
                try invokeFnReturningData {
                    zonarosa_unidentified_sender_message_content_get_contents($0, nativeHandle.const())
                }
            }
        }
    }
    ...
    public var groupId: Data? {
        let result = withNativeHandle { nativeHandle in
            failOnError {
                try invokeFnReturningData {
                    zonarosa_unidentified_sender_message_content_get_group_id_or_empty($0, nativeHandle.const())
                }
            }
        }
        if result.isEmpty {
    ...
    public var contentHint: ContentHint {
        let rawHint = withNativeHandle { nativeHandle in
            failOnError {
                try invokeFnReturningInteger {
                    zonarosa_unidentified_sender_message_content_get_content_hint($0, nativeHandle.const())
                }
            }
        }
    ...
    public func serialize() -> Data {
        return withNativeHandle { nativeHandle in
            failOnError {
                try invokeFnReturningData {
                    zonarosa_unidentified_sender_message_content_serialize($0, nativeHandle.const())
                }
            }
        }
    }
...
extension ZonaRosaMutPointerUnidentifiedSenderMessageContent: ZonaRosaMutPointer {
    public typealias ConstPointer = ZonaRosaConstPointerUnidentifiedSenderMessageContent
...
extension ZonaRosaConstPointerUnidentifiedSenderMessageContent: ZonaRosaConstPointer {
    public func toOpaque() -> OpaquePointer? {
        self.raw
    }
}
...
public func sealedSenderEncrypt(
    ...
    return try withAllBorrowed(recipient, content) { recipientHandle, contentHandle in
        try withIdentityKeyStore(identityStore, context) { ffiIdentityStore in
            try invokeFnReturningData {
                zonarosa_sealed_session_cipher_encrypt(
                    $0,
...
public func sealedSenderMultiRecipientEncrypt(
    ...
    return try withExtendedLifetime((recipients, sessions)) {
        ...
        return try withAllBorrowed(
            ...
            try withIdentityKeyStore(identityStore, context) { ffiIdentityStore in
                try invokeFnReturningData {
                    zonarosa_sealed_sender_multi_recipient_encrypt(
                        $0,
...
internal func sealedSenderMultiRecipientMessageForSingleRecipient(_ message: Data) throws -> Data {
    return try message.withUnsafeBorrowedBuffer { message in
        try invokeFnReturningData {
            zonarosa_sealed_sender_multi_recipient_message_for_single_recipient($0, message)
        }
    }
}
