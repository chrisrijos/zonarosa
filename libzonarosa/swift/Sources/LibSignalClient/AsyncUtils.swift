...
/// so we can't go from, say, `Int32` to `ZonaRosaCPromisei32` (a function taking `UnsafePointer<Int32>`,
...
internal protocol PromiseStruct {
    ...
    var cancellation_id: ZonaRosaCancellationId { get }
}

extension ZonaRosaCPromisebool: PromiseStruct {
    typealias Result = Bool
}

extension ZonaRosaCPromisei32: PromiseStruct {
    typealias Result = Int32
}

extension ZonaRosaCPromiseRawPointer: PromiseStruct {
    typealias Result = UnsafeRawPointer
}

extension ZonaRosaCPromiseMutPointerCdsiLookup: PromiseStruct {
    typealias Result = ZonaRosaMutPointerCdsiLookup
}

extension ZonaRosaCPromiseFfiCdsiLookupResponse: PromiseStruct {
    typealias Result = ZonaRosaFfiCdsiLookupResponse
}

extension ZonaRosaCPromiseFfiChatResponse: PromiseStruct {
    typealias Result = ZonaRosaFfiChatResponse
}

extension ZonaRosaCPromiseMutPointerAuthenticatedChatConnection: PromiseStruct {
    typealias Result = ZonaRosaMutPointerAuthenticatedChatConnection
}

extension ZonaRosaCPromiseMutPointerUnauthenticatedChatConnection: PromiseStruct {
    typealias Result = ZonaRosaMutPointerUnauthenticatedChatConnection
}

extension ZonaRosaCPromiseMutPointerProvisioningChatConnection: PromiseStruct {
    typealias Result = ZonaRosaMutPointerProvisioningChatConnection
}

extension ZonaRosaCPromiseMutPointerRegistrationService: PromiseStruct {
    typealias Result = ZonaRosaMutPointerRegistrationService
}

extension ZonaRosaCPromiseFfiCheckSvr2CredentialsResponse: PromiseStruct {
    typealias Result = ZonaRosaFfiCheckSvr2CredentialsResponse
}

extension ZonaRosaCPromiseMutPointerRegisterAccountResponse: PromiseStruct {
    typealias Result = ZonaRosaMutPointerRegisterAccountResponse
}

extension ZonaRosaCPromiseOptionalUuid: PromiseStruct {
    typealias Result = ZonaRosaOptionalUuid
}

extension ZonaRosaCPromiseMutPointerBackupStoreResponse: PromiseStruct {
    typealias Result = ZonaRosaMutPointerBackupStoreResponse
}

extension ZonaRosaCPromiseMutPointerBackupRestoreResponse: PromiseStruct {
    typealias Result = ZonaRosaMutPointerBackupRestoreResponse
}

extension ZonaRosaCPromiseOwnedBufferOfc_uchar: PromiseStruct {
    typealias Result = ZonaRosaOwnedBuffer
}

extension ZonaRosaCPromiseOwnedBufferOfServiceIdFixedWidthBinaryBytes: PromiseStruct {
    typealias Result = ZonaRosaOwnedBufferOfServiceIdFixedWidthBinaryBytes
}

extension ZonaRosaCPromiseOptionalPairOfc_charu832: PromiseStruct {
    typealias Result = ZonaRosaOptionalPairOfc_charu832
}
...
private class CompleterBase {
    typealias RawCompletion = @Sendable (_ error: ZonaRosaFfiErrorRef?, _ valuePtr: sending UnsafeRawPointer?) -> Void
...
/// so that it can be completed from a libzonarosa\_ffi Promise without needing
...
private class Completer<Promise: PromiseStruct>: CompleterBase {
        ...
        super.init { error, valuePtr in
            do {
                ...
                guard let valuePtr else {
                    throw ZonaRosaError.internalError("produced neither an error nor a value")
                }
    ...
    func makePromiseStruct() -> Promise {
        typealias RawPromiseCallback =
            @convention(c) (
                _ error: ZonaRosaFfiErrorRef?, _ value: sending UnsafeRawPointer?, _ context: UnsafeRawPointer?
            ) -> Void
        ...
        typealias RawPointerPromiseCallback =
            @convention(c) (
                _ error: ZonaRosaFfiErrorRef?, _ value: UnsafePointer<UnsafeRawPointer?>?, _ context: UnsafeRawPointer?
            ) -> Void
        let rawPromiseStruct = ZonaRosaCPromiseRawPointer(
        ...
        precondition(MemoryLayout<ZonaRosaCPromiseRawPointer>.size == MemoryLayout<Promise>.size)
...
/// Provides a context struct for calling Promise-based libzonarosa\_ffi functions.
///
/// Example:
///
/// ```
/// let result: Int32 = try await invokeAsyncFunction {
///   zonarosa_do_async_work($0, someInput, someOtherInput)
/// }
/// ```
///
...
internal func invokeAsyncFunction<Promise: PromiseStruct>(
    _ body: (UnsafeMutablePointer<Promise>) -> ZonaRosaFfiErrorRef?,
    saveCancellationId: (ZonaRosaCancellationId) -> Void = { _ in }
