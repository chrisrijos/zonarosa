...
///
/// - SeeAlso: ``BackupKey``
public class MessageBackupKey: NativeHandleOwner<ZonaRosaMutPointerMessageBackupKey>, @unchecked Sendable {
    ...
    public convenience init(
        ...
        let handle = try withAllBorrowed(aci, .fixed(forwardSecrecyToken)) {
            ...
            try invokeFnReturningValueByPointer(.init()) {
                zonarosa_message_backup_key_from_account_entropy_pool(
                    $0,
                    accountEntropy,
                    aci,
                    forwardSecrecyToken
    ...
    public convenience init(
        ...
        let handle = try withAllBorrowed(.fixed(backupKey), .fixed(backupId), .fixed(forwardSecrecyToken)) {
            ...
            forwardSecrecyToken in
            try invokeFnReturningValueByPointer(.init()) {
                zonarosa_message_backup_key_from_backup_key_and_backup_id(
                    $0,
                    backupKey,
                    backupId,
                    forwardSecrecyToken
    ...
    internal required init(owned handle: NonNull<ZonaRosaMutPointerMessageBackupKey>) {
        super.init(owned: handle)
    }

    override internal class func destroyNativeHandle(
        _ handle: NonNull<ZonaRosaMutPointerMessageBackupKey>
    ) -> ZonaRosaFfiErrorRef? {
        zonarosa_message_backup_key_destroy(handle.pointer)
    }
        ...
        failOnError {
            try withNativeHandle { keyHandle in
                try invokeFnReturningFixedLengthArray {
                    zonarosa_message_backup_key_get_hmac_key($0, keyHandle.const())
                }
            }
        }
    }
    ...
    public var aesKey: Data {
        failOnError {
            try withNativeHandle { keyHandle in
                try invokeFnReturningFixedLengthArray {
                    zonarosa_message_backup_key_get_aes_key($0, keyHandle.const())
                }
            }
        }
    }
...
extension ZonaRosaMutPointerMessageBackupKey: ZonaRosaMutPointer {
    public typealias ConstPointer = ZonaRosaConstPointerMessageBackupKey
...
extension ZonaRosaConstPointerMessageBackupKey: ZonaRosaConstPointer {
    public func toOpaque() -> OpaquePointer? {
        self.raw
    }
}
...
///
/// - Throws:
///  - ``ZonaRosaError/ioError(_:)``: If an IO error on the input occurs.
...
public func validateMessageBackup(
    ...
    length: UInt64,
    makeStream: () throws -> ZonaRosaInputStream
        ...
        try withInputStream(try makeStream()) { secondInput in
            try key.withNativeHandle { key in
                try invokeFnReturningNativeHandle {
                    zonarosa_message_backup_validator_validate(
                        $0,
                        key.const(),
                        firstInput,
                        secondInput,
...
/// ```
public class OnlineBackupValidator: NativeHandleOwner<ZonaRosaMutPointerOnlineBackupValidator> {
    ...
    public convenience init<Bytes: ContiguousBytes>(backupInfo: Bytes, purpose: MessageBackupPurpose) throws {
        let handle = try backupInfo.withUnsafeBorrowedBuffer { backupInfo in
            try invokeFnReturningValueByPointer(.init()) {
                zonarosa_online_backup_validator_new($0, backupInfo, purpose.rawValue)
            }
        }
        self.init(owned: NonNull(handle)!)
    }

    internal required init(owned handle: NonNull<ZonaRosaMutPointerOnlineBackupValidator>) {
        super.init(owned: handle)
    }

    override internal class func destroyNativeHandle(
        _ handle: NonNull<ZonaRosaMutPointerOnlineBackupValidator>
    ) -> ZonaRosaFfiErrorRef? {
        zonarosa_online_backup_validator_destroy(handle.pointer)
    }
    ...
    public func addFrame<Bytes: ContiguousBytes>(_ frame: Bytes) throws {
        try withNativeHandle { handle in
            try frame.withUnsafeBorrowedBuffer { frame in
                try checkError(zonarosa_online_backup_validator_add_frame(handle, frame))
            }
        }
    }
    ...
    public func finalize() throws {
        try withNativeHandle { handle in
            try checkError(zonarosa_online_backup_validator_finalize(handle))
        }
    }
}

extension ZonaRosaMutPointerOnlineBackupValidator: ZonaRosaMutPointer {
...
private class ValidationOutcome: NativeHandleOwner<ZonaRosaMutPointerMessageBackupValidationOutcome> {
    public var unknownFields: MessageBackupUnknownFields {
        let fields = failOnError {
            try self.withNativeHandle { result in
                try invokeFnReturningStringArray {
                    zonarosa_message_backup_validation_outcome_get_unknown_fields($0, result.const())
                }
            }
        }
    ...
    public var errorMessage: String? {
        try! self.withNativeHandle { result in
            try invokeFnReturningOptionalString {
                zonarosa_message_backup_validation_outcome_get_error_message($0, result.const())
            }
        }
    }

    override internal class func destroyNativeHandle(
        _ handle: NonNull<ZonaRosaMutPointerMessageBackupValidationOutcome>
    ) -> ZonaRosaFfiErrorRef? {
        zonarosa_message_backup_validation_outcome_destroy(handle.pointer)
    }
}

extension ZonaRosaMutPointerMessageBackupValidationOutcome: ZonaRosaMutPointer {
    public typealias ConstPointer = ZonaRosaConstPointerMessageBackupValidationOutcome
...
extension ZonaRosaConstPointerMessageBackupValidationOutcome: ZonaRosaConstPointer {
    public func toOpaque() -> OpaquePointer? {
        self.raw
    }
}
