...
final class KeyTransparencyTests: TestCaseBase {
    ...
    override func nonHermeticTest() throws {
        if ProcessInfo.processInfo.environment["LIBZONAROSA_TESTING_IGNORE_KT_TESTS"] != nil {
            throw XCTSkip("disabled via LIBZONAROSA_TESTING_IGNORE_KT_TESTS")
        }
    ...
    func testNonFatalErrorBridging() throws {
        do {
            try checkError(zonarosa_testing_key_trans_non_fatal_verification_failure())
            XCTFail("should have failed")
        } catch ZonaRosaError.keyTransparencyError(_) {
        } catch {
    ...
    func testFatalErrorBridging() throws {
        do {
            try checkError(zonarosa_testing_key_trans_fatal_verification_failure())
            XCTFail("should have failed")
        } catch ZonaRosaError.keyTransparencyVerificationFailed(_) {
        } catch {
            XCTFail("unexpected exception thrown: \(error)")
        }
    }
    ...
    func testChatSendErrorBridging() throws {
        do {
            try checkError(zonarosa_testing_key_trans_chat_send_error())
            XCTFail("should have failed")
    ...
    func testRetryAfter() async throws {
        do {
            ...
            XCTFail("should have failed")
        } catch ZonaRosaError.rateLimitedError(_, _) {
        } catch {
            XCTFail("Unexpected exception thrown: \(error)")
        }
    }
