...
class Mp4SanitizerTests: TestCaseBase {
    func testEmptyMp4() {
        let input: [UInt8] = []
        XCTAssertThrowsError(try sanitizeMp4(input: ZonaRosaInputStreamAdapter(input), len: UInt64(input.count))) {
            error in
            if case ZonaRosaError.invalidMediaInput = error {} else { XCTFail("\(error)") }
        }
    }
    ...
    func testNoopMinimalMp4() throws {
        ...
        let sanitized = try sanitizeMp4(input: ZonaRosaInputStreamAdapter(input), len: UInt64(input.count))
    ...
    func testMp4IoError() throws {
        XCTAssertThrowsError(try sanitizeMp4(input: ErrorInputStream(), len: 1)) { error in
            if case ZonaRosaError.ioError = error {} else { XCTFail("\(error)") }
        }
    }
}
...
class WebpSanitizerTests: TestCaseBase {
    func testEmptyWebp() {
        let input: [UInt8] = []
        XCTAssertThrowsError(try sanitizeWebp(input: ZonaRosaInputStreamAdapter(input))) { error in
            if case ZonaRosaError.invalidMediaInput = error {} else { XCTFail("\(error)") }
        }
    }
    ...
    func testMinimalWebp() throws {
        let input = webp()
        try sanitizeWebp(input: ZonaRosaInputStreamAdapter(input))
    }
