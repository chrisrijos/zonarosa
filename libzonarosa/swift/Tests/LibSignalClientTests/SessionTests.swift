...
typealias InitSession = (
    _ aliceStore: InMemoryZonaRosaProtocolStore,
    _ bobStore: InMemoryZonaRosaProtocolStore,
...
class SessionTests: TestCaseBase {
    func testSessionCipher() {
        ...
        func run(_ initSessions: InitSession) {
            ...
            let alice_store = InMemoryZonaRosaProtocolStore()
            let bob_store = InMemoryZonaRosaProtocolStore()
            ...
            let ctext_a = try! zonarosaEncrypt(
                message: ptext_a,
                for: bob_address,
            ...
            let ctext_b = try! PreKeyZonaRosaMessage(bytes: ctext_a.serialize())

            let ptext_b = try! zonarosaDecryptPreKey(
                message: ctext_b,
                from: alice_address,
            ...
            let ctext2_b = try! zonarosaEncrypt(
                message: ptext2_b,
                for: alice_address,
                sessionStore: bob_store,
                identityStore: bob_store,
            ...
            XCTAssertEqual(ctext2_b.messageType, .zonarosa)

            let ctext2_a = try! ZonaRosaMessage(bytes: ctext2_b.serialize())

            let ptext2_a = try! zonarosaDecrypt(
                message: ctext2_a,
                from: bob_address,
                sessionStore: alice_store,
                identityStore: alice_store,
    ...
    func testRejectsPreKeyMessageSentFromDifferentUser() {
        ...
        } catch ZonaRosaError.invalidMessage(_) {
            // okay, this is what InMemoryZonaRosaProtocolStore throws in this case
        } catch {
    ...
    func testSealedSenderSession() throws {
        ...
        func sealedSenderEncryptPlaintext<Bytes: ContiguousBytes>(
            ...
            let ciphertextMessage = try zonarosaEncrypt(
                message: message,
                for: address,
                sessionStore: sessionStore,
        ...
        let plaintext = try zonarosaDecryptPreKey(
            message: try! PreKeyZonaRosaMessage(bytes: usmc.contents),
            from: alice_address,
            sessionStore: bob_store,
            identityStore: bob_store,
            preKeyStore: bob_store,
        ...
        let innerMessage = try zonarosaEncrypt(
            message: [],
            for: bob_address,
            sessionStore: alice_store,
            identityStore: alice_store,
    ...
    func testSealedSenderGroupCipherWithBadRegistrationId() throws {
        ...
        let alice_store = InMemoryZonaRosaProtocolStore()
        let bob_store = InMemoryZonaRosaProtocolStore(identity: IdentityKeyPair.generate(), registrationId: 0x4000)
        ...
        do {
            ...
            XCTFail("should have thrown")
        } catch ZonaRosaError.invalidRegistrationId(address: let address, message: _) {
    ...
    func testSealedSenderGroupCipherWithExcludedRecipients() throws {
        ...
        let alice_store = InMemoryZonaRosaProtocolStore()
        let bob_store = InMemoryZonaRosaProtocolStore(identity: IdentityKeyPair.generate(), registrationId: 0x2000)
    ...
    func testDecryptionErrorMessage() throws {
        ...
        let bob_first_message = try zonarosaEncrypt(
        ...
        _ = try zonarosaDecryptPreKey(
            message: PreKeyZonaRosaMessage(bytes: bob_first_message),
            from: bob_address,
            sessionStore: alice_store,
            identityStore: alice_store,
        ...
        let bob_message = try zonarosaEncrypt(
...
private func initializeSessionsV4(
    alice_store: InMemoryZonaRosaProtocolStore,
    bob_store: InMemoryZonaRosaProtocolStore,
