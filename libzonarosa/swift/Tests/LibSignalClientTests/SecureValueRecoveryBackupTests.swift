...
final class SecureValueRecoveryBackupTests: TestCaseBase {
    ...
    private lazy var testAuth: Auth = {
        ...
        if let enclaveSecret = process.environment["LIBZONAROSA_TESTING_SVRB_ENCLAVE_SECRET"] {
        ...
        return Auth(
            username: process.environment["LIBZONAROSA_TESTING_SVRB_USERNAME"] ?? "",
            password: process.environment["LIBZONAROSA_TESTING_SVRB_PASSWORD"] ?? ""
        )
    }()
    ...
    func testPrepareBackupWithInvalidPreviousSecretDataThrowsInvalidArgument() async throws {
        await assertThrowsErrorAsync {
            ...
            guard let zonarosaError = error as? ZonaRosaError,
                case .invalidArgument(let message) = zonarosaError
            else {
                XCTFail("Expected ZonaRosaError.invalidArgument, got \(error)")
                return
            }
    ...
    func testFullBackupFlowWithPreviousSecretData() async throws {
        ...
        if testAuth.username.isEmpty || testAuth.password.isEmpty {
            throw XCTSkip("requires LIBZONAROSA_TESTING_SVRB_USERNAME and LIBZONAROSA_TESTING_SVRB_PASSWORD")
        }
