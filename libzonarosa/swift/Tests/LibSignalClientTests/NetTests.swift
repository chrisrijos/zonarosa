...
final class NetTests {
    ...
    func cdsiLookupResultConversion() async throws {
        ...
        let output: ZonaRosaFfiCdsiLookupResponse = try await asyncContext.invokeAsyncFunction { promise, asyncContext in
            zonarosa_testing_cdsi_lookup_response_convert(promise, asyncContext.const())
        }
    ...
    func cdsiLookupErrorConversion() async throws {
        let failWithError = {
            try checkError(zonarosa_testing_cdsi_lookup_error_convert($0))
            Issue.record("should have failed")
        }
        do {
            try failWithError("Protocol")
        } catch ZonaRosaError.networkProtocolError(let message) {
            #expect(
                message
        ...
        } catch ZonaRosaError.invalidAttestationData(let message) {
