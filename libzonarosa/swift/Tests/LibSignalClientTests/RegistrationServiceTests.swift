...
class RegistrationServiceConversionTests {
    ...
    func registrationSessionStateConversion() throws {
        let sessionState: RegistrationSessionState = try invokeFnReturningNativeHandle {
            zonarosa_testing_registration_session_info_convert($0)
        }
    ...
    func registerAccountResponseConversion() throws {
        let response: RegisterAccountResponse = try invokeFnReturningNativeHandle {
            zonarosa_testing_register_account_response_create_test_value($0)
        }
    ...
    func errorConversion() {
        let retryLaterCase = ("RetryAfter42Seconds", { (e: Error) in if case ZonaRosaError.rateLimitedError(retryAfter: 42, message: "Rate limited; try again after 42s") = e { true } else { false }})
        let unknownCase = ("Unknown", { (e: Error) in if case RegistrationError.unknown("some message") = e { true } else { false }})
        let timeoutCase = ("Timeout", { (e: Error) in if case ZonaRosaError.requestTimeoutError("the request timed out") = e { true } else { false }})
        ...
        let pushChallengeCase = ("PushChallenge", { (e: Error) in if case ZonaRosaError.rateLimitChallengeError(token: "token", options: [.pushChallenge], message: "retry after completing a rate limit challenge [PushChallenge]") = e { true } else { false }})

        let cases = [
            ErrorTest("CreateSession", zonarosa_testing_registration_service_create_session_error_convert, [
            ...
            ErrorTest("ResumeSession", zonarosa_testing_registration_service_resume_session_error_convert, [
                ("InvalidSessionId", { if case RegistrationError.invalidSessionId("invalid session ID value") = $0 { true } else { false }}),
            ...
            ErrorTest(
                "UpdateSession",
                zonarosa_testing_registration_service_update_session_error_convert,
                [
            ...
            ErrorTest(
                ...
                zonarosa_testing_registration_service_request_verification_code_error_convert,
                [
                    ("InvalidSessionId", { if case RegistrationError.invalidSessionId("invalid session ID value") = $0 { true } else { false }}),
                    ("SessionNotFound", { if case RegistrationError.sessionNotFound("session not found") = $0 { true } else { false }}),
            ...
            ErrorTest(
                "SubmitVerification",
                zonarosa_testing_registration_service_submit_verification_error_convert,
                [
                    ("InvalidSessionId", { if case RegistrationError.invalidSessionId("invalid session ID value") = $0 { true } else { false }}),
                    ("SessionNotFound", { if case RegistrationError.sessionNotFound("session not found") = $0 { true } else { false }}),
            ...
            ErrorTest(
                ...
                zonarosa_testing_registration_service_check_svr2_credentials_error_convert,
                [
            ...
            ErrorTest(
                "RegisterAccount",
                zonarosa_testing_registration_service_register_account_error_convert,
                [
    ...
    func checkSvr2CredentialsConvert() throws {
        ...
        #expect(
            try invokeFnReturningCheckSvr2CredentialsResponse(
                fn: zonarosa_testing_registration_service_check_svr2_credentials_response_convert
            ) == expectedEntries
        )
    }
    ...
    func convertSignedPreKey() throws {
        ...
        try key.withNativeHandle { key in
            try signedPublicPreKey.withNativeStruct { signedPublicPreKey in
                try checkError(
                    zonarosa_testing_signed_public_pre_key_check_bridges_correctly(key.const(), signedPublicPreKey)
                )
            }
        }
    }
    ...
    func fakeRemoteCreateSession() async throws {
        ...
        #expect(
            secondRequest.body
                == Data(
                    """
                    {"transport":"voice","client":"libzonarosa test"}
                    """.utf8
                )
        )
        #expect(
...
extension RegistrationService {
    static func fakeCreateSession(
        ...
        let registrationService: ZonaRosaMutPointerRegistrationService = try await fakeChatServer.asyncContext
            .invokeAsyncFunction { promise, asyncContext in
                ZonaRosaFfiRegistrationCreateSessionRequest.withNativeStruct(
                    ...
                    fakeChatServer.withNativeHandle { fakeChatServer in
                        zonarosa_testing_fake_registration_session_create_session(
                            promise,
                            asyncContext.const(),
                            request,
