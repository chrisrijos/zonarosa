            ...
            withNativeHandle { handle in
                failOnError {
                    try invokeFnReturningInteger {
                        zonarosa_testing_connection_manager_is_using_proxy($0, handle.const())
                    }
                }
            }
...
final class ChatServiceTests: TestCaseBase {
    ...
    func testConvertResponse() throws {
        do {
            ...
            let rawResponse = try invokeFnReturningValueByPointer(.init()) {
                zonarosa_testing_chat_response_convert($0, false)
            }
        ...
        do {
            // Present body
            let rawResponse = try invokeFnReturningValueByPointer(.init()) {
                zonarosa_testing_chat_response_convert($0, true)
            }
            let response = try ChatConnection.Response(consuming: rawResponse)
            XCTAssertEqual(Self.expectedStatus, response.status)
            XCTAssertEqual(Self.expectedMessage, response.message)
    ...
    func testConvertConnectError() throws {
        let failWithError = {
            try checkError(zonarosa_testing_chat_connect_error_convert($0))
        ...
        } catch ZonaRosaError.possibleCaptiveNetwork(_) {}

        do {
            try failWithError("WebSocketConnectionFailed")
        } catch ZonaRosaError.webSocketError(_) {}
        do {
            try failWithError("Timeout")
        } catch ZonaRosaError.connectionTimeoutError(_) {}
        do {
            try failWithError("AllAttemptsFailed")
        } catch ZonaRosaError.connectionFailed(_) {}
        do {
        ...
        } catch ZonaRosaError.rateLimitedError(retryAfter: 42, let message) {
    ...
    func testConvertSendError() throws {
        let failWithError = {
            try checkError(zonarosa_testing_chat_send_error_convert($0))
            XCTFail("should have failed")
        }
        do {
            try failWithError("Disconnected")
        } catch ZonaRosaError.chatServiceInactive(_) {}
        ...
        } catch ZonaRosaError.networkProtocolError(_) {}
        do {
            try failWithError("RequestTimedOut")
        } catch ZonaRosaError.requestTimeoutError(_) {}
        do {
            try failWithError("ConnectionInvalidated")
        } catch ZonaRosaError.connectionInvalidated(_) {}
        do {
            try failWithError("ConnectedElsewhere")
        } catch ZonaRosaError.connectedElsewhere(_) {}
        ...
        try internalRequest.withNativeHandle { internalRequest in
            XCTAssertEqual(
                ...
                try invokeFnReturningString {
                    zonarosa_testing_chat_request_get_method($0, internalRequest.const())
                }
            )
            XCTAssertEqual(
                expectedPathAndQuery,
                try invokeFnReturningString {
                    zonarosa_testing_chat_request_get_path($0, internalRequest.const())
                }
            )
            XCTAssertEqual(
                Self.expectedContent,
                try invokeFnReturningData {
                    zonarosa_testing_chat_request_get_body($0, internalRequest.const())
                }
            )
            for (k, v) in Self.expectedHeaders {
                XCTAssertEqual(
                    v,
                    try invokeFnReturningString {
                        zonarosa_testing_chat_request_get_header_value($0, internalRequest.const(), k)
                    }
                )
            }
        }
    ...
    func testInvalidProxyRejected() {
        ...
        check {
            try net.setProxy(host: "zonarosafoundation.org", port: 0)
        }
        check {
            try net.setProxy(scheme: "socks+shoes", host: "zonarosafoundation.org")
        }
        check {
            net.setInvalidProxy()
            throw ZonaRosaError.ioError("to match all the other test cases")
        }
    }
}
...
final class ChatConnectionTests: TestCaseBase {
    ...
    func testListenerCallbacks() async throws {
        ...
        // % protoscope -s | base64
        // The fields are described by chat_websocket.proto in the libzonarosa-net crate.

        // 1: {"PUT"}
        // 2: {"/api/v1/message"}
        // 3: {1000i64}
        // 5: {"x-zonarosa-timestamp:1000"}
        // 4: 1
    ...
    func testListenerCleanup() async throws {
        ...
        guard ProcessInfo.processInfo.environment["LIBZONAROSA_TESTING_RUN_NONHERMETIC_TESTS"] != nil else {
            throw XCTSkip()
        }
    ...
    func testConnectUnauthThroughProxy() async throws {
        guard let PROXY_SERVER = ProcessInfo.processInfo.environment["LIBZONAROSA_TESTING_PROXY_SERVER"] else {
            throw XCTSkip()
        }
