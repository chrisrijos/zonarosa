...
#
# Copyright (C) 2020-2022 ZonaRosa Platform.
...
if [[ -n "${JNI_TYPE_TAGGING:-}" ]]; then
    FEATURES+=("libzonarosa-bridge-types/jni-type-tagging")
fi
if [[ -n "${JNI_CHECK_ANNOTATIONS:-}" ]]; then
    FEATURES+=("libzonarosa-bridge-types/jni-invoke-annotated")
fi
...
check_for_debug_level_logs_if_needed () {
    ...
    if [[ -z "${DEBUG_LEVEL_LOGS:-}" ]]; then
        # See libzonarosa-jni's logging.rs for the strings matched by this pattern.
...
build_desktop_for_arch () {
    ...
    echo_then_run cargo build -p libzonarosa-jni -p libzonarosa-jni-testing ${RUST_RELEASE:+--release} ${FEATURES:+--features "${FEATURES[*]}"} --target "$1"
    copy_built_library "target/${1}/${RUST_RELEASE:-debug}" zonarosa_jni "$lib_dir" "zonarosa_jni_${suffix}"
    copy_built_library "target/${1}/${RUST_RELEASE:-debug}" zonarosa_jni_testing "$lib_dir" "zonarosa_jni_testing_${suffix}"
...
for abi in "${android_abis[@]}"; do
    ...
    echo_then_run cargo build -p libzonarosa-jni -p libzonarosa-jni-testing ${RUST_RELEASE:+--release} ${FEATURES:+--features "${FEATURES[*]}"} -Z unstable-options --target "$rust_target" --artifact-dir "${ANDROID_LIB_DIR}/$abi" --timings
