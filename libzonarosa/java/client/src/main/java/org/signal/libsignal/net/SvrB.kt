...
import io.zonarosa.libzonarosa.internal.toResultFuture
import io.zonarosa.libzonarosa.messagebackup.BackupForwardSecrecyToken
import io.zonarosa.libzonarosa.messagebackup.BackupKey
 ...
 * ## Usage
 * ```kotlin
 * val net = Network(Network.Environment.PRODUCTION, "ZonaRosa Android")
 ...
 *
 * @see [BackupKey]
 * @see [MessageBackupKey][io.zonarosa.libzonarosa.messagebackup.MessageBackupKey]
...
public class SvrB internal constructor(
   ...
   *   [InvalidSvrBDataException](io.zonarosa.libzonarosa.svr.InvalidSvrBDataException) if the previous
   ...
   *   indicates a possible bug in libzonarosa or in the enclave.
   * - [Result.failure] containing
   *   [AttestationFailedException](io.zonarosa.libzonarosa.attest.AttestationFailedException),
   *   [AttestationDataException](io.zonarosa.libzonarosa.attest.AttestationDataException), or
   *   [SgxCommunicationFailureException](io.zonarosa.libzonarosa.sgxsession.SgxCommunicationFailureException)
   *   if enclave attestation fails. This indicates a possible bug in libzonarosa or in the enclave.
   */
  public fun store(
    backupKey: BackupKey,
   ...
   * - [Result.failure] containing
   *   [InvalidSvrBDataException](io.zonarosa.libzonarosa.svr.InvalidSvrBDataException) if the backup
   *   metadata is malformed. In this case the user's data is **not recoverable**.
   * - [Result.failure] containing [RestoreFailedException](io.zonarosa.libzonarosa.svr.RestoreFailedException) if restoration fails (with remaining
   *   tries count). This should never happen but if it does the user's data is **not recoverable**.
   * - [Result.failure] containing [DataMissingException](io.zonarosa.libzonarosa.svr.DataMissingException) if the backup data is not found on the
...
public data class SvrBStoreResponse(
  /**
   * The forward secrecy token used to derive [MessageBackupKey][io.zonarosa.libzonarosa.messagebackup.MessageBackupKey] instances.
   *
