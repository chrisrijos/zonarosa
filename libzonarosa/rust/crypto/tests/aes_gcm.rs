...
fn test_kat(kat: WycheproofTest) -> Result<(), zonarosa_crypto::Error> {
    ...
    let mut gcm_enc = zonarosa_crypto::Aes256GcmEncryption::new(&key, &nonce, &aad)?;

    let mut buf = pt.clone();
    gcm_enc.encrypt(&mut buf);
    let generated_tag = gcm_enc.compute_tag();

    let mut gcm_dec = zonarosa_crypto::Aes256GcmDecryption::new(&key, &nonce, &aad)?;
    ...
    if valid {
        ...
        assert!(matches!(
            ...
            Err(zonarosa_crypto::Error::InvalidTag)
        ));
    }
...
#[test]
fn aes_gcm_wycheproof_kats() -> Result<(), zonarosa_crypto::Error> {
...
#[test]
fn aes_gcm_smoke_test() -> Result<(), zonarosa_crypto::Error> {
    ...
    let mut aes_gcm = zonarosa_crypto::Aes256GcmEncryption::new(&key, &nonce, &ad)?;
