...
/// A type byte marking one of ZonaRosa's chosen instantiations of HPKE.
#[derive(Clone, Copy, PartialEq, Eq, Debug, derive_more::TryFrom)]
#[repr(u8)]
#[allow(non_camel_case_types)]
#[try_from(repr)]
pub enum ZonaRosaHpkeCiphertextType {
    Base_X25519_HkdfSha256_Aes256Gcm = 1,
}

impl From<ZonaRosaHpkeCiphertextType> for u8 {
    fn from(value: ZonaRosaHpkeCiphertextType) -> Self {
        value as Self
    }
}

impl ZonaRosaHpkeCiphertextType {
    ...
    fn mode(self) -> HpkeMode {
        match self {
            ZonaRosaHpkeCiphertextType::Base_X25519_HkdfSha256_Aes256Gcm => HpkeMode::Base,
        }
    }

    fn kem_algorithm(self) -> hpke_types::KemAlgorithm {
        match self {
            ZonaRosaHpkeCiphertextType::Base_X25519_HkdfSha256_Aes256Gcm => {
...
impl SimpleHpkeSender for libzonarosa_core::curve::PublicKey {
    fn seal(&self, info: &[u8], aad: &[u8], plaintext: &[u8]) -> Result<Vec<u8>, HpkeError> {
        let ciphertext_type = match self.key_type() {
            libzonarosa_core::curve::KeyType::Djb => {
                ZonaRosaHpkeCiphertextType::Base_X25519_HkdfSha256_Aes256Gcm
            }
        };
...
impl SimpleHpkeReceiver for libzonarosa_core::curve::PrivateKey {
    fn open(&self, info: &[u8], aad: &[u8], ciphertext: &[u8]) -> Result<Vec<u8>, HpkeError> {
...
mod test {
    ...
    use libzonarosa_core::curve::KeyPair;
