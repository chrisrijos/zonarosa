...
use boring_zonarosa::asn1::{Asn1Integer, Asn1IntegerRef};
use boring_zonarosa::bn::{BigNum, BigNumContext};
use boring_zonarosa::ec::{EcGroup, EcKey, EcKeyRef};
use boring_zonarosa::ecdsa::EcdsaSig;
use boring_zonarosa::hash::{Hasher, MessageDigest};
use boring_zonarosa::nid::Nid;
use boring_zonarosa::pkey::{PKey, Private, Public};
use chrono::Utc;
...
impl FakeAttestationBuilder {
    fn sign_data(data: &[u8], key: &EcKeyRef<Private>) -> EcdsaSig {
        let hash = boring_zonarosa::hash::hash(MessageDigest::sha256(), data).unwrap();
