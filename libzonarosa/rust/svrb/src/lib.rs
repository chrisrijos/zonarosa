...
fn auth_commitments(
    ...
    server_ids
        ...
        .map(|id| {
            Kdf::make(
                b"ZonaRosa_SVR_ServerAuthorizationKey_20240823",
                &k_auth,
                &id.to_be_bytes(),
            )
        })
...
fn auth_secret(input: &[u8; 64], auth_pt: &RistrettoPoint) -> [u8; 32] {
    Kdf::make(
        b"ZonaRosa_SVR_MasterAuthorizationKey_20240823",
        input,
...
fn password_to_uniform_input(passwd: &[u8]) -> [u8; 64] {
    Kdf::make(b"ZonaRosa_SVR_InputFromPassword_20240823", passwd, b"")
}
...
fn encryption_key(s_enc: &[u8], k_auth: &[u8]) -> Secret {
    Kdf::make(b"ZonaRosa_SVR_EncryptionKey_20240823", s_enc, k_auth)
}
