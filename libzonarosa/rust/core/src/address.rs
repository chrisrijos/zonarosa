...
//! Types for identifying an individual ZonaRosa client instance.
...
{
    /// The standard variable-width binary representation for a ZonaRosa service ID.
    ///
    ...
    /// The standard fixed-width binary representation for a ZonaRosa service ID.
    #[inline]
    ...
    /// The standard string representation for a ZonaRosa service ID.
...
/// The type used in memory to represent a *device*, i.e. a particular ZonaRosa client instance which
...
impl ProtocolAddress {
    ...
    ///   user.
    /// - Each ZonaRosa client instance then has its own `device_id`, which must be unique among
    ///   all clients for that user.
    ///
    ///```
    /// use libzonarosa_core::{DeviceId, ProtocolAddress};
    ///
    ...
    /// An identifier representing a particular ZonaRosa client instance to send to.
    ///
    /// For example, if a user has set up ZonaRosa on both their phone and laptop, a particular
