...
pub fn run(action: &str) {
    ...
    for possible_library_name in &[
        "libzonarosa_neon_futures_tests.so",
        "libzonarosa_neon_futures_tests.dylib",
        "zonarosa_neon_futures_tests.dll",
    ] {
    ...
    let node_library_path = std::env::temp_dir().join("zonarosa_neon_futures_tests.node");
    ...
    let status = Command::new("npm")
        .arg("run")
        .arg(action)
        .env("ZONAROSA_NEON_FUTURES_TEST_LIB", &node_library_path)
        .env("ZONAROSA_NEON_FUTURES_TEST_SMOKE_ONLY", smoke_test_only)
    ...
    if !status.success() {
        eprintln!(
            "\ncd {test_cases:?} && ZONAROSA_NEON_FUTURES_TEST_LIB={node_library_path:?} npm run {action}\n"
        );
