...
use libzonarosa_bridge::jni::call_static_method_unchecked;
use libzonarosa_bridge::{describe_panic, jni_signature};

// Keep this in sync with ZonaRosaProtocolLogger.java, as well as the list below.
#[derive(Clone, Copy)]
enum JavaLogLevel {
    Verbose = 2,
    Debug = 3,
...
impl log::Log for JniLogger {
    fn enabled(&self, metadata: &log::Metadata) -> bool {
        libzonarosa_bridge::logging::log_enabled_in_apps(metadata)
    }
...
fn set_max_level_from_java_level(max_level: jint) {
    // Keep this in sync with ZonaRosaProtocolLogger.java.
    let level = match max_level {
        ...
        _ => panic!("invalid log level (see ZonaRosaProtocolLogger)"),
    };
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn Java_org_zonarosa_libzonarosa_internal_Native_Logger_1Initialize(
    env: JNIEnv,
    _class: JClass,
    max_level: jint,
    logger_class: JClass,
    ...
    abort_on_panic(|| {
        ...
        match log::set_logger(Box::leak(Box::new(logger))) {
            Ok(_) => {
                ...
                log::info!(
                    "Initializing libzonarosa version:{}",
            ...
            Err(_) => {
                log::warn!("logging already initialized for libzonarosa; ignoring later call");
            }
        }
    });
}
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn Java_org_zonarosa_libzonarosa_internal_Native_Logger_1SetMaxLevel(
    _env: JNIEnv,
    _class: JClass,
    max_level: jint,
) {
