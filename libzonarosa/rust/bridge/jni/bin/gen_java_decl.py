...
IGNORE_THIS_WARNING = re.compile(
    ...
    r'WARN: Skip libzonarosa-bridge(-testing)?::.+ - \(not `(pub|no_mangle)`\)\.|'
...
JAVA_DECL = re.compile(r"""
    ...
    Java_org_zonarosa_libzonarosa_internal_Native(?:Testing)?_ # The required JNI prefix
...
def main() -> None:
    ...
    convert_to_java(
        ...
        out_path=os.path.join(repo_root, 'java', 'shared', 'java', 'org', 'zonarosa', 'libzonarosa', 'internal', 'Native.kt'),
        verify=args.verify,
    )
    ...
    convert_to_java(
        ...
        out_path=os.path.join(repo_root, 'java', 'shared', 'java', 'org', 'zonarosa', 'libzonarosa', 'internal', 'NativeTesting.kt'),
        verify=args.verify,
    )
