...
use libzonarosa_bridge::ffi::{
    self, NullPointerError, ZonaRosaFfiError, run_ffi_safe, write_result_to,
};
use libzonarosa_bridge::{IllegalArgumentError, ffi_arg_type, ffi_result_type};
use libzonarosa_bridge_macros::bridge_fn;
use libzonarosa_core::ProtocolAddress;
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn zonarosa_error_get_type(err: *const ZonaRosaFfiError) -> u32 {
...
fn Error_GetMessage(err: &ZonaRosaFfiError) -> String {
    err.to_string()
}

#[bridge_fn(jni = false, node = false)]
fn Error_GetAddress(err: &ZonaRosaFfiError) -> Result<ProtocolAddress, IllegalArgumentError> {
    err.provide_address()
        .map_err(|_| IllegalArgumentError::new(format!("cannot get address from error ({err})")))
}

#[bridge_fn(jni = false, node = false)]
fn Error_GetUuid(err: &ZonaRosaFfiError) -> Result<Uuid, IllegalArgumentError> {
    err.provide_uuid()
...
#[bridge_fn(jni = false, node = false)]
fn Error_GetUnknownFields(err: &ZonaRosaFfiError) -> Result<Box<[String]>, IllegalArgumentError> {
    Ok(err
...
fn Error_GetRegistrationErrorNotDeliverable(
    ...
    let libzonarosa_net_chat::api::registration::VerificationCodeNotDeliverable {
        reason,
        permanent_failure,
    } = err
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn zonarosa_error_get_registration_lock(
    out_time_remaining_seconds: *mut u64,
    out_svr2_username: *mut *const c_char,
    out_svr2_password: *mut *const c_char,
    err: *const ZonaRosaFfiError,
) -> *mut ZonaRosaFfiError {
    let err = AssertUnwindSafe(err);
    run_ffi_safe(|| {
        let err = unsafe { err.as_ref().ok_or(NullPointerError)? };

        let libzonarosa_net_chat::api::registration::RegistrationLock {
            time_remaining,
            svr2_credentials:
                libzonarosa_net::auth::Auth {
...
#[bridge_fn(jni = false, node = false)]
fn Error_GetOurFingerprintVersion(err: &ZonaRosaFfiError) -> Result<u32, IllegalArgumentError> {
    Ok(err
...
#[bridge_fn(jni = false, node = false)]
fn Error_GetTheirFingerprintVersion(err: &ZonaRosaFfiError) -> Result<u32, IllegalArgumentError> {
    Ok(err
        .provide_fingerprint_versions()
        .map_err(|_| {
            IllegalArgumentError::new(format!(
...
#[bridge_fn(jni = false, node = false)]
fn Error_GetRetryAfterSeconds(err: &ZonaRosaFfiError) -> Result<u32, IllegalArgumentError> {
...
#[bridge_fn(jni = false, node = false)]
fn Error_GetTriesRemaining(err: &ZonaRosaFfiError) -> Result<u32, IllegalArgumentError> {
...
fn Error_GetRateLimitChallenge(
    ...
    let libzonarosa_net_chat::api::RateLimitChallenge { token, options } =
