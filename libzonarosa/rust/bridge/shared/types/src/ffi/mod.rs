...
#[inline(always)]
pub fn run_ffi_safe<F: FnOnce() -> Result<(), ZonaRosaFfiError> + std::panic::UnwindSafe>(
    f: F,
...
pub unsafe fn native_handle_cast<T>(handle: *const T) -> Result<&'static T, ZonaRosaFfiError> {
    if handle.is_null() {
...
pub unsafe fn native_handle_cast_mut<T>(handle: *mut T) -> Result<&'static mut T, ZonaRosaFfiError> {
    if handle.is_null() {
        return Err(NullPointerError.into());
    }
