...
pub struct FutureResultReporter<T: ResultTypeInfo>(ZonaRosaFfiResult<T>);

impl<T: ResultTypeInfo> FutureResultReporter<T> {
    pub fn new(result: ZonaRosaFfiResult<T>) -> Self {
...
/// # use libzonarosa_bridge_types::{AsyncRuntime, ResultReporter};
...
/// [`ZonaRosaFfiError`] error containing an [`UnexpectedPanic`].
pub fn catch_unwind<T>(
    future: impl Future<Output = ZonaRosaFfiResult<T>> + Send + std::panic::UnwindSafe + 'static,
) -> impl Future<Output = ZonaRosaFfiResult<T>> + Send + std::panic::UnwindSafe + 'static {
    future
        .catch_unwind()
        .unwrap_or_else(|panic| Err(UnexpectedPanic(panic).into()))
}
