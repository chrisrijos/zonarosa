...
use libzonarosa_net::chat::fake::FakeChatRemote;
use libzonarosa_net::chat::server_requests::DisconnectCause;
use libzonarosa_net::chat::ws::ListenerEvent;
use libzonarosa_net::chat::{
...
use libzonarosa_net::connect_state::ConnectionResources;
use libzonarosa_net::env::constants::{CHAT_PROVISIONING_PATH, CHAT_WEBSOCKET_PATH};
use libzonarosa_net::env::{ConnectionConfig, Env};
use libzonarosa_net::infra::route::{
...
use libzonarosa_net::infra::tcp_ssl::InvalidProxyConfig;
use libzonarosa_net::infra::{EnableDomainFronting, EnforceMinimumTls};
use libzonarosa_net_chat::api::Unauth;
use libzonarosa_protocol::Timestamp;
        ...
        Ok(connection) => {
            match (
                ...
                (None, DirectOrProxyModeDiscriminants::ProxyOnly) => unreachable!(
                    "made a direct connection despite using only proxy routes; this is a bug in libzonarosa"
                ),
                (Some(_), DirectOrProxyModeDiscriminants::DirectOnly) => unreachable!(
                    "made a proxy connection despite not having proxy config; this is a bug in libzonarosa"
                ),
            }
        }
    ...
    DirectOrProxyProvider<
        impl RouteProvider<
            Route = UnresolvedHttpsServiceRoute<
                TlsRoute<TcpRoute<libzonarosa_net::infra::route::UnresolvedHost>>,
            >,
        > + use<>,
    >,
    ConnectError,
...
#[bridge_callbacks(jni = "io.zonarosa.libzonarosa.net.internal.BridgeChatListener")]
...
impl dyn ChatListener {
    /// A helper to translate from the libzonarosa-net enum to the separate callback methods in this
    /// trait.
...
///
/// Done as multiple functions so we can adjust the types to be more suitable for bridging.
#[bridge_callbacks(jni = "io.zonarosa.libzonarosa.net.internal.BridgeProvisioningListener")]
