...
/// ZonaRosa's Remote Config provides an interface for clients to access feature flags and configuration values for
...
/// - Keys that do not start with `"[platform].libzonarosa."` are discarded.
/// - The prefix `"[platform].libzonarosa."` is stripped from the remaining keys.
...
mod tests {
    ...
    fn grpc_keys_are_from_some_grpc_service() {
        use libzonarosa_net_grpc::proto::chat::services;
