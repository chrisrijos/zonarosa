...
pub struct FutureResultReporter<T, U> {
    result: ZonaRosaJniResult<T>,
    to_drop: U,
}

impl<T, U> FutureResultReporter<T, U> {
    pub fn new(result: ZonaRosaJniResult<T>, to_drop: U) -> Self {
        Self { result, to_drop }
    }
}
    ...
    fn report_to(self, receiver: Self::Receiver) {
        ...
        let result = env.with_local_frame(
            ...
            move |mut env| -> jni::errors::Result<()> {
                ...
                // libzonarosa-internal bug if we can help it.)
                let maybe_error: ZonaRosaJniResult<()> = {
                ...
                // From this point on we can't catch panics, because ZonaRosaJniError isn't UnwindSafe. This
...
/// # use libzonarosa_bridge_types::support::NoOpAsyncRuntime;
/// # fn test(env: &mut JNIEnv, async_runtime: &NoOpAsyncRuntime) -> ZonaRosaJniResult<()> {
...
) -> ZonaRosaJniResult<JavaCompletableFuture<'local, <O as ResultTypeInfo<'local>>::ResultType>>
where
    R: AsyncRuntime<F>,
    ...
    O: for<'a> ResultTypeInfo<'a> + std::panic::UnwindSafe + 'static,
{
    let java_future = new_instance(
        env,
        ClassName("io.zonarosa.libzonarosa.internal.CompletableFuture"),
        jni_args!(() -> void),
    )?;
...
pub fn catch_unwind<'a, O>(
    future: impl Future<Output = ZonaRosaJniResult<O>> + Send + std::panic::UnwindSafe + 'a,
) -> impl Future<Output = ZonaRosaJniResult<O>> + Send + std::panic::UnwindSafe + 'a {
    future
        .catch_unwind()
        .unwrap_or_else(|panic| Err(BridgeLayerError::UnexpectedPanic(panic).into()))
}
