...
use libzonarosa_core::try_scoped;
use libzonarosa_net::chat::{ConnectError as ChatConnectError, SendError as ChatSendError};
use libzonarosa_net::infra::errors::{RetryLater, TransportConnectError};
use libzonarosa_net::infra::ws::{WebSocketConnectError, WebSocketError};
use libzonarosa_net::svrb::Error as SvrbError;
use libzonarosa_net_chat::api::{RateLimitChallenge, RequestError as ChatRequestError};
...
impl JniError for ZonaRosaProtocolError {
    fn to_throwable_impl<'a>(
        &self,
        env: &mut JNIEnv<'a>,
    ) -> Result<JThrowable<'a>, BridgeLayerError> {
        ...
        let class_name = match self {
            ZonaRosaProtocolError::ApplicationCallbackError(_callback, exception) => {
            ...
            ZonaRosaProtocolError::UntrustedIdentity(addr) => {
                let addr_name = to_java_string(env, addr.name())?;
                return new_instance(
                    env,
                    ClassName("io.zonarosa.libzonarosa.protocol.UntrustedIdentityException"),
                    jni_args!((addr_name => java.lang.String) -> void),
                )
                .map(Into::into);
            }
            ZonaRosaProtocolError::SessionNotFound(addr) => {
                let addr_object = protocol_address_to_jobject(env, addr)?;
                let message = to_java_string(env, self.to_string())?;
                return new_instance(
                    env,
                    ClassName("io.zonarosa.libzonarosa.protocol.NoSessionException"),
                    jni_args!((
                        addr_object => io.zonarosa.libzonarosa.protocol.ZonaRosaProtocolAddress,
            ...
            ZonaRosaProtocolError::InvalidRegistrationId(addr, _value) => {
                let addr_object = protocol_address_to_jobject(env, addr)?;
                let message = to_java_string(env, self.to_string())?;
                return new_instance(
                    env,
                    ClassName("io.zonarosa.libzonarosa.protocol.InvalidRegistrationIdException"),
                    jni_args!((
                        addr_object => io.zonarosa.libzonarosa.protocol.ZonaRosaProtocolAddress,
                        message => java.lang.String,
                    ) -> void),
            ...
            ZonaRosaProtocolError::InvalidSenderKeySession { distribution_id } => {
                let distribution_id = distribution_id.convert_into(env)?;
                let message = to_java_string(env, self.to_string())?;
                return new_instance(
                    env,
                    ClassName(
                        "io.zonarosa.libzonarosa.protocol.groups.InvalidSenderKeySessionException",
                    ),
                    jni_args!((
            ...
            ZonaRosaProtocolError::SealedSenderSelfSend => {
                return new_instance(
                    env,
                    ClassName("io.zonarosa.libzonarosa.metadata.SelfSendException"),
                    jni_args!(() -> void),
                )
                .map(Into::into);
            }

            ZonaRosaProtocolError::InvalidState(_, _) => ClassName("java.lang.IllegalStateException"),

            ZonaRosaProtocolError::InvalidProtocolAddress {
                name: _,
                device_id: _,
            }
            | ZonaRosaProtocolError::InvalidArgument(_) => {
                ClassName("java.lang.IllegalArgumentException")
            }
            ZonaRosaProtocolError::FfiBindingError(_) => ClassName("java.lang.RuntimeException"),

            ZonaRosaProtocolError::DuplicatedMessage(_, _) => {
                ClassName("io.zonarosa.libzonarosa.protocol.DuplicateMessageException")
            }

            ZonaRosaProtocolError::InvalidPreKeyId
            | ZonaRosaProtocolError::InvalidSignedPreKeyId
            | ZonaRosaProtocolError::InvalidKyberPreKeyId => {
                ClassName("io.zonarosa.libzonarosa.protocol.InvalidKeyIdException")
            }

            ZonaRosaProtocolError::NoKeyTypeIdentifier
            | ZonaRosaProtocolError::SignatureValidationFailed
            | ZonaRosaProtocolError::UnknownSealedSenderServerCertificateId(_)
            | ZonaRosaProtocolError::BadKeyType(_)
            | ZonaRosaProtocolError::BadKeyLength(_, _)
            | ZonaRosaProtocolError::InvalidKeyAgreement
            | ZonaRosaProtocolError::InvalidMacKeyLength(_)
            | ZonaRosaProtocolError::BadKEMKeyType(_)
            | ZonaRosaProtocolError::WrongKEMKeyType(_, _)
            | ZonaRosaProtocolError::BadKEMKeyLength(_, _) => {
                ClassName("io.zonarosa.libzonarosa.protocol.InvalidKeyException")
            }

            ZonaRosaProtocolError::NoSenderKeyState { .. } => {
                ClassName("io.zonarosa.libzonarosa.protocol.NoSessionException")
            }

            ZonaRosaProtocolError::InvalidSessionStructure(_) => {
                ClassName("io.zonarosa.libzonarosa.protocol.InvalidSessionException")
            }

            ZonaRosaProtocolError::InvalidMessage(..)
            | ZonaRosaProtocolError::CiphertextMessageTooShort(_)
            | ZonaRosaProtocolError::InvalidProtobufEncoding
            | ZonaRosaProtocolError::InvalidSealedSenderMessage(_)
            | ZonaRosaProtocolError::BadKEMCiphertextLength(_, _) => {
                ClassName("io.zonarosa.libzonarosa.protocol.InvalidMessageException")
            }
            ZonaRosaProtocolError::UnrecognizedCiphertextVersion(_)
            | ZonaRosaProtocolError::UnrecognizedMessageVersion(_)
            | ZonaRosaProtocolError::UnknownSealedSenderVersion(_) => {
                ClassName("io.zonarosa.libzonarosa.protocol.InvalidVersionException")
            }

            ZonaRosaProtocolError::LegacyCiphertextVersion(_) => {
                ClassName("io.zonarosa.libzonarosa.protocol.LegacyMessageException")
            }
        };
...
impl JniError for libzonarosa_protocol::FingerprintError {
    fn to_throwable_impl<'a>(
        &self,
        env: &mut JNIEnv<'a>,
    ) -> Result<JThrowable<'a>, BridgeLayerError> {
        let class_name = match self {
            Self::VersionMismatch { theirs, ours } => return new_instance(
                env,
                ClassName(
                    "io.zonarosa.libzonarosa.protocol.fingerprint.FingerprintVersionMismatchException",
                ),
                jni_args!((*theirs as jint => int, *ours as jint => int) -> void),
            )
            .map(Into::into),
            Self::ParsingError(_) => {
                ClassName("io.zonarosa.libzonarosa.protocol.fingerprint.FingerprintParsingException")
            }
...
impl MessageOnlyExceptionJniError for AllConnectionAttemptsFailed {
    fn exception_class(&self) -> ClassName<'static> {
        ClassName("io.zonarosa.libzonarosa.net.NetworkException")
    }
}
...
impl JniError for libzonarosa_message_backup::ReadError {
    fn to_throwable_impl<'a>(
        &self,
        env: &mut JNIEnv<'a>,
    ) -> Result<JThrowable<'a>, BridgeLayerError> {
        ...
        new_instance(
            env,
            ClassName("io.zonarosa.libzonarosa.messagebackup.ValidationError"),
            jni_args!((message => java.lang.String, found_unknown_fields => [java.lang.String]) -> void),
        ).map(Into::into)
    }
}

impl MessageOnlyExceptionJniError for ZonaRosaCryptoError {
    fn exception_class(&self) -> ClassName<'static> {
        match self {
            ZonaRosaCryptoError::UnknownAlgorithm(_, _)
            | ZonaRosaCryptoError::InvalidInputSize
            | ZonaRosaCryptoError::InvalidNonceSize => {
                ClassName("java.lang.IllegalArgumentException")
            }
            ZonaRosaCryptoError::InvalidKeySize => {
                ClassName("io.zonarosa.libzonarosa.protocol.InvalidKeyException")
            }
            ZonaRosaCryptoError::InvalidTag => {
                ClassName("io.zonarosa.libzonarosa.protocol.InvalidMessageException")
            }
        }
    }
    ...
    fn exception_class(&self) -> ClassName<'static> {
        match self {
            HsmEnclaveError::HSMHandshakeError(_) | HsmEnclaveError::HSMCommunicationError(_) => {
                ClassName("io.zonarosa.libzonarosa.hsmenclave.EnclaveCommunicationFailureException")
            }
            HsmEnclaveError::TrustedCodeError => {
                ClassName("io.zonarosa.libzonarosa.hsmenclave.TrustedCodeMismatchException")
            }
    ...
    fn exception_class(&self) -> ClassName<'static> {
        match self {
            EnclaveError::NoiseHandshakeError(_) | EnclaveError::NoiseError(_) => {
                ClassName("io.zonarosa.libzonarosa.sgxsession.SgxCommunicationFailureException")
            }
            EnclaveError::AttestationError(_) => {
                ClassName("io.zonarosa.libzonarosa.attest.AttestationFailedException")
            }
            EnclaveError::AttestationDataError { .. } => {
                ClassName("io.zonarosa.libzonarosa.attest.AttestationDataException")
            }
...
impl MessageOnlyExceptionJniError for ZkGroupDeserializationFailure {
    fn exception_class(&self) -> ClassName<'static> {
        ClassName("io.zonarosa.libzonarosa.zkgroup.InvalidInputException")
    }
}

impl MessageOnlyExceptionJniError for ZkGroupVerificationFailure {
    fn exception_class(&self) -> ClassName<'static> {
        ClassName("io.zonarosa.libzonarosa.zkgroup.VerificationFailedException")
    }
}
    ...
    fn exception_class(&self) -> ClassName<'static> {
        match self {
            UsernameError::NicknameCannotBeEmpty => {
                ClassName("io.zonarosa.libzonarosa.usernames.CannotBeEmptyException")
            }
            UsernameError::NicknameCannotStartWithDigit => {
                ClassName("io.zonarosa.libzonarosa.usernames.CannotStartWithDigitException")
            }
            UsernameError::MissingSeparator => {
                ClassName("io.zonarosa.libzonarosa.usernames.MissingSeparatorException")
            }
            UsernameError::BadNicknameCharacter => {
                ClassName("io.zonarosa.libzonarosa.usernames.BadNicknameCharacterException")
            }
            UsernameError::NicknameTooShort => {
                ClassName("io.zonarosa.libzonarosa.usernames.NicknameTooShortException")
            }
            UsernameError::NicknameTooLong => {
                ClassName("io.zonarosa.libzonarosa.usernames.NicknameTooLongException")
            }
            UsernameError::DiscriminatorCannotBeEmpty => {
                ClassName("io.zonarosa.libzonarosa.usernames.DiscriminatorCannotBeEmptyException")
            }
            UsernameError::DiscriminatorCannotBeZero => {
                ClassName("io.zonarosa.libzonarosa.usernames.DiscriminatorCannotBeZeroException")
            }
            UsernameError::DiscriminatorCannotBeSingleDigit => ClassName(
                "io.zonarosa.libzonarosa.usernames.DiscriminatorCannotBeSingleDigitException",
            ),
            UsernameError::DiscriminatorCannotHaveLeadingZeros => ClassName(
                "io.zonarosa.libzonarosa.usernames.DiscriminatorCannotHaveLeadingZerosException",
            ),
            UsernameError::BadDiscriminatorCharacter => {
                ClassName("io.zonarosa.libzonarosa.usernames.BadDiscriminatorCharacterException")
            }
            UsernameError::DiscriminatorTooLarge => {
                ClassName("io.zonarosa.libzonarosa.usernames.DiscriminatorTooLargeException")
            }
        }
    }
}
...
impl MessageOnlyExceptionJniError for usernames::ProofVerificationFailure {
    fn exception_class(&self) -> ClassName<'static> {
        ClassName("io.zonarosa.libzonarosa.usernames.ProofVerificationFailureException")
    }
}
    ...
    fn exception_class(&self) -> ClassName<'static> {
        match self {
            UsernameLinkError::InputDataTooLong => {
                ClassName("io.zonarosa.libzonarosa.usernames.UsernameLinkInputDataTooLong")
            }
            UsernameLinkError::InvalidEntropyDataLength => {
                ClassName("io.zonarosa.libzonarosa.usernames.UsernameLinkInvalidEntropyDataLength")
            }
            _ => ClassName("io.zonarosa.libzonarosa.usernames.UsernameLinkInvalidLinkData"),
        }
    }
}

#[cfg(feature = "zonarosa-media")]
impl MessageOnlyExceptionJniError for zonarosa_media::sanitize::mp4::ParseErrorReport {
    fn exception_class(&self) -> ClassName<'static> {
        ClassName("io.zonarosa.libzonarosa.media.ParseException")
    }
}

#[cfg(feature = "zonarosa-media")]
impl MessageOnlyExceptionJniError for zonarosa_media::sanitize::webp::ParseErrorReport {
    fn exception_class(&self) -> ClassName<'static> {
        ClassName("io.zonarosa.libzonarosa.media.ParseException")
    }
}
...
mod registration {
        ...
        fn to_throwable_impl<'a>(
            ...
            let message = match self {
                ...
                RequestError::Timeout => {
                    return libzonarosa_net::chat::SendError::RequestTimedOut.to_throwable_impl(env);
                }
            ...
            make_single_message_throwable(
                env,
                message,
                ClassName("io.zonarosa.libzonarosa.net.RegistrationException"),
            )
        }
    }
    ...
    impl MessageOnlyExceptionJniError for InvalidSessionId {
        fn exception_class(&self) -> ClassName<'static> {
            ClassName("io.zonarosa.libzonarosa.net.RegistrationSessionIdInvalidException")
        }
    }
    ...
    fn session_not_found<'a>(
        ...
        make_single_message_throwable(
            env,
            message,
            ClassName("io.zonarosa.libzonarosa.net.RegistrationSessionNotFoundException"),
        )
    }
    fn not_ready_for_verification<'a>(
        ...
        make_single_message_throwable(
            env,
            message,
            ClassName("io.zonarosa.libzonarosa.net.RegistrationSessionNotReadyException"),
        )
    }
        ...
        fn to_throwable_impl<'a>(
            ...
            match self {
                ...
                RequestVerificationCodeError::SendFailed => make_single_message_throwable(
                    env,
                    &self.to_string(),
                    ClassName("io.zonarosa.libzonarosa.net.RegistrationSessionSendCodeException"),
                ),
                RequestVerificationCodeError::CodeNotDeliverable(
                    ...
                    new_instance(
                        env,
                        ClassName(
                            "io.zonarosa.libzonarosa.net.RegistrationCodeNotDeliverableException",
                        ),
                        args,
                    )
                    .map(Into::into)
        ...
        fn to_throwable_impl<'a>(
            ...
            let class_name = match self {
                RegisterAccountError::RegistrationLock(registration_lock) => {
                    let class_name =
                        ClassName("io.zonarosa.libzonarosa.net.RegistrationLockException");
                ...
                RegisterAccountError::DeviceTransferIsPossibleButNotSkipped => {
                    ClassName("io.zonarosa.libzonarosa.net.DeviceTransferPossibleException")
                }
                RegisterAccountError::RegistrationRecoveryVerificationFailed => {
                    ClassName("io.zonarosa.libzonarosa.net.RegistrationRecoveryFailedException")
                }
            };
    ...
    fn to_throwable_impl<'a>(
        ...
        let class = match *self {
            ...
            CdsiError::InvalidToken => {
                ClassName("io.zonarosa.libzonarosa.net.CdsiInvalidTokenException")
            }
            CdsiError::Protocol | CdsiError::CdsiProtocol(_) | CdsiError::Server { reason: _ } => {
                ClassName("io.zonarosa.libzonarosa.net.CdsiProtocolException")
            }
        };
    ...
    fn exception_class(&self) -> ClassName<'static> {
        match self {
            WebSocketError::Http(_) => {
                ...
                ClassName("io.zonarosa.libzonarosa.net.NetworkProtocolException")
            }

            _ => ClassName("io.zonarosa.libzonarosa.net.NetworkException"),
        }
    }
}
    ...
    fn to_throwable_impl<'a>(
        ...
        let class = match *self {
            ...
            ChatConnectError::AppExpired => {
                ClassName("io.zonarosa.libzonarosa.net.AppExpiredException")
            }
            ChatConnectError::DeviceDeregistered => {
                ClassName("io.zonarosa.libzonarosa.net.DeviceDeregisteredException")
            }
            ...
            ChatConnectError::WebSocket(WebSocketConnectError::Transport(
                ...
                ClassName("io.zonarosa.libzonarosa.net.PossibleCaptiveNetworkException")
            }
            ...
            | ChatConnectError::InvalidConnectionConfiguration => {
                ClassName("io.zonarosa.libzonarosa.net.ChatServiceException")
            }
        };
        make_single_message_throwable(env, &self.to_string(), class)
    }
    ...
    fn exception_class(&self) -> ClassName<'static> {
        match self {
            ChatSendError::Disconnected => {
                ClassName("io.zonarosa.libzonarosa.net.ChatServiceInactiveException")
            }
            ChatSendError::ConnectionInvalidated => {
                ClassName("io.zonarosa.libzonarosa.net.ConnectionInvalidatedException")
            }
            ChatSendError::ConnectedElsewhere => {
                ClassName("io.zonarosa.libzonarosa.net.ConnectedElsewhereException")
            }
            ChatSendError::WebSocket(_) => {
                ClassName("io.zonarosa.libzonarosa.net.TransportFailureException")
            }
    ...
    fn to_throwable_impl<'a>(
        ...
        match self {
            SvrbError::RestoreFailed(tries_remaining) => {
                ...
                new_instance(
                    env,
                    ClassName("io.zonarosa.libzonarosa.svr.RestoreFailedException"),
            ...
            SvrbError::DataMissing => make_single_message_throwable(
                env,
                &self.to_string(),
                ClassName("io.zonarosa.libzonarosa.svr.DataMissingException"),
            ),
            ...
            | SvrbError::DecryptionError(_) => make_single_message_throwable(
                env,
                &self.to_string(),
                ClassName("io.zonarosa.libzonarosa.svr.InvalidSvrBDataException"),
            ),
        }
    }
}

impl MessageOnlyExceptionJniError for libzonarosa_net_chat::api::DisconnectedError {
    fn exception_class(&self) -> ClassName<'static> {
        match self {
...
impl MessageOnlyExceptionJniError for libzonarosa_net_chat::api::keytrans::Error {
    fn exception_class(&self) -> ClassName<'static> {
        match self {
            Self::VerificationFailed(libzonarosa_keytrans::Error::VerificationFailed(_)) => {
                ClassName("io.zonarosa.libzonarosa.keytrans.VerificationFailedException")
            }
            Self::VerificationFailed(_) | Self::InvalidResponse(_) | Self::InvalidRequest(_) => {
                ClassName("io.zonarosa.libzonarosa.keytrans.KeyTransparencyException")
            }
        }
    }
}
...
impl JniError for RetryLater {
    fn to_throwable_impl<'a>(
        ...
        new_instance(
            env,
            ClassName("io.zonarosa.libzonarosa.net.RetryLaterException"),
...
impl JniError for RateLimitChallenge {
    fn to_throwable_impl<'a>(
        ...
        new_instance(
            env,
            ClassName("io.zonarosa.libzonarosa.net.RateLimitChallengeException"),
            jni_args!((
                message => java.lang.String,
                token => java.lang.String,
                options => [io.zonarosa.libzonarosa.net.ChallengeOption]) -> void),
        )
        .map(Into::into)
    }
}
    ...
    fn to_throwable_impl<'a>(
        ...
        match self {
            ChatRequestError::Timeout => make_single_message_throwable(
                env,
                "Request timed out",
                ClassName("io.zonarosa.libzonarosa.net.TimeoutException"),
            ),
            ...
            ChatRequestError::ServerSideError => make_single_message_throwable(
                env,
                "Server-side error",
                ClassName("io.zonarosa.libzonarosa.net.ServerSideErrorException"),
            ),
            ChatRequestError::Unexpected { log_safe } => make_single_message_throwable(
                env,
                &format!("Unexpected error: {}", log_safe),
                ClassName("io.zonarosa.libzonarosa.net.UnexpectedResponseException"),
            ),
...
impl JniError for libzonarosa_net_chat::api::messages::MultiRecipientSendFailure {
    fn to_throwable_impl<'a>(
        &self,
        env: &mut JNIEnv<'a>,
    ) -> Result<JThrowable<'a>, BridgeLayerError> {
        ...
        match self {
            Self::Unauthorized => make_single_message_throwable(
                env,
                &message,
                ClassName("io.zonarosa.libzonarosa.net.RequestUnauthorizedException"),
            ),
            Self::MismatchedDevices(mismatched_device_errors) => {
                ...
                new_instance(
                    env,
                    ClassName("io.zonarosa.libzonarosa.net.MismatchedDeviceException"),
                    jni_args!((
                        message => java.lang.String,
                        java_error_entries => [io.zonarosa.libzonarosa.net.MismatchedDeviceException::Entry]
                    ) -> void),
                )
                .map(Into::into)
            }
...
#[cold]
fn throw_error(env: &mut JNIEnv, error: ZonaRosaJniError) {
...
where
    F: for<'a> FnOnce(&'a mut JNIEnv<'local>) -> Result<R, ZonaRosaJniError> + std::panic::UnwindSafe,
    R: Default,
{
...
/// Constructs a Java ZonaRosaProtocolAddress from a ProtocolAddress value.
///
/// A convenience wrapper around `jobject_from_native_handle` for ZonaRosaProtocolAddress.
fn protocol_address_to_jobject<'a>(
    ...
    jobject_from_native_handle(
        env,
        ClassName("io.zonarosa.libzonarosa.protocol.ZonaRosaProtocolAddress"),
        handle,
    )
}
...
pub fn with_local_frame_returning_local<'env>(
    ...
    capacity: i32,
    context: &'static str,
    body: impl for<'local> FnOnce(&mut JNIEnv<'local>) -> ZonaRosaJniResult<JObject<'local>>,
) -> ZonaRosaJniResult<JObject<'env>> {
...
pub enum CiphertextMessageRef<'a> {
    ZonaRosaMessage(&'a ZonaRosaMessage),
    PreKeyZonaRosaMessage(&'a PreKeyZonaRosaMessage),
...
impl<'a> CiphertextMessageRef<'a> {
    pub fn message_type(self) -> CiphertextMessageType {
        match self {
            CiphertextMessageRef::ZonaRosaMessage(_) => CiphertextMessageType::Whisper,
            CiphertextMessageRef::PreKeyZonaRosaMessage(_) => CiphertextMessageType::PreKey,
    ...
    pub fn serialize(self) -> &'a [u8] {
        match self {
            CiphertextMessageRef::ZonaRosaMessage(x) => x.serialized(),
            CiphertextMessageRef::PreKeyZonaRosaMessage(x) => x.serialized(),
    ...
    ( $typ:ty as $jni_name:ident ) => {
        ::paste::paste! {
            #[unsafe(export_name = concat!(
                env!("LIBZONAROSA_BRIDGE_FN_PREFIX_JNI"),
                stringify!($jni_name),
                "_1Destroy"
            ))]
