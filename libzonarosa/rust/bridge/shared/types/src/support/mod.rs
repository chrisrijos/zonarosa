...
///   `zonarosa_foo_destroy` and `Native.Foo_Destroy`.
///
/// - If `clone = true` is passed to `bridge_handle_fns`, a `zonarosa_foo_clone` function will be
...
///
/// - FFI: boxed values are opaque structs with manual memory management (`ZonaRosaFoo *`).
...
/// A wrapper struct so we can implement e.g. [`PreKeyStore`](libzonarosa_protocol::PreKeyStore) for
/// all `BridgePreKeyStore`s (the corresponding
/// [`bridge_callbacks`](libzonarosa_bridge_macros::bridge_callbacks) trait).
///
