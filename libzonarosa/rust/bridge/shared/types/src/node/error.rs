...
use libzonarosa_net::infra::errors::TransportConnectError;
use libzonarosa_net::infra::ws::WebSocketConnectError;
use neon::thread::LocalKey;
#[cfg(feature = "zonarosa-media")]
use zonarosa_media::sanitize::mp4::{Error as Mp4Error, ParseError as Mp4ParseError};
#[cfg(feature = "zonarosa-media")]
use zonarosa_media::sanitize::webp::{Error as WebpError, ParseError as WebpParseError};

use super::*;
use crate::support::{IllegalArgumentError, WithContext};

static ERRORS_MODULE: LocalKey<Root<JsObject>> = LocalKey::new();
const ERROR_CLASS_NAME: &str = "LibZonaRosaErrorBase";
...
fn new_js_error<'a, C: Context<'a>>(
    ...
    result.unwrap_or_else(|failure| {
        ...
        let name = name.unwrap_or("LibZonaRosaError");
...
/// Provides a simple [`ZonaRosaNodeError`] implementation.
///
/// Implementing types get a straightforward blanket implementation of
/// [`ZonaRosaNodeError`] that converts to a generic error with
/// [`self.to_string()`](ToString::to_string) as the error message.
pub trait DefaultZonaRosaNodeError: ToString {}

impl<S: DefaultZonaRosaNodeError> ZonaRosaNodeError for S {
    fn into_throwable<'a, C: Context<'a>>(
        self,
        cx: &mut C,
        operation_name: &str,
...
impl DefaultZonaRosaNodeError for IllegalArgumentError {}

impl ZonaRosaNodeError for ZonaRosaProtocolError {
    fn into_throwable<'a, C: Context<'a>>(
        self,
        cx: &mut C,
        operation_name: &str,
        ...
        let message = self.to_string();
        match self {
            ZonaRosaProtocolError::DuplicatedMessage(..) => new_js_error(
                cx,
            ...
            ZonaRosaProtocolError::SealedSenderSelfSend => new_js_error(
                cx,
            ...
            ZonaRosaProtocolError::InvalidProtocolAddress { name, device_id } => {
                let make_extra_props = |cx: &mut C| {
                    let props = cx.empty_object();
            ...
            ZonaRosaProtocolError::InvalidSessionStructure(..) => new_js_error(
                cx,
                Some("InvalidSession"),
                &message,
                operation_name,
...
impl DefaultZonaRosaNodeError for libzonarosa_protocol::FingerprintError {}

impl DefaultZonaRosaNodeError for device_transfer::Error {}

impl DefaultZonaRosaNodeError for attest::hsm_enclave::Error {}

impl DefaultZonaRosaNodeError for attest::enclave::Error {}

impl DefaultZonaRosaNodeError for zonarosa_crypto::Error {}

impl ZonaRosaNodeError for libzonarosa_net::svrb::Error {
    fn into_throwable<'a, C: Context<'a>>(
        self,
        cx: &mut C,
        operation_name: &str,
...
impl DefaultZonaRosaNodeError for zkgroup::ZkGroupVerificationFailure {}

impl DefaultZonaRosaNodeError for zkgroup::ZkGroupDeserializationFailure {}

impl ZonaRosaNodeError for usernames::UsernameError {
    fn into_throwable<'a, C: Context<'a>>(
        self,
        cx: &mut C,
        operation_name: &str,
...
impl DefaultZonaRosaNodeError for usernames::ProofVerificationFailure {}

impl ZonaRosaNodeError for usernames::UsernameLinkError {
    fn into_throwable<'a, C: Context<'a>>(
        self,
        cx: &mut C,
        operation_name: &str,
...
#[cfg(feature = "zonarosa-media")]
impl ZonaRosaNodeError for Mp4Error {
    fn into_throwable<'a, C: Context<'a>>(
        self,
        cx: &mut C,
        operation_name: &str,
...
#[cfg(feature = "zonarosa-media")]
impl ZonaRosaNodeError for WebpError {
    fn into_throwable<'a, C: Context<'a>>(
        self,
        cx: &mut C,
        operation_name: &str,
...
impl ZonaRosaNodeError for std::io::Error {
    fn into_throwable<'a, C: Context<'a>>(
        mut self,
        cx: &mut C,
        _operation_name: &str,
...
impl ZonaRosaNodeError for libzonarosa_net::chat::ConnectError {
    fn into_throwable<'a, C: Context<'a>>(
        self,
        cx: &mut C,
        operation_name: &str,
...
impl ZonaRosaNodeError for libzonarosa_net::chat::SendError {
    fn into_throwable<'a, C: Context<'a>>(
        self,
        cx: &mut C,
        operation_name: &str,
...
impl ZonaRosaNodeError for libzonarosa_net::infra::errors::RetryLater {
    fn into_throwable<'a, C: Context<'a>>(
        self,
        cx: &mut C,
        operation_name: &str,
...
impl ZonaRosaNodeError for libzonarosa_net_chat::api::RateLimitChallenge {
    fn into_throwable<'a, C: Context<'a>>(
        self,
        cx: &mut C,
        operation_name: &str,
...
impl ZonaRosaNodeError for http::uri::InvalidUri {
    fn into_throwable<'a, C: Context<'a>>(
        self,
        cx: &mut C,
        operation_name: &str,
...
impl ZonaRosaNodeError for libzonarosa_net::cdsi::LookupError {
    fn into_throwable<'a, C: Context<'a>>(
        self,
        cx: &mut C,
        operation_name: &str,
...
impl<E: ZonaRosaNodeError> ZonaRosaNodeError for libzonarosa_net_chat::api::RequestError<E> {
    fn into_throwable<'a, C: Context<'a>>(
        self,
        cx: &mut C,
        operation_name: &str,
        ...
        let io_error_message: Cow<'static, str> = match self {
            ...
            Self::Timeout => {
                return libzonarosa_net::chat::SendError::RequestTimedOut
...
impl ZonaRosaNodeError for std::convert::Infallible {
    fn into_throwable<'a, C: Context<'a>>(
        self,
        _cx: &mut C,
        _operation_name: &str,
...
impl ZonaRosaNodeError for libzonarosa_net_chat::api::messages::MultiRecipientSendFailure {
    fn into_throwable<'a, C: Context<'a>>(
        self,
        cx: &mut C,
        operation_name: &str,
...
mod registration {
    ...
    impl<E: Into<BridgedErrorVariant> + std::fmt::Display> ZonaRosaNodeError for RequestError<E> {
        fn into_throwable<'a, C: Context<'a>>(
            self,
            cx: &mut C,
            operation_name: &str,
            ...
            ZonaRosaNodeError::into_throwable(inner, cx, operation_name)
        }
    }
    ...
    impl ZonaRosaNodeError for BridgedErrorVariant {
        fn into_throwable<'a, C: Context<'a>>(
            self,
            cx: &mut C,
            operation_name: &str,
...
impl ZonaRosaNodeError for CancellationError {
    fn into_throwable<'a, C: Context<'a>>(
        self,
        cx: &mut C,
        operation_name: &str,
...
impl ZonaRosaNodeError for libzonarosa_message_backup::ReadError {
    fn into_throwable<'a, C: Context<'a>>(
        self,
        cx: &mut C,
        operation_name: &str,
    ) -> Handle<'a, JsError> {
        let libzonarosa_message_backup::ReadError {
            error,
...
impl ZonaRosaNodeError for libzonarosa_net_chat::api::DisconnectedError {
    fn into_throwable<'a, C: Context<'a>>(
        self,
        cx: &mut C,
        operation_name: &str,
...
impl ZonaRosaNodeError for libzonarosa_net_chat::api::keytrans::Error {
    fn into_throwable<'a, C: Context<'a>>(
        self,
        cx: &mut C,
        operation_name: &str,
    ) -> Handle<'a, JsError> {
        use libzonarosa_keytrans::Error as KtError;

        let message = self.to_string();
        let name = match self {
            libzonarosa_net_chat::api::keytrans::Error::VerificationFailed(
                KtError::VerificationFailed(_),
            ) => "KeyTransparencyVerificationFailed",
            libzonarosa_net_chat::api::keytrans::Error::VerificationFailed(_)
            | libzonarosa_net_chat::api::keytrans::Error::InvalidResponse(_)
            | libzonarosa_net_chat::api::keytrans::Error::InvalidRequest(_) => "KeyTransparencyError",
        };
        new_js_error(
            cx,
            Some(name),
...
/// Converts a JavaScript error message to a [`ZonaRosaProtocolError::ApplicationCallbackError`].
pub fn js_error_to_rust(func: &'static str, err: String) -> ZonaRosaProtocolError {
    ZonaRosaProtocolError::ApplicationCallbackError(func, Box::new(CallbackError::new(err)))
}

impl From<WithContext<String>> for ZonaRosaProtocolError {
