...
pub use zonarosa_neon_futures::call_method;
use zonarosa_neon_futures::{JsFuture, JsPromiseResult};
...
macro_rules! node_register {
    ( $name:ident ) => {
        ::paste::paste! {
            ...
            #[linkme::distributed_slice($crate::node::LIBZONAROSA_FNS)]
            static [<zonarosa_register_node_ $name>]: (&str, $crate::node::JsFn) =
...
impl RootAndChannel {
    ...
    ///
    /// Used by [`bridge_callbacks`](libzonarosa_bridge_macros::bridge_callbacks), but can be invoked
    /// directly as well.
