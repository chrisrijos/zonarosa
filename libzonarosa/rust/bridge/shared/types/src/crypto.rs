...
pub struct Aes256GcmEncryption {
    gcm: Option<zonarosa_crypto::Aes256GcmEncryption>,
}

impl Aes256GcmEncryption {
    pub fn new(key: &[u8], nonce: &[u8], associated_data: &[u8]) -> Result<Self> {
        let gcm = zonarosa_crypto::Aes256GcmEncryption::new(key, nonce, associated_data)?;
...
pub struct Aes256GcmDecryption {
    gcm: Option<zonarosa_crypto::Aes256GcmDecryption>,
}

impl Aes256GcmDecryption {
    pub fn new(key: &[u8], nonce: &[u8], associated_data: &[u8]) -> Result<Self> {
        let gcm = zonarosa_crypto::Aes256GcmDecryption::new(key, nonce, associated_data)?;
        Ok(Self { gcm: Some(gcm) })
    }
