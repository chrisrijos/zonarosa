...
use libzonarosa_net::connect_state::{
...
use libzonarosa_net::enclave::{EnclaveEndpoint, EnclaveKind};
use libzonarosa_net::env::{Env, StaticIpOrder, UserAgent};
...
use libzonarosa_net::infra::tcp_ssl::{InvalidProxyConfig, TcpSslConnector};
use libzonarosa_net::infra::{AsHttpHeader as _, EnableDomainFronting, OverrideNagleAlgorithm};
...
pub use libzonarosa_net::infra::EnforceMinimumTls;
...
impl Environment {
    pub fn env(self) -> Env<'static> {
        match self {
            Self::Staging => libzonarosa_net::env::STAGING,
            Self::Prod => libzonarosa_net::env::PROD,
        }
    }
}
...
impl EndpointConnections {
    fn new(
        ...
        log::info!(
            ...
            // Either it's chat.zonarosa.io, chat.staging.zonarosa.io, or something that indicates
            // testing. (Or the person running this isn't ZonaRosa.)
...
impl ConnectionManager {
    ...
    pub fn new_from_static_environment(
        ...
        let user_agent = UserAgent::with_libzonarosa_version(user_agent);
    ...
    fn chat_grpc_overrides(&self) -> HashMap<&'static str, libzonarosa_net::chat::GrpcOverride> {
        use libzonarosa_net::chat::GrpcOverride;
        let guard = self.remote_config.lock().expect("not poisoned");
        guard
            .iter_enabled()
            .filter_map(|(k, v)| {
...
mod test {
    ...
    use libzonarosa_net::chat::ConnectError;
