...
use libzonarosa_bridge_types::net::registration::{
...
use libzonarosa_net_chat::api::{ChallengeOption, RateLimitChallenge, Unauth};
use libzonarosa_net_chat::registration::*;
use uuid::uuid;
...
impl ConnectUnauthChat for ConnectFakeChat {
    fn connect_chat(
    ...
    ) -> BoxFuture<'_, Result<Unauth<ChatConnection>, libzonarosa_net::chat::ConnectError>> {
        let mut on_disconnect = Some(on_disconnect);
        let listener = move |event| match event {
            libzonarosa_net::chat::ws::ListenerEvent::Finished(_) => drop(on_disconnect.take()),
            libzonarosa_net::chat::ws::ListenerEvent::ReceivedAlerts(_)
            | libzonarosa_net::chat::ws::ListenerEvent::ReceivedMessage(_, _) => (),
        };
        ...
        std::future::ready(
            self.1
                .send(remote)
                .map_err(|_| libzonarosa_net::chat::ConnectError::AllAttemptsFailed)
                .map(|()| Unauth(chat)),
        )
        .boxed()
    }
