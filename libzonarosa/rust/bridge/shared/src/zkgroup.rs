...
use libzonarosa_bridge_types::zkgroup::validate_serialization;
use libzonarosa_protocol::{Aci, Pni, ServiceId};
use uuid::Uuid;
...
fn BackupAuthCredentialRequestContext_New(backup_key: &[u8; 32], uuid: Uuid) -> Vec<u8> {
    let backup_key: libzonarosa_account_keys::BackupKey =
        libzonarosa_account_keys::BackupKey(*backup_key);
