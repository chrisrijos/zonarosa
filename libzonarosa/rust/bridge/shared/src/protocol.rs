...
use libzonarosa_bridge_macros::*;
#[cfg(feature = "jni")]
use libzonarosa_bridge_types::jni;
use libzonarosa_core::InvalidDeviceId;
use libzonarosa_protocol::error::Result;
...
pub(crate) use libzonarosa_protocol::Timestamp;
...
bridge_handle_fns!(PreKeyZonaRosaMessage);
...
bridge_handle_fns!(ZonaRosaMessage, ffi = message);
...
fn HKDF_DeriveSecrets(
    ...
    hkdf::Hkdf::<sha2::Sha256>::new(salt, ikm)
        ...
        .map_err(|_| {
            ZonaRosaProtocolError::InvalidArgument(format!("output too long ({output_length})"))
        })?;
    Ok(buffer)
}
...
fn HKDF_Derive(output: &mut [u8], ikm: &[u8], label: &[u8], salt: &[u8]) -> Result<()> {
    hkdf::Hkdf::<sha2::Sha256>::new(Some(salt), ikm)
        .expand(label, output)
        .map_err(|_| {
            ZonaRosaProtocolError::InvalidArgument(format!("output too long ({})", output.len()))
        })?;
    Ok(())
}
...
fn ServiceId_ParseFromServiceIdBinary(input: &[u8]) -> Result<ServiceId> {
    ServiceId::parse_from_service_id_binary(input).ok_or_else(|| {
        ZonaRosaProtocolError::InvalidArgument("invalid Service-Id-Binary".to_string())
    })
}
...
fn ServiceId_ParseFromServiceIdString(input: String) -> Result<ServiceId> {
    ServiceId::parse_from_service_id_string(&input).ok_or_else(|| {
        ZonaRosaProtocolError::InvalidArgument("invalid Service-Id-String".to_string())
    })
}
...
fn ZonaRosaMessage_Deserialize(data: &[u8]) -> Result<ZonaRosaMessage> {
    ZonaRosaMessage::try_from(data)
}

bridge_get!(ZonaRosaMessage::body -> &[u8], ffi = "message_get_body");
bridge_get!(ZonaRosaMessage::serialized -> &[u8], ffi = "message_get_serialized");
bridge_get!(ZonaRosaMessage::counter -> u32, ffi = "message_get_counter");
bridge_get!(ZonaRosaMessage::message_version -> u32, ffi = "message_get_message_version");
...
fn ZonaRosaMessage_GetPqRatchet(msg: &ZonaRosaMessage) -> &[u8] {
    msg.pq_ratchet()
}

#[bridge_fn(ffi = "message_new")]
#[allow(clippy::too_many_arguments)]
fn ZonaRosaMessage_New(
    message_version: u8,
    mac_key: &[u8],
...
fn ZonaRosaMessage_GetSenderRatchetKey(m: &ZonaRosaMessage) -> PublicKey {
    *m.sender_ratchet_key()
}

#[bridge_fn]
fn PreKeyZonaRosaMessage_New(
    message_version: u8,
    registration_id: u32,
    ...
    zonarosa_message: &ZonaRosaMessage,
) -> Result<PreKeyZonaRosaMessage> {
    PreKeyZonaRosaMessage::new(
        ...
        *base_key,
        ...
        zonarosa_message.clone(),
    )
}

#[bridge_fn(node = false)]
fn PreKeyZonaRosaMessage_GetBaseKey(m: &PreKeyZonaRosaMessage) -> PublicKey {
    *m.base_key()
}

#[bridge_fn(node = false)]
fn PreKeyZonaRosaMessage_GetIdentityKey(m: &PreKeyZonaRosaMessage) -> PublicKey {
    *m.identity_key().public_key()
}

#[bridge_fn(node = false)]
fn PreKeyZonaRosaMessage_GetZonaRosaMessage(m: &PreKeyZonaRosaMessage) -> ZonaRosaMessage {
    m.message().clone()
}

bridge_deserialize!(PreKeyZonaRosaMessage::try_from);
bridge_get!(
    PreKeyZonaRosaMessage::serialized as Serialize -> &[u8],
    jni = "PreKeyZonaRosaMessage_1GetSerialized"
);

bridge_get!(PreKeyZonaRosaMessage::registration_id -> u32);
bridge_get!(PreKeyZonaRosaMessage::signed_pre_key_id -> u32);
bridge_get!(PreKeyZonaRosaMessage::pre_key_id -> Option<u32>);
bridge_get!(PreKeyZonaRosaMessage::message_version as GetVersion -> u32);
...
fn DecryptionErrorMessage_ForOriginalMessage(
    ...
    let original_type = CiphertextMessageType::try_from(original_type).map_err(|_| {
        ZonaRosaProtocolError::InvalidArgument(format!("unknown message type {original_type}"))
    })?;
...
fn PreKeyBundle_New(
    ...
    let device_id = device_id
        .try_into()
        .map_err(|e: InvalidDeviceId| ZonaRosaProtocolError::InvalidArgument(e.to_string()))?;
...
fn SenderCertificate_New(
    ...
    let sender_device_id = DeviceId::try_from(sender_device_id)
        .map_err(|e| ZonaRosaProtocolError::InvalidArgument(e.to_string()))?;
...
fn UnidentifiedSenderMessageContentNewFromContentAndType(
    ...
    let message_type = CiphertextMessageType::try_from(message_type).map_err(|_| {
        ZonaRosaProtocolError::InvalidArgument(format!("unknown message type {message_type}"))
    })?;
...
fn SessionRecord_GetSessionVersion(s: &SessionRecord) -> Result<u32> {
    match s.session_version() {
        Ok(v) => Ok(v),
        Err(ZonaRosaProtocolError::InvalidState(_, _)) => Ok(0),
        Err(e) => Err(e),
    }
}
...
async fn SessionCipher_DecryptZonaRosaMessage(
...
async fn SessionCipher_DecryptPreKeyZonaRosaMessage(
