...
//!
//! ```c
//! ZonaRosaFfiError *zonarosa_sender_key_message_new(
//!     ZonaRosaSenderKeyMessage **out,
//!     uint32_t key_id,
//!     uint32_t iteration,
//!     const unsigned char *ciphertext,
//!     size_t ciphertext_len,
//!     const ZonaRosaPrivateKey *pk);
//! ```
//!
//! ```java
...
//! Promise and run the function on the JavaScript event loop using the `zonarosa-neon-futures`
...
//!   variable `LIBZONAROSA_BRIDGE_FN_PREFIX_FFI`, which the client crate should set in its build.rs.
//! - JNI: Escape any underscores in the function's name per the [JNI spec][], then prepend the
//!   value of environment variable `LIBZONAROSA_BRIDGE_FN_PREFIX_JNI`, which the client should set in
//!   its build.rs. The value should be something like `Java_io.zonarosa_libzonarosa_internal_Native_`
//!   to expose the function as a static method of the class `io.zonarosa.libzonarosa.internal.Native`.
...
///
/// ```ignore
/// // Produces a C function named "zonarosa_checksum_buffer"
