...
use libzonarosa_message_backup::frame::{CursorFactory, FileReaderFactory, VerifyHmac};
use libzonarosa_message_backup::key::MessageBackupKey;
use libzonarosa_message_backup::{BackupReader, ReadResult};
...
fn is_valid_json_proto(input: Fixture<&str>) {
    ...
    let binproto =
        libzonarosa_message_backup::backup::convert_from_json(json_array).expect("failed to convert");
...
fn can_serialize_json_proto(input: Fixture<&str>) {
    ...
    println!(
        "{}",
        libzonarosa_message_backup::backup::serialize::Backup::from(result).to_string_pretty()
    )
}
