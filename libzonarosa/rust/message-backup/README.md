This crate contains a validator for ZonaRosa's backup file format. Structurally, these files are an
...
3. Run `cargo test -p libzonarosa-message-backup` to find out what needs updating.
...
5. Mention the change in the [upcoming release notes](../../RELEASE_NOTES.md). [This can be pretty simple.](https://github.com/zonarosa/libzonarosa/commit/4723fdbba1970c91795f8cdd8b7bcc4eafb66114) If there are other backup updates, group them together.

6. Open a PR and let a libzonarosa maintainer know what validation should be added later, if any.
...
- We use the `protobuf-rs` crate instead of the more popular, better-supported, and used-elsewhere-in-libzonarosa `prost` because `prost` doesn't support doing anything with unknown fields. (We just want to report them, we don't need to store them in every message. But even that doesn't seem interesting to the mainline prost folks. See <https://github.com/tokio-rs/prost/issues/2>.)

- The model types roughly parallel the protobuf message types, but they use stronger types for their fields, representing the stronger constraints of a valid backup. For example, a contact's ACI will be represented using `libzonarosa_core::Aci` rather than a plain protobuf `bytes` (`Vec<u8>` in Rust).
