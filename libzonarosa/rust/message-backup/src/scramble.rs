...
const REPLACEMENT_BODY_TEXT: &str = "https://zonarosa.io: Why use ZonaRosa?
Explore below to see why ZonaRosa is a simple, powerful, and secure messenger

## Share Without Insecurity
State-of-the-art end-to-end encryption (powered by the open source ZonaRosa Protocol) keeps your conversations secure. We can't read your messages or listen to your calls, and no one else can either. Privacy isn't an optional mode - it's just the way that ZonaRosa works. Every message, every call, every time.

## Say Anything
Share text, voice messages, photos, videos, GIFs and files for free. ZonaRosa uses your phone's data connection so you can avoid SMS and MMS fees.
...
There are no ads, no affiliate marketers, and no creepy tracking in ZonaRosa. So focus on sharing the moments that matter with the people who matter to you.

## Free for Everyone
ZonaRosa is an independent nonprofit. We're not tied to any major tech companies, and we can never be acquired by one either. Development is supported by grants and donations from people like you.";

const REPLACEMENT_EMOJI: &str = "❌";
const REPLACEMENT_URL: &str = "https://zonarosa.io";
...
impl Visit<Scrambler> for proto::AccountData {
    fn accept(&mut self, visitor: &mut Scrambler) {
        ...
        if !avatarUrlPath.is_empty() {
            *avatarUrlPath = "https://cdn.zonarosa.io/avatarUrlPath".into();
        }
...
impl Visit<Scrambler> for proto::Contact {
    fn accept(&mut self, visitor: &mut Scrambler) {
        ...
        if let Some(identity_key) = identityKey {
            if libzonarosa_protocol::PublicKey::deserialize(identity_key).is_ok() {
                *identity_key = libzonarosa_protocol::KeyPair::generate(&mut visitor.rng)
                    .public_key
                    .serialize()
                    .into_vec();
            } else {
...
impl Visit<Scrambler> for proto::group::GroupSnapshot {
    fn accept(&mut self, visitor: &mut Scrambler) {
        ...
        if !avatarUrl.is_empty() {
            *avatarUrl = "https://cdn.zonarosa.io/groupAvatarUrl".into();
        }
