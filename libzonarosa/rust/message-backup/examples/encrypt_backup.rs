...
use libzonarosa_cli_utils::read_file;
use libzonarosa_message_backup::args::parse_hex_bytes;
use libzonarosa_message_backup::export::{
    aes_cbc_encrypt, gzip_compress, hmac_checksum, pad_gzipped_bucketed,
};
use libzonarosa_message_backup::key::MessageBackupKey;
use libzonarosa_svrb::proto::Message as _;
use libzonarosa_svrb::proto::backup_metadata::{MetadataPb, metadata_pb};
...
fn main() {
    ...
    if let Format::Modern = format {
        write_bytes(
            "magic number",
            libzonarosa_message_backup::frame::forward_secrecy::MAGIC_NUMBER,
        );
