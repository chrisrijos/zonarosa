...
//! `LIBZONAROSA_TESTING`-prefixed environment variables below.

use criterion::{BenchmarkId, Criterion, black_box, criterion_group, criterion_main};
use futures::AsyncRead;
use futures::io::{BufReader, Cursor};
use libzonarosa_account_keys::{BackupForwardSecrecyToken, BackupKey};
use libzonarosa_core::Aci;
use libzonarosa_message_backup::BackupReader;
use libzonarosa_message_backup::backup::{CompletedBackup, PartialBackup, ValidateOnly};
use libzonarosa_message_backup::frame::{
    AES_IV_SIZE, AES_KEY_SIZE, Aes256CbcReader, CursorFactory, FramesReader, MacReader,
    ReaderFactory,
};
use libzonarosa_message_backup::key::MessageBackupKey;
use libzonarosa_message_backup::parse::VarintDelimitedReader;
...
const CUSTOM_BACKUP_FILE_ENV_VAR: &str = "LIBZONAROSA_TESTING_BACKUP_FILE";
const CUSTOM_BACKUP_FILE_HMAC_KEY_ENV_VAR: &str = "LIBZONAROSA_TESTING_BACKUP_FILE_HMAC_KEY";
const CUSTOM_BACKUP_FILE_AES_KEY_ENV_VAR: &str = "LIBZONAROSA_TESTING_BACKUP_FILE_AES_KEY";
...
fn decrypt_and_decompress_and_hmac_and_segment_and_parse(c: &mut Criterion) {
    fn process<R: ReaderFactory<Reader: Unpin>, R2: AsyncRead + Unpin>(
        ...
        futures::executor::block_on(async move {
            ...
            while let Some(next) = stream.read_next().await.expect("valid") {
                black_box(
                    libzonarosa_message_backup::proto::backup::Frame::parse_from_bytes(&next)
                        .expect("valid"),
                );
            }
        })
    ...
    fn process<R: ReaderFactory<Reader: Unpin>, R2: AsyncRead + Unpin>(
        ...
        futures::executor::block_on(async move {
            ...
            let mut backup = PartialBackup::<ValidateOnly>::by_parsing(
                &backup_info,
                libzonarosa_message_backup::backup::Purpose::RemoteBackup,
                |_| (),
            )
            .expect("valid");
            while let Some(next) = stream.read_next().await.expect("valid") {
