...
use libzonarosa_net::certs::ZONAROSA_ROOT_CERTIFICATES;
use libzonarosa_net::chat::test_support::simple_chat_connection;
use libzonarosa_net::connect_state::{ConnectState, ConnectionResources, SUGGESTED_CONNECT_CONFIG};
use libzonarosa_net::infra::dns::DnsResolver;
use libzonarosa_net::infra::host::Host;
use libzonarosa_net::infra::http_client::{Http2Client, Http2Connector};
use libzonarosa_net::infra::route::provider::EmptyProvider;
...
use libzonarosa_net::infra::timeouts::TimeoutOr;
use libzonarosa_net::infra::utils::no_network_change_events;
use libzonarosa_net::infra::{EnableDomainFronting, OverrideNagleAlgorithm};
use libzonarosa_net_chat::api::Unauth;
use libzonarosa_net_chat::api::usernames::UnauthenticatedChatApi;
...
async fn main() -> anyhow::Result<()> {
    ...
    let mut env = match env {
        Environment::Staging => libzonarosa_net::env::STAGING,
        Environment::Production => libzonarosa_net::env::PROD,
    };
    ...
    let username = if let Some(offset) = username.find("//zonarosa.me/#eu/") {
        let payload = username[offset..]
            .strip_prefix("//zonarosa.me/#eu/")
...
/// Connect to a gRPC server that uses ZonaRosa's pinned root certificate.
///
/// Eventually this should be covered by a libzonarosa-net-level API like `simple_chat_connection`.
