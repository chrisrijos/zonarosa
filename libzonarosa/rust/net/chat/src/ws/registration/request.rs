...
use libzonarosa_net::chat::{LanguageList, Request as ChatRequest};
use libzonarosa_net::infra::AsHttpHeader as _;
...
impl RegistrationResponse {
    pub(crate) fn into_websocket_response(
        self,
        ws_request_id: u64,
    ) -> libzonarosa_net::proto::chat_websocket::WebSocketResponseMessage {
        libzonarosa_net::proto::chat_websocket::WebSocketResponseMessage {
...
mod test {
    ...
    use base64::Engine;
    use http::StatusCode;
    use libzonarosa_core::{Aci, Pni};
    use libzonarosa_net::chat::Response as ChatResponse;
    use libzonarosa_protocol::{GenericSignedPreKey as _, KeyPair, KyberPreKeyRecord};
    ...
    static REGISTER_KEYS: LazyLock<ForServiceIds<OwnedAccountKeys>> = LazyLock::new(|| {
        ...
        ForServiceIds::generate(|_| {
            ...
            let pq_last_resort_pre_key = {
                let kem_keypair = libzonarosa_protocol::kem::KeyPair::generate(
                    libzonarosa_protocol::kem::KeyType::Kyber1024,
                    &mut rng,
                );
