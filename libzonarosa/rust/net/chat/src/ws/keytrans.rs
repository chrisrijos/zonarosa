...
use libzonarosa_core::{Aci, E164};
use libzonarosa_keytrans::{AccountData, LastTreeHead};
use libzonarosa_net::chat;
use libzonarosa_protocol::PublicKey;
...
mod test_support {
    ...
    use libzonarosa_keytrans::ChatSearchResponse;
    use libzonarosa_net::chat::ChatConnection;
    use libzonarosa_net::env;
    use libzonarosa_net::infra::EnableDomainFronting;
    use libzonarosa_net::infra::route::DirectOrProxyMode;
    ...
    // - Execute the test as `cargo test --package libzonarosa-net-chat --all-features collect_test_data -- --nocapture`
    // - Follow the prompts
...
mod test {
    ...
    use libzonarosa_keytrans::LocalStateUpdate;
    use libzonarosa_protocol::IdentityKeyPair;
    use rand::TryRngCore;
    use rand::rngs::OsRng;
    ...
    fn kt_integration_enabled() -> bool {
        let run_nonhermetic = std::env::var_os("LIBZONAROSA_TESTING_RUN_NONHERMETIC_TESTS").is_some();
        let ignore_tests = std::env::var_os("LIBZONAROSA_TESTING_IGNORE_KT_TESTS").is_some();
