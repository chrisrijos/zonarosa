...
use libzonarosa_net::chat::{LanguageList, Request as ChatRequest, Response as ChatResponse};
...
/// A client for the ZonaRosa registration API endpoints.
///
...
/// used to communicate with ZonaRosa servers.
#[derive(Debug)]
...
mod testutil {
    ...
    use libzonarosa_net::chat::{ChatConnection, ConnectError as ChatConnectError};
    ...
    impl<T> DropOnDisconnect<T> {
        ...
        pub(super) fn into_listener(mut self) -> libzonarosa_net::chat::ws::EventListener
        where
            T: Send + 'static,
        {
...
mod test {
    ...
    use futures_util::future::BoxFuture;
    use libzonarosa_net::chat::fake::FakeChatRemote;
    use libzonarosa_net::chat::{ChatConnection, ConnectError};
    use libzonarosa_net::proto::chat_websocket::WebSocketRequestMessage;
    use tokio::sync::mpsc;
