...
use libzonarosa_core::{Aci, Pni, ServiceIdKind};
use libzonarosa_net::auth::Auth;
use libzonarosa_net::chat::LanguageList;
use libzonarosa_protocol::{GenericSignedPreKey, PublicKey};
use serde_with::{
...
pub struct VerificationCodeNotDeliverable {
    // This could be a stronger type but we don't need it to be in libzonarosa and
...
impl<T> ForServiceIds<T> {
    ...
    pub fn generate(mut f: impl FnMut(libzonarosa_core::ServiceIdKind) -> T) -> Self {
        ForServiceIds {
            aci: f(libzonarosa_core::ServiceIdKind::Aci),
            pni: f(libzonarosa_core::ServiceIdKind::Pni),
        }
    }
}
