...
use libzonarosa_net::infra::route::UnsuccessfulOutcome;
...
const CHAT_CONNECT_DELAY_PARAMS: libzonarosa_net::infra::route::ConnectionOutcomeParams =
    libzonarosa_net::infra::route::ConnectionOutcomeParams {
                ...
                match err {
                    ...
                    ChatConnectError::AppExpired => {
                        return Err(FatalConnectError::Unexpected(
                            "unauthenticated socket zonarosaed app expired",
                        ));
                    }
                    ChatConnectError::DeviceDeregistered => {
                        return Err(FatalConnectError::Unexpected(
                            "unauthenticated socket zonarosaed deregistration",
                        ));
                    }
                }
            }
...
mod test {
    ...
    async fn send_request_retries_connect_on_transient_failure() {
        ...
        let response = libzonarosa_net::chat::ResponseProto {
            id: request.id,
            status: Some(200),
            ..Default::default()
        };
    ...
    async fn request_sent_to_task_cancelled_before_send() {
        ...
        fake_chat_remote
            .send_response(libzonarosa_net::chat::ResponseProto {
                id: request.id,
                status: Some(200),
                ..Default::default()
            })
