...
//! [libzonarosa-net-grpc](libzonarosa_net_grpc).
...
use libzonarosa_net::infra::errors::{LogSafeDisplay, RetryLater};
use libzonarosa_net::infra::http_client::{Http2TransportError, Http2TransportErrorKind};
use libzonarosa_net_grpc::proto::google;
    ...
    fn from(status: tonic::Status) -> Self {
        ...
        match status.code() {
            ...
            tonic::Code::ResourceExhausted => {
                ...
                {
                    return libzonarosa_net::infra::errors::RetryLater {
                        retry_after_seconds,
                    }
                    .into();
                }
...
const ZONAROSA_ERRORINFO_DOMAIN: &str = "grpc.chat.zonarosa.io";
...
fn extract_server_side_error(
    ...
    for next_info in all_detail_info {
        if next_info.domain == ZONAROSA_ERRORINFO_DOMAIN {
            if info.is_none() {
                info = Some(next_info);
            } else {
                log::warn!(
                    "multiple '{ZONAROSA_ERRORINFO_DOMAIN}' errors; ignoring later {}",
                    next_info.reason
                );
            }
        } else {
            log::warn!(
                "ignoring non-ZonaRosa error info with domain {}",
                next_info.domain,
            );
        }
    }
...
fn request_error_from_server_side_error_info<E>(
    ...
    debug_assert_eq!(info.domain, ZONAROSA_ERRORINFO_DOMAIN);
    ...
    match info.reason.as_str() {
        ...
        reason => RequestError::Unexpected {
            log_safe: format!("unexpected error in domain '{ZONAROSA_ERRORINFO_DOMAIN}': {reason}"),
        },
    }
}
...
pub(crate) mod testutil {
    ...
    use libzonarosa_net::chat::{Request, Response, SendError};
    use tonic::Status;
...
mod test {
    ...
    use libzonarosa_net::infra::http_client::Http2Client;
    ...
    fn test_extract_server_side_error() {
        ...
        let original_status = google::rpc::Status {
            ...
            details: vec![
                prost_types::Any {
                    type_url: "test.zonarosa.io/RandomTestingTypeToIgnore".into(),
                    value: vec![],
                },
