...
pub mod proto {
    pub mod chat {
        pub mod common {
            tonic::include_proto!("io.zonarosa.chat.common");
        }
        pub mod account {
            tonic::include_proto!("io.zonarosa.chat.account");
        }
        pub mod device {
            tonic::include_proto!("io.zonarosa.chat.device");
        }
        pub mod errors {
            tonic::include_proto!("io.zonarosa.chat.errors");
        }
...
impl From<libzonarosa_core::ServiceId> for proto::chat::common::ServiceIdentifier {
    fn from(value: libzonarosa_core::ServiceId) -> Self {
        let kind = match value.kind() {
            libzonarosa_core::ServiceIdKind::Aci => proto::chat::common::IdentityType::Aci,
            libzonarosa_core::ServiceIdKind::Pni => proto::chat::common::IdentityType::Pni,
        };
...
impl From<libzonarosa_core::Aci> for proto::chat::common::ServiceIdentifier {
    fn from(value: libzonarosa_core::Aci) -> Self {
        libzonarosa_core::ServiceId::from(value).into()
    }
}

impl From<libzonarosa_core::Pni> for proto::chat::common::ServiceIdentifier {
    fn from(value: libzonarosa_core::Pni) -> Self {
        libzonarosa_core::ServiceId::from(value).into()
    }
}

impl proto::chat::common::ServiceIdentifier {
    pub fn try_as_service_id(&self) -> Option<libzonarosa_core::ServiceId> {
        let Self {
            identity_type,
            uuid,
        } = self;
        Some(match (*identity_type).try_into().ok()? {
            proto::chat::common::IdentityType::Aci => {
                libzonarosa_core::Aci::from_uuid_bytes(uuid.as_slice().try_into().ok()?).into()
            }
            proto::chat::common::IdentityType::Pni => {
                libzonarosa_core::Pni::from_uuid_bytes(uuid.as_slice().try_into().ok()?).into()
            }
