...
use boring_zonarosa::ssl::SslVersion;
use const_str::{hex, ip_addr};
use http::HeaderValue;
use libzonarosa_keytrans::{DeploymentMode, PublicConfig, VerifyingKey, VerifyingKeys, VrfPublicKey};
use libzonarosa_net_infra::certs::RootCertificates;
use libzonarosa_net_infra::dns::lookup_result::LookupResult;
use libzonarosa_net_infra::host::Host;
use libzonarosa_net_infra::route::{
    DirectTcpRouteProvider, DomainFrontConfig, DomainFrontRouteProvider, HttpVersion,
    HttpsProvider, TlsRouteProvider,
};
use libzonarosa_net_infra::{
...
use crate::certs::{PROXY_G_ROOT_CERTIFICATES, ZONAROSA_ROOT_CERTIFICATES};
...
pub const TIMESTAMP_HEADER_NAME: &str = "x-zonarosa-timestamp";
pub(crate) const ALERT_HEADER_NAME: &str = "x-zonarosa-alert";
...
const DOMAIN_CONFIG_CHAT: DomainConfig = DomainConfig {
    ...
    connect: ConnectionConfig {
        hostname: "chat.zonarosa.io",
        port: DEFAULT_HTTPS_PORT,
        cert: ZONAROSA_ROOT_CERTIFICATES,
...
const DOMAIN_CONFIG_EXPERIMENTAL_CHAT_H2: DomainConfig = DomainConfig {
    ...
    ip_v6: &[],
    connect: ConnectionConfig {
        hostname: "grpc.chat.zonarosa.io",
        port: DEFAULT_HTTPS_PORT,
        cert: ZONAROSA_ROOT_CERTIFICATES,
        min_tls_version: Some(SslVersion::TLS1_3),
...
const DOMAIN_CONFIG_CHAT_STAGING: DomainConfig = DomainConfig {
    ...
    connect: ConnectionConfig {
        hostname: "chat.staging.zonarosa.io",
        port: DEFAULT_HTTPS_PORT,
        cert: ZONAROSA_ROOT_CERTIFICATES,
        min_tls_version: Some(SslVersion::TLS1_3),
        http_version: Some(HttpVersion::Http1_1),
...
const DOMAIN_CONFIG_EXPERIMENTAL_CHAT_H2_STAGING: DomainConfig = DomainConfig {
    ...
    ip_v6: &[],
    connect: ConnectionConfig {
        hostname: "grpc.chat.staging.zonarosa.io",
        port: DEFAULT_HTTPS_PORT,
        cert: ZONAROSA_ROOT_CERTIFICATES,
        min_tls_version: Some(SslVersion::TLS1_3),
        http_version: Some(HttpVersion::Http2),
...
const DOMAIN_CONFIG_CDSI_STAGING: DomainConfig = DomainConfig {
    connect: ConnectionConfig {
        hostname: "cdsi.staging.zonarosa.io",
        port: DEFAULT_HTTPS_PORT,
        cert: ZONAROSA_ROOT_CERTIFICATES,
        min_tls_version: Some(SslVersion::TLS1_3),
        http_version: Some(HttpVersion::Http1_1),
...
const DOMAIN_CONFIG_SVR2_STAGING: DomainConfig = DomainConfig {
    connect: ConnectionConfig {
        hostname: "svr2.staging.zonarosa.io",
        port: DEFAULT_HTTPS_PORT,
        cert: ZONAROSA_ROOT_CERTIFICATES,
        min_tls_version: Some(SslVersion::TLS1_3),
        http_version: Some(HttpVersion::Http1_1),
...
const DOMAIN_CONFIG_SVRB_STAGING: DomainConfig = DomainConfig {
    connect: ConnectionConfig {
        hostname: "svrb.staging.zonarosa.io",
        port: DEFAULT_HTTPS_PORT,
        cert: ZONAROSA_ROOT_CERTIFICATES,
        min_tls_version: Some(SslVersion::TLS1_3),
        http_version: Some(HttpVersion::Http1_1),
...
pub const PROXY_CONFIG_F_PROD: ProxyConfig = ProxyConfig {
    ...
    http_host: "reflector-zonarosa.global.ssl.fastly.net",
    sni_list: &[
...
pub const PROXY_CONFIG_F_STAGING: ProxyConfig = ProxyConfig {
    route_type: RouteType::ProxyF,
    http_host: "reflector-staging-zonarosa.global.ssl.fastly.net",
    sni_list: &[
        "github.githubassets.com",
        "pinterest.com",
        "www.redditstatic.com",
...
/// Configuration for a target network resource, like `chat.zonarosa.io`.
#[derive(Clone)]
...
impl UserAgent {
    pub fn with_libzonarosa_version(user_agent: &str) -> Self {
        let with_lib_version = format!("{} libzonarosa/{}", user_agent, libzonarosa_core::VERSION);
...
mod test {
    ...
    use libzonarosa_net_infra::Alpn;
    use libzonarosa_net_infra::dns::build_custom_resolver_cloudflare_doh;
    use libzonarosa_net_infra::dns::dns_lookup::DnsLookupRequest;
    use libzonarosa_net_infra::route::testutils::FakeContext;
    use libzonarosa_net_infra::route::{
        HttpRouteFragment, HttpsTlsRoute, RouteProvider as _, TcpRoute, TlsRoute, TlsRouteFragment,
        UnresolvedHost,
    };
    use libzonarosa_net_infra::utils::no_network_change_events;
    ...
    async fn live_resolve_eq_static_resolution(config: &DomainConfig) {
        if std::env::var("LIBZONAROSA_TESTING_RUN_NONHERMETIC_TESTS").is_err() {
