...
use hmac::{Hmac, Mac};
use libzonarosa_account_keys::{
    BackupForwardSecrecyEncryptionKey, BackupForwardSecrecyToken, BackupKey,
};
use libzonarosa_net_infra::errors::{LogSafeDisplay, RetryLater};
use libzonarosa_net_infra::ws::attested::AttestedConnectionError;
use libzonarosa_net_infra::ws::{WebSocketConnectError, WebSocketError};
use libzonarosa_svrb::proto::backup_metadata;
use libzonarosa_svrb::{Backup4, Secret};
use protobuf::Message;
use rand::rngs::OsRng;
use rand::{CryptoRng, Rng, TryRngCore};
use sha2::Sha256;
use zonarosa_crypto::Aes256Ctr32;
...
/// - libzonarosa_svrb::Error for the errors originating in the PPSS implementation. Most of them are
///   unlikely due to the way higher level APIs invoke the lower-level primitives from
///   libzonarosa_svrb.
/// - libzonarosa_net::svr::Error for network related errors.
...
pub enum Error {
    ...
    MetadataInvalid,
    /// Decryption error: {0}
    DecryptionError(#[from] zonarosa_crypto::DecryptionError),
}

impl From<libzonarosa_svrb::Error> for Error {
    fn from(err: libzonarosa_svrb::Error) -> Self {
        use libzonarosa_svrb::Error as LogicError;
        match err {
            LogicError::RestoreFailed(tries_remaining) => Self::RestoreFailed(tries_remaining),
            LogicError::BadResponseStatus4(libzonarosa_svrb::V4Status::MISSING) => Self::DataMissing,
            LogicError::BadData
...
impl Error {
    ...
    fn prioritize_error(first: Self, second: Self) -> Self {
        match (first, second) {
            ...
            // These are sorted by "errors that indicate issues that ZonaRosa is responsible for"...
...
fn aes_256_ctr_hmacsha256_decrypt(
    ...
    iv: &[u8; IV_SIZE],
    ctext: &[u8],
) -> Result<Vec<u8>, zonarosa_crypto::DecryptionError> {
    let (ctext, their_mac) = ctext
        .split_last_chunk::<HMAC_SHA256_TRUNCATED_BYTES>()
        .ok_or(zonarosa_crypto::DecryptionError::BadCiphertext(
        ...
        .into()
    {
        ...
        Err(zonarosa_crypto::DecryptionError::BadCiphertext(
...
> {
    ...
    let token = aes_256_ctr_hmacsha256_decrypt(&encryption_key, iv, &pair.ct)?
        .try_into()
        .map_err(|_| zonarosa_crypto::DecryptionError::BadCiphertext("should decrypt to 32 bytes"))?;
...
mod test {
    ...
    use libzonarosa_account_keys::{AccountEntropyPool, BackupKey};
    use libzonarosa_svrb::{Backup4, Secret};
    ...
    fn aes_roundtrip() -> Result<(), Error> {
        ...
        assert!(matches!(
            ...
            zonarosa_crypto::DecryptionError::BadCiphertext("MAC verification failed")
        ));
        Ok(())
    }
    ...
    fn proptest_latest_backup_is_always_restored() {
        ...
        proptest!(|(actions in proptest::collection::vec(Action::arbitrary(), ..20))| {
            ...
            rt.block_on(async {
                ...
                // irreplaceable in ZonaRosa yet anyway!
                scenario.complete_one_successful_backup().await;
