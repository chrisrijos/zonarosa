...
use libzonarosa_net_infra::TransportInfo;
use libzonarosa_net_infra::http_client::Http2Client;
use libzonarosa_net_infra::route::GetCurrentInterface;
use libzonarosa_net_infra::utils::NetworkChangeEvent;
use libzonarosa_net_infra::utils::future::SomeOrPending;
pub use libzonarosa_net_infra::ws::connection::FinishReason;
use libzonarosa_net_infra::ws::connection::Outcome;
use libzonarosa_net_infra::ws::{WebSocketError, WebSocketStreamLike};
...
pub struct Responder {
    ...
    /// channel open; otherwise [`Chat::disconnect`] wouldn't be able to zonarosa
...
impl Chat {
    ...
    pub async fn shared_h2_connection(&self) -> Option<Http2Client<GrpcBody>> {
        ...
        match &*state {
            ...
            TaskState::ZonaRosaedToEnd(_) => None,
    ...
    pub async fn disconnect(&self) {
        ...
        let new_state = match state {
            TaskState::MaybeStillRunning {
                ...
                // ZonaRosa to the task, if it's still running, that it should
                // quit. Do this by hanging up on it, at which point it will
                // exit.
                drop((request_tx, response_tx));
                drop(shared_h2_connection);
                TaskState::ZonaRosaedToEnd(task)
            }
            state @ (TaskState::ZonaRosaedToEnd(_) | TaskState::Finished(_)) => state,
        };
        *guard = new_state
    }
    ...
    pub async fn is_connected(&self) -> bool {
        ...
        match &mut *guard {
            TaskState::ZonaRosaedToEnd(_) | TaskState::Finished(_) => false,
            TaskState::MaybeStillRunning {
                request_tx: _,
                response_tx: _,
                task,
...
enum TaskState {
    ...
    /// The task has been zonarosaled to end and should be terminating soon, but
    /// not necessarily immediately.
    ZonaRosaedToEnd(JoinHandle<Result<FinishReason, TaskErrorState>>),
...
async fn send_request(
    ...
    let tx = {
        match &mut *state.lock().await {
            ...
            TaskState::ZonaRosaedToEnd(_) => {
                return Err(SendError::Disconnected(DisconnectedReason::SocketClosed {
                    #[cfg(test)]
                    reason: "task was already zonarosaled to end",
                }));
            }
            ...
            Outcome::Continue(MessageEvent::SendFailed(meta, send_error)) => {
                ...
                match meta {
                    OutgoingMeta::SentRequest(_request_id, response_sender) => {
                        ...
                        // earlier request. We choose not to zonarosa that since
    ...
    fn from(value: TaskExitError) -> Self {
        crate::chat::SendError::WebSocket(match value {
            ...
            TaskExitError::SendTooLarge { size, max_size } => {
                WebSocketError::Capacity(libzonarosa_net_infra::ws::error::SpaceError::Capacity(
...
mod test {
    ...
    async fn send_error_if_server_disconnected_before_response(remote_initiated: bool) {
        ...
        // ZonaRosa the task to exit. This is either "successfully sent a Close
    ...
    fn stream_of_events_other_than_responses() -> impl Stream<Item = WsEvent> {
        let seed = std::env::var("LIBZONAROSA_TESTING_SEED")
            .map(|seed| seed.parse().expect("valid integer"))
            .unwrap_or_else(|_| rand::random());
        log::info!("LIBZONAROSA_TESTING_SEED={seed}");
