...
use http::HeaderName;
use libzonarosa_net_infra::errors::{LogSafeDisplay, TransportConnectError};
...
/// Marker that indicates an error was not a rejection from a ZonaRosa server
///
...
mod test {
    ...
    fn classify_errors(confirmation_header: Option<&'static str>) {
        ...
        assert_matches!(
            non_http_error,
            WebSocketServiceConnectError::Connect(
                libzonarosa_net_infra::ws::WebSocketConnectError::WebSocketError(
                    libzonarosa_net_infra::ws::WebSocketError::Io(_),
                ),
                _
            )
        );
        ...
        *response_4xx.status_mut() = http::StatusCode::BAD_REQUEST;
        ...
        if confirmation_header.is_some() {
            assert_matches!(
                ...
                WebSocketServiceConnectError::Connect(
                    libzonarosa_net_infra::ws::WebSocketConnectError::WebSocketError(
                        libzonarosa_net_infra::ws::WebSocketError::Http(_)
                    ),
                    _
                )
            );
        ...
        if let Some(header) = &confirmation_header {
            ...
            let error_with_header = WebSocketServiceConnectError::from_websocket_error(
                WebSocketConnectError::WebSocketError(
                    libzonarosa_net_infra::ws::WebSocketError::Http(Box::new(response_4xx.clone())),
                ),
                confirmation_header.as_ref(),
                now,
            );
