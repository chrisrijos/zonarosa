...
pub struct ResettingConnectionOutcomes<R> {
    ...
    reset_zonarosa: NetworkChangeEvent,
}

impl<R> ResettingConnectionOutcomes<R> {
    pub fn new(outcomes: ConnectionOutcomes<R>, reset_zonarosa: &NetworkChangeEvent) -> Self {
        Self {
            outcomes: Some(outcomes),
            reset_zonarosa: reset_zonarosa.clone(),
        }
    }
}
...
impl<R: Eq + Hash> RouteDelayPolicy<R> for ResettingConnectionOutcomes<R> {
    ...
    async fn wants_recalculation(&mut self) {
        ...
        match self.reset_zonarosa.changed().await {
            Ok(()) => {}
            Err(_) => {
                // If the sender for the reset zonarosa dropped, we'll never want recalculation.
                () = std::future::pending().await;
            }
        }
        self.outcomes = None;
