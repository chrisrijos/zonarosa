...
use boring_zonarosa::ssl::SslRef;
use http::{HeaderName, HeaderValue};
use tokio_boring_zonarosa::HandshakeError;
...
#[derive(Debug)]
pub struct SslErrorReasons(boring_zonarosa::error::ErrorStack);
...
pub enum FailedHandshakeReason {
    Io(std::io::ErrorKind),
    Cert {
        error: boring_zonarosa::x509::X509VerifyError,
        cert_hashes: Vec<X509CertSha256>,
    },
    OtherBoring(boring_zonarosa::ssl::ErrorCode),
}
...
static_assertions::assert_impl_all!(boring_zonarosa::x509::X509VerifyError: Copy);
static_assertions::assert_impl_all!(boring_zonarosa::ssl::ErrorCode: Copy);
...
impl FailedHandshakeReason {
    ...
    pub fn is_possible_captive_network(&self) -> bool {
        matches!(
            self,
            Self::Cert {
                error: boring_zonarosa::x509::X509VerifyError::SELF_SIGNED_CERT_IN_CHAIN
                    | boring_zonarosa::x509::X509VerifyError::DEPTH_ZERO_SELF_SIGNED_CERT,
                cert_hashes: _,
            }
        )
    }
    ...
    ssl.peer_cert_chain()
        .map(|chain| {
            chain
                .iter()
                .map(|cert| {
                    cert.digest(boring_zonarosa::hash::MessageDigest::sha256())
                        .ok()
                        .map(|digest| {
...
impl<S> From<HandshakeError<S>> for FailedHandshakeReason {
    fn from(value: HandshakeError<S>) -> Self {
        ...
        let code = value.code().unwrap_or(boring_zonarosa::ssl::ErrorCode::NONE);
        ...
        if code == boring_zonarosa::ssl::ErrorCode::SSL
            && let Some(cert_error_code) = value.ssl().and_then(|ssl| ssl.verify_result().err())
            && cert_error_code != boring_zonarosa::x509::X509VerifyError::INVALID_CALL
        {
...
impl Display for FailedHandshakeReason {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::OtherBoring(boring_zonarosa::ssl::ErrorCode::NONE) => write!(f, "unknown error"),
...
impl From<boring_zonarosa::error::ErrorStack> for TransportConnectError {
    fn from(value: boring_zonarosa::error::ErrorStack) -> Self {
