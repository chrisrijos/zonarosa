...
use boring_zonarosa::ssl::{ConnectConfiguration, SslConnector, SslMethod, SslSignatureAlgorithm};
...
{
    type Connection = tokio_boring_zonarosa::SslStream<Inner>;
    ...
    fn connect_over(
        ...
        async move {
            ...
            tokio_boring_zonarosa::connect(ssl_config, &domain, inner)
                .await
...
fn ssl_config(
    ...
    host: Host<&str>,
    alpn: Option<Alpn>,
    min_required_tls_version: Option<boring_zonarosa::ssl::SslVersion>,
...
pub(crate) mod testutil {
    ...
    pub(crate) const SERVER_HOSTNAME: &str = "test-server.zonarosa.io.local";
            ...
            + 'static,
    {
        ...
        let listener_task = async move {
            ...
            let private_key = boring_zonarosa::pkey::PKey::private_key_from_der(
                SERVER_CERTIFICATE.signing_key.serialized_der(),
            )
            .expect("valid key");
            let cert = boring_zonarosa::x509::X509::from_der(SERVER_CERTIFICATE.cert.der())
            ...
            loop {
                ...
                let mut tls_acceptor = boring_zonarosa::ssl::SslAcceptor::mozilla_modern(
                    boring_zonarosa::ssl::SslMethod::tls(),
                )
                .expect("can build");
                tls_acceptor
                ...
                tls_acceptor.set_alpn_select_callback(move |_, client| {
                    boring_zonarosa::ssl::select_next_proto(supported_alpn_encoded, client)
                        .ok_or(boring_zonarosa::ssl::AlpnError::ALERT_FATAL)
                });
                let tls_acceptor = tls_acceptor.build();

                let stream = match tokio_boring_zonarosa::accept(&tls_acceptor, stream).await {
