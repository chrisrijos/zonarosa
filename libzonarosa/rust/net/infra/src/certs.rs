...
use boring_zonarosa::error::ErrorStack;
use boring_zonarosa::ssl::{SslAlert, SslConnectorBuilder, SslVerifyMode};
use boring_zonarosa::x509::X509;
use boring_zonarosa::x509::store::X509StoreBuilder;
...
impl RootCertificates {
    ...
    /// blocking task thread). Moreover, when using the resulting [`Ssl`](boring_zonarosa::ssl::Ssl)
...
fn set_up_platform_verifier(
    ...
    connector.set_async_custom_verify_callback(SslVerifyMode::PEER, move |ssl| {
        ...
        let intermediates: Vec<CertificateDer<'static>> = cert_chain
            .collect::<Result<_, boring_zonarosa::error::ErrorStack>>()
        ...
        Ok(Box::pin(async move {
            ...
            Ok(Box::new(|_: &mut _| Ok(())) as boring_zonarosa::ssl::BoxCustomVerifyFinish)
        }))
    });
...
mod test {
    ...
    use boring_zonarosa::ssl::{ErrorCode, SslConnector, SslMethod};
    ...
    async fn verify_certificate_via_rustls<V: LimitedServerCertVerifier + 'static>(
        ...
        let connection = tokio_boring_zonarosa::connect(
