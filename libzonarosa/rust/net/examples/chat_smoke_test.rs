...
use libzonarosa_net::connect_state::infer_proxy_mode_for_config;
use libzonarosa_net_infra::EnableDomainFronting;
use libzonarosa_net_infra::host::Host;
use libzonarosa_net_infra::route::{
    ConnectionProxyConfig, DirectOrProxyMode, ZONAROSA_TLS_PROXY_SCHEME,
};
...
async fn main() -> ExitCode {
    ...
    let snis = allowed_route_types.iter().flat_map(|route_type| {
        let (index, libzonarosa_net_type) = match route_type {
            RouteType::Direct => {
                return std::slice::from_ref(&env.chat_domain_config.connect.hostname);
            }
            RouteType::ProxyF => (0, libzonarosa_net_infra::RouteType::ProxyF),
            RouteType::ProxyG => (1, libzonarosa_net_infra::RouteType::ProxyG),
        };
        let config = &env
            .chat_domain_config
            .connect
    ...
    let proxy_mode = proxy_url.map_or(DirectOrProxyMode::DirectOnly, |url| {
        let url = Url::parse(&url)
            .inspect_err(|_| {
                log::warn!("did you mean to prefix with {ZONAROSA_TLS_PROXY_SCHEME}:// ?");
            })
...
async fn test_connection(
    env: &libzonarosa_net::env::Env<'static>,
    snis: HashSet<&str>,
