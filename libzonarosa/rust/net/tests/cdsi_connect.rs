...
async fn can_connect_to_cdsi_staging() {
    ...
    let Some(secret) = get_enclave_secret() else {
        println!(
            "LIBZONAROSA_TESTING_CDSI_ENCLAVE_SECRET environment variable is not set. The test will be ignored."
        );
        return;
    };
...
fn get_enclave_secret() -> Option<[u8; 32]> {
    std::env::var("LIBZONAROSA_TESTING_CDSI_ENCLAVE_SECRET")
