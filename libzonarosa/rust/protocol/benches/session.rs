...
pub fn session_encrypt_result(c: &mut Criterion) -> Result<(), ZonaRosaProtocolError> {
    ...
    c.bench_function(
        "decrypting a PreKeyZonaRosaMessage for an unknown session",
        |b| {
            b.iter(|| {
                let mut bob_store = bob_store.clone();
                support::decrypt(&mut bob_store, &alice_address, &message_to_decrypt)
...
pub fn session_encrypt_decrypt_result(c: &mut Criterion) -> Result<(), ZonaRosaProtocolError> {
    let (alice_session_record, bob_session_record) = support::initialize_sessions_v4()?;
