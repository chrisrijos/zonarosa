...
#[test]
fn test_server_cert() -> Result<(), ZonaRosaProtocolError> {
...
#[test]
fn test_revoked_server_cert() -> Result<(), ZonaRosaProtocolError> {
    let mut rng = OsRng.unwrap_err();
    let trust_root = KeyPair::generate(&mut rng);
    let server_key = KeyPair::generate(&mut rng);
...
#[test]
fn test_sender_cert() -> Result<(), ZonaRosaProtocolError> {
    let mut rng = OsRng.unwrap_err();
    let trust_root = KeyPair::generate(&mut rng);
    let server_key = KeyPair::generate(&mut rng);
    ...
    // Cfr. https://github.com/zonarosa/libzonarosa/pull/643
    ...
    for b in 0..sender_cert_bits {
        ...
        match cert {
            Ok(cert) => match cert.validate(&trust_root.public_key, expires) {
                ...
                Ok(false) => {}
                Err(ZonaRosaProtocolError::UnknownSealedSenderServerCertificateId(_)) => {}
                Err(unexpected_err) => {
...
#[test]
fn test_sealed_sender() -> Result<(), ZonaRosaProtocolError> {
    async {
        let mut rng = OsRng.unwrap_err();
        ...
        match bob_ptext {
            Err(ZonaRosaProtocolError::InvalidSealedSenderMessage(_)) => { /* ok */ }
            Err(err) => {
...
#[test]
fn test_sender_key_in_sealed_sender() -> Result<(), ZonaRosaProtocolError> {
    async {
        let mut rng = OsRng.unwrap_err();
...
#[test]
fn test_sealed_sender_multi_recipient() -> Result<(), ZonaRosaProtocolError> {
    async {
        let mut rng = OsRng.unwrap_err();
        ...
        // libzonarosa doesn't support encoding it, so we're going to hand-edit.
...
-> Result<(), ZonaRosaProtocolError> {
    async {
        ...
        let mut alice_store = support::test_in_memory_protocol_store()?;
        let mut bob_store =
            InMemZonaRosaProtocolStore::new(IdentityKeyPair::generate(&mut rng), 0x4000)?;
        ...
        {
            Ok(_) => panic!("should have failed"),
            Err(ZonaRosaProtocolError::InvalidRegistrationId(address, _id)) => {
...
#[test]
fn test_decryption_error_in_sealed_sender() -> Result<(), ZonaRosaProtocolError> {
    async {
        let mut rng = OsRng.unwrap_err();
        ...
        let original_ratchet_key = match bob_message {
            CiphertextMessage::PreKeyZonaRosaMessage(ref m) => m.message().sender_ratchet_key(),
            _ => panic!("without ACKs, every message should be a PreKeyZonaRosaMessage"),
        };
...
#[test]
fn test_sealed_sender_multi_recipient_redundant_empty_devices() -> Result<(), ZonaRosaProtocolError> {
    async {
