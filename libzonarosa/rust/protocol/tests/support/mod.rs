...
pub fn test_in_memory_protocol_store() -> Result<InMemZonaRosaProtocolStore, ZonaRosaProtocolError> {
...
pub async fn encrypt(
    store: &mut InMemZonaRosaProtocolStore,
    remote_address: &ProtocolAddress,
    msg: &str,
) -> Result<CiphertextMessage, ZonaRosaProtocolError> {
    let mut csprng = OsRng.unwrap_err();
    message_encrypt(
        msg.as_bytes(),
        remote_address,
...
) -> Result<Vec<u8>, ZonaRosaProtocolError> {
    let mut csprng = OsRng.unwrap_err();
    message_decrypt(
        msg,
        remote_address,
...
pub async fn create_pre_key_bundle<R: Rng + CryptoRng>(
    ...
    mut csprng: &mut R,
) -> Result<PreKeyBundle, ZonaRosaProtocolError> {
...
pub fn initialize_sessions_v4() -> Result<(SessionRecord, SessionRecord), ZonaRosaProtocolError> {
    let mut csprng = OsRng.unwrap_err();
    ...
    let alice_params = AliceZonaRosaProtocolParameters::new(
        alice_identity,
        alice_base_key,
    ...
    let bob_params = BobZonaRosaProtocolParameters::new(
        bob_identity,
        bob_base_key,
        None,
        bob_ephemeral_key,
...
pub struct TestStoreBuilder {
    ...
    pub(crate) store: InMemZonaRosaProtocolStore,
...
impl TestStoreBuilder {
    pub fn new() -> Self {
        ...
        let store = InMemZonaRosaProtocolStore::new(identity_key, registration_id as u32)
    ...
    pub fn from_store(store: &InMemZonaRosaProtocolStore) -> Self {
        Self {
...
pub trait HasSessionVersion {
    fn session_version(&self, address: &ProtocolAddress) -> Result<u32, ZonaRosaProtocolError>;
}
...
impl HasSessionVersion for InMemZonaRosaProtocolStore {
    fn session_version(&self, address: &ProtocolAddress) -> Result<u32, ZonaRosaProtocolError> {
