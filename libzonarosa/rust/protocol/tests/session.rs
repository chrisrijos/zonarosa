...
type TestResult = Result<(), ZonaRosaProtocolError>;
...
fn test_basic_prekey() -> TestResult {
        ...
        F: Fn(&mut TestStoreBuilder),
    {
        async {
            ...
            let incoming_message = CiphertextMessage::PreKeyZonaRosaMessage(
                PreKeyZonaRosaMessage::try_from(outgoing_message.serialize())?,
            );
            ...
            assert!(matches!(
                decrypt(&mut bob_store_builder.store, &alice_address, &outgoing_message)
                    .await
                    .unwrap_err(),
                ZonaRosaProtocolError::UntrustedIdentity(a) if a == alice_address
            ));
...
fn test_repeat_bundle_message() -> TestResult {
    ...
    fn run(
        ...
        async {
            ...
            let incoming_message = CiphertextMessage::PreKeyZonaRosaMessage(
                PreKeyZonaRosaMessage::try_from(outgoing_message1.serialize())?,
            );
            ...
            let incoming_message2 = CiphertextMessage::PreKeyZonaRosaMessage(
                PreKeyZonaRosaMessage::try_from(outgoing_message2.serialize())?,
            );
...
fn test_bad_message_bundle() -> TestResult {
    ...
    fn run(
        ...
        async {
            ...
            let incoming_message = CiphertextMessage::PreKeyZonaRosaMessage(
                PreKeyZonaRosaMessage::try_from(corrupted_message.as_slice())?,
            );
            ...
            let incoming_message = CiphertextMessage::PreKeyZonaRosaMessage(
                PreKeyZonaRosaMessage::try_from(outgoing_message.as_slice())?,
            );
...
fn test_message_key_limits() -> TestResult {
    ...
    fn run(sessions: (SessionRecord, SessionRecord)) -> TestResult {
        async {
            ...
            assert!(matches!(
                err,
                ZonaRosaProtocolError::DuplicatedMessage(2300, 5)
            ));
            Ok(())
        }
        .now_or_never()
    ...
    fn run(
        ...
        async {
            ...
            let alice_plaintext = decrypt(
                alice_store,
                &bob_address,
                &CiphertextMessage::PreKeyZonaRosaMessage(PreKeyZonaRosaMessage::try_from(
            ...
            let response_plaintext = decrypt(
                bob_store,
                &alice_address,
                &CiphertextMessage::ZonaRosaMessage(ZonaRosaMessage::try_from(
...
fn test_unacknowledged_sessions_eventually_expire() -> TestResult {
    async {
        ...
        .unwrap_err();
        assert!(
            matches!(&error, ZonaRosaProtocolError::SessionNotFound(addr) if addr == &bob_address),
            "{error:?}"
        );
...
fn prekey_message_failed_decryption_does_not_update_stores() -> TestResult {
    async {
        ...
        let pre_key_message = {
            ...
            .await;
            let message =
                assert_matches!(message, Ok(CiphertextMessage::PreKeyZonaRosaMessage(m)) => m);

            // Perturb the ciphertext so it doesn't decrypt successfully, but
            // don't touch anything else.
            let mut zonarosa_message = message.message().serialized().to_owned();
            let last_byte = zonarosa_message.last_mut().unwrap();
            *last_byte = last_byte.wrapping_add(1);

            PreKeyZonaRosaMessage::new(
                ...
                *message.identity_key(),
                (&*zonarosa_message).try_into().unwrap(),
            )
            .unwrap()
        };
        ...
        assert_matches!(
            decrypt(
                &mut alice_store,
                &bob_address,
                &CiphertextMessage::PreKeyZonaRosaMessage(pre_key_message),
            )
            .await,
            Err(ZonaRosaProtocolError::InvalidMessage(
                CiphertextMessageType::PreKey,
                "decryption failed"
            ))
        );
...
async fn run_interaction(
    alice_store: &mut InMemZonaRosaProtocolStore,
    alice_address: &ProtocolAddress,
    bob_store: &mut InMemZonaRosaProtocolStore,
...
fn test_signedprekey_not_saved() -> TestResult {
        ...
        F: Fn(&mut TestStoreBuilder),
    {
        async {
            ...
            let pksm_og2 = PreKeyZonaRosaMessage::try_from(outgoing_message2.serialize())?;
            ...
            let pksm_mal = PreKeyZonaRosaMessage::new(
            ...
            let bob_session_state_before = bob_store_builder
                ...
                .expect("session found")
                .serialize()?;
            let incoming_message = CiphertextMessage::PreKeyZonaRosaMessage(pksm_mal);
            assert_matches!(
                decrypt(
                    &mut bob_store_builder.store,
                    &alice_address,
                ...
                .await
                .expect_err("invalid"),
                ZonaRosaProtocolError::InvalidMessage(CiphertextMessageType::PreKey, _)
            );
...
async fn is_session_id_equal(
    ...
    bob_address: &ProtocolAddress,
) -> Result<bool, ZonaRosaProtocolError> {
    Ok(alice_store
        .load_session(bob_address)
        .await?
        .expect("session found")
...
fn test_duplicate_message_error_returned() -> TestResult {
    async {
        ...
        assert!(matches!(err, ZonaRosaProtocolError::DuplicatedMessage(_, _)));
        Ok(())
    }
    .now_or_never()
    .expect("sync")
...
fn test_pqr_state_and_message_contents_nonempty() -> TestResult {
    async {
        ...
        assert_matches!(&msg, CiphertextMessage::PreKeyZonaRosaMessage(m) if !m.message().pq_ratchet().is_empty());
        decrypt(bob_store, &alice_address, &msg).await?;

        let msg = encrypt(bob_store, &alice_address, "msg2").await?;
        assert_matches!(&msg, CiphertextMessage::ZonaRosaMessage(m) if !m.pq_ratchet().is_empty());
        decrypt(alice_store, &bob_address, &msg).await?;
...
fn x3dh_prekey_rejected_as_invalid_message_specifically() {
    async {
        ...
        let original =
            assert_matches!(pre_key_message, CiphertextMessage::PreKeyZonaRosaMessage(m) => m);
        let modified_message = PreKeyZonaRosaMessage::new(
        ...
        let err = support::decrypt(
            &mut bob_store_builder.store,
            &alice_address,
            &CiphertextMessage::PreKeyZonaRosaMessage(modified_message.clone()),
        )
        .await
        .expect_err("we changed the version, it should be rejected early");
        assert_matches!(
            err,
            ZonaRosaProtocolError::InvalidMessage(CiphertextMessageType::PreKey, msg)
...
fn prekey_message_sent_from_different_user_is_rejected() {
    async {
        ...
        assert_matches!(
            err,
            ZonaRosaProtocolError::InvalidMessage(CiphertextMessageType::PreKey, "reused base key")
        );
        assert!(
            bob_store
...
fn proptest_session_resets() {
    ...
    proptest!(|(actions in proptest::collection::vec(
        (proptest::bool::ANY, proptest_arbitrary_interop::arb::<libzonarosa_protocol_test_support::Event>()),
        ..40
    ))| {
        async {
            let mut csprng = rand::rngs::StdRng::seed_from_u64(0);

            let mut alice = libzonarosa_protocol_test_support::Participant::new(
                "alice",
                ProtocolAddress::new("+14151111111".to_owned(), DeviceId::new(1).unwrap()),
                &mut csprng,
            );
            let mut bob = libzonarosa_protocol_test_support::Participant::new(
                "bob",
                ProtocolAddress::new("+14151111112".to_owned(), DeviceId::new(1).unwrap()),
                &mut csprng,
            );
            ...
            async fn exchange_messages_until_agreement(
                attempts: usize,
                alice: &mut libzonarosa_protocol_test_support::Participant,
                bob: &mut libzonarosa_protocol_test_support::Participant,
