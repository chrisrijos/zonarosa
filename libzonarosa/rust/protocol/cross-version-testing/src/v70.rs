...
pub struct LibZonaRosaProtocolV70(InMemZonaRosaProtocolStore);

impl LibZonaRosaProtocolV70 {
    pub fn new() -> Self {
        ...
        Self(
            InMemZonaRosaProtocolStore::new(identity_key, registration_id as u32)
                .expect("can initialize"),
        )
    }
}

impl super::LibZonaRosaProtocolStore for LibZonaRosaProtocolV70 {
    ...
    fn process_pre_key_bundle(&mut self, remote: &str, pre_key_bundle: super::PreKeyBundle) {
        let pre_key_bundle = (|| {
            ...
            Ok::<_, libzonarosa_protocol_current::ZonaRosaProtocolError>(bundle)
        })()
    ...
    fn decrypt(
        ...
        match ConvertVersion::from_current(msg_type) {
            CiphertextMessageType::Whisper => message_decrypt_zonarosa(
                &ZonaRosaMessage::try_from(msg).expect("valid"),
                &address(remote),
                &mut self.0.session_store,
                &mut self.0.identity_store,
                &mut thread_rng(),
            ...
            CiphertextMessageType::PreKey => message_decrypt_prekey(
                &PreKeyZonaRosaMessage::try_from(msg).expect("valid"),
                &address(remote),
                &mut self.0.session_store,
                &mut self.0.identity_store,
    ...
    fn encrypt_sealed_sender_v1(
        &self,
        remote: &str,
        msg: &libzonarosa_protocol_current::UnidentifiedSenderMessageContent,
    ) -> Vec<u8> {
    ...
    fn decrypt_sealed_sender(
        &self,
        msg: &[u8],
    ) -> libzonarosa_protocol_current::UnidentifiedSenderMessageContent {
        let decrypted = sealed_sender_decrypt_to_usmc(msg, &self.0)
            .now_or_never()
            .expect("synchronous")
            .expect("can decrypt messages");
        libzonarosa_protocol_current::UnidentifiedSenderMessageContent::deserialize(
...
impl_convert_version!(
    PublicKey,
    libzonarosa_protocol_current::PublicKey as serializable
);
impl_convert_version!(
    IdentityKey,
    libzonarosa_protocol_current::IdentityKey as serializable
);
impl_convert_version!(
    kem::PublicKey,
    libzonarosa_protocol_current::kem::PublicKey as serializable
);
impl_convert_version!(
    kem::SecretKey,
    libzonarosa_protocol_current::kem::SecretKey as serializable
);
impl_convert_version!(DeviceId, libzonarosa_protocol_current::DeviceId as u32);
impl_convert_version!(PreKeyId, libzonarosa_protocol_current::PreKeyId as u32);
impl_convert_version!(
    SignedPreKeyId,
    libzonarosa_protocol_current::SignedPreKeyId as u32
);
impl_convert_version!(
    KyberPreKeyId,
    libzonarosa_protocol_current::KyberPreKeyId as u32
);

impl ConvertVersion for CiphertextMessageType {
    type Current = libzonarosa_protocol_current::CiphertextMessageType;
