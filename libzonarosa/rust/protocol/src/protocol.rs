...
use crate::{
    IdentityKey, PrivateKey, PublicKey, Result, ZonaRosaProtocolError, Timestamp, kem, proto,
};
...
pub enum CiphertextMessage {
    ...
    PreKeyZonaRosaMessage(PreKeyZonaRosaMessage),
...
impl CiphertextMessage {
    pub fn message_type(&self) -> CiphertextMessageType {
        match self {
            CiphertextMessage::ZonaRosaMessage(_) => CiphertextMessageType::Whisper,
            CiphertextMessage::PreKeyZonaRosaMessage(_) => CiphertextMessageType::PreKey,
    ...
    pub fn serialize(&self) -> &[u8] {
        match self {
            CiphertextMessage::ZonaRosaMessage(x) => x.serialized(),
            CiphertextMessage::PreKeyZonaRosaMessage(x) => x.serialized(),
...
impl ZonaRosaMessage {
    ...
    pub fn new(
        ...
        let message = proto::wire::ZonaRosaMessage {
    ...
    fn compute_mac(
        ...
        if mac_key.len() != 32 {
            return Err(ZonaRosaProtocolError::InvalidMacKeyLength(mac_key.len()));
        }
...
impl AsRef<[u8]> for ZonaRosaMessage {
    fn as_ref(&self) -> &[u8] {
        &self.serialized
    }
}

impl TryFrom<&[u8]> for ZonaRosaMessage {
    type Error = ZonaRosaProtocolError;

    fn try_from(value: &[u8]) -> Result<Self> {
        if value.len() < ZonaRosaMessage::MAC_LENGTH + 1 {
            return Err(ZonaRosaProtocolError::CiphertextMessageTooShort(value.len()));
        }
        let message_version = value[0] >> 4;
        if message_version < CIPHERTEXT_MESSAGE_PRE_KYBER_VERSION {
            return Err(ZonaRosaProtocolError::LegacyCiphertextVersion(
                message_version,
            ));
        }
        if message_version > CIPHERTEXT_MESSAGE_CURRENT_VERSION {
            return Err(ZonaRosaProtocolError::UnrecognizedCiphertextVersion(
                message_version,
            ));
        }

        let proto_structure =
            proto::wire::ZonaRosaMessage::decode(&value[1..value.len() - ZonaRosaMessage::MAC_LENGTH])
                .map_err(|_| ZonaRosaProtocolError::InvalidProtobufEncoding)?;

        let sender_ratchet_key = proto_structure
            .ratchet_key
            .ok_or(ZonaRosaProtocolError::InvalidProtobufEncoding)?;
...
#[derive(Debug, Clone)]
pub struct PreKeyZonaRosaMessage {
    message_version: u8,
    registration_id: u32,
...
impl PreKeyZonaRosaMessage {
    pub fn new(
        ...
        let proto_message = proto::wire::PreKeyZonaRosaMessage {
    ...
    #[inline]
    pub fn message(&self) -> &ZonaRosaMessage {
        &self.message
    }
...
impl AsRef<[u8]> for PreKeyZonaRosaMessage {
    fn as_ref(&self) -> &[u8] {
        &self.serialized
    }
}

impl TryFrom<&[u8]> for PreKeyZonaRosaMessage {
    type Error = ZonaRosaProtocolError;
    ...
    fn try_from(value: &[u8]) -> Result<Self> {
        ...
        let proto_structure = proto::wire::PreKeyZonaRosaMessage::decode(&value[1..])
            .map_err(|_| ZonaRosaProtocolError::InvalidProtobufEncoding)?;
        ...
        let kyber_payload = match (
            ...
            (None, None) => {
                return Err(ZonaRosaProtocolError::InvalidMessage(
        ...
        Ok(PreKeyZonaRosaMessage {
            ...
            message: ZonaRosaMessage::try_from(message.as_ref())?,
            serialized: Box::from(value),
        })
    }
}
...
impl TryFrom<&[u8]> for SenderKeyDistributionMessage {
    ...
    fn try_from(value: &[u8]) -> Result<Self> {
        ...
        if chain_key.len() != 32 || signing_key.len() != 33 {
            return Err(ZonaRosaProtocolError::InvalidProtobufEncoding);
        }
...
impl TryFrom<&[u8]> for PlaintextContent {
    ...
    fn try_from(value: &[u8]) -> Result<Self> {
        if value.is_empty() {
            return Err(ZonaRosaProtocolError::CiphertextMessageTooShort(0));
        }
        if value[0] != Self::PLAINTEXT_CONTEXT_IDENTIFIER_BYTE {
            return Err(ZonaRosaProtocolError::UnrecognizedMessageVersion(
                value[0] as u32,
            ));
        }
        Ok(Self {
    ...
    pub fn for_original(
        ...
        let ratchet_key = match original_type {
            CiphertextMessageType::Whisper => {
                Some(*ZonaRosaMessage::try_from(original_bytes)?.sender_ratchet_key())
            }
            CiphertextMessageType::PreKey => Some(
                *PreKeyZonaRosaMessage::try_from(original_bytes)?
                    .message()
                    .sender_ratchet_key(),
            ),
            CiphertextMessageType::SenderKey => None,
            CiphertextMessageType::Plaintext => {
                return Err(ZonaRosaProtocolError::InvalidArgument(
...
pub fn extract_decryption_error_message_from_serialized_content(
    ...
    content
        ...
        .as_deref()
        .ok_or_else(|| {
            ZonaRosaProtocolError::InvalidArgument(
...
mod tests {
    ...
    fn create_zonarosa_message<T>(csprng: &mut T) -> Result<ZonaRosaMessage>
    where
        T: Rng + CryptoRng,
    {
    ...
    fn assert_zonarosa_message_equals(m1: &ZonaRosaMessage, m2: &ZonaRosaMessage) {
    ...
    #[test]
    fn test_zonarosa_message_serialize_deserialize() -> Result<()> {
        let mut csprng = OsRng.unwrap_err();
        let message = create_zonarosa_message(&mut csprng)?;
        let deser_message =
            ZonaRosaMessage::try_from(message.as_ref()).expect("should deserialize without error");
        assert_zonarosa_message_equals(&message, &deser_message);
        Ok(())
    }

    #[test]
    fn test_pre_key_zonarosa_message_serialize_deserialize() -> Result<()> {
        let mut csprng = OsRng.unwrap_err();
        let identity_key_pair = KeyPair::generate(&mut csprng);
        let base_key_pair = KeyPair::generate(&mut csprng);
        let message = create_zonarosa_message(&mut csprng)?;
        let pre_key_zonarosa_message = PreKeyZonaRosaMessage::new(
            3,
            365,
            None,
            97.into(),
        ...
        let deser_pre_key_zonarosa_message =
            PreKeyZonaRosaMessage::try_from(pre_key_zonarosa_message.as_ref())
                .expect("should deserialize without error");
        assert_eq!(
            pre_key_zonarosa_message.message_version,
            deser_pre_key_zonarosa_message.message_version
        );
        assert_eq!(
            pre_key_zonarosa_message.registration_id,
            deser_pre_key_zonarosa_message.registration_id
        );
        assert_eq!(
            pre_key_zonarosa_message.pre_key_id,
            deser_pre_key_zonarosa_message.pre_key_id
        );
        assert_eq!(
            pre_key_zonarosa_message.signed_pre_key_id,
            deser_pre_key_zonarosa_message.signed_pre_key_id
        );
        assert_eq!(
            pre_key_zonarosa_message.base_key,
            deser_pre_key_zonarosa_message.base_key
        );
        assert_eq!(
            pre_key_zonarosa_message.identity_key.public_key(),
            deser_pre_key_zonarosa_message.identity_key.public_key()
        );
        assert_zonarosa_message_equals(
            &pre_key_zonarosa_message.message,
            &deser_pre_key_zonarosa_message.message,
        );
        assert_eq!(
            pre_key_zonarosa_message.serialized,
            deser_pre_key_zonarosa_message.serialized
        );
        Ok(())
    }
    ...
    fn test_decryption_error_message() -> Result<()> {
...

        {
            ...
            assert_eq!(
                error_message.ratchet_key(),
                Some(pre_key_zonarosa_message.message().sender_ratchet_key())
            );
            assert_eq!(error_message.timestamp(), timestamp);
            assert_eq!(error_message.device_id(), device_id);
        }
    ...
    fn test_decryption_error_message_for_plaintext() {
        assert!(matches!(
            ...
            Err(ZonaRosaProtocolError::InvalidArgument(_))
        ));
    }
}
