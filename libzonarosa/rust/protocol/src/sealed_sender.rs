...
use crate::{
    ...
    IdentityKeyStore, KeyPair, KyberPreKeyStore, PreKeyZonaRosaMessage, PreKeyStore, PrivateKey,
    ProtocolAddress, PublicKey, Result, ServiceId, ServiceIdFixedWidthBinaryBytes, SessionRecord,
    SessionStore, ZonaRosaMessage, ZonaRosaProtocolError, SignedPreKeyStore, Timestamp, crypto,
    ...
    pub fn signer(&self) -> Result<&ServerCertificate> {
        ...
        match &self.signer {
            ...
            SenderCertificateSigner::Reference(id) => CERT_MAP
                ...
                .ok_or_else(|| ZonaRosaProtocolError::UnknownSealedSenderServerCertificateId(*id)),
        }
    }
...
impl<'a> UnidentifiedSenderMessage<'a> {
    fn deserialize(data: &'a [u8]) -> Result<Self> {
        let (version_byte, remaining) = data.split_first().ok_or_else(|| {
            ZonaRosaProtocolError::InvalidSealedSenderMessage("Message was empty".to_owned())
        })?;
        ...
        match version {
            ...
            _ => Err(ZonaRosaProtocolError::UnknownSealedSenderVersion(version)),
        }
    }
}
...
mod sealed_sender_v1 {
    ...
    use libzonarosa_core::derive_arrays;
...
/// This method implements the single-key single-recipient [KEM] described in [this ZonaRosa blog
/// post], a.k.a. Sealed Sender v1.
///
/// [KEM]: https://en.wikipedia.org/wiki/Key_encapsulation
/// [this ZonaRosa blog post]: https://zonarosa.io/blog/sealed-sender/
///
...
/// The KEM scheme implemented by this method is described in [this ZonaRosa blog post]. The
...
pub async fn sealed_sender_encrypt_from_usmc<R: Rng + CryptoRng>(
    ...
    let their_identity = identity_store
        ...
        .await?
        .ok_or_else(|| ZonaRosaProtocolError::SessionNotFound(destination.clone()))?;
...
///    [`PreKeyZonaRosaMessage`] is received.
...
/// ```
///
/// Each individual Sealed Sender message received from the server is decoded in the ZonaRosa client
            ...
            let their_identity =
                identity_store
                    ...
                    .ok_or_else(|| {
                        ...
                        ZonaRosaProtocolError::SessionNotFound(destination.clone())
                    })?;
    ...
    let serialize_recipient_destinations_into = |serialized: &mut Vec<u8>,
     ...
     -> Result<()> {
        let their_service_id = ServiceId::parse_from_service_id_string(destinations[0].name())
            .ok_or_else(|| {
                ZonaRosaProtocolError::InvalidArgument(format!(
        ...
        while let Some((&destination, session)) = destinations_and_sessions.next() {
            ...
            {
                return Err(ZonaRosaProtocolError::InvalidRegistrationId(
                    destination.clone(),
                    their_registration_id,
                ));
            }
...
impl<'a> SealedSenderV2SentMessage<'a> {
    ...
    pub fn parse(data: &'a [u8]) -> Result<Self> {
        if data.is_empty() {
            return Err(ZonaRosaProtocolError::InvalidSealedSenderMessage(
        ...
        if !matches!(
            ...
            return Err(ZonaRosaProtocolError::UnknownSealedSenderVersion(version));
        }
    ...
    match UnidentifiedSenderMessage::deserialize(ciphertext)? {
        ...
        UnidentifiedSenderMessage::V2 {
            ...
            Aes256GcmSiv::new(&keys.derive_k().into())
                ...
                .map_err(|err| {
                    ZonaRosaProtocolError::InvalidSealedSenderMessage(format!(
...
pub async fn sealed_sender_decrypt(
    ...
    if (is_local_e164 || is_local_uuid) && usmc.sender()?.sender_device_id()? == local_device_id {
        return Err(ZonaRosaProtocolError::SealedSenderSelfSend);
    }
    ...
    let message = match usmc.msg_type()? {
        CiphertextMessageType::Whisper => {
            let ctext = ZonaRosaMessage::try_from(usmc.contents()?)?;
            session_cipher::message_decrypt_zonarosa(
                &ctext,
                &remote_address,
                session_store,
                identity_store,
        ...
        CiphertextMessageType::PreKey => {
            let ctext = PreKeyZonaRosaMessage::try_from(usmc.contents()?)?;
