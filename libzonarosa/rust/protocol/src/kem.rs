...
use crate::{Result, ZonaRosaProtocolError};
...
/// Each KEM supported by libzonarosa-protocol implements this trait.
///
...
impl<T: Parameters> DynParameters for T {
    ...
    fn encapsulate(
        ...
        Self::encapsulate(pub_key, csprng).map_err(|BadKEMKeyLength| {
            ZonaRosaProtocolError::BadKEMKeyLength(T::KEY_TYPE, pub_key.len())
        })
    }
    ...
    fn decapsulate(
        ...
        Self::decapsulate(secret_key, ciphertext).map_err(|e| match e {
            DecapsulateError::BadKeyLength => {
                ZonaRosaProtocolError::BadKEMKeyLength(T::KEY_TYPE, secret_key.len())
            }
            DecapsulateError::BadCiphertext => {
                ZonaRosaProtocolError::BadKEMCiphertextLength(T::KEY_TYPE, ciphertext.len())
            }
        })
    }
}
...
impl TryFrom<u8> for KeyType {
    ...
    fn try_from(x: u8) -> Result<Self> {
        match x {
            ...
            t => Err(ZonaRosaProtocolError::BadKEMKeyType(t)),
        }
    }
}
...
impl<T: KeyKind> Key<T> {
    ...
    pub fn deserialize(value: &[u8]) -> Result<Self> {
        if value.is_empty() {
            return Err(ZonaRosaProtocolError::NoKeyTypeIdentifier);
        }
        let key_type = KeyType::try_from(value[0])?;
        if value.len() != T::key_length(key_type) + 1 {
            return Err(ZonaRosaProtocolError::BadKEMKeyLength(key_type, value.len()));
        }
        Ok(Key {
            key_type,
...
impl Key<Secret> {
    ...
    pub fn decapsulate(&self, ct_bytes: &SerializedCiphertext) -> Result<Box<[u8]>> {
        ...
        if ct.key_type != self.key_type {
            return Err(ZonaRosaProtocolError::WrongKEMKeyType(
                ct.key_type.value(),
                self.key_type.value(),
            ));
        }
...
impl KeyPair {
    ...
    pub fn from_public_and_private(public_key: &[u8], secret_key: &[u8]) -> Result<Self> {
        ...
        if public_key.key_type != secret_key.key_type {
            Err(ZonaRosaProtocolError::WrongKEMKeyType(
...
impl<'a> Ciphertext<'a> {
    ...
    pub fn deserialize(value: &'a [u8]) -> Result<Self> {
        ...
        if value.len() != key_type.parameters().ciphertext_length() + 1 {
            return Err(ZonaRosaProtocolError::BadKEMCiphertextLength(
                key_type,
                value.len(),
            ));
        }
