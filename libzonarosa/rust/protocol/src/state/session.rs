...
use crate::{IdentityKey, KeyPair, PrivateKey, PublicKey, ZonaRosaProtocolError, consts, kem};
...
impl From<InvalidSessionError> for ZonaRosaProtocolError {
...
bitflags! {
    ...
    pub struct SessionUsabilityRequirements : u32 {
        ...
        /// - no more than a few weeks old (the exact time is chosen by libzonarosa)
        const NotStale = 1 << 0;
...
impl SessionRecord {
    ...
    pub fn deserialize(bytes: &[u8]) -> Result<Self, ZonaRosaProtocolError> {
    ...
    pub fn archive_current_state(&mut self) -> Result<(), ZonaRosaProtocolError> {
    ...
    pub fn serialize(&self) -> Result<Vec<u8>, ZonaRosaProtocolError> {
    ...
    pub fn remote_registration_id(&self) -> Result<u32, ZonaRosaProtocolError> {
        Ok(self
            .session_state()
            .ok_or_else(|| {
    ...
    pub fn local_registration_id(&self) -> Result<u32, ZonaRosaProtocolError> {
        Ok(self
            .session_state()
            .ok_or_else(|| {
                ZonaRosaProtocolError::InvalidState(
    ...
    pub fn session_version(&self) -> Result<u32, ZonaRosaProtocolError> {
        Ok(self
            .session_state()
            .ok_or_else(|| {
                ZonaRosaProtocolError::InvalidState("session_version", "No current session".into())
            })?
            .session_version()?)
    }

    pub fn local_identity_key_bytes(&self) -> Result<Vec<u8>, ZonaRosaProtocolError> {
        Ok(self
            .session_state()
            .ok_or_else(|| {
                ZonaRosaProtocolError::InvalidState(
    ...
    pub fn remote_identity_key_bytes(&self) -> Result<Option<Vec<u8>>, ZonaRosaProtocolError> {
        Ok(self
            .session_state()
            .ok_or_else(|| {
                ZonaRosaProtocolError::InvalidState(
    ...
    pub fn alice_base_key(&self) -> Result<&[u8], ZonaRosaProtocolError> {
        Ok(self
            .session_state()
            .ok_or_else(|| {
                ZonaRosaProtocolError::InvalidState("alice_base_key", "No current session".into())
            })?
            .alice_base_key())
    }
    ...
    pub fn get_receiver_chain_key_bytes(
        &self,
        sender: &PublicKey,
    ) -> Result<Option<Box<[u8]>>, ZonaRosaProtocolError> {
        Ok(self
            .session_state()
            .ok_or_else(|| {
                ZonaRosaProtocolError::InvalidState(
    ...
    pub fn get_sender_chain_key_bytes(&self) -> Result<Vec<u8>, ZonaRosaProtocolError> {
        Ok(self
            .session_state()
            .ok_or_else(|| {
                ZonaRosaProtocolError::InvalidState(
    ...
    pub fn get_kyber_ciphertext(&self) -> Result<Option<&Vec<u8>>, ZonaRosaProtocolError> {
        Ok(self
            .session_state()
            .ok_or_else(|| {
                ZonaRosaProtocolError::InvalidState(
