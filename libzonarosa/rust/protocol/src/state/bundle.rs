...
use crate::{DeviceId, IdentityKey, KyberPreKeyId, PublicKey, Result, ZonaRosaProtocolError, kem};
    ...
    fn try_from(content: PreKeyBundleContent) -> Result<Self> {
        PreKeyBundle::new(
            content.registration_id.ok_or_else(|| {
                ZonaRosaProtocolError::InvalidArgument("registration_id is required".to_string())
            })?,
            content.device_id.ok_or_else(|| {
                ZonaRosaProtocolError::InvalidArgument("device_id is required".to_string())
            })?,
            content
                .pre_key_id
                .and_then(|id| content.pre_key_public.map(|public| (id, public))),
            content.signed_pre_key_id.ok_or_else(|| {
                ZonaRosaProtocolError::InvalidArgument("signed_pre_key_id is required".to_string())
            })?,
            ...
            content.kyber_pre_key_id.ok_or_else(|| {
                ZonaRosaProtocolError::InvalidArgument("kyber_pre_key_id is required".to_string())
            })?,
            content.kyber_pre_key_public.ok_or_else(|| {
                ZonaRosaProtocolError::InvalidArgument("kyber_pre_key_public is required".to_string())
            })?,
            ...
            content.identity_key.ok_or_else(|| {
                ZonaRosaProtocolError::InvalidArgument("identity_key is required".to_string())
            })?,
        )
    }
}
