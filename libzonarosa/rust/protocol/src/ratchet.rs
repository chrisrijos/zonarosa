...
pub use self::params::{AliceZonaRosaProtocolParameters, BobZonaRosaProtocolParameters};
use crate::protocol::CIPHERTEXT_MESSAGE_CURRENT_VERSION;
use crate::state::SessionState;
use crate::{KeyPair, Result, SessionRecord, ZonaRosaProtocolError, consts};
...
pub(crate) fn initialize_alice_session<R: Rng + CryptoRng>(
    parameters: &AliceZonaRosaProtocolParameters,
    mut csprng: &mut R,
) -> Result<SessionState> {
...
pub(crate) fn initialize_bob_session(
    parameters: &BobZonaRosaProtocolParameters,
) -> Result<SessionState> {
