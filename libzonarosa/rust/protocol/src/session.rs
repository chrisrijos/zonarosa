...
use crate::ratchet::{AliceZonaRosaProtocolParameters, BobZonaRosaProtocolParameters};
use crate::state::GenericSignedPreKey;
use crate::{
    CiphertextMessageType, Direction, IdentityKey, IdentityKeyStore, KeyPair, KyberPreKeyId,
    KyberPreKeyStore, PreKeyBundle, PreKeyId, PreKeyZonaRosaMessage, PreKeyStore, ProtocolAddress,
    Result, SessionRecord, SessionStore, ZonaRosaProtocolError, SignedPreKeyId, SignedPreKeyStore,
    ratchet,
};
...
pub async fn process_prekey<'a>(
    message: &'a PreKeyZonaRosaMessage,
...
async fn process_prekey_impl(
    ...
    if message.message_version() == CIPHERTEXT_MESSAGE_PRE_KYBER_VERSION {
        // Specifically return InvalidMessage here rather than LegacyCiphertextVersion; the ZonaRosa
    ...
    let kyber_ciphertext =
        message
            .kyber_ciphertext()
            .ok_or(ZonaRosaProtocolError::InvalidMessage(
                CiphertextMessageType::PreKey,
    ...
    let parameters = BobZonaRosaProtocolParameters::new(
...
pub async fn process_prekey_bundle<R: Rng + CryptoRng>(
    ...
    let mut parameters = AliceZonaRosaProtocolParameters::new(
