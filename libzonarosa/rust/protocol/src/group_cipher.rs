...
use crate::{
    ...
    SenderKeyMessage, SenderKeyRecord, SenderKeyStore, ZonaRosaProtocolError, consts,
};
...
pub async fn group_encrypt<R: Rng + CryptoRng>(
    ...
    let mut record = sender_key_store
        ...
        .await?
        .ok_or(ZonaRosaProtocolError::NoSenderKeyState { distribution_id })?;

    let sender_key_state = record
        .sender_key_state_mut()
        .map_err(|_| ZonaRosaProtocolError::InvalidSenderKeySession { distribution_id })?;
    ...
    let sender_chain_key = sender_key_state
        .sender_chain_key()
        .ok_or(ZonaRosaProtocolError::InvalidSenderKeySession { distribution_id })?;

    let message_keys = sender_chain_key.sender_message_key();

    let ciphertext =
        zonarosa_crypto::aes_256_cbc_encrypt(plaintext, message_keys.cipher_key(), message_keys.iv())
            .map_err(|_| {
                log::error!(
...
fn get_sender_key(
    ...
    if current_iteration > iteration {
        if let Some(smk) = state.remove_sender_message_key(iteration) {
            ...
            return Err(ZonaRosaProtocolError::DuplicatedMessage(
                current_iteration,
                iteration,
            ));
        }
...
pub async fn group_decrypt(
    ...
    let sender_key_state = match record.sender_key_state_for_chain_id(chain_id) {
        ...
        None => {
            ...
            return Err(ZonaRosaProtocolError::NoSenderKeyState { distribution_id });
        }
    };
    ...
    if !skm.verify_signature(&signing_key)? {
        return Err(ZonaRosaProtocolError::SignatureValidationFailed);
    }

    let sender_key = get_sender_key(sender_key_state, skm.iteration(), distribution_id)?;

    let plaintext = match zonarosa_crypto::aes_256_cbc_decrypt(
        skm.ciphertext(),
        ...
        Err(zonarosa_crypto::DecryptionError::BadKeyOrIv) => {
            ...
            return Err(ZonaRosaProtocolError::InvalidSenderKeySession { distribution_id });
        }
...
pub async fn create_sender_key_distribution_message<R: Rng + CryptoRng>(
    ...
    let sender_key_record = match sender_key_record {
        Some(record) => record,
        None => {
            // libzonarosa-protocol-java uses 31-bit integers for sender key chain IDs
