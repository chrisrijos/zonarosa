...
use crate::{
    ...
    ZonaRosaProtocolError, SignedPreKeyId, SignedPreKeyRecord,
};
    ...
    pub fn load_existing_sessions(
        ...
        addresses
            ...
            .map(|&address| {
                self.sessions
                    .get(address)
                    .ok_or_else(|| ZonaRosaProtocolError::SessionNotFound(address.clone()))
            })
            .collect()
    }
}
...
#[allow(missing_docs)]
#[derive(Clone)]
pub struct InMemZonaRosaProtocolStore {
...
impl InMemZonaRosaProtocolStore {
...
#[async_trait(?Send)]
impl traits::IdentityKeyStore for InMemZonaRosaProtocolStore {
    async fn get_identity_key_pair(&self) -> Result<IdentityKeyPair> {
...
#[async_trait(?Send)]
impl traits::PreKeyStore for InMemZonaRosaProtocolStore {
    async fn get_pre_key(&self, id: PreKeyId) -> Result<PreKeyRecord> {
...
#[async_trait(?Send)]
impl traits::SignedPreKeyStore for InMemZonaRosaProtocolStore {
    async fn get_signed_pre_key(&self, id: SignedPreKeyId) -> Result<SignedPreKeyRecord> {
...
#[async_trait(?Send)]
impl traits::KyberPreKeyStore for InMemZonaRosaProtocolStore {
    async fn get_kyber_pre_key(&self, kyber_prekey_id: KyberPreKeyId) -> Result<KyberPreKeyRecord> {
...
#[async_trait(?Send)]
impl traits::SessionStore for InMemZonaRosaProtocolStore {
    async fn load_session(&self, address: &ProtocolAddress) -> Result<Option<SessionRecord>> {
...
#[async_trait(?Send)]
impl traits::SenderKeyStore for InMemZonaRosaProtocolStore {
    async fn store_sender_key(
        &mut self,
        sender: &ProtocolAddress,
        distribution_id: Uuid,
...
impl traits::ProtocolStore for InMemZonaRosaProtocolStore {}
