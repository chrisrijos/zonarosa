...
//! Traits defining several stores used throughout the ZonaRosa Protocol.
...
/// Each ZonaRosa message can be considered to have exactly two participants, a sender and receiver.
///
...
///
/// ZonaRosa clients usually use the identity store in a [TOFU] manner, but this is not required.
///
...
///
/// NB: libzonarosa makes no distinction between one-time and last-resort pre-keys.
#[async_trait(?Send)]
...
/// Interface for a ZonaRosa client instance to store a session associated with another particular
/// separate ZonaRosa client instance.
///
/// This [SessionRecord] object between a pair of ZonaRosa clients is used to drive the state for the
