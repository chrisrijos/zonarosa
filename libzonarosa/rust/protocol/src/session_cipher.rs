...
use crate::{
    ...
    KyberPreKeyStore, PreKeyZonaRosaMessage, PreKeyStore, ProtocolAddress, PublicKey, Result,
    SessionRecord, SessionStore, ZonaRosaMessage, ZonaRosaProtocolError, SignedPreKeyStore, session,
};
...
pub async fn message_encrypt<R: Rng + CryptoRng>(
    ...
    let mut session_record = session_store
        ...
        .await?
        .ok_or_else(|| ZonaRosaProtocolError::SessionNotFound(remote_address.clone()))?;
    ...
    let (pqr_msg, pqr_key) = session_state.pq_ratchet_send(csprng).map_err(|e| {
        ...
        ZonaRosaProtocolError::InvalidState(
            "message_encrypt",
    ...
    let session_version = session_state
        .session_version()?
        .try_into()
        .map_err(|_| ZonaRosaProtocolError::InvalidSessionStructure("version does not fit in u8"))?;
    ...
    let ctext =
        zonarosa_crypto::aes_256_cbc_encrypt(ptext, message_keys.cipher_key(), message_keys.iv())
            .map_err(|_| {
                log::error!("session state corrupt for {remote_address}");
                ZonaRosaProtocolError::InvalidSessionStructure("invalid sender chain message keys")
            })?;
    ...
    let message = if let Some(items) = session_state.unacknowledged_pre_key_message_items()? {
        ...
        if items.timestamp() + MAX_UNACKNOWLEDGED_SESSION_AGE < now {
            ...
            return Err(ZonaRosaProtocolError::SessionNotFound(remote_address.clone()));
        }
        ...
        let message = ZonaRosaMessage::new(
            session_version,
        ...
        CiphertextMessage::PreKeyZonaRosaMessage(PreKeyZonaRosaMessage::new(
            session_version,
        ...
        CiphertextMessage::ZonaRosaMessage(ZonaRosaMessage::new(
            session_version,
            message_keys.mac_key(),
            sender_ephemeral,
            chain_key.index(),
        ...
        .await?
    {
        ...
        return Err(ZonaRosaProtocolError::UntrustedIdentity(
...
pub async fn message_decrypt<R: Rng + CryptoRng>(
    ...
    match ciphertext {
        CiphertextMessage::ZonaRosaMessage(m) => {
            message_decrypt_zonarosa(m, remote_address, session_store, identity_store, csprng).await
        }
        ...
        _ => Err(ZonaRosaProtocolError::InvalidArgument(format!(
...
pub async fn message_decrypt_prekey<R: Rng + CryptoRng>(
    ciphertext: &PreKeyZonaRosaMessage,
    remote_address: &ProtocolAddress,
    session_store: &mut dyn SessionStore,
    identity_store: &mut dyn IdentityKeyStore,
    pre_key_store: &mut dyn PreKeyStore,
...
pub async fn message_decrypt_zonarosa<R: Rng + CryptoRng>(
    ciphertext: &ZonaRosaMessage,
    remote_address: &ProtocolAddress,
    session_store: &mut dyn SessionStore,
    identity_store: &mut dyn IdentityKeyStore,
    csprng: &mut R,
...
fn create_decryption_failure_log(
    remote_address: &ProtocolAddress,
    mut errs: &[ZonaRosaProtocolError],
    ...
    fn append_session_summary(
        ...
        err: Option<&ZonaRosaProtocolError>,
    ) {
...
fn decrypt_message_with_record<R: Rng + CryptoRng>(
    ...
    let log_decryption_failure = |state: &SessionState, error: &ZonaRosaProtocolError| {
    ...
    if let Some(current_state) = record.session_state() {
        ...
        match result {
            ...
            Err(ZonaRosaProtocolError::DuplicatedMessage(_, _)) => {
                return result;
            }
            Err(e) => {
...
fn decrypt_message_with_state<R: Rng + CryptoRng>(
    ...
    let pqr_key = state
        ...
        .map_err(|e| match e {
            spqr::Error::StateDecode => ZonaRosaProtocolError::InvalidState(
    ...
    let their_identity_key =
        state
            .remote_identity_key()?
            .ok_or(ZonaRosaProtocolError::InvalidSessionStructure(
    ...
    let ptext = match zonarosa_crypto::aes_256_cbc_decrypt(
        ciphertext.body(),
        message_keys.cipher_key(),
        message_keys.iv(),
    ) {
        Ok(ptext) => ptext,
        Err(zonarosa_crypto::DecryptionError::BadKeyOrIv) => {
            log::warn!("{current_or_previous} session state corrupt for {remote_address}",);
            return Err(ZonaRosaProtocolError::InvalidSessionStructure(
                "invalid receiver chain message keys",
            ));
        }
        Err(zonarosa_crypto::DecryptionError::BadCiphertext(msg)) => {
    ...
    if chain_index > counter {
        return match state.get_message_keys(their_ephemeral, counter)? {
            ...
            None => {
                ...
                Err(ZonaRosaProtocolError::DuplicatedMessage(chain_index, counter))
            }
        };
    }
