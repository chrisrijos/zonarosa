...
pub struct LocalState {
    store: InMemZonaRosaProtocolStore,
    pre_key_count: u32,
}
...
/// Represents a (single-device) ZonaRosa protocol user.
///
...
impl Participant {
    pub fn new(
        ...
        Self {
            ...
            state: LocalState {
                store: InMemZonaRosaProtocolStore::new(IdentityKeyPair::generate(rng), rng.random())
                    .unwrap(),
                pre_key_count: 0,
            },
            snapshots: Vec::new(),
    ...
    pub fn current_store(&self) -> &InMemZonaRosaProtocolStore {
        &self.state.store
    }
        ...
        them.state
            ...
            .save_signed_pre_key(
                ...
                &SignedPreKeyRecord::new(
                    signed_pre_key_id,
                    libzonarosa_protocol::Timestamp::from_epoch_millis(42),
    ...
    async fn send_message_with_id(
        ...
        let incoming_message = match outgoing_message.message_type() {
            CiphertextMessageType::PreKey => CiphertextMessage::PreKeyZonaRosaMessage(
                PreKeyZonaRosaMessage::try_from(outgoing_message.serialize()).unwrap(),
            ),
            CiphertextMessageType::Whisper => CiphertextMessage::ZonaRosaMessage(
                ZonaRosaMessage::try_from(outgoing_message.serialize()).unwrap(),
            ),
    ...
    pub async fn receive_messages(&mut self, them: &mut Self, rng: &mut (impl Rng + CryptoRng)) {
        ...
        for (incoming_message, expected) in self.message_queue.split_off(0) {
            match incoming_message {
                CiphertextMessage::ZonaRosaMessage(_) | CiphertextMessage::PreKeyZonaRosaMessage(_) => {
