//
// Copyright (C) 2023 ZonaRosa Platform.
...
use hkdf::Hkdf;
use libzonarosa_core::curve::PrivateKey;
use libzonarosa_core::{Aci, derive_arrays};
...
impl BackupKey<V1> {
    ...
    pub fn derive_from_account_entropy_pool(entropy: &AccountEntropyPool) -> Self {
        ...
        Hkdf::<Sha256>::new(None, &entropy.entropy_pool)
            .expand(b"20240801_ZONAROSA_BACKUP_KEY", &mut key)
            .expect("valid length");
        Self(key)
    }

    pub fn derive_ec_key(&self, aci: &Aci) -> PrivateKey {
        const INFO: &[u8] = b"20241024_ZONAROSA_BACKUP_ID_KEYPAIR:";
    ...
    pub fn derive_local_backup_metadata_key(&self) -> [u8; LOCAL_BACKUP_METADATA_KEY_LEN] {
        const INFO: &[u8] = b"20241011_ZONAROSA_LOCAL_BACKUP_METADATA_KEY";
    ...
    pub fn derive_media_id(&self, media_name: &str) -> [u8; MEDIA_ID_LEN] {
        const INFO: &[u8] = b"20241007_ZONAROSA_BACKUP_MEDIA_ID:";
    ...
    pub fn derive_media_encryption_key_data(
        ...
        const INFO: &[u8] = b"20241007_ZONAROSA_BACKUP_ENCRYPT_MEDIA:";
    ...
    pub fn derive_thumbnail_transit_encryption_key_data(
        ...
        const INFO: &[u8] = b"20241030_ZONAROSA_BACKUP_ENCRYPT_THUMBNAIL:";
        let mut bytes = [0; MEDIA_ENCRYPTION_KEY_LEN];
        Hkdf::<Sha256>::new(None, &self.0)
            .expand_multi_info(&[INFO, media_id], &mut bytes)
            .expect("valid length");
...
impl<const VERSION: u8> BackupKey<VERSION> {
    ...
    pub fn derive_backup_id(&self, aci: &Aci) -> BackupId {
        ...
        match VERSION {
            V1 => {
                ...
                // generation scheme.
                const INFO: &[u8] = b"20241024_ZONAROSA_BACKUP_ID:";
    ...
    pub fn derive_forward_secrecy_password(&self, salt: &[u8]) -> BackupForwardSecrecyPassword {
        let mut bytes = [0; 32];
        const INFO: &[u8] = b"ZonaRosa Message Backup 20250627:SVR PIN";
    ...
    pub fn derive_forward_secrecy_encryption_key(
        ...
        let (cipher_key, hmac_key, []) = derive_arrays(|bytes| {
            const INFO: &[u8] =
                b"ZonaRosa Message Backup 20250627:BackupForwardSecrecyToken Encryption Key";
            Hkdf::<Sha256>::new(Some(salt), &self.0)
                .expand(INFO, bytes)
                .expect("valid length");
        });
