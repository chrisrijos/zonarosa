...
fn benchmark_integration_auth(c: &mut Criterion) {
    ...
    let aci = libzonarosa_core::Aci::from_uuid_bytes(zkgroup::TEST_ARRAY_16);
    let pni = libzonarosa_core::Pni::from_uuid_bytes(zkgroup::TEST_ARRAY_16_1);
...
pub fn benchmark_group_send_endorsements(c: &mut Criterion) {
    ...
    let all_members: Vec<libzonarosa_core::ServiceId> = std::iter::once(aci)
        .chain((1u16..).map(|i| {
            // Generate arbitrary v5 (hash-based) UUIDs for the rest of the group.
            libzonarosa_core::Aci::from(uuid::Uuid::new_v5(
                &uuid::Uuid::from_bytes(zkgroup::TEST_ARRAY_16_1),
                &i.to_be_bytes(),
            ))
        }))
        .map(libzonarosa_core::ServiceId::from)
