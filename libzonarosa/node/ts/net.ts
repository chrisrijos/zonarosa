...
// This must match the libzonarosa-bridge Rust enum of the same name.
...
/** The "scheme" for ZonaRosa TLS proxies. See {@link Net.setProxy()}. */
export const ZONAROSA_TLS_PROXY_SCHEME = 'io.zonarosa.tls';
...
export class Net {
   ...
   * @param options.abortZonaRosa an {@link AbortZonaRosa} that will cancel the connection attempt.
   */
  public preconnectChat(options?: {
    abortZonaRosa?: AbortZonaRosa;
  }): Promise<void> {
   ...
   * ZonaRosa server will always consider the list to be in priority order.
   * @param options.abortZonaRosa an {@link AbortZonaRosa} that will cancel the connection attempt.
   */
  public async connectUnauthenticatedChat(
    listener: ConnectionEventsListener,
    options?: { languages?: string[]; abortZonaRosa?: AbortZonaRosa }
  ...
  public async connectProvisioning(
    ...
    options?: { abortZonaRosa?: AbortZonaRosa }
   ...
   *
   * If CC is enabled, *new* connections and services may try additional routes to the ZonaRosa
   ...
   *
   * To specify a ZonaRosa transparent TLS proxy, use {@link ZONAROSA_TLS_PROXY_SCHEME}, or the
  ...
  setProxy(
    ...
    if (typeof hostOrOptions === 'string') {
      ...
      hostOrOptions = {
        scheme: ZONAROSA_TLS_PROXY_SCHEME,
        host: domain,
        port: portOrNothing,
        username,
      };
  ...
  /**
   * Updates libzonarosa's remote configuration settings.
   *
   ...
   * - Keys must have had the platform-specific prefix (e.g., `"desktop.libzonarosa."`) removed.
   ...
   *
   * These constraints ensure configurations passed to libzonarosa precisely reflect enabled
   ...
   * @param remoteConfig A map containing preprocessed libzonarosa configuration keys and their associated values.
   */
  setRemoteConfig(
