...
import { ErrorCode, LibZonaRosaError, LibZonaRosaErrorBase } from '../Errors.js';
...
function assertThrowsLibZonaRosaError(
  expression: () => void,
  code: ErrorCode,
  message?: string
) {
  try {
    ...
    assert.instanceOf(e, LibZonaRosaErrorBase, message);
    const err = e as LibZonaRosaError;
...
describe('usernames', () => {
  ...
  describe('link', () => {
    it('works end to end with valid data', () => {
      const expectedUsername = 'zonarosa_test.42';
