...
it('Fingerprint', () => {
  const aliceKey = ZonaRosaClient.PublicKey.deserialize(
    Buffer.from(
  ...
  const bobKey = ZonaRosaClient.PublicKey.deserialize(
    Buffer.from(
  ...
  const aFprint1 = ZonaRosaClient.Fingerprint.new(
    iterations,
    1,
    aliceIdentifier,
    aliceKey,
  ...
  const bFprint1 = ZonaRosaClient.Fingerprint.new(
    iterations,
    1,
    bobIdentifier,
    bobKey,
...
it('DecryptionErrorMessage', async () => {
  ...
  await ZonaRosaClient.processPreKeyBundle(bPreKeyBundle, bAddress, aSess, aKeys);
  ...
  const bCiphertext = await ZonaRosaClient.zonarosaEncrypt(
  ...
  const errorMessage = ZonaRosaClient.DecryptionErrorMessage.forOriginal(
    bCiphertext.serialize(),
    bCiphertext.type(),
    45, // timestamp
    bAddress.deviceId()
  );
  const errorContent = ZonaRosaClient.PlaintextContent.from(errorMessage);
  const errorUSMC = ZonaRosaClient.UnidentifiedSenderMessageContent.new(
    ZonaRosaClient.CiphertextMessage.from(errorContent),
    senderCert,
    ZonaRosaClient.ContentHint.Implicit,
    null // group ID
  );
  const errorSealedSenderMessage = await ZonaRosaClient.sealedSenderEncrypt(
    errorUSMC,
    bAddress,
    aKeys
  );

  const bErrorUSMC = await ZonaRosaClient.sealedSenderDecryptToUsmc(
    errorSealedSenderMessage,
    bKeys
  );
  assert.equal(
    bErrorUSMC.msgType(),
    ZonaRosaClient.CiphertextMessageType.Plaintext
  );
  const bErrorContent = ZonaRosaClient.PlaintextContent.deserialize(
    bErrorUSMC.contents()
  );
  const bErrorMessage =
    ZonaRosaClient.DecryptionErrorMessage.extractFromSerializedBody(
      bErrorContent.body()
    );
...
it('SenderCertificate', () => {
  ...
  const serverCertFromBytes = ZonaRosaClient.ServerCertificate.deserialize(
  ...
  const senderKey = ZonaRosaClient.PrivateKey.generate();
  ...
  const senderCertFromBytes = ZonaRosaClient.SenderCertificate.deserialize(
  ...
  const senderCertWithoutE164 = ZonaRosaClient.SenderCertificate.new(
    senderUuid,
    null,
    senderDeviceId,
    senderKey.getPublicKey(),
...
it('SenderKeyMessage', () => {
  ...
  const pk = ZonaRosaClient.PrivateKey.generate();

  const skm = ZonaRosaClient.SenderKeyMessage._new(
    3,
    distributionId,
    chainId,
    iteration,
  ...
  const skmFromBytes = ZonaRosaClient.SenderKeyMessage.deserialize(
    skm.serialize()
  );
...
it('SenderKeyDistributionMessage', () => {
  ...
  const skdm = ZonaRosaClient.SenderKeyDistributionMessage._new(
    3,
    distributionId,
    chainId,
    iteration,
  ...
  const skdmFromBytes = ZonaRosaClient.SenderKeyDistributionMessage.deserialize(
    skdm.serialize()
  );
...
describe('SenderKeyDistributionMessage Store API', () => {
  it('can encrypt and decrypt', async () => {
    const sender = ZonaRosaClient.ProtocolAddress.new('sender', 1);
    const distributionId = 'd1d1d1d1-7000-11eb-b32a-33b8a8a487a6';
    ...
    const anotherSkdm = await ZonaRosaClient.SenderKeyDistributionMessage.create(
      sender,
      distributionId,
      aSenderKeyStore
    );
  ...
  it("does not panic if there's an error", async () => {
    ...
    const messagePromise = ZonaRosaClient.SenderKeyDistributionMessage.create(
      sender,
      distributionId,
      undefined as unknown as ZonaRosaClient.SenderKeyStore
    );
    await assert.isRejected(messagePromise, TypeError);

    const messagePromise2 = ZonaRosaClient.SenderKeyDistributionMessage.create(
      {} as unknown as ZonaRosaClient.ProtocolAddress,
      distributionId,
      aSenderKeyStore
    );
...
it('PublicKeyBundle Kyber', () => {
  const signingKey = ZonaRosaClient.PrivateKey.generate();
  const registrationId = 5;
  const deviceId = 23;
  const prekeyId = 42;
  const prekey = ZonaRosaClient.PrivateKey.generate().getPublicKey();
  const signedPrekeyId = 2300;
  const signedPrekey = ZonaRosaClient.PrivateKey.generate().getPublicKey();
  const signedPrekeySignature = signingKey.sign(signedPrekey.serialize());
  const identityKey = ZonaRosaClient.PrivateKey.generate().getPublicKey();
  const kyberPrekeyId = 8888;
  const kyberPrekey = ZonaRosaClient.KEMKeyPair.generate().getPublicKey();
  const kyberPrekeySignature = signingKey.sign(kyberPrekey.serialize());

  const pkb = ZonaRosaClient.PreKeyBundle.new(
    registrationId,
    deviceId,
    prekeyId,
    prekey,
  ...
  const pkb2 = ZonaRosaClient.PreKeyBundle.new(
    registrationId,
    deviceId,
    null,
    null,
...
it('PreKeyRecord', () => {
  const privKey = ZonaRosaClient.PrivateKey.generate();
  const pubKey = privKey.getPublicKey();
  const pkr = ZonaRosaClient.PreKeyRecord.new(23, pubKey, privKey);

  assert.deepEqual(pkr.id(), 23);
  assert.deepEqual(pkr.publicKey(), pubKey);
  assert.deepEqual(pkr.privateKey(), privKey);

  const pkr2 = ZonaRosaClient.PreKeyRecord.deserialize(pkr.serialize());
...
it('SignedPreKeyRecord', () => {
  ...
  const spkr = ZonaRosaClient.SignedPreKeyRecord.new(
    keyId,
    timestamp,
    pubKey,
    privKey,
  ...
  const spkrFromBytes = ZonaRosaClient.SignedPreKeyRecord.deserialize(
    spkr.serialize()
  );
...
it('KyberPreKeyRecord', () => {
  const keyPair = ZonaRosaClient.KEMKeyPair.generate();
  const publicKey = keyPair.getPublicKey();
  const secretKey = keyPair.getSecretKey();
  const timestamp = 9000;
  const keyId = 23;
  const signature = Buffer.alloc(64, 64);
  const record = ZonaRosaClient.KyberPreKeyRecord.new(
    keyId,
    timestamp,
    keyPair,
    signature
  ...
  const recordFromBytes = ZonaRosaClient.KyberPreKeyRecord.deserialize(
    record.serialize()
  );
  assert.deepEqual(recordFromBytes, record);
});

it('ZonaRosaMessage and PreKeyZonaRosaMessage', () => {
  const messageVersion = 3;
  const macKey = Buffer.alloc(32, 0xab);
  const senderRatchetKey = ZonaRosaClient.PrivateKey.generate().getPublicKey();
  const counter = 9;
  const previousCounter = 8;
  const senderIdentityKey = ZonaRosaClient.PrivateKey.generate().getPublicKey();
  const receiverIdentityKey = ZonaRosaClient.PrivateKey.generate().getPublicKey();
  const ciphertext = Buffer.from('01020304', 'hex');

  const sm = ZonaRosaClient.ZonaRosaMessage._new(
    messageVersion,
    macKey,
    senderRatchetKey,
    counter,
  ...
  const sm2 = ZonaRosaClient.ZonaRosaMessage.deserialize(sm_bytes);
  ...
  const baseKey = ZonaRosaClient.PrivateKey.generate().getPublicKey();
  const identityKey = ZonaRosaClient.PrivateKey.generate().getPublicKey();

  const pkm = ZonaRosaClient.PreKeyZonaRosaMessage._new(
    messageVersion,
    registrationId,
    preKeyId,
    signedPreKeyId,
  ...
  const pkm2 = ZonaRosaClient.PreKeyZonaRosaMessage.deserialize(pkm_bytes);
...
async function makePQXDHBundle(
  address: ZonaRosaClient.ProtocolAddress,
  stores: TestStores,
  excludeOneTimePreKey?: boolean
): Promise<ZonaRosaClient.PreKeyBundle> {
  const identityKey = await stores.identity.getIdentityKey();
  const prekeyId = chance.natural({ max: 10000 });
  const prekey = ZonaRosaClient.PrivateKey.generate();
  const signedPrekeyId = chance.natural({ max: 10000 });
  const signedPrekey = ZonaRosaClient.PrivateKey.generate();
  ...
  const kyberKeyPair = ZonaRosaClient.KEMKeyPair.generate();
  ...
  await stores.prekey.savePreKey(
    prekeyId,
    ZonaRosaClient.PreKeyRecord.new(prekeyId, prekey.getPublicKey(), prekey)
  );

  await stores.signed.saveSignedPreKey(
    signedPrekeyId,
    ZonaRosaClient.SignedPreKeyRecord.new(
      signedPrekeyId,
      chance.timestamp(),
  ...
  await stores.kyber.saveKyberPreKey(
    kyberPrekeyId,
    ZonaRosaClient.KyberPreKeyRecord.new(
      kyberPrekeyId,
      chance.timestamp(),
      kyberKeyPair,
      kyberPrekeySignature
  ...
  return ZonaRosaClient.PreKeyBundle.new(
...
for (const testCase of sessionVersionTestCases) {
  describe(`Session ${testCase.suffix}`, () => {
    it('BasicPreKeyMessaging', async () => {
      ...
      const aAddress = ZonaRosaClient.ProtocolAddress.new('+14151111111', 1);
      const bAddress = ZonaRosaClient.ProtocolAddress.new('+19192222222', 1);
      ...
      const aCiphertext = await ZonaRosaClient.zonarosaEncrypt(
        aMessage,
        bAddress,
        aliceStores.session,
        aliceStores.identity
      ...
      assert.deepEqual(
        aCiphertext.type(),
        ZonaRosaClient.CiphertextMessageType.PreKey
      );

      const aCiphertextR = ZonaRosaClient.PreKeyZonaRosaMessage.deserialize(
        aCiphertext.serialize()
      );

      const bDPlaintext = await ZonaRosaClient.zonarosaDecryptPreKey(
        aCiphertextR,
        aAddress,
        bobStores.session,
        bobStores.identity,
      ...
      assert.deepEqual(
        bCiphertext.type(),
        ZonaRosaClient.CiphertextMessageType.Whisper
      );

      const bCiphertextR = ZonaRosaClient.ZonaRosaMessage.deserialize(
        bCiphertext.serialize()
      );

      const aDPlaintext = await ZonaRosaClient.zonarosaDecrypt(
        bCiphertextR,
        bAddress,
        aliceStores.session,
        aliceStores.identity
    ...
    it('handles duplicated messages', async () => {
      ...
      try {
        await ZonaRosaClient.zonarosaDecryptPreKey(
          aCiphertextR,
          aAddress,
          bobStores.session,
          bobStores.identity,
        ...
        assert.equal(err.code, ZonaRosaClient.ErrorCode.DuplicatedMessage);
        assert.equal(err.operation, 'SessionCipher_DecryptPreKeyZonaRosaMessage'); // the Rust entry point
      ...
      try {
        await ZonaRosaClient.zonarosaDecrypt(
          bCiphertextR,
          bAddress,
          aliceStores.session,
          aliceStores.identity
        ...
        assert.equal(err.name, 'DuplicatedMessage');
        assert.equal(err.code, ZonaRosaClient.ErrorCode.DuplicatedMessage);
        assert.equal(err.operation, 'SessionCipher_DecryptZonaRosaMessage'); // the Rust entry point
        assert.exists(err.stack); // Make sure we're still getting the benefits of Error.
      }
    });
    ...
    it('expires unacknowledged sessions', async () => {
      ...
      await assert.isRejected(
        ZonaRosaClient.zonarosaEncrypt(
          aMessage,
          bAddress,
          aliceStores.session,
          aliceStores.identity,
    ...
    it('rejects pre-key messages sent from a second user', async () => {
      ...
      const aAddress = ZonaRosaClient.ProtocolAddress.new('+14151111111', 1);
      const bAddress = ZonaRosaClient.ProtocolAddress.new('+14151111112', 1);
      const mAddress = ZonaRosaClient.ProtocolAddress.new('+14151111113', 1);
      ...
      void (await ZonaRosaClient.zonarosaDecryptPreKey(
        aCiphertextR,
        aAddress,
        bobStores.session,
        bobStores.identity,
      ...
      await assert.isRejected(
        ZonaRosaClient.zonarosaDecryptPreKey(
          aCiphertextR,
          mAddress,
          bobStores.session,
          bobStores.identity,
