...
import { ErrorCode, LibZonaRosaErrorBase } from '../Errors.js';
import {
...
describe('chat service api', () => {
  ...
  it('rejects invalid proxies', () => {
    ...
    check(() => net.setProxy('zonarosafoundation.org', 0));
    check(() => net.setProxy('zonarosafoundation.org', 100_000));
    check(() => net.setProxy('zonarosafoundation.org', -1));
    check(() => net.setProxy('zonarosafoundation.org', 0.1));
    check(() => net.setProxy('zonarosafoundation.org', Number.MAX_SAFE_INTEGER));
    check(() => net.setProxy('zonarosafoundation.org', Number.MAX_VALUE));
    check(() => net.setProxy('zonarosafoundation.org', Number.POSITIVE_INFINITY));

    check(() =>
      net.setProxy({ scheme: 'socks+shoes', host: 'zonarosafoundation.org' })
    );

    check(() => net.setProxyFromUrl('not a url'));
    check(() => net.setProxyFromUrl('socks+shoes://zonarosafoundation.org'));
    check(() => net.setProxyFromUrl('https://zonarosafoundation.org:0x50'));
    check(() =>
      net.setProxyFromUrl('https://zonarosafoundation.org/path-for-some-reason')
    );
    check(() =>
      net.setProxyFromUrl('https://zonarosafoundation.org?query-for-some-reason')
    );
    check(() =>
      net.setProxyFromUrl(
        'https://zonarosafoundation.org#fragment-for-some-reason'
      )
    );
  ...
  it('parses proxy URLs the way we expect, if not always ideally', () => {
    ...
    // This is parsed "correctly" but the libzonarosa side doesn't support it, though this test doesn't exercise that.
  ...
  describe('ChatConnection integration tests', function (this: Mocha.Suite) {
    ...
    it('can connect unauthenticated', async function () {
      if (!process.env.LIBZONAROSA_TESTING_RUN_NONHERMETIC_TESTS) {
        this.skip();
      }
      const net = new Net({
        env: Environment.Production,
    ...
    it('can preconnect and then connect authenticated (partly)', async function () {
      ...
      try {
        ...
        assert.instanceOf(e, LibZonaRosaErrorBase);
        assert.include(e, {
    ...
    it('can connect through a proxy server', async function () {
      const PROXY_SERVER = process.env.LIBZONAROSA_TESTING_PROXY_SERVER;
      if (!PROXY_SERVER) {
        this.skip();
      }
    ...
    it('can connect through a proxy server using a URL', async function () {
      ...
      net.setProxyFromUrl(`${ZONAROSA_TLS_PROXY_SCHEME}://${PROXY_SERVER}`);
      expect(
        TESTING_ConnectionManager_isUsingProxy(net._connectionManager)
      ).equals(1);
      await connectChatUnauthenticated(net);
      ...
      const callsExpected: [string, ((value: unknown) => void)[]][] = [
        ...
        [
          ...
          [
            (error) =>
              expect(error)
                .instanceOf(LibZonaRosaErrorBase)
