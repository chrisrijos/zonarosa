...
const userAgent = 'libzonarosa-mock-test';
...
class ChatServer {
  ...
  public constructor() {
    this.cdn3Path = path.join(os.tmpdir(), `mock-zonarosa-cdn3-${this.randomId}`);
    ...
    (alertList) => {
      it(`can receive alerts: [${alertList.join(',')}]`, async () => {
        const headers: Record<string, string> = {
          unrelated: 'ignore',
          'x-zonarosa-alert': alertList.join(','),
        };
