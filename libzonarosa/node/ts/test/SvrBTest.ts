...
describe('SecureValueRecoveryBackup', () => {
  ...
  const enclaveSecret = process.env.LIBZONAROSA_TESTING_SVRB_ENCLAVE_SECRET;
  const testAuth = enclaveSecret
    ...
    : {
        username: process.env.LIBZONAROSA_TESTING_SVRB_USERNAME || '',
        password: process.env.LIBZONAROSA_TESTING_SVRB_PASSWORD || '',
      };
  ...
  describe('restoreBackup', () => {
    ...
    it('supports abort zonarosa', () => {
      const abortController = new AbortController();
      const result = svrB.restore(testBackupKey, new Uint8Array(), {
        abortZonaRosa: abortController.zonarosa,
      });
      assert.instanceOf(result, Promise);
      // eslint-disable-next-line promise/prefer-await-to-then
      result.catch(() => {});
  ...
  describe('Integration test with network calls', () => {
    beforeEach(function () {
      if (
        !process.env.LIBZONAROSA_TESTING_RUN_NONHERMETIC_TESTS ||
        testAuth.username == '' ||
        testAuth.password == ''
      ) {
        this.skip();
