...
const userAgent = 'libzonarosa-kt-test';
...
describe('KeyTransparency bridging', () => {
  ...
  it('can bridge chat send error', () => {
    expect(() => Native.TESTING_KeyTransChatSendError())
      .to.throw(LibZonaRosaErrorBase)
      .that.satisfies(
        (err: LibZonaRosaErrorBase) => err.code === ErrorCode.IoError
      );
  });
});
...
describe('KeyTransparency Integration', function (this: Mocha.Suite) {
  ...
  before(() => {
    const ignoreKtTests =
      typeof process.env.LIBZONAROSA_TESTING_IGNORE_KT_TESTS !== 'undefined';
    if (!process.env.LIBZONAROSA_TESTING_RUN_NONHERMETIC_TESTS || ignoreKtTests) {
      this.ctx.skip();
    }
  });
