...
#
# Copyright (C) 2021 ZonaRosa Platform.
...
def check_for_debug_level_logs(src_path: str) -> None:
    with open(src_path, 'rb') as f:
        with mmap.mmap(f.fileno(), length=0, access=mmap.ACCESS_READ) as m:
            # See libzonarosa-node's logging.rs.
...
def main(args: Optional[List[str]] = None) -> int:
    ...
    cmdline = ['cargo', 'build', '--target', cargo_target, '-p', 'libzonarosa-node', '--features', ','.join(features)]
    ...
    if node_os_name == 'win32':
        ...
        if 'GITHUB_WORKSPACE' in cargo_env:
            ...
            if len(tmpdir) < len(abs_build_dir):
                cargo_env['CARGO_BUILD_TARGET_DIR'] = os.path.join(tmpdir, 'libzonarosa')
    ...
    src_path = os.path.join(libs_in, lib_format.format('zonarosa_node'))
    if os.access(src_path, os.R_OK):
        if not allow_debug_level_logs:
            check_for_debug_level_logs(src_path)

        dst_base = 'libzonarosa_client_%s_%s' % (node_os_name, node_arch)
        ...
        maybe_dump_debug_symbols(
            src_path=os.path.join(libs_in, debug_format.format('zonarosa_node')),
            src_checksum_path=os.path.join(libs_in, debug_format_for_checksum.format('zonarosa_node')),
        ...
        if options.copy_to_prebuilds:
            ...
            prebuild_path = os.path.join(prebuild_dir, '@zonarosa+libzonarosa-client.node')
