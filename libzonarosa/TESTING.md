For the most part, libzonarosa is tested using each language's usual testing infrastructure, as described in the main [README.md](./README.md):
...
- Similarly, although most tests are not very sensitive to the speed of SHA-2, you should also compile with `--features sha2/asm`. (`libzonarosa-message-backup` turns this on by default as a dev-dependency.) This will go away when we get to update to sha2 0.11.
...
3. Set the following environment variables, filling in `path/to/ndk` and `YOUR_HOST_HERE`:
    ...
    (If working with a different target architecture, don't forget to change the environment variables above. You may need to set additional environment variables depending on what you're building; see [`build_jni.sh`][java/build_jni.sh] for the full set that libzonarosa-jni uses.)
