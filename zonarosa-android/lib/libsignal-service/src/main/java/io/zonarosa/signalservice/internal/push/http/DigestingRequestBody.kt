...
import io.zonarosa.libzonarosa.protocol.incrementalmac.ChunkSizeChoice
import io.zonarosa.libzonarosa.protocol.logging.Log
import io.zonarosa.service.api.crypto.DigestingOutputStream
import io.zonarosa.service.api.crypto.SkippingOutputStream
...
class DigestingRequestBody(
  ...
  private val progressListener: ZonaRosaServiceAttachment.ProgressListener?,
  private val cancelationZonaRosa: CancelationZonaRosa?,
  ...
  override fun writeTo(sink: BufferedSink) {
    ...
    while (inputStream.read(buffer, 0, buffer.size).also { read = it } != -1) {
      if (cancelationZonaRosa?.isCanceled == true) {
