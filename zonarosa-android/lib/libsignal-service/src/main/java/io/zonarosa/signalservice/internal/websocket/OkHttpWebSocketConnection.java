...
public class OkHttpWebSocketConnection extends WebSocketListener implements WebSocketConnection {
  ...
  public OkHttpWebSocketConnection(String name,
                                   ZonaRosaServiceConfiguration serviceConfiguration,
                                   Optional<CredentialsProvider> credentialsProvider,
                                   String zonarosaAgent,
                                   HealthMonitor healthMonitor,
                                   boolean allowStories) {
    this(name, serviceConfiguration, credentialsProvider, zonarosaAgent, healthMonitor, "", allowStories);
  }
                                   ...
                                   boolean allowStories)
  {
    ...
    this.trustStore          = serviceConfiguration.getZonaRosaServiceUrls()[0].getTrustStore();
    ...
    this.zonarosaProxy         = serviceConfiguration.getZonaRosaProxy();
    ...
    this.serviceUrls         = serviceConfiguration.getZonaRosaServiceUrls();
  ...
  private Pair<ZonaRosaServiceUrl, String> getConnectionInfo() {
    ZonaRosaServiceUrl serviceUrl = serviceUrls[random.nextInt(serviceUrls.length)];
  ...
  public synchronized Observable<WebSocketConnectionState> connect() {
    ...
    if (client == null) {
      Pair<ZonaRosaServiceUrl, String> connectionInfo = getConnectionInfo();
      ZonaRosaServiceUrl               serviceUrl     = connectionInfo.getFirst();
      ...
      if (zonarosaProxy.isPresent()) {
        clientBuilder.socketFactory(new TlsProxySocketFactory(zonarosaProxy.get().getHost(), zonarosaProxy.get().getPort(), dns));
      }
      ...
      requestBuilder.addHeader("X-ZonaRosa-Receive-Stories", allowStories ? "true" : "false");
