...
import io.zonarosa.service.api.crypto.ProfileCipherOutputStream
...
import io.zonarosa.service.internal.push.PaymentAddress
import io.zonarosa.service.internal.push.ProfileAvatarData
import io.zonarosa.service.internal.push.ProfileAvatarUploadAttributes
import io.zonarosa.service.internal.push.PushServiceSocket
import io.zonarosa.service.internal.push.http.ProfileCipherOutputStreamFactory
...
class ProfileApi(
  ...
  fun setVersionedProfile(
    ...
    val profileWrite = ZonaRosaServiceProfileWrite(
      version = profileKey.getProfileKeyVersion(aci.libZonaRosaAci).serialize(),
      ...
      commitment = profileKey.getCommitment(aci.libZonaRosaAci).serialize(),
   ...
   * - 200: Success
   * - 404: Recipient is not a registered ZonaRosa user
   * - 429: Rate-limited
   */
  fun getVersionedProfileAndCredential(aci: ServiceId.ACI, profileKey: ProfileKey, sealedSenderAccess: SealedSenderAccess?): NetworkResult<Pair<ZonaRosaServiceProfile, ExpiringProfileKeyCredential>> {
    val profileVersion = profileKey.getProfileKeyVersion(aci.libZonaRosaAci).serialize()
    val profileRequestContext = clientZkProfileOperations.createProfileKeyCredentialRequestContext(SecureRandom(), aci.libZonaRosaAci, profileKey)
   ...
   * - 429: Rate-limited
   */
  fun getVersionedProfile(aci: ServiceId.ACI, profileKey: ProfileKey, sealedSenderAccess: SealedSenderAccess?): NetworkResult<ZonaRosaServiceProfile> {
    val profileKeyIdentifier = profileKey.getProfileKeyVersion(aci.libZonaRosaAci)
    val profileVersion = profileKeyIdentifier.serialize()

    val request = WebSocketRequestMessage.get("/v1/profile/$aci/$profileVersion")
    val converter = NetworkResult.DefaultWebSocketConverter(ZonaRosaServiceProfile::class)
   ...
   * - 429: Rate-limited
   */
  fun getUnversionedProfile(serviceId: ServiceId, sealedSenderAccess: SealedSenderAccess?): NetworkResult<ZonaRosaServiceProfile> {
  ...
  ) : NetworkResult.WebSocketResponseConverter<Pair<ZonaRosaServiceProfile, ExpiringProfileKeyCredential>> {

    override fun convert(response: WebsocketResponse): NetworkResult<Pair<ZonaRosaServiceProfile, ExpiringProfileKeyCredential>> {
      ...
      return try {
        response
          .toSuccess(ZonaRosaServiceProfile::class)
          .map {
