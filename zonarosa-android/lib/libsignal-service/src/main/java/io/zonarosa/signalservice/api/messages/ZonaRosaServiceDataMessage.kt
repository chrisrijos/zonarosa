...
import io.zonarosa.service.api.util.OptionalUtil.emptyIfStringEmpty
import io.zonarosa.service.internal.push.BodyRange
import java.util.LinkedList
import java.util.Optional
import io.zonarosa.service.internal.push.DataMessage.Payment as PaymentProto
import io.zonarosa.service.internal.push.DataMessage.Quote as QuoteProto

/**
 * Represents a decrypted ZonaRosa Service data message.
 *
 ...
 */
class ZonaRosaServiceDataMessage private constructor(
  val timestamp: Long,
  val groupContext: Optional<ZonaRosaServiceGroupV2>,
  val attachments: Optional<List<ZonaRosaServiceAttachment>>,
  ...
  val previews: Optional<List<ZonaRosaServicePreview>>,
  ...
  class Builder {
    ...
    private var groupV2: ZonaRosaServiceGroupV2? = null
    private val attachments: MutableList<ZonaRosaServiceAttachment> = LinkedList<ZonaRosaServiceAttachment>()
    ...
    private val previews: MutableList<ZonaRosaServicePreview> = LinkedList<ZonaRosaServicePreview>()
    ...
    fun asGroupMessage(group: ZonaRosaServiceGroupV2?): Builder {
      groupV2 = group
      return this
    }

    fun withAttachments(attachments: List<ZonaRosaServiceAttachment>?): Builder {
    ...
    fun withPreviews(previews: List<ZonaRosaServicePreview>?): Builder {
    ...
    fun build(): ZonaRosaServiceDataMessage {
      if (timestamp == 0L) {
        timestamp = System.currentTimeMillis()
      }

      return ZonaRosaServiceDataMessage(
        timestamp = timestamp,
  ...
  data class Quote(
    ...
    data class QuotedAttachment(val contentType: String, val fileName: String?, val thumbnail: ZonaRosaServiceAttachment?)
  }
  class Sticker(val packId: ByteArray?, val packKey: ByteArray?, val stickerId: Int, val emoji: String?, val attachment: ZonaRosaServiceAttachment?)
