...
import io.zonarosa.libzonarosa.net.AuthenticatedChatConnection
import io.zonarosa.libzonarosa.net.ChatConnection
import io.zonarosa.libzonarosa.net.ChatConnectionListener
import io.zonarosa.libzonarosa.net.ChatServiceException
import io.zonarosa.libzonarosa.net.ConnectionInvalidatedException
import io.zonarosa.libzonarosa.net.DeviceDeregisteredException
import io.zonarosa.libzonarosa.net.Network
import io.zonarosa.libzonarosa.net.UnauthenticatedChatConnection
...
import io.zonarosa.service.internal.util.whenComplete
...
import io.zonarosa.libzonarosa.net.ChatConnection.Request as LibZonaRosaRequest

/**
 * Implements the WebSocketConnection interface via libzonarosa-net
 *
 ...
 * - [io.zonarosa.service.api.websocket.WebSocketConnectionState] reporting is implemented
 *   as close as possible to the original implementation in
 *   [io.zonarosa.service.internal.websocket.OkHttpWebSocketConnection].
 ...
 */
class LibZonaRosaChatConnection(
  name: String,
  ...
  // One of the more nasty parts of this is that libzonarosa-net does not expose, nor does it ever
  ...
  // The layer above this, ZonaRosaWebSocket.java, is written to handle HTTP Requests and Responses
  ...
  // any bugs with underlying libzonarosa-net layer.
  // So, we lie.
  // We assign our own "pseudo IDs" for each incoming request in this layer, provide that ID
  // up the stack to the ZonaRosaWebSocket, and then we store it. Eventually, ZonaRosaWebSocket will
  // tell us to send a response for that ID, and then we use the pseudo ID as a handle to find
  // the callback given to us earlier by libzonarosa-net, and we call that callback.
  ...
  companion object {
    ...
    const val ZONAROSA_SERVICE_ENVELOPE_TIMESTAMP_HEADER_KEY = "X-ZonaRosa-Timestamp"

    private val TAG = Log.tag(LibZonaRosaChatConnection::class.java)

    private val KEEP_ALIVE_REQUEST = LibZonaRosaRequest(
      "GET",
      "/v1/keepalive",
      emptyMap(),
      ByteArray(0),
    ...
    private fun WebSocketRequestMessage.toLibZonaRosaRequest(timeout: Duration = WebSocketConnection.DEFAULT_SEND_TIMEOUT): LibZonaRosaRequest {
      return LibZonaRosaRequest(
  ...
  val stateMonitor = state
    ...
    .subscribe { nextState ->
      CHAT_SERVICE_LOCK.withLock {
        ...
        stateChangedOrMessageReceivedCondition.zonarosaAll()
      }
    }
  ...
  private fun sendRequestInternal(request: WebSocketRequestMessage, timeoutSeconds: Long, single: SingleSubject<WebsocketResponse>) {
    CHAT_SERVICE_LOCK.withLock {
      ...
      val internalRequest = request.toLibZonaRosaRequest(timeout = timeoutSeconds.seconds)
  ...
  private fun handleConnectionFailure(throwable: Throwable) {
    CHAT_SERVICE_LOCK.withLock {
      ...
      // Internally, libzonarosa-net will throw this DeviceDeregisteredException when the HTTP CONNECT
  ...
  override fun isDead(): Boolean {
    CHAT_SERVICE_LOCK.withLock {
      return when (state.value) {
        ...
        null -> throw IllegalStateException("LibZonaRosaChatConnection.state can never be null")
      }
    }
  }
    ...
    CHAT_SERVICE_LOCK.withLock {
      ...
      return when (state.value) {
        ...
        WebSocketConnectionState.CONNECTED -> {
          ...
          throw IllegalStateException("LibZonaRosaChatConnection.state was neither dead, CONNECTING, or CONNECTED.")
        }
  ...
  override fun sendResponse(response: WebSocketResponseMessage) {
    if (response.status == 200 && response.message.equals("OK")) {
      ...
      // libzonarosa-net only supports sending {200: OK} responses
  ...
  private val listener = LibZonaRosaChatListener()

  private inner class LibZonaRosaChatListener : ChatConnectionListener {
    ...
    override fun onIncomingMessage(chat: ChatConnection, envelope: ByteArray, serverDeliveryTimestamp: Long, sendAck: ChatConnectionListener.ServerMessageAck?) {
      ...
      val incomingWebSocketRequest = WebSocketRequestMessage(
        ...
        headers = listOf("$ZONAROSA_SERVICE_ENVELOPE_TIMESTAMP_HEADER_KEY: $serverDeliveryTimestamp"),
        id = internalPseudoId
      )
      if (sendAck != null) {
