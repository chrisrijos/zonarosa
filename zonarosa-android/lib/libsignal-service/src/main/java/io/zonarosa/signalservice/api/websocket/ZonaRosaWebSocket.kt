...
import io.zonarosa.service.api.messages.EnvelopeResponse
...
import io.zonarosa.service.internal.websocket.WebSocketResponseMessage
 ...
 * interactions.
 */
sealed class ZonaRosaWebSocket(
  ...
  companion object {
    private val TAG = Log.tag(ZonaRosaWebSocket::class)

    const val SERVER_DELIVERED_TIMESTAMP_HEADER = "X-ZonaRosa-Timestamp"
  ...
  /**
   * Executes the given callback with the underlying libzonarosa chat connection when available.
   *
   * This is only supported for LibZonaRosa-based connections.
   *
   ...
   */
  suspend fun <T> runWithChatConnection(callback: (io.zonarosa.libzonarosa.net.ChatConnection) -> T): T {
  ...
  protected fun WebSocketRequestMessage.isZonaRosaServiceEnvelope(): Boolean {
  ...
  private fun WebSocketRequestMessage.getWebSocketResponse(): WebSocketResponseMessage {
    return if (this.isZonaRosaServiceEnvelope()) {
   ...
   */
  class UnauthenticatedWebSocket(connectionFactory: WebSocketFactory, canConnect: CanConnect, sleepTimer: SleepTimer, disconnectTimeoutMs: Long) : ZonaRosaWebSocket(connectionFactory, canConnect, sleepTimer, disconnectTimeoutMs.milliseconds) {
   ...
   */
  class AuthenticatedWebSocket(connectionFactory: WebSocketFactory, canConnect: CanConnect, sleepTimer: SleepTimer, disconnectTimeoutMs: Long) : ZonaRosaWebSocket(connectionFactory, canConnect, sleepTimer, disconnectTimeoutMs.milliseconds) {
    ...
    fun readMessageBatch(timeout: Long, batchSize: Int, callback: MessageReceivedCallback): Boolean {
      ...
      if (!hitEndOfQueue) {
        for (i in 1 until batchSize) {
          ...
          if (request != null) {
            if (request.isZonaRosaServiceEnvelope()) {
