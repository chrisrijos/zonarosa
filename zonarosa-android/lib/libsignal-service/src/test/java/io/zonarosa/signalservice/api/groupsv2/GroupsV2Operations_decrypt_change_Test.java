...
public final class GroupsV2Operations_decrypt_change_Test {
  ...
  private ByteString encryptProfileKey(ACI aci, ProfileKey profileKey) {
    return ByteString.of(new ClientZkGroupCipher(groupSecretParams).encryptProfileKey(profileKey, aci.getLibZonaRosaAci()).serialize());
  }
  ...
  GroupCandidate groupCandidate(ACI aci, ProfileKey profileKey) {
    try {
      ...
      ProfileKeyCommitment                 commitment                           = profileKey.getCommitment(aci.getLibZonaRosaAci());
      ProfileKeyCredentialRequestContext   requestContext                       = profileOperations.createProfileKeyCredentialRequestContext(aci.getLibZonaRosaAci(), profileKey);
