package io.zonarosa.devicetransfer;
...
import io.zonarosa.devicetransfer.WifiDirectUnavailableException.Reason;
...
public final class WifiDirect {
  ...
  private static final String  SERVICE_INSTANCE_TEMPLATE = "_devicetransfer" + EXTRA_INFO_PLACEHOLDER + "._zonarosa.io";
  private static final Pattern SERVICE_INSTANCE_PATTERN  = Pattern.compile("_devicetransfer(\\._(.+))?\\._zonarosa\\.org");
  ...
  WifiDirect(@NonNull Context context) {
    ...
    this.wifiDirectCallbacksHandler = ZonaRosaExecutors.getAndStartHandlerThread("wifi-direct-cb", ThreadUtil.PRIORITY_IMPORTANT_BACKGROUND_THREAD);
  }
  ...
  private void retryAsync(@NonNull ManagerRetry retryFunction, @NonNull String message) {
    ZonaRosaExecutors.BOUNDED.execute(() -> retrySync(retryFunction, message, WifiDirect.NOT_SAFE_FOR_LONG_AWAIT_TIMEOUT));
  }
