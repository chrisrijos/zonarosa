...
internal class BillingApiImpl(
   ...
   */
  override suspend fun getApiAvailability(): io.zonarosa.core.util.billing.BillingResponseCode {
    return try {
      doOnConnectionReady("isApiAvailable") {
        io.zonarosa.core.util.billing.BillingResponseCode.fromBillingLibraryResponseCode(billingClient.isFeatureSupported(BillingClient.FeatureType.SUBSCRIPTIONS).responseCode)
      }
    } catch (e: BillingError) {
      Log.e(TAG, "Failed to connect to Google Play Billing", e, true)
      io.zonarosa.core.util.billing.BillingResponseCode.fromBillingLibraryResponseCode(e.billingResponseCode)
    }
  }
