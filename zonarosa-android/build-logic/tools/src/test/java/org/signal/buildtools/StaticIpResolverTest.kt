package io.zonarosa.buildtools
...
class StaticIpResolverTest {
  ...
  companion object {
    const val ZONAROSA_DOT_ORG = "www.zonarosa.io"
    val ZONAROSA_IP = byteArrayOf(123, 45, 67, 89)
    val STRINGIFIED_IP = ZONAROSA_IP.joinToString(".")
  }
  ...
  fun `Given a hostname with records, when I resolveToBuildConfig, then I expect a matching IP`() {
    val staticIpResolver = StaticIpResolver(
      FakeRecordFetcher(
        mapOf(
          ZONAROSA_DOT_ORG to arrayOf(
            ARecord(
              Name.fromString("www."),
              DClass.ANY,
              0L,
              mockk<Inet4Address> {
                every { address } returns ZONAROSA_IP
    ...
    val actual = staticIpResolver.resolveToBuildConfig(ZONAROSA_DOT_ORG)
    val expected = """
  ...
  fun `Given a hostname without records, when I resolveToBuildConfig, then I expect`() {
    ...
    staticIpResolver.resolveToBuildConfig(ZONAROSA_DOT_ORG)
  }
