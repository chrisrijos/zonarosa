...
class ZonaRosaLogDetector : Detector(), Detector.UastScanner {
  ...
  override fun visitMethodCall(context: JavaContext, node: UCallExpression, method: PsiMethod) {
    ...
    if (evaluator.isMemberInClass(method, "android.util.Log")) {
      context.report(
        issue = LOG_NOT_ZONAROSA,
        scope = node,
        location = context.getLocation(node),
        message = "Using 'android.util.Log' instead of a ZonaRosa Logger",
        quickfixData = quickFixIssueLog(node)
      )
    }

    if (evaluator.isMemberInClass(method, "io.zonarosa.glide.Log")) {
      context.report(
        issue = LOG_NOT_ZONAROSA,
        scope = node,
        location = context.getLocation(node),
        message = "Using 'io.zonarosa.glide.Log' instead of a ZonaRosa Logger",
        quickfixData = quickFixIssueLog(node)
      )
    }

    if (evaluator.isMemberInClass(method, "io.zonarosa.libzonarosa.protocol.logging.Log")) {
      context.report(
        issue = LOG_NOT_APP,
        scope = node,
        location = context.getLocation(node),
        message = "Using ZonaRosa server logger instead of app level Logger",
        quickfixData = quickFixIssueLog(node)
      )
    }

    if (evaluator.isMemberInClass(method, "io.zonarosa.core.util.logging.Log")) {
  ...
  private fun quickFixIssueLog(logCall: UCallExpression): LintFix {
    ...
    var fixSource = "io.zonarosa.core.util.logging.Log."
  ...
  companion object {
    val LOG_NOT_ZONAROSA: Issue = Issue.create(
      id = "LogNotZonaRosa",
      briefDescription = "Logging call to Android Log instead of ZonaRosa's Logger",
      explanation = "ZonaRosa has its own logger which must be used.",
      category = MESSAGES,
      priority = 5,
      severity = ERROR,
      implementation = Implementation(ZonaRosaLogDetector::class.java, JAVA_FILE_SCOPE)
    )

    val LOG_NOT_APP: Issue = Issue.create(
      id = "LogNotAppZonaRosa",
      briefDescription = "Logging call to ZonaRosa Service Log instead of App level Logger",
      explanation = "ZonaRosa app layer has its own logger which must be used.",
      category = MESSAGES,
      priority = 5,
      severity = ERROR,
      implementation = Implementation(ZonaRosaLogDetector::class.java, JAVA_FILE_SCOPE)
