...
class DatabaseConsistencyTest {
  ...
  fun testUpgradeConsistency() {
    val currentVersionStatements = ZonaRosaDatabase.rawDatabase.getAllCreateStatements()
    val testHelper = InMemoryTestHelper(AppDependencies.application).also {
      it.onUpgrade(it.writableDatabase, 181, ZonaRosaDatabaseMigrations.DATABASE_VERSION)
    }
  ...
  fun testForeignKeyIndexCoverage() {
    ...
    val foreignKeys: List<ForeignKeyConstraint> = ZonaRosaDatabase.rawDatabase.getForeignKeys()
    val indexesByFirstColumn: List<Index> = ZonaRosaDatabase.rawDatabase.getIndexes()
  ...
  private class InMemoryTestHelper(private val application: Application) : SQLiteOpenHelper(application, null, null, 1) {
    ...
    override fun onUpgrade(db: SQLiteDatabase, oldVersion: Int, newVersion: Int) {
      ZonaRosaDatabaseMigrations.migrate(application, ZonaRosaSQLiteDatabase(db), 181, ZonaRosaDatabaseMigrations.DATABASE_VERSION)
    }
