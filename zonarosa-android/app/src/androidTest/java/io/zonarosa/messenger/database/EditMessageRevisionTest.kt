...
class EditMessageRevisionTest {
  ...
  fun setUp() {
    ...
    senderId = ZonaRosaDatabase.recipients.getOrInsertFromServiceId(senderAci)
    threadId = ZonaRosaDatabase.threads.getOrCreateThreadIdFor(senderId, false, ThreadTable.DistributionTypes.DEFAULT)
  }
  ...
  private fun insertOriginalMessage(sentTimeMillis: Long): Long {
    ...
    return ZonaRosaDatabase.messages.insertMessageInbox(message, threadId).get().messageId
  }
  ...
  private fun insertEdit(originalSentTimestamp: Long, editSentTimeMillis: Long): Long {
    val targetMessage = ZonaRosaDatabase.messages.getMessageFor(originalSentTimestamp, senderId) as MmsMessageRecord
    ...
    return ZonaRosaDatabase.messages.insertEditMessageInbox(editMessage, targetMessage).get().messageId
  }
  ...
  private fun getNotificationStateMessageIds(): List<Long> {
    return ZonaRosaDatabase.messages.getMessagesForNotificationState(emptyList()).use { cursor ->
  ...
  private fun markAsRead(messageId: Long) {
    ZonaRosaDatabase.rawDatabase.execSQL(
