...
import io.zonarosa.messenger.database.JobDatabase.Companion.getInstance
...
class JobManagerPerformanceTests {
  ...
  private fun testJobManager(context: Application): JobManager {
    val config = JobManager.Configuration.Builder()
      ...
      .setJobMigrator(JobMigrator(ZonaRosaPreferences.getJobManagerVersion(context), JobManager.CURRENT_VERSION, JobManagerFactories.getJobMigrations(context)))
      .build()
