...
import io.zonarosa.messenger.testing.CoroutineDispatcherRule
...
class MessageBackupsCheckoutActivityTest {
  ...
  @get:Rule val activityRule = ZonaRosaActivityRule()
  ...
  fun e2e_paid_happy_path() {
    ...
    val iap = ZonaRosaDatabase.inAppPayments.getLatestInAppPaymentByType(InAppPaymentType.RECURRING_BACKUP)
  ...
  fun e2e_free_happy_path() {
    ...
    assertThat(ZonaRosaStore.backup.backupTier).isEqualTo(MessageBackupTier.FREE)
  }
  ...
  private fun e2e_shared_happy_path(context: Context, scenario: ActivityScenario<MessageBackupsCheckoutActivity>) {
    ...
    composeTestRule.onNodeWithText(context.getString(R.string.RemoteBackupsSettingsFragment__zonarosa_backups)).assertIsDisplayed()
    ...
    scenario.onActivity {
      val backupKeyString = ZonaRosaStore.account.accountEntropyPool.displayValue.chunked(4).joinToString("  ")
    ...
    composeTestRule.onNodeWithTag("message-backups-key-verify-screen-backup-key-input-field").performTextInput(
      ZonaRosaStore.account.accountEntropyPool.displayValue
    )
