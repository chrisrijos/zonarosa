...
import io.zonarosa.messenger.database.model.databaseprotos.addMember
import io.zonarosa.messenger.database.model.databaseprotos.addRequestingMember
import io.zonarosa.messenger.database.model.databaseprotos.deleteRequestingMember
import io.zonarosa.messenger.database.model.databaseprotos.groupChange
import io.zonarosa.messenger.database.model.databaseprotos.groupContext
...
class SmsDatabaseTest_collapseJoinRequestEventsIfPossible {
  ...
  @Before
  fun setUp() {
    recipients = ZonaRosaDatabase.recipients
    sms = ZonaRosaDatabase.messages
  ...
  private fun groupUpdateMessage(sender: RecipientId, groupContext: DecryptedGroupV2Context): IncomingMessage {
    ...
    val updateDescription = GV2UpdateDescription(
      ...
      groupChangeUpdate = GroupsV2UpdateMessageConverter.translateDecryptedChangeUpdate(ZonaRosaStore.account.getServiceIds(), groupContext)
    )
