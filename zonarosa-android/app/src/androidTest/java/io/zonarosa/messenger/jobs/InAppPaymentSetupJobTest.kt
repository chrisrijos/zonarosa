...
import io.zonarosa.messenger.jobs.protos.InAppPaymentSourceData
...
class InAppPaymentSetupJobTest {
  ...
  fun givenAnInAppPaymentInRequiredActionCompletedWithoutCompletedState_whenIRun_thenIExpectFailure() {
    ...
    assertThat(result.isFailure).isEqualTo(true)
    assertThat(ZonaRosaDatabase.inAppPayments.getById(id)?.state).isEqualTo(InAppPaymentTable.State.END)
  }
  ...
  fun givenRequiredActionComplete_whenIRun_thenIBypassPerformPreUserAction() {
    ...
    val testJob = TestInAppPaymentSetupJob(
      ...
      postUserActionResult = {
        assertThat(ZonaRosaDatabase.inAppPayments.getById(id)?.state).isEqualTo(InAppPaymentTable.State.TRANSACTING)
        Job.Result.success()
      }
    )
  ...
  fun givenPayPalUserActionRequired_whenIRun_thenIDoNotPerformPostUserActionResult() {
    ...
    val fresh = ZonaRosaDatabase.inAppPayments.getById(id)!!
  ...
  private fun insertInAppPayment(
    ...
    return ZonaRosaDatabase.inAppPayments.insert(
