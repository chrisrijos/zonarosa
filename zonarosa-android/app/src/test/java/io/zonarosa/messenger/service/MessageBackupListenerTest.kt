...
import io.zonarosa.messenger.testutil.MockRandom
...
class MessageBackupListenerTest {
  ...
  @get:Rule
  val rule = MockZonaRosaStoreRule()
  ...
  fun testGetNextScheduledExecutionTime() {
    ...
    every { ZonaRosaStore.backup.nextBackupTime } returns nextTime
    ...
    every { ZonaRosaStore.backup.nextBackupTime } returns nextTime
    every { ZonaRosaStore.settings.zonarosaBackupHour } returns 2
    every { ZonaRosaStore.settings.zonarosaBackupMinute } returns 0
    every { ZonaRosaStore.backup.nextBackupTime = any() } just runs
  ...
  fun testSetNextBackupTimeToIntervalFromNow() {
    ...
    every { ZonaRosaStore.settings.zonarosaBackupHour } returns 2
    every { ZonaRosaStore.settings.zonarosaBackupMinute } returns 1
    every { ZonaRosaStore.backup.nextBackupTime = any() } just runs
