...
import io.zonarosa.core.util.Hex.fromStringCondensed
...
import io.zonarosa.messenger.database.model.databaseprotos.pendingMember
import io.zonarosa.messenger.database.model.databaseprotos.requestingMember
import io.zonarosa.messenger.database.setNewDescription
import io.zonarosa.messenger.database.setNewTitle
...
import io.zonarosa.messenger.groups.v2.processing.GroupsV2StateProcessor.ProfileAndMessageHelper
...
class GroupsV2StateProcessorTest {
  ...
  fun setUp() {
    ...
    every { ZonaRosaStore.internal.gv2IgnoreP2PChanges } returns false
    ...
    every { AppDependencies.zonarosaServiceAccountManager.getGroupsV2Api() } returns groupsV2API
    ...
    every { ZonaRosaDatabase.groups } returns groupTable
    every { ZonaRosaDatabase.recipients } returns recipientTable
    every { ZonaRosaDatabase.threads } returns threadTable
  ...
  fun updateFromServerIfUnableToApplyP2PChange() {
    given {
      ...
      serverState(
        revision = 2,
        title = "Breaking ZonaRosa for Science",
        members = selfAndOthers
      )
      changeSet {
        changeLog(2) {
    ...
    val signedChange = DecryptedGroupChange(
      revision = 2,
      newTitle = DecryptedString("Breaking ZonaRosa for Science")
    )
  ...
  fun `when freshly added to a group, with additional group changes after being added, then only update from server at the revision we were added, and then schedule pulling additional changes later`() {
    given {
      changeSet {
        changeLog(2) {
          fullSnapshot(
            title = "Baking ZonaRosa for Science",
            description = "We break stuff, because we must.",
            members = listOf(member(otherAci), member(selfAci, joinedAt = 2))
          )
        }
        changeLog(3) {
          change {
            setNewTitle("Breaking ZonaRosa for Science")
          }
        }
      }
      apiCallParameters(2, true)
    ...
    assertThat(result.latestServer)
      ...
      .given {
        ...
        assertThat(it.title, "title matches that as it was in revision added").isEqualTo("Baking ZonaRosa for Science")
      }
   ...
   */
  @Test
  fun ignore403sWithoutZonaRosaTimestampHeader() {
    given {
      localState(
        active = false,
        revision = 5,
