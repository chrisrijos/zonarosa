...
class InAppPaymentsTestRule : ExternalResource() {
  ...
  override fun before() {
    ...
    mockkObject(ZonaRosaDatabase.Companion)
    every { ZonaRosaDatabase.Companion.donationReceipts } returns mockk {
      every { ZonaRosaDatabase.Companion.donationReceipts.addReceipt(any()) } returns Unit
    }

    every { ZonaRosaDatabase.Companion.inAppPayments } returns mockk {
      every { ZonaRosaDatabase.Companion.inAppPayments.insert(any(), any(), any(), any(), any()) } answers {
      ...
      every { ZonaRosaDatabase.Companion.inAppPayments.update(any()) } answers {
        val inAppPayment = firstArg<InAppPaymentTable.InAppPayment>()
        inAppPaymentCache[inAppPayment.id] = inAppPayment
      }

      every { ZonaRosaDatabase.Companion.inAppPayments.getById(any()) } answers {
    ...
    every { ZonaRosaStore.Companion.backup } returns mockk {
  ...
  override fun after() {
    ...
    unmockkObject(InAppDonations, ZonaRosaDatabase.Companion, ZonaRosaStore.Companion)
  }
  ...
  companion object {
    fun mockLocalSubscriberAccess(initialSubscriber: InAppPaymentSubscriberRecord? = null): AtomicReference<InAppPaymentSubscriberRecord?> {
      ...
      every { ZonaRosaDatabase.inAppPaymentSubscribers.getBySubscriberId(any()) } answers {
        ref.get()
      }
      every { ZonaRosaDatabase.inAppPaymentSubscribers.insertOrReplace(any()) } answers {
        ref.set(firstArg())
      }
