...
class InAppPaymentRecurringContextJobTest {
  ...
  fun setUp() {
    ...
    every { mockZonaRosaStore.account.isRegistered } returns true
    every { mockZonaRosaStore.account.isLinkedDevice } returns false
    every { mockZonaRosaStore.inAppPayments.setLastEndOfPeriod(any()) } returns Unit
  ...
  fun `Given a CREATED IAP, when I onAdded, then I expect PENDING`() {
    ...
    job.onAdded()
    val updatedIap = ZonaRosaDatabase.inAppPayments.getById(iap.id)
  ...
  fun `Given user has donor entitlement already, when I run, then I do not expect receipt request`() {
    ...
    every { AppDependencies.zonarosaServiceAccountManager.whoAmI } returns mockk {
