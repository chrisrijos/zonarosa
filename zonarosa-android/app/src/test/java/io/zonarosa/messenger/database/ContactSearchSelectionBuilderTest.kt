...
class ContactSearchSelectionBuilderTest {
  ...
  @Test
  fun `Given registered, when I build, then I expect ZONAROSA_CONTACT`() {
    val result = RecipientTable.ContactSearchSelection.Builder()
      .withRegistered(true)
      .build()

    Assert.assertTrue(result.where.contains(RecipientTable.ContactSearchSelection.ZONAROSA_CONTACT))
  ...
  fun `Given all non group contacts, when I build, then I expect both CONTACT and FILTER_GROUP`() {
    ...
    Assert.assertTrue(result.where.contains(RecipientTable.ContactSearchSelection.NON_ZONAROSA_CONTACT))
  ...
  @Test
  fun `Given a query, when I build, then I expect QUERY_ZONAROSA_CONTACT`() {
    val result = RecipientTable.ContactSearchSelection.Builder()
      .withRegistered(true)
      .withGroups(false)
    ...
    Assert.assertTrue(result.where.contains(RecipientTable.ContactSearchSelection.QUERY_ZONAROSA_CONTACT))
