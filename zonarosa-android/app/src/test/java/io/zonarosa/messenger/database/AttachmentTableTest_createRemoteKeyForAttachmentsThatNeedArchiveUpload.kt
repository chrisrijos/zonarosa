...
class AttachmentTableTest_createRemoteKeyForAttachmentsThatNeedArchiveUpload {
  ...
  @get:Rule val zonarosaDatabaseRule = ZonaRosaDatabaseRule()
  ...
  fun whenNoEligibleAttachments_returnsZero() {
    val result = ZonaRosaDatabase.attachments.createRemoteKeyForAttachmentsThatNeedArchiveUpload()
  ...
  fun whenAttachmentHasArchiveTransferStateInProgress_returnsZero() {
    ...
    ZonaRosaDatabase.attachments.setArchiveTransferState(attachmentId, AttachmentTable.ArchiveTransferState.UPLOAD_IN_PROGRESS)
  ...
  fun whenTransferStateNotDone_returnsZero() {
    val attachmentId = insertWithData()
    ZonaRosaDatabase.attachments.setTransferState(1L, attachmentId, 1) // Not TRANSFER_PROGRESS_DONE (0)
  ...
  fun whenOneEligibleAttachment_returnsOneAndCreatesRemoteKey() {
    ...
    val attachmentBefore = ZonaRosaDatabase.attachments.getAttachment(attachmentId)
    ...
    val attachmentAfter = ZonaRosaDatabase.attachments.getAttachment(attachmentId)
  ...
  fun whenMultipleEligibleAttachments_returnsCorrectCountAndCreatesKeys() {
    ...
    assertThat(ZonaRosaDatabase.attachments.getAttachment(attachmentId1)?.remoteKey).isNull()
    assertThat(ZonaRosaDatabase.attachments.getAttachment(attachmentId2)?.remoteKey).isNull()
    assertThat(ZonaRosaDatabase.attachments.getAttachment(attachmentId3)?.remoteKey).isNull()

    val result = ZonaRosaDatabase.attachments.createRemoteKeyForAttachmentsThatNeedArchiveUpload()
    assertThat(result.totalCount).isEqualTo(3)

    // Verify all remote keys were created
    assertThat(ZonaRosaDatabase.attachments.getAttachment(attachmentId1)?.remoteKey).isNotNull()
    assertThat(ZonaRosaDatabase.attachments.getAttachment(attachmentId2)?.remoteKey).isNotNull()
    assertThat(ZonaRosaDatabase.attachments.getAttachment(attachmentId3)?.remoteKey).isNotNull()
  }
  ...
  fun whenMixedScenarios_returnsCorrectCount() {
    ...
    ZonaRosaDatabase.attachments.setArchiveTransferState(inProgressAttachmentId, AttachmentTable.ArchiveTransferState.UPLOAD_IN_PROGRESS)
    ...
    assertThat(ZonaRosaDatabase.attachments.getAttachment(eligibleAttachmentId)?.remoteKey).isNotNull()
    assertThat(ZonaRosaDatabase.attachments.getAttachment(attachmentWithKeyId)?.remoteKey).isNotNull() // Already had one
    assertThat(ZonaRosaDatabase.attachments.getAttachment(inProgressAttachmentId)?.remoteKey).isNull()
    assertThat(ZonaRosaDatabase.attachments.getAttachment(noDataFileAttachmentId)?.remoteKey).isNull()
  }
  ...
  fun whenCalledTwice_secondCallReturnsZero() {
    ...
    val firstResult = ZonaRosaDatabase.attachments.createRemoteKeyForAttachmentsThatNeedArchiveUpload()
    assertThat(firstResult.totalCount).isEqualTo(1)

    // Second call should find no eligible attachments since remote key now exists
    val secondResult = ZonaRosaDatabase.attachments.createRemoteKeyForAttachmentsThatNeedArchiveUpload()
    assertThat(secondResult.totalCount).isEqualTo(0)

    // Verify attachment still has remote key
    assertThat(ZonaRosaDatabase.attachments.getAttachment(attachmentId)?.remoteKey).isNotNull()
  }
  ...
  fun whenMatchingDataFileAndHashExists_reusesRemoteKey() {
    ...
    val targetAttachment = ZonaRosaDatabase.attachments.getAttachment(targetAttachmentId)!!
  ...
  fun whenSourceHasNoRemoteData_generatesNewKey() {
    ...
    assertThat(ZonaRosaDatabase.attachments.getAttachment(sourceAttachmentId)?.remoteKey).isNotNull()
    assertThat(ZonaRosaDatabase.attachments.getAttachment(targetAttachmentId)?.remoteKey).isNotNull()
  }
  ...
  fun whenTwoAttachmentsShareDataFileAndBothNeedNewKey_bothGetSameKey() {
    ...
    val attachment1 = ZonaRosaDatabase.attachments.getAttachment(attachmentId1)!!
    val attachment2 = ZonaRosaDatabase.attachments.getAttachment(attachmentId2)!!
  ...
  private fun insertAttachmentDirectly(
    ...
    val db = ZonaRosaDatabase.attachments.writableDatabase
