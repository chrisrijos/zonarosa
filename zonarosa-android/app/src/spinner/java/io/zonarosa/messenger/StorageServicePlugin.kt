...
class StorageServicePlugin : Plugin {
  ...
  override fun get(parameters: Map<String, List<String>>): PluginResult {
    ...
    val zonarosaStorageRecords = when (val result = repository.readStorageRecords(storageServiceKey, manifest.recordIkm, manifest.storageIds)) {
    ...
    for (record in zonarosaStorageRecords) {
