...
class BackupPlugin : Plugin {
  ...
  private fun getSelectedRemoteBackup(): String {
    ...
    val frameReader = EncryptedBackupReader.createForZonaRosaBackup(
      key = backupKey,
      aci = selfData.aci,
  ...
  private fun extractRecipientName(recipient: io.zonarosa.messenger.backup.v2.proto.Recipient?): String {
  ...
  private fun getRemoteBackup(): String {
    return if (ZonaRosaStore.backup.hasBackupBeenUploaded) {
      """<a href="$PATH?remoteBackup=true">${formatTimestamp(ZonaRosaStore.backup.lastBackupTime)} - ${ZonaRosaStore.backup.lastBackupProtoSize.bytes.toUnitString()}</a>"""
    } else {
      "No remote backup"
    }
  }
