...
class ApiPlugin : Plugin {
  ...
  private fun recipient(parameters: Map<String, List<String>>): PluginResult {
    ...
    val groupType = when {
      ...
      recipient.isPushV2Group -> "ZonaRosa V2"
      recipient.isPushV1Group -> "ZonaRosa V1"
      recipient.isPushGroup -> "ZonaRosa"
      else -> null
    }
  ...
  private fun thread(parameters: Map<String, List<String>>): PluginResult {
    ...
    val threadRecord = ZonaRosaDatabase.threads.getThreadRecord(threadId) ?: return PluginResult.ErrorResult(message = "Thread not found")
  ...
  private fun message(parameters: Map<String, List<String>>): PluginResult {
    ...
    val messageRecord = try {
      ZonaRosaDatabase.messages.getMessageRecord(messageId)
    } catch (e: Exception) {
