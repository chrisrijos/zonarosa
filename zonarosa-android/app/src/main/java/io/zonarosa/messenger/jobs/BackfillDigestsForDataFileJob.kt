...
import io.zonarosa.messenger.jobs.protos.BackfillDigestsForDataFileJobData
...
class BackfillDigestsForDataFileJob private constructor(
  ...
  override fun run(): Result {
    val (originalKey, decryptingStream) = ZonaRosaDatabase.rawDatabase.withinTransaction {
      val attachment = ZonaRosaDatabase.attachments.getMostRecentValidAttachmentUsingDataFile(dataFile)
      if (attachment == null) {
      ...
      val stream = try {
        ZonaRosaDatabase.attachments.getAttachmentStream(attachment.attachmentId, offset = 0)
      } catch (e: IOException) {
    ...
    ZonaRosaDatabase.attachments.updateRemoteKeyAndDigestByDataFile(
      dataFile = dataFile,
      key = key,
      digest = digest
    )
