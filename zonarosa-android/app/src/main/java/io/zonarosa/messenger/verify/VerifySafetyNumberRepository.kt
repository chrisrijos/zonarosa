...
object VerifySafetyNumberRepository {
  ...
  suspend fun verifyAutomatically(recipient: Recipient): VerifyResult {
    ...
    val aci = recipient.requireAci().libZonaRosaAci
    ...
    val result = if (firstSearch) {
      ...
      ZonaRosaNetwork.keyTransparency.search(aci, aciIdentityKey, e164, unidentifiedAccessKey, usernameHash = null, KeyTransparencyStore)
    } else {
      Log.i(TAG, "Monitoring search in key transparency")
      ZonaRosaNetwork.keyTransparency.monitor(KeyTransparency.MonitorMode.OTHER, aci, aciIdentityKey, e164, unidentifiedAccessKey, usernameHash = null, KeyTransparencyStore)
    }
