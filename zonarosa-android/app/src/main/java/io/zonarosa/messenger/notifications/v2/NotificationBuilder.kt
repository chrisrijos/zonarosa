...
import io.zonarosa.core.util.PendingIntentFlags.mutable
...
sealed class NotificationBuilder(protected val context: Context) {
  ...
  private val privacy: NotificationPrivacyPreference = ZonaRosaStore.settings.messageNotificationsPrivacy
  ...
  fun addReplyActions(conversation: NotificationConversation) {
    if (privacy.isDisplayMessage && isNotLocked && !conversation.recipient.isPushV1Group && RecipientUtil.isMessageRequestAccepted(context, conversation.recipient)) {
      if (conversation.recipient.isPushV2Group) {
        val group: Optional<GroupRecord> = ZonaRosaDatabase.groups.getGroup(conversation.recipient.requireGroupId())
  ...
  fun setLights() {
    val ledColor: String = ZonaRosaStore.settings.messageLedColor

    if (ledColor != "none") {
      var blinkPattern = ZonaRosaStore.settings.messageLedBlinkPattern
      if (blinkPattern == "custom") {
        blinkPattern = ZonaRosaPreferences.getNotificationLedPatternCustom(context)
      }
  ...
  private class NotificationBuilderCompat(context: Context) : NotificationBuilder(context) {
    ...
    override fun setAlarms(recipient: Recipient?) {
      ...
      val defaultRingtone: Uri = ZonaRosaStore.settings.messageNotificationSound
      val defaultVibrate: Boolean = ZonaRosaStore.settings.isMessageVibrateEnabled
...
private fun ReplyMethod.toLongDescription(): Int {
  return when (this) {
    ...
    ReplyMethod.SecureMessage -> R.string.MessageNotifier_zonarosa_message
  }
}
