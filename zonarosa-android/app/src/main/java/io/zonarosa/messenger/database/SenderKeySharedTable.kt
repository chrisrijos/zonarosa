 ...
 */
class SenderKeySharedTable internal constructor(context: Context?, databaseHelper: ZonaRosaDatabase?) : DatabaseTable(context, databaseHelper) {
  companion object {
...
/**
   ...
   */
  fun markAsShared(distributionId: DistributionId, addresses: Collection<ZonaRosaProtocolAddress>) {
   ...
   */
  fun getSharedWith(distributionId: DistributionId): Set<ZonaRosaProtocolAddress> {
   ...
   */
  fun delete(distributionId: DistributionId, addresses: Collection<ZonaRosaProtocolAddress>) {
    writableDatabase.withinTransaction { db ->
      for (address in addresses) {
   ...
   */
  fun deleteAllFor(addresses: Collection<ZonaRosaProtocolAddress>) {
    writableDatabase.withinTransaction { db ->
      for (address in addresses) {
        db.delete(TABLE_NAME)
