...
import io.zonarosa.messenger.linkpreview.LinkPreviewUtil.OpenGraph;
import io.zonarosa.messenger.mms.PushMediaConstraints;
import io.zonarosa.messenger.net.CallRequestController;
import io.zonarosa.messenger.net.CompositeRequestController;
import io.zonarosa.messenger.net.RequestController;
import io.zonarosa.messenger.net.UserAgentInterceptor;
...
import io.zonarosa.messenger.util.OkHttpUtil;
...
import io.zonarosa.service.api.messages.ZonaRosaServiceStickerManifest.StickerInfo;
...
public class LinkPreviewRepository {
  ...
  {
    if (!ZonaRosaStore.settings().isLinkPreviewsEnabled()) {
                                                               ...
                                                               @NonNull Callback callback)
  {
    ZonaRosaExecutors.UNBOUNDED.execute(() -> {
      try {
        ...
        ZonaRosaServiceStickerManifest manifest = receiver.retrieveStickerManifest(packIdBytes, packKeyBytes);
  ...
  private static RequestController fetchCallLinkPreview(@NonNull Context context,
    ...
    Disposable disposable = AppDependencies.getZonaRosaCallManager()
                                           .getCallLinkManager()
