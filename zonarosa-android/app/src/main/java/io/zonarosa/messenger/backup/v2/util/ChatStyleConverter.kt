...
import io.zonarosa.messenger.util.UriUtil
import io.zonarosa.messenger.wallpaper.ChatWallpaper
import io.zonarosa.messenger.wallpaper.ChatWallpaperFactory
import io.zonarosa.messenger.wallpaper.GradientChatWallpaper
import io.zonarosa.messenger.wallpaper.SingleColorChatWallpaper
...
fun ChatStyle.toLocal(importState: ImportState): ChatColors? {
  ...
  if (this.customColorId != null) {
    return importState.remoteToLocalColorId[this.customColorId]?.let { localId ->
      ...
      return ZonaRosaDatabase.chatColors.getById(colorId)
    }
  }
...
private fun Wallpaper.File.toFilePointer(db: ZonaRosaDatabase, backupMode: BackupMode): FilePointer? {
