...
import io.zonarosa.messenger.components.settings.app.subscription.DonationEvent
...
class GiftFlowViewModel(
  ...
  private val store = RxStore(
    GiftFlowState(
      currency = ZonaRosaStore.inAppPayments.getOneTimeCurrency()
    )
  )
  ...
  fun refresh() {
    disposables.clear()
    disposables += ZonaRosaStore.inAppPayments.observableOneTimeCurrency.subscribe { currency ->
      store.update {
        it.copy(
          currency = currency
        )
