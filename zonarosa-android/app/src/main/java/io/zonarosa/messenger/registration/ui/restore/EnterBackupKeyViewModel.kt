...
class EnterBackupKeyViewModel : ViewModel() {
  ...
  fun handleRegistrationFailure(registerAccountResult: RegisterAccountResult) {
    store.update {
      if (it.isRegistering) {
        ...
        if (incorrectKeyError && ZonaRosaStore.account.restoredAccountEntropyPool) {
          ZonaRosaStore.account.resetAccountEntropyPool()
          ZonaRosaStore.account.resetAciAndPniIdentityKeysAfterFailedRestore()
        }
  ...
  fun handleBackupTimestampNotRestored() {
    store.update {
      it.copy(
        showBackupTierNotRestoreError = if (ZonaRosaStore.backup.isBackupTimestampRestored) TierRestoreError.NOT_FOUND else TierRestoreError.NETWORK_ERROR
      )
    }
  }
