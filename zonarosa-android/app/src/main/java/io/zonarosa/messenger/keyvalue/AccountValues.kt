...
import io.zonarosa.messenger.crypto.MasterCipher
...
import io.zonarosa.libzonarosa.messagebackup.AccountEntropyPool as LibZonaRosaAccountEntropyPool

class AccountValues internal constructor(store: KeyValueStore, context: Context) : ZonaRosaStoreValues(store) {
  ...
  val accountEntropyPool: AccountEntropyPool
    get() {
      AEP_LOCK.withLock {
        ...
        val newAep = LibZonaRosaAccountEntropyPool.generate()
  ...
  /** The password for communicating with the ZonaRosa service. */
  ...
  /** Whether or not the user is registered with the ZonaRosa service. */
  ...
  var deviceId: Int by integerValue(KEY_DEVICE_ID, ZonaRosaServiceAddress.DEFAULT_DEVICE_ID)

  val isPrimaryDevice: Boolean
    get() = deviceId == ZonaRosaServiceAddress.DEFAULT_DEVICE_ID
  ...
  private fun clearLocalCredentials() {
    ...
    ZonaRosaDatabase.recipients.setProfileKey(self.id, newProfileKey)
  ...
  private fun migrateFromSharedPrefsV1(context: Context) {
    ...
    putString(KEY_ACI, ZonaRosaPreferences.getStringPreference(context, "pref_local_uuid", null))
    putString(KEY_E164, ZonaRosaPreferences.getStringPreference(context, "pref_local_number", null))
    putString(KEY_SERVICE_PASSWORD, ZonaRosaPreferences.getStringPreference(context, "pref_gcm_password", null))
    putBoolean(KEY_IS_REGISTERED, ZonaRosaPreferences.getBooleanPreference(context, "pref_gcm_registered", false))
    putInteger(KEY_REGISTRATION_ID, ZonaRosaPreferences.getIntegerPreference(context, "pref_local_registration_id", 0))
    putBoolean(KEY_FCM_ENABLED, !ZonaRosaPreferences.getBooleanPreference(context, "pref_gcm_disabled", false))
    putString(KEY_FCM_TOKEN, ZonaRosaPreferences.getStringPreference(context, "pref_gcm_registration_id", null))
    putInteger(KEY_FCM_TOKEN_VERSION, ZonaRosaPreferences.getIntegerPreference(context, "pref_gcm_registration_id_version", 0))
    putLong(KEY_FCM_TOKEN_LAST_SET_TIME, ZonaRosaPreferences.getLongPreference(context, "pref_gcm_registration_id_last_set_time", 0))
  }
  ...
  private fun migrateFromSharedPrefsV3(context: Context) {
    ...
    putBoolean(KEY_HAS_LINKED_DEVICES, ZonaRosaPreferences.getBooleanPreference(context, "pref_multi_device", false))
  }
