...
import io.zonarosa.core.util.Bitmask
...
import io.zonarosa.core.util.hasUnknownFields
...
import io.zonarosa.core.util.readToSingleBoolean
...
import io.zonarosa.messenger.badges.Badges.toDatabaseBadge
import io.zonarosa.messenger.badges.models.Badge
import io.zonarosa.messenger.color.MaterialColor
import io.zonarosa.messenger.color.MaterialColor.UnknownColorException
import io.zonarosa.messenger.contacts.paged.ContactSearchSortOrder
...
import io.zonarosa.messenger.conversation.colors.ChatColors.Companion.forChatColor
import io.zonarosa.messenger.conversation.colors.ChatColors.Id.Companion.forLongValue
import io.zonarosa.messenger.conversation.colors.ChatColorsMapper.getChatColors
import io.zonarosa.messenger.crypto.ProfileKeyUtil
import io.zonarosa.messenger.database.GroupTable.LegacyGroupInsertException
import io.zonarosa.messenger.database.GroupTable.ShowAsStoryState
import io.zonarosa.messenger.database.IdentityTable.VerifiedStatus
import io.zonarosa.messenger.database.RecipientTableCursorUtil.getRecipientExtras
import io.zonarosa.messenger.database.ZonaRosaDatabase.Companion.groups
import io.zonarosa.messenger.database.ZonaRosaDatabase.Companion.identities
import io.zonarosa.messenger.database.ZonaRosaDatabase.Companion.runPostSuccessfulTransaction
import io.zonarosa.messenger.database.ZonaRosaDatabase.Companion.sessions
import io.zonarosa.messenger.database.ZonaRosaDatabase.Companion.threads
...
import io.zonarosa.messenger.database.model.databaseprotos.ThreadMergeEvent
...
import io.zonarosa.messenger.groups.GroupId.V1
import io.zonarosa.messenger.groups.GroupId.V2
import io.zonarosa.messenger.groups.v2.ProfileKeySet
...
open class RecipientTable(context: Context, databaseHelper: ZonaRosaDatabase) : DatabaseTable(context, databaseHelper) {
  ...
  companion object {
    ...
    const val SEARCH_PROFILE_NAME = "search_zonarosa_profile"
    ...
    @JvmStatic
    fun maskCapabilitiesToLong(capabilities: ZonaRosaServiceProfile.Capabilities): Long {
      var value: Long = 0
  ...
  fun getAndPossiblyMerge(aci: ACI?, pni: PNI?, e164: String?, pniVerified: Boolean = false, changeSelf: Boolean = false): RecipientId {
    ...
    val e164 = e164?.let { ZonaRosaE164Util.formatAsE164(it) }
  ...
  fun applyStorageSyncContactInsert(insert: ZonaRosaContactRecord, rotateProfileKeyOnBlock: Boolean): Boolean {
    val db = writableDatabase
    ...
    if (insert.proto.identityKey.isNotEmpty() && (insert.proto.zonarosaAci != null || insert.proto.zonarosaPni != null)) {
      try {
        val serviceId: ServiceId = insert.proto.zonarosaAci ?: insert.proto.zonarosaPni!!
  ...
  fun applyStorageSyncContactUpdate(update: StorageRecordUpdate<ZonaRosaContactRecord>, rotateProfileKeyOnBlock: Boolean): Boolean {
    val db = writableDatabase
    ...
    try {
      ...
      if (update.new.proto.identityKey.isNotEmpty() && update.new.proto.zonarosaAci != null) {
        val identityKey = IdentityKey(update.new.proto.identityKey.toByteArray(), 0)
        identities.updateIdentityAfterSync(update.new.proto.zonarosaAci!!.toString(), recipientId, identityKey, StorageSyncModels.remoteToLocalIdentityStatus(update.new.proto.identityState))
      }
  ...
  fun applyStorageSyncGroupV1Insert(insert: ZonaRosaGroupV1Record) {
  ...
  fun applyStorageSyncGroupV1Update(update: StorageRecordUpdate<ZonaRosaGroupV1Record>) {
  ...
  fun applyStorageSyncGroupV2Insert(insert: ZonaRosaGroupV2Record) {
  ...
  fun applyStorageSyncGroupV2Update(update: StorageRecordUpdate<ZonaRosaGroupV2Record>) {
  ...
  fun applyStorageSyncAccountUpdate(update: StorageRecordUpdate<ZonaRosaAccountRecord>) {
  ...
  fun setCapabilities(id: RecipientId, capabilities: ZonaRosaServiceProfile.Capabilities) {
    val values = ContentValues(1).apply {
  ...
  fun markHidden(id: RecipientId, clearProfileKey: Boolean = false, showMessageRequest: Boolean = false) {
    ...
    if (updated) {
      ZonaRosaDatabase.distributionLists.removeMemberFromAllLists(id)
      ZonaRosaDatabase.messages.deleteStoriesForRecipient(id)
      rotateStorageId(id)
      AppDependencies.databaseObserver.notifyRecipientChanged(id)
      StorageSyncHelper.scheduleSyncForDataChange()
    } else {
  ...
  fun updateSelfE164(e164: String, pni: PNI) {
    ...
    try {
      ...
      val newId = getAndPossiblyMerge(aci = ZonaRosaStore.account.requireAci(), pni = pni, e164 = e164, pniVerified = true, changeSelf = true)
      ...
      ZonaRosaDatabase.pendingPniSignatureMessages.deleteAll()
  ...
  private fun markUnregisteredAndSplit(id: RecipientId, record: RecipientRecord) {
    ...
    if (!ZonaRosaE164Util.isPotentialE164(record.e164)) {
  ...
  fun splitForStorageSyncIfNecessary(aci: ACI) {
    ...
    val pni = record.pni
    val e164 = record.e164?.takeIf { ZonaRosaE164Util.isPotentialNonShortCodeE164(it) }
      ...
      when (operation) {
        ...
        is PnpOperation.SessionSwitchoverInsert -> {
          if (hadThreadMerge) {
            ...
            if (threadId != null) {
              ...
              try {
                ZonaRosaDatabase.messages.insertSessionSwitchoverEvent(operation.recipientId, threadId, event)
              } catch (e: Exception) {
                ...
                if (record.aci != null && record.aci == ZonaRosaStore.account.aci) {
                  if (pnisWithSessions.contains(ZonaRosaStore.account.pni!!)) {
                ...
                if (record.pni != null && record.pni == ZonaRosaStore.account.pni) {
                  if (pnisWithSessions.contains(ZonaRosaStore.account.pni!!)) {
                ...
                if (record.e164 != null && record.e164 == ZonaRosaStore.account.e164) {
                  if (pnisWithSessions.contains(ZonaRosaStore.account.pni!!)) {
                ...
                if (pnisWithSessions.isEmpty()) {
                  ...
                  if (pnisWithSessions.first() == ZonaRosaStore.account.pni) {
        ...
        is PnpOperation.ChangeNumberInsert -> {
          if (changeSet.id is PnpIdResolver.PnpNoopId) {
            ZonaRosaDatabase.messages.insertNumberChangeMessages(changeSet.id.recipientId)
          } else {
  ...
  private fun notSelf(e164: String?, pni: PNI?, aci: ACI?): Boolean {
    return (e164 == null || e164 != ZonaRosaStore.account.e164) &&
      (pni == null || pni != ZonaRosaStore.account.pni) &&
      (aci == null || aci != ZonaRosaStore.account.aci)
  }
  ...
  /** All e164's that are eligible for having a zonarosa link added to their system contact entry. */
  ...
  fun getZonaRosaContacts(includeSelfMode: IncludeSelfMode): Cursor {
    return getZonaRosaContacts(includeSelfMode, "$SORT_NAME, $SYSTEM_JOINED_NAME, $SEARCH_PROFILE_NAME, $USERNAME, $E164")
  }

  fun getZonaRosaContactsCount(includeSelfMode: IncludeSelfMode): Int {
    return getZonaRosaContacts(includeSelfMode).count
  }
  ...
  private fun getZonaRosaContacts(includeSelfMode: IncludeSelfMode, orderBy: String? = null): Cursor {
  ...
  fun queryZonaRosaContacts(contactSearchQuery: ContactSearchQuery): Cursor? {
  ...
  fun queryZonaRosaContactLetterHeaders(inputQuery: String, includeSelfMode: IncludeSelfMode, includePush: Boolean, includeSms: Boolean): Map<RecipientId, String> {
    val searchSelection = ContactSearchSelection.Builder()
  ...
  private fun merge(primaryId: RecipientId, secondaryId: RecipientId, newPni: PNI? = null, pniVerified: Boolean): MergeResult {
    ...
    if (threadMerge.neededMerge) {
      ...
      ZonaRosaDatabase.messages.insertThreadMergeEvent(primaryRecord.id, threadMerge.threadId, mergeEvent.build())
    }
  ...
  private fun buildContentValuesForNewUser(e164: String?, pni: PNI?, aci: ACI?, pniVerified: Boolean): ContentValues {
    ...
    val values = contentValuesOf(
      E164 to e164.nullIfBlank()?.let { ZonaRosaE164Util.formatAsE164(it) },
      ACI_COLUMN to aci?.toString(),
      PNI_COLUMN to pni?.toString(),
      PNI_SIGNATURE_VERIFIED to pniVerified.toInt(),
  ...
  private fun getValuesForStorageContact(contact: ZonaRosaContactRecord, isInsert: Boolean): ContentValues {
    return ContentValues().apply {
      ...
      put(ACI_COLUMN, contact.proto.zonarosaAci?.toString())
      put(PNI_COLUMN, contact.proto.zonarosaPni?.toString())
      put(E164, contact.proto.e164.nullIfBlank()?.let { ZonaRosaE164Util.formatAsE164(it) })
      put(PROFILE_GIVEN_NAME, profileName.givenName)
      put(PROFILE_FAMILY_NAME, profileName.familyName)
      put(PROFILE_JOINED_NAME, profileName.toString())
      ...
      } else if (contact.proto.zonarosaAci != null) {
      ...
      if (ZonaRosaStore.account.isLinkedDevice) {
        val avatarColor = StorageSyncModels.remoteToLocalAvatarColor(contact.proto.avatarColor) ?: AvatarColorHash.forAddress(contact.proto.zonarosaAci ?: contact.proto.zonarosaPni, contact.proto.e164)
        put(AVATAR_COLOR, avatarColor.serialize())
      } else if (isInsert) {
        put(AVATAR_COLOR, AvatarColorHash.forAddress(contact.proto.zonarosaAci ?: contact.proto.zonarosaPni, contact.proto.e164).serialize())
      }
    }
  }

  private fun getValuesForStorageGroupV1(groupV1: ZonaRosaGroupV1Record, isInsert: Boolean): ContentValues {
    return ContentValues().apply {
  ...
  private fun getValuesForStorageGroupV2(groupV2: ZonaRosaGroupV2Record, isInsert: Boolean): ContentValues {
    return ContentValues().apply {
  ...
  fun updatePendingSelfData(selfId: RecipientId) {
    ZonaRosaDatabase.messages.updatePendingSelfData(RecipientId.from(PLACEHOLDER_SELF_ID), selfId)
  ...
  private fun updateProfileValuesForMerge(values: ContentValues, record: RecipientRecord) {
    values.apply {
      ...
      putNull(EXPIRING_PROFILE_KEY_CREDENTIAL)
      put(PROFILE_AVATAR, record.zonarosaProfileAvatar)
      put(PROFILE_GIVEN_NAME, record.zonarosaProfileName.givenName)
      put(PROFILE_FAMILY_NAME, record.zonarosaProfileName.familyName)
      put(PROFILE_JOINED_NAME, record.zonarosaProfileName.toString())
    }
  }
  ...
  internal class ContactSearchSelection private constructor(val where: String, val args: Array<String>) {
      ...
      fun build(): ContactSearchSelection {
        ...
        if (includeRegistered) {
          ...
          if (Util.isEmpty(searchQuery)) {
            stringBuilder.append(ZONAROSA_CONTACT)
          } else {
            stringBuilder.append(QUERY_ZONAROSA_CONTACT)
        ...
        if (includeNonRegistered) {
          ...
          if (Util.isEmpty(searchQuery)) {
            stringBuilder.append(NON_ZONAROSA_CONTACT)
          } else {
            stringBuilder.append(QUERY_NON_ZONAROSA_CONTACT)
            args.add(searchQuery)
            args.add(searchQuery)
            args.add(searchQuery)
          }
    ...
    companion object {
      ...
      const val NON_ZONAROSA_CONTACT = "$REGISTERED != ? AND $SYSTEM_CONTACT_URI NOT NULL AND ($E164 NOT NULL OR $EMAIL NOT NULL)"
      val QUERY_NON_ZONAROSA_CONTACT = "$NON_ZONAROSA_CONTACT AND ($E164_SEARCH OR $EMAIL GLOB ? OR $SYSTEM_JOINED_NAME GLOB ?)"
      const val ZONAROSA_CONTACT = "$REGISTERED = ? AND (NULLIF($SYSTEM_JOINED_NAME, '') NOT NULL OR $PROFILE_SHARING = ?) AND ($SORT_NAME NOT NULL OR $USERNAME NOT NULL)"
      val QUERY_ZONAROSA_CONTACT = "$ZONAROSA_CONTACT AND ($E164_SEARCH OR $SORT_NAME GLOB ? OR $USERNAME GLOB ?)"
