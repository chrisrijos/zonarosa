...
import io.zonarosa.messenger.util.storage.FileStorage
import java.io.File
...
internal class WallpaperStorageMigrationJob(parameters: Parameters = Parameters.Builder().build()) : MigrationJob(parameters) {
  ...
  override fun performMigration() {
    ...
    val currentDefaultWallpaperUri = ZonaRosaStore.wallpaper.currentRawWallpaper?.file_?.uri
    ...
    for (filename in wallpaperFileNames) {
      ...
      val inputStream = try {
        ...
        val updatedUserCount = ZonaRosaDatabase.recipients.migrateWallpaperUri(
          legacyUri = legacyUri,
          newUri = null
        )
      ...
      val wallpaperAttachmentId = ZonaRosaDatabase.attachments.insertWallpaper(inputStream)
      ...
      if (currentDefaultWallpaperUri == legacyUri.toString()) {
        ...
        ZonaRosaStore.wallpaper.setRawWallpaperForMigration(Wallpaper(file_ = Wallpaper.File(uri = newUri.toString())))
      }
