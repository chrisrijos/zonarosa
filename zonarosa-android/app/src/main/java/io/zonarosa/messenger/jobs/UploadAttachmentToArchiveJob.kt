...
import io.zonarosa.messenger.jobs.protos.UploadAttachmentToArchiveJobData
...
import io.zonarosa.service.api.archive.ArchiveMediaUploadFormStatusCodes
...
class UploadAttachmentToArchiveJob private constructor(
  ...
  override fun onAdded() {
    ...
    if (transferStatus == AttachmentTable.ArchiveTransferState.NONE) {
      ...
      ArchiveDatabaseExecutor.runBlocking {
        ZonaRosaDatabase.attachments.setArchiveTransferStateUnlessPermanentFailure(attachmentId, AttachmentTable.ArchiveTransferState.UPLOAD_IN_PROGRESS)
      }
    }
  }
  ...
  override fun run(): Result {
    ...
    progressServiceController.use {
      val uploadResult: AttachmentUploadResult = attachmentStream.use { managedAttachmentStream ->
        when (val result = ZonaRosaNetwork.attachments.uploadAttachmentV4(managedAttachmentStream)) {
          ...
          is NetworkResult.NetworkError -> {
            ...
            if (result.exception.cause is ProtocolException) {
              ...
              val actualLength = ZonaRosaDatabase.attachments.getAttachmentStream(attachmentId, 0)
                .use { it.readLength() }
              if (actualLength != attachment.size) {
                ...
                ArchiveDatabaseExecutor.runBlocking {
                  ZonaRosaDatabase.attachments.updateAttachmentLength(attachmentId, actualLength)
                }
              } else {
      ...
      ArchiveDatabaseExecutor.runBlocking {
        ZonaRosaDatabase.attachments.finalizeAttachmentAfterUpload(attachment.attachmentId, uploadResult)
      }
    }
  ...
  override fun onFailure() {
    if (this.isCanceled) {
      ...
      ArchiveDatabaseExecutor.runBlocking {
        ZonaRosaDatabase.attachments.setArchiveTransferStateFailure(attachmentId, AttachmentTable.ArchiveTransferState.NONE)
      }
    } else {
  ...
  private fun fetchResumableUploadSpec(key: ByteArray, iv: ByteArray): Pair<ResumableUpload?, Result?> {
    val uploadSpec = BackupRepository
      ...
      .then { form -> ZonaRosaNetwork.attachments.getResumableUploadSpec(key, iv, form) }
