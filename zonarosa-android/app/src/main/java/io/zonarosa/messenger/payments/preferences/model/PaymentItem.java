...
import io.zonarosa.messenger.payments.ReconstructedPayment;
import io.zonarosa.messenger.payments.State;
import io.zonarosa.messenger.payments.preferences.PaymentType;
import io.zonarosa.messenger.payments.preferences.details.PaymentDetailsParcelable;
...
import static io.zonarosa.messenger.util.viewholders.RecipientMappingModel.RecipientIdMappingModel;
...
public final class PaymentItem implements MappingModel<PaymentItem> {
  ...
  public @NonNull CharSequence getDate(@NonNull Context context) {
    ...
    if (payment.getState() == State.FAILED) {
      return SpanUtil.color(ContextCompat.getColor(context, R.color.zonarosa_alert_primary), context.getString(R.string.PaymentsHomeFragment__payment_failed));
    }
  ...
  public @ColorRes int getAmountColor() {
    if (isInProgress()) {
      return R.color.zonarosa_text_primary_disabled;
    } else if (payment.getState() == State.FAILED) {
      return R.color.zonarosa_text_secondary;
      ...
      return R.color.zonarosa_text_primary;
    }
  }
