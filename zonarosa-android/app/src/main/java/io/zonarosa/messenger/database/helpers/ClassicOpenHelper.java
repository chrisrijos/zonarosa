...
import io.zonarosa.messenger.util.DelimiterUtil;
...
public class ClassicOpenHelper extends SQLiteOpenHelper {
                                        ...
                                        LegacyMigrationJob.DatabaseUpgradeListener listener)
  {
    ...
    if (fromVersion < LegacyMigrationJob.NO_MORE_KEY_EXCHANGE_PREFIX_VERSION) {
      String KEY_EXCHANGE             = "?ZonaRosaKeyExchange";
      String PROCESSED_KEY_EXCHANGE   = "?ZonaRosaKeyExchangd";
      String STALE_KEY_EXCHANGE       = "?ZonaRosaKeyExchangs";
      int ROW_LIMIT                   = 500;
...
//    DecryptingQueue.schedulePendingDecrypts(context, masterSecret);
  ...
  public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
    ...
    if (oldVersion < INTRODUCED_NEW_TYPES_VERSION) {
      String KEY_EXCHANGE             = "?ZonaRosaKeyExchange";
      String SYMMETRIC_ENCRYPT        = "?ZonaRosaLocalEncrypt";
      String ASYMMETRIC_ENCRYPT       = "?ZonaRosaAsymmetricEncrypt";
      String ASYMMETRIC_LOCAL_ENCRYPT = "?ZonaRosaAsymmetricLocalEncrypt";
      String PROCESSED_KEY_EXCHANGE   = "?ZonaRosaKeyExchangd";
      String STALE_KEY_EXCHANGE       = "?ZonaRosaKeyExchangs";
    ...
    if (oldVersion < NO_MORE_CANONICAL_ADDRESS_DATABASE && isValidNumber(ZonaRosaPreferences.getStringPreference(context, "pref_local_number", null))) {
      ...
      NumberMigrator    numberMigrator                 = new NumberMigrator(ZonaRosaPreferences.getStringPreference(context, "pref_local_number", null));
    ...
    if (oldVersion < NO_MORE_RECIPIENTS_PLURAL) {
      ...
      while (cursor != null && cursor.moveToNext()) {
        ...
        if (addressList.length == 1) {
          ...
          members.add(DelimiterUtil.escape(ZonaRosaPreferences.getStringPreference(context, "pref_local_number", null), ','));

          Collections.sort(members);

          String        encodedGroupId = "__zonarosa_mms_group__!" + Hex.toStringCondensed(groupId);
    ...
    if (oldVersion < INTERNAL_DIRECTORY) {
      ...
      if (isValidNumber(ZonaRosaPreferences.getStringPreference(context, "pref_local_number", null))) {
        ...
        while (cursor != null && cursor.moveToNext()) {
          String        address       = new NumberMigrator(ZonaRosaPreferences.getStringPreference(context, "pref_local_number", null)).migrate(cursor.getString(0));
          ContentValues contentValues = new ContentValues(1);
    ...
    if (oldVersion < PROFILES) {
      ...
      db.execSQL("ALTER TABLE recipient_preferences ADD COLUMN zonarosa_profile_name TEXT DEFAULT NULL");
      db.execSQL("ALTER TABLE recipient_preferences ADD COLUMN zonarosa_profile_avatar TEXT DEFAULT NULL");
    }
  ...
  private static class NumberMigrator {
    ...
    public String migrate(@Nullable String number) {
      ...
      if (number.startsWith("__zonarosa_group__!")) return number;
  ...
  private static class OldDirectoryDatabaseHelper extends SQLiteOpenHelper {
    ...
    private static final String DATABASE_NAME    = "zonarosa_directory.db";
