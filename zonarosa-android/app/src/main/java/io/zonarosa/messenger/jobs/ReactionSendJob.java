...
public class ReactionSendJob extends BaseJob {
  ...
  {
    MessageRecord message = ZonaRosaDatabase.messages().getMessageRecord(messageId.getId());
  ...
  protected void onRun() throws Exception {
    ...
    ReactionTable reactionTable = ZonaRosaDatabase.reactions();
    MessageRecord message       = ZonaRosaDatabase.messages().getMessageRecord(messageId.getId());

    Recipient targetAuthor        = message.getFromRecipient();
    long      targetSentTimestamp = message.getDateSent();

    if (targetAuthor.getId().equals(ZonaRosaStore.releaseChannel().getReleaseChannelRecipientId())) {
      return;
    }
      ...
      throws IOException, UntrustedIdentityException
  {
    ...
    if (includesSelf) {
      results.add(AppDependencies.getZonaRosaServiceMessageSender().sendSyncMessage(dataMessage));
    }
  ...
  private static ZonaRosaServiceDataMessage.Reaction buildReaction(@NonNull Context context,
                                                                 @NonNull ReactionRecord reaction,
                                                                 boolean remove,
      ...
      throws IOException
  {
    return new ZonaRosaServiceDataMessage.Reaction(reaction.getEmoji(),
                                                 remove,
