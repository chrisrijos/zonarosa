 ...
 */
class ZonaRosaContextMenu private constructor(
  val anchor: View,
  ...
  LayoutInflater.from(anchor.context).inflate(R.layout.zonarosa_context_menu, null),
  ...
  private val contextMenuList = ContextMenuList(
    recyclerView = contentView.findViewById(R.id.zonarosa_context_menu_list),
    onItemClick = { dismiss() }
  )

  init {
    setBackgroundDrawable(ContextCompat.getDrawable(context, R.drawable.zonarosa_context_menu_background))
  ...
  private fun show(): ZonaRosaContextMenu {
  ...
  class Builder(
    ...
    fun show(items: List<ActionItem>): ZonaRosaContextMenu {
      return ZonaRosaContextMenu(
        anchor = anchor,
        container = container,
        items = items,
        baseOffsetX = offsetX,
