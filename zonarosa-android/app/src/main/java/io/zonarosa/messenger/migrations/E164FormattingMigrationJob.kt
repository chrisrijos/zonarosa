...
import io.zonarosa.messenger.database.RecipientTable.Companion.ACI_COLUMN
import io.zonarosa.messenger.database.RecipientTable.Companion.E164
import io.zonarosa.messenger.database.RecipientTable.Companion.ID
import io.zonarosa.messenger.database.RecipientTable.Companion.PNI_COLUMN
import io.zonarosa.messenger.database.RecipientTable.Companion.TABLE_NAME
...
internal class E164FormattingMigrationJob(
  ...
  companion object {
    ...
    fun fixE164Formatting() {
      ...
      val e164sByRecipientId = ZonaRosaDatabase.recipients.getAllE164sByRecipientId()
      val invalidE164s: MutableList<RecipientId> = mutableListOf()
      stopwatch.split("fetch")

      for ((id, e164) in e164sByRecipientId) {
        val formattedE164: String? = ZonaRosaE164Util.formatNonShortCodeAsE164(e164)
        ...
        try {
          ...
          ZonaRosaDatabase.recipients.setE164(id, formattedE164)
          ...
          val existing: Optional<RecipientId> = ZonaRosaDatabase.recipients.getByE164(formattedE164)
          if (existing.isPresent) {
            Log.w(TAG, "Merging ${existing.get()} and $id", true)
            ZonaRosaDatabase.rawDatabase.withinTransaction {
              ZonaRosaDatabase.recipients.mergeForMigration(existing.get(), id)
            }
    ...
    private fun attemptToGetRidOfE164(entries: List<RecipientId>): Int {
      ...
      for (id in entries) {
        if (ZonaRosaDatabase.recipients.removeE164IfAnotherIdentifierIsPresent(id)) {
        ...
        if (ZonaRosaDatabase.recipients.deleteRecipientIfItHasNoMessages(id)) {
