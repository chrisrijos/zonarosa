...
import io.zonarosa.messenger.webrtc.audio.AudioDeviceMapping
...
class WebRtcAudioPicker31(private val audioOutputChangedListener: OnAudioOutputChangedListener, private val outputState: ToggleButtonOutputState, private val stateUpdater: AudioStateUpdater) {
  ...
  companion object {
    ...
    private fun WebRtcAudioOutput.toZonaRosaAudioDevice(): ZonaRosaAudioManager.AudioDevice {
      return when (this) {
        WebRtcAudioOutput.HANDSET -> ZonaRosaAudioManager.AudioDevice.EARPIECE
        WebRtcAudioOutput.SPEAKER -> ZonaRosaAudioManager.AudioDevice.SPEAKER_PHONE
        WebRtcAudioOutput.BLUETOOTH_HEADSET -> ZonaRosaAudioManager.AudioDevice.BLUETOOTH
        WebRtcAudioOutput.WIRED_HEADSET -> ZonaRosaAudioManager.AudioDevice.WIRED_HEADSET
      }
    }
  }
  ...
  fun showPicker(fragmentActivity: FragmentActivity, threshold: Int, onDismiss: (DialogInterface) -> Unit): DialogInterface? {
    ...
    val devices: List<AudioOutputOption> = am.availableCommunicationDevices.map { AudioOutputOption(it.toFriendlyName(fragmentActivity).toString(), AudioDeviceMapping.fromPlatformType(it.type), it.id) }.distinctBy { it.deviceType.name }.filterNot { it.deviceType == ZonaRosaAudioManager.AudioDevice.NONE }
  ...
  fun Picker(threshold: Int) {
    ...
    val devices: List<AudioOutputOption> = am.availableCommunicationDevices.map { AudioOutputOption(it.toFriendlyName(context).toString(), AudioDeviceMapping.fromPlatformType(it.type), it.id) }.distinctBy { it.deviceType.name }.filterNot { it.deviceType == ZonaRosaAudioManager.AudioDevice.NONE }
    val currentDeviceId = am.communicationDevice?.id ?: -1
    if (devices.size < threshold) {
      LaunchedEffect(Unit) {
        Log.d(TAG, "Only found $devices devices, not showing picker.")
  ...
  fun cycleToNextDevice() {
    ...
    val devices: List<AudioOutputOption> = am.availableCommunicationDevices
      ...
      .filterNot { it.deviceType == ZonaRosaAudioManager.AudioDevice.NONE }
    ...
    if (index != -1) {
      ...
      val targetDeviceType = nextOutput.toZonaRosaAudioDevice()
  ...
  private fun AudioOutputOption.toWebRtcAudioOutput(): WebRtcAudioOutput {
    return when (this.deviceType) {
      ZonaRosaAudioManager.AudioDevice.WIRED_HEADSET -> WebRtcAudioOutput.WIRED_HEADSET
      ZonaRosaAudioManager.AudioDevice.EARPIECE -> WebRtcAudioOutput.HANDSET
      ZonaRosaAudioManager.AudioDevice.BLUETOOTH -> WebRtcAudioOutput.BLUETOOTH_HEADSET
      ZonaRosaAudioManager.AudioDevice.SPEAKER_PHONE, ZonaRosaAudioManager.AudioDevice.NONE -> WebRtcAudioOutput.SPEAKER
    }
  }
}
