...
import io.zonarosa.messenger.jobs.DeprecatedNotificationJob
import io.zonarosa.messenger.keyvalue.protos.LeastActiveLinkedDevice

class MiscellaneousValues internal constructor(store: KeyValueStore) : ZonaRosaStoreValues(store) {
  companion object {
  ...
  /**
   * The last time we ran an account consistency check via [io.zonarosa.messenger.jobs.AccountConsistencyWorkerJob]
   */
   ...
   *
   * estimatedServerTime = System.currentTimeMillis() - ZonaRosaStore.misc.getLastKnownServerTimeOffset()
   */
  ...
  /**
   * The last time we prompted the user regarding a [io.zonarosa.messenger.util.ConnectivityWarning].
   */
