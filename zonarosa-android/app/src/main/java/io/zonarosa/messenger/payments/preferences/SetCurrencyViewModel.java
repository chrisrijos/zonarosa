...
import io.zonarosa.messenger.components.settings.SettingProgress;
import io.zonarosa.messenger.components.settings.SingleSelectSetting;
...
public final class SetCurrencyViewModel extends ViewModel {
  ...
  public SetCurrencyViewModel(@NonNull CurrencyExchangeRepository currencyExchangeRepository) {
    this.store = new Store<>(new SetCurrencyState(ZonaRosaStore.payments().currentCurrency()));
    this.list  = Transformations.map(this.store.getStateLiveData(), this::createListState);

    this.store.update(ZonaRosaStore.payments().liveCurrentCurrency(), (currency, state) -> state.updateCurrentCurrency(currency));
  ...
  public void select(@NonNull Currency selection) {
    ZonaRosaStore.payments().setCurrentCurrency(selection);
  }
