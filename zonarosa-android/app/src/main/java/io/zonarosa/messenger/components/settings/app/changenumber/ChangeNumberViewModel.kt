...
class ChangeNumberViewModel : ViewModel() {
  ...
  private val serialContext = ZonaRosaExecutors.SERIAL.asCoroutineDispatcher()
  private val smsRetrieverReceiver: SmsRetrieverReceiver = SmsRetrieverReceiver(AppDependencies.application)

  private val initialLocalNumber = ZonaRosaStore.account.e164
  private val password = ZonaRosaStore.account.servicePassword!!
  ...
  init {
    try {
      val countryCode: Int = PhoneNumberUtil.getInstance()
        .parse(ZonaRosaStore.account.e164!!, null)
        .countryCode
  ...
  fun reattemptChangeLocalNumber(onSuccess: () -> Unit, onError: (Throwable) -> Unit) {
    require(ZonaRosaStore.misc.pendingChangeNumberMetadata != null)
    ...
    viewModelScope.launch(Dispatchers.IO) {
      try {
        ...
        val remoteMatchesLocal = remoteE164 == ZonaRosaStore.account.requireE164()
  ...
  private suspend fun changeNumberWithRecoveryPassword(): ChangeLocalNumberOutcome {
    ...
    ZonaRosaStore.svr.recoveryPassword?.let { recoveryPassword ->
  ...
  private suspend fun getRegistrationData(context: Context): RegistrationData {
    ...
    val recoveryPassword = if (currentState.sessionId == null) ZonaRosaStore.svr.recoveryPassword else null
