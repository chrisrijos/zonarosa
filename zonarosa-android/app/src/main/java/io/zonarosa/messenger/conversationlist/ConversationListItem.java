...
import io.zonarosa.messenger.BindableConversationListItem;
import io.zonarosa.messenger.OverlayTransformation;
import io.zonarosa.messenger.R;
import io.zonarosa.messenger.Unbindable;
...
import io.zonarosa.messenger.components.TypingIndicatorView;
...
import io.zonarosa.messenger.database.model.LiveUpdateMessage;
import io.zonarosa.messenger.database.model.MessageRecord;
import io.zonarosa.messenger.database.model.ThreadRecord;
import io.zonarosa.messenger.database.model.UpdateDescription;
import io.zonarosa.messenger.fonts.ZonaRosaSymbols.Glyph;
import io.zonarosa.messenger.glide.targets.GlideLiveDataTarget;
...
import static io.zonarosa.messenger.database.model.LiveUpdateMessage.recipientToStringAsync;
...
public final class ConversationListItem extends ConstraintLayout implements BindableConversationListItem, Unbindable {
  ...
  protected void onFinishInflate() {
    ...
    this.searchStyleFactory      = () -> new CharacterStyle[] { new ForegroundColorSpan(ContextCompat.getColor(getContext(), io.zonarosa.core.ui.R.color.zonarosa_colorOnSurface)), SpanUtil.getBoldSpan() };
                         ...
                         long activeThreadId)
  {
    ...
    if (appendSystemContactIcon && recipient.get().isSystemContact() && !recipient.get().getShowVerified()) {
      ...
      drawable.setTint(ContextCompat.getColor(getContext(), io.zonarosa.core.ui.R.color.zonarosa_colorOnSurface));
    ...
    if (thread.getDate() > 0) {
      ...
      dateView.setTextColor(thread.isRead() ? ContextCompat.getColor(getContext(), R.color.zonarosa_text_secondary)
                                            : ContextCompat.getColor(getContext(), R.color.zonarosa_text_primary));
  ...
  {
    int defaultTint = ContextCompat.getColor(context, R.color.zonarosa_text_secondary);
    ...
    if (!thread.isMessageRequestAccepted()) {
      ...
      return emphasisAdded(recipientToStringAsync(thread.getRecipient().getId(), r -> new SpannableString(context.getString(R.string.ThreadRecord_s_is_on_zonarosa, r.getDisplayName(context)))));
      ...
      if (thread.getRecipient().getE164().isPresent()) {
        return emphasisAdded(context, context.getString(R.string.ThreadRecord_s_belongs_to_s, ZonaRosaE164Util.prettyPrint(thread.getRecipient().requireE164()), thread.getRecipient().getDisplayName(context)), defaultTint);
      } else {
        return emphasisAdded(context, context.getString(R.string.ThreadRecord_safety_number_changed), defaultTint);
      }
