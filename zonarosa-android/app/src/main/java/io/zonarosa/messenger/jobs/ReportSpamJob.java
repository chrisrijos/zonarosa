...
import io.zonarosa.messenger.database.MessageTable.ReportSpamData;
...
public class ReportSpamJob extends BaseJob {
  ...
  public void onRun() throws IOException {
    ...
    Recipient threadRecipient = ZonaRosaDatabase.threads().getRecipientForThreadId(threadId);
    ...
    if (threadRecipient.isGroup()) {
      Recipient inviter = ZonaRosaDatabase.groups().getGroupInviter(threadRecipient.requireGroupId());
      if (inviter == null) {
        Log.w(TAG, "Unable to determine inviter to report");
        return;
      }

      reportSpamData = ZonaRosaDatabase.messages().getGroupReportSpamMessageServerData(threadId, inviter.getId(), timestamp, MAX_MESSAGE_COUNT);
    } else {
      reportSpamData = ZonaRosaDatabase.messages().getReportSpamMessageServerData(threadId, timestamp, MAX_MESSAGE_COUNT);
    }
    ...
    for (ReportSpamData data : reportSpamData) {
      ...
      if (serviceId.isPresent() && !serviceId.get().isUnknown()) {
        ...
        byte[] reportingTokenBytes = ZonaRosaDatabase.recipients().getReportingToken(recipientId);
        if (reportingTokenBytes != null) {
          reportingTokenEncoded = Base64.encodeWithPadding(reportingTokenBytes);
        }

        NetworkResultUtil.toBasicLegacy(ZonaRosaNetwork.message().reportSpam(serviceId.get(), data.getServerGuid(), reportingTokenEncoded));
        count++;
      } else {
