...
import io.zonarosa.messenger.jobs.protos.PreKeysSyncJobData
import io.zonarosa.messenger.keyvalue.ZonaRosaStore
import io.zonarosa.messenger.net.ZonaRosaNetwork
import io.zonarosa.messenger.util.RemoteConfig
import io.zonarosa.messenger.util.isRetryableIOException
...
class PreKeysSyncJob private constructor(
  ...
  companion object {
    ...
    @JvmStatic
    fun enqueueIfNeeded() {
      if (!ZonaRosaStore.account.aciPreKeys.isSignedPreKeyRegistered || !ZonaRosaStore.account.pniPreKeys.isSignedPreKeyRegistered) {
        Log.i(TAG, "Some signed/last-resort prekeys aren't registered yet. Enqueuing a job. ACI: ${ZonaRosaStore.account.aciPreKeys.isSignedPreKeyRegistered} PNI: ${ZonaRosaStore.account.pniPreKeys.isSignedPreKeyRegistered}")
        AppDependencies.jobManager.add(PreKeysSyncJob())
      } else if (ZonaRosaStore.account.aciPreKeys.activeSignedPreKeyId < 0 || ZonaRosaStore.account.pniPreKeys.activeSignedPreKeyId < 0) {
        Log.i(TAG, "Some signed prekeys aren't active yet. Enqueuing a job. ACI: ${ZonaRosaStore.account.aciPreKeys.activeSignedPreKeyId >= 0} PNI: ${ZonaRosaStore.account.pniPreKeys.activeSignedPreKeyId >= 0}")
        AppDependencies.jobManager.add(PreKeysSyncJob())
      } else if (ZonaRosaStore.account.aciPreKeys.lastResortKyberPreKeyId < 0 || ZonaRosaStore.account.pniPreKeys.lastResortKyberPreKeyId < 0) {
        Log.i(TAG, "Some last-resort kyber prekeys aren't active yet. Enqueuing a job. ACI: ${ZonaRosaStore.account.aciPreKeys.lastResortKyberPreKeyId >= 0} PNI: ${ZonaRosaStore.account.pniPreKeys.lastResortKyberPreKeyId >= 0}")
        AppDependencies.jobManager.add(PreKeysSyncJob())
      } else {
        val timeSinceLastFullRefresh = System.currentTimeMillis() - ZonaRosaStore.misc.lastFullPrekeyRefreshTime
  ...
  override fun onRun() {
    if (!ZonaRosaStore.account.isRegistered || ZonaRosaStore.account.aci == null || ZonaRosaStore.account.pni == null) {
    ...
    val forceRotation = if (forceRotationRequested) {
      ...
      warn(TAG, ServiceIdType.ACI, "Active Signed EC: ${ZonaRosaStore.account.aciPreKeys.activeSignedPreKeyId}, Last Resort Kyber: ${ZonaRosaStore.account.aciPreKeys.lastResortKyberPreKeyId}")
      warn(TAG, ServiceIdType.PNI, "Active Signed EC: ${ZonaRosaStore.account.pniPreKeys.activeSignedPreKeyId}, Last Resort Kyber: ${ZonaRosaStore.account.pniPreKeys.lastResortKyberPreKeyId}")

      if (!checkPreKeyConsistency(ServiceIdType.ACI, AppDependencies.protocolStore.aci(), ZonaRosaStore.account.aciPreKeys)) {
        warn(TAG, ServiceIdType.ACI, "Prekey consistency check failed! Must rotate keys!")
        true
      } else if (!checkPreKeyConsistency(ServiceIdType.PNI, AppDependencies.protocolStore.pni(), ZonaRosaStore.account.pniPreKeys)) {
        ...
        val timeSinceLastForcedRotation = System.currentTimeMillis() - ZonaRosaStore.misc.lastForcedPreKeyRefresh
    ...
    if (forceRotation) {
      ...
      warn(TAG, "Forced prekey rotation was requested, but we already did a forced refresh ${System.currentTimeMillis() - ZonaRosaStore.misc.lastForcedPreKeyRefresh} ms ago. Ignoring.")
    }

    syncPreKeys(ServiceIdType.ACI, ZonaRosaStore.account.aci, AppDependencies.protocolStore.aci(), ZonaRosaStore.account.aciPreKeys, forceRotation)
    syncPreKeys(ServiceIdType.PNI, ZonaRosaStore.account.pni, AppDependencies.protocolStore.pni(), ZonaRosaStore.account.pniPreKeys, forceRotation)
    ZonaRosaStore.misc.lastFullPrekeyRefreshTime = System.currentTimeMillis()

    if (forceRotation) {
      ZonaRosaStore.misc.lastForcedPreKeyRefresh = System.currentTimeMillis()
    }
  }

  private fun syncPreKeys(serviceIdType: ServiceIdType, serviceId: ServiceId?, protocolStore: ZonaRosaServiceAccountDataStore, metadataStore: PreKeyMetadataStore, forceRotation: Boolean) {
    if (serviceId == null) {
      warn(TAG, serviceIdType, "AccountId not set!")
      return
    }

    val availablePreKeyCounts = ZonaRosaNetwork.keys.getAvailablePreKeyCounts(serviceIdType).successOrThrow()
  ...
  private fun signedPreKeyUploadIfNeeded(serviceIdType: ServiceIdType, protocolStore: ZonaRosaProtocolStore, metadataStore: PreKeyMetadataStore, forceRotation: Boolean): SignedPreKeyRecord? {
  ...
  private fun lastResortKyberPreKeyUploadIfNeeded(serviceIdType: ServiceIdType, protocolStore: ZonaRosaServiceAccountDataStore, metadataStore: PreKeyMetadataStore, forceRotation: Boolean): KyberPreKeyRecord? {
  ...
  private fun checkPreKeyConsistency(serviceIdType: ServiceIdType, protocolStore: ZonaRosaServiceAccountDataStore, metadataStore: PreKeyMetadataStore): Boolean {
    val result: NetworkResult<Unit> = try {
      ZonaRosaNetwork.keys.checkRepeatedUseKeys(
        serviceIdType = serviceIdType,
