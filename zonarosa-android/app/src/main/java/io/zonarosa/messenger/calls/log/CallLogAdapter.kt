...
import io.zonarosa.messenger.databinding.CallLogAdapterItemBinding
import io.zonarosa.messenger.databinding.CallLogCreateCallLinkItemBinding
import io.zonarosa.messenger.databinding.ConversationListItemClearFilterBinding
import io.zonarosa.messenger.databinding.ConversationListItemClearFilterEmptyBinding
...
import io.zonarosa.messenger.util.setRelativeDrawables
  ...
  private class CallLinkModelViewHolder(
    ...
    override fun bind(model: CallLinkModel) {
      ...
      val callLinkName = model.callLink.record.state.name.takeIf { it.isNotEmpty() }
        ?: context.getString(R.string.WebRtcCallView__zonarosa_call)

      binding.callRecipientName.text = SearchUtil.getHighlightedSpan(
        Locale.getDefault(),
        { arrayOf(TextAppearanceSpan(context, CoreUiR.style.ZonaRosa_Text_TitleSmall)) },
        callLinkName,
      ...
      TextViewCompat.setCompoundDrawableTintList(
        ...
        ColorStateList.valueOf(
          ContextCompat.getColor(context, CoreUiR.color.zonarosa_colorOnSurfaceVariant)
        )
      )
    ...
    private fun presentCallInfo(call: CallLogRow.Call, date: Long) {
      ...
      val color = ContextCompat.getColor(
        ...
        if (call.record.isDisplayedAsMissedCallInUi) {
          CoreUiR.color.zonarosa_colorError
        } else {
          CoreUiR.color.zonarosa_colorOnSurfaceVariant
        }
      )
