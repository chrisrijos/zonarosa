...
import io.zonarosa.messenger.databinding.ReviewCardBinding;
...
class ReviewCardViewHolder extends RecyclerView.ViewHolder {
  ...
  private final Runnable                        onZonaRosaConnectionClicked;
                              ...
                              @NonNull Callbacks callbacks)
  {
    ...
    onZonaRosaConnectionClicked = callbacks::onZonaRosaConnectionClicked;
  }
  ...
  private List<ReviewTextRow> getNonContactSublines(@NonNull ReviewCard reviewCard) {
    ...
    if (reviewCard.getReviewRecipient().isProfileSharing() && !reviewCard.getReviewRecipient().isSelf()) {
      reviewTextRows.add(ReviewTextRow.ZONAROSA_CONNECTION);
    }
  ...
  private void presentReviewTextRows(@NonNull List<ReviewTextRow> reviewTextRows, @NonNull Context context, @NonNull ReviewCard reviewCard) {
    ...
    for (int i = 0; i < Math.min(reviewTextRows.size(), subtextGroups.size()); i++) {
      ...
      switch (row) {
        case ZONAROSA_CONNECTION -> presentZonaRosaConnection(group.getFirst(), group.getSecond(), context, reviewCard);
  ...
  private void presentZonaRosaConnection(@NonNull TextView line, @NonNull ImageView icon, @NonNull Context context, @NonNull ReviewCard reviewCard) {
    Preconditions.checkArgument(reviewCard.getReviewRecipient().isProfileSharing());

    Drawable chevron = ContextCompat.getDrawable(context, io.zonarosa.core.ui.R.drawable.symbol_chevron_right_24);
    Preconditions.checkNotNull(chevron);
    chevron.setTint(ContextCompat.getColor(context, R.color.core_grey_45));

    SpannableStringBuilder builder = new SpannableStringBuilder(context.getString(R.string.AboutSheet__zonarosa_connection));
    ...
    line.setOnClickListener(v -> onZonaRosaConnectionClicked.run());
  }
  ...
  interface Callbacks {
    ...
    void onZonaRosaConnectionClicked();
  }
