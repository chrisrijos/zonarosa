...
import io.zonarosa.messenger.safety.SafetyNumberRecipient;
...
public final class SafetyNumberChangeRepository {
  ...
  @NonNull LiveData<TrustAndVerifyResult> trustOrVerifyChangedRecipients(@NonNull List<ChangedRecipient> changedRecipients) {
    ...
    ZonaRosaExecutors.BOUNDED.execute(() -> liveData.postValue(trustOrVerifyChangedRecipientsInternal(changedRecipients)));
    return liveData;
  }
  ...
  @NonNull LiveData<TrustAndVerifyResult> trustOrVerifyChangedRecipientsAndResend(@NonNull List<ChangedRecipient> changedRecipients, @NonNull MessageRecord messageRecord) {
    ...
    MutableLiveData<TrustAndVerifyResult> liveData = new MutableLiveData<>();
    ZonaRosaExecutors.BOUNDED.execute(() -> liveData.postValue(trustOrVerifyChangedRecipientsAndResendInternal(changedRecipients, messageRecord)));
    return liveData;
  }
  ...
  private @Nullable MessageRecord getMessageRecord(Long messageId, String messageType) {
    try {
      return ZonaRosaDatabase.messages().getMessageRecord(messageId);
                                                                               ...
                                                                               @NonNull MessageRecord messageRecord)
  {
    ...
    try (ZonaRosaSessionLock.Lock unused = ReentrantSessionLock.INSTANCE.acquire()) {
      for (ChangedRecipient changedRecipient : changedRecipients) {
        ZonaRosaProtocolAddress mismatchAddress = changedRecipient.getRecipient().requireServiceId().toProtocolAddress(ZonaRosaServiceAddress.DEFAULT_DEVICE_ID);
        ...
        ZonaRosaIdentityKeyStore.SaveResult result = AppDependencies.getProtocolStore().aci().identities().saveIdentity(mismatchAddress, newIdentityKey, true);

        Log.d(TAG, "Saving identity result: " + result);
        if (result == ZonaRosaIdentityKeyStore.SaveResult.NO_CHANGE) {
          Log.i(TAG, "Archiving sessions explicitly as they appear to be out of sync.");
          AppDependencies.getProtocolStore().aci().sessions().archiveSessions(changedRecipient.getRecipient().getId(), ZonaRosaServiceAddress.DEFAULT_DEVICE_ID);
          AppDependencies.getProtocolStore().aci().sessions().archiveSiblingSessions(mismatchAddress);
          ZonaRosaDatabase.senderKeyShared().deleteAllFor(changedRecipient.getRecipient().getId());
        }
      }
    }
  ...
  private void processOutgoingMessageRecord(@NonNull List<ChangedRecipient> changedRecipients, @NonNull MessageRecord messageRecord) {
    ...
    for (ChangedRecipient changedRecipient : changedRecipients) {
      ...
      if (messageRecord.isMms()) {
        ZonaRosaDatabase.messages().removeMismatchedIdentity(messageRecord.getId(), id, identityKey);
