...
//    static final int DECRYPTIONS_DRAINED           = 80;
    ...
    static final int COPY_USERNAME_TO_ZONAROSA_STORE = 95;
...
//    static final int FIX_INACTIVE_GROUPS           = 127;
  ...
  public static void onApplicationCreate(@NonNull Context context, @NonNull JobManager jobManager) {
    if (isLegacyUpdate(context)) {
      ...
      ZonaRosaPreferences.setAppMigrationVersion(context, 0);
    }
    ...
    if (!isUpdate(context)) {
      ...
      ZonaRosaStore.misc().setClientDeprecated(false);
    }

    final int lastSeenVersion = ZonaRosaPreferences.getAppMigrationVersion(context);
    ...
    if (migrationJobs.size() > 0) {
      ...
      EventBus.getDefault().register(new Object() {
        ...
        public void onMigrationComplete(MigrationCompleteEvent event) {
          ...
          ZonaRosaPreferences.setAppMigrationVersion(context, event.getVersion());
      ...
      ZonaRosaPreferences.setAppMigrationVersion(context, CURRENT_VERSION);
      VersionTracker.updateLastSeenVersion(context);
      UI_BLOCKING_MIGRATION_RUNNING.setValue(false);
    }
  }
  ...
  public static boolean isUpdate(@NonNull Context context) {
    return isLegacyUpdate(context) || ZonaRosaPreferences.getAppMigrationVersion(context) < CURRENT_VERSION;
  }
...
//    }
    ...
    if (lastSeenVersion < Version.COPY_USERNAME_TO_ZONAROSA_STORE) {
      jobs.put(Version.COPY_USERNAME_TO_ZONAROSA_STORE, new CopyUsernameToZonaRosaStoreMigrationJob());
    }
