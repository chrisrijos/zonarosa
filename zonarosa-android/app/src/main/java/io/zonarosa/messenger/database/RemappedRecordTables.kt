...
import io.zonarosa.messenger.database.RemappedRecordTables.SharedColumns.ID
import io.zonarosa.messenger.database.RemappedRecordTables.SharedColumns.NEW_ID
import io.zonarosa.messenger.database.RemappedRecordTables.SharedColumns.OLD_ID
import io.zonarosa.messenger.recipients.RecipientId

/**
 * The backing datastore for [RemappedRecords]. See that class for more details.
 */
class RemappedRecordTables internal constructor(context: Context?, databaseHelper: ZonaRosaDatabase?) : DatabaseTable(context, databaseHelper) {
  ...
  private fun addMapping(table: String, mapping: Mapping) {
    ...
    databaseHelper.zonarosaWritableDatabase.insert(table, null, values)
  }
