...
sealed class GroupId(private val encodedId: String) : DatabaseId, Parcelable {
  ...
  companion object {
    private const val ENCODED_ZONAROSA_GROUP_V1_PREFIX = "__zonarosa_group__!"
    private const val ENCODED_ZONAROSA_GROUP_V2_PREFIX = "__zonarosa_group__v2__!"
    private const val ENCODED_MMS_GROUP_PREFIX = "__zonarosa_mms_group__!"
    ...
    fun parse(encodedGroupId: String): GroupId {
      try {
        ...
        when {
          encodedGroupId.startsWith(ENCODED_ZONAROSA_GROUP_V2_PREFIX) -> return v2(bytes)
          encodedGroupId.startsWith(ENCODED_ZONAROSA_GROUP_V1_PREFIX) -> return v1(bytes)
    ...
    fun isEncodedGroup(groupId: String): Boolean {
      return groupId.startsWith(ENCODED_ZONAROSA_GROUP_V2_PREFIX) ||
        groupId.startsWith(ENCODED_ZONAROSA_GROUP_V1_PREFIX) ||
  ...
  @Parcelize
  @Serializable
  class V1(private val v1Bytes: ByteArray) : Push(ENCODED_ZONAROSA_GROUP_V1_PREFIX, v1Bytes) {
    @Transient
    @IgnoredOnParcel
  ...
  @Parcelize
  @Serializable
  class V2(private val v2Bytes: ByteArray) : Push(ENCODED_ZONAROSA_GROUP_V2_PREFIX, v2Bytes) {
    @Transient
    @IgnoredOnParcel
