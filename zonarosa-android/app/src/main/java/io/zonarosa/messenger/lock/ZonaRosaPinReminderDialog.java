...
public final class ZonaRosaPinReminderDialog {

  private static final String TAG = Log.tag(ZonaRosaPinReminderDialog.class);
  ...
  public static void show(@NonNull Context context, @NonNull Launcher launcher, @NonNull Callback mainCallback) {
    ...
    AlertDialog dialog = new MaterialAlertDialogBuilder(context, R.style.ThemeOverlay_ZonaRosa_MaterialAlertDialog_Wide)
    ...
    ZonaRosaStore.pin().getKeyboardType().applyInputTypeTo(pinEditText);
    ...
    pinEditText.addTextChangedListener(new SimpleTextWatcher() {
      ...
      private final String localHash = Objects.requireNonNull(ZonaRosaStore.svr().getLocalPinHash());
  ...
  private static final class V2PinVerifier implements PinVerifier {
    ...
    V2PinVerifier() {
      localPinHash = ZonaRosaStore.svr().getLocalPinHash();
