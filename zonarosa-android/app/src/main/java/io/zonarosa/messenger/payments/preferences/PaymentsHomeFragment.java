...
import io.zonarosa.messenger.PaymentPreferencesDirections;
import io.zonarosa.messenger.R;
import io.zonarosa.messenger.banner.BannerManager;
import io.zonarosa.messenger.banner.banners.EnclaveFailureBanner;
import io.zonarosa.messenger.components.settings.app.AppSettingsActivity;
import io.zonarosa.messenger.help.HelpFragment;
import io.zonarosa.messenger.keyvalue.ZonaRosaStore;
import io.zonarosa.messenger.lock.v2.CreateSvrPinActivity;
import io.zonarosa.messenger.payments.FiatMoneyUtil;
import io.zonarosa.messenger.payments.MoneyView;
import io.zonarosa.messenger.payments.backup.RecoveryPhraseStates;
import io.zonarosa.messenger.payments.backup.confirm.PaymentsRecoveryPhraseConfirmFragment;
...
public class PaymentsHomeFragment extends LoggingFragment {
  ...
  public void onCreate(@Nullable Bundle savedInstanceState) {
    ...
    long    paymentLockTimestamp = ZonaRosaStore.payments().getPaymentLockTimestamp();
    boolean enablePaymentLock    = PaymentsHomeFragmentArgs.fromBundle(getArguments()).getEnablePaymentLock();
    boolean showPaymentLock      = ZonaRosaStore.payments().getPaymentLockSkipCount() < MAX_PAYMENT_LOCK_SKIP_COUNT &&
    ...
    if (enablePaymentLock && showPaymentLock) {
      ...
      ZonaRosaStore.payments().setPaymentLockTimestamp(waitUntil);
  ...
  private void setSkipCount() {
      int skipCount = ZonaRosaStore.payments().getPaymentLockSkipCount();
      ZonaRosaStore.payments().setPaymentLockSkipCount(++skipCount);
  }
  ...
  public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
    ...
    addMoney.setOnClickListener(v -> {
      ...
      } else if (ZonaRosaStore.payments().getPaymentsAvailability().isSendAllowed()) {
    ...
    viewModel.getBalance().observe(getViewLifecycleOwner(), balanceAmount -> {
      ...
      if (ZonaRosaStore.payments().getShowSaveRecoveryPhrase() &&
          !ZonaRosaStore.payments().getUserConfirmedMnemonic() &&
        ...
        ZonaRosaStore.payments().setShowSaveRecoveryPhrase(false);
      }
    });
    ...
    viewModel.getPaymentStateEvents().observe(getViewLifecycleOwner(), paymentStateEvent -> {
      ...
      switch (paymentStateEvent) {
        ...
        case DEACTIVATE_WITHOUT_BALANCE:
          builder.setPositiveButton(SpanUtil.color(ContextCompat.getColor(requireContext(), R.color.zonarosa_alert_primary),
        ...
        case DEACTIVATE_WITH_BALANCE:
          ...
          break;
        case ACTIVATED:
          if (!ZonaRosaStore.payments().isPaymentLockEnabled()) {
  ...
  private boolean onMenuItemSelected(@NonNull MenuItem item) {
    if (item.getItemId() == R.id.payments_home_fragment_menu_transfer_to_exchange) {
      ...
      SafeNavigation.safeNavigate(NavHostFragment.findNavController(this),
                                  PaymentsHomeFragmentDirections.actionPaymentsHomeToPaymentsBackup().setRecoveryPhraseState(ZonaRosaStore.payments().isMnemonicConfirmed() ?
  ...
  class HomeCallbacks implements PaymentsHomeAdapter.Callbacks {
    ...
    public void onActivatePayments() {
      new MaterialAlertDialogBuilder(requireContext())
                     .setMessage(R.string.PaymentsHomeFragment__you_can_use_zonarosa_to_send_and)
