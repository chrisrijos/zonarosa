...
public final class SenderKeyDistributionSendJob extends BaseJob {
  ...
  protected void onRun() throws Exception {
    ...
    if (threadRecipient.isPushV2Group()) {
      ...
      createGroupSendFullToken = () -> ZonaRosaDatabase.groups().getGroupSendFullToken(groupId, targetRecipientId);
    } else if (threadRecipient.isDistributionList()) {
      groupId        = null;
      distributionId = ZonaRosaDatabase.distributionLists().getDistributionId(threadRecipientId);
    } else {
    ...
    if (groupId != null && !ZonaRosaDatabase.groups().isCurrentMember(groupId, targetRecipientId)) {
      Log.w(TAG, targetRecipientId + " is no longer a member of " + groupId + "! Not sending.");
      return;
    } else if (groupId == null) {
      DistributionListRecord listRecord = ZonaRosaDatabase.distributionLists().getList(threadRecipientId);
    ...
    List<ZonaRosaServiceAddress>   address       = Collections.singletonList(RecipientUtil.toZonaRosaServiceAddress(context, targetRecipient));
