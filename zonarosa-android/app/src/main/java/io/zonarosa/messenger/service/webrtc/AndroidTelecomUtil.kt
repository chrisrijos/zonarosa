 ...
 * maintains a global list of all ZonaRosa [AndroidCallConnection]s associated with their [RecipientId].
...
object AndroidTelecomUtil {
  ...
  fun registerPhoneAccount() {
    if (Build.VERSION.SDK_INT >= 26 && !systemRejected) {
      ...
      val phoneAccount = PhoneAccount.Builder(getPhoneAccountHandle(), "ZonaRosa")
  ...
  fun selectAudioDevice(recipientId: RecipientId, device: ZonaRosaAudioManager.AudioDevice) {
    if (telecomSupported) {
      ...
      if (connection?.callAudioState != null) {
        when (device) {
          ZonaRosaAudioManager.AudioDevice.SPEAKER_PHONE -> connection.setAudioRouteIfDifferent(CallAudioState.ROUTE_SPEAKER)
          ZonaRosaAudioManager.AudioDevice.BLUETOOTH -> connection.setAudioRouteIfDifferent(CallAudioState.ROUTE_BLUETOOTH)
          ZonaRosaAudioManager.AudioDevice.WIRED_HEADSET -> connection.setAudioRouteIfDifferent(CallAudioState.ROUTE_WIRED_HEADSET)
  ...
  @Suppress("DEPRECATION")
  fun getSelectedAudioDevice(recipientId: RecipientId): ZonaRosaAudioManager.AudioDevice {
    if (telecomSupported) {
      val connection: AndroidCallConnection? = connections[recipientId]
      if (connection?.callAudioState != null) {
        return when (connection.callAudioState.route) {
          CallAudioState.ROUTE_SPEAKER -> ZonaRosaAudioManager.AudioDevice.SPEAKER_PHONE
          CallAudioState.ROUTE_BLUETOOTH -> ZonaRosaAudioManager.AudioDevice.BLUETOOTH
          CallAudioState.ROUTE_WIRED_HEADSET -> ZonaRosaAudioManager.AudioDevice.WIRED_HEADSET
          else -> ZonaRosaAudioManager.AudioDevice.EARPIECE
        }
      }
    }
  ...
  private fun isTelecomAllowedForDevice(): Boolean {
    if (RemoteConfig.internalUser) {
      return !ZonaRosaStore.internal.callingDisableTelecom
    }
