package io.zonarosa.messenger.sharing.v2
...
import io.zonarosa.messenger.conversation.mutiselect.forward.MultiselectForwardFullScreenDialogFragment
import io.zonarosa.messenger.mediasend.v2.MediaSelectionActivity.Companion.share
import io.zonarosa.messenger.recipients.RecipientId
import io.zonarosa.messenger.sharing.MultiShareDialogs
import io.zonarosa.messenger.sharing.MultiShareSender
import io.zonarosa.messenger.sharing.MultiShareSender.MultiShareSendResultCollection
import io.zonarosa.messenger.sharing.interstitial.ShareInterstitialActivity
...
class ShareActivity : PassphraseRequiredActivity(), MultiselectForwardFragment.Callback {
  ...
  override fun onCreate(savedInstanceState: Bundle?, ready: Boolean) {
    ...
    var dialog: ZonaRosaProgressDialog? = null
    viewModel
      .state
      ...
      .subscribeBy { state ->
        if (state.loadState == ShareState.ShareDataLoadState.Init) {
          dialog = ZonaRosaProgressDialog.show(this, indeterminate = true)
        } else {
          dialog?.dismiss()
          dialog = null
        }
  ...
  override fun getDialogBackgroundColor(): Int = ContextCompat.getColor(this, R.color.zonarosa_background_primary)
  ...
  private fun openMediaInterstitial(shareEvent: ShareEvent.OpenMediaInterstitial) {
    ...
    val intent = share(
      this,
      MessageSendType.ZonaRosaMessageSendType,
      media,
