package io.zonarosa.messenger.payments.create;
...
import io.zonarosa.messenger.payments.CreatePaymentDetails;
...
public class CreatePaymentViewModel extends ViewModel {
  ...
  private CreatePaymentViewModel(@NonNull PayeeParcelable payee, @Nullable CharSequence note) {
    this.payee            = payee;
    this.spendableBalance = Transformations.map(ZonaRosaStore.payments().liveMobileCoinBalance(), Balance::getTransferableAmount);
    ...
    LiveData<Optional<CurrencyExchange.ExchangeRate>> liveExchangeRate = LiveDataUtil.mapAsync(ZonaRosaStore.payments().liveCurrentCurrency(),
                                                                                               currency -> {
                                                                                                 try {
  ...
  private @NonNull InputState updateAmount(@NonNull Context context, @NonNull InputState inputState, @NonNull AmountKeyboardGlyph glyph) {
    switch (inputState.getInputTarget()) {
      case FIAT_MONEY:
        return updateFiatAmount(context, inputState, glyph, ZonaRosaStore.payments().currentCurrency());
      case MONEY:
