...
import io.zonarosa.core.util.FontUtil.canRenderEmojiAtFontSize
...
import io.zonarosa.messenger.emoji.EmojiFiles.Version.Companion.readVersion
import io.zonarosa.messenger.keyvalue.ZonaRosaStore
import io.zonarosa.messenger.net.StandardUserAgentInterceptor
import io.zonarosa.messenger.notifications.SlowNotificationHeuristics.isHavingDelayedNotifications
import io.zonarosa.messenger.recipients.Recipient.Companion.self
import io.zonarosa.messenger.service.webrtc.AndroidTelecomUtil.telecomSupported
...
import io.zonarosa.messenger.util.ScreenDensity
import io.zonarosa.messenger.util.ServiceUtil
import io.zonarosa.messenger.util.ZonaRosaPreferences
import io.zonarosa.messenger.util.VersionTracker.getDaysSinceFirstInstalled
...
class LogSectionSystemInfo : LogSection {
  ...
  override fun getContent(context: Context): CharSequence {
    return """
      ...
      OS Host           : ${Build.HOST}
      RecipientId       : ${if (ZonaRosaStore.registration.isRegistrationComplete) self().id else "N/A"}
      ACI               : ${getCensoredAci()}
      Device ID         : ${ZonaRosaStore.account.deviceId}
      Censored          : ${AppDependencies.zonarosaServiceNetworkAccess.isCensored()}
      Network Status    : ${NetworkUtil.getNetworkStatus(context)}
      Play Services     : ${getPlayServicesString(context)}
      FCM               : ${ZonaRosaStore.account.fcmEnabled}
      Locale            : ${Locale.getDefault()}
      Linked Devices    : ${ZonaRosaStore.account.isMultiDevice}
      First Version     : ${ZonaRosaPreferences.getFirstInstallVersion(context)}
      ...
      Server Time Offset: ${ZonaRosaStore.misc.lastKnownServerTimeOffset} ms (last updated: ${ZonaRosaStore.misc.lastKnownServerTimeOffsetUpdateTime})
  ...
  private fun getCensoredAci(): String {
    val aci = ZonaRosaStore.account.aci
  ...
  private fun getTimeRegistered(): String {
    if (ZonaRosaStore.account.registeredAtTimestamp <= 0) {
      return "Unknown"
    }

    val timeSince = (System.currentTimeMillis() - ZonaRosaStore.account.registeredAtTimestamp).milliseconds.toString()
    return "${ZonaRosaStore.account.registeredAtTimestamp} ($timeSince ago)"
  }
}
