...
import io.zonarosa.messenger.keyboard.emoji.EmojiCategoryMappingModel;
import io.zonarosa.messenger.keyboard.emoji.RecentsMappingModel;
import io.zonarosa.messenger.keyboard.emoji.search.EmojiSearchRepository;
...
public final class ReactWithAnyEmojiViewModel extends ViewModel {
  ...
  void onEmojiSelected(@NonNull String emoji) {
    if (messageId > 0) {
      ZonaRosaStore.emoji().setPreferredVariation(emoji);
  ...
  private static @NonNull String getStartingKey() {
    if (RecentEmojiPageModel.hasRecents(AppDependencies.getApplication(), ZonaRosaPreferences.RECENT_STORAGE_KEY)) {
