...
import io.zonarosa.messenger.components.settings.app.chats.folders.ChatFolderContextMenu
    ...
    override fun bind(model: ChatFolderMappingModel) {
      ...
      if (model.isSelected) {
        itemView.backgroundTintList = if (callbacks.isScrolled()) {
          ColorStateList.valueOf(ContextCompat.getColor(itemView.context, CoreUiR.color.zonarosa_colorBackground))
        } else {
          ColorStateList.valueOf(ContextCompat.getColor(itemView.context, CoreUiR.color.zonarosa_colorSurface2))
        }
        name.setTextColor(ContextCompat.getColor(itemView.context, CoreUiR.color.zonarosa_colorOnSurface))
      } else {
        itemView.backgroundTintList = ColorStateList.valueOf(ContextCompat.getColor(itemView.context, R.color.transparent))
        name.setTextColor(ContextCompat.getColor(itemView.context, CoreUiR.color.zonarosa_colorOnSurfaceVariant))
      }
    }
