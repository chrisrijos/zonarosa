...
import io.zonarosa.donations.json.StripeIntentStatus
...
import io.zonarosa.messenger.components.settings.app.subscription.errors.DonationError.OneTimeDonationError
...
import io.zonarosa.service.api.subscriptions.StripeClientSecret
 ...
 * 1. Generate and send a SubscriberId, which is a 32 byte ID representing this user, to ZonaRosa Service, which creates a Stripe Customer
 ...
 * 1. Set the default PaymentMethod for the customer, using the PaymentMethod id, via the ZonaRosa service
 *
...
object StripeRepository : StripeApi.PaymentIntentFetcher, StripeApi.SetupIntentHelper {
  ...
  override fun fetchPaymentIntent(price: FiatMoney, level: Long, sourceType: PaymentSourceType.Stripe): StripeIntentAccessor {
    Log.d(TAG, "Fetching payment intent from ZonaRosa service for $price... (Locale.US minimum precision: ${price.minimumUnitPrecisionString})")
  ...
  override fun fetchSetupIntent(inAppPaymentType: InAppPaymentType, sourceType: PaymentSourceType.Stripe): StripeIntentAccessor {
    ...
    Log.d(TAG, "Got setup intent from ZonaRosa service!")
  ...
  /**
   * Sets the default payment method for the given subscriber type via ZonaRosa Service
  ...
  fun setDefaultPaymentMethod(
    ...
    Log.d(TAG, "Setting default payment method via ZonaRosa service...")
    ...
    Log.d(TAG, "Set default payment method via ZonaRosa service!")
    Log.d(TAG, "Storing the subscription payment source type locally.")
    ZonaRosaDatabase.inAppPaymentSubscribers.setPaymentMethod(subscriber.subscriberId, paymentSourceType.toPaymentMethodType())
  }
