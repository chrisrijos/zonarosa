...
class StorageServiceValues internal constructor(store: KeyValueStore) : ZonaRosaStoreValues(store) {
  companion object {
  ...
  val storageKey: StorageKey
    get() {
      return ZonaRosaStore.svr.masterKey.deriveStorageServiceKey()
    }
  ...
  var manifest: ZonaRosaStorageManifest
    get() {
      val data = getBlob(MANIFEST, null)

      return if (data != null) {
        ZonaRosaStorageManifest.deserialize(data)
      } else {
        ZonaRosaStorageManifest.EMPTY
      }
    }
    set(manifest) {
  ...
  val storageKeyForInitialDataRestore: StorageKey?
    get() = ZonaRosaStore.svr.masterKeyForInitialDataRestore?.deriveStorageServiceKey()
}
