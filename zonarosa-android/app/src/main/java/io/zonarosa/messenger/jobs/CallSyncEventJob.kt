...
import io.zonarosa.messenger.jobs.protos.CallSyncEventJobData
import io.zonarosa.messenger.jobs.protos.CallSyncEventJobRecord
import io.zonarosa.messenger.recipients.RecipientId
import io.zonarosa.messenger.ringrtc.RemotePeer
import io.zonarosa.messenger.service.webrtc.CallEventSyncMessageUtil
...
class CallSyncEventJob private constructor(
  ...
  private fun processEvent(callSyncEvent: CallSyncEventJobRecord): CallSyncEventJobRecord? {
    val call = ZonaRosaDatabase.calls.getCallById(callSyncEvent.callId, callSyncEvent.deserializeRecipientId())
    if (call == null) {
    ...
    return try {
      AppDependencies.zonarosaServiceMessageSender.sendSyncMessage(ZonaRosaServiceSyncMessage.forCallEvent(syncMessage))
      null
    } catch (e: Exception) {
