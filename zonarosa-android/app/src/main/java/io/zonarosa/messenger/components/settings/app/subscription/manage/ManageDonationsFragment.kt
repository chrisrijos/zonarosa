...
import io.zonarosa.messenger.badges.gifts.ExpiredGiftSheet
...
class ManageDonationsFragment :
  ...
  companion object {
    ...
    const val DONATE_TROUBLESHOOTING_URL = "https://support.zonarosa.io/hc/articles/360031949872#fix"
  }
  ...
  override fun bindAdapter(adapter: MappingAdapter) {
    ...
    val expiredGiftBadge = ZonaRosaStore.inAppPayments.getExpiredGiftBadge()
  ...
  private fun getConfiguration(state: ManageDonationsState): DSLConfiguration {
    return configure {
      ...
      primaryWrappedButton(
        text = DSLSettingsText.from(R.string.ManageDonationsFragment__donate_to_zonarosa),
        onClick = {
    ...
    presentSubscriptionSettingsWithState(state) {
      customPref(
        ActiveSubscriptionPreference.Model(
          price = FiatMoney.fromZonaRosaNetworkAmount(activeSubscription.amount, Currency.getInstance(activeSubscription.currency)),
          subscription = subscription,
