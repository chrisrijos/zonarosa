...
import io.zonarosa.messenger.conversation.mutiselect.forward.MultiselectForwardActivity
...
import io.zonarosa.messenger.mediasend.v2.MediaSelectionState
import io.zonarosa.messenger.mediasend.v2.MediaSelectionViewModel
import io.zonarosa.messenger.mediasend.v2.stories.StoriesMultiselectForwardActivity
...
import io.zonarosa.messenger.util.SystemWindowInsetsSetter
...
import io.zonarosa.messenger.video.TranscodingQuality
import io.zonarosa.messenger.video.videoconverter.VideoThumbnailsRangeSelectorView
...
class MediaReviewFragment : Fragment(R.layout.v2_media_review_fragment), ScheduleMessageTimePickerBottomSheet.ScheduleCallback, VideoThumbnailsRangeSelectorView.RangeDragListener {
  ...
  private var sentMediaQuality: SentMediaQuality = ZonaRosaStore.settings.sentMediaQuality
  ...
  private fun presentSendButton(enabled: Boolean, sendType: MessageSendType, recipient: Recipient?) {
    val sendButtonBackgroundTint = when {
      ...
      sendType.usesZonaRosaTransport -> ContextCompat.getColor(requireContext(), CoreUiR.color.zonarosa_colorOnSecondaryContainer)
    ...
    val sendButtonForegroundTint = when {
      !enabled -> ContextCompat.getColor(requireContext(), CoreUiR.color.zonarosa_colorSecondaryContainer)
      recipient != null -> ContextCompat.getColor(requireContext(), CoreUiR.color.zonarosa_colorOnCustom)
      else -> ContextCompat.getColor(requireContext(), CoreUiR.color.zonarosa_colorSecondaryContainer)
    }
