...
class PointerAttachment : Attachment {
  ...
  companion object {
    @JvmStatic
    fun forPointers(pointers: Optional<List<ZonaRosaServiceAttachment>>): List<Attachment> {
    ...
    @JvmOverloads
    fun forPointer(
      pointer: Optional<ZonaRosaServiceAttachment>,
    ...
    fun forPointer(quotedAttachment: DataMessage.Quote.QuotedAttachment): Optional<Attachment> {
      val thumbnail: ZonaRosaServiceAttachment? = try {
        if (quotedAttachment.thumbnail != null) {
          AttachmentPointerUtil.createZonaRosaAttachmentPointer(quotedAttachment.thumbnail)
        } else {
          null
        }
