...
package io.zonarosa.messenger.components.verify
...
import io.zonarosa.messenger.qr.QrCodeUtil
...
class SafetyNumberQrView : ConstraintLayout {
  ...
  init {
    ...
    ImageViewCompat.setImageTintList(shareButton, ColorStateList.valueOf(ContextCompat.getColor(context, CoreUiR.color.zonarosa_dark_colorOnSurface)))
    setBackgroundColor(ContextCompat.getColor(context, R.color.safety_number_card_blue))
    codes.forEach {
      it.setTextColor(ContextCompat.getColor(context, CoreUiR.color.zonarosa_light_colorOnPrimary))
    }
  }
