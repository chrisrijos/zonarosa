package io.zonarosa.messenger.components.voice;
...
public class VoiceNotePlaybackService extends MediaSessionService {
  ...
  public static final String ACTION_NEXT_PLAYBACK_SPEED = "io.zonarosa.messenger.components.voice.VoiceNotePlaybackService.action.next_playback_speed";
  public static final String ACTION_SET_AUDIO_STREAM    = "io.zonarosa.messenger.components.voice.VoiceNotePlaybackService.action.set_audio_stream";
    ...
    ContextCompat.getMainExecutor(getApplicationContext()).execute(() -> {
      ...
      for (int i = player.getMediaItemCount() - 1; i >= 0; i--) {
        ...
        try {
          ...
          DatabaseAttachment attachment = ZonaRosaDatabase.attachments().getAttachment(partId);
   ...
   * If that doesn't work, it returns null, zonarosaing the {@link MediaSession} cannot be built on this device.
  ...
  private void sendViewedReceiptForCurrentWindowIndex() {
        ...
        player.getCurrentMediaItemIndex() != C.INDEX_UNSET)
    {
      ...
      ZonaRosaExecutors.BOUNDED.execute(() -> {
        ...
        MessageTable messageDatabase = ZonaRosaDatabase.messages();
  ...
  /**
   * Receiver to stop playback and kill the notification if user locks zonarosa via screen lock.
