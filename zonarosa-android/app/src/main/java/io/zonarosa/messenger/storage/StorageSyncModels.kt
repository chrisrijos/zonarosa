...
import io.zonarosa.core.util.isNullOrEmpty
...
import io.zonarosa.messenger.database.RecipientTable.RecipientType
...
import io.zonarosa.messenger.database.ZonaRosaDatabase.Companion.inAppPaymentSubscribers
...
import io.zonarosa.service.internal.storage.protos.AvatarColor as RemoteAvatarColor
import io.zonarosa.service.internal.storage.protos.ChatFolderRecord as RemoteChatFolder
import io.zonarosa.service.internal.storage.protos.NotificationProfile.DayOfWeek as RemoteDayOfWeek
...
object StorageSyncModels {
  ...
  fun localToRemoteRecord(settings: RecipientRecord): ZonaRosaStorageRecord {
  ...
  fun localToRemoteRecord(settings: RecipientRecord, groupMasterKey: GroupMasterKey): ZonaRosaStorageRecord {
    if (settings.storageId == null) {
      throw AssertionError("Must have a storage key!")
    }

    return localToRemoteGroupV2(settings, settings.storageId, groupMasterKey).toZonaRosaStorageRecord()
  }

  fun localToRemoteRecord(settings: RecipientRecord, rawStorageId: ByteArray): ZonaRosaStorageRecord {
    return when (settings.recipientType) {
      RecipientType.INDIVIDUAL -> localToRemoteContact(settings, rawStorageId).toZonaRosaStorageRecord()
      RecipientType.GV1 -> localToRemoteGroupV1(settings, rawStorageId).toZonaRosaStorageRecord()
      RecipientType.GV2 -> localToRemoteGroupV2(settings, rawStorageId, settings.syncExtras.groupMasterKey!!).toZonaRosaStorageRecord()
      RecipientType.DISTRIBUTION_LIST -> localToRemoteStoryDistributionList(settings, rawStorageId).toZonaRosaStorageRecord()
      RecipientType.CALL_LINK -> localToRemoteCallLink(settings, rawStorageId).toZonaRosaStorageRecord()
      else -> throw AssertionError("Unsupported type!")
    }
  }

  fun localToRemoteRecord(folder: ChatFolderRecord, rawStorageId: ByteArray): ZonaRosaStorageRecord {
    return localToRemoteChatFolder(folder, rawStorageId).toZonaRosaStorageRecord()
  }

  fun localToRemoteRecord(profile: NotificationProfile, rawStorageId: ByteArray): ZonaRosaStorageRecord {
    return localToRemoteNotificationProfile(profile, rawStorageId).toZonaRosaStorageRecord()
  }
  ...
  private fun localToRemoteContact(recipient: RecipientRecord, rawStorageId: ByteArray): ZonaRosaContactRecord {
    ...
    return ZonaRosaContactRecord.newBuilder(recipient.syncExtras.storageProto).apply {
      ...
      givenName = recipient.zonarosaProfileName.givenName
      familyName = recipient.zonarosaProfileName.familyName
    ...
    }.build().toZonaRosaContactRecord(StorageId.forContact(rawStorageId))
  }

  private fun localToRemoteGroupV1(recipient: RecipientRecord, rawStorageId: ByteArray): ZonaRosaGroupV1Record {
    ...
    return ZonaRosaGroupV1Record.newBuilder(recipient.syncExtras.storageProto).apply {
      ...
      markedUnread = recipient.syncExtras.isForcedUnread
      mutedUntilTimestamp = recipient.muteUntil
    }.build().toZonaRosaGroupV1Record(StorageId.forGroupV1(rawStorageId))
  }

  private fun localToRemoteGroupV2(recipient: RecipientRecord, rawStorageId: ByteArray?, groupMasterKey: GroupMasterKey): ZonaRosaGroupV2Record {
    val groupId = recipient.groupId ?: throw AssertionError("Must have a groupId!")
    ...
    return ZonaRosaGroupV2Record.newBuilder(recipient.syncExtras.storageProto).apply {
    ...
    }.build().toZonaRosaGroupV2Record(StorageId.forGroupV2(rawStorageId))
  }

  private fun localToRemoteCallLink(recipient: RecipientRecord, rawStorageId: ByteArray): ZonaRosaCallLinkRecord {
    ...
    }.build().toZonaRosaCallLinkRecord(StorageId.forCallLink(rawStorageId))
  }

  private fun localToRemoteStoryDistributionList(recipient: RecipientRecord, rawStorageId: ByteArray): ZonaRosaStoryDistributionListRecord {
    ...
    if (record.deletedAtTimestamp > 0L) {
      return ZonaRosaStoryDistributionListRecord.newBuilder(recipient.syncExtras.storageProto).apply {
        identifier = UuidUtil.toByteArray(record.distributionId.asUuid()).toByteString()
        deletedAtTimestamp = record.deletedAtTimestamp
      }.build().toZonaRosaStoryDistributionListRecord(StorageId.forStoryDistributionList(rawStorageId))
    }
  ...
  fun localToRemoteChatFolder(folder: ChatFolderRecord, rawStorageId: ByteArray?): ZonaRosaChatFolderRecord {
    if (folder.chatFolderId == null) {
      throw AssertionError("Chat folder must have a chat folder id.")
    }
    return ZonaRosaChatFolderRecord.newBuilder(folder.storageServiceProto).apply {
    ...
    }.build().toZonaRosaChatFolderRecord(StorageId.forChatFolder(rawStorageId))
  }

  fun localToRemoteNotificationProfile(profile: NotificationProfile, rawStorageId: ByteArray?): ZonaRosaNotificationProfileRecord {
    return ZonaRosaNotificationProfileRecord.newBuilder(profile.storageServiceProto).apply {
    ...
    }.build().toZonaRosaNotificationProfileRecord(StorageId.forNotificationProfile(rawStorageId))
  }
  ...
  private fun localToRemoteChatFolderRecipients(threadIds: List<Long>): List<RemoteRecipient> {
    val recipientIds = ZonaRosaDatabase.threads.getRecipientIdsForThreadIds(threadIds)
  ...
  private fun localToRemoteRecipients(recipientIds: List<RecipientId>): List<RemoteRecipient> {
    return recipientIds.mapNotNull { id ->
      val recipient = ZonaRosaDatabase.recipients.getRecordForSync(id)
      if (recipient == null) {
  ...
  fun remoteToLocalRecipient(remoteRecipient: RemoteRecipient): Recipient? {
    return if (remoteRecipient.contact != null) {
      ...
      Recipient.externalPush(ZonaRosaServiceAddress(serviceId, e164))
