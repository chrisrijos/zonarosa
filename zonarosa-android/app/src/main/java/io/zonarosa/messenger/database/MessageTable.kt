...
import io.zonarosa.core.util.SqlUtil.buildArgs
import io.zonarosa.core.util.SqlUtil.buildCustomCollectionQuery
import io.zonarosa.core.util.SqlUtil.buildSingleCollectionQuery
import io.zonarosa.core.util.SqlUtil.buildTrueUpdateQuery
import io.zonarosa.core.util.SqlUtil.getNextAutoIncrementId
...
import io.zonarosa.core.util.toSingleLine
...
import io.zonarosa.messenger.attachments.DatabaseAttachment.DisplayOrderComparator
import io.zonarosa.messenger.backup.v2.exporters.ChatItemArchiveExporter
import io.zonarosa.messenger.contactshare.Contact
import io.zonarosa.messenger.conversation.MessageStyler
import io.zonarosa.messenger.database.EarlyDeliveryReceiptCache.Receipt
import io.zonarosa.messenger.database.MentionUtil.UpdatedBodyAndMentions
import io.zonarosa.messenger.database.ZonaRosaDatabase.Companion.attachments
import io.zonarosa.messenger.database.ZonaRosaDatabase.Companion.calls
import io.zonarosa.messenger.database.ZonaRosaDatabase.Companion.distributionLists
import io.zonarosa.messenger.database.ZonaRosaDatabase.Companion.groupReceipts
import io.zonarosa.messenger.database.ZonaRosaDatabase.Companion.groups
import io.zonarosa.messenger.database.ZonaRosaDatabase.Companion.mentions
import io.zonarosa.messenger.database.ZonaRosaDatabase.Companion.messages
import io.zonarosa.messenger.database.ZonaRosaDatabase.Companion.polls
import io.zonarosa.messenger.database.ZonaRosaDatabase.Companion.reactions
import io.zonarosa.messenger.database.ZonaRosaDatabase.Companion.recipients
import io.zonarosa.messenger.database.ZonaRosaDatabase.Companion.storySends
import io.zonarosa.messenger.database.ZonaRosaDatabase.Companion.threads
import io.zonarosa.messenger.database.documents.Document
import io.zonarosa.messenger.database.documents.IdentityKeyMismatch
import io.zonarosa.messenger.database.documents.IdentityKeyMismatchSet
import io.zonarosa.messenger.database.documents.NetworkFailure
import io.zonarosa.messenger.database.documents.NetworkFailureSet
import io.zonarosa.messenger.database.model.GroupCallUpdateDetailsUtil
import io.zonarosa.messenger.database.model.Mention
import io.zonarosa.messenger.database.model.MessageExportStatus
...
import io.zonarosa.messenger.database.model.databaseprotos.GroupCallUpdateDetails
import io.zonarosa.messenger.database.model.databaseprotos.MessageExportState
...
import io.zonarosa.messenger.jobs.OptimizeMessageSearchIndexJob
import io.zonarosa.messenger.jobs.ThreadUpdateJob
...
import io.zonarosa.messenger.notifications.v2.DefaultMessageNotifier.StickyThread
import io.zonarosa.messenger.polls.Poll
import io.zonarosa.messenger.polls.PollRecord
import io.zonarosa.messenger.recipients.Recipient
import io.zonarosa.messenger.recipients.RecipientId
import io.zonarosa.messenger.revealable.ViewOnceExpirationInfo
import io.zonarosa.messenger.revealable.ViewOnceUtil
import io.zonarosa.messenger.sms.GroupV2UpdateMessageUtil
import io.zonarosa.messenger.stories.Stories.isFeatureEnabled
...
open class MessageTable(context: Context?, databaseHelper: ZonaRosaDatabase) : DatabaseTable(context, databaseHelper), MessageTypes, RecipientIdDatabaseReference, ThreadIdDatabaseReference {
  ...
  fun updateGroupCall(
    ...
    writableDatabase.withinTransaction { db ->
      ...
      val updateDetail = GroupCallUpdateDetailsUtil.parse(message.body)
      val containsSelf = joinedUuids.contains(ZonaRosaStore.account.requireAci().rawUuid)
  ...
  fun updatePreviousGroupCall(
    ...
    writableDatabase.withinTransaction { db ->
      ...
      MmsReader(cursor).use { reader ->
        ...
        val containsSelf = peekJoinedUuids.contains(ZonaRosaStore.account.requireAci().rawUuid)
  ...
  fun insertProfileNameChangeMessages(recipient: Recipient, newProfileName: String, previousProfileName: String) {
    writableDatabase.withinTransaction { db ->
      ...
      groupRecords.filter { it.isV2Group }.forEach {
        ZonaRosaDatabase.nameCollisions.handleGroupNameCollisions(it.id.requireV2(), setOf(recipient.id))
      }
    }
  }
  ...
  fun insertLearnedProfileNameChangeMessage(recipient: Recipient, e164: String?, username: String?) {
    ...
    val threadId: Long? = ZonaRosaDatabase.threads.getThreadIdFor(recipient.id)
  ...
  private fun insertGroupV1MigrationMembershipChanges(recipientId: RecipientId, threadId: Long, membershipChange: GroupMigrationMembershipChange) {
    ...
    databaseHelper.zonarosaWritableDatabase.insert(TABLE_NAME, null, values)
  }
  ...
  fun ensureMigration() {
    databaseHelper.zonarosaWritableDatabase
  }
  ...
  fun markOnboardingStoryRead() {
    val recipientId = ZonaRosaStore.releaseChannel.releaseChannelRecipientId ?: return
  ...
  fun setNetworkFailures(messageId: Long, failures: Set<NetworkFailure?>?) {
    try {
      setDocument(databaseHelper.zonarosaWritableDatabase, messageId, NETWORK_FAILURES, NetworkFailureSet(failures))
    } catch (e: IOException) {
      Log.w(TAG, e)
    }
  }
  ...
  private fun rawQueryWithAttachments(where: String, arguments: Array<String>?, reverse: Boolean = false, limit: Long = 0, orderBy: String = ""): Cursor {
    val database = databaseHelper.zonarosaReadableDatabase
    var rawQueryString = """
      SELECT 
    ...
    return mmsReaderFor(cursor).use { reader ->
      reader.mapNotNull {
        if (!it.isMms) {
          ...
          ZonaRosaDatabase.payments.updateMessageWithPayment(it) as MmsMessageRecord
        } else {
  ...
  private fun markAsRemoteDeleteInternal(messageId: Long, deletedBy: RecipientId) {
    ...
    writableDatabase.withinTransaction { db ->
      ...
      ZonaRosaDatabase.messageLog.deleteAllRelatedToMessage(messageId)
  ...
  fun insertMessageOutbox(
    ...
    contentValues.put(FROM_DEVICE_ID, ZonaRosaStore.account.deviceId)
  ...
  private fun insertMediaMessage(
    ...
    val (messageId, insertedAttachments) = writableDatabase.withinTransaction { db ->
      ...
      ZonaRosaDatabase.mentions.insert(threadId, messageId, mentions)

      val insertedAttachments = ZonaRosaDatabase.attachments.insertAttachmentsForMessage(messageId, allAttachments, quoteAttachments)
  ...
  fun getAllRateLimitedMessageIds(): Set<Long> {
    val db = databaseHelper.zonarosaReadableDatabase
  ...
  fun deleteMessagesInThread(threadIds: Collection<Long>, extraWhere: String = ""): Int {
    ...
    writableDatabase.withinTransaction { db ->
      ZonaRosaDatabase.messageSearch.dropAfterMessageDeleteTrigger()
      ZonaRosaDatabase.messageLog.dropAfterMessageDeleteTrigger()
      ...
      ZonaRosaDatabase.messageSearch.restoreAfterMessageDeleteTrigger()
      ZonaRosaDatabase.messageLog.restoreAfterMessageDeleteTrigger()
    }
  ...
  fun deleteMessages(messagesToDelete: List<SyncMessageId>): List<SyncMessageId> {
    ...
    threads
      .forEach { threadId ->
        ZonaRosaDatabase.threads.update(threadId, unarchive = false)
        notifyConversationListeners(threadId)
      }
  ...
  fun setMismatchedIdentities(messageId: Long, mismatches: Set<IdentityKeyMismatch?>) {
    try {
      setDocument(
        database = databaseHelper.zonarosaWritableDatabase,
        messageId = messageId,
        column = MISMATCHED_IDENTITIES,
  ...
  private fun getReleaseChannelThreadId(hasSeenReleaseChannelStories: Boolean): Long {
    ...
    val releaseChannelRecipientId = ZonaRosaStore.releaseChannel.releaseChannelRecipientId ?: return -1L
   ...
   *
   * Heavily lifted from [io.zonarosa.messenger.database.helpers.migration.V191_UniqueMessageMigrationV2].
   */
