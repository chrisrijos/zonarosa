...
class SafetyNumberBottomSheetRepository {
    ...
    return filterForDistributionLists(destinations).flatMapCompletable { distributionRecipients ->
      Completable.fromAction {
        distributionRecipients
          .mapNotNull { ZonaRosaDatabase.distributionLists.getList(it.requireDistributionListId()) }
          .filter { it.members.contains(recipientId) }
          .forEach {
            ZonaRosaDatabase.distributionLists.excludeFromStory(recipientId, it)
  ...
  fun removeAllFromStory(recipientIds: List<RecipientId>, distributionList: DistributionListId): Completable {
    return Completable.fromAction {
      val record = ZonaRosaDatabase.distributionLists.getList(distributionList)
      if (record != null) {
        ZonaRosaDatabase.distributionLists.excludeAllFromStory(recipientIds, record)
  ...
  private fun observeDistributionList(recipient: Recipient): Observable<DistributionListRecord> {
    return Recipient.observable(recipient.id).map { ZonaRosaDatabase.distributionLists.getList(it.requireDistributionListId())!! }
  }
