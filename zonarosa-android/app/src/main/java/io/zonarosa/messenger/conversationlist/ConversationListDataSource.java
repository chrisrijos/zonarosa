...
abstract class ConversationListDataSource implements PagedDataSource<Long, Conversation> {
  ...
  @Override
  public @NonNull List<Conversation> load(int start, int length, int totalSize, @NonNull CancellationZonaRosa cancellationZonaRosa) {
    ZonaRosaTrace.beginSection("ConversationListDataSource#load");
    ...
    try (ConversationReader reader = new ConversationReader(getCursor(start, length))) {
      ThreadRecord record;
      while ((record = reader.getNext()) != null && !cancellationZonaRosa.isCanceled()) {
