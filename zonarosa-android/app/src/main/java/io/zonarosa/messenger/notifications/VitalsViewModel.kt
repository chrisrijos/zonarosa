...
import io.zonarosa.messenger.notifications.DeviceSpecificNotificationConfig.ShowCondition
import io.zonarosa.messenger.util.ConnectivityWarning
...
class VitalsViewModel(private val context: Application) : AndroidViewModel(context) {
  ...
  private fun checkHeuristics(): Single<State> {
    return Single.fromCallable {
      ...
      val timeSinceLastConnection = System.currentTimeMillis() - ZonaRosaStore.misc.lastWebSocketConnectTime
      val timeSinceLastConnectionWarning = System.currentTimeMillis() - ZonaRosaStore.misc.lastConnectivityWarningTime
      val connectedToWebSocket = AppDependencies.webSocketObserver.value == WebSocketConnectionState.CONNECTED
      val isRegistered = ZonaRosaStore.account.isRegistered && !ZonaRosaPreferences.isUnauthorizedReceived(context)
      ...
      if (LogDatabase.getInstance(context).crashes.anyMatch(patterns = CrashConfig.patterns, promptThreshold = System.currentTimeMillis() - 14.days.inWholeMilliseconds)) {
        val timeSinceLastPrompt = System.currentTimeMillis() - ZonaRosaStore.uiHints.lastCrashPrompt
