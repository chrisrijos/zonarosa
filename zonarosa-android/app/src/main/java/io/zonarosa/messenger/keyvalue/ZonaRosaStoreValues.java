...
import io.zonarosa.messenger.database.model.databaseprotos.ZonaRosaStoreList;
...
public abstract class ZonaRosaStoreValues {

  private final KeyValueStore store;

  ZonaRosaStoreValues(@NonNull KeyValueStore store) {
    this.store = store;
  }
  ...
  <T> List<T> getList(@NonNull String key, @NonNull StringSerializer<T> serializer) {
    ...
    try {
      ZonaRosaStoreList zonarosaStoreList = ZonaRosaStoreList.ADAPTER.decode(blob);

      return zonarosaStoreList.contents
                            .stream()
  ...
  <T> void putList(@NonNull String key, @NonNull List<T> values, @NonNull StringSerializer<T> serializer) {
    putBlob(key, new ZonaRosaStoreList.Builder()
