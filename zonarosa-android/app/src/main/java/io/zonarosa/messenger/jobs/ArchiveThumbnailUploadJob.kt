...
import io.zonarosa.messenger.jobs.protos.ArchiveThumbnailUploadJobData
...
class ArchiveThumbnailUploadJob private constructor(
  ...
  override fun onAdded() {
    val transferStatus = ZonaRosaDatabase.attachments.getArchiveThumbnailTransferState(attachmentId) ?: return

    if (transferStatus == AttachmentTable.ArchiveTransferState.NONE) {
      ArchiveDatabaseExecutor.runBlocking {
        ZonaRosaDatabase.attachments.setArchiveThumbnailTransferState(attachmentId, AttachmentTable.ArchiveTransferState.UPLOAD_IN_PROGRESS)
      }
    }
  }
  ...
  override fun run(): Result {
    ...
    if (!MediaUtil.isImageOrVideoType(attachment.contentType)) {
      ...
      ArchiveDatabaseExecutor.runBlocking {
        ZonaRosaDatabase.attachments.setArchiveThumbnailTransferState(attachmentId, AttachmentTable.ArchiveTransferState.NONE)
      }
      return Result.success()
    }
    ...
    if (thumbnailResult == null) {
      ...
      ArchiveDatabaseExecutor.runBlocking {
        ZonaRosaDatabase.attachments.setArchiveThumbnailTransferState(attachmentId, AttachmentTable.ArchiveTransferState.PERMANENT_FAILURE)
      }
      return Result.success()
    }
    ...
    if (isCanceled) {
      ArchiveDatabaseExecutor.runBlocking {
        ZonaRosaDatabase.attachments.setArchiveThumbnailTransferState(attachmentId, AttachmentTable.ArchiveTransferState.TEMPORARY_FAILURE)
      }
      return Result.failure()
    }
    ...
    val attachmentPointer = try {
      buildZonaRosaServiceAttachmentStream(thumbnailResult, resumableUpload).use { stream ->
        val pointer = AppDependencies.zonarosaServiceMessageSender.uploadAttachment(stream)
    ...
    return when (val result = BackupRepository.copyThumbnailToArchive(attachmentPointer, attachment)) {
      is NetworkResult.Success -> {
        ...
        ArchiveDatabaseExecutor.runBlocking {
          ZonaRosaDatabase.attachments.finalizeAttachmentThumbnailAfterUpload(
          ...
          ZonaRosaDatabase.attachments.setArchiveThumbnailTransferState(attachmentId, AttachmentTable.ArchiveTransferState.FINISHED)
        }
        Result.success()
      }
  ...
  override fun onFailure() {
    if (this.isCanceled) {
      ...
      ArchiveDatabaseExecutor.runBlocking {
        ZonaRosaDatabase.attachments.setArchiveThumbnailTransferStateFailure(attachmentId, AttachmentTable.ArchiveTransferState.TEMPORARY_FAILURE)
      }
    }
  }
  ...
  private fun buildZonaRosaServiceAttachmentStream(result: ImageCompressionUtil.Result, uploadSpec: ResumableUpload): ZonaRosaServiceAttachmentStream {
