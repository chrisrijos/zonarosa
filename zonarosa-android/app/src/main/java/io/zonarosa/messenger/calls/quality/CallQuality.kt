...
object CallQuality {
  ...
  fun handleGroupCallSummary(callSummary: CallSummary, kind: GroupCall.Kind) {
    val callType = when (kind) {
      GroupCall.Kind.ZONAROSA_GROUP -> CallType.GROUP
  ...
  private fun handleCallSummary(callSummary: CallSummary, callType: CallType) {
    if (isCallQualitySurveyRequired(callSummary)) {
      ZonaRosaStore.callQuality.surveyRequest = SubmitCallQualitySurveyRequest.Builder()
      ...
      ZonaRosaStore.callQuality.surveyRequest = null
    }
  }
  ...
  fun consumeQualityRequest(): SubmitCallQualitySurveyRequest? {
    val request = ZonaRosaStore.callQuality.surveyRequest ?: return null
    ZonaRosaStore.callQuality.surveyRequest = null
  ...
  private fun isCallQualitySurveyRequired(callSummary: CallSummary): Boolean {
    ...
    val lastFailure = ZonaRosaStore.callQuality.lastFailureReportTime ?: 0.milliseconds
    val failureDelta = now - lastFailure

    if (!isSuccess && (failureDelta < 1.days)) {
      ZonaRosaStore.callQuality.lastFailureReportTime = now
      return true
    }

    if (isSuccess) {
      val lastSurveyPromptTime = ZonaRosaStore.callQuality.lastSurveyPromptTime ?: 0.milliseconds
  ...
  private fun isFeatureEnabled(): Boolean {
    return RemoteConfig.callQualitySurvey && ZonaRosaStore.callQuality.isQualitySurveyEnabled
  }
