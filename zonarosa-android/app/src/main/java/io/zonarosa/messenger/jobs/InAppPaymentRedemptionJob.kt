...
import io.zonarosa.messenger.jobs.protos.InAppPaymentRedemptionJobData
...
class InAppPaymentRedemptionJob private constructor(
  ...
  override fun onAdded() {
    if (jobData.giftMessageId != null) {
      ...
      ZonaRosaDatabase.messages.markGiftRedemptionStarted(jobData.giftMessageId)
    }
  }
  ...
  override fun onFailure() {
    if (jobData.giftMessageId != null) {
      ...
      ZonaRosaDatabase.messages.markGiftRedemptionFailed(jobData.giftMessageId)
    }

    if (jobData.inAppPaymentId != null) {
      Log.w(TAG, "A permanent failure occurred.")

      val inAppPayment = ZonaRosaDatabase.inAppPayments.getById(InAppPaymentTable.InAppPaymentId(jobData.inAppPaymentId))
  ...
  private fun onRunForGiftMessageId(messageId: Long) {
    val giftBadge = ZonaRosaDatabase.messages.getMessageRecord(messageId)
    ...
    val serviceResponse = AppDependencies
      ...
      .redeemDonationReceipt(
        ...
        ZonaRosaStore.inAppPayments.getDisplayBadgesOnProfile(),
        jobData.makePrimary
      )
    ...
    ZonaRosaDatabase.messages.markGiftRedemptionCompleted(messageId)
  }
