package io.zonarosa.messenger.gcm
...
import io.zonarosa.messenger.messages.WebSocketDrainer
...
object FcmFetchManager {
  ...
  private val EXECUTOR = SerialMonoLifoExecutor(ZonaRosaExecutors.UNBOUNDED)

  private val KEEP_ALIVE_TOKEN = "FcmFetch"

  val WEBSOCKET_DRAIN_TIMEOUT: Long
    get() {
      return if (AppDependencies.zonarosaServiceNetworkAccess.isCensored()) {
  ...
  fun startBackgroundService(context: Context) {
    ...
    ZonaRosaLocalMetrics.FcmServiceStartSuccess.onFcmStarted()
  }
  ...
  fun startForegroundService(context: Context) {
    ...
    if (FcmFetchForegroundService.startServiceIfNecessary(context)) {
      ...
      ZonaRosaLocalMetrics.FcmServiceStartFailure.onFcmFailedToStart()
    }
  }
  ...
  private fun fetch(context: Context) {
    ...
    val metricId = ZonaRosaLocalMetrics.PushWebsocketFetch.startFetch()
    val success = retrieveMessages(context)
    if (success) {
      ZonaRosaLocalMetrics.PushWebsocketFetch.onDrained(metricId)
      cancelMayHaveMessagesNotification(context)
    } else {
      ZonaRosaLocalMetrics.PushWebsocketFetch.onTimedOut(metricId)
