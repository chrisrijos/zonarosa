...
import io.zonarosa.messenger.util.RemoteConfig.REMOTE_VALUES
import io.zonarosa.messenger.util.RemoteConfig.asBoolean
import io.zonarosa.messenger.util.RemoteConfig.asInteger
import io.zonarosa.messenger.util.RemoteConfig.remoteBoolean
import io.zonarosa.messenger.util.RemoteConfig.remoteValue
import io.zonarosa.messenger.util.RemoteConfig.retryReceiptMaxCount
import io.zonarosa.messenger.util.RemoteConfig.retryReceiptMaxCountResetAge
...
object RemoteConfig {
  ...
  @JvmStatic
  val libzonarosaConfigs: Map<String, String>
    get() {
      if (!initialized) {
        Log.w(TAG, "Tried to read libzonarosaConfigs before initialization. Initializing now.")
        initLock.withLock {
          if (!initialized) {
            init()
          }
      ...
      return computeLibzonarosaConfigs(REMOTE_VALUES)
    }
  ...
  fun init() {
    initLock.withLock {
      val current = parseStoredConfig(ZonaRosaStore.remoteConfig.currentConfig)
      val pending = parseStoredConfig(ZonaRosaStore.remoteConfig.pendingConfig)
      val changes = computeChanges(current, pending)

      ZonaRosaStore.remoteConfig.currentConfig = mapToJson(pending)
  ...
  fun refreshIfNecessary() {
    val timeSinceLastFetch = System.currentTimeMillis() - ZonaRosaStore.remoteConfig.lastFetchTime
  ...
  fun refreshSync() {
    val result = NetworkResultUtil.toBasicLegacy(ZonaRosaNetwork.remoteConfig.getRemoteConfig())
  ...
  fun update(config: Map<String, Any?>) {
    ...
    val disk = parseStoredConfig(ZonaRosaStore.remoteConfig.pendingConfig)
    ...
    ZonaRosaStore.remoteConfig.pendingConfig = mapToJson(result.disk)
    REMOTE_VALUES.clear()
  ...
  val debugDiskValues: Map<String, Any>
    get() = TreeMap(parseStoredConfig(ZonaRosaStore.remoteConfig.currentConfig))

  /** Only for rendering debug info.  */
  @JvmStatic
  @get:Synchronized
  val debugPendingDiskValues: Map<String, Any>
    get() = TreeMap(parseStoredConfig(ZonaRosaStore.remoteConfig.pendingConfig))
  ...
  fun updateInternal(
    ...
    allKeys
      .filter { remoteCapable.contains(it) || it.startsWith("android.libzonarosa.") }
      .forEach { key: String ->
    ...
    // Libzonarosa does not support sticky flags and is okay with any flag that is not known to the server being
    // forgotten about.
    val libzonarosaConfigsKnownToServer = remote.keys.filter { it.startsWith("android.libzonarosa.") }.toSet()

    allKeys
      .filterNot { key ->
        remoteCapable.contains(key) || libzonarosaConfigsKnownToServer.contains(key)
      }
  ...
  private fun computeLibzonarosaConfigs(config: Map<String, Any?>): Map<String, String> {
    return config
      .filterKeys { it.startsWith("android.libzonarosa.") }
      .mapNotNull { (key, value) ->
        val newKey = key.removePrefix("android.libzonarosa.")
        when (value) {
          ...
          // Libzonarosa expects that disabled values are simply absent from the
          ...
          is Boolean -> if (value) newKey to "true" else null
            ...
            Log.w(TAG, "[libzonarosa] Unexpected type for $newKey! Was a $type")
  ...
  @JvmStatic
  @get:JvmName("libzonarosaEnforceMinTlsVersion")
  val libzonarosaEnforceMinTlsVersion by remoteBoolean(
    key = "android.libzonarosaEnforceMinTlsVersion",
    defaultValue = false,
    hotSwappable = false
  )
