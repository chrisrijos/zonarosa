...
import io.zonarosa.messenger.backup.FullBackupExporter.BackupCanceledException
...
class LocalArchiveJob internal constructor(parameters: Parameters) : Job(parameters) {
  ...
  override fun run(): Result {
    ...
    try {
      ...
      try {
        ZonaRosaDatabase.attachmentMetadata.insertNewKeysForExistingAttachments()

        try {
          val result = LocalArchiver.export(snapshotFileSystem, archiveFileSystem.filesFileSystem, stopwatch, cancellationZonaRosa = { isCanceled })
          Log.i(TAG, "Archive finished with result: $result")
          if (result !is io.zonarosa.core.util.Result.Success) {
            return Result.failure()
          }
        } catch (e: Exception) {
      ...
      ZonaRosaStore.backup.newLocalBackupsLastBackupTime = System.currentTimeMillis()
    } finally {
      notification?.close()
