...
import io.zonarosa.messenger.databinding.FragmentMediaPreviewV2Binding
import io.zonarosa.messenger.dependencies.AppDependencies
import io.zonarosa.messenger.mediapreview.caption.ExpandingCaptionView
import io.zonarosa.messenger.mediapreview.mediarail.CenterDecoration
import io.zonarosa.messenger.mediapreview.mediarail.MediaRailAdapter
import io.zonarosa.messenger.mediapreview.mediarail.MediaRailAdapter.ImageLoadingListener
...
class MediaPreviewV2Fragment :
  ...
  private fun initializeToolbar(toolbar: MaterialToolbar) {
    ...
    toolbar.setTitleTextAppearance(requireContext(), CoreUiR.style.ZonaRosa_Text_TitleMedium)
    toolbar.setSubtitleTextAppearance(requireContext(), CoreUiR.style.ZonaRosa_Text_BodyMedium)
  ...
  private fun getSubTitleText(mediaRecord: MediaTable.MediaRecord): Pair<CharSequence, CharSequence> {
    ...
    val onSurfaceColor = ContextCompat.getColor(requireContext(), CoreUiR.color.zonarosa_colorOnSurface)
  ...
  private fun deleteMedia(mediaItem: MediaTable.MediaRecord) {
    ...
    val messageRecord = ZonaRosaDatabase.messages.getMessageRecord(attachment.mmsId)
  ...
  private fun canRemotelyDelete(attachment: DatabaseAttachment, messageRecord: MessageRecord): Boolean {
    ...
    val attachmentCount = ZonaRosaDatabase.attachments.getAttachmentsForMessage(mmsId).size
