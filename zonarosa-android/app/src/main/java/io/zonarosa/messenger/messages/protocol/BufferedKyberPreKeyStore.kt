...
import io.zonarosa.messenger.database.KyberPreKeyTable.KyberPreKey
 ...
 */
class BufferedKyberPreKeyStore(private val selfServiceId: ServiceId) : ZonaRosaServiceKyberPreKeyStore {
  ...
  override fun loadKyberPreKey(kyberPreKeyId: Int): KyberPreKeyRecord {
    return store.computeIfAbsent(kyberPreKeyId) {
      ZonaRosaDatabase.kyberPreKeys.get(selfServiceId, kyberPreKeyId) ?: throw InvalidKeyIdException("Missing kyber prekey with ID: $kyberPreKeyId")
    }.record
  }
  ...
  override fun loadKyberPreKeys(): List<KyberPreKeyRecord> {
    return if (hasLoadedAll) {
      ...
      val models = ZonaRosaDatabase.kyberPreKeys.getAll(selfServiceId)
