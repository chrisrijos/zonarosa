...
import io.zonarosa.messenger.components.settings.app.usernamelinks.drawQr
import io.zonarosa.messenger.components.settings.app.usernamelinks.main.UsernameLinkSettingsState.ActiveTab
...
class UsernameLinkSettingsViewModel : ViewModel() {
  ...
  private val _state = mutableStateOf(
    UsernameLinkSettingsState(
      ...
      usernameLinkState = ZonaRosaStore.account.usernameLink?.let { UsernameLinkState.Present(it.toLink()) } ?: UsernameLinkState.NotSet,
      qrCodeState = QrCodeState.Loading,
      qrCodeColorScheme = ZonaRosaStore.misc.usernameQrCodeColorScheme
    )
  )
  val state: State<UsernameLinkSettingsState> = _state

  private val disposable: CompositeDisposable = CompositeDisposable()
  private val usernameLink: BehaviorSubject<Optional<UsernameLinkComponents>> = BehaviorSubject.createDefault(Optional.ofNullable(ZonaRosaStore.account.usernameLink))
  ...
  init {
    ...
    if (ZonaRosaStore.account.usernameLink == null) {
      onUsernameLinkReset()
    }
  }
  ...
  fun generateQrCodeImage(helpText: String): Bitmap? {
    ...
    // Draw the zonarosa logo -- unfortunately can't have the normal QR code drawing handle it because it requires a composable ImageBitmap
