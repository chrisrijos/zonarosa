...
import io.zonarosa.messenger.util.MemoryFileDescriptor.MemoryFileException;
import io.zonarosa.messenger.util.RemoteConfig;
import io.zonarosa.messenger.video.StreamingTranscoder;
import io.zonarosa.messenger.video.TranscoderOptions;
import io.zonarosa.messenger.video.exceptions.VideoPostProcessingException;
import io.zonarosa.messenger.video.exceptions.VideoSourceException;
import io.zonarosa.messenger.video.interfaces.TranscoderCancelationZonaRosa;
import io.zonarosa.messenger.video.postprocessing.Mp4FaststartPostProcessor;
import io.zonarosa.messenger.video.videoconverter.exceptions.CodecUnavailableException;
import io.zonarosa.messenger.video.videoconverter.exceptions.EncodingException;
...
public final class AttachmentCompressionJob extends BaseJob {
  ...
  public void onAdded() {
    ...
    final AttachmentTable    database   = ZonaRosaDatabase.attachments();
  ...
  private static @NonNull DatabaseAttachment transcodeVideoIfNeededToDatabase(@NonNull Context context,
                                                                              ...
                                                                              @NonNull TranscoderCancelationZonaRosa cancelationZonaRosa)
      throws UndeliverableMessageException
  {

    if (cancelationZonaRosa.isCanceled()) {
            ...
            try (OutputStream outputStream = ModernEncryptingPartOutputStream.createFor(attachmentSecret, file, true).getSecond()) {
              mdatLength = (int) transcoder.transcode(percent -> {
                ...
                eventBus.postSticky(new PartProgressEvent(attachment,
                                                          ...
                                                          100,
                                                          percent));
              }, outputStream, cancelationZonaRosa);
