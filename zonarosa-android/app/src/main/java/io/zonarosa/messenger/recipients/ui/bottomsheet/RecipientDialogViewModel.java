...
final class RecipientDialogViewModel extends ViewModel {
                                   ...
                                   @NonNull RecipientDialogRepository recipientDialogRepository)
  {
    ...
    this.isDeprecatedOrUnregistered = ZonaRosaStore.misc().isClientDeprecated() || ZonaRosaPreferences.isUnauthorizedReceived(context);
  ...
  private void updateRecipientDetailsState(@NonNull Recipient recipient) {
    ...
    String  aboutText = recipient.isReleaseNotes() ? context.getString(R.string.ReleaseNotes__zonarosa_release_notes_and_news) : recipient.getCombinedAboutAndEmoji();
    ...
    if (groupId != null && groupId.isV2() && recipient.isIndividual() && !recipient.isSelf()) {
      ZonaRosaExecutors.BOUNDED.execute(() -> {
        ...
        if (label != null) {
          ...
          Optional<GroupRecord> groupRecord = ZonaRosaDatabase.groups().getGroup(v2GroupId);
  ...
  public void onTapToViewAvatar(@NonNull Recipient recipient) {
    ZonaRosaExecutors.BOUNDED.execute(() -> ZonaRosaDatabase.recipients().manuallyUpdateShowAvatar(recipient.getId(), true));
  ...
  public void onResetBlurAvatar(@NonNull Recipient recipient) {
    ZonaRosaExecutors.BOUNDED.execute(() -> ZonaRosaDatabase.recipients().manuallyUpdateShowAvatar(recipient.getId(), false));
  }

  public void refreshGroupId(@Nullable GroupId groupId) {
    if (groupId != null) {
      ZonaRosaExecutors.BOUNDED.execute(() -> {
        RecipientId groupRecipientId = ZonaRosaDatabase.groups().getGroup(groupId).get().getRecipientId();
