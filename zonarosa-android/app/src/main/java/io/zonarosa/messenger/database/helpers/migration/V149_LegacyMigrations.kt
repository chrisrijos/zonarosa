...
import io.zonarosa.messenger.contacts.avatars.ContactColorsLegacy
...
import io.zonarosa.messenger.database.helpers.RecipientIdCleanupHelper
import io.zonarosa.messenger.database.helpers.RecipientIdMigrationHelper
...
import io.zonarosa.messenger.database.model.databaseprotos.ReactionList
...
import io.zonarosa.messenger.util.Triple
 ...
 */
@Suppress("ClassName")
object V149_LegacyMigrations : ZonaRosaDatabaseMigration {

  private val TAG: String = ZonaRosaDatabaseMigrations.TAG
  ...
  override fun migrate(context: Application, db: SQLiteDatabase, oldVersion: Int, newVersion: Int) {
    ...
    if (oldVersion < NOTIFICATION_CHANNELS && !SqlUtil.columnExists(db, "recipient_preferences", "notification_channel")) {
      ...
      db.rawQuery("SELECT recipient_ids, system_display_name, zonarosa_profile_name, notification, vibrate FROM recipient_preferences WHERE notification NOT NULL OR vibrate != 0", null).use { cursor ->
        while (cursor != null && cursor.moveToNext()) {
          val rawAddress: String = cursor.getString(cursor.getColumnIndexOrThrow("recipient_ids"))
          val address: String = ZonaRosaE164Util.formatAsE164(rawAddress) ?: ""
          val systemName: String = cursor.getString(cursor.getColumnIndexOrThrow("system_display_name"))
          val profileName: String = cursor.getString(cursor.getColumnIndexOrThrow("zonarosa_profile_name"))
          ...
          val vibrateEnabled: Boolean = if (vibrateState == 0) ZonaRosaStore.settings.isMessageVibrateEnabled else vibrateState == 1
    ...
    if (oldVersion < STORAGE_SERVICE_ACTIVE) {
      ...
      db.execSQL("UPDATE recipient set group_type = 1 WHERE group_id NOT NULL AND group_id LIKE '__zonarosa_mms_group__%'")
      db.execSQL("UPDATE recipient set group_type = 2 WHERE group_id NOT NULL AND group_id LIKE '__zonarosa_group__%'")
    ...
    if (oldVersion < PROFILE_DATA_MIGRATION) {
      ...
      if (localNumber != null) {
        ...
        db.execSQL("UPDATE recipient SET zonarosa_profile_name = ?, profile_family_name = ?, profile_joined_name = ? WHERE phone = ?", arrayOf(profileName.givenName, profileName.familyName, profileName.toString(), localNumber))
      }
    }
    ...
    if (oldVersion < AVATAR_LOCATION_MIGRATION) {
      ...
      db.execSQL("UPDATE recipient SET zonarosa_profile_avatar = NULL")
    ...
    if (oldVersion < GV1_MIGRATION) {
      ...
      var count = 0
      db.rawQuery("SELECT * FROM groups WHERE group_id LIKE '__zonarosa_group__!%' AND LENGTH(group_id) = 53", null).use { cursor ->
    ...
    if (oldVersion < CLEAN_UP_GV1_IDS) {
      ...
      db.rawQuery(
        ...
        """
          ...
          WHERE group_id NOT IN (SELECT group_id FROM groups)
            AND group_id LIKE '__zonarosa_group__!%' AND length(group_id) <> 53
      ...
      for (recipientId: String in remainingInvalidGV1Recipients) {
        val newId = "__zonarosa_group__!" + Hex.toStringCondensed(Util.getSecretBytes(16))
        val values = ContentValues(1)
