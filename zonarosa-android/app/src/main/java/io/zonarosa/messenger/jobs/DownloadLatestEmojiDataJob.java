...
import io.zonarosa.messenger.emoji.EmojiData;
import io.zonarosa.messenger.emoji.EmojiDownloader;
import io.zonarosa.messenger.emoji.EmojiFiles;
import io.zonarosa.messenger.emoji.EmojiImageRequest;
import io.zonarosa.messenger.emoji.EmojiJsonRequest;
import io.zonarosa.messenger.emoji.EmojiPageCache;
import io.zonarosa.messenger.emoji.EmojiRemote;
...
public class DownloadLatestEmojiDataJob extends BaseJob {
  ...
  public static void scheduleIfNecessary(@NonNull Context context) {
    long nextScheduledCheck = ZonaRosaStore.emoji().getNextScheduledImageCheck();
    ...
    if (nextScheduledCheck <= System.currentTimeMillis()) {
      ...
      ZonaRosaStore.emoji().setNextScheduledImageCheck(System.currentTimeMillis() + interval);
    }
  }
  ...
  private static void clearOldEmojiData(@NonNull Context context, @Nullable EmojiFiles.Version newVersion) {
    ...
    if (version != null) {
      ZonaRosaStore.emoji().clearJumboEmojiSheets(version.getVersion());
    }
  }
