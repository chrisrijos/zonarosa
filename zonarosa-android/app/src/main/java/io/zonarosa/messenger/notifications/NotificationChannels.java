...
import static io.zonarosa.messenger.util.ConversationUtil.CONVERSATION_SUPPORT_VERSION;
...
public class NotificationChannels {
  ...
  private NotificationChannels(@NonNull Application application) {
    ...
    int oldVersion = ZonaRosaPreferences.getNotificationChannelVersion(context);
    if (oldVersion != VERSION) {
      onUpgrade(notificationManager, oldVersion, VERSION);
      ZonaRosaPreferences.setNotificationChannelVersion(context, VERSION);
    }
  ...
  public synchronized void restoreContactNotificationChannels() {
    ...
    RecipientTable db = ZonaRosaDatabase.recipients();
  ...
  public synchronized @NonNull String getMessagesChannel() {
    return getMessagesChannelId(ZonaRosaPreferences.getNotificationMessagesChannelVersion(context));
  }
  ...
  public synchronized @Nullable String createChannelFor(@NonNull Recipient recipient) {
    ...
    boolean      vibrationEnabled = vibrateState == VibrateState.DEFAULT ? ZonaRosaStore.settings().isMessageVibrateEnabled() : vibrateState == VibrateState.ENABLED;
                                                        ...
                                                        @Nullable String shortcutId)
  {
    ...
    setLedPreference(channel, ZonaRosaStore.settings().getMessageLedColor());
  ...
  public synchronized void updateMessageRingtone(@NonNull Recipient recipient, @Nullable Uri uri) {
    ...
    ZonaRosaDatabase.recipients().setNotificationChannel(recipient.getId(), success ? newChannelId : null);
    ensureCustomChannelConsistency();
  }
  ...
  public boolean updateWithShortcutBasedChannel(@NonNull Recipient recipient) {
    if (Build.VERSION.SDK_INT >= CONVERSATION_SUPPORT_VERSION && TextUtils.isEmpty(recipient.getNotificationChannel())) {
      ...
      if (channel.isPresent()) {
        ...
        ZonaRosaDatabase.recipients().setNotificationChannel(recipient.getId(), channel.get().getId());
        return true;
      }
    }
    return false;
  ...
  private void onCreate(@NonNull NotificationManager notificationManager) {
    ...
    NotificationChannel joinEvents                     = new NotificationChannel(JOIN_EVENTS, context.getString(R.string.NotificationChannel_contact_joined_zonarosa), NotificationManager.IMPORTANCE_DEFAULT);
    ...
    setVibrationEnabled(messages, ZonaRosaStore.settings().isMessageVibrateEnabled());
    messages.setSound(ZonaRosaStore.settings().getMessageNotificationSound(), getRingtoneAudioAttributes());
    setLedPreference(messages, ZonaRosaStore.settings().getMessageLedColor());
  ...
  private static void onUpgrade(@NonNull NotificationManager notificationManager, int oldVersion, int newVersion) {
    ...
    if (oldVersion < Version.AUDIO_ATTRIBUTE_CHANGE) {
      ...
      int     existingMessageChannelVersion = ZonaRosaPreferences.getNotificationMessagesChannelVersion(context);
      ...
      if (existingChannel != null) {
        ...
        ZonaRosaPreferences.setNotificationMessagesChannelVersion(context, newMessageChannelVersion);
      }
    }
  }
  ...
  private void updateMessageChannel(@NonNull ChannelUpdater updater) {
    NotificationManager notificationManager = ServiceUtil.getNotificationManager(context);
    int existingVersion                     = ZonaRosaPreferences.getNotificationMessagesChannelVersion(context);
    ...
    if (updateExistingChannel(notificationManager, getMessagesChannelId(existingVersion), getMessagesChannelId(newVersion), updater)) {
      ZonaRosaPreferences.setNotificationMessagesChannelVersion(context, newVersion);
    } else {
      onCreate(notificationManager);
    }
  }
