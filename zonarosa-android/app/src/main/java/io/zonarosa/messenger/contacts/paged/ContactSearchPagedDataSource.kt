...
import io.zonarosa.messenger.contacts.paged.collections.ContactSearchCollection
import io.zonarosa.messenger.contacts.paged.collections.ContactSearchIterator
import io.zonarosa.messenger.contacts.paged.collections.CursorSearchIterator
import io.zonarosa.messenger.contacts.paged.collections.StoriesSearchCollection
...
import io.zonarosa.messenger.search.MessageSearchResult
import io.zonarosa.messenger.search.SearchRepository
import io.zonarosa.messenger.search.ThreadSearchResult
...
class ContactSearchPagedDataSource(
  ...
  override fun load(start: Int, length: Int, totalSize: Int, cancellationZonaRosa: PagedDataSource.CancellationZonaRosa): MutableList<ContactSearchData> {
  ...
  private fun getNonGroupSearchIterator(section: ContactSearchConfiguration.Section.Individuals, query: String?): ContactSearchIterator<Cursor> {
    ...
    return CursorSearchIterator(wrapRecipientCursor(contactSearchPagedDataSourceRepository.queryZonaRosaContacts(searchQuery)))
  }
  ...
  private fun getNonGroupHeaderLetterMap(section: ContactSearchConfiguration.Section.Individuals, query: String?): Map<RecipientId, String> {
    return contactSearchPagedDataSourceRepository.queryZonaRosaContactLetterHeaders(
      query = query,
