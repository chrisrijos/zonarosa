...
import io.zonarosa.messenger.mediasend.CompositeMediaTransform
import io.zonarosa.messenger.mediasend.ImageEditorModelRenderMediaTransform
import io.zonarosa.messenger.mediasend.MediaRepository
import io.zonarosa.messenger.mediasend.MediaSendActivityResult
import io.zonarosa.messenger.mediasend.MediaTransform
import io.zonarosa.messenger.mediasend.MediaUploadRepository
import io.zonarosa.messenger.mediasend.SentMediaQualityTransform
import io.zonarosa.messenger.mediasend.VideoTrimTransform
...
import io.zonarosa.messenger.sms.MessageSender.SendType
...
class MediaSelectionRepository(context: Context) {
  ...
  fun send(
    ...
    return Maybe.create { emitter ->
      ...
      val storyType: StoryType = if (singleRecipient?.isDistributionList == true) {
        ZonaRosaDatabase.distributionLists.getStoryType(singleRecipient.requireDistributionListId())
  ...
  private fun scheduleMessages(
    ...
    for (recipientId in recipientIds) {
      ...
      MessageSender.send(context, outgoingMessage, thread, SendType.ZONAROSA, null, null)
    }
  }
  ...
  private fun sendMessages(
    ...
    for (contact in contacts) {
      ...
      if (isStory && !recipient.isMyStory) {
        ZonaRosaStore.story.setLatestStorySend(StorySend.newSend(recipient))
      }

      val storyType: StoryType = when {
        recipient.isDistributionList -> ZonaRosaDatabase.distributionLists.getStoryType(recipient.requireDistributionListId())
