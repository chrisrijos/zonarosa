...
class RefreshOwnProfileJob private constructor(parameters: Parameters) : BaseJob(parameters) {
  ...
  override fun onRun() {
    if (!ZonaRosaStore.account.isRegistered || ZonaRosaStore.account.e164.isNullOrEmpty()) {
      Log.w(TAG, "Not yet registered!")
      return
    }

    if ((ZonaRosaStore.svr.hasPin() || ZonaRosaStore.account.restoredAccountEntropyPool) && !ZonaRosaStore.svr.hasOptedOut() && ZonaRosaStore.storageService.lastSyncTime == 0L) {
      Log.i(TAG, "Registered with PIN or AEP but haven't completed storage sync yet.")
      return
    }

    if (!ZonaRosaStore.registration.hasUploadedProfile && ZonaRosaStore.account.isPrimaryDevice) {
    ...
    ZonaRosaStore.registration.hasDownloadedProfile = true
  ...
  private fun setProfileName(encryptedName: String?) {
    try {
      ...
      if (!profileName.isEmpty()) {
        ...
        ZonaRosaDatabase.recipients.setProfileName(Recipient.self().id, profileName)
      } else {
  ...
  private fun setProfileAbout(encryptedAbout: String?, encryptedEmoji: String?) {
    try {
      ...
      ZonaRosaDatabase.recipients.setAbout(Recipient.self().id, plaintextAbout, plaintextEmoji)
    } catch (e: InvalidCiphertextException) {
      Log.w(TAG, e)
    } catch (e: IOException) {
      Log.w(TAG, e)
  ...
  private fun setProfileCapabilities(capabilities: ZonaRosaServiceProfile.Capabilities?) {
    if (capabilities == null) {
      return
    }

    ZonaRosaDatabase.recipients.setCapabilities(Recipient.self().id, capabilities)
  }
  ...
  private fun ensureUnidentifiedAccessCorrect(unidentifiedAccessVerifier: String?, universalUnidentifiedAccess: Boolean) {
    ...
    if (ZonaRosaPreferences.isUniversalUnidentifiedAccess(context) != universalUnidentifiedAccess) {
      Log.w(TAG, "The universal access flag doesn't match our local value (local: " + ZonaRosaPreferences.isUniversalUnidentifiedAccess(context) + ", remote: " + universalUnidentifiedAccess + ")! Refreshing attributes.")
      AppDependencies.jobManager.add(RefreshAttributesJob())
      return
    }
  ...
  private fun ensurePhoneNumberSharingIsCorrect(phoneNumberSharingCiphertext: String?) {
    ...
    try {
      ...
      if (remotePhoneNumberSharing == PhoneNumberSharingState.UNKNOWN || remotePhoneNumberSharing.enabled != ZonaRosaStore.phoneNumberPrivacy.isPhoneNumberSharingEnabled()) {
  ...
  private fun setProfileBadges(badges: List<ZonaRosaServiceProfile.Badge>?) {
    if (badges == null) {
      return
    }
    ...
    if (!remoteHasSubscriptionBadges && localHasSubscriptionBadges) {
      ...
      ZonaRosaStore.inAppPayments.setExpiredBadge(mostRecentExpiration)
      ...
      val badge = ZonaRosaStore.inAppPayments.getExpiredBadge()
    ...
    if (!remoteHasGiftBadges && localHasGiftBadges) {
      ...
      ZonaRosaStore.inAppPayments.setExpiredGiftBadge(mostRecentExpiration)
    ...
    if (userHasVisibleBadges && userHasInvisibleBadges) {
      val displayBadgesOnProfile = ZonaRosaStore.inAppPayments.getDisplayBadgesOnProfile()
      Log.d(
        TAG,
      ...
      ZonaRosaDatabase.recipients.setBadges(Recipient.self().id, updatedBadges)
    } else {
      ZonaRosaDatabase.recipients.setBadges(Recipient.self().id, appBadges)
    }
  }

  private fun checkUsernameIsInSync() {
    try {
      val localUsername = ZonaRosaStore.account.username

      val whoAmIResponse = AppDependencies.zonarosaServiceAccountManager.getWhoAmI()
    ...
    val localUsernameLink = ZonaRosaStore.account.usernameLink ?: return

    when (val usernameFetchResult = ZonaRosaNetwork.username.getDecryptedUsernameFromLinkServerIdAndEntropy(localUsernameLink.serverId, localUsernameLink.entropy)) {
      is RequestResult.Success -> {
        ...
        if (remoteUsername.getUsername() != ZonaRosaStore.account.username) {
  ...
  private fun getRequestType(recipient: Recipient): ZonaRosaServiceProfile.RequestType {
    return if (ExpiringProfileCredentialUtil.isValid(recipient.expiringProfileKeyCredential))
      ZonaRosaServiceProfile.RequestType.PROFILE
    else
      ZonaRosaServiceProfile.RequestType.PROFILE_AND_CREDENTIAL
  }
