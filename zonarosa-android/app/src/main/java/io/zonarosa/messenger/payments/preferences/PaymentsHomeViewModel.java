...
import io.zonarosa.messenger.keyvalue.PaymentsAvailability;
...
import io.zonarosa.messenger.payments.UnreadPaymentsRepository;
import io.zonarosa.messenger.payments.currency.CurrencyExchange;
import io.zonarosa.messenger.payments.currency.CurrencyExchangeRepository;
...
public class PaymentsHomeViewModel extends ViewModel {
                        ...
                        @NonNull CurrencyExchangeRepository currencyExchangeRepository)
  {
    ...
    this.balance                    = LiveDataUtil.mapDistinct(ZonaRosaStore.payments().liveMobileCoinBalance(), Balance::getFullAmount);
    ...
    this.enclaveFailure             = LiveDataUtil.mapDistinct(ZonaRosaStore.payments().enclaveFailure(), isFailure -> isFailure);
    this.store.update(paymentsRepository.getRecentPayments(), this::updateRecentPayments);

    LiveData<CurrencyExchange.ExchangeRate> liveExchangeRate = LiveDataUtil.combineLatest(ZonaRosaStore.payments().liveCurrentCurrency(),
  ...
  private static PaymentsHomeState.PaymentsState getPaymentsState() {
    PaymentsValues paymentsValues = ZonaRosaStore.payments();
  ...
  void checkPaymentActivationState() {
    ...
    boolean                         paymentsEnabled = ZonaRosaStore.payments().mobileCoinPaymentsEnabled();
