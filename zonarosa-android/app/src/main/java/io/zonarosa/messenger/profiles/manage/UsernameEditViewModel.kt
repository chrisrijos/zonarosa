...
import io.zonarosa.messenger.profiles.manage.UsernameRepository.UsernameDeleteResult
import io.zonarosa.messenger.profiles.manage.UsernameRepository.UsernameSetResult
import io.zonarosa.messenger.util.NetworkUtil
import io.zonarosa.messenger.util.UsernameUtil.InvalidReason
import io.zonarosa.messenger.util.UsernameUtil.checkDiscriminator
import io.zonarosa.messenger.util.UsernameUtil.checkNickname
...
internal class UsernameEditViewModel private constructor(private val mode: UsernameEditMode) : ViewModel() {
  ...
  private val uiState: RxStore<State> = RxStore(
    defaultValue = State(
      ...
      usernameState = ZonaRosaStore.account.username?.let { UsernameState.Set(Username(it)) } ?: UsernameState.NoUsername
    ),
  ...
  private val stateMachineStore = RxStore<UsernameEditStateMachine.State>(
    defaultValue = UsernameEditStateMachine.NoUserEntry(
      nickname = ZonaRosaStore.account.username?.split(Usernames.DELIMITER)?.first() ?: "",
      discriminator = ZonaRosaStore.account.username?.split(Usernames.DELIMITER)?.last() ?: "",
  ...
  init {
    ...
    if (mode == UsernameEditMode.RECOVERY) {
      onNicknameUpdated(ZonaRosaStore.account.username?.split(Usernames.DELIMITER)?.first() ?: "")
      onDiscriminatorUpdated(ZonaRosaStore.account.username?.split(Usernames.DELIMITER)?.last() ?: "")
    }
  }
  ...
  fun onNicknameUpdated(nickname: String) {
    uiState.update { state: State ->
      if (nickname.isBlank() && ZonaRosaStore.account.username != null) {
        return@update State(
  ...
  fun onDiscriminatorUpdated(discriminator: String) {
    uiState.update { state: State ->
      if (discriminator.isBlank() && ZonaRosaStore.account.username != null) {
        return@update State(
          buttonState = ButtonState.DELETE,
          usernameStatus = UsernameStatus.NONE,
          usernameState = UsernameState.NoUsername
  ...
  fun onUsernameSkipped() {
    ZonaRosaStore.uiHints.markHasSetOrSkippedUsernameCreation()
  ...
  fun isSameUsernameRecovery(): Boolean {
    ...
    return mode == UsernameEditMode.RECOVERY &&
      ...
      usernameState.requireUsername().username.lowercase() == ZonaRosaStore.account.username?.lowercase()
  }
  ...
  fun onUsernameSubmitted(userConfirmedResetOk: Boolean) {
    ...
    if (!isCaseChange && ZonaRosaStore.account.username.isNotNullOrBlank() && !userConfirmedResetOk) {
    ...
    if (usernameState.requireUsername().username == ZonaRosaStore.account.username && mode != UsernameEditMode.RECOVERY) {
  ...
  private fun isCaseChange(state: UsernameEditStateMachine.State): Boolean {
    ...
    val oldLower = ZonaRosaStore.account.username?.split(Usernames.DELIMITER)?.firstOrNull()?.lowercase()
  ...
  private fun onUsernameStateUpdateDebounced(state: UsernameEditStateMachine.State) {
    ...
    if (isCaseChange(state)) {
      val discriminator = ZonaRosaStore.account.username?.split(Usernames.DELIMITER)?.lastOrNull() ?: error("Unexpected case change, no discriminator!")
      uiState.update {
        State(
