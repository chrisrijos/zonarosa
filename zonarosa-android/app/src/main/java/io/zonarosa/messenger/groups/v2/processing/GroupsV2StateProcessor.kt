...
import io.zonarosa.messenger.groups.GroupMutation
...
import io.zonarosa.messenger.groups.v2.processing.GroupsV2StateProcessor.Companion.LATEST
import io.zonarosa.messenger.jobs.AvatarGroupsV2DownloadJob
import io.zonarosa.messenger.jobs.DirectoryRefreshJob
import io.zonarosa.messenger.jobs.LeaveGroupV2Job
...
import io.zonarosa.service.api.groupsv2.GroupChangeReconstruct
import io.zonarosa.service.api.groupsv2.GroupHistoryPage
import io.zonarosa.service.api.groupsv2.InvalidGroupStateException
import io.zonarosa.service.api.groupsv2.NotAbleToApplyGroupV2ChangeException
import io.zonarosa.service.api.groupsv2.ReceivedGroupSendEndorsements
import io.zonarosa.service.api.groupsv2.getChangedFields
import io.zonarosa.service.api.groupsv2.isSilent
...
class GroupsV2StateProcessor private constructor(
  ...
  private val groupsApi = AppDependencies.zonarosaServiceAccountManager.getGroupsV2Api()
  ...
  fun updateGroupSendEndorsements() {
    ...
    if (receivedGroupSendEndorsements != null) {
      ...
      ZonaRosaDatabase.groups.updateGroupSendEndorsements(groupId, receivedGroupSendEndorsements)
    } else {
  ...
  fun updateLocalGroupToRevision(
    ...
    groupRecord: Optional<GroupRecord> = ZonaRosaDatabase.groups.getGroup(groupId),
  ...
  private fun canApplyP2pChange(
    ...
    if (ZonaRosaStore.internal.gv2IgnoreP2PChanges) {
      ...
      if (groupRecord.map { it.isActive }.orNull() == false && updatedGroupState != null && updatedGroupState == remoteGroupStateDiff.previousGroupState) {
        ...
        if (currentLocalState != null && applyGroupStateDiffResult.remainingRemoteGroupChanges.isEmpty()) {
          ...
          if (endorsements != null) {
            ...
            ZonaRosaDatabase.groups.updateGroupSendEndorsements(groupId, endorsements)
          }
        }
  ...
  private fun localIsAtLeast(groupRecord: Optional<GroupRecord>, revision: Int): Boolean {
    if (revision == LATEST || groupRecord.isEmpty || ZonaRosaDatabase.groups.isUnknownGroup(groupRecord)) {
      return false
    }
  ...
  private fun saveGroupUpdate(
    ...
    if (forceSave && updatedGroupState != null) {
      ...
      if (groupSendEndorsements != null) {
        ...
        ZonaRosaDatabase.groups.updateGroupSendEndorsements(groupId, groupSendEndorsements)
      }
  ...
  private fun saveGroupState(groupStateDiff: GroupStateDiff, updatedGroupState: DecryptedGroup, groupSendEndorsements: ReceivedGroupSendEndorsements?) {
    ...
    val needsAvatarFetch = if (previousGroupState == null) {
      val groupId = ZonaRosaDatabase.groups.create(groupMasterKey, updatedGroupState, groupSendEndorsements)

      if (groupId == null) {
        Log.w(TAG, "$logPrefix Group create failed, trying to update")
        ZonaRosaDatabase.groups.update(groupMasterKey, updatedGroupState, groupSendEndorsements)
      }
  ...
  internal class ProfileAndMessageHelper(private val aci: ACI, private val masterKey: GroupMasterKey, private val groupId: GroupId.V2) {
      ...
      if (selfAsMember != null) {
        ...
        if (addedBy != null) {
          ...
          if (addedBy.isBlocked && (previousGroupState == null || !DecryptedGroupUtil.isRequesting(previousGroupState, aci))) {
            ...
            ZonaRosaDatabase.recipients.setProfileSharing(Recipient.externalGroupExact(groupId).id, true)
            if (needsAvatarFetch) {
    ...
    fun leaveGroupLocally(serviceIds: ServiceIds) {
      if (!ZonaRosaDatabase.groups.isActive(groupId)) {
        Log.w(TAG, "Group $groupId has already been left.")

        val groupRecipient = Recipient.externalGroupExact(groupId)
        val threadId = ZonaRosaDatabase.threads.getThreadIdFor(groupRecipient.id)

        if (threadId != null) {
          ZonaRosaDatabase.drafts.clearDrafts(threadId)
      ...
      val decryptedGroup = ZonaRosaDatabase
        .groups
      ...
      try {
        val threadId = ZonaRosaDatabase.threads.getOrCreateThreadIdFor(groupRecipient)
        val id = ZonaRosaDatabase.messages.insertMessageOutbox(leaveMessage, threadId, false, null).messageId
      ...
      ZonaRosaDatabase.groups.setActive(groupId, false)
      ZonaRosaDatabase.groups.remove(groupId, Recipient.self().id)
    }
    ...
    fun persistLearnedProfileKeys(profileKeys: ProfileKeySet) {
      val updated = ZonaRosaDatabase.recipients.persistProfileKeySet(profileKeys)
    ...
    fun storeMessage(decryptedGroupV2Context: DecryptedGroupV2Context, timestamp: Long, serverGuid: String?) {
      ...
      val serviceIds = ZonaRosaStore.account.getServiceIds()
      ...
      if (outgoing) {
        ...
        try {
          val recipientId = ZonaRosaDatabase.recipients.getOrInsertFromGroupId(groupId)
          ...
          val messageId = ZonaRosaDatabase.messages.insertMessageOutbox(outgoingMessage, threadId, false, null).messageId
        ...
        try {
          ...
          val insertResult = ZonaRosaDatabase.messages.insertMessageInbox(groupMessage)

          if (insertResult.isPresent) {
            ZonaRosaDatabase.threads.update(insertResult.get().threadId, unarchive = false, allowDeletion = false)
