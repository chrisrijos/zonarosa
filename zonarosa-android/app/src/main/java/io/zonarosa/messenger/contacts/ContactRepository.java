...
public class ContactRepository {
  ...
  private static final List<Pair<String, ValueMapper>> SEARCH_CURSOR_MAPPERS = new ArrayList<Pair<String, ValueMapper>>() {{
    ...
    add(new Pair<>(NUMBER_COLUMN, cursor -> {
      ...
      if (phone != null) {
        phone = ZonaRosaE164Util.prettyPrint(phone);
      }
  ...
  @WorkerThread
  public @NonNull Cursor queryZonaRosaContacts(@NonNull String query) {
    return queryZonaRosaContacts(new RecipientTable.ContactSearchQuery(query, new RecipientTable.IncludeSelfMode.IncludeWithRemap(noteToSelfTitle), ContactSearchSortOrder.NATURAL));
  }

  @WorkerThread
  public @NonNull Cursor queryZonaRosaContacts(@NonNull RecipientTable.ContactSearchQuery contactSearchQuery) {
    Cursor cursor = TextUtils.isEmpty(contactSearchQuery.getQuery()) ? recipientTable.getZonaRosaContacts(contactSearchQuery.getIncludeSelfMode())
                                                                     : recipientTable.queryZonaRosaContacts(contactSearchQuery);
