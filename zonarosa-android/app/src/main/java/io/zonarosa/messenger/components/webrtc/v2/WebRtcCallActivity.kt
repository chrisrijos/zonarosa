...
import io.zonarosa.core.util.isInMultiWindowModeCompat
...
import io.zonarosa.messenger.components.webrtc.GroupCallSafetyNumberChangeNotificationUtil
import io.zonarosa.messenger.components.webrtc.InCallStatus
import io.zonarosa.messenger.components.webrtc.PendingParticipantsBottomSheet
...
import io.zonarosa.messenger.components.webrtc.WebRtcControls
import io.zonarosa.messenger.components.webrtc.requests.CallLinkIncomingRequestSheet
import io.zonarosa.messenger.conversation.ui.error.SafetyNumberChangeDialog
import io.zonarosa.messenger.dependencies.AppDependencies
import io.zonarosa.messenger.events.WebRtcViewModel
import io.zonarosa.messenger.keyvalue.ZonaRosaStore
import io.zonarosa.messenger.messagerequests.CalleeMustAcceptMessageRequestActivity
...
import io.zonarosa.messenger.util.EllapsedTimeFormatter
...
import io.zonarosa.messenger.webrtc.CallParticipantsViewState
import io.zonarosa.messenger.webrtc.audio.ZonaRosaAudioManager
import io.zonarosa.messenger.webrtc.audio.ZonaRosaAudioManager.ChosenAudioDeviceIdentifier
...
class WebRtcCallActivity : BaseActivity(), SafetyNumberChangeDialog.Callback, ReactWithAnyEmojiBottomSheetDialogFragment.Callback, RecaptchaProofBottomSheetFragment.Callback {
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    windowInfoTrackerCallbackAdapter.addWindowLayoutInfoListener(this, ZonaRosaExecutors.BOUNDED, windowLayoutInfoConsumer)
  ...
  override fun onStart() {
    ...
    ephemeralStateDisposable = AppDependencies.zonarosaCallManager
      .ephemeralStates()
  ...
  override fun onStop() {
    ...
    if (!isChangingConfigurations && !isInMultiWindowModeCompat()) {
      AppDependencies.zonarosaCallManager.setEnableVideo(false)
    }
    ...
    if (!viewModel.isCallStarting && !isChangingConfigurations) {
      ...
      if (state.callState.isPreJoinOrNetworkUnavailable || state.callState.isIncomingOrHandledElsewhere) {
        AppDependencies.zonarosaCallManager.cancelPreJoin()
      } else if (state.callState.inOngoingCall && isInPipMode()) {
        AppDependencies.zonarosaCallManager.relaunchPipOnForeground()
      }
    }
  }
  ...
  override fun onSendAnywayAfterSafetyNumberChange(changedRecipients: MutableList<RecipientId>) {
    ...
    if (state.groupCallState.isConnected) {
      AppDependencies.zonarosaCallManager.groupApproveSafetyChange(changedRecipients)
    } else {
  ...
  override fun onReactWithAnyEmojiSelected(emoji: String) {
    AppDependencies.zonarosaCallManager.react(emoji)
    callScreen.dismissCallOverflowPopup()
  }

  override fun onProofCompleted() {
    AppDependencies.zonarosaCallManager.resendMediaKeys()
  }
  ...
  private fun initializeViewModel() {
    ...
    AppDependencies.zonarosaCallManager.orientationChanged(true, orientation.degrees)
    ...
    rootView().viewTreeObserver.addOnGlobalLayoutListener {
      ...
      if (state.needsNewRequestSizes()) {
        requestNewSizesThrottle.publish { AppDependencies.zonarosaCallManager.updateRenderedResolutions() }
      }
    }
  ...
  private suspend fun observeVideoSinkAspectRatio(videoSink: io.zonarosa.messenger.components.webrtc.BroadcastVideoSink?) {
          ...
          MaterialAlertDialogBuilder(this)
            ...
            .setPositiveButton(R.string.WebRtcCallActivity__approve_all) { _, _ ->
              for (id in recipientIds) {
                AppDependencies.zonarosaCallManager.setCallLinkJoinRequestAccepted(id)
              }
            }
            .show()
        }
        ...
        PendingParticipantsBottomSheet.Action.DENY_ALL -> {
          MaterialAlertDialogBuilder(this)
            ...
            .setPositiveButton(R.string.WebRtcCallActivity__deny_all) { _, _ ->
              for (id in recipientIds) {
                AppDependencies.zonarosaCallManager.setCallLinkJoinRequestRejected(id)
              }
            }
            .show()
        }
  ...
  private fun handleSetAudioHandset() {
    AppDependencies.zonarosaCallManager.selectAudioDevice(ChosenAudioDeviceIdentifier(ZonaRosaAudioManager.AudioDevice.EARPIECE))
  }

  private fun handleSetAudioSpeaker() {
    AppDependencies.zonarosaCallManager.selectAudioDevice(ChosenAudioDeviceIdentifier(ZonaRosaAudioManager.AudioDevice.SPEAKER_PHONE))
  }

  private fun handleSetAudioBluetooth() {
    AppDependencies.zonarosaCallManager.selectAudioDevice(ChosenAudioDeviceIdentifier(ZonaRosaAudioManager.AudioDevice.BLUETOOTH))
  }

  private fun handleSetAudioWiredHeadset() {
    AppDependencies.zonarosaCallManager.selectAudioDevice(ChosenAudioDeviceIdentifier(ZonaRosaAudioManager.AudioDevice.WIRED_HEADSET))
  }

  private fun handleSetMuteAudio(enabled: Boolean) {
    AppDependencies.zonarosaCallManager.setMuteAudio(enabled)
  }
  ...
  private fun handleSetMuteVideo(muted: Boolean) {
    ...
    if (recipient != Recipient.UNKNOWN) {
      askCameraPermissions {
        AppDependencies.zonarosaCallManager.setEnableVideo(!muted)
      }
    }
  }

  private fun handleFlipCamera() {
    AppDependencies.zonarosaCallManager.flipCamera()
  }
  ...
  private fun handleAnswerWithVideo() {
    val onGranted: () -> Unit = {
      callScreen.setStatus(getString(R.string.RedPhone_answering))
      AppDependencies.zonarosaCallManager.acceptCall(true)
      handleSetMuteVideo(false)
    }
  ...
  private fun handleDenyCall() {
    val recipient = viewModel.recipient.get()
    if (recipient != Recipient.UNKNOWN) {
      AppDependencies.zonarosaCallManager.denyCall()
  ...
  private fun handleEndCall() {
    ...
    AppDependencies.zonarosaCallManager.localHangup()
  }
  ...
  private fun handleCallBusy() {
    ...
    delayedFinish(ZonaRosaCallManager.BUSY_TONE_LENGTH.toLong())
  }
  ...
  private fun handleCallPreJoin(event: WebRtcViewModel) {
    if (event.groupState.isNotIdle) {
      ...
      if (event.ringGroup && event.areRemoteDevicesInCall()) {
        AppDependencies.zonarosaCallManager.setRingGroup(false)
      }
    }
  }
  ...
  private fun handleInCallStatus(inCallStatus: InCallStatus) {
    when (inCallStatus) {
      is InCallStatus.ElapsedTime -> {
        ...
        callScreen.setStatus(getString(R.string.WebRtcCallActivity__zonarosa_s, formatter.toString()))
      }
  ...
  private fun updateGroupMembersForGroupCall() {
    AppDependencies.zonarosaCallManager.requestUpdateGroupMembers()
  }
  ...
  private fun handleGroupMemberCountChange(count: Int) {
    ...
    AppDependencies.zonarosaCallManager.setRingGroup(canRing)
  }
  ...
  private fun updateIncomingRingingVanity() {
    ...
    AppDependencies.zonarosaCallManager.setIncomingRingingVanity(shouldEnable)
  }

  private fun disableIncomingRingingVanity() {
    val event = previousEvent ?: return
    if (event.state == WebRtcViewModel.State.CALL_INCOMING) {
      AppDependencies.zonarosaCallManager.setIncomingRingingVanity(false)
    }
  }

  private fun initializeScreenshotSecurity() {
    if (ZonaRosaPreferences.isScreenSecurityEnabled(this)) {
  ...
  private fun startCall(isVideoCall: Boolean) {
    ...
    if (isVideoCall) {
      AppDependencies.zonarosaCallManager.startOutgoingVideoCall(viewModel.recipient.get())
    } else {
      AppDependencies.zonarosaCallManager.startOutgoingAudioCall(viewModel.recipient.get())
    }
  ...
  private inner class ControlsListener : CallScreenControlsListener {
    ...
    override fun onAudioOutputChanged31(audioOutput: WebRtcAudioDevice) {
      ...
      AppDependencies.zonarosaCallManager.selectAudioDevice(ChosenAudioDeviceIdentifier(audioOutput.deviceId!!))
    }
    ...
    override fun onRingGroupChanged(ringGroup: Boolean, ringingAllowed: Boolean) {
      if (ringingAllowed) {
        AppDependencies.zonarosaCallManager.setRingGroup(ringGroup)
  ...
  private inner class PendingParticipantsViewListener : PendingParticipantsListener {
    ...
    override fun onAllowPendingRecipient(pendingRecipient: Recipient) {
      AppDependencies.zonarosaCallManager.setCallLinkJoinRequestAccepted(pendingRecipient.id)
    }

    override fun onRejectPendingRecipient(pendingRecipient: Recipient) {
      AppDependencies.zonarosaCallManager.setCallLinkJoinRequestRejected(pendingRecipient.id)
    }
