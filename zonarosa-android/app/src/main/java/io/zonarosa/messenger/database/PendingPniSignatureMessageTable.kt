 ...
 */
class PendingPniSignatureMessageTable(context: Context, databaseHelper: ZonaRosaDatabase) : DatabaseTable(context, databaseHelper), RecipientIdDatabaseReference {
    ...
    pendingPniSignaturesCacheLock.writeLock().withLock {
      writableDatabase.withinTransaction { db ->
        ...
        if (!stillPending) {
          ...
          ZonaRosaDatabase.recipients.clearNeedsPniSignature(recipientId)
