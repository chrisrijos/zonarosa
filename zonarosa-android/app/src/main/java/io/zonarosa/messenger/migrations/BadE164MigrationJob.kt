...
internal class BadE164MigrationJob(
  ...
  override fun performMigration() {
    ...
    val invalidWithOtherIdentifiersCount = ZonaRosaDatabase.recipients.removeInvalidE164sFromRecipientsWithOtherIdentifiers()
    ...
    val invalidWithNoMessagesCount = ZonaRosaDatabase.recipients.deleteRecipientsWithInvalidE164sAndNoMessages()
    Log.d(TAG, "Deleted $invalidWithNoMessagesCount recipients with invalid e164's and no messages.")
    stopwatch.split("no-msg")

    val invalidEntries = ZonaRosaDatabase.recipients.getRecipientsWithInvalidE164s()
    ...
    for (entry in invalidEntries) {
      ...
      val formattedNumber = ZonaRosaE164Util.formatAsE164(entry.e164)
      ...
      ZonaRosaDatabase.recipients.updateE164(entry.id, entry.e164)
