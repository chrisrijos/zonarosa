...
import io.zonarosa.messenger.restore.devicetransfer.DeviceTransferFragment
  ...
  private inner class ServerTaskListener {
    ...
    fun onEventMainThread(event: NewDeviceServerTask.Status) {
      ...
      when (event.state) {
        ...
        NewDeviceServerTask.Status.State.FAILURE_VERSION_DOWNGRADE -> abort(R.string.NewDeviceTransfer__cannot_transfer_from_a_newer_version_of_zonarosa)
