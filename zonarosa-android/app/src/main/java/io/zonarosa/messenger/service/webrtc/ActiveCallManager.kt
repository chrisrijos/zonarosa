...
import io.zonarosa.messenger.service.SafeForegroundService
...
import io.zonarosa.messenger.webrtc.UncaughtExceptionHandlerManager
...
import io.zonarosa.messenger.webrtc.audio.ZonaRosaAudioManager.Companion.create
import io.zonarosa.messenger.webrtc.locks.LockManager
import java.util.concurrent.locks.ReentrantLock
import kotlin.concurrent.withLock

/**
 * Entry point for [ZonaRosaCallManager] and friends to interact with the Android system.
 *
...
class ActiveCallManager(
...
) : ZonaRosaAudioManager.EventListener {
  ...
  private val callManager = AppDependencies.zonarosaCallManager
  ...
  private var zonarosaAudioManager: ZonaRosaAudioManager? = null
  ...
  fun shutdown(fromTimeout: Boolean = false) {
    ...
    zonarosaAudioManager?.shutdown()
    zonarosaAudioManager = null
  ...
  fun sendAudioCommand(audioCommand: AudioManagerCommand) {
    if (zonarosaAudioManager == null) {
      zonarosaAudioManager = create(application, this)
    }

    Log.i(TAG, "Sending audio command [" + audioCommand.javaClass.simpleName + "] to " + zonarosaAudioManager?.javaClass?.simpleName)
    zonarosaAudioManager!!.handleCommand(audioCommand)
  }
  ...
  override fun onAudioDeviceChanged(activeDevice: ZonaRosaAudioManager.AudioDevice, devices: Set<ZonaRosaAudioManager.AudioDevice>) {
    ...
    private class HangUpRtcOnPstnCallAnsweredListener : PhoneStateListener() {
      ...
      override fun onCallStateChanged(state: Int, phoneNumber: String) {
        ...
        if (state == TelephonyManager.CALL_STATE_OFFHOOK) {
          hangup()
          Log.i(TAG, "Device phone call ended ZonaRosa call.")
        }
      }
  ...
  class ActiveCallServiceReceiver : BroadcastReceiver() {
    ...
    companion object {
      const val ACTION_DENY = "io.zonarosa.messenger.service.webrtc.ActiveCallAction.DENY"
      const val ACTION_HANGUP = "io.zonarosa.messenger.service.webrtc.ActiveCallAction.HANGUP"
    }

    override fun onReceive(context: Context?, intent: Intent?) {
      Log.d(TAG, "action: ${intent?.action}")
      when (intent?.action) {
        ACTION_DENY -> AppDependencies.zonarosaCallManager.denyCall()
        ACTION_HANGUP -> AppDependencies.zonarosaCallManager.localHangup()
      }
    }
  }
  ...
  private class NetworkReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
      ...
      AppDependencies.zonarosaCallManager.apply {
  ...
  private class PowerButtonReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
      if (Intent.ACTION_SCREEN_OFF == intent.action) {
        AppDependencies.zonarosaCallManager.screenOff()
      }
    }
  }
