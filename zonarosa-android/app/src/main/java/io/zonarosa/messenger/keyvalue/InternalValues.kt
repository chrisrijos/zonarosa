...
import io.zonarosa.ringrtc.CallManager.DataMode
import io.zonarosa.messenger.BuildConfig
import io.zonarosa.messenger.backup.v2.proto.BackupDebugInfo
import io.zonarosa.messenger.util.Environment.Calling.defaultSfuUrl
import io.zonarosa.messenger.util.RemoteConfig

class InternalValues internal constructor(store: KeyValueStore) : ZonaRosaStoreValues(store) {
  companion object {
  ...
  /**
   * Show detailed recipient info in the [io.zonarosa.messenger.components.settings.conversation.InternalConversationSettingsFragment].
   */
   ...
   * the default SFU will be returned. This ensures that if the [BuildConfig.ZONAROSA_SFU_INTERNAL_URLS] list changes,
  ...
  var groupCallingServer: String
    get() {
      ...
      if (internalServer != null && !listOf(*BuildConfig.ZONAROSA_SFU_INTERNAL_URLS).contains(internalServer)) {
        internalServer = null
      }
      return internalServer ?: BuildConfig.ZONAROSA_SFU_URL
    }
  ...
  private fun <T> ZonaRosaStoreValueDelegate<T>.defaultForExternalUsers(): ZonaRosaStoreValueDelegate<T> {
    return this.withPrecondition { RemoteConfig.internalUser }
  }

  private fun ZonaRosaStoreValueDelegate<Boolean>.falseForExternalUsers(): ZonaRosaStoreValueDelegate<Boolean> {
