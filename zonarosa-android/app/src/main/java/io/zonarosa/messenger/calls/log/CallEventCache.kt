...
import io.zonarosa.messenger.database.CallTable.Direction
import io.zonarosa.messenger.database.CallTable.Event
import io.zonarosa.messenger.database.CallTable.Type
...
class CallEventCache(
  private val executor: Executor = ZonaRosaExecutors.newCachedSingleThreadExecutor("call-event-cache", ThreadUtil.PRIORITY_IMPORTANT_BACKGROUND_THREAD)
) {
  companion object {
    ...
    private fun readDataFromDatabase(): List<CacheRecord> {
      ...
      val events = ZonaRosaDatabase.calls.getCallsForCache(10_000).readToList { it.readCacheRecord() }
    ...
    private fun canUserBeginCall(peer: Recipient, decryptedGroup: ByteArray?): Boolean {
      return if (peer.isGroup && decryptedGroup != null) {
        ...
        return proto.isAnnouncementGroup != EnabledState.ENABLED ||
          proto.members.firstOrNull() { it.aciBytes == ZonaRosaStore.account.aci?.toByteString() }?.role == Member.Role.ADMINISTRATOR
      } else {
        true
      }
    }
