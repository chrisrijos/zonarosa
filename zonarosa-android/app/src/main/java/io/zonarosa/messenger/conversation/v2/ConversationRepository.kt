...
import io.zonarosa.messenger.ShortcutLauncherActivity
...
import io.zonarosa.messenger.conversation.v2.RequestReviewState.GroupReviewState
import io.zonarosa.messenger.conversation.v2.RequestReviewState.IndividualReviewState
import io.zonarosa.messenger.conversation.v2.data.ConversationDataSource
...
import io.zonarosa.messenger.dependencies.AppDependencies.databaseObserver
import io.zonarosa.messenger.dependencies.AppDependencies.expiringMessageManager
import io.zonarosa.messenger.groups.GroupNotAMemberException
import io.zonarosa.messenger.jobs.GroupSendJobHelper
import io.zonarosa.messenger.jobs.MultiDeviceViewOnceOpenJob
import io.zonarosa.messenger.jobs.UnpinMessageJob
import io.zonarosa.messenger.keyboard.KeyboardUtil
import io.zonarosa.messenger.keyvalue.ZonaRosaStore.Companion.settings
...
class ConversationRepository(
  ...
  private val oldConversationRepository = io.zonarosa.messenger.conversation.ConversationRepository()
  ...
  fun getConversationThreadState(threadId: Long, requestedStartPosition: Int): Single<ConversationThreadState> {
    return Single.fromCallable {
      ...
      ZonaRosaLocalMetrics.ConversationOpen.onMetadataLoadStarted()
      val metadata = oldConversationRepository.getConversationData(threadId, recipient, requestedStartPosition)
      ZonaRosaLocalMetrics.ConversationOpen.onMetadataLoaded()
  ...
  fun endPoll(pollId: Long): Completable {
    return Completable.create { emitter ->
      ...
      val messageRecord = ZonaRosaDatabase.messages.getMessageRecord(poll!!.messageId)
      val threadRecipient = ZonaRosaDatabase.threads.getRecipientForThreadId(messageRecord.threadId)!!
      ...
      val possibleTargets: List<Recipient> = if (threadRecipient.isPushV2Group) {
        ZonaRosaDatabase.groups.getGroupMembers(threadRecipient.requireGroupId().requireV2(), GroupTable.MemberSet.FULL_MEMBERS_EXCLUDING_SELF)
          .map { it.resolve() }
          .distinctBy { it.id }
      } else {
      ...
      if (sendResults.completed.isNotEmpty() || possibleTargets.isEmpty()) {
        val allocatedThreadId = ZonaRosaDatabase.threads.getOrCreateValidThreadId(threadRecipient, messageRecord.threadId, message.distributionType)
        val outgoingMessage = applyUniversalExpireTimerIfNecessary(applicationContext, threadRecipient, message, allocatedThreadId)
        val insertResult = ZonaRosaDatabase.messages.insertMessageOutbox(outgoingMessage, allocatedThreadId, false, null)
        val messageId = insertResult.messageId

        ZonaRosaDatabase.threads.update(threadId = allocatedThreadId, unarchive = true, syncThreadDelete = true)
  ...
  fun sendEndPoll(threadRecipient: Recipient, message: OutgoingMessage, destinations: List<Recipient>, isSelf: Boolean, messageId: Long): List<SendMessageResult?> {
    ...
    val groupRecord: GroupRecord? = if (threadRecipient.isPushV2Group) ZonaRosaDatabase.groups.getGroup(threadRecipient.requireGroupId()).getOrNull() else null
    ...
    val message = builder
      ...
      .withPollTerminate(ZonaRosaServiceDataMessage.PollTerminate(message.messageExtras!!.pollTerminate!!.targetTimestamp))
      .build()
  ...
  fun getPinnedMessages(threadId: Long): List<MmsMessageRecord> {
    return ZonaRosaDatabase.messages.getPinnedMessages(threadId = threadId, orderByPinned = true)
  }
  ...
  fun pinMessage(messageRecord: MessageRecord, duration: Duration, threadRecipient: Recipient): Completable {
    return Completable.create { emitter ->
      ...
      val possibleTargets: List<Recipient> = if (isGroup) {
        ZonaRosaDatabase.groups.getGroupMembers(threadRecipient.requireGroupId().requireV2(), GroupTable.MemberSet.FULL_MEMBERS_EXCLUDING_SELF).map { it.resolve() }.distinctBy { it.id }
      } else {
        listOf(threadRecipient)
      }
      ...
      if (sendResults.completed.isNotEmpty() || possibleTargets.isEmpty()) {
        ...
        if (outgoingMessage.expiresIn > 0) {
          ZonaRosaDatabase.messages.markExpireStarted(insertResult.messageId)
          expiringMessageManager.scheduleDeletion(insertResult.messageId, true, message.expiresIn)
        }

        if (sendResults.skipped.isNotEmpty()) {
          val messageRecord = ZonaRosaDatabase.messages.getMessageRecord(insertResult.messageId)
          val filterRecipientIds = (sendResults.skipped - sendResults.completed.map { it.id }).toSet()
          Log.i(TAG, "Some recipients skipped when sending pin message. Resending to $filterRecipientIds")
          MessageSender.resendGroupMessage(applicationContext, messageRecord, filterRecipientIds)
        } else {
          ZonaRosaDatabase.messages.markAsSent(insertResult.messageId, true)
        }
        emitter.onComplete()
      } else {
  ...
  fun unpinMessage(messageId: Long): Completable {
    return Completable.create { emitter ->
      ...
      if (sendResults.completed.isNotEmpty() || possibleTargets.isEmpty()) {
        ZonaRosaDatabase.messages.unpinMessage(messageId = messageId, threadId = message.threadId)
  ...
  fun setLastVisibleMessageTimestamp(threadId: Long, lastVisibleMessageTimestamp: Long) {
    ZonaRosaExecutors.BOUNDED_IO.execute { ZonaRosaDatabase.threads.setLastScrolled(threadId, lastVisibleMessageTimestamp) }
  }
  ...
  fun getQuotedMessagePosition(threadId: Long, quoteId: Long, authorId: RecipientId): Single<Int> {
    return Single.fromCallable {
      ZonaRosaDatabase.messages.getQuotedMessagePosition(threadId, quoteId, authorId)
    }.subscribeOn(Schedulers.io())
  }

  fun getMessageResultPosition(threadId: Long, receivedTimestamp: Long): Single<Int> {
    return Single.fromCallable {
      ZonaRosaDatabase.messages.getMessagePositionInConversation(threadId, receivedTimestamp)
    }.subscribeOn(Schedulers.io())
  }

  fun getNextMentionPosition(threadId: Long): Single<Int> {
    return Single.fromCallable {
      val details = ZonaRosaDatabase.messages.getOldestUnreadMentionDetails(threadId)
      if (details == null) {
        -1
      } else {
        ZonaRosaDatabase.messages.getMessagePositionInConversation(threadId, details.second, details.first)
      }
    }.subscribeOn(Schedulers.io())
  }
  ...
  fun getMessagePosition(threadId: Long, messageId: Long): Single<Int> {
    return Single.fromCallable {
      ...
      ZonaRosaDatabase.messages.getMessagePositionInConversation(threadId, message.dateReceived, message.fromRecipient.id)
    }.subscribeOn(Schedulers.io())
  }

  fun getMessagePosition(threadId: Long, dateReceived: Long, authorId: RecipientId): Single<Int> {
    return Single.fromCallable {
      ZonaRosaDatabase.messages.getMessagePositionInConversation(threadId, dateReceived, authorId)
    }.subscribeOn(Schedulers.io())
  }
  ...
  private fun getUnreadCount(threadId: Long): Int {
    return ZonaRosaDatabase.messages.getUnreadCount(threadId)
  }

  private fun getUnreadMentionsCount(threadId: Long): Int {
    return ZonaRosaDatabase.messages.getUnreadMentionCount(threadId)
  }
  ...
  fun dismissRequestReviewState(threadRecipientId: RecipientId) {
    ZonaRosaExecutors.BOUNDED_IO.execute {
      ZonaRosaDatabase.nameCollisions.markCollisionsForThreadRecipientDismissed(threadRecipientId)
    }
  }
  ...
  fun getRequestReviewState(recipient: Recipient, group: GroupRecord?, messageRequest: MessageRequestState): Single<RequestReviewState> {
    return Single.fromCallable {
      ...
      if (group == null) {
        val recipientsToReview = ZonaRosaDatabase.nameCollisions.getCollisionsForThreadRecipientId(recipient.id)
      ...
      if (group != null && group.isV2Group) {
        ...
        val duplicateRecipients: List<ReviewRecipient> = ZonaRosaDatabase.nameCollisions.getCollisionsForThreadRecipientId(group.recipientId)
  ...
  fun setConversationMuted(recipientId: RecipientId, until: Long) {
    ZonaRosaExecutors.BOUNDED_IO.execute { recipients.setMuted(recipientId, until) }
  }
  ...
  fun updateStickerLastUsedTime(stickerRecord: StickerRecord, timestamp: Duration) {
    ZonaRosaExecutors.BOUNDED_IO.execute {
      ZonaRosaDatabase.stickers.updateStickerLastUsedTime(stickerRecord.rowId, timestamp.inWholeMilliseconds)
    }
  }

  fun startExpirationTimeout(expirationInfos: List<MessageTable.ExpirationInfo>) {
    ZonaRosaDatabase.messages.markExpireStarted(expirationInfos.map { it.id to it.expireStarted })
  ...
  fun getEarliestMessageSentDate(threadId: Long): Single<Long> {
    return Single
      .fromCallable { ZonaRosaDatabase.messages.getEarliestMessageSentDate(threadId) }
      .subscribeOn(Schedulers.io())
  }
