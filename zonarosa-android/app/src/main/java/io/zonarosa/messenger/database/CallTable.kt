...
import io.zonarosa.core.util.IntSerializer
...
import io.zonarosa.core.util.flatten
 ...
 */
class CallTable(context: Context, databaseHelper: ZonaRosaDatabase) : DatabaseTable(context, databaseHelper), RecipientIdDatabaseReference {
  ...
  fun insertOneToOneCall(callId: Long, timestamp: Long, peer: RecipientId, type: Type, direction: Direction, event: Event) {
    ...
    writableDatabase.withinTransaction {
      val result = ZonaRosaDatabase.messages.insertCallLog(peer, messageType, timestamp, direction == Direction.OUTGOING)
    ...
    return writableDatabase.withinTransaction {
      ...
      if (call != null) {
        ...
        if (call.messageId == null) {
          ...
          ZonaRosaDatabase.messages.updateCallLog(call.messageId, call.messageType)
        }
  ...
  fun markCallDeletedFromSyncEvent(call: Call) {
    ...
    writableDatabase.withinTransaction { db ->
      ...
      if (call.messageId != null) {
        ZonaRosaDatabase.messages.deleteMessage(call.messageId)
      }
    }
  ...
  fun insertAcceptedGroupCall(
    ...
    writableDatabase.withinTransaction { db ->
      val messageId: MessageId? = if (type == Type.GROUP_CALL) {
        ZonaRosaDatabase.messages.insertGroupCall(
  ...
  private fun handleGroupUpdate(
    ...
    writableDatabase.withinTransaction {
      ...
      val messageId: MessageId = if (call != null) {
        ...
        ZonaRosaDatabase.messages.updateGroupCall(
          call.messageId,
          peekGroupCallEraId,
          peekJoinedUuids,
          isCallFull,
  ...
  fun updateGroupCallFromPeek(
    ...
    val recipientId = ZonaRosaDatabase.threads.getRecipientIdForThreadId(threadId)
    ...
    ZonaRosaDatabase.messages.updatePreviousGroupCall(
      threadId = threadId,
  ...
  private fun createEventFromRingState(
    ...
    writableDatabase.withinTransaction { db ->
      val messageId = ZonaRosaDatabase.messages.insertGroupCall(
  ...
  fun setTimestamp(callId: Long, recipientId: RecipientId, timestamp: Long) {
    writableDatabase.withinTransaction { db ->
      ...
      if (call.messageId != null) {
        ZonaRosaDatabase.messages.updateCallTimestamps(call.messageId, timestamp)
      }
    }
  ...
  fun deleteNonAdHocCallEvents(callRowIds: Set<Long>) {
    ...
    ZonaRosaDatabase.messages.deleteCallUpdates(messageIds)
  ...
  fun deleteAllNonAdHocCallEventsExcept(callRowIds: Set<Long>, missedOnly: Boolean) {
    ...
    if (callRowIds.isEmpty()) {
      ...
      threadIds.forEach {
        ZonaRosaDatabase.threads.update(
          threadId = it,
          unarchive = false,
          allowDeletion = true
        )
  ...
  fun markRingingCallsAsMissed() {
    writableDatabase.withinTransaction { db ->
      ...
      ZonaRosaDatabase.messages.clearIsRingingOnLocalDeviceFlag(messageIds)
    }
  }
