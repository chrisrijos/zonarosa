...
class ArchiveCommitAttachmentDeletesJob private constructor(parameters: Parameters) : Job(parameters) {
  ...
  companion object {
     ...
     */
    fun deleteMediaObjectsFromCdn(tag: String, attachmentsToDelete: Set<ArchivedMediaObject>, backoffGenerator: () -> Long, cancellationZonaRosa: () -> Boolean): Result? {
  ...
  override fun run(): Result {
    ...
    var mediaObjects = ZonaRosaDatabase.backupMediaSnapshots.getPageOfOldMediaObjects(REMOTE_DELETE_BATCH_SIZE)
    ...
    while (mediaObjects.isNotEmpty()) {
      ...
      ZonaRosaDatabase.backupMediaSnapshots.deleteOldMediaObjects(mediaObjects)

      mediaObjects = ZonaRosaDatabase.backupMediaSnapshots.getPageOfOldMediaObjects(REMOTE_DELETE_BATCH_SIZE)
    }
