...
import io.zonarosa.messenger.conversation.ConversationAdapterBridge
...
import io.zonarosa.messenger.conversation.v2.data.ConversationUpdate
import io.zonarosa.messenger.conversation.v2.data.IncomingMedia
import io.zonarosa.messenger.conversation.v2.data.IncomingTextOnly
import io.zonarosa.messenger.conversation.v2.data.OutgoingMedia
import io.zonarosa.messenger.conversation.v2.data.OutgoingTextOnly
import io.zonarosa.messenger.conversation.v2.data.ThreadHeader
import io.zonarosa.messenger.conversation.v2.items.ChatColorsDrawable
import io.zonarosa.messenger.conversation.v2.items.V2ConversationContext
import io.zonarosa.messenger.conversation.v2.items.V2ConversationItemMediaViewHolder
import io.zonarosa.messenger.conversation.v2.items.V2ConversationItemTextOnlyViewHolder
import io.zonarosa.messenger.conversation.v2.items.V2Payload
import io.zonarosa.messenger.conversation.v2.items.bridge
...
import io.zonarosa.messenger.groups.v2.GroupDescriptionUtil
...
class ConversationAdapterV2(
  ...
  init {
    ...
    if (ZonaRosaStore.internal.useConversationItemV2Media) {
  ...
  inner class ThreadHeaderViewHolder(itemView: View) : MappingViewHolder<ThreadHeader>(itemView) {
    ...
    override fun bind(model: ThreadHeader) {
      ...
      if (recipient.isGroup) {
        ...
        val subtitle: String? = recipient.takeIf { it.shouldShowE164 }?.e164?.map { e164: String? -> ZonaRosaE164Util.prettyPrint(e164!!) }?.orElse(null)
