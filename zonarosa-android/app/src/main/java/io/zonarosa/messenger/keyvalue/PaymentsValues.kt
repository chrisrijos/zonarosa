...
import io.zonarosa.messenger.payments.GeographicalRestrictions
import io.zonarosa.messenger.payments.Mnemonic
import io.zonarosa.messenger.payments.MobileCoinLedgerWrapper
import io.zonarosa.messenger.payments.currency.CurrencyUtil
import io.zonarosa.messenger.payments.proto.MobileCoinLedger
...
class PaymentsValues internal constructor(store: KeyValueStore) : ZonaRosaStoreValues(store) {
    ...
    get() {
      ...
      return if (RemoteConfig.payments) {
        if (mobileCoinPaymentsEnabled()) {
          if (GeographicalRestrictions.e164Allowed(ZonaRosaStore.account.e164)) {
            PaymentsAvailability.WITHDRAW_AND_SEND
          } else {
            return PaymentsAvailability.WITHDRAW_ONLY
          }
        } else if (GeographicalRestrictions.e164Allowed(ZonaRosaStore.account.e164)) {
  ...
  fun showUpdatePinInfoCard(): Boolean {
    return if (userHasLargeBalance() &&
      ...
      ZonaRosaStore.pin.keyboardType == PinKeyboardType.NUMERIC
    ) {
  ...
  private fun determineCurrency(): Currency {
    val localE164: String = ZonaRosaStore.account.e164 ?: ""
