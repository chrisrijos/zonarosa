...
import static io.zonarosa.messenger.jobs.SendReadReceiptJob.MAX_TIMESTAMPS;
...
public class SendViewedReceiptJob extends BaseJob {
  ...
  public void onRun() throws IOException, UntrustedIdentityException {
    ...
    boolean canSendNonStoryReceipts = ZonaRosaPreferences.isReadReceiptsEnabled(context);
    boolean canSendStoryReceipts    = ZonaRosaStore.story().getViewedReceiptsEnabled();

    List<MessageId> foundMessageIds       = new LinkedList<>();
    List<Long>      messageSentTimestamps = new LinkedList<>();
    List<StoryType> storyTypes            = ZonaRosaDatabase.messages().getStoryTypes(this.messageIds);
    ...
    if (storyTypes.isEmpty() && !ZonaRosaPreferences.isReadReceiptsEnabled(context)) {
    ...
    ZonaRosaServiceReceiptMessage receiptMessage = new ZonaRosaServiceReceiptMessage(ZonaRosaServiceReceiptMessage.Type.VIEWED,
    ...
    if (Util.hasItems(foundMessageIds)) {
      ZonaRosaDatabase.messageLog().insertIfPossible(recipientId, timestamp, result, ContentHint.IMPLICIT, foundMessageIds, false);
    }
  }
