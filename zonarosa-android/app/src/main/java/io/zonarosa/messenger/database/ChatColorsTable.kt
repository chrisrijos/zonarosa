...
class ChatColorsTable(context: Context, databaseHelper: ZonaRosaDatabase) : DatabaseTable(context, databaseHelper) {
  ...
  private fun insertChatColors(chatColors: ChatColors): ChatColors {
    ...
    val db: SQLiteDatabase = databaseHelper.zonarosaWritableDatabase
  ...
  private fun updateChatColors(chatColors: ChatColors): ChatColors {
    ...
    if (ZonaRosaStore.chatColors.chatColors?.id == chatColors.id) {
      ZonaRosaStore.chatColors.chatColors = chatColors
    }

    ZonaRosaDatabase.recipients.onUpdatedChatColors(chatColors)
    notifyListeners()
  ...
  fun deleteChatColors(chatColors: ChatColors) {
    ...
    ZonaRosaDatabase.recipients.onDeletedChatColors(chatColors)
    notifyListeners()
  }
