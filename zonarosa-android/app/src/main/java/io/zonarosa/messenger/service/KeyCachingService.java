...
import io.zonarosa.messenger.DummyActivity;
import io.zonarosa.messenger.MainActivity;
import io.zonarosa.messenger.R;
import io.zonarosa.messenger.crypto.InvalidPassphraseException;
...
import io.zonarosa.messenger.util.DynamicLanguage;
...
public class KeyCachingService extends Service {
  ...
  public static synchronized boolean isLocked(Context context) {
    boolean locked = masterSecret == null && (!ZonaRosaStore.settings().getPassphraseDisabled() || ZonaRosaStore.settings().getScreenLockEnabled());

    if (locked) {
      Log.d(TAG, "Locked! PasswordDisabled: " + ZonaRosaStore.settings().getPassphraseDisabled() + ", ScreenLock: " + ZonaRosaStore.settings().getScreenLockEnabled());
    }
  ...
  public static synchronized @Nullable MasterSecret getMasterSecret(Context context) {
    if (masterSecret == null && (ZonaRosaStore.settings().getPassphraseDisabled() && !ZonaRosaStore.settings().getScreenLockEnabled())) {
      try {
  ...
  public void onCreate() {
    ...
    if (ZonaRosaStore.settings().getPassphraseDisabled() && !ZonaRosaStore.settings().getScreenLockEnabled()) {
      try {
  ...
  private void handleDisableService() {
    if (ZonaRosaStore.settings().getPassphraseDisabled() &&
        !ZonaRosaStore.settings().getScreenLockEnabled())
    {
      stopForeground(true);
    }
  }
  ...
  private static void startTimeoutIfAppropriate(@NonNull Context context) {
    ...
    boolean timeoutEnabled   = ZonaRosaStore.settings().getPassphraseTimeoutEnabled();
    boolean passLockActive   = timeoutEnabled && !ZonaRosaStore.settings().getPassphraseDisabled();

    long    screenTimeout       = ZonaRosaStore.settings().getScreenLockTimeout();
    boolean screenLockActive    = ZonaRosaStore.settings().getScreenLockEnabled();
    ...
    if (immediateScreenLock) {
      ...
      ZonaRosaStore.settings().setScreenLockTimeout(60);
    ...
    if (!appVisible && secretSet && (passLockActive || screenLockActive)) {
      if (immediateScreenLock) {
        ...
        long passphraseTimeoutMinutes = ZonaRosaStore.settings().getPassphraseTimeout();
        long screenLockTimeoutSeconds = ZonaRosaStore.settings().getScreenLockTimeout();

        long timeoutMillis;

        if (!ZonaRosaStore.settings().getPassphraseDisabled()) timeoutMillis = TimeUnit.MINUTES.toMillis(passphraseTimeoutMinutes);
  ...
  private void foregroundService() {
    ...
    builder.setContentText(getString(R.string.KeyCachingService_zonarosa_passphrase_cached));
    builder.setSmallIcon(R.drawable.ic_notification_unlocked);
    builder.setColor(ContextCompat.getColor(this, io.zonarosa.core.ui.R.color.zonarosa_light_colorSecondary));
    builder.setWhen(0);
    builder.setPriority(Notification.PRIORITY_MIN);
    builder.setOngoing(true);

    builder.addAction(io.zonarosa.core.ui.R.drawable.symbol_lock_24, getString(R.string.KeyCachingService_lock), buildLockIntent());
