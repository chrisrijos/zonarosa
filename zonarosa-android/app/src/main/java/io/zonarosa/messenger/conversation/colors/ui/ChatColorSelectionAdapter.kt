...
import io.zonarosa.messenger.util.withFixedSize
...
class ChatColorSelectionAdapter(
  ...
  class ChatColorMappingViewHolder(
    ...
    override fun bind(model: ChatColorMappingModel) {
      ...
      if (model.isAuto && ZonaRosaStore.chatColors.shouldShowAutoTooltip) {
        ZonaRosaStore.chatColors.shouldShowAutoTooltip = false
        TooltipPopup.forTarget(itemView)
          .setText(R.string.ChatColorSelectionFragment__auto_matches_the_color_to_the_wallpaper)
          .setBackgroundTint(ContextCompat.getColor(context, R.color.zonarosa_accent_primary))
