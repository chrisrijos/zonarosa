...
import io.zonarosa.core.util.readParcelableCompat
import io.zonarosa.core.util.readSerializableCompat
import io.zonarosa.messenger.recipients.RecipientId
import io.zonarosa.messenger.util.ParcelUtil

/**
 * Commands that can be issued to [ZonaRosaAudioManager] to perform various tasks.
 *
 * Additional context: The audio management is tied closely with the Android audio and thus benefits from being
 * tied to the [io.zonarosa.messenger.service.webrtc.ActiveCallManager] lifecycle. Because of this, all
...
sealed class AudioManagerCommand : Parcelable {
  ...
  class SetDefaultDevice(val recipientId: RecipientId?, val device: ZonaRosaAudioManager.AudioDevice, val clearUserEarpieceSelection: Boolean) : AudioManagerCommand() {
    override fun writeToParcel(parcel: Parcel, flags: Int) {
      parcel.writeParcelable(recipientId, flags)
    ...
    companion object {
      ...
      val CREATOR: Parcelable.Creator<SetDefaultDevice> = ParcelCheat { parcel ->
        SetDefaultDevice(
          ...
          device = parcel.readSerializableCompat(ZonaRosaAudioManager.AudioDevice::class.java)!!,
