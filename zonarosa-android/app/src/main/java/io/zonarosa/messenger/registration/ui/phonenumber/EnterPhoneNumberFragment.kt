...
import io.zonarosa.messenger.databinding.FragmentRegistrationEnterPhoneNumberBinding
...
import io.zonarosa.messenger.registration.ui.RegistrationState
...
import io.zonarosa.messenger.registration.ui.countrycode.CountryCodeFragment
import io.zonarosa.messenger.registration.ui.toE164
import io.zonarosa.messenger.registration.util.CountryPrefix
import io.zonarosa.messenger.util.CommunicationActions
import io.zonarosa.messenger.util.Dialogs
import io.zonarosa.messenger.util.PlayServicesUtil
...
import io.zonarosa.messenger.util.livedata.LiveDataObserverCallback
...
class EnterPhoneNumberFragment : LoggingFragment(R.layout.fragment_registration_enter_phone_number) {
  ...
  private fun handleNonNormalizedNumberError(originalNumber: String, normalizedNumber: String, mode: RegistrationRepository.E164VerificationMode) {
    try {
      ...
      MaterialAlertDialogBuilder(requireContext()).apply {
        ...
        setNeutralButton(R.string.RegistrationActivity_contact_zonarosa_support) { dialogInterface, _ ->
          val subject = getString(R.string.RegistrationActivity_zonarosa_android_phone_number_format)
          val body = SupportEmailUtil.generateSupportEmailBody(requireContext(), R.string.RegistrationActivity_zonarosa_android_phone_number_format, null, null)
  ...
  private fun onRegistrationButtonClicked() {
    when (enterPhoneNumberMode) {
      ...
      EnterPhoneNumberMode.COLLECT_FOR_MANUAL_ZONAROSA_BACKUPS_RESTORE -> findNavController().safeNavigate(EnterPhoneNumberFragmentDirections.goToEnterBackupKey())
      EnterPhoneNumberMode.COLLECT_FOR_LOCAL_V2_ZONAROSA_BACKUPS_RESTORE -> findNavController().safeNavigate(EnterPhoneNumberFragmentDirections.goToRestoreLocalBackupFragment())
    }
  }
  ...
  private fun presentConfirmNumberDialog(phoneNumber: PhoneNumber, isReRegister: Boolean, canSkipSms: Boolean, missingFcmConsentRequired: Boolean) {
    ...
    val message: CharSequence = SpannableStringBuilder().apply {
      append(SpanUtil.bold(ZonaRosaE164Util.prettyPrint(phoneNumber.toE164())))
      if (!canSkipSms) {
        append("\n\n")
