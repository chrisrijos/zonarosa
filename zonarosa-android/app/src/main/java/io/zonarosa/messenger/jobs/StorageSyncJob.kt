...
import io.zonarosa.messenger.jobs.protos.StorageSyncJobData
import io.zonarosa.messenger.keyvalue.ZonaRosaStore
import io.zonarosa.messenger.net.ZonaRosaNetwork
import io.zonarosa.messenger.recipients.Recipient
import io.zonarosa.messenger.storage.AccountRecordProcessor
import io.zonarosa.messenger.storage.CallLinkRecordProcessor
import io.zonarosa.messenger.storage.ChatFolderRecordProcessor
import io.zonarosa.messenger.storage.ContactRecordProcessor
import io.zonarosa.messenger.storage.GroupV1RecordProcessor
import io.zonarosa.messenger.storage.GroupV2RecordProcessor
import io.zonarosa.messenger.storage.NotificationProfileRecordProcessor
import io.zonarosa.messenger.storage.StorageSyncHelper
import io.zonarosa.messenger.storage.StorageSyncHelper.WriteOperationResult
import io.zonarosa.messenger.storage.StorageSyncModels
import io.zonarosa.messenger.storage.StorageSyncValidations
import io.zonarosa.messenger.storage.StoryDistributionListRecordProcessor
...
import io.zonarosa.service.api.storage.StorageServiceRepository.ManifestIfDifferentVersionResult
...
import io.zonarosa.service.api.storage.toZonaRosaChatFolderRecord
...
import io.zonarosa.service.api.storage.toZonaRosaNotificationProfileRecord
...
class StorageSyncJob private constructor(parameters: Parameters, private var localManifestOutOfDate: Boolean) : BaseJob(parameters) {
  ...
  override fun onRun() {
    if (!(ZonaRosaStore.svr.hasPin() || ZonaRosaStore.account.restoredAccountEntropyPool || ZonaRosaStore.account.restoredAccountEntropyPoolFromPrimary) && !ZonaRosaStore.svr.hasOptedOut()) {
    ...
    if (ZonaRosaStore.internal.storageServiceDisabled) {
      Log.w(TAG, "Storage service has been manually disabled. Skipping.")
      return
    }

    if (ZonaRosaStore.account.isLinkedDevice && !ZonaRosaStore.account.restoredAccountEntropyPoolFromPrimary) {
      Log.w(TAG, "Have not restored AEP from primary, skipping.")
      return
    }

    val (storageServiceKey, usingTempKey) = ZonaRosaStore.storageService.storageKeyForInitialDataRestore?.let {
      Log.i(TAG, "Using temporary storage key.")
      it to true
    } ?: run {
      ZonaRosaStore.storageService.storageKey to false
    }
    ...
    try {
      ...
      if (ZonaRosaStore.account.isMultiDevice && needsMultiDeviceSync) {
        AppDependencies.jobManager.add(MultiDeviceStorageSyncRequestJob())
      }

      ZonaRosaStore.storageService.lastSyncTime = System.currentTimeMillis()
      ...
      if (ZonaRosaStore.account.isPrimaryDevice) {
        ...
        AppDependencies.zonarosaServiceMessageSender.sendSyncMessage(ZonaRosaServiceSyncMessage.forRequest(RequestMessage.forType(SyncMessage.Request.Type.KEYS)))
      }
    }
  }
  ...
  private fun performSync(storageServiceKey: StorageKey): Boolean {
    ...
    val db = ZonaRosaDatabase.rawDatabase
    val repository = StorageServiceRepository(ZonaRosaNetwork.storageService)

    val localManifest = ZonaRosaStore.storageService.manifest
    ...
    if (self.storageId == null) {
      ...
      ZonaRosaDatabase.recipients.updateStorageId(self.id, StorageSyncHelper.generateKey())
      self = freshSelf()
    }
    ...
    if (remoteManifest.version > localManifest.version) {
      ...
      if (idDifference.hasTypeMismatches && ZonaRosaStore.account.isPrimaryDevice) {
      ...
      if (idDifference.localOnlyIds.isNotEmpty()) {
        val updatedRecipients = ZonaRosaDatabase.recipients.removeStorageIdsFromLocalOnlyUnregisteredRecipients(idDifference.localOnlyIds)
        val updatedFolders = ZonaRosaDatabase.chatFolders.removeStorageIdsFromLocalOnlyDeletedFolders(idDifference.localOnlyIds)
        val updatedProfiles = ZonaRosaDatabase.notificationProfiles.removeStorageIdsFromLocalOnlyDeletedProfiles(idDifference.localOnlyIds)
      ...
      if (!idDifference.isEmpty) {
        ...
        try {
          ...
          val unknownInserts: List<ZonaRosaStorageRecord> = remoteOnly.unknown
          ...
          ZonaRosaDatabase.unknownStorageIds.insert(unknownInserts)
          ZonaRosaDatabase.unknownStorageIds.delete(unknownDeletes)
    ...
    if (remoteManifest !== localManifest) {
      ...
      ZonaRosaStore.storageService.manifest = remoteManifest
    }
    ...
    val remoteWriteOperation: WriteOperationResult = db.withinTransaction {
      ...
      val removedUnregistered = ZonaRosaDatabase.recipients.removeStorageIdsFromOldUnregisteredRecipients(System.currentTimeMillis())
      val removedDeletedFolders = ZonaRosaDatabase.chatFolders.removeStorageIdsFromOldDeletedFolders(System.currentTimeMillis())
      val removedDeletedProfiles = ZonaRosaDatabase.notificationProfiles.removeStorageIdsFromOldDeletedProfiles(System.currentTimeMillis())
      ...
      WriteOperationResult(
        manifest = ZonaRosaStorageManifest(
          ...
          sourceDeviceId = ZonaRosaStore.account.deviceId,
    ...
    if (!remoteWriteOperation.isEmpty) {
      ...
      ZonaRosaStore.storageService.manifest = remoteWriteOperation.manifest
    ...
    val knownUnknownIds = ZonaRosaDatabase.unknownStorageIds.getAllWithTypes(knownTypes)
    ...
    if (knownUnknownIds.isNotEmpty()) {
      ...
      db.withinTransaction {
        ...
        ZonaRosaDatabase.unknownStorageIds.deleteAllWithTypes(knownTypes)
      }
    ...
    if (needsForcePush && ZonaRosaStore.account.isPrimaryDevice) {
  ...
  private fun getAllLocalStorageIds(self: Recipient): List<StorageId> {
    return ZonaRosaDatabase.recipients.getContactStorageSyncIds() +
      listOf(StorageId.forAccount(self.storageId)) +
      ZonaRosaDatabase.chatFolders.getStorageSyncIds() +
      ZonaRosaDatabase.notificationProfiles.getStorageSyncIds() +
      ZonaRosaDatabase.unknownStorageIds.allUnknownIds
  }

  private fun buildLocalStorageRecords(context: Context, self: Recipient, ids: Collection<StorageId>): List<ZonaRosaStorageRecord> {
    if (ids.isEmpty()) {
      return emptyList()
    }

    val records: MutableList<ZonaRosaStorageRecord> = ArrayList(ids.size)
    ...
    for (id in ids) {
      ...
      when (type) {
        ManifestRecord.Identifier.Type.CONTACT, ManifestRecord.Identifier.Type.GROUPV1, ManifestRecord.Identifier.Type.GROUPV2 -> {
          val settings = ZonaRosaDatabase.recipients.getByStorageId(id.raw)
          if (settings != null) {
        ...
        ManifestRecord.Identifier.Type.STORY_DISTRIBUTION_LIST -> {
          val record = ZonaRosaDatabase.recipients.getByStorageId(id.raw)
          if (record != null) {
        ...
        ManifestRecord.Identifier.Type.CALL_LINK -> {
          val callLinkRecord = ZonaRosaDatabase.recipients.getByStorageId(id.raw)
        ...
        ManifestRecord.Identifier.Type.CHAT_FOLDER -> {
          ...
          val chatFolderRecord = ZonaRosaDatabase.chatFolders.getChatFolder(query)
        ...
        ManifestRecord.Identifier.Type.NOTIFICATION_PROFILE -> {
          ...
          val unknown = ZonaRosaDatabase.unknownStorageIds.getById(id.raw)
          if (unknown != null) {
            records.add(unknown)
          } else {
  ...
  private class StorageRecordCollection(records: Collection<ZonaRosaStorageRecord>) {
    val contacts: MutableList<ZonaRosaContactRecord> = mutableListOf()
    val gv1: MutableList<ZonaRosaGroupV1Record> = mutableListOf()
    val gv2: MutableList<ZonaRosaGroupV2Record> = mutableListOf()
    val account: MutableList<ZonaRosaAccountRecord> = mutableListOf()
    val unknown: MutableList<ZonaRosaStorageRecord> = mutableListOf()
    val storyDistributionLists: MutableList<ZonaRosaStoryDistributionListRecord> = mutableListOf()
    val callLinkRecords: MutableList<ZonaRosaCallLinkRecord> = mutableListOf()
    val chatFolderRecords: MutableList<ZonaRosaChatFolderRecord> = mutableListOf()
    val notificationProfileRecords: MutableList<ZonaRosaNotificationProfileRecord> = mutableListOf()

    init {
      for (record in records) {
        if (record.proto.contact != null) {
          contacts += record.proto.contact!!.toZonaRosaContactRecord(record.id)
        } else if (record.proto.groupV1 != null) {
          gv1 += record.proto.groupV1!!.toZonaRosaGroupV1Record(record.id)
        } else if (record.proto.groupV2 != null) {
          gv2 += record.proto.groupV2!!.toZonaRosaGroupV2Record(record.id)
        } else if (record.proto.account != null) {
          account += record.proto.account!!.toZonaRosaAccountRecord(record.id)
        } else if (record.proto.storyDistributionList != null) {
          storyDistributionLists += record.proto.storyDistributionList!!.toZonaRosaStoryDistributionListRecord(record.id)
        } else if (record.proto.callLink != null) {
          callLinkRecords += record.proto.callLink!!.toZonaRosaCallLinkRecord(record.id)
        } else if (record.proto.chatFolder != null) {
          chatFolderRecords += record.proto.chatFolder!!.toZonaRosaChatFolderRecord(record.id)
        } else if (record.proto.notificationProfile != null) {
          notificationProfileRecords += record.proto.notificationProfile!!.toZonaRosaNotificationProfileRecord(record.id)
