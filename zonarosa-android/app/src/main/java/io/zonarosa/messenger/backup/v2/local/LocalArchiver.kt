...
import io.zonarosa.core.models.backup.BackupId
...
import io.zonarosa.messenger.backup.v2.local.proto.Metadata
...
typealias ArchiveResult = io.zonarosa.core.util.Result<LocalArchiver.ArchiveSuccess, LocalArchiver.ArchiveFailure>
typealias RestoreResult = io.zonarosa.core.util.Result<LocalArchiver.RestoreSuccess, LocalArchiver.RestoreFailure>
...
object LocalArchiver {
   ...
   */
  fun export(snapshotFileSystem: SnapshotFileSystem, filesFileSystem: FilesFileSystem, stopwatch: Stopwatch, cancellationZonaRosa: () -> Boolean = { false }): ArchiveResult {
    ...
    try {
      ...
      BackupRepository.exportForLocalBackup(mainStream, LocalExportProgressListener(), cancellationZonaRosa) { attachment, source ->
        if (cancellationZonaRosa() || createFailures.size > MAX_CREATE_FAILURES) {
  ...
  private fun getEncryptedBackupId(): Metadata.EncryptedBackupId {
    val metadataKey = ZonaRosaStore.backup.messageBackupKey.deriveLocalBackupMetadataKey()
    val iv = Util.getSecretBytes(12)
    val backupId = ZonaRosaStore.backup.messageBackupKey.deriveBackupId(ZonaRosaStore.account.requireAci())
  ...
  fun import(snapshotFileSystem: SnapshotFileSystem, selfData: BackupRepository.SelfData): RestoreResult {
    ...
    try {
      ...
      if (!backupId.value.contentEquals(ZonaRosaStore.backup.messageBackupKey.deriveBackupId(ZonaRosaStore.account.requireAci()).value)) {
