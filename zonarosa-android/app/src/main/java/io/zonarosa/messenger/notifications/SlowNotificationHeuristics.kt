...
object SlowNotificationHeuristics {
  ...
  fun shouldPromptUserForDelayedNotificationLogs(): Boolean {
    if (!LocaleRemoteConfig.isDelayedNotificationPromptEnabled() || ZonaRosaStore.uiHints.hasDeclinedToShareNotificationLogs()) {
      return false
    }
    if (System.currentTimeMillis() - ZonaRosaStore.uiHints.lastNotificationLogsPrompt < TimeUnit.DAYS.toMillis(7)) {
      return false
    }
  ...
  fun shouldPromptBatterySaver(): Boolean {
    ...
    if (!remoteEnabled || ZonaRosaStore.uiHints.hasDismissedBatterySaverPrompt()) {
      return false
    }

    if (System.currentTimeMillis() - ZonaRosaStore.uiHints.lastBatterySaverPrompt < TimeUnit.DAYS.toMillis(7)) {
      return false
    }
  ...
  fun isHavingDelayedNotifications(): Boolean {
    if (!ZonaRosaStore.settings.isMessageNotificationsEnabled ||
  ...
  fun shouldShowDeviceSpecificDialog(): Boolean {
    return LocaleRemoteConfig.isDeviceSpecificNotificationEnabled() && ZonaRosaStore.uiHints.lastSupportVersionSeen < DeviceSpecificNotificationConfig.currentConfig.version
  }
  ...
  private fun hasRepeatedFailedServiceStarts(metrics: List<LocalMetricsDatabase.EventMetrics>, minimumEventAgeMs: Long, minimumEventCount: Int, failurePercentage: Float): Boolean {
    if (!haveEnoughData(ZonaRosaLocalMetrics.FcmServiceStartSuccess.NAME, minimumEventAgeMs) && !haveEnoughData(ZonaRosaLocalMetrics.FcmServiceStartFailure.NAME, minimumEventAgeMs)) {
      Log.d(TAG, "insufficient data for service starts")
      return false
    }

    val successes = metrics.filter { it.name == ZonaRosaLocalMetrics.FcmServiceStartSuccess.NAME }
    val failures = metrics.filter { it.name == ZonaRosaLocalMetrics.FcmServiceStartFailure.NAME }
  ...
  private fun isFailingToDrainQueue(metrics: List<LocalMetricsDatabase.EventMetrics>, minimumEventAgeMs: Long, failureThreshold: Int): Boolean {
    if (!haveEnoughData(ZonaRosaLocalMetrics.PushWebsocketFetch.SUCCESS_EVENT, minimumEventAgeMs) && !haveEnoughData(ZonaRosaLocalMetrics.PushWebsocketFetch.TIMEOUT_EVENT, minimumEventAgeMs)) {
      Log.d(TAG, "insufficient data for failed queue drains")
      return false
    }
    val failures = metrics.filter { it.name == ZonaRosaLocalMetrics.PushWebsocketFetch.TIMEOUT_EVENT }
  ...
  private fun hasLongMessageLatency(metrics: List<LocalMetricsDatabase.EventMetrics>, minimumEventAgeMs: Long, messageThreshold: Int, percentiles: Map<Int, Long>): Boolean {
    if (!haveEnoughData(ZonaRosaLocalMetrics.MessageLatency.NAME_HIGH, minimumEventAgeMs)) {
      Log.d(TAG, "insufficient data for message latency")
      return false
    }
    val eventCount = metrics.find { it.name == ZonaRosaLocalMetrics.MessageLatency.NAME_HIGH }?.count ?: 0
    ...
    for ((percentage, threshold) in percentiles.entries) {
      val averageLatency = db.eventPercent(ZonaRosaLocalMetrics.MessageLatency.NAME_HIGH, percentage.coerceAtMost(100).coerceAtLeast(0))
