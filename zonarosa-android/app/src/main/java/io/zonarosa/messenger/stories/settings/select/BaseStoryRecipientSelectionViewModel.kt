...
class BaseStoryRecipientSelectionViewModel(
  ...
  fun toggleSelectAll(): Single<Set<RecipientId>> {
    return Single.create { emitter ->
      disposable += repository.getAllZonaRosaContacts().subscribeBy { allZonaRosaRecipients ->
        store.update { it.copy(selection = allZonaRosaRecipients) }
        emitter.onSuccess(allZonaRosaRecipients)
      }
    }
  }
