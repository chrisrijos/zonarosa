...
import io.zonarosa.messenger.restore.restorelocalbackup.PassphraseAsYouTypeFormatter;
      ...
      button.setOnClickListener(v -> {
        ...
        if (confirmationCheckBox.isChecked()) {
          if (backupDirectorySelectionIntent != null && backupDirectorySelectionIntent.getData() != null) {
            ...
            ZonaRosaStore.settings().setZonaRosaBackupDirectory(backupDirectoryUri);
            context.getContentResolver()
                   .takePersistableUriPermission(backupDirectoryUri, takeFlags);
          }

          BackupPassphrase.set(context, Util.join(password, " "));
          ZonaRosaPreferences.setNextBackupTime(context, 0);
          ZonaRosaStore.settings().setBackupEnabled(true);
    ...
    new MaterialAlertDialogBuilder(fragment.requireContext())
                   ...
                   .setPositiveButton(R.string.BackupDialog_choose_folder, ((dialog, which) -> {
                     ...
                     if (Build.VERSION.SDK_INT >= 26) {
                       intent.putExtra(DocumentsContract.EXTRA_INITIAL_URI, ZonaRosaStore.settings().getLatestZonaRosaBackupDirectory());
                     }
