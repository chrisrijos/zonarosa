...
import io.zonarosa.messenger.backup.v2.database.getAllForBackup
import io.zonarosa.messenger.backup.v2.database.getCallLinksForBackup
import io.zonarosa.messenger.backup.v2.database.getContactsForBackup
import io.zonarosa.messenger.backup.v2.database.getGroupsForBackup
import io.zonarosa.messenger.backup.v2.database.restoreReleaseNotes
import io.zonarosa.messenger.backup.v2.importer.CallLinkArchiveImporter
import io.zonarosa.messenger.backup.v2.importer.ContactArchiveImporter
import io.zonarosa.messenger.backup.v2.importer.DistributionListArchiveImporter
import io.zonarosa.messenger.backup.v2.importer.GroupArchiveImporter
import io.zonarosa.messenger.backup.v2.proto.Frame
import io.zonarosa.messenger.backup.v2.proto.ReleaseNotes
...
object RecipientArchiveProcessor {
  ...
  fun export(db: ZonaRosaDatabase, zonarosaStore: ZonaRosaStore, exportState: ExportState, selfAci: ServiceId.ACI, emitter: BackupFrameEmitter) {
    val releaseChannelId = zonarosaStore.releaseChannelValues.releaseChannelRecipientId
  ...
  fun import(recipient: ArchiveRecipient, importState: ImportState) {
    val newId: RecipientId? = when {
      ...
      recipient.releaseNotes != null -> ZonaRosaDatabase.recipients.restoreReleaseNotes()
