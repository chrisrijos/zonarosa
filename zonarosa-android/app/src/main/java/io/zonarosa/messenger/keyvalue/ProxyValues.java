...
public final class ProxyValues extends ZonaRosaStoreValues {
  ...
  public void enableProxy(@NonNull ZonaRosaProxy proxy) {
   ...
   */
  public void setProxy(@Nullable ZonaRosaProxy proxy) {
    if (proxy != null) {
      getStore().beginWrite()
                .putString(KEY_HOST, proxy.getHost())
                .putInteger(KEY_PORT, proxy.getPort())
  ...
  public @Nullable ZonaRosaProxy getProxy() {
    String host = getString(KEY_HOST, null);
    int    port = getInteger(KEY_PORT, 0);

    if (host != null) {
      return new ZonaRosaProxy(host, port);
    } else {
      return null;
    }
  }
