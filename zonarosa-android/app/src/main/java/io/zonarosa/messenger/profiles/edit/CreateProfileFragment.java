...
import io.zonarosa.core.util.EditTextUtil;
...
import io.zonarosa.messenger.avatar.picker.AvatarPickerFragment;
import io.zonarosa.messenger.databinding.CreateProfileFragmentBinding;
...
import io.zonarosa.messenger.profiles.edit.pnp.WhoCanFindMeByPhoneNumberFragment;
import io.zonarosa.messenger.profiles.manage.EditProfileNameFragment;
...
import io.zonarosa.messenger.util.text.AfterTextChanged;

import java.io.IOException;
import java.io.InputStream;
import java.util.Objects;

import static io.zonarosa.messenger.profiles.edit.CreateProfileActivity.EXCLUDE_SYSTEM;
import static io.zonarosa.messenger.profiles.edit.CreateProfileActivity.GROUP_ID;
import static io.zonarosa.messenger.profiles.edit.CreateProfileActivity.IS_DESCRIPTION_FOCUSED;
import static io.zonarosa.messenger.profiles.edit.CreateProfileActivity.NEXT_BUTTON_TEXT;
import static io.zonarosa.messenger.profiles.edit.CreateProfileActivity.NEXT_INTENT;
import static io.zonarosa.messenger.profiles.edit.CreateProfileActivity.SHOW_TOOLBAR;
...
public class CreateProfileFragment extends LoggingFragment {
  ...
  private void initializeResources(@Nullable GroupId groupId) {
    ...
    if (isEditingGroup) {
      ...
      binding.profileDescriptionText.setLinkColor(ContextCompat.getColor(requireContext(), io.zonarosa.core.ui.R.color.zonarosa_colorPrimary));
      ...
      getParentFragmentManager().setFragmentResultListener(WhoCanFindMeByPhoneNumberFragment.REQUEST_KEY, getViewLifecycleOwner(), (requestKey, result) -> {
        if (WhoCanFindMeByPhoneNumberFragment.REQUEST_KEY.equals(requestKey)) {
          presentWhoCanFindMeDescription(ZonaRosaStore.phoneNumberPrivacy().getPhoneNumberDiscoverabilityMode());
        }
      });
  ...
  private void presentWhoCanFindMeDescription(PhoneNumberPrivacyValues.PhoneNumberDiscoverabilityMode phoneNumberListingMode) {
    switch (phoneNumberListingMode) {
      ...
      case NOT_DISCOVERABLE:
        binding.whoCanFindMeIcon.setImageResource(io.zonarosa.core.ui.R.drawable.symbol_lock_24);
