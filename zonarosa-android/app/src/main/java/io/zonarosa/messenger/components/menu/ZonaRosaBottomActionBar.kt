 ...
 *
 * Overflow items are rendered in a [ZonaRosaContextMenu].
 */
class ZonaRosaBottomActionBar(context: Context, attributeSet: AttributeSet?) : LinearLayout(context, attributeSet) {
  ...
  init {
    ...
    setBackgroundResource(R.drawable.zonarosa_bottom_action_bar_background)
    elevation = 20f
  }
  ...
  private fun present(items: List<ActionItem>) {
    ...
    renderableItems.forEach { item ->
      val view: View = LayoutInflater.from(context).inflate(R.layout.zonarosa_bottom_action_bar_item, this, false)
      addView(view)
      bindItem(view, item)
    }
    ...
    if (overflowItems.isNotEmpty()) {
      ...
      bindItem(
        ...
        ActionItem(
          ...
          title = context.getString(R.string.ZonaRosaBottomActionBar_more),
          action = {
            ZonaRosaContextMenu.Builder(view, parent as ViewGroup)
  ...
  private fun bindItem(view: View, item: ActionItem) {
    val icon: ImageView = view.findViewById(R.id.zonarosa_bottom_action_bar_item_icon)
    val title: TextView = view.findViewById(R.id.zonarosa_bottom_action_bar_item_title)
...
fun ZonaRosaBottomActionBar(
  ...
  AnimatedVisibility(
    ...
    AndroidView(
      factory = { context ->
        ZonaRosaBottomActionBar(context, null)
          .apply {
            elevation = 0f
            setItems(items)
          }
