...
import io.zonarosa.messenger.components.webrtc.WebRtcControls.FoldableState
import io.zonarosa.messenger.events.CallParticipant
import io.zonarosa.messenger.events.CallParticipant.Companion.HAND_LOWERED
import io.zonarosa.messenger.events.CallParticipant.Companion.createLocal
import io.zonarosa.messenger.events.GroupCallRaiseHandEvent
...
import io.zonarosa.messenger.service.webrtc.collections.ParticipantCollection
  ...
  fun getPreJoinGroupDescription(context: Context): String? {
    ...
    return if (remoteParticipants.isEmpty) {
      describeGroupMembers(
        context = context,
        noParticipants = if (isCallLink) R.string.WebRtcCallView__zonarosa_call_link else null,
        oneParticipant = if (ringGroup) R.string.WebRtcCallView__zonarosa_will_ring_s else R.string.WebRtcCallView__s_will_be_notified,
        twoParticipants = if (ringGroup) R.string.WebRtcCallView__zonarosa_will_ring_s_and_s else R.string.WebRtcCallView__s_and_s_will_be_notified,
        multipleParticipants = if (ringGroup) R.plurals.WebRtcCallView__zonarosa_will_ring_s_s_and_d_others else R.plurals.WebRtcCallView__s_s_and_d_others_will_be_notified,
