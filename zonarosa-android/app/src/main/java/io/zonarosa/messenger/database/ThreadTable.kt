...
import io.zonarosa.core.util.readToSingleIntOrNull
...
import io.zonarosa.messenger.database.ZonaRosaDatabase.Companion.drafts
...
import io.zonarosa.messenger.database.ZonaRosaDatabase.Companion.messageLog
...
import io.zonarosa.messenger.database.ThreadBodyUtil.ThreadBody
...
import io.zonarosa.messenger.database.model.serialize
...
import io.zonarosa.messenger.util.isScheduled
...
import io.zonarosa.service.api.storage.toZonaRosaServiceAddress
...
class ThreadTable(context: Context, databaseHelper: ZonaRosaDatabase) : DatabaseTable(context, databaseHelper) {
  ...
  private fun allowedToUnarchive(threadId: Long): Boolean {
    if (!ZonaRosaStore.settings.shouldKeepMutedChatsArchived()) {
      return true
    }
  ...
  fun trimAllThreads(length: Int, trimBeforeDate: Long) {
    ...
    val syncThreadTrimDeletes = ZonaRosaStore.settings.shouldSyncThreadTrimDeletes()
  ...
  private fun trimThreadInternal(
    ...
    if (finalTrimBeforeDate != NO_TRIM_BEFORE_DATE_SET) {
      ...
      if (deletes > 0) {
        ...
        ZonaRosaDatabase.calls.updateCallEventDeletionTimestamps()
  ...
  fun getRecentConversationList(limit: Int, includeInactiveGroups: Boolean, individualsOnly: Boolean, groupsOnly: Boolean, hideV1Groups: Boolean, hideSms: Boolean, hideSelf: Boolean): Cursor {
    ...
    if (ZonaRosaStore.releaseChannel.releaseChannelRecipientId != null) {
      where += " AND $TABLE_NAME.$RECIPIENT_ID != ${ZonaRosaStore.releaseChannel.releaseChannelRecipientId!!.toLong()}"
    }
  ...
  fun deleteAllConversations() {
    writableDatabase.withinTransaction { db ->
      ...
      ZonaRosaDatabase.calls.deleteAllCalls()
      synchronized(threadIdCache) {
        threadIdCache.clear()
      }
    }
  ...
  fun applyStorageSyncUpdate(recipientId: RecipientId, record: ZonaRosaContactRecord) {
    applyStorageSyncUpdate(recipientId, record.proto.archived, record.proto.markedUnread, isGroup = false)
  }

  fun applyStorageSyncUpdate(recipientId: RecipientId, record: ZonaRosaGroupV1Record) {
    applyStorageSyncUpdate(recipientId, record.proto.archived, record.proto.markedUnread, isGroup = true)
  }

  fun applyStorageSyncUpdate(recipientId: RecipientId, record: ZonaRosaGroupV2Record) {
    applyStorageSyncUpdate(recipientId, record.proto.archived, record.proto.markedUnread, isGroup = true)
  }

  fun applyStorageSyncUpdate(recipientId: RecipientId, record: ZonaRosaAccountRecord) {
    writableDatabase.withinTransaction { db ->
      ...
      for (pinned: AccountRecord.PinnedConversation in record.proto.pinnedConversations) {
        val pinnedRecipient: Recipient? = if (pinned.contact != null) {
          if (ServiceId.parseOrNull(pinned.contact!!.serviceId, pinned.contact!!.serviceIdBinary) != null) {
            Recipient.externalPush(pinned.contact!!.toZonaRosaServiceAddress())
          } else {
  ...
  private fun hasMoreRecentDraft(threadId: Long, timestamp: Long): Boolean {
    val drafts: DraftTable.Drafts = ZonaRosaDatabase.drafts.getDrafts(threadId)
  ...
  fun merge(primaryRecipientId: RecipientId, secondaryRecipientId: RecipientId): MergeResult {
    check(databaseHelper.zonarosaWritableDatabase.inTransaction()) { "Must be in a transaction!" }
  ...
  open class StaticReader(private val cursor: Cursor, private val context: Context) : Closeable {
    ...
    open fun getCurrent(): ThreadRecord? {
      ...
      val hasReadReceipt = ZonaRosaPreferences.isReadReceiptsEnabled(context) && cursor.requireBoolean(HAS_READ_RECEIPT)
