...
import io.zonarosa.messenger.stories.settings.custom.PrivateStorySettingsFragment
import io.zonarosa.messenger.stories.settings.my.MyStorySettingsFragment
import io.zonarosa.messenger.stories.settings.privacy.ChooseInitialMyStoryMembershipBottomSheetDialogFragment
...
class ContactSearchMediator(
  ...
  private fun toggleStorySelection(view: View, contactSearchData: ContactSearchData.Story, isSelected: Boolean) {
    if (contactSearchData.recipient.isMyStory && !ZonaRosaStore.story.userHasBeenNotifiedAboutStories) {
  ...
  private inner class StoryContextMenuCallbacks : ContactSearchAdapter.StoryContextMenuCallbacks {
    ...
    override fun onDeletePrivateStory(story: ContactSearchData.Story, isSelected: Boolean) {
      MaterialAlertDialogBuilder(fragment.requireContext())
        ...
        .setPositiveButton(SpanUtil.color(ContextCompat.getColor(fragment.requireContext(), CoreUiR.color.zonarosa_colorError), fragment.getString(R.string.ContactSearchMediator__delete))) { _, _ -> viewModel.deletePrivateStory(story) }
        .setNegativeButton(android.R.string.cancel) { _, _ -> }
        .show()
    }
  }
