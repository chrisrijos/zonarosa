...
import io.zonarosa.messenger.contacts.avatars.ContactPhoto
import io.zonarosa.messenger.contacts.avatars.GroupRecordContactPhoto
import io.zonarosa.messenger.contacts.avatars.ProfileContactPhoto
import io.zonarosa.messenger.contacts.avatars.SystemContactPhoto
import io.zonarosa.messenger.conversation.colors.AvatarColor
import io.zonarosa.messenger.conversation.colors.ChatColors
import io.zonarosa.messenger.conversation.colors.ChatColors.Id.Auto
...
import io.zonarosa.messenger.phonenumbers.NumberUtil
import io.zonarosa.messenger.profiles.ProfileName
import io.zonarosa.messenger.recipients.Recipient.Companion.external
import io.zonarosa.messenger.service.webrtc.links.CallLinkRoomId
import io.zonarosa.messenger.util.ZonaRosaE164Util
import io.zonarosa.messenger.util.UsernameUtil.isValidUsernameForSearch
...
class Recipient(
  ...
  /** Whether the recipient represents a group. It could be a ZonaRosa group or MMS group. */
  val isGroup: Boolean
  ...
  /** Whether the recipient represents a ZonaRosa group. */
  ...
  /** Whether the recipient represents a V1 ZonaRosa group. These types of groups were deprecated in 2020. */
  ...
  /** Whether the recipient represents a V2 ZonaRosa group. */
  ...
  val contactPhoto: ContactPhoto?
    ...
    } else if (systemContactPhoto != null && ZonaRosaStore.settings.isPreferSystemContactPhotos) {
  ...
  /** The [RegisteredState] of this recipient. ZonaRosa groups/lists are always registered. */
  ...
  val wallpaper: ChatWallpaper?
    get() {
      return if (wallpaperValue != null) {
        ...
        ZonaRosaStore.wallpaper.getWallpaper()
      }
    }
  ...
  val hasWallpaper: Boolean
    get() = wallpaperValue != null || ZonaRosaStore.wallpaper.hasWallpaperSet()
  ...
  val chatColors: ChatColors
    get() {
      return if (chatColorsValue != null && chatColorsValue.id !is Auto) {
        ...
        val global = ZonaRosaStore.chatColors.chatColors
   ...
   * This value respects the local user's [ZonaRosaStore.inAppPayments.getDisplayBadgesOnProfile()] preference.
   */
  val featuredBadge: Badge? get() {
    return if (isSelf && !ZonaRosaStore.inAppPayments.getDisplayBadgesOnProfile()) {
      null
    } else {
      badges.firstOrNull()
    }
  ...
  fun getGroupName(context: Context): String? {
    return if (groupIdValue != null && Util.isEmpty(groupName)) {
      ...
      context.getString(R.string.Recipient_zonarosa_call)
    } else {
      groupName
    }
  }
  ...
  private fun getNameFromLocalData(context: Context): String? {
    ...
    if (name.isBlank() && e164Value.isNotNullOrBlank()) {
      name = ZonaRosaE164Util.prettyPrint(e164Value)
    }
  ...
  companion object {
    ...
    fun distributionList(distributionListId: DistributionListId): Recipient {
      val id = ZonaRosaDatabase.recipients.getOrInsertFromDistributionListId(distributionListId)
      return resolved(id)
    }
    ...
    fun externalUsername(serviceId: ServiceId, username: String): Recipient {
      ...
      ZonaRosaDatabase.recipients.setUsername(recipient.id, username)
      return recipient
    }

    /**
     * Returns a fully-populated [Recipient] based off of a [ZonaRosaServiceAddress], creating one in the database if necessary.
     */
    @JvmStatic
    @WorkerThread
    fun externalPush(zonarosaServiceAddress: ZonaRosaServiceAddress): Recipient {
      return externalPush(zonarosaServiceAddress.serviceId, zonarosaServiceAddress.number.orElse(null))
    }
    ...
    private fun externalPush(serviceId: ServiceId, e164: String?): Recipient {
      ...
      val recipientId = RecipientId.from(ZonaRosaServiceAddress(serviceId, e164))
      ...
      if (!resolved.isRegistered) {
        ...
        ZonaRosaDatabase.recipients.markRegistered(recipientId, serviceId)
      }
    ...
    fun trustedPush(aci: ACI, pni: PNI?, e164: String?): Recipient {
      ...
      val recipientId = ZonaRosaDatabase.recipients.getAndPossiblyMergePnpVerified(aci, pni, e164)
      val resolved = resolved(recipientId)
    ...
    fun externalContact(identifier: String): Recipient? {
      val id: RecipientId = if (UuidUtil.isUuid(identifier)) {
        ...
        ZonaRosaDatabase.recipients.getOrInsertFromEmail(identifier)
      } else {
        val e164 = ZonaRosaE164Util.formatAsE164(identifier) ?: return null
        ZonaRosaDatabase.recipients.getOrInsertFromE164(e164)
      }
    ...
    fun externalGroupExact(groupId: GroupId): Recipient {
      return resolved(ZonaRosaDatabase.recipients.getOrInsertFromGroupId(groupId))
    }
    ...
    fun externalPossiblyMigratedGroup(groupId: GroupId): Recipient {
      ...
      return try {
        ...
        resolved(ZonaRosaDatabase.recipients.getOrInsertFromPossiblyMigratedGroupId(groupId))
      }
    }
     ...
     *
     * If the identifier is a UUID of a ZonaRosa user, prefer using
    ...
    fun external(identifier: String): Recipient? {
      ...
      val id: RecipientId = if (serviceId != null) {
        ...
        ZonaRosaDatabase.recipients.getOrInsertFromGroupId(GroupId.parseOrThrow(identifier))
      } else if (NumberUtil.isValidEmail(identifier)) {
        ZonaRosaDatabase.recipients.getOrInsertFromEmail(identifier)
      ...
      } else if (ZonaRosaE164Util.isPotentialE164(identifier)) {
        ZonaRosaDatabase.recipients.getOrInsertFromE164(identifier)
      } else {
        return null
      }
