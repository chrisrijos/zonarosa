...
import io.zonarosa.messenger.lock.v2.SvrConstants
import io.zonarosa.messenger.pin.RegistrationLockV2Dialog
  ...
  private fun setPinRemindersEnabled(enabled: Boolean) {
    if (!enabled) {
      ...
      turnOffButton.setOnClickListener {
        ...
        val correct = PinHashUtil.verifyLocalPinHash(ZonaRosaStore.svr.localPinHash!!, pin)
        if (correct) {
          ZonaRosaStore.pin.setPinRemindersEnabled(false)
          viewModel.refreshState()
          dialog.dismiss()
        } else {
      ...
      ZonaRosaStore.pin.setPinRemindersEnabled(true)
      viewModel.refreshState()
    }
  }
...
object AccountSettingsTestTags {
  ...
  const val ROW_UPDATE_ZONAROSA = "row-update-zonarosa"
...
fun AccountSettingsScreen(
  ...
  Scaffolds.Settings(
    ...
    LazyColumn(
      ...
      item {
        Texts.SectionHeader(
          text = stringResource(R.string.preferences_app_protection__zonarosa_pin)
        )
      }
      ...
      item {
        Rows.ToggleRow(
          ...
          label = stringResource(R.string.AccountSettingsFragment__require_your_zonarosa_pin),
      ...
      if (!state.isNotDeprecatedOrUnregistered()) {
        if (state.clientDeprecated) {
          item {
            Rows.TextRow(
              text = stringResource(R.string.preferences_account_update_zonarosa),
              onClick = callbacks::openUpdateAppFlow,
              modifier = Modifier.testTag(AccountSettingsTestTags.ROW_UPDATE_ZONAROSA)
            )
          }
          ...
          Rows.TextRow(
            text = {
              Text(
                ...
                color = colorResource(R.color.zonarosa_alert_primary)
              )
            },
            onClick = {
              displayDialog = true
      ...
      item {
        @ColorRes val textColor = if (state.isNotDeprecatedOrUnregistered()) {
          R.color.zonarosa_alert_primary
        } else {
          R.color.zonarosa_alert_primary_50
        }
