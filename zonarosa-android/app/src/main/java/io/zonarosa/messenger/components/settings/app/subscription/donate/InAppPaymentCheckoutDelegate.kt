...
import io.zonarosa.messenger.components.settings.app.subscription.donate.card.CreditCardFragment
import io.zonarosa.messenger.components.settings.app.subscription.donate.paypal.PayPalPaymentInProgressFragment
import io.zonarosa.messenger.components.settings.app.subscription.donate.stripe.StripePaymentInProgressFragment
import io.zonarosa.messenger.components.settings.app.subscription.donate.stripe.StripePaymentInProgressViewModel
import io.zonarosa.messenger.components.settings.app.subscription.donate.transfer.BankTransferRequestKeys
import io.zonarosa.messenger.components.settings.app.subscription.errors.DonationError
import io.zonarosa.messenger.components.settings.app.subscription.errors.DonationErrorDialogs
import io.zonarosa.messenger.components.settings.app.subscription.errors.DonationErrorParams
    ...
    if (result.action == InAppPaymentProcessorAction.CANCEL_SUBSCRIPTION) {
      ...
      fragment.lifecycleScope.launch {
        val inAppPayment = withContext(ZonaRosaDispatchers.IO) {
          ZonaRosaDatabase.inAppPayments.getById(result.inAppPaymentId!!)!!
        }
  ...
  class ErrorHandler : DefaultLifecycleObserver {
    ...
    fun attach(
      ...
      disposables += inAppPaymentIdSource
        ...
        .doOnNext {
          ZonaRosaDatabase.inAppPayments.update(it.copy(notified = true))
        }
      ...
      disposables += inAppPaymentIdSource
        ...
        .concatMapSingle { (id, err) -> Single.fromCallable { ZonaRosaDatabase.inAppPayments.getById(id)!! to err } }
        .subscribeOn(Schedulers.io())
        .observeOn(AndroidSchedulers.mainThread())
