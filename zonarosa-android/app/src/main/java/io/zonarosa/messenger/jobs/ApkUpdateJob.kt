...
import io.zonarosa.core.util.getDownloadManager
...
import io.zonarosa.messenger.apkupdate.ApkUpdateDownloadManagerReceiver
...
class ApkUpdateJob private constructor(parameters: Parameters) : BaseJob(parameters) {
  ...
  public override fun onRun() {
    ...
    if (shouldUpdate(getCurrentAppVersionCode(), updateDescriptor, ZonaRosaStore.apkUpdate.lastApkUploadTime, Environment.IS_WEBSITE)) {
      Log.i(TAG, "Newer version code available. Current: (versionCode: ${getCurrentAppVersionCode()}, uploadTime: ${ZonaRosaStore.apkUpdate.lastApkUploadTime}), Update: (versionCode: ${updateDescriptor.versionCode}, uploadTime: ${updateDescriptor.uploadTimestamp})")
      ...
      Log.d(TAG, "Version code is the same or older than our own. Current: (versionCode: ${getCurrentAppVersionCode()}, uploadTime: ${ZonaRosaStore.apkUpdate.lastApkUploadTime}), Update: (versionCode: ${updateDescriptor.versionCode}, uploadTime: ${updateDescriptor.uploadTimestamp})")
    }

    ZonaRosaStore.apkUpdate.lastSuccessfulCheck = System.currentTimeMillis()
  }
  ...
  private fun getDownloadStatus(uri: String, remoteDigest: ByteArray): DownloadStatus {
    val pendingDownloadId: Long = ZonaRosaStore.apkUpdate.downloadId
    val pendingDigest: ByteArray? = ZonaRosaStore.apkUpdate.digest

    if (pendingDownloadId == -1L || pendingDigest == null || !MessageDigest.isEqual(pendingDigest, remoteDigest)) {
      ZonaRosaStore.apkUpdate.clearDownloadAttributes()
  ...
  private fun handleDownloadStart(uri: String?, versionName: String?, digest: ByteArray, uploadTimestamp: Long) {
    ...
    val downloadRequest = DownloadManager.Request(Uri.parse(uri)).apply {
      ...
      setTitle("Downloading ZonaRosa update")
      setDescription("Downloading ZonaRosa $versionName")
      setDestinationInExternalFilesDir(context, null, "zonarosa-update.apk")
    ...
    ZonaRosaStore.apkUpdate.setDownloadAttributes(downloadId, digest, uploadTimestamp)
  }
  ...
  private fun deleteExistingDownloadedApks(context: Context) {
    ...
    for (file in directory.listFiles() ?: emptyArray()) {
      if (file.name.startsWith("zonarosa-update")) {
        if (file.delete()) {
