...
import io.zonarosa.messenger.backup.v2.proto.ChatFolder
...
import io.zonarosa.messenger.database.ChatFolderTables.ChatFolderMembershipTable
import io.zonarosa.messenger.database.ChatFolderTables.ChatFolderTable
import io.zonarosa.messenger.database.ChatFolderTables.MembershipType
import io.zonarosa.messenger.database.ZonaRosaDatabase
import io.zonarosa.messenger.storage.StorageSyncHelper
import io.zonarosa.messenger.backup.v2.proto.ChatFolder as ChatFolderProto
...
object ChatFolderArchiveProcessor {
  ...
  fun import(chatFolder: ChatFolderProto, importState: ImportState) {
    ...
    val chatFolderId = ZonaRosaDatabase
      .writableDatabase
    ...
    includedChatsQueries.forEach {
      ZonaRosaDatabase.writableDatabase.execSQL(it.where, it.whereArgs)
    }
...
private fun List<Long>.toMembershipInsertQueries(chatFolderId: Long, importState: ImportState, membershipType: MembershipType): List<SqlUtil.Query> {
  val values = this
    ...
    .map { recipientId -> importState.recipientIdToLocalThreadId[recipientId] ?: ZonaRosaDatabase.threads.getOrCreateThreadIdFor(recipientId, importState.recipientIdToIsGroup[recipientId] == true) }
    .map { threadId ->
      contentValuesOf(
