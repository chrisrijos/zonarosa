...
import io.zonarosa.messenger.components.verify.SafetyNumberQrView.Companion.getSegments
import io.zonarosa.messenger.crypto.IdentityKeyParcelable
import io.zonarosa.messenger.databinding.VerifyDisplayFragmentBinding
...
class VerifyDisplayFragment : Fragment() {
  ...
  override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
    ...
    if (RemoteConfig.internalUser && ZonaRosaStore.settings.automaticVerificationEnabled && !ZonaRosaStore.uiHints.hasSeenVerifyAutomaticallySheet()) {
    ...
    binding.automaticVerification.visible = RemoteConfig.internalUser && ZonaRosaStore.settings.automaticVerificationEnabled
    ...
    binding.caption.setLinkColor(ContextCompat.getColor(requireContext(), CoreUiR.color.zonarosa_colorPrimary))
  ...
  private fun updateStatus(status: AutomaticVerificationStatus) {
    when (status) {
      ...
      AutomaticVerificationStatus.UNAVAILABLE_PERMANENT -> {
        ...
        ImageViewCompat.setImageTintList(binding.autoVerifyIcon, ColorStateList.valueOf(ContextCompat.getColor(requireContext(), CoreUiR.color.zonarosa_colorOnSurfaceVariant)))
  ...
  private fun handleShare(fingerprint: Fingerprint) {
    val shareString = """
        ${getString(R.string.VerifyIdentityActivity_our_zonarosa_safety_number)}
  ...
  private fun setRecipientText(recipient: Recipient) {
    ...
    binding.description.setLink("https://zonarosa.io/redirect/safety-numbers")
    binding.description.setLinkColor(ContextCompat.getColor(requireContext(), CoreUiR.color.zonarosa_colorPrimary))
  }
