...
object BackupVerifier {
  ...
  fun verifyFile(cipherStream: InputStream, passphrase: String, expectedCount: Long, cancellationZonaRosa: FullBackupExporter.BackupCancellationZonaRosa): Boolean {
    ...
    cipherStream.use {
      while (frame.end != true && !cancellationZonaRosa.isCanceled) {
        val verified = when {
