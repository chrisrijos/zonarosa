 ...
 * reports that to the [io.zonarosa.messenger.webrtc.audio.ZonaRosaAudioManager], and then handles connecting/disconnecting
 * to the device if requested by [io.zonarosa.messenger.webrtc.audio.ZonaRosaAudioManager].
 */
@SuppressLint("MissingPermission") // targetSdkVersion is still 30 (https://issuetracker.google.com/issues/201454155)
class ZonaRosaBluetoothManager(
  private val context: Context,
  private val audioDeviceUpdatedListener: AudioDeviceUpdatedListener,
  private val handler: ZonaRosaAudioHandler
) {
  ...
  companion object {
    private val TAG = Log.tag(ZonaRosaBluetoothManager::class.java)
