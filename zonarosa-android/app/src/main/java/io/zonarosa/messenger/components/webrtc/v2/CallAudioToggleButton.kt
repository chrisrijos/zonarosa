...
import io.zonarosa.messenger.components.webrtc.AudioStateUpdater
import io.zonarosa.messenger.components.webrtc.ToggleButtonOutputState
...
import io.zonarosa.messenger.components.webrtc.WebRtcAudioPicker31
...
fun CallAudioToggleButton(
  ...
  val containerColor = if (currentOutput == WebRtcAudioOutput.HANDSET || allOutputs.size >= SHOW_PICKER_THRESHOLD) {
    ...
    colorResource(id = CoreUiR.color.zonarosa_light_colorSecondaryContainer)
  }

  val contentColor = if (currentOutput == WebRtcAudioOutput.HANDSET || allOutputs.size >= SHOW_PICKER_THRESHOLD) {
    colorResource(id = CoreUiR.color.zonarosa_light_colorOnPrimary)
  } else {
    colorResource(id = CoreUiR.color.zonarosa_light_colorOnSecondaryContainer)
  }
