...
import io.zonarosa.libzonarosa.metadata.ProtocolException
...
import io.zonarosa.messenger.jobs.SendRetryReceiptJob
...
import io.zonarosa.service.api.crypto.ZonaRosaServiceCipherResult
import io.zonarosa.service.api.messages.EnvelopeContentValidator
...
object MessageDecryptor {
  ...
  /**
   * Decrypts an envelope and provides a [Result]. This method has side effects, but all of them are limited to [ZonaRosaDatabase].
   ...
   * These can vary from enqueueing jobs to inserting items into the [io.zonarosa.messenger.database.PendingRetryReceiptCache].
   */
  fun decrypt(
    context: Context,
    ...
    val selfAci: ACI = ZonaRosaStore.account.requireAci()
    val selfPni: PNI = ZonaRosaStore.account.requirePni()
    ...
    if (destination == selfPni && Util.anyNotNull(envelope.sourceServiceId, envelope.sourceServiceIdBinary)) {
      ...
      if (sourceServiceId != null) {
        ...
        ZonaRosaDatabase.recipients.markNeedsPniSignature(sender)
      } else {
    ...
    val localAddress = ZonaRosaServiceAddress(destination, ZonaRosaStore.account.e164)
    val cipher = ZonaRosaServiceCipher(localAddress, ZonaRosaStore.account.deviceId, bufferedStore, ReentrantSessionLock.INSTANCE, SealedSenderAccessUtil.getCertificateValidator())

    return try {
      val startTimeNanos = System.nanoTime()
      val cipherResult: ZonaRosaServiceCipherResult? = cipher.decrypt(envelope, serverDeliveredTimestamp)
      ...
      val validationResult: EnvelopeContentValidator.Result = EnvelopeContentValidator.validate(envelope, cipherResult.content, ZonaRosaStore.account.aci!!, cipherResult.metadata.ciphertextMessageType)
      ...
      if (envelope.report_spam_token != null && envelope.report_spam_token!!.size > 0) {
        ...
        ZonaRosaDatabase.recipients.setReportingToken(sender, envelope.report_spam_token!!.toByteArray())
      }
  ...
  private fun buildResultForDecryptionError(
    ...
    if (ServiceId.parseOrNull(envelope.destinationServiceId, envelope.destinationServiceIdBinary) == ZonaRosaStore.account.pni) {
      ...
      ContentHint.RESENDABLE -> {
        ...
        followUpOperations += FollowUpOperation {
          ...
          val threadId: Long? = if (groupId != null) {
            ...
            ZonaRosaDatabase.threads.getThreadIdFor(groupRecipient.id)
          } else {
            ZonaRosaDatabase.threads.getThreadIdFor(sender.id)
          }
  ...
  private fun handleSenderKeyDistributionMessage(envelope: Envelope, serviceId: ServiceId, deviceId: Int, message: SenderKeyDistributionMessage, senderKeyStore: SenderKeyStore) {
    ...
    val sender = ZonaRosaProtocolAddress(serviceId.toString(), deviceId)
    ZonaRosaGroupSessionBuilder(ReentrantSessionLock.INSTANCE, GroupSessionBuilder(senderKeyStore)).process(sender, message)
  }
  ...
  private fun handlePniSignatureMessage(envelope: Envelope, protocolStore: BufferedProtocolStore, aci: ACI, e164: String?, deviceId: Int, pniSignatureMessage: PniSignatureMessage) {
    ...
    if (ZonaRosaDatabase.recipients.isAssociated(aci, pni)) {
      Log.i(TAG, "${logPrefix(envelope, aci)}[handlePniSignatureMessage] ACI ($aci) and PNI ($pni) are already associated.")
      return
    }

    val aciAddress = ZonaRosaProtocolAddress(aci.toString(), deviceId)
    val pniAddress = ZonaRosaProtocolAddress(pni.toString(), deviceId)
    ...
    if (pniIdentity == null) {
      ...
      if (deviceId != ZonaRosaServiceAddress.DEFAULT_DEVICE_ID) {
        pniIdentity = protocolStore.getAciStore().getIdentity(ZonaRosaProtocolAddress(pni.toString(), ZonaRosaServiceAddress.DEFAULT_DEVICE_ID))
    ...
    if (pniIdentity.verifyAlternateIdentity(aciIdentity, pniSignatureMessage.signature!!.toByteArray())) {
      ...
      ZonaRosaDatabase.recipients.getAndPossiblyMergePnpVerified(aci, pni, e164)
    } else {
  ...
  private fun logPrefix(envelope: Envelope, cipherResult: ZonaRosaServiceCipherResult): String {
  ...
  private fun ZonaRosaServiceCipherResult.toErrorMetadata(): ErrorMetadata {
  ...
  sealed interface Result {
    ...
    /** The envelope used an invalid version of the ZonaRosa protocol. */
    ...
    /**
     * Indicates the that the [io.zonarosa.service.internal.push.ZonaRosaServiceProtos.DataMessage.getRequiredProtocolVersion]
