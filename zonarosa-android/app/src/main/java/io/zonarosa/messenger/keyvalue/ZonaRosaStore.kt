 ...
 */
class ZonaRosaStore(context: Application, private val store: KeyValueStore) {
  ...
  companion object {
    ...
    private var instance: ZonaRosaStore? = null

    @JvmStatic
    fun init(context: Application) {
      if (instance == null) {
        synchronized(ZonaRosaStore::class.java) {
          if (instance == null) {
            instance = ZonaRosaStore(context, KeyValueStore(KeyValueDatabase.getInstance(context)))
          }
        }
      }
    }
    ...
    val groupsV2AciAuthorizationCache: GroupsV2AuthorizationZonaRosaStoreCache
      get() = GroupsV2AuthorizationZonaRosaStoreCache.createAciCache(instance!!.store)

    val plaintext: PlainTextSharedPrefsDataStore
      get() = instance!!.plainTextValues

    fun getPreferenceDataStore(): PreferenceDataStore {
      return ZonaRosaPreferenceDataStore(instance!!.store)
    }

    /**
     * Ensures any pending writes are finished. Only intended to be called by
     * [ZonaRosaUncaughtExceptionHandler].
     */
    @JvmStatic
