...
import io.zonarosa.core.util.SqlUtil.appendArg
import io.zonarosa.core.util.SqlUtil.buildArgs
import io.zonarosa.core.util.SqlUtil.buildCaseInsensitiveGlobPattern
import io.zonarosa.core.util.SqlUtil.buildCollectionQuery
...
import io.zonarosa.core.util.readToMap
...
import io.zonarosa.messenger.groups.GroupId.Push
...
import io.zonarosa.service.api.groupsv2.findMemberByAci
import io.zonarosa.service.api.groupsv2.findPendingByServiceId
import io.zonarosa.service.api.groupsv2.findRequestingByAci
import io.zonarosa.service.api.groupsv2.toAciList
import io.zonarosa.service.api.groupsv2.toAciListWithUnknowns
...
class GroupTable(context: Context?, databaseHelper: ZonaRosaDatabase?) :
  ...
  class MembershipTable(context: Context, databaseHelper: ZonaRosaDatabase) : DatabaseTable(context, databaseHelper) {
    companion object {
  ...
  fun queryGroupsByMemberName(inputQuery: String): Cursor {
    ...
    return databaseHelper.zonarosaReadableDatabase.query(statement, subquery.whereArgs)
  }
  ...
  fun queryGroupsByTitle(inputQuery: String, includeInactive: Boolean, excludeV1: Boolean, excludeMms: Boolean): Reader {
    ...
    val cursor = databaseHelper.zonarosaReadableDatabase.query(statement, query.whereArgs)
  ...
  private fun queryGroupsByRecency(groupQuery: GroupQuery): Reader {
    ...
    return Reader(databaseHelper.zonarosaReadableDatabase.rawQuery(sql, query.whereArgs))
  }
  ...
  @CheckReturnValue
  fun create(groupId: GroupId.V1, title: String?, members: Collection<RecipientId>, avatar: ZonaRosaServiceAttachmentPointer?): Boolean {
  ...
  private fun create(
    ...
    avatar: ZonaRosaServiceAttachmentPointer?,
    groupMasterKey: GroupMasterKey?,
    ...
    if (avatar != null) {
      values.put(AVATAR_ID, (avatar.remoteId as ZonaRosaServiceAttachmentRemoteId.V2).cdnId)
    ...
    if (groupId.isV2) {
      ZonaRosaDatabase.nameCollisions.handleGroupNameCollisions(groupId.requireV2(), members.toSet())
    }

    return true
  }

  fun update(groupId: GroupId.V1, title: String?, avatar: ZonaRosaServiceAttachmentPointer?) {
    val contentValues = ContentValues().apply {
      ...
      if (avatar != null) {
        put(AVATAR_ID, (avatar.remoteId as ZonaRosaServiceAttachmentRemoteId.V2).cdnId)
  ...
  fun update(groupId: GroupId.V2, decryptedGroup: DecryptedGroup, receivedGroupSendEndorsements: ReceivedGroupSendEndorsements?) {
    ...
    if (groupId.isV2 && addedMembers.isNotEmpty()) {
      ZonaRosaDatabase.nameCollisions.handleGroupNameCollisions(groupId.requireV2(), addedMembers.toSet())
    }
  }
  ...
  fun getGroupSendFullToken(groupId: GroupId.V2, recipientId: RecipientId): GroupSendFullToken? {
    val groupRecord = ZonaRosaDatabase.groups.getGroup(groupId).orElse(null) ?: return null
    val endorsement = ZonaRosaDatabase.groups.getGroupSendEndorsement(groupId, recipientId) ?: return null
  ...
  class V2GroupProperties(val groupMasterKey: GroupMasterKey, val groupRevision: Int, private val decryptedGroupBytes: ByteArray) {
    ...
    fun getMemberRecipientIds(memberSet: MemberSet): List<RecipientId> {
      ...
      val selfAci = ZonaRosaStore.account.requireAci()
