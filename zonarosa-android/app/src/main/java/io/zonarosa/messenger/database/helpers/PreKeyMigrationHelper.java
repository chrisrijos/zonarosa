  ...
  public static boolean migratePreKeys(Context context, SQLiteDatabase database) {
    ...
    if (oneTimePreKeyIndex.exists()) {
      try {
        ...
        ZonaRosaStore.account().aciPreKeys().setNextEcOneTimePreKeyId(index.nextPreKeyId);
      } catch (IOException e) {
        Log.w(TAG, e);
      }
    }
    ...
    if (signedPreKeyIndex.exists()) {
      try {
        ...
        ZonaRosaStore.account().aciPreKeys().setNextSignedPreKeyId(index.nextSignedPreKeyId);
        ZonaRosaStore.account().aciPreKeys().setActiveSignedPreKeyId(index.activeSignedPreKeyId);
      } catch (IOException e) {
        Log.w(TAG, e);
      }
    }
