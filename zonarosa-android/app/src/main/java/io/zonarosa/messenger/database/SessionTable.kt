...
class SessionTable(context: Context, databaseHelper: ZonaRosaDatabase) : DatabaseTable(context, databaseHelper) {
  companion object {
  ...
  fun store(serviceId: ServiceId, address: ZonaRosaProtocolAddress, record: SessionRecord) {
  ...
  fun load(serviceId: ServiceId, address: ZonaRosaProtocolAddress): SessionRecord? {
  ...
  fun load(serviceId: ServiceId, addresses: List<ZonaRosaProtocolAddress>): List<SessionRecord?> {
    ...
    val sessions: HashMap<ZonaRosaProtocolAddress, SessionRecord?> = LinkedHashMap(addresses.size)
      ...
      readableDatabase.query(TABLE_NAME, projection, combinedQuery.where, combinedQuery.whereArgs, null, null, null).use { cursor ->
        while (cursor.moveToNext()) {
          ...
          try {
            ...
            sessions[ZonaRosaProtocolAddress(address, device)] = record
          } catch (e: IOException) {
            Log.w(TAG, e)
          }
        }
  ...
  fun getSubDevices(serviceId: ServiceId, addressName: String): List<Int> {
    ...
    val selection = "$ACCOUNT_ID = ? AND $ADDRESS = ? AND $DEVICE != ?"
    val args = SqlUtil.buildArgs(serviceId, addressName, ZonaRosaServiceAddress.DEFAULT_DEVICE_ID)
  ...
  fun delete(serviceId: ServiceId, address: ZonaRosaProtocolAddress) {
