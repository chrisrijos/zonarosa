...
import io.zonarosa.messenger.jobs.protos.ArchiveAttachmentReconciliationJobData
...
class ArchiveAttachmentReconciliationJob private constructor(
  ...
  companion object {
    ...
    fun enqueueIfRetryAllowed(forced: Boolean) {
      if (ZonaRosaStore.backup.archiveAttachmentReconciliationAttempts < 3) {
        ZonaRosaStore.backup.archiveAttachmentReconciliationAttempts++
  ...
  override fun run(): Result {
    ...
    if (ZonaRosaStore.backup.lastAttachmentReconciliationTime < 0) {
      ...
      ZonaRosaStore.backup.lastAttachmentReconciliationTime = System.currentTimeMillis()
      return Result.success()
    }

    val timeSinceLastSync = System.currentTimeMillis() - ZonaRosaStore.backup.lastAttachmentReconciliationTime
    ...
    snapshotVersion = snapshotVersion ?: ZonaRosaDatabase.backupMediaSnapshots.getCurrentSnapshotVersion()
  ...
  private fun syncDataFromCdn(snapshotVersion: Long): Result? {
    ...
    Log.d(TAG, "BEFORE:\n" + ZonaRosaDatabase.attachments.debugGetAttachmentStats().shortPrettyString(), true)
    stopwatch.split("stats-before")

    val mediaObjectsThatMayNeedReUpload = ZonaRosaDatabase.backupMediaSnapshots.getMediaObjectsLastSeenOnCdnBeforeSnapshotVersion(snapshotVersion)
    ...
    if (mayNeedReUploadCount > 0) {
      ...
      mediaObjectsThatMayNeedReUpload.forEach { mediaObjectCursor ->
        ...
        if (entry.isThumbnail) {
          ...
          val wasReset = ZonaRosaDatabase.attachments.resetArchiveThumbnailTransferStateByPlaintextHashAndRemoteKeyIfNecessary(entry.plaintextHash, entry.remoteKey)
          if (wasReset) {
          ...
          val wasReset = ZonaRosaDatabase.attachments.resetArchiveTransferStateByPlaintextHashAndRemoteKeyIfNecessary(entry.plaintextHash, entry.remoteKey)
          if (wasReset) {
      ...
      if (internalUser && mediaIdsThatNeedUpload.isNotEmpty()) {
        ...
        val matchingAttachments = ZonaRosaDatabase.attachments.getAttachmentDataForMediaIds(mediaIdsThatNeedUpload)
        ...
        matchingAttachments.take(250).forEach { match ->
          if (match.isThumbnail) {
            val thumbnailTransferState = ZonaRosaDatabase.attachments.getArchiveThumbnailTransferState(match.attachment.attachmentId)
    ...
    ZonaRosaStore.backup.remoteStorageGarbageCollectionPending = false
    ZonaRosaStore.backup.lastAttachmentReconciliationTime = System.currentTimeMillis()
  ...
  private fun syncCdnPage(archivedItemPage: ArchiveGetMediaItemsResponse, currentSnapshotVersion: Long): Set<ArchivedMediaObject> {
    ...
    ZonaRosaDatabase.backupMediaSnapshots.markSeenOnRemote(
      mediaIdBatch = mediaObjects.map { it.mediaId },
      snapshotVersion = currentSnapshotVersion
    )

    val mediaOnRemoteButNotLocal = ZonaRosaDatabase.backupMediaSnapshots.getMediaObjectsThatCantBeFound(mediaObjects)
    val mediaObjectsOnBothRemoteAndLocal = mediaObjects - mediaOnRemoteButNotLocal

    val cdnMismatches = ZonaRosaDatabase.backupMediaSnapshots.getMediaObjectsWithNonMatchingCdn(mediaObjectsOnBothRemoteAndLocal)
    if (cdnMismatches.isNotEmpty()) {
      ...
      for (mismatch in cdnMismatches) {
        ZonaRosaDatabase.attachments.setArchiveCdnByPlaintextHashAndRemoteKey(mismatch.plaintextHash, mismatch.remoteKey, mismatch.cdn)
      }
    }
  ...
  /**
   * Deletes attachments from the archive CDN, after verifying that they also can't be found anywhere in [io.zonarosa.messenger.database.AttachmentTable]
  ...
  private fun validateAndDeleteFromRemote(deletes: Set<ArchivedMediaObject>): Result? {
    ...
    val validatedDeletes: MutableSet<ArchivedMediaObject> = ZonaRosaDatabase.attachments.getMediaObjectsThatCantBeFound(deletes).toMutableSet()
    ...
    if (foundLocally.isNotEmpty()) {
      ...
      val matches = ZonaRosaDatabase.attachments.getAttachmentDataForMediaIds(foundLocally.map { MediaId(it.mediaId) })
      for (match in matches) {
        if (match.messageRecord?.fromRecipient != null && match.messageRecord.fromRecipient.id == ZonaRosaStore.releaseChannel.releaseChannelRecipientId) {
          ...
          val stringMediaId = match.mediaId.encode()
          validatedDeletes += foundLocally.first { it.mediaId == stringMediaId }
          ZonaRosaDatabase.attachments.deleteAttachment(match.attachment.attachmentId)
    ...
    if (updatedFoundLocally.isNotEmpty()) {
      val fixedCount = ZonaRosaDatabase.attachments.setArchiveFinishedForMatchingMediaObjects(updatedFoundLocally)
      if (fixedCount > 0) {
