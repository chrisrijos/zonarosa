...
import io.zonarosa.messenger.stories.my.MyStoriesActivity
...
object NotificationFactory {
  ...
  private fun notifyForConversation(
    ...
    builder.apply {
      ...
      setPriority(ZonaRosaPreferences.getNotificationPriority(context))
      setLights()
  ...
  private fun notifyInThread(context: Context, recipient: Recipient, lastAudibleNotification: Long) {
    if (!NotificationChannels.getInstance().areNotificationsEnabled() ||
      !ZonaRosaStore.settings.isMessageNotificationsInChatSoundsEnabled ||
    ...
    val uri: Uri = if (NotificationChannels.supported()) {
      ...
      recipient.messageRingtone ?: ZonaRosaStore.settings.messageNotificationSound
    }
  ...
  fun notifyStoryDeliveryFailed(context: Context, recipient: Recipient, thread: ConversationId) {
    ...
    val contentTitle = if (ZonaRosaStore.settings.messageNotificationsPrivacy.isDisplayContact) {
    ...
    val largeIcon = if (ZonaRosaStore.settings.messageNotificationsPrivacy.isDisplayContact) {
  ...
  fun notifyProofRequired(context: Context, recipient: Recipient, thread: ConversationId, visibleThread: ConversationId?) {
    ...
    builder.apply {
      ...
      setContentText(context.getString(R.string.MessageNotifier_verify_to_continue_messaging_on_zonarosa))
      setContentIntent(NotificationPendingIntentHelper.getActivity(context, 0, intent, PendingIntentFlags.mutable()))
    ...
    try {
      ...
      if (threadRecipient != null) {
        ZonaRosaExecutors.BOUNDED.execute {
          ZonaRosaDatabase.recipients.setMessageRingtone(threadRecipient.id, null)
