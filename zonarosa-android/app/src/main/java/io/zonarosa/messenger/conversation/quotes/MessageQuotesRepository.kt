package io.zonarosa.messenger.conversation.quotes
...
class MessageQuotesRepository {
  ...
  fun getMessagesInQuoteChain(application: Application, messageId: MessageId): Observable<List<ConversationMessage>> {
    return Observable.create { emitter ->
      val threadId: Long = ZonaRosaDatabase.messages.getThreadIdForMessage(messageId.id)
      if (threadId < 0) {
  ...
  private fun getMessagesInQuoteChainSync(application: Application, messageId: MessageId): List<ConversationMessage> {
    val rootMessageId: MessageId = ZonaRosaDatabase.messages.getRootOfQuoteChain(messageId)

    var originalRecord: MessageRecord? = ZonaRosaDatabase.messages.getMessageRecordOrNull(rootMessageId.id)

    if (originalRecord == null) {
      return emptyList()
    }

    var replyRecords: List<MessageRecord> = ZonaRosaDatabase.messages.getAllMessagesThatQuote(rootMessageId)

    val reactionHelper = ReactionHelper()
    val attachmentHelper = AttachmentHelper()
    val threadRecipient = requireNotNull(ZonaRosaDatabase.threads.getRecipientForThreadId(originalRecord.threadId))
    ...
    if (originalRecord.isPaymentNotification) {
      originalRecord = ZonaRosaDatabase.payments.updateMessageWithPayment(originalRecord)
    }
