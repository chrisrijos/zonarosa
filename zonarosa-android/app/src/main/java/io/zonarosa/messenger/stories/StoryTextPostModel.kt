...
data class StoryTextPostModel(
  ...
  class Decoder : ResourceDecoder<StoryTextPostModel, Bitmap> {
    ...
    override fun decode(source: StoryTextPostModel, width: Int, height: Int, options: Options): Resource<Bitmap> {
      val message = ZonaRosaDatabase.messages.getMessageFor(source.storySentAtMillis, source.storyAuthor).run {
        if (this is MmsMessageRecord) {
          this.withAttachments(ZonaRosaDatabase.attachments.getAttachmentsForMessage(this.id))
        } else {
          this
        }
      }
      val view = StoryTextPostView(ContextThemeWrapper(AppDependencies.application, R.style.ZonaRosa_DarkNoActionBar))
