...
import io.zonarosa.service.api.storage.recipientServiceAddresses
import java.util.UUID

/**
 * Stores distribution lists, which represent different sets of people you may want to share a story with.
 */
class DistributionListTables constructor(context: Context?, databaseHelper: ZonaRosaDatabase?) : DatabaseTable(context, databaseHelper), RecipientIdDatabaseReference {
  ...
  fun createList(
    ...
    try {
      ...
      val recipientId = ZonaRosaDatabase.recipients.getOrInsertFromDistributionListId(DistributionListId.from(id), storageId)
    ...
    return when (privacyMode) {
      DistributionListPrivacyMode.ALL -> {
        ZonaRosaDatabase.recipients
          .getZonaRosaContacts(RecipientTable.IncludeSelfMode.Exclude)
  ...
  fun getPrivacyData(listId: DistributionListId): DistributionListPrivacyData {
    ...
    readableDatabase.withinTransaction {
      ...
      totalContactCount = ZonaRosaDatabase.recipients.getZonaRosaContactsCount(RecipientTable.IncludeSelfMode.Exclude)
    }
  ...
  fun getRecipientIdForSyncRecord(record: ZonaRosaStoryDistributionListRecord): RecipientId? {
  ...
  fun applyStorageSyncStoryDistributionListInsert(insert: ZonaRosaStoryDistributionListRecord) {
  ...
  fun applyStorageSyncStoryDistributionListUpdate(update: StorageRecordUpdate<ZonaRosaStoryDistributionListRecord>) {
    ...
    ZonaRosaDatabase.recipients.updateStorageId(recipientId, update.new.id.raw)
