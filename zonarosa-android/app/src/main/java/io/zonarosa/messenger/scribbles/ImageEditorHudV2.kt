...
import io.zonarosa.messenger.components.TooltipPopup
import io.zonarosa.messenger.keyvalue.ZonaRosaStore
import io.zonarosa.messenger.mediasend.v2.MediaAnimations
import io.zonarosa.messenger.scribbles.HSVColorSlider.getColor
import io.zonarosa.messenger.scribbles.HSVColorSlider.setColor
import io.zonarosa.messenger.scribbles.HSVColorSlider.setUpForColor
...
class ImageEditorHudV2 @JvmOverloads constructor(
  ...
  private fun setupWidthSeekBar() {
    ...
    widthSeekBar.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
      override fun onProgressChanged(seekBar: SeekBar?, progress: Int, fromUser: Boolean) {
        ...
        when (currentMode) {
          Mode.DRAW -> ZonaRosaStore.imageEditor.setMarkerPercentage(progress)
          Mode.BLUR -> ZonaRosaStore.imageEditor.setBlurPercentage(progress)
          Mode.HIGHLIGHT -> ZonaRosaStore.imageEditor.setHighlighterPercentage(progress)
          else -> Unit
        }
      }
  ...
  private fun presentModeDraw() {
    ...
    widthSeekBar.progress = ZonaRosaStore.imageEditor.getMarkerPercentage()
    listener?.onColorChange(getActiveColor())
    updateColorIndicator()
    animateModeChange(
  ...
  private fun presentModeHighlight() {
    ...
    widthSeekBar.progress = ZonaRosaStore.imageEditor.getHighlighterPercentage()
    listener?.onColorChange(getActiveColor())
    updateColorIndicator()
    animateModeChange(
      inSet = drawButtonRow + drawTools,
  ...
  private fun presentModeBlur() {
    ...
    widthSeekBar.progress = ZonaRosaStore.imageEditor.getBlurPercentage()
    listener?.onColorChange(getActiveColor())
    updateColorIndicator()
    animateModeChange(
  ...
  companion object {
    ...
    private val DRAW_WIDTH_BOUNDARIES: Map<Mode, Pair<Float, Float>> = mapOf(
      Mode.DRAW to ZonaRosaStore.imageEditor.getMarkerWidthRange(),
      Mode.HIGHLIGHT to ZonaRosaStore.imageEditor.getHighlighterWidthRange(),
      Mode.BLUR to ZonaRosaStore.imageEditor.getBlurWidthRange()
    )
