...
class SetCurrencyViewModel(
  ...
  private val store = Store(
    SetCurrencyState(
      selectedCurrencyCode = if (inAppPaymentType.recurring) {
        ZonaRosaStore.inAppPayments.getRecurringDonationCurrency().currencyCode
      } else {
        ZonaRosaStore.inAppPayments.getOneTimeCurrency().currencyCode
      },
  ...
  fun setSelectedCurrency(selectedCurrencyCode: String) {
    ...
    if (!inAppPaymentType.recurring) {
      ZonaRosaStore.inAppPayments.setOneTimeCurrency(Currency.getInstance(selectedCurrencyCode))
    } else {
