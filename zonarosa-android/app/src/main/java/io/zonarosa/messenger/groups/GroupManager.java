...
public final class GroupManager {
      ...
      throws GroupChangeBusyException, GroupChangeFailedException, IOException
  {
    ...
    ZonaRosaDatabase.recipients().getByGroupId(groupId).ifPresent(id -> ZonaRosaDatabase.messages().deleteScheduledMessages(id));
  }
      ...
      throws GroupChangeBusyException, GroupChangeFailedException, GroupNotAMemberException, GroupInsufficientRightsException, IOException
  {
    try (GroupManagerV2.GroupEditor editor = new GroupManagerV2(context).edit(groupId.requireV2())) {
      editor.acceptInvite();
      ZonaRosaDatabase.groups().setActive(groupId, true);
    }
  }
  ...
  {
    GroupTable.V2GroupProperties groupProperties = ZonaRosaDatabase.groups().requireGroup(groupId).requireV2GroupProperties();
