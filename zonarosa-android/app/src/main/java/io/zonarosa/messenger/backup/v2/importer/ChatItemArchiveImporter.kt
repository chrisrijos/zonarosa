...
import io.zonarosa.core.util.asList
...
import io.zonarosa.messenger.backup.v2.proto.BodyRange
import io.zonarosa.messenger.backup.v2.proto.ChatItem
import io.zonarosa.messenger.backup.v2.proto.ChatUpdateMessage
import io.zonarosa.messenger.backup.v2.proto.ContactAttachment
import io.zonarosa.messenger.backup.v2.proto.DirectStoryReplyMessage
import io.zonarosa.messenger.backup.v2.proto.GroupCall
import io.zonarosa.messenger.backup.v2.proto.IndividualCall
import io.zonarosa.messenger.backup.v2.proto.LinkPreview
import io.zonarosa.messenger.backup.v2.proto.MessageAttachment
import io.zonarosa.messenger.backup.v2.proto.PaymentNotification
import io.zonarosa.messenger.backup.v2.proto.Quote
import io.zonarosa.messenger.backup.v2.proto.Reaction
import io.zonarosa.messenger.backup.v2.proto.SendStatus
import io.zonarosa.messenger.backup.v2.proto.SimpleChatUpdate
import io.zonarosa.messenger.backup.v2.proto.StandardMessage
import io.zonarosa.messenger.backup.v2.proto.Sticker
import io.zonarosa.messenger.backup.v2.proto.ViewOnceMessage
...
import io.zonarosa.messenger.database.ReactionTable
...
import io.zonarosa.messenger.database.model.databaseprotos.CryptoValue
...
import io.zonarosa.messenger.database.model.databaseprotos.PaymentTombstone
...
import io.zonarosa.messenger.payments.CryptoValueUtil
import io.zonarosa.messenger.payments.Direction
import io.zonarosa.messenger.payments.FailureReason
import io.zonarosa.messenger.payments.State
...
import io.zonarosa.messenger.backup.v2.proto.GiftBadge as BackupGiftBadge
...
class ChatItemArchiveImporter(
  ...
  private fun ChatItem.toMessageInsert(fromRecipientId: RecipientId, chatRecipientId: RecipientId, threadId: Long): MessageInsert {
    ...
    if (this.updateMessage != null) {
      if (this.updateMessage.individualCall != null && this.updateMessage.individualCall.callId != null) {
        ...
        followUps += { endPollMessageId ->
          val pollMessageId = ZonaRosaDatabase.messages.getMessageFor(updateMessage.pollTerminate.targetSentTimestamp, fromRecipientId)?.id ?: -1
          val pollId = ZonaRosaDatabase.polls.getPollId(pollMessageId)
          ...
          if (pollId != null) {
            ZonaRosaDatabase.polls.endPoll(pollId = pollId, endingMessageId = endPollMessageId)
          }
        }
        ...
        followUps += { pinUpdateMessageId ->
          ...
          if (targetAuthorId != null) {
            val pinnedMessageId = ZonaRosaDatabase.messages.getMessageFor(updateMessage.pinMessage.targetSentTimestamp, targetAuthorId)?.id ?: -1
      ...
      if (contact != null) {
        ...
        followUps += { messageRowId ->
          val attachmentMap = if (contactAttachment != null) {
            ZonaRosaDatabase.attachments.insertAttachmentsForMessage(messageRowId, listOf(contactAttachment), emptyList())
          } else {
            emptyMap()
          }
          db.update(
            ...
            contentValuesOf(
              MessageTable.SHARED_CONTACTS to ZonaRosaDatabase.messages.getSerializedSharedContacts(attachmentMap, listOf(contact))
            ),
            "${MessageTable.ID} = ?",
    ...
    if (this.directStoryReplyMessage != null) {
      ...
      if (longTextAttachment != null) {
        followUps += { messageRowId ->
          val ids = ZonaRosaDatabase.attachments.insertAttachmentsForMessage(messageRowId, listOf(longTextAttachment), emptyList())
          ids.values.firstOrNull()?.let { attachmentId ->
            ZonaRosaDatabase.attachments.setTransferState(messageRowId, attachmentId, AttachmentTable.TRANSFER_PROGRESS_DONE)
          }
        }
      }
    }
      ...
      if (hasAttachments || linkPreviews.isNotEmpty()) {
        followUps += { messageRowId ->
          val attachmentMap = if (hasAttachments) {
            ZonaRosaDatabase.attachments.insertAttachmentsForMessage(messageRowId, attachments + linkPreviewAttachments + longTextAttachment.asList(), quoteAttachments)
          } else {
            emptyMap()
          }
          ...
          if (linkPreviews.isNotEmpty()) {
            db.update(MessageTable.TABLE_NAME)
              .values(MessageTable.LINK_PREVIEWS to ZonaRosaDatabase.messages.getSerializedLinkPreviews(attachmentMap, linkPreviews))
              .where("${MessageTable.ID} = ?", messageRowId)
              .run()
          }
        }
    ...
    if (this.stickerMessage != null) {
      ...
      if (attachment != null) {
        followUps += { messageRowId ->
          ZonaRosaDatabase.attachments.insertAttachmentsForMessage(messageRowId, listOf(attachment), emptyList())
        }
      }
    }
    ...
    if (this.poll != null) {
      ...
      followUps += { messageRowId ->
        val pollId = ZonaRosaDatabase.polls.insertPoll(
        ...
        val localOptionIds = ZonaRosaDatabase.polls.getPollOptionIds(pollId)
        poll.options.forEachIndexed { index, option ->
          ...
          ZonaRosaDatabase.polls.addPollVotes(pollId = pollId, optionId = localOptionIds[index], voters = localVoters)
        }
        ...
        if (poll.hasEnded) {
          ...
          ZonaRosaDatabase.polls.endPoll(pollId = pollId, endingMessageId = -1)
        }
      }
    }
  ...
  private fun StandardMessage.parseBodyText(importState: ImportState): Pair<String?, Attachment?> {
    if (this.longText != null) {
      return null to this.longText.toLocalAttachment(contentType = "text/x-zonarosa-plain")
    }
  ...
  private fun DirectStoryReplyMessage.parseBodyText(importState: ImportState): Pair<String?, Attachment?> {
    if (this.textReply?.longText != null) {
      return null to this.textReply.longText.toLocalAttachment(contentType = "text/x-zonarosa-plain")
    }
  ...
  private fun tryRestorePayment(chatItem: ChatItem, chatRecipientId: RecipientId): UUID? {
    ...
    return ZonaRosaDatabase.payments.restoreFromBackup(
      chatRecipientId,
    ...
    when {
      updateMessage.simpleUpdate != null -> {
        ...
        typeFlags = when (updateMessage.simpleUpdate.type) {
          ...
          SimpleChatUpdate.Type.JOINED_ZONAROSA -> MessageTypes.JOINED_TYPE or typeWithoutBase
  ...
  private fun ContentValues.addFailedPaymentNotification(chatItem: ChatItem, amount: Money, fee: Money, chatRecipientId: RecipientId) {
    val uuid = ZonaRosaDatabase.payments.restoreFromBackup(
      chatRecipientId,
      0,
      0,
      0,
  ...
  private fun LinkPreview.toLocalLinkPreview(): io.zonarosa.messenger.linkpreview.LinkPreview {
    return io.zonarosa.messenger.linkpreview.LinkPreview(
      this.url,
      this.title ?: "",
      this.description ?: "",
      this.date ?: 0,
