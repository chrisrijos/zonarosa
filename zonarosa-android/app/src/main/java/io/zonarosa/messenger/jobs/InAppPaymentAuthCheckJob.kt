...
import io.zonarosa.donations.json.StripePaymentIntent
import io.zonarosa.donations.json.StripeSetupIntent
...
class InAppPaymentAuthCheckJob private constructor(parameters: Parameters) : BaseJob(parameters), StripeApi.PaymentIntentFetcher, StripeApi.SetupIntentHelper {
  ...
  companion object {
    ...
    @JvmStatic
    fun enqueueIfNeeded() {
      if (ZonaRosaDatabase.inAppPayments.hasWaitingForAuth()) {
  ...
  override fun onRun() {
    ...
    val unauthorizedInAppPayments = ZonaRosaDatabase.inAppPayments.getAllWaitingForAuth()
  ...
  private fun migrateLegacyData() {
    val pending3DSData = ZonaRosaStore.inAppPayments.consumePending3DSData()
  ...
  private fun checkOneTimePayment(inAppPayment: InAppPaymentTable.InAppPayment): CheckResult<Unit> {
    ...
    val updatedPayment = ZonaRosaDatabase.inAppPayments.getById(inAppPayment.id)
  ...
  private fun checkRecurringPayment(inAppPayment: InAppPaymentTable.InAppPayment): CheckResult<Unit> {
    ...
    Log.d(TAG, "Set default payment method via ZonaRosa service.", true)

    ZonaRosaDatabase.inAppPaymentSubscribers.setPaymentMethod(subscriber.subscriberId, inAppPayment.data.paymentMethodType)
    ...
    try {
      ...
      if (updateLevelResponse.status in listOf(200, 204)) {
        ...
        with(ZonaRosaDatabase.inAppPayments.getById(inAppPayment.id)!!) {
  ...
  private fun markChecked(inAppPayment: InAppPaymentTable.InAppPayment) {
    val fresh = ZonaRosaDatabase.inAppPayments.getById(inAppPayment.id)
    ZonaRosaDatabase.inAppPayments.update(
