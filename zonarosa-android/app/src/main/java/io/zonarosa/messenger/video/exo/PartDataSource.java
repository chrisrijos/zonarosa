...
import io.zonarosa.messenger.backup.v2.DatabaseAttachmentArchiveUtil;
...
import io.zonarosa.core.util.stream.TailerInputStream;
    ...
    if (inProgress && !hasData && hasIncrementalDigest && attachmentKey != null) {
      ...
      if (attachment.transferState == AttachmentTable.TRANSFER_RESTORE_IN_PROGRESS && attachment.archiveTransferState == AttachmentTable.ArchiveTransferState.FINISHED) {
        ...
        try {
          ...
          String mediaId   = MediaName.toMediaIdString(mediaName, ZonaRosaStore.backup().getMediaRootBackupKey());

          MediaRootBackupKey.MediaKeyMaterial mediaKeyMaterial     = ZonaRosaStore.backup().getMediaRootBackupKey().deriveMediaSecretsFromMediaId(mediaId);
