...
import io.zonarosa.messenger.backup.v2.ArchiveGroup
import io.zonarosa.messenger.backup.v2.proto.Group
...
object GroupArchiveImporter {
  fun import(group: ArchiveGroup): RecipientId {
    ...
    val recipientId = ZonaRosaDatabase.writableDatabase.insert(RecipientTable.TABLE_NAME, null, values)
    val restoredId = ZonaRosaDatabase.groups.create(masterKey, decryptedState, groupSendEndorsements = null)
    if (restoredId != null) {
      ZonaRosaDatabase.groups.setShowAsStoryState(restoredId, group.storySendMode.toLocal())
    }
...
private fun Group.GroupSnapshot.toLocal(operations: GroupsV2Operations.GroupOperations): DecryptedGroup {
  val selfAciBytes = ZonaRosaStore.account.aci?.toByteString()
