...
class CheckRestoreMediaLeftJob private constructor(parameters: Parameters) : Job(parameters) {
  ...
  override fun run(): Result {
    val remainingAttachmentSize = ZonaRosaDatabase.attachments.getRemainingRestorableAttachmentSize()
    ...
    if (remainingAttachmentSize == 0L) {
      ...
      if (!ZonaRosaStore.backup.backsUpMedia) {
        ZonaRosaDatabase.attachments.markQuotesThatNeedReconstruction()
