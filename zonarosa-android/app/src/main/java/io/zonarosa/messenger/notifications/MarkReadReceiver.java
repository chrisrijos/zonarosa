...
import io.zonarosa.messenger.jobs.SendReadReceiptJob;
...
public class MarkReadReceiver extends BroadcastReceiver {
  ...
  public static final  String CLEAR_ACTION          = "io.zonarosa.messenger.notifications.CLEAR";
    ...
    if (threads != null) {
      ...
      ZonaRosaExecutors.BOUNDED.execute(() -> {
        ...
        for (ConversationId thread : threads) {
          ...
          List<MarkedMessageInfo> messageIds = ZonaRosaDatabase.threads().setRead(thread);
  ...
  public static void processCallEvents(@NonNull List<ConversationId> threads, long timestamp) {
    List<RecipientId> peers = ZonaRosaDatabase.threads().getRecipientIdsForThreadIds(threads.stream()
                                                                                          .filter(it -> it.getGroupStoryId() == null)
                                                                                          .map(ConversationId::getThreadId)
                                                                                          .collect(java.util.stream.Collectors.toList()));

    for (RecipientId peer : peers) {
      CallTable.Call lastCallInThread = ZonaRosaDatabase.calls().markAllCallEventsWithPeerBeforeTimestampRead(peer, timestamp);
  ...
  private static void scheduleDeletion(@NonNull List<ExpirationInfo> expirationInfo) {
    if (expirationInfo.size() > 0) {
      ...
      ZonaRosaDatabase.messages().markExpireStarted(Stream.of(expirationInfo).map(info -> new kotlin.Pair<>(info.getId(), now)).toList());
