...
import io.zonarosa.messenger.avatar.Avatars.getForegroundColor
import io.zonarosa.messenger.avatar.Avatars.getTextSizeForLength
import io.zonarosa.messenger.avatar.picker.AvatarPickerFragment
import io.zonarosa.messenger.badges.models.Badge
import io.zonarosa.messenger.badges.self.none.BecomeASustainerFragment.Companion.show
import io.zonarosa.messenger.components.emoji.EmojiUtil
import io.zonarosa.messenger.databinding.EditProfileFragmentBinding
import io.zonarosa.messenger.keyvalue.AccountValues
import io.zonarosa.messenger.keyvalue.ZonaRosaStore
import io.zonarosa.messenger.profiles.ProfileName
import io.zonarosa.messenger.profiles.manage.EditProfileViewModel.AvatarState
...
import io.zonarosa.messenger.util.NameUtil.getAbbreviation
...
class EditProfileFragment : LoggingFragment() {
  ...
  override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
    ...
    binding.manageProfileUsernameContainer.setOnClickListener { v: View ->
      if (!viewModel.isRegisteredAndUpToDate) {
        onClickWhenUnregisteredOrDeprecated()
      } else if (ZonaRosaStore.account.username != null) {
        MaterialAlertDialogBuilder(requireContext(), R.style.ThemeOverlay_ZonaRosa_MaterialAlertDialog_List)
  ...
  private fun presentUsername(username: String?) {
    ...
    if (ZonaRosaStore.account.usernameSyncState == AccountValues.UsernameSyncState.USERNAME_AND_LINK_CORRUPTED) {
      binding.usernameErrorIndicator.visibility = View.VISIBLE
    } else {
      binding.usernameErrorIndicator.visibility = View.GONE
    }

    if (ZonaRosaStore.account.username != null && ZonaRosaStore.account.usernameSyncState != AccountValues.UsernameSyncState.USERNAME_AND_LINK_CORRUPTED) {
      ...
      if (ZonaRosaStore.tooltips.showProfileSettingsQrCodeTooltop()) {
        ...
        binding.linkTooltipCloseButton.setOnClickListener {
          ...
          ZonaRosaStore.tooltips.markProfileSettingsQrCodeTooltipSeen()
        }
      }
  ...
  private fun displayConfirmUsernameDeletionDialog() {
    MaterialAlertDialogBuilder(requireContext())
      ...
      .setMessage(requireContext().getString(R.string.ManageProfileFragment__delete_username_dialog_body, ZonaRosaStore.account.username))
