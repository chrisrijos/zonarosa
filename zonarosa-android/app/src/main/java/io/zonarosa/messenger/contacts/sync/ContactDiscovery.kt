...
import io.zonarosa.contacts.SystemContactsRepository.ContactIterator
import io.zonarosa.contacts.SystemContactsRepository.ContactPhoneDetails
...
import io.zonarosa.messenger.jobs.SyncSystemContactLinksJob
...
object ContactDiscovery {
  ...
  fun refreshAll(context: Context, notifyOfNewUsers: Boolean) {
    if (TextUtils.isEmpty(ZonaRosaStore.account.e164)) {
    ...
    if (!ZonaRosaStore.registration.isRegistrationComplete) {
      if (ZonaRosaStore.account.isRegistered && ZonaRosaStore.svr.lastPinCreateFailed()) {
  ...
  private fun phoneNumberFormatter(): (String) -> String? {
    return { ZonaRosaE164Util.formatNonShortCodeAsE164(it) }
  }
  ...
  private fun refreshRecipients(
    ...
    val preExistingRegisteredIds: Set<RecipientId> = ZonaRosaDatabase.recipients.getRegistered()
    ...
    if (hasContactsPermissions(context)) {
      ...
      if (ZonaRosaPreferences.hasSuccessfullyRetrievedDirectory(context) && notifyOfNewUsers) {
        val systemContacts: Set<RecipientId> = ZonaRosaDatabase.recipients.getSystemContacts().toSet()
  ...
  private fun notifyNewUsers(context: Context, newUserIds: Collection<RecipientId>) {
    if (!ZonaRosaStore.settings.isNotifyWhenContactJoinsZonaRosa) return
    ...
    Recipient.resolvedList(newUserIds)
      ...
      .map {
        ...
        ZonaRosaDatabase.messages.insertMessageInbox(message)
      }
      .filter { it.isPresent }
      .map { it.get() }
      .forEach { result ->
  ...
  private fun syncRecipientsWithSystemContacts(
    ...
    val localNumber: String = ZonaRosaStore.account.e164 ?: ""
    ...
    if (contactInfos.isNotEmpty()) {
      val handle = ZonaRosaDatabase.recipients.beginBulkSystemContactUpdate(clearInfoForMissingContacts)
      try {
    ...
    if (NotificationChannels.supported()) {
      ZonaRosaDatabase.recipients.getRecipientsWithNotificationChannels().use { reader ->
  ...
  private fun hasSession(id: RecipientId): Boolean {
    ...
    val protocolAddress = Recipient.resolved(id).requireServiceId().toProtocolAddress(ZonaRosaServiceAddress.DEFAULT_DEVICE_ID)
