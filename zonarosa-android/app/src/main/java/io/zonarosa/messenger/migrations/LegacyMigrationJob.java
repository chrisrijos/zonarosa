...
import io.zonarosa.messenger.database.MessageTable.MmsReader;
...
public class LegacyMigrationJob extends MigrationJob {
  ...
  private static final int ZONAROSAING_KEY_DEPRECATION            = 447;
  ...
  void performMigration() throws RetryLaterException {
    ...
    if (lastSeenVersion < SQLCIPHER && masterSecret != null) {
      ZonaRosaDatabase.onApplicationLevelUpgrade(context, masterSecret, lastSeenVersion, (progress, total) -> {
...
////        new ZonaRosaSessionStore(context, masterSecret).migrateSessions();
////        new ZonaRosaPreKeyStore(context, masterSecret).migrateRecords();
//
//      scheduleMessagesInPushDatabase(context);;
//    }
    ...
    if (lastSeenVersion < SCREENSHOTS) {
      boolean screenSecurity = PreferenceManager.getDefaultSharedPreferences(context).getBoolean(ZonaRosaPreferences.SCREEN_SECURITY_PREF, true);
      ZonaRosaPreferences.setScreenSecurityEnabled(context, screenSecurity);
    }
    ...
    if (lastSeenVersion < INTERNALIZE_CONTACTS) {
      if (ZonaRosaStore.account().isRegistered()) {
        ZonaRosaPreferences.setHasSuccessfullyRetrievedDirectory(context, true);
      }
    }
...
//        PersistentStorage storage    = new PersistentStorage(getApplicationContext(), "ZonaRosaJobs", new JavaJobSerializer());
//
...
//      }
    ...
    if (lastSeenVersion < COLOR_MIGRATION) {
      ...
      ZonaRosaDatabase.recipients().updateSystemContactColors();
    ...
    if (lastSeenVersion < ZONAROSAING_KEY_DEPRECATION) {
      Log.i(TAG, "Scheduling a RefreshAttributesJob to remove the zonarosaing key remotely.");
      AppDependencies.getJobManager().add(new RefreshAttributesJob());
    }
  }
  ...
  private void schedulePendingIncomingParts(Context context) {
    final AttachmentTable          attachmentDb       = ZonaRosaDatabase.attachments();
    final MessageTable             mmsDb              = ZonaRosaDatabase.messages();
    final List<DatabaseAttachment> pendingAttachments = ZonaRosaDatabase.attachments().getPendingAttachments();
