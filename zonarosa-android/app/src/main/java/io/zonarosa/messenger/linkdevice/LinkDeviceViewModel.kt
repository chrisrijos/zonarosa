...
import io.zonarosa.messenger.jobs.NewLinkedDeviceNotificationJob
import io.zonarosa.messenger.keyvalue.ZonaRosaStore
import io.zonarosa.messenger.linkdevice.LinkDeviceRepository.LinkDeviceResult
import io.zonarosa.messenger.linkdevice.LinkDeviceRepository.getPlaintextDevice
import io.zonarosa.messenger.linkdevice.LinkDeviceSettingsState.DialogState
import io.zonarosa.messenger.linkdevice.LinkDeviceSettingsState.OneTimeEvent
import io.zonarosa.messenger.linkdevice.LinkDeviceSettingsState.QrCodeState
...
class LinkDeviceViewModel : ViewModel() {
  ...
  private fun checkForNewDevice(devices: List<Device>) {
    val newLinkedDeviceId = ZonaRosaStore.misc.newLinkedDeviceId
    val newLinkedDeviceCreatedAt = ZonaRosaStore.misc.newLinkedDeviceCreatedTime
    ...
    if (hasNewLinkedDevice) {
      ...
      ZonaRosaStore.misc.newLinkedDeviceId = 0
      ZonaRosaStore.misc.newLinkedDeviceCreatedTime = 0
    }
  ...
  fun markQrEducationSheetSeen() {
    ZonaRosaStore.uiHints.markHasSeenLinkDeviceQrEducationSheet()
    _state.update {
      it.copy(
  ...
  private fun addDeviceWithSync(linkUri: Uri) {
    ...
    val uploadResult = LinkDeviceRepository.createAndUploadArchive(
      ...
      cancellationZonaRosa = { _state.value.shouldCancelArchiveUpload }
    )
