...
object RecipientCreator {
  ...
  fun forIndividual(context: Context, record: RecipientRecord): Recipient {
    val isSelf = record.e164 != null && record.e164 == ZonaRosaStore.account.e164 || record.aci != null && record.aci == ZonaRosaStore.account.aci
    val isReleaseChannel = record.id == ZonaRosaStore.releaseChannel.releaseChannelRecipientId
    var registeredState = record.registered

    if (isSelf) {
      registeredState = if (ZonaRosaStore.account.isRegistered && !ZonaRosaPreferences.isUnauthorizedReceived(context)) {
  ...
  private fun getGroupRecipientDetails(record: RecipientRecord): Recipient {
    val groupRecord = ZonaRosaDatabase.groups.getGroup(record.id)
  ...
  private fun getDistributionListRecipientDetails(record: RecipientRecord): Recipient {
    val groupRecord = ZonaRosaDatabase.distributionLists.getList(record.distributionListId!!)
  ...
  private fun getCallLinkRecipientDetails(record: RecipientRecord): Recipient {
    val callLink = ZonaRosaDatabase.callLinks.getCallLinkByRoomId(record.callLinkRoomId!!)
