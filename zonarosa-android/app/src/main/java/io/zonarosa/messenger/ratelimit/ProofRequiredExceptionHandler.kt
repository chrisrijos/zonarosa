...
import io.zonarosa.messenger.jobs.SubmitRateLimitPushChallengeJob.SuccessEvent
...
object ProofRequiredExceptionHandler {
  ...
  fun handle(context: Context, proofRequired: ProofRequiredException, recipient: Recipient?, threadId: Long, messageId: Long): Result {
    ...
    if (ProofRequiredException.Option.PUSH_CHALLENGE in proofRequired.options) {
      when (val result = ZonaRosaNetwork.rateLimitChallenge.requestPushChallenge()) {
    ...
    if (messageId > 0) {
      ...
      ZonaRosaDatabase.messages.markAsRateLimited(messageId)
    }

    if (ProofRequiredException.Option.CAPTCHA in proofRequired.options) {
      Log.i(TAG, "[Proof Required] CAPTCHA required.")
      ZonaRosaStore.rateLimit.markNeedsRecaptcha(proofRequired.token)

      if (recipient != null && messageId > -1L) {
        val groupReply: ParentStoryId.GroupReply? = ZonaRosaDatabase.messages.getParentStoryIdForGroupReply(messageId)
