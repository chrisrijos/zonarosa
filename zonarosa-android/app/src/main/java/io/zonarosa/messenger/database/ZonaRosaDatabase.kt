...
import io.zonarosa.messenger.crypto.MasterSecret
import io.zonarosa.messenger.database.helpers.ClassicOpenHelper
import io.zonarosa.messenger.database.helpers.PreKeyMigrationHelper
import io.zonarosa.messenger.database.helpers.SQLCipherMigrationHelper
import io.zonarosa.messenger.database.helpers.SessionStoreMigrationHelper
...
import io.zonarosa.messenger.migrations.LegacyMigrationJob.DatabaseUpgradeListener
...
open class ZonaRosaDatabase(private val context: Application, databaseSecret: DatabaseSecret, attachmentSecret: AttachmentSecret, name: String = DATABASE_NAME) :
  SQLiteOpenHelper(
    context,
    name,
    databaseSecret.asString(),
    null,
    ZonaRosaDatabaseMigrations.DATABASE_VERSION,
    0,
    SqlCipherErrorHandler(context, name),
    SqlCipherDatabaseHook(),
    true
  ),
  ZonaRosaDatabaseOpenHelper {
  ...
  override fun onCreate(db: net.zetetic.database.sqlcipher.SQLiteDatabase) {
    val zonarosaDb = ZonaRosaSQLiteDatabase(db)
    onCreateTablesIndexesAndTriggers(zonarosaDb)

    // Requires FTS5
    executeStatements(zonarosaDb, SearchTable.CREATE_TABLE)
    executeStatements(zonarosaDb, SearchTable.CREATE_TRIGGERS)
    ...
    if (context.getDatabasePath(ClassicOpenHelper.NAME).exists()) {
      ...
      if (masterSecret != null) SQLCipherMigrationHelper.migrateCiphertext(context, masterSecret, legacyDb, db, null) else ZonaRosaPreferences.setNeedsSqlCipherMigration(context, true)
  ...
  @VisibleForTesting
  fun onCreateTablesIndexesAndTriggers(db: ZonaRosaSQLiteDatabase) {
  ...
  override fun onUpgrade(db: net.zetetic.database.sqlcipher.SQLiteDatabase, oldVersion: Int, newVersion: Int) {
    ...
    try {
      ...
      ZonaRosaDatabaseMigrations.migrate(context, ZonaRosaSQLiteDatabase(db), oldVersion, newVersion)
    } finally {
      db.setForeignKeyConstraintsEnabled(true)
    ...
    ZonaRosaDatabaseMigrations.migratePostTransaction(context, oldVersion)
    Log.i(TAG, "Upgrade complete. Took " + (System.currentTimeMillis() - startTime) + " ms.")
  }

  override val readableDatabase: net.zetetic.database.sqlcipher.SQLiteDatabase
    get() = throw UnsupportedOperationException("Call getZonaRosaReadableDatabase() instead!")

  override val writableDatabase: net.zetetic.database.sqlcipher.SQLiteDatabase
    get() = throw UnsupportedOperationException("Call getZonaRosaWritableDatabase() instead!")
  ...
  open val zonarosaReadableDatabase: ZonaRosaSQLiteDatabase
    get() = ZonaRosaSQLiteDatabase(super.readableDatabase)

  open val zonarosaWritableDatabase: ZonaRosaSQLiteDatabase
    get() = ZonaRosaSQLiteDatabase(super.writableDatabase)
  ...
  open fun markCurrent(db: net.zetetic.database.sqlcipher.SQLiteDatabase) {
    db.version = ZonaRosaDatabaseMigrations.DATABASE_VERSION
  }
  ...
  companion object {
    private val TAG = Log.tag(ZonaRosaDatabase::class.java)
    const val DATABASE_NAME = "zonarosa.db"

    @JvmStatic
    @Volatile
    var instance: ZonaRosaDatabase? = null
      private set

    @JvmStatic
    fun init(application: Application, databaseSecret: DatabaseSecret, attachmentSecret: AttachmentSecret) {
      if (instance == null) {
        synchronized(ZonaRosaDatabase::class.java) {
          if (instance == null) {
            instance = ZonaRosaDatabase(application, databaseSecret, attachmentSecret)
          }
        }
      }
    }
    ...
    @JvmStatic
    @VisibleForTesting
    fun setZonaRosaDatabaseInstanceForTesting(zonarosaDatabase: ZonaRosaDatabase) {
      this.instance = zonarosaDatabase
    }
    ...
    @JvmStatic
    val readableDatabase: ZonaRosaSQLiteDatabase
      get() = instance!!.zonarosaReadableDatabase

    @JvmStatic
    val writableDatabase: ZonaRosaSQLiteDatabase
      get() = instance!!.zonarosaWritableDatabase
    ...
    fun runPostSuccessfulTransaction(dedupeKey: String, task: Runnable) {
      instance!!.zonarosaWritableDatabase.runPostSuccessfulTransaction(dedupeKey, task)
    }

    @JvmStatic
    fun runPostSuccessfulTransaction(task: Runnable) {
      instance!!.zonarosaWritableDatabase.runPostSuccessfulTransaction(task)
    }
    ...
    fun triggerDatabaseAccess() {
      instance!!.zonarosaWritableDatabase
    }
    ...
    fun onApplicationLevelUpgrade(
      ...
      if (fromVersion < LegacyMigrationJob.SQLCIPHER && ZonaRosaPreferences.getNeedsSqlCipherMigration(context)) {
    ...
    @JvmStatic
    fun <T> runInTransaction(block: (ZonaRosaSQLiteDatabase) -> T): T {
      return instance!!.zonarosaWritableDatabase.withinTransaction {
        block(it)
      }
    }
