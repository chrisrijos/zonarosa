...
import io.zonarosa.messenger.database.ThreadTable.Companion.ID
 ...
 */
class ChatFolderTables(context: Context?, databaseHelper: ZonaRosaDatabase?) : DatabaseTable(context, databaseHelper), ThreadIdDatabaseReference {
  ...
  fun getUnreadCountAndEmptyAndMutedStatusForFolders(folders: List<ChatFolderRecord>): HashMap<Long, Triple<Int, Boolean, Boolean>> {
    ...
    folders.map { folder ->
      val unreadCount = ZonaRosaDatabase.threads.getUnreadCountByChatFolder(folder)
      val isEmpty = !ZonaRosaDatabase.threads.hasChatInFolder(folder)
      val isMuted = !ZonaRosaDatabase.threads.hasUnmutedChatsInFolder(folder)
   ...
   */
  fun insertChatFolderFromStorageSync(record: ZonaRosaChatFolderRecord) {
   ...
   */
  fun updateChatFolderFromStorageSync(record: ZonaRosaChatFolderRecord) {
  ...
  /**
   * Parses a remote chat folder [ZonaRosaChatFolderRecord] into a local folder [ChatFolderRecord]
   */
  private fun remoteChatFolderRecordToLocal(record: ZonaRosaChatFolderRecord): ChatFolderRecord {
    ...
    return ChatFolderRecord(
      ...
      includedChats = record.proto.includedRecipients
        ...
        .map { recipient -> ZonaRosaDatabase.threads.getOrCreateThreadIdFor(recipient) },
