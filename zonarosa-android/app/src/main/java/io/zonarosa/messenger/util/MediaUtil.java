...
import io.zonarosa.core.util.ContentTypeUtil;
...
import io.zonarosa.messenger.mms.MmsSlide;
...
import io.zonarosa.messenger.mms.ViewOnceSlide;
    ...
    try {
      ...
      try {
        if (BlobProvider.isAuthority(uri) && MediaUtil.isVideo(BlobProvider.getMimeType(uri))) {
          ...
          MediaDataSource source = ZonaRosaDatabase.attachments().mediaDataSourceFor(PartAuthority.requireAttachmentId(uri), false);
          if (source == null) {
  ...
  public static @Nullable Bitmap getVideoThumbnail(@NonNull Context context, @Nullable Uri uri, long timeUs) {
               ...
               MediaUtil.isVideoType(PartAuthority.getAttachmentContentType(context, uri)))
    {
      try {
        ...
        MediaDataSource source       = ZonaRosaDatabase.attachments().mediaDataSourceFor(attachmentId, false);
        return extractFrame(source, timeUs);
      } catch (IOException e) {
        Log.w(TAG, "Failed to extract frame for URI: " + uri, e);
      }
