...
class OneTimePreKeyTable(context: Context, databaseHelper: ZonaRosaDatabase) : DatabaseTable(context, databaseHelper) {
  companion object {
  ...
  fun delete(serviceId: ServiceId, keyId: Int) {
    val database = databaseHelper.zonarosaWritableDatabase
