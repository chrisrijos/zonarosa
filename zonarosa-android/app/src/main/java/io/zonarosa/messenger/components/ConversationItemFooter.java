...
import io.zonarosa.messenger.util.dualsim.SubscriptionInfoCompat;
import io.zonarosa.messenger.util.dualsim.SubscriptionManagerCompat;
  ...
  private void init(@Nullable AttributeSet attrs) {
    ...
    if (typedArray != null) {
      ...
      if (isOutgoing) {
        ...
        playbackSpeedToggleTextView.setTextColor(getResources().getColor(R.color.zonarosa_text_secondary));
    ...
    if (messageRecord.getExpiresIn() > 0 && !messageRecord.isPending()) {
      ...
      if (messageRecord.getExpireStarted() > 0) {
        ...
        ZonaRosaExecutors.BOUNDED.execute(() -> {
          ...
          ZonaRosaDatabase.messages().markExpireStarted(id, now);
  ...
  private void presentDeliveryStatus(@NonNull MessageRecord messageRecord) {
    ...
    if (previousMessageId == newMessageId && deliveryStatusView.isPending() && !messageRecord.isPending()) {
      if (messageRecord.getToRecipient().isGroup()) {
        ZonaRosaLocalMetrics.GroupMessageSend.onUiUpdated(messageRecord.getId());
      } else {
        ZonaRosaLocalMetrics.IndividualMessageSend.onUiUpdated(messageRecord.getId());
      }
    }
