...
object Stories {
  ...
  fun isFeatureEnabled(): Boolean {
    return !ZonaRosaStore.story.isFeatureDisabled
  }
  ...
  fun getRecipientsToSendTo(messageId: Long, sentTimestamp: Long, allowsReplies: Boolean): SendData {
    val recipientIds: List<RecipientId> = ZonaRosaDatabase.storySends.getRecipientsToSendTo(messageId, sentTimestamp, allowsReplies)
  ...
  fun onStorySettingsChanged(distributionListId: DistributionListId) {
    val recipientId = ZonaRosaDatabase.distributionLists.getRecipientId(distributionListId) ?: error("Cannot find recipient id for distribution list.")
    onStorySettingsChanged(recipientId)
  }

  @WorkerThread
  fun onStorySettingsChanged(storyRecipientId: RecipientId) {
    ZonaRosaDatabase.recipients.markNeedsSync(storyRecipientId)
  ...
  fun enqueueNextStoriesForDownload(recipientId: RecipientId, force: Boolean = false, limit: Int) {
    ...
    ZonaRosaDatabase.messages.getUnreadStories(recipientId, limit).use { reader ->
      reader.forEach {
  ...
  fun enqueueAttachmentsFromStoryForDownloadSync(record: MmsMessageRecord, ignoreAutoDownloadConstraints: Boolean) {
    ZonaRosaDatabase.attachments.getAttachmentsForMessage(record.id).filterNot { it.isSticker }.forEach {
