...
import io.zonarosa.messenger.database.model.addLink
...
class RetrieveRemoteAnnouncementsJob private constructor(private val force: Boolean, parameters: Parameters) : BaseJob(parameters) {
  ...
  companion object {
    ...
    fun enqueue(force: Boolean = false) {
      ...
      if (!force && System.currentTimeMillis() < ZonaRosaStore.releaseChannel.nextScheduledCheck) {
  ...
  override fun onRun() {
    ...
    val values = ZonaRosaStore.releaseChannel
  ...
  private fun updateReleaseNotes(announcements: List<ReleaseNote>) {
    ...
    if (!values.hasMetConversationRequirement) {
      if ((ZonaRosaDatabase.threads.getArchivedConversationListCount(ConversationFilter.OFF) + ZonaRosaDatabase.threads.getUnarchivedConversationListCount(ConversationFilter.OFF)) < 6) {
    ...
    val threadId = ZonaRosaDatabase.threads.getOrCreateThreadIdFor(Recipient.resolved(values.releaseChannelRecipientId!!))
    ...
    if (addedNewNotes) {
      ThreadUtil.sleep(5)
      ZonaRosaDatabase.messages.insertBoostRequestMessage(values.releaseChannelRecipientId!!, threadId)
    }
  ...
  private fun updateMegaphones(megaphones: List<RemoteMegaphone>) {
    ...
    val existingMegaphones: Map<String, RemoteMegaphoneRecord> = ZonaRosaDatabase.remoteMegaphones.getAll().associateBy { it.uuid }
    ...
    resolvedMegaphones
      ...
      .forEach { megaphone ->
        ...
        if (existingMegaphones.contains(uuid)) {
          ZonaRosaDatabase.remoteMegaphones.update(
            uuid = uuid,
    ...
    if (megaphonesToDelete.isNotEmpty()) {
      ...
      for ((uuid, megaphone) in megaphonesToDelete) {
        ZonaRosaDatabase.remoteMegaphones.clear(uuid)
  ...
  private fun String.getLocaleUrls(): List<String> {
    ...
    if (ZonaRosaStore.settings.language != "zz") {
      potentialNoteUrls += "$this/${ZonaRosaStore.settings.language}.json"
    }
