...
import io.zonarosa.messenger.audio.AudioDeviceUpdatedListener
import io.zonarosa.messenger.audio.ZonaRosaBluetoothManager
...
private val TAG = Log.tag(ZonaRosaAudioManager::class.java)

sealed class ZonaRosaAudioManager(protected val context: Context, protected val eventListener: EventListener?) {

  private var commandAndControlThread = ZonaRosaExecutors.getAndStartHandlerThread("call-audio", ThreadUtil.PRIORITY_IMPORTANT_BACKGROUND_THREAD)
  protected val handler = ZonaRosaAudioHandler(commandAndControlThread.looper)
  ...
  companion object {
    @JvmStatic
    fun create(context: Context, eventListener: EventListener?): ZonaRosaAudioManager {
      return if (Build.VERSION.SDK_INT >= 31) {
        FullZonaRosaAudioManagerApi31(context, eventListener)
      } else {
        FullZonaRosaAudioManager(context, eventListener)
      }
    }
  }
 ...
 */
class FullZonaRosaAudioManager(context: Context, eventListener: EventListener?) : ZonaRosaAudioManager(context, eventListener), AudioDeviceUpdatedListener {
  private val zonarosaBluetoothManager = ZonaRosaBluetoothManager(context, this, handler)
  ...
  private var previousBluetoothState: ZonaRosaBluetoothManager.State? = null
  ...
  override fun onAudioDeviceUpdated() {
    ...
    Log.i(
      ...
      "updateAudioDeviceState(): " +
        ...
        "bt: ${zonarosaBluetoothManager.state} " +
    ...
    if (zonarosaBluetoothManager.state.hasDevice()) {
    ...
    if (zonarosaBluetoothManager.state == ZonaRosaBluetoothManager.State.UNAVAILABLE && userSelectedAudioDevice == AudioDevice.BLUETOOTH) {
    ...
    val needBluetoothAudioStart = zonarosaBluetoothManager.state == ZonaRosaBluetoothManager.State.AVAILABLE &&
    ...
    val needBluetoothAudioStop = (zonarosaBluetoothManager.state == ZonaRosaBluetoothManager.State.CONNECTED || zonarosaBluetoothManager.state == ZonaRosaBluetoothManager.State.CONNECTING) &&
      (userSelectedAudioDevice != AudioDevice.NONE && userSelectedAudioDevice != AudioDevice.BLUETOOTH)

    if (zonarosaBluetoothManager.state.hasDevice()) {
      Log.i(TAG, "Need bluetooth audio: state: ${zonarosaBluetoothManager.state} start: $needBluetoothAudioStart stop: $needBluetoothAudioStop")
    }
    ...
    if (!autoSwitchToBluetooth && zonarosaBluetoothManager.state == ZonaRosaBluetoothManager.State.UNAVAILABLE) {
      autoSwitchToBluetooth = true
    }

    if (needBluetoothAudioStart && !needBluetoothAudioStop) {
      if (!zonarosaBluetoothManager.startScoAudio()) {
        audioDevices.remove(AudioDevice.BLUETOOTH)
        audioDeviceSetUpdated = true
      }
    }

    if (autoSwitchToBluetooth && zonarosaBluetoothManager.state == ZonaRosaBluetoothManager.State.CONNECTED) {
      userSelectedAudioDevice = AudioDevice.BLUETOOTH
      autoSwitchToBluetooth = false
    }

    if (previousBluetoothState != null && previousBluetoothState != ZonaRosaBluetoothManager.State.PERMISSION_DENIED && zonarosaBluetoothManager.state == ZonaRosaBluetoothManager.State.PERMISSION_DENIED) {
      eventListener?.onBluetoothPermissionDenied()
    }
    previousBluetoothState = zonarosaBluetoothManager.state
