...
import io.zonarosa.core.ui.compose.Fragments
...
import io.zonarosa.messenger.conversation.RecipientSearchBar
...
import io.zonarosa.messenger.recipients.ui.RecipientPicker.DisplayMode.Companion.flag
import io.zonarosa.messenger.recipients.ui.RecipientPicker.KeyboardType
...
private fun ContactSelectionListFragment.setUpCallbacks(
  ...
  if (callbacks.newConversation != null) {
    fragment.setNewConversationCallback(object : ContactSelectionListFragment.NewConversationCallback {
      override fun onInvite() = callbacks.newConversation.onInviteToZonaRosa()
  ...
  if (callbacks.newCall != null) {
    fragment.setNewCallCallback { callbacks.newCall.onInviteToZonaRosa() }
  } else {
...
private suspend fun showItemContextMenu(
  ...
  val actions = buildList {
    ...
    if (!recipient.isSelf) {
      val blockItem = ActionItem(
        ...
        tintRes = CoreUiR.color.zonarosa_colorError,
  ...
  ZonaRosaContextMenu.Builder(anchorView, anchorView.getRootView() as ViewGroup)
...
object RecipientPicker {
  /**
   * Enum wrapper for [io.zonarosa.messenger.contacts.ContactSelectionDisplayMode].
   */
