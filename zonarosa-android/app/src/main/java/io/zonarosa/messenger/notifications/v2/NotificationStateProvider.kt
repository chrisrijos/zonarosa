...
object NotificationStateProvider {
  ...
  fun constructNotificationState(stickyThreads: Map<ConversationId, DefaultMessageNotifier.StickyThread>, notificationProfile: NotificationProfile?): NotificationState {
    ...
    ZonaRosaDatabase.messages.getMessagesForNotificationState(stickyThreads.values).use { unreadMessages ->
      ...
      MessageTable.mmsReaderFor(unreadMessages).use { reader ->
        ...
        while (record != null) {
          val threadRecipient: Recipient? = ZonaRosaDatabase.threads.getRecipientForThreadId(record.threadId)
          if (threadRecipient != null) {
            ...
            val parentRecord = conversationId.groupStoryId?.let {
              try {
                ZonaRosaDatabase.messages.getMessageRecord(it)
            ...
            val hasSelfRepliedToGroupStory = conversationId.groupStoryId?.let {
              ZonaRosaDatabase.messages.hasGroupReplyOrReactionInStory(it)
            }

            if (record is MmsMessageRecord) {
              val attachments = ZonaRosaDatabase.attachments.getAttachmentsForMessage(record.id)
              if (attachments.isNotEmpty()) {
                record = record.withAttachments(attachments)
              }
              val poll = ZonaRosaDatabase.polls.getPoll(record.id)
              if (poll != null) {
            ...
            messages += NotificationMessage(
              ...
              reactions = if (hasUnreadReactions) ZonaRosaDatabase.reactions.getReactions(MessageId(record.id)) else emptyList(),
              pollVotes = if (hasUnreadVotes) ZonaRosaDatabase.polls.getAllVotes(record.id) else emptyList(),
