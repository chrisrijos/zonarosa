...
data class SentStorySyncManifest(
  ...
  fun toRecipientsSet(): Set<ZonaRosaServiceStoryMessageRecipient> {
    ...
    return recipients
      ...
      .map { recipient ->
        ...
        ZonaRosaServiceStoryMessageRecipient(
          ZonaRosaServiceAddress(serviceId),
  ...
  companion object {
    @WorkerThread
    @JvmStatic
    fun fromRecipientsSet(recipientsSet: Set<ZonaRosaServiceStoryMessageRecipient>): SentStorySyncManifest {
      val entries = recipientsSet.map { recipient ->
        Entry(
          recipientId = RecipientId.from(recipient.zonarosaServiceAddress),
