...
import io.zonarosa.core.util.NoExternalStorageException;
import io.zonarosa.core.util.ThreadUtil;
import io.zonarosa.core.util.logging.Log;
import io.zonarosa.messenger.R;
import io.zonarosa.messenger.backup.BackupDialog;
import io.zonarosa.messenger.backup.BackupEvent;
import io.zonarosa.messenger.dependencies.AppDependencies;
import io.zonarosa.messenger.jobs.LocalBackupJob;
import io.zonarosa.messenger.keyvalue.ZonaRosaStore;
import io.zonarosa.messenger.preferences.widgets.UpgradeLocalBackupCard;
...
public class BackupsPreferenceFragment extends Fragment {
  ...
  private void setBackupFolderName() {
    ...
    if (BackupUtil.canUserAccessBackupDirectory(requireContext())) {
      ...
      {
        Uri backupUri = Objects.requireNonNull(ZonaRosaStore.settings().getZonaRosaBackupDirectory());

        folder.setVisibility(View.VISIBLE);
        folderName.setText(StorageUtil.getDisplayPath(requireContext(), backupUri));
      } else if (StorageUtil.canWriteInZonaRosaStorageDir()) {
        try {
          folder.setVisibility(View.VISIBLE);
  ...
  private void setUpdateState() {
    if (ZonaRosaStore.settings().isBackupEnabled() && Environment.Backups.isNewFormatSupportedForLocalBackup()) {
  ...
  private void onToggleClickedLegacy() {
    Permissions.with(this)
               ...
               .withPermanentDenialDialog(getString(R.string.BackupsPreferenceFragment_zonarosa_requires_external_storage_permission_in_order_to_create_backups))
               .execute();
  }
  ...
  private void pickTime() {
    ...
    final MaterialTimePicker timePickerFragment = new MaterialTimePicker.Builder()
        ...
        .setHour(ZonaRosaStore.settings().getBackupHour())
        .setMinute(ZonaRosaStore.settings().getBackupMinute())
    ...
    timePickerFragment.addOnPositiveButtonClickListener(v -> {
      ...
      ZonaRosaStore.settings().setBackupSchedule(hour, minute);
      updateTimeLabel();
      ZonaRosaPreferences.setNextBackupTime(requireContext(), 0);
  ...
  private void updateTimeLabel() {
    final int backupHour   = ZonaRosaStore.settings().getBackupHour();
    final int backupMinute = ZonaRosaStore.settings().getBackupMinute();
  ...
  private void updateToggle() {
    boolean userUnregistered          = ZonaRosaPreferences.isUnauthorizedReceived(AppDependencies.getApplication()) || !ZonaRosaStore.account().isRegistered();
    boolean clientDeprecated          = ZonaRosaStore.misc().isClientDeprecated();
    boolean legacyLocalBackupsEnabled = ZonaRosaStore.settings().isBackupEnabled() && BackupUtil.canUserAccessBackupDirectory(AppDependencies.getApplication());
