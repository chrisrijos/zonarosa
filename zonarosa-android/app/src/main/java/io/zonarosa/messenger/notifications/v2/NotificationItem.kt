...
import io.zonarosa.messenger.database.ThreadBodyUtil
...
import io.zonarosa.messenger.polls.PollVote
...
import io.zonarosa.messenger.util.hasPoll
import io.zonarosa.messenger.util.hasPollTerminate
import io.zonarosa.messenger.util.hasSharedContact
import io.zonarosa.messenger.util.hasSticker
import io.zonarosa.messenger.util.isMediaMessage
...
sealed class NotificationItem(val threadRecipient: Recipient, protected val record: MessageRecord) : Comparable<NotificationItem> {
  ...
  fun getStyledPrimaryText(context: Context, trimmed: Boolean = false): CharSequence {
    return if (ZonaRosaStore.settings.messageNotificationsPrivacy.isDisplayNothing) {
  ...
  fun getPersonUri(context: Context): String? {
    return if (ZonaRosaStore.settings.messageNotificationsPrivacy.isDisplayContact && authorRecipient.isSystemContact && Permissions.hasAny(context, Manifest.permission.READ_CONTACTS, Manifest.permission.WRITE_CONTACTS)) {
  ...
  fun getPrimaryText(context: Context): CharSequence {
    return if (ZonaRosaStore.settings.messageNotificationsPrivacy.isDisplayMessage) {
  ...
  fun getInboxLine(context: Context): CharSequence? {
    return when {
      ZonaRosaStore.settings.messageNotificationsPrivacy.isDisplayNothing -> null
...
class MessageNotification(threadRecipient: Recipient, record: MessageRecord) : NotificationItem(threadRecipient, record) {
  ...
  override fun getStartingPosition(context: Context): Int {
    return if (thread.groupStoryId != null) {
      ZonaRosaDatabase.messages.getMessagePositionInConversation(thread.threadId, thread.groupStoryId, record.dateReceived)
    } else {
      -1
    }
  }
  ...
  override fun getThumbnailInfo(context: Context): ThumbnailInfo {
    return if (ZonaRosaStore.settings.messageNotificationsPrivacy.isDisplayMessage && !KeyCachingService.isLocked(context)) {
...
class ReactionNotification(threadRecipient: Recipient, record: MessageRecord, val reaction: ReactionRecord) : NotificationItem(threadRecipient, record) {
  ...
  override fun getStartingPosition(context: Context): Int {
    return ZonaRosaDatabase.messages.getMessagePositionInConversation(thread.threadId, thread.groupStoryId ?: 0L, record.dateReceived)
  }
...
class VoteNotification(threadRecipient: Recipient, record: MessageRecord, val vote: PollVote) : NotificationItem(threadRecipient, record) {
  ...
  override fun getStartingPosition(context: Context): Int {
    return ZonaRosaDatabase.messages.getMessagePositionInConversation(threadId = thread.threadId, groupStoryId = 0L, receivedTimestamp = record.dateReceived)
  }
