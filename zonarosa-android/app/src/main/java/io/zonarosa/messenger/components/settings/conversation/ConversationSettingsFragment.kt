...
import io.zonarosa.core.util.getParcelableArrayListExtraCompat
...
import io.zonarosa.messenger.PushContactSelectionActivity
...
import io.zonarosa.messenger.components.recyclerview.OnScrollAnimationHelper
...
import io.zonarosa.messenger.components.settings.conversation.preferences.AvatarPreference
import io.zonarosa.messenger.components.settings.conversation.preferences.BioTextPreference
import io.zonarosa.messenger.components.settings.conversation.preferences.ButtonStripPreference
import io.zonarosa.messenger.components.settings.conversation.preferences.CallPreference
import io.zonarosa.messenger.components.settings.conversation.preferences.GroupDescriptionPreference
import io.zonarosa.messenger.components.settings.conversation.preferences.InternalPreference
import io.zonarosa.messenger.components.settings.conversation.preferences.LargeIconClickPreference
import io.zonarosa.messenger.components.settings.conversation.preferences.LegacyGroupPreference
import io.zonarosa.messenger.components.settings.conversation.preferences.RecipientPreference
import io.zonarosa.messenger.components.settings.conversation.preferences.SharedMediaPreference
import io.zonarosa.messenger.components.settings.conversation.preferences.Utils.formatMutedUntil
...
import io.zonarosa.messenger.groups.ui.GroupLimitDialog
import io.zonarosa.messenger.groups.ui.GroupMemberEntry
import io.zonarosa.messenger.groups.ui.LeaveGroupDialog
import io.zonarosa.messenger.groups.ui.addmembers.AddMembersActivity
import io.zonarosa.messenger.groups.ui.addtogroup.AddToGroupsActivity
...
import io.zonarosa.messenger.groups.ui.managegroup.dialogs.GroupInviteSentDialog
import io.zonarosa.messenger.groups.ui.managegroup.dialogs.GroupsLearnMoreBottomSheetDialogFragment
...
import io.zonarosa.messenger.stories.viewer.AddToGroupStoryDelegate
...
import io.zonarosa.messenger.util.ExpirationUtil
...
import io.zonarosa.messenger.wallpaper.ChatWallpaperActivity
...
class ConversationSettingsFragment :
  ...
  private val alertTint by lazy { ContextCompat.getColor(requireContext(), R.color.zonarosa_alert_primary) }
  private val alertDisabledTint by lazy { ContextCompat.getColor(requireContext(), R.color.zonarosa_alert_primary_50) }
  ...
  override fun getMaterial3OnScrollHelper(toolbar: Toolbar?): Material3OnScrollHelper {
    return object : Material3OnScrollHelper(activity = requireActivity(), views = listOf(toolbar!!), lifecycleOwner = viewLifecycleOwner) {
      override val inactiveColorSet = ColorSet(
        toolbarColorRes = CoreUiR.color.zonarosa_colorBackground_0,
        statusBarColorRes = CoreUiR.color.zonarosa_colorBackground
      )
    }
  }
    ...
    return configure {
          ...
          onAddToStoryClick = {
            if (state.recipient.isPushV2Group && state.requireGroupSettingsState().isAnnouncementGroup && !state.requireGroupSettingsState().isSelfAdmin) {
              ...
              Permissions.with(this@ConversationSettingsFragment)
                ...
                .onAnyDenied { Toast.makeText(requireContext(), R.string.CameraXFragment_zonarosa_needs_camera_access_capture_photos, Toast.LENGTH_LONG).show() }
                .execute()
            }
          },
          onVideoClick = {
      ...
      if (state.recipient.isReleaseNotes) {
        ...
        clickPref(
          ...
          title = DSLSettingsText.from(R.string.preferences__donate_to_zonarosa),
      ...
      if (state.canModifyBlockedState) {
        ...
        if (!state.recipient.isReleaseNotes) {
          val reportSpamTint = if (state.isDeprecatedOrUnregistered) R.color.zonarosa_alert_primary_50 else R.color.zonarosa_alert_primary
          clickPref(
