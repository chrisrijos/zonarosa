...
public class BackupUtil {
  ...
  public static boolean canUserAccessUnifiedBackupDirectory(@NonNull Context context) {
    if (isUserSelectionRequired(context)) {
      Uri backupDirectoryUri = Uri.parse(ZonaRosaStore.backup().getNewLocalBackupsDirectory());
      if (backupDirectoryUri == null) {
        return false;
      }
  ...
  public static void disableBackups(@NonNull Context context) {
    ...
    ZonaRosaStore.settings().setBackupEnabled(false);
    BackupUtil.deleteAllBackups();

    if (BackupUtil.isUserSelectionRequired(context)) {
      Uri backupLocationUri = ZonaRosaStore.settings().getZonaRosaBackupDirectory();

      if (backupLocationUri == null) {
        return;
      }

      ZonaRosaStore.settings().clearZonaRosaBackupDirectory();
