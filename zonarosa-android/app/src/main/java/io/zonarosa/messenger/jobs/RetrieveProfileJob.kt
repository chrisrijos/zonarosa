...
import io.zonarosa.messenger.database.RecipientTable.Companion.maskCapabilitiesToLong
...
import io.zonarosa.service.api.crypto.ProfileCipher
import io.zonarosa.service.api.profiles.ProfileRepository
import io.zonarosa.service.api.profiles.ProfileRepository.IdProfilePair
import io.zonarosa.service.api.profiles.ProfileRepository.ProfileFetchRequest
import io.zonarosa.service.api.profiles.ProfileRepository.ProfileFetchResult
import io.zonarosa.service.api.profiles.ProfileRepository.ZonaRosaServiceProfileWithCredential
...
class RetrieveProfileJob private constructor(parameters: Parameters, private val recipientIds: MutableSet<RecipientId>, private val skipDebounce: Boolean) : BaseJob(parameters) {
  ...
  public override fun onRun() {
    ...
    val response: ProfileFetchResult<RecipientId> = runBlocking {
      withContext(Dispatchers.IO) {
        ProfileRepository(ZonaRosaNetwork.profile).fetchProfiles(requests)
      }
    }
    stopwatch.split("responses")

    val localRecords = ZonaRosaDatabase.recipients.getExistingRecords(fetchingRecipientIds)
    ...
    ZonaRosaDatabase.recipients.markProfilesFetched(successIds, System.currentTimeMillis())
    ...
    if (newlyRegisteredIds.isNotEmpty()) {
      ...
      ZonaRosaDatabase.recipients.bulkUpdatedRegisteredStatus(newlyRegisteredIds, emptySet())
    }
    if (response.unregistered.isNotEmpty()) {
      ...
      for (recipientId in response.unregistered) {
        ZonaRosaDatabase.recipients.markUnregistered(recipientId)
      }
    }
    ...
    if (response.verificationFailures.isNotEmpty()) {
      ...
      for (recipientId in response.verificationFailures) {
        ZonaRosaDatabase.recipients.clearProfileKeyData(recipientId)
      }
    }
  ...
  private fun isUpdated(localRecipientRecord: RecipientRecord, remoteProfile: ZonaRosaServiceProfile, remoteExpiringProfileKeyCredential: ExpiringProfileKeyCredential?): Boolean {
    if (localRecipientRecord.zonarosaProfileAvatar != remoteProfile.avatar) {
      return true
    }
    ...
    if (localRecipientRecord.zonarosaProfileName != newProfileName) {
      return true
    }
  ...
  private fun process(recipient: Recipient, profileAndCredential: ZonaRosaServiceProfileWithCredential) {
  ...
  private fun setProfileBadges(recipient: Recipient, serviceBadges: List<ZonaRosaServiceProfile.Badge>?) {
    ...
    ZonaRosaDatabase.recipients.setBadges(recipient.id, badges)
  }
  ...
  private fun setExpiringProfileKeyCredential(
    ...
    ZonaRosaDatabase.recipients.setProfileKeyCredential(recipient.id, recipientProfileKey, credential)
  }
  ...
  private fun setUnidentifiedAccessMode(recipient: Recipient, unidentifiedAccessVerifier: String?, unrestrictedUnidentifiedAccess: Boolean) {
    ...
    if (recipient.sealedSenderAccessMode !== newMode) {
      ...
      ZonaRosaDatabase.recipients.setSealedSenderAccessMode(recipient.id, newMode)
    }
  }
  ...
  private fun setProfileName(recipient: Recipient, profileName: String?): Boolean {
    try {
      ...
      if (localProfileName.isEmpty &&
        ...
        val username = ZonaRosaDatabase.recipients.getUsername(recipient.id)
        val e164 = if (username == null) ZonaRosaDatabase.recipients.getE164sForIds(listOf(recipient.id)).firstOrNull() else null

        if (username != null || e164 != null) {
          Log.i(TAG, "Learned profile name for first time, inserting event")
          ZonaRosaDatabase.messages.insertLearnedProfileNameChangeMessage(recipient, e164, username)
        } else {
      ...
      if (remoteProfileName != localProfileName) {
        ...
        ZonaRosaDatabase.recipients.setProfileName(recipient.id, remoteProfileName)
        ...
        if (writeChangeEvent) {
          ...
          ZonaRosaDatabase.messages.insertProfileNameChangeMessages(recipient, remoteDisplayName, localDisplayName)
        } else {
        ...
        if (recipient.isIndividual &&
          ...
          val threadId = ZonaRosaDatabase.threads.getThreadIdFor(recipient.id)
          if (threadId != null && !RecipientUtil.isMessageRequestAccepted(threadId, recipient)) {
            ZonaRosaDatabase.nameCollisions.handleIndividualNameCollision(recipient.id)
          }
        }
  ...
  private fun setProfileAbout(recipient: Recipient, encryptedAbout: String?, encryptedEmoji: String?) {
    try {
      ...
      ZonaRosaDatabase.recipients.setAbout(recipient.id, plaintextAbout, plaintextEmoji)
    } catch (e: InvalidCiphertextException) {
      Log.w(TAG, e)
    } catch (e: IOException) {
      Log.w(TAG, e)
  ...
  private fun clearUsername(recipient: Recipient) {
    ZonaRosaDatabase.recipients.setUsername(recipient.id, null)
  }

  private fun setProfileCapabilities(recipient: Recipient, capabilities: ZonaRosaServiceProfile.Capabilities?) {
    if (capabilities == null) {
      return
    }

    ZonaRosaDatabase.recipients.setCapabilities(recipient.id, capabilities)
  }
  ...
  private fun setPhoneNumberSharingMode(recipient: Recipient, phoneNumberSharing: String?) {
    ...
    try {
      ...
      if (recipient.phoneNumberSharing !== remotePhoneNumberSharing) {
        ...
        ZonaRosaDatabase.recipients.setPhoneNumberSharing(recipient.id, remotePhoneNumberSharing)
      }
    } catch (e: InvalidCiphertextException) {
  ...
  private fun setProfileAvatar(recipient: Recipient, profileAvatar: String?) {
    ...
    if (profileAvatar != recipient.profileAvatar) {
      ZonaRosaDatabase.runPostSuccessfulTransaction(DEDUPE_KEY_RETRIEVE_AVATAR + recipient.id) {
  ...
  companion object {
    ...
    fun forRecipients(recipientIds: Set<RecipientId>, skipDebounce: Boolean = false): List<Job> {
      ...
      for (recipientId in recipientIds) {
        ...
        when {
          ...
          recipient.isGroup -> combined += ZonaRosaDatabase.groups.getGroupMemberIds(recipient.requireGroupId(), GroupTable.MemberSet.FULL_MEMBERS_EXCLUDING_SELF)
    ...
    fun enqueueRoutineFetchIfNecessary() {
      if (!ZonaRosaStore.registration.isRegistrationComplete || !ZonaRosaStore.account.isRegistered || ZonaRosaStore.account.aci == null) {
        Log.i(TAG, "Registration not complete. Skipping.")
        return
      }

      val timeSinceRefresh = System.currentTimeMillis() - ZonaRosaStore.misc.lastProfileRefreshTime
      ...
      ZonaRosaExecutors.BOUNDED.execute {
        ...
        val ids: List<RecipientId> = ZonaRosaDatabase.recipients.getRecipientsForRoutineProfileFetch(
        ...
        ZonaRosaStore.misc.lastProfileRefreshTime = System.currentTimeMillis()
      }
    }
  }
}
