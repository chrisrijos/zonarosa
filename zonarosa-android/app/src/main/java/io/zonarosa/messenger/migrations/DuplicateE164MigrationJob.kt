...
internal class DuplicateE164MigrationJob(
    ...
    for ((_, entries) in dupes) {
      ...
      ZonaRosaDatabase.rawDatabase.withinTransaction {
        ...
        for (entry in resolved.drop(1)) {
          ...
          ZonaRosaDatabase.recipients.mergeForMigration(first.id, entry.id)
        }
      }
    }
  ...
  private fun attemptToGetRidOfE164(entries: List<E164Entry>): List<E164Entry> {
    ...
    for (invalidPrefix in entries) {
      if (ZonaRosaDatabase.recipients.removeE164IfAnotherIdentifierIsPresent(invalidPrefix.id)) {
      ...
      if (ZonaRosaDatabase.recipients.deleteRecipientIfItHasNoMessages(invalidPrefix.id)) {
