...
import io.zonarosa.messenger.jobs.protos.Svr3MirrorJobData
...
class Svr3MirrorJob private constructor(parameters: Parameters, private var serializedChangeSession: String?) : Job(parameters) {
  ...
  override fun run(): Result {
    ...
    SvrRepository.operationLock.withLock {
      ...
      val svr3: SecureValueRecoveryV3 = AppDependencies.zonarosaServiceAccountManager.getSecureValueRecoveryV3(AppDependencies.libzonarosaNetwork)

      val session: PinChangeSession = serializedChangeSession?.let { session ->
        svr3.resumePinChangeSession(pin, ZonaRosaStore.svr.masterKey, session)
      } ?: svr3.setPin(pin, ZonaRosaStore.svr.masterKey)
      ...
      return when (val response: BackupResponse = session.execute()) {
        is BackupResponse.Success -> {
          ...
          ZonaRosaStore.svr.appendSvr3AuthTokenToList(response.authorization.asBasic())
