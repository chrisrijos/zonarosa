...
import io.zonarosa.messenger.util.MediaUtil.SlideType

@SuppressLint("RecipientIdDatabaseReferenceUsage", "ThreadIdDatabaseReferenceUsage") // Not a real table, just a view
class MediaTable internal constructor(context: Context?, databaseHelper: ZonaRosaDatabase?) : DatabaseTable(context, databaseHelper) {
  ...
  companion object {
    ...
    private val ALL_MEDIA_QUERY = String.format(
      ...
      """
        ${AttachmentTable.DATA_FILE} IS NOT NULL AND
        ${AttachmentTable.CONTENT_TYPE} NOT LIKE 'text/x-zonarosa-plain' AND
        ${MessageTable.LINK_PREVIEWS} IS NULL AND
        ${MessageTable.SCHEDULED_DATE} < 0
      """
    )
  ...
  data class MediaRecord constructor(
    ...
    companion object {
      ...
      fun from(cursor: Cursor): MediaRecord {
        val attachments = ZonaRosaDatabase.attachments.getAttachments(cursor)
