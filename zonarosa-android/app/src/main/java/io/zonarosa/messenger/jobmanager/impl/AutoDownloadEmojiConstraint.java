...
public class AutoDownloadEmojiConstraint implements Constraint {
  ...
  public void applyToJobInfo(@NonNull JobInfo.Builder jobInfoBuilder) {
    boolean canDownloadWhileRoaming = ZonaRosaPreferences.getRoamingMediaDownloadAllowed(context).contains(IMAGE_TYPE);
    boolean canDownloadWhileMobile  = ZonaRosaPreferences.getMobileMediaDownloadAllowed(context).contains(IMAGE_TYPE);
  ...
  private static @NonNull Set<String> getAllowedAutoDownloadTypes(@NonNull Context context, boolean forceWifi) {
    if      (NetworkUtil.isConnectedWifi(context))    return forceWifi ? Collections.singleton(IMAGE_TYPE) : ZonaRosaPreferences.getWifiMediaDownloadAllowed(context);
