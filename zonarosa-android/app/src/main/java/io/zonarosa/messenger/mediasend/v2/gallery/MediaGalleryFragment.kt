...
import io.zonarosa.messenger.components.recyclerview.GridDividerDecoration
import io.zonarosa.messenger.conversation.ManageContextMenu
import io.zonarosa.messenger.databinding.V2MediaGalleryFragmentBinding
import io.zonarosa.messenger.mediasend.MediaRepository
import io.zonarosa.messenger.mediasend.camerax.CameraXUtil
import io.zonarosa.messenger.mediasend.v2.review.MediaGalleryGridItemTouchListener
  ...
  override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (item.itemId == R.id.action_camera) {
          if (CameraXUtil.isSupported()) {
            ...
            Permissions.with(this)
              ...
              .withPermanentDenialDialog(getString(R.string.CameraXFragment_zonarosa_needs_camera_access_capture_photos), null, R.string.CameraXFragment_allow_access_camera, R.string.CameraXFragment_to_capture_photos_videos, getParentFragmentManager())
    ...
    galleryItemsWithSelection.observe(viewLifecycleOwner) {
      if (StorageUtil.canOnlyReadSelectedMediaStore() && it.isEmpty()) {
        ...
        binding.mediaGalleryMissingPermissions.visible = true
        binding.mediaGalleryManageContainer.visible = false
        binding.mediaGalleryPermissionText.text = getString(R.string.AttachmentKeyboard_ZonaRosa_needs_permission_to_show_your_photos_and_videos)
