...
import io.zonarosa.core.ui.BottomSheetUtil.requireCoordinatorLayout
...
import io.zonarosa.messenger.conversation.mutiselect.MultiselectPart.Attachments
...
class ScheduledMessagesBottomSheet : FixedRoundedCornerBottomSheetDialogFragment(), ScheduleMessageTimePickerBottomSheet.RescheduleCallback {
  ...
  private fun showScheduledMessageContextMenu(view: View, conversationMessage: ConversationMessage) {
    ZonaRosaContextMenu.Builder(view, requireCoordinatorLayout())
      .offsetX(12.dp)
      .offsetY(12.dp)
  ...
  private fun deleteMessage(messageId: Long) {
    context?.let { context ->
      val progressDialog = ZonaRosaProgressDialog.show(
        context = context,
        message = resources.getString(R.string.ScheduledMessagesBottomSheet_deleting_progress_message),
        indeterminate = true
      )
      SimpleTask.run({
        ZonaRosaDatabase.messages.deleteScheduledMessage(messageId)
      }, {
