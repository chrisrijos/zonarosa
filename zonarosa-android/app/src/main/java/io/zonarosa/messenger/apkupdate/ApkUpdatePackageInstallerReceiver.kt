...
import io.zonarosa.messenger.apkupdate.ApkUpdateNotifications.FailureReason
...
class ApkUpdatePackageInstallerReceiver : BroadcastReceiver() {
  ...
  companion object {
    ...
    const val EXTRA_USER_INITIATED = "zonarosa.user_initiated"
    const val EXTRA_DOWNLOAD_ID = "zonarosa.download_id"
  }
  ...
  override fun onReceive(context: Context, intent: Intent?) {
    ...
    when (statusCode) {
      PackageInstaller.STATUS_SUCCESS -> {
        if (ZonaRosaStore.apkUpdate.lastApkUploadTime != ZonaRosaStore.apkUpdate.pendingApkUploadTime) {
          Log.i(TAG, "Update installed successfully! Updating our lastApkUploadTime to ${ZonaRosaStore.apkUpdate.pendingApkUploadTime}")
          ZonaRosaStore.apkUpdate.lastApkUploadTime = ZonaRosaStore.apkUpdate.pendingApkUploadTime
