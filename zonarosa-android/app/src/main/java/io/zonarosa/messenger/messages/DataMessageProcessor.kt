...
import io.zonarosa.messenger.attachments.LocalStickerAttachment
...
import io.zonarosa.messenger.database.PaymentTable.PublicKeyConflictException
...
import io.zonarosa.messenger.jobs.GroupCallPeekJob
...
import io.zonarosa.messenger.jobs.TrimThreadJob
...
import io.zonarosa.messenger.linkpreview.LinkPreviewUtil
import io.zonarosa.messenger.messages.MessageContentProcessor.Companion.debug
...
import io.zonarosa.messenger.messages.ZonaRosaServiceProtoUtil.isInvalid
import io.zonarosa.messenger.messages.ZonaRosaServiceProtoUtil.isMediaMessage
import io.zonarosa.messenger.messages.ZonaRosaServiceProtoUtil.isPaymentActivated
import io.zonarosa.messenger.messages.ZonaRosaServiceProtoUtil.isPaymentActivationRequest
import io.zonarosa.messenger.messages.ZonaRosaServiceProtoUtil.isStoryReaction
...
import io.zonarosa.messenger.recipients.Recipient.HiddenState
...
import io.zonarosa.messenger.util.LinkUtil
...
object DataMessageProcessor {
  ...
  fun process(
    ...
    localMetrics: ZonaRosaLocalMetrics.MessageReceive?,
    ...
    if (groupId != null) {
      ZonaRosaTrace.beginSection("DataMessageProcessor#gv2PreProcessing")
    ...
    ZonaRosaTrace.beginSection("DataMessageProcessor#messageInsert")
    when {
    ...
    ZonaRosaTrace.beginSection("DataMessageProcessor#postProcess")
    ...
    if (groupId != null) {
      val unknownGroup = when (groupProcessResult) {
        ...
        else -> ZonaRosaDatabase.groups.isUnknownGroup(groupId)
      }
      if (unknownGroup) {
    ...
    if (groupId == null && senderRecipient.hiddenState == HiddenState.HIDDEN) {
      ZonaRosaDatabase.recipients.markHidden(senderRecipient.id, clearProfileKey = false, showMessageRequest = true)
    }
    ...
    if (metadata.sealedSender && messageId != null) {
      ...
      if (RecipientUtil.shouldHaveProfileKey(threadRecipient)) {
        ...
        if (groupId != null) {
          ...
          val profileSendJob = ProfileKeySendJob.create(ZonaRosaDatabase.threads.getOrCreateThreadIdFor(threadRecipient), true)
    ...
    if (insertResult != null && insertResult.threadWasNewlyCreated && !threadRecipient.isGroup && !threadRecipient.isSelf && !senderRecipient.isSystemContact) {
      val timeSinceLastSync = System.currentTimeMillis() - ZonaRosaStore.misc.lastCdsForegroundSyncTime
      if (timeSinceLastSync > RemoteConfig.cdsForegroundSyncInterval || timeSinceLastSync < 0) {
        ...
        ZonaRosaStore.misc.lastCdsForegroundSyncTime = System.currentTimeMillis()
      } else {
  ...
  private fun handleProfileKey(
    ...
    if (senderRecipient.isSelf) {
      ...
      if (ZonaRosaDatabase.recipients.setProfileKey(senderRecipient.id, messageProfileKey)) {
  ...
  fun handleUnknownGroupMessage(timestamp: Long, groupContextV2: GroupContextV2) {
    ...
    ZonaRosaDatabase.groups.fixMissingMasterKey(groupContextV2.groupMasterKey)
  }
  ...
  private fun handleInvalidMessage(
    ...
    if (insertResult != null) {
      ZonaRosaDatabase.messages.markAsInvalidMessage(insertResult.messageId)
  ...
  private fun handleEndSessionMessage(
    ...
    val insertResult: InsertResult? = ZonaRosaDatabase.messages.insertMessageInbox(incomingMessage).orNull()
  ...
  private fun handleExpirationUpdate(
    ...
    if (ZonaRosaDatabase.recipients.getExpiresInSeconds(threadRecipientId) == expiresIn.inWholeSeconds) {
    ...
    try {
      ...
      val insertResult: InsertResult? = ZonaRosaDatabase.messages.insertMessageInbox(mediaMessage, -1).orNull()

      if (expireTimerVersion != null) {
        ZonaRosaDatabase.recipients.setExpireMessages(threadRecipientId, expiresIn.inWholeSeconds.toInt(), expireTimerVersion)
      } else {
        // TODO [expireVersion] After unsupported builds expire, we can remove this branch
        ZonaRosaDatabase.recipients.setExpireMessagesWithoutIncrementingVersion(threadRecipientId, expiresIn.inWholeSeconds.toInt())
      }
  ...
  private fun handleStoryReaction(
    ...
    return try {
      ...
      try {
        val storyId = ZonaRosaDatabase.messages.getStoryId(authorRecipientId, sentTimestamp).id

        if (groupId != null) {
          parentStoryId = GroupReply(storyId)
        } else if (ZonaRosaDatabase.storySends.canReply(senderRecipientId, sentTimestamp)) {
          val story = ZonaRosaDatabase.messages.getMessageRecord(storyId) as MmsMessageRecord
          var displayText = ""
  ...
  fun handleReaction(
    ...
    val targetMessage = ZonaRosaDatabase.messages.getMessageFor(targetSentTimestamp, targetAuthor.id)
    ...
    val targetThread = ZonaRosaDatabase.threads.getThreadRecord(targetMessage.threadId)
    ...
    val groupRecord = ZonaRosaDatabase.groups.getGroup(targetThreadRecipientId).orNull()
    ...
    if (isRemove) {
      ZonaRosaDatabase.reactions.deleteReaction(targetMessageId, senderRecipientId)
      AppDependencies.messageNotifier.updateNotification(context)
    } else {
      val reactionRecord = ReactionRecord(emoji!!, senderRecipientId, message.timestamp!!, System.currentTimeMillis())
      ZonaRosaDatabase.reactions.addReaction(targetMessageId, reactionRecord)
  ...
  fun handleRemoteDelete(context: Context, envelope: Envelope, message: DataMessage, senderRecipientId: RecipientId, earlyMessageCacheEntry: EarlyMessageCacheEntry?): MessageId? {
    ...
    val targetMessage: MessageRecord? = ZonaRosaDatabase.messages.getMessageFor(targetSentTimestamp, senderRecipientId)
    ...
    return if (targetMessage != null && MessageConstraintsUtil.isValidRemoteDeleteReceive(targetMessage, senderRecipientId, envelope.serverTimestamp!!)) {
      ZonaRosaDatabase.messages.markAsRemoteDelete(targetMessage, senderRecipientId)
      if (targetMessage.isStory()) {
        ZonaRosaDatabase.messages.deleteRemotelyDeletedStory(targetMessage.id)
      }
  ...
  private fun handlePaymentActivation(
    ...
    try {
      ...
      return ZonaRosaDatabase.messages.insertMessageInbox(mediaMessage, -1).orNull()
    } catch (e: MmsException) {
      throw StorageFailedException(e, metadata.sourceServiceId.toString(), metadata.sourceDeviceId)
    }
    return null
  ...
  private fun handlePayment(
    ...
    try {
      ZonaRosaDatabase.payments.createIncomingPayment(
        uuid,
        senderRecipientId,
        message.timestamp!!,
    ...
    return try {
      ...
      try {
        if (selfId == storyAuthorRecipientId) {
          storyMessageId = ZonaRosaDatabase.storySends.getStoryMessageFor(senderRecipient.id, sentTimestamp)
        }

        if (storyMessageId == null) {
          storyMessageId = ZonaRosaDatabase.messages.getStoryId(storyAuthorRecipientId, sentTimestamp)
        }

        val story: MmsMessageRecord = ZonaRosaDatabase.messages.getMessageRecord(storyMessageId.id) as MmsMessageRecord
        var threadRecipient: Recipient = ZonaRosaDatabase.threads.getRecipientForThreadId(story.threadId)!!
        val groupRecord: GroupRecord? = ZonaRosaDatabase.groups.getGroup(threadRecipient.id).orNull()
        ...
        } else if (groupStory || ZonaRosaDatabase.storySends.canReply(senderRecipient.id, sentTimestamp)) {
  ...
  private fun handleGiftMessage(
    ...
    val insertResult: InsertResult? = try {
      ...
      ZonaRosaDatabase.messages.insertMessageInbox(mediaMessage, -1).orNull()
    } catch (e: MmsException) {
      throw StorageFailedException(e, metadata.sourceServiceId.toString(), metadata.sourceDeviceId)
    }
  ...
  private fun handleMediaMessage(
    ...
    try {
      ...
      insertResult = ZonaRosaDatabase.messages.insertMessageInbox(retrieved = mediaMessage, candidateThreadId = -1, skipThreadUpdate = batchCache.batchThreadUpdates).orNull()
      if (insertResult != null) {
        ZonaRosaDatabase.messages.setTransactionSuccessful()
  ...
  private fun handleTextMessage(
    ...
    val insertResult: InsertResult? = ZonaRosaDatabase.messages.insertMessageInbox(textMessage, skipThreadUpdate = batchCache.batchThreadUpdates).orNull()
  ...
  fun handleGroupCallUpdateMessage(
    ...
    if (!ZonaRosaDatabase.groups.groupExists(groupId)) {
      warn(envelope.timestamp!!, "Received group call update message for unknown groupId: $groupId")
      return
    }

    val groupRecipientId = ZonaRosaDatabase.recipients.getOrInsertFromPossiblyMigratedGroupId(groupId)
  ...
  fun handlePollCreate(
    ...
    if (groupId != null) {
      val groupRecord = ZonaRosaDatabase.groups.getGroup(groupId).orNull()
    ...
    val insertResult: InsertResult? = ZonaRosaDatabase.messages.insertMessageInbox(pollMessage).orNull()
    return if (insertResult != null) {
      AppDependencies.messageNotifier.updateNotification(context, ConversationId.forConversation(insertResult.threadId))
      insertResult
    } else {
  ...
  fun handlePollVote(
    ...
    val targetMessage = ZonaRosaDatabase.messages.getMessageRecord(messageId.id)
    val pollId = ZonaRosaDatabase.polls.getPollId(messageId.id)
    if (pollId == null) {
      warn(envelope.timestamp!!, "[handlePollVote] Poll was not found. timestamp: $targetSentTimestamp  author: ${senderRecipient.id}")
      return null
    }

    val existingVoteCount = ZonaRosaDatabase.polls.getCurrentPollVoteCount(pollId, senderRecipient.id.toLong())
    ...
    val allOptionIds = ZonaRosaDatabase.polls.getPollOptionIds(pollId)
    ...
    if (!ZonaRosaDatabase.polls.canAllowMultipleVotes(pollId) && pollVote.optionIndexes.size > 1) {
  ...
  fun handlePinMessage(
    ...
    val targetMessage: MmsMessageRecord? = ZonaRosaDatabase.messages.getMessageFor(pinMessage.targetSentTimestamp!!, targetAuthor.id) as? MmsMessageRecord
    if (targetMessage == null) {
    ...
    val groupRecord = ZonaRosaDatabase.groups.getGroup(threadRecipient.id).orNull()
    if (groupRecord != null && !groupRecord.members.contains(senderRecipient.id)) {
    ...
    val insertResult: InsertResult? = ZonaRosaDatabase.messages.insertMessageInbox(pinnedMessage).orNull()
  ...
  fun handleUnpinMessage(
    ...
    val targetMessage: MmsMessageRecord? = ZonaRosaDatabase.messages.getMessageFor(unpinMessage.targetSentTimestamp!!, targetAuthor.id) as? MmsMessageRecord
    if (targetMessage == null) {
    ...
    val targetMessageId = targetMessage.latestRevisionId?.id ?: targetMessage.id
    ZonaRosaDatabase.messages.unpinMessage(targetMessageId, targetMessage.threadId)
  ...
  fun handleAdminRemoteDelete(envelope: Envelope, message: DataMessage, senderRecipient: Recipient, threadRecipient: Recipient, earlyMessageCacheEntry: EarlyMessageCacheEntry?): MessageId? {
    ...
    val targetMessage: MessageRecord? = ZonaRosaDatabase.messages.getMessageFor(targetSentTimestamp, targetAuthor.id)
    ...
    return if (targetMessage != null && MessageConstraintsUtil.isValidAdminDeleteReceive(targetMessage, senderRecipient, envelope.serverTimestamp!!, groupRecord)) {
      ZonaRosaDatabase.messages.markAsRemoteDelete(targetMessage, senderRecipient.id)
      MessageId(targetMessage.id)
    } else if (targetMessage == null) {
  ...
  fun notifyTypingStoppedFromIncomingMessage(context: Context, senderRecipient: Recipient, threadRecipientId: RecipientId, device: Int) {
    val threadId = ZonaRosaDatabase.threads.getThreadIdIfExistsFor(threadRecipientId)

    if (threadId > 0 && ZonaRosaPreferences.isTypingIndicatorsEnabled(context)) {
  ...
  private fun insertPlaceholder(sender: RecipientId, timestamp: Long, groupId: GroupId?): InsertResult? {
    ...
    return ZonaRosaDatabase.messages.insertMessageInbox(textMessage).orNull()
  }
  ...
  fun getValidatedQuote(context: Context, timestamp: Long, message: DataMessage, senderRecipient: Recipient, threadRecipient: Recipient): QuoteModel? {
    ...
    var quotedMessage = ZonaRosaDatabase.messages.getMessageFor(quote.id!!, authorId) as? MmsMessageRecord
    ...
    if (quotedMessage != null && isSenderValid(quotedMessage, timestamp, senderRecipient, threadRecipient) && !quotedMessage.isRemoteDelete && !quotedMessage.isUnsupported) {
      ...
      val targetMessageAttachments = ZonaRosaDatabase.attachments.getAttachmentsForMessage(quotedMessage.id)
      val mentions: List<Mention> = ZonaRosaDatabase.mentions.getMentionsForMessage(quotedMessage.id)
      ...
      if (quotedMessage.isPaymentNotification) {
        quotedMessage = ZonaRosaDatabase.payments.updateMessageWithPayment(quotedMessage) as MmsMessageRecord
      }
  ...
  private fun handlePollValidation(
    ...
    val groupRecord = ZonaRosaDatabase.groups.getGroup(targetThread.recipient.id).orNull()
    if (groupRecord != null && !groupRecord.members.contains(senderRecipient.id)) {
  ...
  fun getStickerAttachment(timestamp: Long, message: DataMessage): Attachment? {
    ...
    val stickerRecord: StickerRecord? = ZonaRosaDatabase.stickers.getSticker(stickerLocator.packId, stickerLocator.stickerId, false)
