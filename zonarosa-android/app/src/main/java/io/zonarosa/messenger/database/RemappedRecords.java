...
class RemappedRecords {
  ...
  void deleteThread(long oldId) {
    ...
    ZonaRosaDatabase.remappedRecords().deleteThreadMapping(oldId);
  }
  ...
  void addRecipient(@NonNull RecipientId oldId, @NonNull RecipientId newId) {
    ...
    ZonaRosaDatabase.remappedRecords().addRecipientMapping(oldId, newId);
  }
  ...
  void addThread(long oldId, long newId) {
    ...
    ZonaRosaDatabase.remappedRecords().addThreadMapping(oldId, newId);
  }
  ...
  private void ensureRecipientMapIsPopulated() {
    if (recipientMap == null) {
      recipientMap = ZonaRosaDatabase.remappedRecords().getAllRecipientMappings();
    }
  }

  private void ensureThreadMapIsPopulated() {
    if (threadMap == null) {
      threadMap = ZonaRosaDatabase.remappedRecords().getAllThreadMappings();
    }
  }

  private void ensureInTransaction() {
    if (!ZonaRosaDatabase.inTransaction()) {
