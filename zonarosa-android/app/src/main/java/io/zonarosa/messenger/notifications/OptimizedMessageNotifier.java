...
import io.zonarosa.messenger.util.LeakyBucketLimiter;
...
public class OptimizedMessageNotifier implements MessageNotifier {
  ...
  public OptimizedMessageNotifier(@NonNull Application context) {
    this.limiter                = new LeakyBucketLimiter(3, 1000, new Handler(ZonaRosaExecutors.getAndStartHandlerThread("zonarosa-notifier", ThreadUtil.PRIORITY_IMPORTANT_BACKGROUND_THREAD).getLooper()));
  ...
  public void updateNotification(@NonNull Context context) {
    ZonaRosaDatabase.runPostSuccessfulTransaction(DEDUPE_KEY_GENERAL, () -> {
  ...
  public void updateNotification(@NonNull Context context, @NonNull ConversationId conversationId) {
    ZonaRosaDatabase.runPostSuccessfulTransaction(DEDUPE_KEY_CHAT + conversationId.getThreadId(), () -> {
