...
public class ResendMessageJob extends BaseJob {
  ...
  protected void onRun() throws Exception {
    if (ZonaRosaStore.internal().getDelayResends()) {
    ...
    if (distributionId != null) {
      if (groupId != null) {
        ...
        Optional<GroupRecord> groupRecord = ZonaRosaDatabase.groups().getGroupByDistributionId(distributionId);
        ...
        createGroupSendToken = () -> ZonaRosaDatabase.groups().getGroupSendFullToken(groupId, recipientId);
      } else {
        Log.d(TAG, "GroupId is not present. Assuming this is a message for a distribution list.");
        DistributionListRecord listRecord = ZonaRosaDatabase.distributionLists().getListByDistributionId(distributionId);
    ...
    try {
      ...
      AppDependencies.getProtocolStore().aci().sessions().archiveSessions(recipientId, ZonaRosaServiceAddress.DEFAULT_DEVICE_ID);
      AppDependencies.getProtocolStore().aci().sessions().archiveSiblingSessions(recipient.requireServiceId().toProtocolAddress(ZonaRosaServiceAddress.DEFAULT_DEVICE_ID));
      AppDependencies.getProtocolStore().pni().sessions().archiveSessions(recipientId, ZonaRosaServiceAddress.DEFAULT_DEVICE_ID);
      AppDependencies.getProtocolStore().pni().sessions().archiveSiblingSessions(recipient.requireServiceId().toProtocolAddress(ZonaRosaServiceAddress.DEFAULT_DEVICE_ID));
    ...
    if (result.isSuccess() && distributionId != null) {
      List<ZonaRosaProtocolAddress> addresses = result.getSuccess()
                                                    .getDevices()
                                                    .stream()
