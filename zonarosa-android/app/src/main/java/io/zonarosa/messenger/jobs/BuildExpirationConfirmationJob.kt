...
import io.zonarosa.service.api.remoteconfig.RemoteConfigResult
...
class BuildExpirationConfirmationJob private constructor(params: Parameters) : Job(params) {
  ...
  override fun run(): Result {
    if (Util.getTimeUntilBuildExpiry(ZonaRosaStore.misc.estimatedServerTime) > 0) {
    ...
    return when (val result: NetworkResult<RemoteConfigResult> = ZonaRosaNetwork.remoteConfig.getRemoteConfig()) {
      is NetworkResult.Success -> {
        val serverTimeMs = result.result.serverEpochTimeMilliseconds
        ZonaRosaStore.misc.setLastKnownServerTime(serverTimeMs, System.currentTimeMillis())
