...
import io.zonarosa.core.util.groupBy
import io.zonarosa.core.util.isNull
...
import io.zonarosa.messenger.attachments.WallpaperAttachment
...
import io.zonarosa.messenger.crypto.ModernEncryptingPartOutputStream
import io.zonarosa.messenger.database.AttachmentTable.ArchiveTransferState.COPY_PENDING
import io.zonarosa.messenger.database.AttachmentTable.ArchiveTransferState.FINISHED
import io.zonarosa.messenger.database.AttachmentTable.ArchiveTransferState.NONE
import io.zonarosa.messenger.database.AttachmentTable.ArchiveTransferState.PERMANENT_FAILURE
import io.zonarosa.messenger.database.AttachmentTable.ArchiveTransferState.UPLOAD_IN_PROGRESS
import io.zonarosa.messenger.database.AttachmentTable.Companion.DATA_FILE
import io.zonarosa.messenger.database.AttachmentTable.Companion.DATA_HASH_END
import io.zonarosa.messenger.database.AttachmentTable.Companion.PREUPLOAD_MESSAGE_ID
import io.zonarosa.messenger.database.AttachmentTable.Companion.REMOTE_KEY
import io.zonarosa.messenger.database.AttachmentTable.Companion.TRANSFER_PROGRESS_DONE
...
import io.zonarosa.messenger.database.model.databaseprotos.AudioWaveFormData
import io.zonarosa.messenger.dependencies.AppDependencies
import io.zonarosa.messenger.jobs.AttachmentDownloadJob
import io.zonarosa.messenger.jobs.AttachmentUploadJob
import io.zonarosa.messenger.jobs.GenerateAudioWaveFormJob
...
import io.zonarosa.messenger.util.FileUtils
import io.zonarosa.messenger.util.ImageCompressionUtil
import io.zonarosa.messenger.util.JsonUtils.SaneJSONObject
import io.zonarosa.messenger.util.MediaUtil
import io.zonarosa.messenger.util.RemoteConfig
import io.zonarosa.messenger.video.EncryptedMediaDataSource
...
class AttachmentTable(
  context: Context,
  databaseHelper: ZonaRosaDatabase,
  ...
  fun deleteAttachmentsForMessage(mmsId: Long): Boolean {
    ...
    val deleteCount = writableDatabase.withinTransaction { db ->
      ...
      ZonaRosaDatabase.attachmentMetadata.cleanup()
  ...
  fun deleteAbandonedAttachmentFiles(): Int {
    ...
    filesInDb += ZonaRosaDatabase.stickers.getAllStickerFiles()
    ...
    readableDatabase
      ...
      .forEach { cursor ->
        ...
        val mediaId = MediaName.fromPlaintextHashAndRemoteKey(plaintextHash, remoteKey)
          .toMediaId(ZonaRosaStore.backup.mediaRootBackupKey)
          .encode()
  ...
  private fun insertArchivedAttachment(messageId: Long, attachment: ArchivedAttachment, quote: Boolean, quoteTargetContentType: String?): AttachmentId {
    ...
    val attachmentId: AttachmentId = writableDatabase.withinTransaction { db ->
      ...
      val metadataId: Long? = if (plaintextHash != null && attachment.localBackupKey != null) {
        ZonaRosaDatabase.attachmentMetadata.insert(plaintextHash, attachment.localBackupKey)
      } else {
        null
      }
  ...
  private fun buildAttachmentsThatCanArchiveQuery(archiveTransferStateFilter: String, includeOptimized: Boolean = false): String {
    val notReleaseChannelClause = ZonaRosaStore.releaseChannel.releaseChannelRecipientId?.let {
  ...
  fun getMediaObjectsThatCantBeFound(objects: Set<ArchivedMediaObject>): Set<ArchivedMediaObject> {
    ...
    readableDatabase
      ...
      .forEach { cursor ->
        ...
        val mediaIdThumbnail = MediaName.fromPlaintextHashAndRemoteKeyForThumbnail(plaintextHash, remoteKey).toMediaId(ZonaRosaStore.backup.mediaRootBackupKey).encode()
  ...
  fun debugGetAttachmentStats(): DebugAttachmentStats {
    ...
    val lastSnapshotFullSizeCount = ZonaRosaDatabase.backupMediaSnapshots.debugGetFullSizeAttachmentCountForMostRecentSnapshot()
    val lastSnapshotThumbnailCount = ZonaRosaDatabase.backupMediaSnapshots.debugGetThumbnailAttachmentCountForMostRecentSnapshot()
