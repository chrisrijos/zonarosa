...
class InAppPaymentKeepAliveJob private constructor(
  ...
  companion object {
    ...
    fun enqueueAndTrackTimeIfNecessary() {
      ...
      val lastKeepAliveTime = ZonaRosaStore.inAppPayments.getLastKeepAliveLaunchTime().milliseconds
    ...
    fun enqueueAndTrackTime(now: Duration) {
      ...
      ZonaRosaStore.inAppPayments.setLastKeepAliveLaunchTime(now.inWholeMilliseconds)
    }
  }
  ...
  private fun doRun() {
    ...
    if (ZonaRosaDatabase.inAppPayments.hasPrePendingRecurringTransaction(type.inAppPaymentType)) {
    ...
    when (activeInAppPayment.data.redemption?.stage) {
      InAppPaymentData.RedemptionState.Stage.INIT -> {
        ...
        ZonaRosaDatabase.inAppPayments.update(payment)
  ...
  private fun clearOldPendingPaymentsIfRequired() {
    ...
    val inAppPayments = ZonaRosaDatabase.inAppPayments.getOldPendingPayments(type.inAppPaymentType)
  ...
  private fun getActiveInAppPayment(
    ...
    val current: InAppPaymentTable.InAppPayment? = ZonaRosaDatabase.inAppPayments.getByEndOfPeriod(type.inAppPaymentType, endOfCurrentPeriod)

    val inAppPayment = if (current == null) {
      val oldInAppPayment = ZonaRosaDatabase.inAppPayments.getByLatestEndOfPeriod(type.inAppPaymentType)
      ...
      if (type == InAppPaymentSubscriberRecord.Type.DONATION) {
        ZonaRosaStore.inAppPayments.setLastEndOfPeriod(endOfCurrentPeriod.inWholeSeconds)
      }
      ...
      ZonaRosaDatabase.inAppPaymentSubscribers.setPaymentMethod(subscriber.subscriberId, paymentMethodType = newInAppPaymentMethodType)
      ...
      val inAppPaymentId = ZonaRosaDatabase.inAppPayments.insert(
        ...
        inAppPaymentData = InAppPaymentData(
          ...
          badge = badge,
          amount = FiatMoney.fromZonaRosaNetworkAmount(subscription.amount, Currency.getInstance(subscription.currency)).toFiatValue(),
          error = null,
      ...
      ZonaRosaDatabase.inAppPayments.getById(current.id)
    ...
    if (
      ...
      return ZonaRosaDatabase.inAppPayments.getById(inAppPayment.id)
    } else {
      return inAppPayment
    }
  }
