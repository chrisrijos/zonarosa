...
import io.zonarosa.core.util.CursorExtensionsKt;
import io.zonarosa.core.util.CursorUtil;
import io.zonarosa.core.util.SQLiteDatabaseExtensionsKt;
...
public final class PaymentTable extends DatabaseTable implements RecipientIdDatabaseReference {
  ...
  private final MutableLiveData<Object> changeZonaRosa;

  PaymentTable(@NonNull Context context, @NonNull ZonaRosaDatabase databaseHelper) {
    super(context, databaseHelper);

    this.changeZonaRosa = new MutableLiveData<>(new Object());
  }
      ...
      throws PublicKeyConflictException, SerializationException
  {
    ...
    SQLiteDatabase database = databaseHelper.getZonaRosaWritableDatabase();
  ...
  public @NonNull List<PaymentTransaction> getAll() {
    SQLiteDatabase           database = databaseHelper.getZonaRosaReadableDatabase();
  ...
  public @NonNull List<PaymentTransaction> getUnseenPayments() {
    SQLiteDatabase           db      = databaseHelper.getZonaRosaReadableDatabase();
  ...
  public @NonNull LiveData<List<PaymentTransaction>> getAllLive() {
    return LiveDataUtil.mapAsync(changeZonaRosa, change -> getAll());
  }
  ...
  private void notifyChanged() {
    changeZonaRosa.postValue(new Object());
  ...
  private void notifyUuidChanged(@Nullable UUID uuid) {
    if (uuid != null) {
      ...
      MessageId messageId = ZonaRosaDatabase.messages().getPaymentMessage(uuid);
      if (messageId != null) {
