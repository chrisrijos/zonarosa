package io.zonarosa.messenger.emoji
...
import io.zonarosa.messenger.components.emoji.parsing.EmojiDrawInfo
import io.zonarosa.messenger.emoji.protos.JumbomojiPack
import io.zonarosa.messenger.jobmanager.impl.AutoDownloadEmojiConstraint
import io.zonarosa.messenger.keyvalue.ZonaRosaStore
import io.zonarosa.messenger.util.ListenableFutureTask
import io.zonarosa.messenger.util.SoftHashMap
...
object JumboEmoji {
  ...
  private val executor = ZonaRosaExecutors.newCachedSingleThreadExecutor("jumbo-emoji", ThreadUtil.PRIORITY_IMPORTANT_BACKGROUND_THREAD)
  ...
  fun updateCurrentVersion(context: Context) {
    ZonaRosaExecutors.BOUNDED.execute {
      ...
      if (EmojiFiles.getLatestEmojiData(context, version)?.format != null) {
        ...
        ThreadUtil.runOnMain { downloadedJumbos.addAll(ZonaRosaStore.emoji.getJumboEmojiSheets(version.version)) }
      }
    }
  }
  ...
  fun loadJumboEmoji(context: Context, drawInfo: EmojiDrawInfo): LoadResult {
    ...
    val newTask = ListenableFutureTask<Bitmap> {
      ...
      if (uuid == null) {
        ...
        ZonaRosaStore.emoji.addJumboEmojiSheet(version.version, drawInfo.jumboSheet)
      }
