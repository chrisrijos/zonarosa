...
import io.zonarosa.messenger.badges.gifts.Gifts.formatExpiry
...
class GiftMessageView @JvmOverloads constructor(
  ...
  fun setGiftBadge(requestManager: RequestManager, giftBadge: GiftBadge, isOutgoing: Boolean, callback: Callback, fromRecipient: Recipient, toRecipient: Recipient) {
    ...
    if (isOutgoing) {
      ...
      titleView.text = context.getString(R.string.GiftMessageView__s_donated_to_zonarosa_on, fromRecipient.getShortDisplayName(context))
