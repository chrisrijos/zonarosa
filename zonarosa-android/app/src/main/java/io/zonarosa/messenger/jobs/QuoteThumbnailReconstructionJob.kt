...
class QuoteThumbnailReconstructionJob private constructor(params: Parameters) : Job(params) {
  ...
  override fun run(): Result {
    val quoteAttachment = ZonaRosaDatabase.attachments.getNewestQuotePendingReconstruction()
    ...
    val message = ZonaRosaDatabase.messages.getMessageRecordOrNull(quoteAttachment.mmsId)
    if (message == null) {
      ...
      ZonaRosaDatabase.attachments.clearQuotePendingReconstruction(quoteAttachment.attachmentId)
    ...
    if (messageAge > RemoteConfig.messageQueueTime.milliseconds) {
      ...
      ZonaRosaDatabase.attachments.clearAllQuotesPendingReconstruction()
      return Result.success()
    }

    val targetMessage = ZonaRosaDatabase.messages.getMessageFor(message.getQuote()!!.id, message.getQuote()!!.author)
    ...
    val targetAttachment = ZonaRosaDatabase.attachments.getAttachmentsForMessage(targetMessage.id).firstOrNull { MediaUtil.isImageOrVideoType(it.contentType) && it.uri != null }
    ...
    val thumbnailData = ZonaRosaDatabase.attachments.generateQuoteThumbnail(DecryptableUri(targetAttachment.uri!!), targetAttachment.contentType, quiet = true)
    ...
    ZonaRosaDatabase.attachments.applyReconstructedQuoteData(quoteAttachment.attachmentId, thumbnailData)
  ...
  override fun onFailure() {
    activeQuoteAttachment?.let { attachment ->
      ...
      ZonaRosaDatabase.attachments.clearQuotePendingReconstruction(attachment.attachmentId)
