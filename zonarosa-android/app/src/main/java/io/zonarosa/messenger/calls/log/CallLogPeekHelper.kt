...
class CallLogPeekHelper : DefaultLifecycleObserver {
  ...
  private val executor: Executor = SerialExecutor(ZonaRosaExecutors.BOUNDED_IO)
  ...
  private fun performPeeks() {
    executor.execute {
      ...
      AppDependencies.zonarosaCallManager.isCallActive(object : ResultReceiver(null) {
      ...
      items.forEach {
        when (it.identifier.peekEntryType) {
          PeekEntryType.CALL_LINK -> AppDependencies.zonarosaCallManager.peekCallLinkCall(it.recipientId)
          PeekEntryType.GROUP_CALL -> AppDependencies.zonarosaCallManager.peekGroupCall(it.recipientId)
        }
      }
