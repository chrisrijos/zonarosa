...
import io.zonarosa.messenger.badges.gifts.ExpiredGiftSheetConfiguration.forExpiredGiftBadge
...
import io.zonarosa.messenger.badges.models.BadgeDisplay160
...
import io.zonarosa.messenger.components.settings.models.OutlinedSwitch
...
class ViewReceivedGiftBottomSheet : DSLSettingsBottomSheetFragment() {
  ...
  private fun DSLConfiguration.presentSubheading(recipient: Recipient) {
    noPadTextPref(
      title = DSLSettingsText.from(
        charSequence = requireContext().getString(R.string.ViewReceivedGiftBottomSheet__s_made_a_donation_to_zonarosa, recipient.getShortDisplayName(requireContext())),
        DSLSettingsText.CenterModifier
      )
    )
  }
