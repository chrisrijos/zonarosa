 ...
 */
public final class PinValues extends ZonaRosaStoreValues {
  ...
  public void onEntrySuccess(@NonNull String pin) {
    long nextInterval = ZonaRosaPinReminders.getNextInterval(getCurrentInterval());
    ...
    ZonaRosaStore.svr().setPinIfNotPresent(pin);
  }

  public void onEntrySuccessWithWrongGuess(@NonNull String pin) {
    long nextInterval = ZonaRosaPinReminders.getPreviousInterval(getCurrentInterval());
  ...
  public void onEntrySkip(boolean includedFailure) {
    ...
    if (includedFailure) {
      nextInterval = ZonaRosaPinReminders.getPreviousInterval(getCurrentInterval());
    } else {
  ...
  public void resetPinReminders() {
    long nextInterval = ZonaRosaPinReminders.INITIAL_INTERVAL;
  ...
  public long getCurrentInterval() {
    return getLong(NEXT_INTERVAL, ZonaRosaPreferences.getRegistrationLockNextReminderInterval(AppDependencies.getApplication()));
  }

  public long getLastSuccessfulEntryTime() {
    return getLong(LAST_SUCCESSFUL_ENTRY, ZonaRosaPreferences.getRegistrationLockLastReminderTime(AppDependencies.getApplication()));
  }
  ...
  public @NonNull PinKeyboardType getKeyboardType() {
    String pin = ZonaRosaStore.svr().getPin();
