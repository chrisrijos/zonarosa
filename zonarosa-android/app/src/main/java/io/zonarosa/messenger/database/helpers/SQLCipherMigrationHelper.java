...
import io.zonarosa.messenger.crypto.AsymmetricMasterCipher;
...
public class SQLCipherMigrationHelper {
                                      ...
                                      @NonNull SQLiteDatabase modernDb)
  {
    ...
    try (NotificationController controller = GenericForegroundService.startForegroundTask(context, context.getString(R.string.SQLCipherMigrationHelper_migrating_zonarosa_database))) {
                                       ...
                                       @Nullable LegacyMigrationJob.DatabaseUpgradeListener listener)
  {
    ...
    try (NotificationController controller = GenericForegroundService.startForegroundTask(context, context.getString(R.string.SQLCipherMigrationHelper_migrating_zonarosa_database))) {
      ...
      ZonaRosaPreferences.setNeedsSqlCipherMigration(context, false);
      modernDb.setTransactionSuccessful();
    } catch (UnableToStartException e) {
      throw new IllegalStateException(e);
    } finally {
