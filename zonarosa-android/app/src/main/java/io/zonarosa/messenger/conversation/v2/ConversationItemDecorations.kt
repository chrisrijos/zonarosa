...
import io.zonarosa.messenger.util.drawAsTopItemDecoration
import io.zonarosa.messenger.util.layoutIn
import io.zonarosa.messenger.util.toLocalDate
 ...
 *
 * This is a converted and trimmed down version of [io.zonarosa.messenger.util.StickyHeaderDecoration].
 */
  ...
  private inner class DateHeaderViewHolder(val itemView: View) {
    ...
    fun updateForWallpaper() {
      if (hasWallpaper) {
        ...
        date.setTextColor(ContextCompat.getColor(itemView.context, CoreUiR.color.zonarosa_colorNeutralInverse))
      } else {
        date.background = null
        date.setTextColor(ContextCompat.getColor(itemView.context, CoreUiR.color.zonarosa_colorOnSurfaceVariant))
      }
    }
  }
