...
import io.zonarosa.core.util.firstOrNull
 ...
 */
class SenderKeyTable internal constructor(context: Context?, databaseHelper: ZonaRosaDatabase?) : DatabaseTable(context, databaseHelper) {
  companion object {
  ...
  fun store(address: ZonaRosaProtocolAddress, distributionId: DistributionId, record: SenderKeyRecord) {
  ...
  fun load(address: ZonaRosaProtocolAddress, distributionId: DistributionId): SenderKeyRecord? {
   ...
   */
  fun getCreatedTime(address: ZonaRosaProtocolAddress, distributionId: DistributionId): Long {
    return readableDatabase
      .select(CREATED_AT)
      .from(TABLE_NAME)
      .where("$ADDRESS = ? AND $DEVICE = ? AND $DISTRIBUTION_ID = ?", address.name, address.deviceId, distributionId)
  ...
  fun getAllCreatedBySelf(): Cursor {
    return readableDatabase
      ...
      .from(TABLE_NAME)
      .where("$ADDRESS = ?", ZonaRosaStore.account.requireAci())
