...
import io.zonarosa.messenger.database.model.IncomingSticker;
...
public class StickerPackDownloadJob extends BaseJob {
  ...
  protected void onRun() throws IOException, InvalidMessageException {
    if (isReferencePack && !ZonaRosaDatabase.attachments().containsStickerPackId(packId) && !BlessedPacks.contains(packId)) {
      Log.w(TAG, "There are no attachments with the requested packId present for this reference pack. Skipping.");
      return;
    }

    if (isReferencePack && ZonaRosaDatabase.stickers().isPackAvailableAsReference(packId)) {
  ...
  public void onFailure() {
    ...
    ZonaRosaDatabase.stickers().uninstallPack(packId);
    ZonaRosaDatabase.stickers().deleteOrphanedPacks();
  }
