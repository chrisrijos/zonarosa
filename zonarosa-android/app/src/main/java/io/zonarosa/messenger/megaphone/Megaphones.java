...
import io.zonarosa.core.util.MapUtil;
import io.zonarosa.core.util.SetUtil;
import io.zonarosa.core.util.TranslationDetection;
...
import io.zonarosa.messenger.database.model.MegaphoneRecord;
...
import io.zonarosa.messenger.lock.ZonaRosaPinReminderDialog;
import io.zonarosa.messenger.lock.ZonaRosaPinReminders;
import io.zonarosa.messenger.lock.v2.CreateSvrPinActivity;
import io.zonarosa.messenger.lock.v2.SvrMigrationActivity;
import io.zonarosa.messenger.notifications.NotificationChannels;
import io.zonarosa.messenger.notifications.TurnOnNotificationsBottomSheet;
import io.zonarosa.messenger.profiles.AvatarHelper;
import io.zonarosa.messenger.profiles.manage.EditProfileActivity;
import io.zonarosa.messenger.profiles.username.NewWaysToConnectDialogFragment;
...
public final class Megaphones {
  ...
  private static Map<Event, MegaphoneSchedule> buildDisplayOrder(@NonNull Context context, @NonNull Map<Event, MegaphoneRecord> records) {
    return new LinkedHashMap<>() {{
      ...
      put(Event.CLIENT_DEPRECATED, ZonaRosaStore.misc().isClientDeprecated() ? ALWAYS : NEVER);
      ...
      put(Event.PIN_REMINDER, new ZonaRosaPinReminderSchedule());
      ...
      put(Event.TURN_ON_ZONAROSA_BACKUPS, shouldShowTurnOnBackupsMegaphone(context) ? new RecurringSchedule(TimeUnit.DAYS.toMillis(30), TimeUnit.DAYS.toMillis(90)) : NEVER);
  ...
  private static boolean shouldShowLinkedDeviceInactiveMegaphone() {
    ...
    LeastActiveLinkedDevice device = ZonaRosaStore.misc().getLeastActiveLinkedDevice();
    if (device == null) {
      return false;
    }
  ...
  private static @Nullable Megaphone forRecord(@NonNull Context context, @NonNull MegaphoneRecord record) {
    switch (record.getEvent()) {
      ...
      case TURN_ON_ZONAROSA_BACKUPS:
        return buildTurnOnZonaRosaBackupsMegaphone();
  ...
  private static @NonNull Megaphone buildPinsForAllMegaphone(@NonNull MegaphoneRecord record) {
    if (PinsForAllSchedule.shouldDisplayFullScreen(record.getFirstVisible(), System.currentTimeMillis())) {
      ...
      return new Megaphone.Builder(Event.PINS_FOR_ALL, Megaphone.Style.BASIC)
          ...
          .setBody(R.string.KbsMegaphone__pins_keep_information_thats_stored_with_zonarosa_encrytped)
  ...
  private static @NonNull Megaphone buildPinReminderMegaphone(@NonNull Context context) {
    return new Megaphone.Builder(Event.PIN_REMINDER, Megaphone.Style.BASIC)
        .setTitle(R.string.Megaphones_verify_your_zonarosa_pin)
        ...
        .setActionButton(R.string.Megaphones_verify_pin, (megaphone, controller) -> {
          ZonaRosaPinReminderDialog.show(controller.getMegaphoneActivity(), controller::onMegaphoneNavigationRequested, new ZonaRosaPinReminderDialog.Callback() {
            @Override
            public void onReminderDismissed(boolean includedFailure) {
              Log.i(TAG, "[PinReminder] onReminderDismissed(" + includedFailure + ")");

              ZonaRosaStore.pin().onEntrySkip(includedFailure);
              controller.onMegaphoneSnooze(Event.PIN_REMINDER);
              controller.onMegaphoneToastRequested(controller.getMegaphoneActivity().getString(ZonaRosaPinReminders.getSkipReminderString(ZonaRosaStore.pin().getCurrentInterval())));
            }
            ...
            public void onReminderCompleted(@NonNull String pin, boolean includedFailure) {
              ...
              if (includedFailure) {
                ZonaRosaStore.pin().onEntrySuccessWithWrongGuess(pin);
              } else {
                ZonaRosaStore.pin().onEntrySuccess(pin);
              }

              controller.onMegaphoneSnooze(Event.PIN_REMINDER);
              controller.onMegaphoneToastRequested(controller.getMegaphoneActivity().getString(ZonaRosaPinReminders.getReminderString(ZonaRosaStore.pin().getCurrentInterval())));
            }
          });
        })
        .build();
  ...
  private static @NonNull Megaphone buildNewLinkedDeviceMegaphone(@NonNull Context context) {
    String createdAt = DateUtils.getDateTimeString(context, Locale.getDefault(), ZonaRosaStore.misc().getNewLinkedDeviceCreatedTime());
    return new Megaphone.Builder(Event.NEW_LINKED_DEVICE, Megaphone.Style.BASIC)
        ...
        .setActionButton(R.string.NewLinkedDeviceMegaphone__ok, (megaphone, listener) -> {
          ZonaRosaStore.misc().setNewLinkedDeviceId(0);
          ZonaRosaStore.misc().setNewLinkedDeviceCreatedTime(0);
  ...
  private static @NonNull Megaphone buildTurnOffCircumventionMegaphone(@NonNull Context context) {
    return new Megaphone.Builder(Event.TURN_OFF_CENSORSHIP_CIRCUMVENTION, Megaphone.Style.BASIC)
        ...
        .setBody(R.string.CensorshipCircumventionMegaphone_you_can_now_connect_to_the_zonarosa_service)
        .setActionButton(R.string.CensorshipCircumventionMegaphone_turn_off, (megaphone, listener) -> {
          ZonaRosaStore.settings().setCensorshipCircumventionEnabled(false);
  ...
  public static @NonNull Megaphone buildTurnOnZonaRosaBackupsMegaphone() {
    return new Megaphone.Builder(Event.TURN_ON_ZONAROSA_BACKUPS, Megaphone.Style.BASIC)
        .setImage(R.drawable.backups_megaphone_image)
        .setTitle(R.string.TurnOnZonaRosaBackups__title)
        .setBody(R.string.TurnOnZonaRosaBackups__body)
        .setActionButton(R.string.TurnOnZonaRosaBackups__turn_on, (megaphone, controller) -> {
          Intent intent = AppSettingsActivity.remoteBackups(controller.getMegaphoneActivity());

          controller.onMegaphoneNavigationRequested(intent);
          controller.onMegaphoneSnooze(Event.TURN_ON_ZONAROSA_BACKUPS);
        })
        .setSecondaryButton(R.string.TurnOnZonaRosaBackups__not_now, (megaphone, controller) -> {
          controller.onMegaphoneToastRequested(controller.getMegaphoneActivity().getString(R.string.TurnOnZonaRosaBackups__toast_not_now));
          controller.onMegaphoneSnooze(Event.TURN_ON_ZONAROSA_BACKUPS);
        })
        .build();
  }
  ...
  public static @NonNull Megaphone buildVerifyBackupKeyMegaphone() {
    Megaphone.Builder builder = new Megaphone.Builder(Event.VERIFY_BACKUP_KEY, Megaphone.Style.BASIC)
        .setImage(R.drawable.image_zonarosa_backups_key)
    ...
    if (!ZonaRosaStore.backup().getHasSnoozedVerified()) {
      builder.setSecondaryButton(R.string.VerifyBackupKey__not_now, (megaphone, controller) -> {
        ZonaRosaStore.backup().setHasSnoozedVerified(true);
  ...
  private static boolean shouldShowOnboardingMegaphone(@NonNull Context context) {
    return ZonaRosaStore.account().isPrimaryDevice() && ZonaRosaStore.onboarding().hasOnboarding(context);
  }

  private static boolean shouldShowNewLinkedDeviceMegaphone() {
    return ZonaRosaStore.account().isPrimaryDevice() && ZonaRosaStore.misc().getNewLinkedDeviceId() > 0 && !NotificationChannels.getInstance().areNotificationsEnabled();
  }

  private static boolean shouldShowTurnOffCircumventionMegaphone() {
    return AppDependencies.getZonaRosaServiceNetworkAccess().isCensored() &&
           ZonaRosaStore.misc().isServiceReachableWithoutCircumvention();
  }

  private static boolean shouldShowNotificationsMegaphone(@NonNull Context context) {
    boolean shouldShow = !ZonaRosaStore.settings().isMessageNotificationsEnabled() ||
  ...
  private static boolean shouldShowAddAProfilePhotoMegaphone(@NonNull Context context) {
    if (ZonaRosaStore.misc().getHasEverHadAnAvatar()) {
      return false;
    }
  ...
  private static boolean shouldShowSetUpYourUsernameMegaphone(@NonNull Map<Event, MegaphoneRecord> records) {
    boolean                        hasUsername                    = ZonaRosaStore.account().isRegistered() && ZonaRosaStore.account().getUsername() != null;
    boolean                        hasCompleted                   = MapUtil.mapOrDefault(records, Event.SET_UP_YOUR_USERNAME, MegaphoneRecord::isFinished, false);
    long                           phoneNumberDiscoveryDisabledAt = ZonaRosaStore.phoneNumberPrivacy().getPhoneNumberDiscoverabilityModeTimestamp();
    PhoneNumberDiscoverabilityMode listingMode                    = ZonaRosaStore.phoneNumberPrivacy().getPhoneNumberDiscoverabilityMode();

    return ZonaRosaStore.account().isPrimaryDevice() &&
           !hasUsername &&
  ...
  private static boolean shouldShowPnpLaunchMegaphone() {
    return ZonaRosaStore.account().isPrimaryDevice() && TextUtils.isEmpty(ZonaRosaStore.account().getUsername()) && !ZonaRosaStore.uiHints().hasCompletedUsernameOnboarding();
  }
  ...
  private static boolean shouldShowTurnOnBackupsMegaphone(@NonNull Context context) {
    ...
    if (ZonaRosaStore.backup().getLatestBackupTier() != null) {
      return false;
    }

    if (!ZonaRosaStore.account().isRegistered() || ZonaRosaPreferences.isUnauthorizedReceived(context) || ZonaRosaStore.account().isLinkedDevice()) {
      return false;
    }
  ...
  private static boolean shouldShowUseNewOnDeviceBackupsMegaphone() {
    return Environment.Backups.isNewFormatSupportedForLocalBackup() && ZonaRosaStore.settings().isBackupEnabled();
  }
  ...
  private static boolean shouldShowBackupSchedulePermissionMegaphone(@NonNull Context context) {
    boolean backupsEnabled = ZonaRosaStore.settings().isBackupEnabled() || ZonaRosaStore.backup().getAreBackupsEnabled();
    return ZonaRosaStore.account().isPrimaryDevice() && Build.VERSION.SDK_INT >= 31 && backupsEnabled && !ServiceUtil.getAlarmManager(context).canScheduleExactAlarms();
  }
  ...
  public enum Event {
    ...
    TURN_ON_ZONAROSA_BACKUPS("turn_on_zonarosa_backups"),
