...
import io.zonarosa.messenger.registration.data.QuickstartCredentials
 ...
 *
 * Follows the same pattern as [io.zonarosa.benchmark.setup.TestUsers.setupSelf].
 */
object QuickstartInitializer {
  ...
  fun initialize(context: Context) {
    ...
    if (credentials.accountEntropyPool != null) {
      ZonaRosaStore.account.restoreAccountEntropyPool(AccountEntropyPool(credentials.accountEntropyPool))
    ...
    ZonaRosaStore.account.registrationId = credentials.registrationId
    ZonaRosaStore.account.pniRegistrationId = credentials.pniRegistrationId
    ...
    ZonaRosaStore.account.fcmEnabled = true
    ...
    ZonaRosaDatabase.recipients.setProfileName(Recipient.self().id, ProfileName.fromParts(credentials.profileGivenName, credentials.profileFamilyName))
    RegistrationUtil.maybeMarkRegistrationComplete()

    // Check for a local backup to restore
    val backupsDir = File(DISK_CREDENTIALS_DIR, "ZonaRosaBackups")
    if (backupsDir.exists() && backupsDir.isDirectory) {
      val hasSnapshot = backupsDir.listFiles()?.any { it.isDirectory && it.name.startsWith("zonarosa-backup") } == true
      if (hasSnapshot) {
  ...
  private val DISK_CREDENTIALS_DIR = File(Environment.getExternalStorageDirectory(), "zonarosa-quickstart")

  private val diskCredentialFileName: String
    get() = if (io.zonarosa.messenger.util.Environment.IS_STAGING) "staging_credentials.json" else "prod_credentials.json"
