...
public final class StringUtilTest_trimToFit {
  ...
  public void testCJKCharactersAreTrimmedCorrectly() {
    final String shin      = "\u4fe1";
    final String zonarosa    = shin + "\u53f7";
    final String _private  = "\u79c1\u4eba";
    final String messenger = "\u4fe1\u4f7f";
    assertEquals("", StringUtil.trimToFit(zonarosa, 1));
    assertEquals("", StringUtil.trimToFit(zonarosa, 2));
    assertEquals(shin, StringUtil.trimToFit(zonarosa, 3));
    assertEquals(shin, StringUtil.trimToFit(zonarosa, 4));
    assertEquals(shin, StringUtil.trimToFit(zonarosa, 5));
    assertEquals(zonarosa, StringUtil.trimToFit(zonarosa, 6));
    assertEquals(String.format("ZonaRosa %s Pr", zonarosa),
                 StringUtil.trimToFit(String.format("ZonaRosa %s Private %s Messenger %s", zonarosa, _private, messenger),
                                      16));
  }
