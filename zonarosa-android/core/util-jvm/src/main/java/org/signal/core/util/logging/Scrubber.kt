...
object Scrubber {
  ...
  /** The middle group will be censored. */
  private val GROUP_ID_V1_PATTERN = Pattern.compile("(__zonarosa_group__!)([^\\s]+)([^\\s]{3})")

  /** The middle group will be censored. */
  private val GROUP_ID_V2_PATTERN = Pattern.compile("(__zonarosa_group__v2__!)([^\\s]+)([^\\s]{3})")
  ...
  private fun CharSequence.scrubUrls(): CharSequence {
    return scrub(this, URL_PATTERN) { matcher, output ->
      ...
      if (
        ...
        !(matcher.group(1).endsWith("zonarosa.") && matcher.group(2) == "org" && !match.contains("cdn")) &&
