...
The `domain` for a status error generated by the chat server will always be `grpc.chat.zonarosa.io`
...
For errors with domain `grpc.chat.zonarosa.io` and reason `CONSTRAINT_VIOLATED`, clients should check the `details` field for a `BadRequest` proto. If present, they should log the `field` of each violation in `field_violations`.
