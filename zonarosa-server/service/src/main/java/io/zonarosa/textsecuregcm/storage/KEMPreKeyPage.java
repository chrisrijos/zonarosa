...
class KEMPreKeyPage {
  ...
  // The length of libzonarosa's serialized KEM public key, which is a single-byte version followed by the public key
  ...
  // The internal prefix byte libzonarosa uses to indicate a key is of type KEMKeyType.KYBER_1024. Currently, this
  ...
  static ByteBuffer serialize(final byte format, final List<KEMSignedPreKey> preKeys) {
    ...
    for (KEMSignedPreKey preKey : preKeys) {
      ...
      if (publicKeyBytes[0] != KEM_KEY_TYPE_KYBER_1024) {
        // 0x08 is libzonarosa's current KEM key format. If some future version of libzonarosa supports additional KEM
