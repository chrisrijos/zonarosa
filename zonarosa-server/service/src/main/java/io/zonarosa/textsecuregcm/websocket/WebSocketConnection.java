...
import io.zonarosa.server.auth.DisconnectionRequestListener;
...
import io.zonarosa.server.storage.ConflictingMessageConsumerException;
import io.zonarosa.server.storage.Device;
import io.zonarosa.server.storage.MessageStream;
import io.zonarosa.server.storage.MessageStreamEntry;
...
import io.zonarosa.websocket.messages.WebSocketResponseMessage;
...
public class WebSocketConnection implements DisconnectionRequestListener {
  ...
  public void start() {
    ...
    messageSubscription.set(JdkFlowAdapter.flowPublisherToFlux(messageStream.getMessages())
        ...
        // process messages before sending the "connected elsewhere" zonarosa, and while that's ultimately not harmful,
        // it's also not ideal.
  ...
  private CompletableFuture<Void> sendMessage(final Envelope message) {
    ...
    // X-ZonaRosa-Key: false must be sent until Android stops assuming it missing means true
    return client.sendRequest("PUT", "/api/v1/message",
            List.of(HeaderUtils.X_ZONAROSA_KEY + ": false", HeaderUtils.getTimestampHeader()), body)
