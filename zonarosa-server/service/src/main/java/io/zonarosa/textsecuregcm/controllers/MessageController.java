...
import io.zonarosa.server.auth.Anonymous;
import io.zonarosa.server.auth.AuthenticatedDevice;
import io.zonarosa.server.auth.CombinedUnidentifiedSenderAccessKeys;
import io.zonarosa.server.auth.GroupSendTokenHeader;
import io.zonarosa.server.auth.OptionalAccess;
import io.zonarosa.server.auth.UnidentifiedAccessUtil;
import io.zonarosa.server.entities.AccountMismatchedDevices;
import io.zonarosa.server.entities.AccountStaleDevices;
...
import io.zonarosa.server.entities.MismatchedDevicesResponse;
...
import io.zonarosa.server.entities.SendMultiRecipientMessageResponse;
import io.zonarosa.server.entities.SpamReport;
import io.zonarosa.server.entities.StaleDevicesResponse;
...
import io.zonarosa.server.push.MessageUtil;
...
import io.zonarosa.websocket.WebsocketHeaders;
...
public class MessageController {
  ...
  // The ZonaRosa desktop client (really, JavaScript in general) can handle message timestamps at most 100,000,000 days
  ...
  @ApiResponse(
      responseCode="404",
      description="The message is not a story and some the recipient service ID does not correspond to a registered ZonaRosa user")
  @ApiResponse(
  ...
  private void sendSealedSenderMessage(final ServiceIdentifier destinationIdentifier,
    ...
    if (groupSendToken != null) {
      checkGroupSendToken(List.of(destinationIdentifier.toLibzonarosa()), groupSendToken);
    } else {
  ...
  @ApiResponse(
      responseCode="404",
      description="The message is not a story and some of the recipient service IDs do not correspond to registered ZonaRosa users")
  @ApiResponse(
  ...
  public Response sendMultiRecipientMessage(
      ...
      @Parameter(description="The sealed-sender multi-recipient message payload as serialized by libzonarosa")
  ...
  private SendMultiRecipientMessageResponse sendMultiRecipientMessage(final SealedSenderMultiRecipientMessage multiRecipientMessage,
    ...
    // message. Attempt to resolve the destination service identifiers to ZonaRosa accounts.
  ...
  public CompletableFuture<OutgoingMessageEntityList> getPendingMessages(@Auth AuthenticatedDevice auth,
      @HeaderParam(WebsocketHeaders.X_ZONAROSA_RECEIVE_STORIES) String receiveStoriesHeader,
