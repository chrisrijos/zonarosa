...
import static io.zonarosa.server.entities.MessageProtos.Envelope;
...
import io.zonarosa.server.controllers.MismatchedDevices;
import io.zonarosa.server.controllers.MismatchedDevicesException;
import io.zonarosa.server.controllers.MultiRecipientMismatchedDevicesException;
...
import io.zonarosa.server.spam.MessageDeliveryListener;
...
/**
 * A MessageSender sends ZonaRosa messages to destination devices. Messages may be "normal" user-to-user messages,
 * ephemeral ("online") messages like typing indicators, or delivery receipts.
 * <p/>
 * If a client is not actively connected to a ZonaRosa server to receive a message as soon as it is sent, the
 ...
 * isn't actively connected to a ZonaRosa server.
 *
 * @see ReceiptSender
 */
public class MessageSender {
   ...
   * notification token and does not have an active connection to a ZonaRosa server, then this method will also send a
   ...
   * have an active connection to a ZonaRosa server, then this method will also send a push notification to that device to
   ...
   * @param resolvedRecipients a map of recipients to resolved ZonaRosa accounts
  ...
  public CompletableFuture<Void> sendMultiRecipientMessage(final SealedSenderMultiRecipientMessage multiRecipientMessage,
    ...
    multiRecipientMessage.getRecipients().forEach((serviceId, recipient) -> {
      ...
      final ServiceIdentifier serviceIdentifier = ServiceIdentifier.fromLibzonarosa(serviceId);
