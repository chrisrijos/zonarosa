...
import io.zonarosa.server.push.MessageAvailabilityListener;
...
/// for message availability zonarosas from [RedisMessageAvailabilityManager] and emits new messages to its subscriber
/// when available.
///
...
class RedisDynamoDbMessagePublisher implements MessageAvailabilityListener, Flow.Publisher<MessageStreamEntry> {
  ...
  // in Redis, but are eventually "persisted" to DynamoDB. This state changes in response to zonarosas this publisher
  ...
  // Indicates whether we've sent an "initial queue drain complete" zonarosa to subscribers. This state will transition
  // from "not ready" to "pending" as soon as the initial message source subscription completes, and then from "pending"
  // to "send" when the zonarosa has been sent. This state will never roll backwards.
  private QueueEmptyZonaRosaState queueEmptyZonaRosaState = QueueEmptyZonaRosaState.NOT_READY;

  // The total requested demand from the subscriber across the whole lifetime of this publisher
  private long requestedDemand = 0;

  // The total number of zonarosas (new messages or "queue empty" zonarosas) published during the lifetime of this
  // publisher. Must not exceed `requestedDemand`.
  private long publishedEntries = 0;

  // The total number of published zonarosas that have been acknowledged by the subscriber; to avoid re-reading messages,
  ...
  // If terminated (i.e. by an error or by downstream cancellation), this publisher will stop emitting zonarosas. Once
  ...
  // publisher relays zonarosas the message source subscriber to the downstream subscriber. The message source subscriber
  ...
  private enum QueueEmptyZonaRosaState {
    // Indicates that we are not yet ready to send the "initial queue drain complete" zonarosa regardless of outstanding
    // demand
    NOT_READY,

    // Indicates that we are ready to send the "queue empty" zonarosa as soon as demand is available
    PENDING,

    // Indicates that we have sent the "queue empty" zonarosa and must never send it again
    SENT
  }

  /// A message source subscriber subscribes to upstream message source publishers and relays zonarosas to the downstream
  ...
  public synchronized void subscribe(final Flow.Subscriber<? super MessageStreamEntry> subscriber) {
    ...
    // Listen for zonarosas indicating that new messages are available in Redis, that messages have been persisted from
  ...
  public synchronized void handleConflictingMessageConsumer() {
    // We don't register as a listener for conflicting consumer zonarosas until we have a subscriber
  ...
  private synchronized boolean maybeSendQueueEmptyZonaRosa() {
    ...
    if (queueEmptyZonaRosaState == QueueEmptyZonaRosaState.PENDING && publishedEntries < requestedDemand) {
      queueEmptyZonaRosaState = QueueEmptyZonaRosaState.SENT;
      publishedEntries += 1;

      // Subscribers don't explicitly acknowledge "queue empty" zonarosas, and we can consider them automatically
      // acknowledged
      acknowledgedEntries += 1;
  ...
  private synchronized void maybeGenerateMessageSource() {
    ...
    if (acknowledgedEntries < publishedEntries) {
      ...
      // unless all previously-published zonarosas have been acknowledged (when messages are acknowledged, we'll come back
    ...
    // messages; new zonarosas about persisted messages or newly-arrived messages will change this state
  ...
  private synchronized void handleNextMessage(final MessageProtos.Envelope message) {
    ...
    if (!terminated) {
      ...
      // emits a new zonarosa, it should fit within the unfulfilled demand from the downstream subscriber
      assert publishedEntries < requestedDemand;
  ...
  private synchronized void handleMessageSourceComplete() {
    ...
    if (queueEmptyZonaRosaState == QueueEmptyZonaRosaState.NOT_READY) {
      queueEmptyZonaRosaState = QueueEmptyZonaRosaState.PENDING;
      maybeSendQueueEmptyZonaRosa();
    }
  ...
  private synchronized void addDemand(final long demand) {
    ...
    final boolean sentQueueEmptyZonaRosa = maybeSendQueueEmptyZonaRosa();
    ...
    final long newDemand = demand - (sentQueueEmptyZonaRosa ? 1 : 0);
  ...
  private synchronized void terminate() {
    if (!terminated) {
      ...
      // Stop receiving zonarosas about new messages/conflicting consumers
