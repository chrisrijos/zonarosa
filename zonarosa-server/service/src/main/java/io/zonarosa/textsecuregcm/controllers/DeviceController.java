...
import io.zonarosa.server.auth.ChangesLinkedDevices;
import io.zonarosa.server.entities.AccountAttributes;
import io.zonarosa.server.entities.DeviceActivationRequest;
import io.zonarosa.server.entities.DeviceInfo;
import io.zonarosa.server.entities.DeviceInfoList;
import io.zonarosa.server.entities.LinkDeviceRequest;
import io.zonarosa.server.entities.LinkDeviceResponse;
import io.zonarosa.server.entities.PreKeySignatureValidator;
import io.zonarosa.server.entities.ProvisioningMessage;
import io.zonarosa.server.entities.RemoteAttachment;
import io.zonarosa.server.entities.RemoteAttachmentError;
import io.zonarosa.server.entities.RestoreAccountRequest;
import io.zonarosa.server.entities.SetPublicKeyRequest;
import io.zonarosa.server.entities.TransferArchiveUploadedRequest;
import io.zonarosa.server.identity.IdentityType;
import io.zonarosa.server.limits.RateLimitedByIp;
import io.zonarosa.server.limits.RateLimiters;
import io.zonarosa.server.metrics.DevicePlatformUtil;
...
import io.zonarosa.server.storage.LinkDeviceTokenAlreadyUsedException;
...
import io.zonarosa.server.util.ExceptionUtils;
import io.zonarosa.server.util.LinkDeviceToken;
import io.zonarosa.server.util.Pair;
...
public class DeviceController {
  ...
  @Operation(
      summary = "ZonaRosas that a new device is requesting restoration of account data by some method",
      description = """
          ZonaRosas that a new device is requesting restoration of account data by some method. Devices waiting via the
  ...
  @Operation(
      summary = "ZonaRosas that a transfer archive has been uploaded for a specific linked device",
      description = """
          ZonaRosas that a transfer archive has been uploaded or failed for a specific linked device. Devices waiting via
          the "wait for transfer archive" endpoint will be notified that the new archive is available.

          If the uploader cannot upload the transfer archive, they must zonarosa an error.
          """)
  @ApiResponse(responseCode = "204", description = "Success")
  @ApiResponse(responseCode = "422", description = "The request object could not be parsed or was otherwise invalid")
  @ApiResponse(responseCode = "429", description = "Rate-limited; try again after the prescribed delay")
