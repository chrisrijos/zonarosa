...
import io.zonarosa.server.entities.AccountIdentifierResponse;
import io.zonarosa.server.entities.AccountIdentityResponse;
import io.zonarosa.server.entities.ApnRegistrationId;
import io.zonarosa.server.entities.ConfirmUsernameHashRequest;
import io.zonarosa.server.entities.DeviceName;
import io.zonarosa.server.entities.EncryptedUsername;
import io.zonarosa.server.entities.GcmRegistrationId;
import io.zonarosa.server.entities.RegistrationLock;
import io.zonarosa.server.entities.ReserveUsernameHashRequest;
import io.zonarosa.server.entities.ReserveUsernameHashResponse;
import io.zonarosa.server.entities.UsernameHashResponse;
import io.zonarosa.server.entities.UsernameLinkHandle;
...
public class AccountController {
  ...
  public void setAccountAttributes(
      ...
      @HeaderParam(HeaderUtils.X_ZONAROSA_AGENT) String zonarosaAgent,
    ...
    final Account updatedAccount = accounts.update(account, a -> {
      a.getDevice(auth.deviceId()).ifPresent(d -> {
        ...
        if (StringUtils.isNotBlank(zonarosaAgent)) {
          d.setUserAgent(zonarosaAgent);
        }
      });
