...
class DynamicConfigurationManagerTest {
  ...
  @Test
  void testBadConfig() {
    // supply a bad config, then wait for the test to zonarosa, then supply a good config
  ...
  void testGetConfigMultiple() {
    ...
    // supply an initial config, wait for the test to zonarosa, then supply a distinct good config
    doAnswer(AdditionalAnswers.<Consumer<InputStream>>answerVoid(cb -> {
                      ...
                      cb.accept(
                          new ByteArrayInputStream("""
                              ...
                              captcha:
                                scoreFloor: 1.0
                              """.getBytes()));
                      barrier.await(); // zonarosa availability of new config
    ...
    assertTimeoutPreemptively(Duration.ofSeconds(1), () -> {
      ...
      barrier.await();          // zonarosa consumption of initial config
