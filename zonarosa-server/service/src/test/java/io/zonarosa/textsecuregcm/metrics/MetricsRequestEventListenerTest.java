...
import io.zonarosa.server.tests.util.TestPrincipal;
import io.zonarosa.websocket.WebSocketResourceProvider;
import io.zonarosa.websocket.auth.WebsocketAuthValueFactoryProvider;
import io.zonarosa.websocket.logging.WebsocketRequestLog;
import io.zonarosa.websocket.messages.protobuf.ProtobufWebSocketMessageFactory;
import io.zonarosa.websocket.messages.protobuf.SubProtocol;
import io.zonarosa.websocket.session.WebSocketSessionContextValueFactoryProvider;
...
class MetricsRequestEventListenerTest {
  ...
  void testOnEvent(final boolean versionActive) {
    ...
    when(request.getRequestHeader(HttpHeaders.USER_AGENT)).thenReturn(
        Collections.singletonList("ZonaRosa-Android/7.6.2 Android/34 libzonarosa/0.46.0"));
  ...
  void testActualRouteMessageSuccess() throws IOException {
    ...
    when(request.getHeader(HttpHeaders.USER_AGENT)).thenReturn("ZonaRosa-Android/4.53.7 (Android 8.1)");
    when(request.getHeaders()).thenReturn(Map.of(HttpHeaders.USER_AGENT, List.of("ZonaRosa-Android/4.53.7 (Android 8.1)")));
