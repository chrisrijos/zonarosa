...
///         or a linked device.
/// 4) On your OD, you will accept the prompt in the ZonaRosa app to enter transfer mode.
...
class DeviceTransferService: NSObject, DeviceTransferServiceProtocol {
    ...
    private let serialQueue = DispatchQueue(label: "io.zonarosa.device-transfer")
...
class DeviceTransferServiceMock: DeviceTransferServiceProtocol {
    func startAcceptingTransfersFromOldDevices(mode: ZonaRosa.DeviceTransferService.TransferMode) throws -> URL {
