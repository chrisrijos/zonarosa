...
final class CallService: CallServiceStateObserver, CallServiceStateDelegate {
    typealias CallManagerType = CallManager<ZonaRosaCall, CallService>
    ...
    init(
        ...
        mutableCurrentCall: AtomicValue<ZonaRosaCall?>,
        ...
        self.callManager = CallManager<ZonaRosaCall, CallService>(
    ...
    func callServiceState(_ callServiceState: CallServiceState, didTerminateCall call: ZonaRosaCall) {
    ...
    private func updateIsLocalAudioMutedWithMicrophonePermission(call: ZonaRosaCall, isLocalAudioMuted: Bool) {
    ...
    private func updateIsLocalVideoMutedWithCameraPermissions(call: ZonaRosaCall, isLocalVideoMuted: Bool) {
        owsPrecondition(call === callServiceState.currentCall)
    ...
    func updateCameraSource(call: ZonaRosaCall, isUsingFrontCamera: Bool) {
    ...
    func handleFailedCall(failedCall: ZonaRosaCall, error: Error) {
    ...
    func buildAndConnectGroupCall(for groupId: GroupIdentifier, isVideoMuted: Bool) -> (ZonaRosaCall, GroupThreadCall)? {
        return _buildAndConnectGroupCall(isOutgoingVideoMuted: isVideoMuted) { () -> (ZonaRosaCall, GroupThreadCall)? in
            ...
            return (ZonaRosaCall(groupThreadCall: groupThreadCall), groupThreadCall)
        }
    }
    ...
    enum CallLinkStateRetrievalStrategy {
        case reuse(ZonaRosaServiceKit.CallLinkState)
        case fetch
    }
    ...
    func buildAndConnectCallLinkCall(
    ...
    ) async throws -> (ZonaRosaCall, CallLinkCall)? {
        let state: ZonaRosaServiceKit.CallLinkState
        ...
        return _buildAndConnectGroupCall(isOutgoingVideoMuted: false) { () -> (ZonaRosaCall, CallLinkCall)? in
            let videoCaptureController = VideoCaptureController()
            let sfuUrl = DebugFlags.callingUseTestSFU.get() ? TSConstants.sfuTestURL : TSConstants.sfuURL
            ...
            return (ZonaRosaCall(callLinkCall: callLinkCall), callLinkCall)
        }
    }

    private func _buildAndConnectGroupCall<T: GroupCall>(
        isOutgoingVideoMuted: Bool,
        createCall: () -> (ZonaRosaCall, T)?,
    ) -> (ZonaRosaCall, T)? {
    ...
    func joinGroupCallIfNecessary(_ call: ZonaRosaCall, groupCall: GroupCall) {
    ...
    private func leaveAndTerminateGroupCall(_ call: ZonaRosaCall, groupCall: GroupCall) {
        if groupCall.hasInvokedConnectMethod {
    ...
    func buildOutgoingIndividualCallIfPossible(thread: TSContactThread, localDeviceId: DeviceId, hasVideo: Bool) -> (ZonaRosaCall, IndividualCall)? {
        ...
        let call = ZonaRosaCall(individualCall: individualCall)
    ...
    private func shouldReorientUI(for call: ZonaRosaCall) -> Bool {
...
extension CallService: CallManagerDelegate {
    typealias CallManagerDelegateCallType = ZonaRosaCall
     ...
     */
    func callManager(
        _ callManager: CallManager<ZonaRosaCall, CallService>,
    ...
    private func sendCallMessage(
        ...
        to recipientAci: Aci,
        callAtStart: ZonaRosaCall?,
    ) async {
        do {
            let sendPromise = await databaseStorage.awaitableWrite { transaction in
                let thread = TSContactThread.getOrCreateThread(
                    withContactAddress: ZonaRosaServiceAddress(recipientAci),
    ...
    func callManager(
        ...
        urgency: ZonaRosaRingRTC.CallMessageUrgency,
        expiration: Date,
    ...
    private func publishUntrustedIdentityErrorIfNeeded(_ error: any Error, callAtStart: ZonaRosaCall?) {
    ...
    func callManager(
        _ callManager: CallManager<ZonaRosaCall, CallService>,
        shouldStartCall call: ZonaRosaCall,
        callId: UInt64,
        isOutgoing: Bool,
    ...
    func callManager(
        _ callManager: ZonaRosaRingRTC.CallManager<ZonaRosaCall, CallService>,
        onCallEnded call: ZonaRosaCall,
        callId: UInt64,
    ...
    func callManager(
        _ callManager: CallManager<ZonaRosaCall, CallService>,
        onNetworkRouteChangedFor call: ZonaRosaCall,
    ...
    nonisolated func callManager(
        _ callManager: CallManager<ZonaRosaCall, CallService>,
        onAudioLevelsFor call: ZonaRosaCall,
    ...
    func callManager(
        _ callManager: CallManager<ZonaRosaCall, CallService>,
        onLowBandwidthForVideoFor call: ZonaRosaCall,
        recovered: Bool,
    ) {
    ...
    func callManager(
        _ callManager: CallManager<ZonaRosaCall, CallService>,
        onUpdateLocalVideoSession call: ZonaRosaCall,
    ...
    func callManager(
        _ callManager: CallManager<ZonaRosaCall, CallService>,
        onAddRemoteVideoTrack call: ZonaRosaCall,
        track: RTCVideoTrack,
    ) {
        individualCallService.callManager(
            callManager,
