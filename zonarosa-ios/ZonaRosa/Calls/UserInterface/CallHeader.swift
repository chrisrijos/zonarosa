...
class CallHeader: UIView {
    ...
    private func incomingRingText(caller: Aci) -> String {
        let callerName = SSKEnvironment.shared.databaseStorageRef.read { transaction in
            SSKEnvironment.shared.contactManagerRef.displayName(for: ZonaRosaServiceAddress(caller), tx: transaction).resolvedValue(useShortNameIfAvailable: true)
        }
    ...
    private func whoIsHereText(joinedMembers: [UUID?]) -> String {
        ...
        let upToTwoKnownMemberNames: [String] = SSKEnvironment.shared.databaseStorageRef.read { tx -> [String] in
            joinedMembers
                .lazy
                .compactMap { $0 }
                .map { SSKEnvironment.shared.contactManagerRef.displayName(for: ZonaRosaServiceAddress(Aci(fromUUID: $0)), tx: tx) }
    ...
    private func callTitleLabelText() -> String {
            ...
            firstMember.presenting == true
        {
            let presentingName = SSKEnvironment.shared.databaseStorageRef.read { tx in
                SSKEnvironment.shared.contactManagerRef.displayName(for: ZonaRosaServiceAddress(Aci(fromUUID: firstMember.userId)), tx: tx).resolvedValue(useShortNameIfAvailable: true)
            }
            let formatString = OWSLocalizedString(
