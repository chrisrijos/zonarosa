...
final class GroupCallViewController: UIViewController {
    ...
    private let call: ZonaRosaCall
    private let groupCall: GroupCall
    private let ringRtcCall: ZonaRosaRingRTC.GroupCall
    ...
    private var membersAtJoin: Set<ZonaRosaServiceAddress>?
    ...
    static func load(call: ZonaRosaCall, groupCall: GroupCall, tx: DBReadTransaction) -> GroupCallViewController {
    ...
    private static func _prepareLobby(
        ...
        buildAndStartConnecting: () async throws -> (ZonaRosaCall, GroupCall)?,
    ) async rethrows -> (() -> Void)? {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if let splitViewSnapshot = ZonaRosaApp.shared.snapshotSplitViewController(afterScreenUpdates: false) {
    ...
    private func didHangupCall() {
        ...
        guard
            let splitViewSnapshot = ZonaRosaApp.shared.snapshotSplitViewController(afterScreenUpdates: false),
    ...
    private func otherUsersProfileChanged(notification: Notification) {
        ...
        guard
            let changedAddress = notification.userInfo?[UserProfileNotifications.profileAddressKey] as? ZonaRosaServiceAddress,
        ...
        if let peekInfo = self.ringRtcCall.peekInfo {
            ...
            if
                joinedAndPendingMembers.contains(where: { uuid in
                    changedAddress == ZonaRosaServiceAddress(Aci(fromUUID: uuid))
                })
            {
...
extension GroupCallViewController: CallViewControllerWindowReference {
    ...
    var remoteVideoAddress: ZonaRosaServiceAddress {
    ...
    func returnFromPip(pipWindow: UIWindow) {
        ...
        guard let splitViewSnapshot = ZonaRosaApp.shared.snapshotSplitViewController(afterScreenUpdates: false) else {
    ...
    private func safetyNumberMismatchAddresses(untrustedThreshold: Date?) -> [ZonaRosaServiceAddress] {
        SSKEnvironment.shared.databaseStorageRef.read { transaction in
            let addressesToCheck: [ZonaRosaServiceAddress]
            if
                case .groupThread(let groupThreadCall) = groupCall.concreteType,
