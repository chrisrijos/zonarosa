...
class CallAudioService: IndividualCallObserver, GroupCallObserver {
    ...
    func groupCallPeekChanged(_ call: GroupCall) {
        ...
        // (iOS 15)
        // CMSUtility_DeactivateTimerHandler: Deactivating client 'sid:<ID>, ZonaRosa(<pid>), 'prim'' because it has not been playing for 8 minutes
        // (iOS 18)
        // CMSUtility_DeactivateTimerHandler: INTERRUPTING client 'sid:<ID>, ZonaRosa(<pid>), 'prim'' because there has been no activity since <time> ( 8 minutes )
        //
    ...
    @MainActor
    func requestSpeakerphone(call: ZonaRosaCall, isEnabled: Bool) {
        switch call.mode {
    ...
    @MainActor
    private func ensureProperAudioSession(call: ZonaRosaCall) {
        switch call.mode {
