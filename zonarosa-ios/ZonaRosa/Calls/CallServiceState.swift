...
class CallServiceState {
    ...
    init(currentCall: AtomicValue<ZonaRosaCall?>) {
    ...
    private let _currentCall: AtomicValue<ZonaRosaCall?>

    /// Represents the call currently occuring on this device.
    var currentCall: ZonaRosaCall? { _currentCall.get() }

    @MainActor
    func setCurrentCall(_ currentCall: ZonaRosaCall?) {
     ...
     */
    @MainActor
    func terminateCall(_ call: ZonaRosaCall) {
