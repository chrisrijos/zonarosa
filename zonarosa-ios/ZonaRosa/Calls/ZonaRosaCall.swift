...
enum CallMode {
    ...
    var caller: ZonaRosaServiceAddress? {
        switch self {
        case .individual(let call):
        ...
        case .groupThread(let call):
            ...
            return ZonaRosaServiceAddress(caller)
        case .callLink:
            return nil
        }
    }
...
enum CallError: Error {
    ...
    func shouldSilentlyDropCall() -> Bool {
        switch self {
        case .providerReset, .disconnected, .externalError, .timeout, .zonarosaing:
            return false
...
class ZonaRosaCall: CallManagerCallReference {
    let mode: CallMode
    ...
    var caller: ZonaRosaServiceAddress? { mode.caller }
