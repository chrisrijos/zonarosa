...
final class IndividualCallService: CallServiceStateObserver {
    ...
    @MainActor
    func didUpdateCall(from oldValue: ZonaRosaCall?, to newValue: ZonaRosaCall?) {
        stopAnyCallTimer()
        if let newValue {
     ...
     */
    @MainActor
    func handleOutgoingCall(_ call: ZonaRosaCall) {
     ...
     */
    @MainActor
    func handleAcceptCall(_ call: ZonaRosaCall) {
        Logger.info("\(call)")
     ...
     */
    @MainActor
    func handleLocalHangupCall(_ call: ZonaRosaCall) {
        Logger.info("\(call)")
    ...
    func handleReceivedOffer(
        ...
        let newCall = ZonaRosaCall(individualCall: individualCall)
    ...
    @MainActor
    func callManager(_ callManager: CallService.CallManagerType, shouldStartCall call: ZonaRosaCall, callId: UInt64, isOutgoing: Bool, callMediaType: CallMediaType, shouldEarlyRing: Bool) {
        Logger.info("call: \(call)")
        ...
        Task { @MainActor in
            do {
                ...
                let isZonaRosaConnection = self.databaseStorage.read { tx in
                    return profileManager.isThread(inProfileWhitelist: call.individualCall.thread, transaction: tx)
                }
                if !isZonaRosaConnection {
                    Logger.warn("Using relay server because remote user is not a ZonaRosa Connection")
                }

                let useTurnOnly = !isZonaRosaConnection || self.preferences.doCallsHideIPAddress
    ...
    @MainActor
    func callManager(_ callManager: CallService.CallManagerType, onCallEnded call: ZonaRosaCall, callId: UInt64, reason: CallEndReason, summary: CallSummary) {
        ...
        case .zonarosaingFailure:
            handleFailedCall(failedCall: call, error: CallError.zonarosaing, shouldResetUI: true, shouldResetRingRTC: false)
    ...
    @MainActor
    func callManager(_ callManager: CallService.CallManagerType, onEvent call: ZonaRosaCall, event: CallManagerEvent) {
    ...
    @MainActor
    func callManager(_ callManager: CallService.CallManagerType, onUpdateLocalVideoSession call: ZonaRosaCall, session: AVCaptureSession?) {
    ...
    @MainActor
    func callManager(_ callManager: CallService.CallManagerType, onAddRemoteVideoTrack call: ZonaRosaCall, track: RTCVideoTrack) {
    ...
    // MARK: - Call Manager ZonaRosaing

    @MainActor
    func callManager(_ callManager: CallService.CallManagerType, shouldSendOffer callId: UInt64, call: ZonaRosaCall, destinationDeviceId: UInt32?, opaque: Data, callMediaType: CallMediaType) {
        ...
        Task {
            do {
                ...
                try self.callManager.zonarosaingMessageDidSend(callId: callId)
            } catch {
                Logger.error("failed to send offer message to \(call.individualCall.thread.contactAddress) with error: \(error)")
                self.callManager.zonarosaingMessageDidFail(callId: callId)
            }
        }
    }

    @MainActor
    func callManager(_ callManager: CallService.CallManagerType, shouldSendAnswer callId: UInt64, call: ZonaRosaCall, destinationDeviceId: UInt32?, opaque: Data) {
    ...
    @MainActor
    func callManager(_ callManager: CallService.CallManagerType, shouldSendIceCandidates callId: UInt64, call: ZonaRosaCall, destinationDeviceId: UInt32?, candidates: [Data]) {
        ...
        Task {
            do {
                ...
                callManager.zonarosaingMessageDidFail(callId: callId)
            }
        }
    }

    @MainActor
    func callManager(_ callManager: CallService.CallManagerType, shouldSendHangup callId: UInt64, call: ZonaRosaCall, destinationDeviceId: UInt32?, hangupType: HangupType, deviceId: UInt32) {
    ...
    @MainActor
    func callManager(_ callManager: CallService.CallManagerType, shouldSendBusy callId: UInt64, call: ZonaRosaCall, destinationDeviceId: UInt32?) {
     ...
     */
    @MainActor
    func handleMissedCall(_ call: ZonaRosaCall, error: CallError? = nil) {
        Logger.info("call: \(call)")
    ...
    @MainActor
    func handleAnsweredElsewhere(call: ZonaRosaCall) {
    ...
    @MainActor
    func handleDeclinedElsewhere(call: ZonaRosaCall) {
    ...
    @MainActor
    func handleBusyElsewhere(call: ZonaRosaCall) {
     ...
     */
    @MainActor
    private func handleRinging(call: ZonaRosaCall, isAnticipatory: Bool = false) {
    ...
    @MainActor
    private func handleReconnecting(call: ZonaRosaCall) {
        Logger.info("call: \(call)")
    ...
    @MainActor
    private func handleReconnected(call: ZonaRosaCall) {
        Logger.info("call: \(call)")
     ...
     */
    @MainActor
    private func handleConnected(call: ZonaRosaCall) {
     ...
     */
    @MainActor
    func setIsOnHold(call: ZonaRosaCall, isOnHold: Bool) {
        Logger.info("call: \(call)")
    ...
    @MainActor
    func cleanUpStaleCall(_ staleCall: ZonaRosaCall, function: StaticString = #function, line: UInt = #line) {
    ...
    @MainActor
    func handleFailedCall(failedCall: ZonaRosaCall, error: Error, shouldResetUI: Bool, shouldResetRingRTC: Bool) {
        Logger.debug("")
    ...
    @MainActor
    func ensureAudioState(call: ZonaRosaCall) {
    ...
    @MainActor
    func startCallTimer(for call: ZonaRosaCall) {
    ...
    @MainActor
    private func ensureCallScreenPresented(call: ZonaRosaCall, hasUsedUpTimerSlop: inout Bool) {
...
private extension ZonaRosaCall {
    ...
    /// - Important
    /// Callers must be *sure* that this `ZonaRosaCall` represents an individual
