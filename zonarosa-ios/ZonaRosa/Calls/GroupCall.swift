...
protocol GroupCallObserver: AnyObject {
    ...
    func groupCallReceivedReactions(_ call: GroupCall, reactions: [ZonaRosaRingRTC.Reaction])
...
class GroupCall: ZonaRosaRingRTC.GroupCallDelegate {
    enum Constants {
    ...
    let ringRtcCall: ZonaRosaRingRTC.GroupCall
    ...
    @MainActor
    func groupCall(onLocalDeviceStateChanged groupCall: ZonaRosaRingRTC.GroupCall) {
    ...
    @MainActor
    func groupCall(onRemoteDeviceStatesChanged groupCall: ZonaRosaRingRTC.GroupCall) {
    ...
    func groupCall(onAudioLevels groupCall: ZonaRosaRingRTC.GroupCall) {
        // TODO: Implement audio level handling for group calls.
    }

    func groupCall(onLowBandwidthForVideo groupCall: ZonaRosaRingRTC.GroupCall, recovered: Bool) {
        // TODO: Implement handling of the "low outgoing bandwidth for video" notification.
    }

    func groupCall(onReactions groupCall: ZonaRosaRingRTC.GroupCall, reactions: [ZonaRosaRingRTC.Reaction]) {
        observers.elements.forEach { $0.groupCallReceivedReactions(self, reactions: reactions) }
    }

    func groupCall(onRaisedHands groupCall: ZonaRosaRingRTC.GroupCall, raisedHands: [DemuxId]) {
    ...
    @MainActor
    func groupCall(onPeekChanged groupCall: ZonaRosaRingRTC.GroupCall) {
        observers.elements.forEach { $0.groupCallPeekChanged(self) }
    }

    @MainActor
    func groupCall(requestMembershipProof groupCall: ZonaRosaRingRTC.GroupCall) {
    }

    @MainActor
    func groupCall(requestGroupMembers groupCall: ZonaRosaRingRTC.GroupCall) {
    }

    @MainActor
    func groupCall(onEnded groupCall: ZonaRosaRingRTC.GroupCall, reason: CallEndReason, summary: CallSummary) {
    ...
    @MainActor
    func groupCall(onSpeakingNotification groupCall: ZonaRosaRingRTC.GroupCall, event: SpeechEvent) {
        // TODO: Implement speaking notification handling for group calls.
    }

    @MainActor
    func groupCall(onRemoteMuteRequest groupCall: ZonaRosaRingRTC.GroupCall, muteSource: UInt32) {
    ...
    @MainActor
    func groupCall(onObservedRemoteMute groupCall: ZonaRosaRingRTC.GroupCall, muteSource: UInt32, muteTarget: UInt32) {
