...
class UserProfileTest: ZonaRosaBaseTest {
    override func setUp() {
        super.setUp()
    ...
    func testGetUserProfile() {
        let addresses: [ZonaRosaServiceAddress] = [
            ZonaRosaServiceAddress(Aci.randomForTesting()),
            ZonaRosaServiceAddress(Pni.randomForTesting()),
            ZonaRosaServiceAddress(phoneNumber: "+16505550101"),
        ]
        write { tx in
        ...
        read { tx in
            let actual = OWSUserProfile.getUserProfile(for: .otherUser(ZonaRosaServiceAddress.randomForTesting()), tx: tx)
            XCTAssertNil(actual)
        }
    }
    ...
    func testGetFor() {
        ...
        read { tx in
            let addressesToFetch: [ZonaRosaServiceAddress] = addresses + [ZonaRosaServiceAddress.randomForTesting()]
