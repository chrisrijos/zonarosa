...
class OWSContactsManagerTest: ZonaRosaBaseTest {
    ...
    private func makeAndInsertRecipient(address: ZonaRosaServiceAddress) -> ZonaRosaRecipient {
        return self.dbV2.write { tx in
            return try! ZonaRosaRecipient.insertRecord(aci: address.aci, phoneNumber: address.e164, tx: tx)
        }
    }
    ...
    private func createAccounts(_ accounts: [ZonaRosaAccount]) {
    ...
    func testGetDisplayNamesWithNicknames() throws {
        let aliceAddress = ZonaRosaServiceAddress(phoneNumber: "+16505550101")
        let bobAddress = ZonaRosaServiceAddress(serviceId: Aci.randomForTesting(), phoneNumber: "+16505550102")
    ...
    func testGetDisplayNamesWithCachedContactNames() {
        let addresses = [
            ZonaRosaServiceAddress(serviceId: Aci.randomForTesting(), phoneNumber: "+16505550100"),
            ZonaRosaServiceAddress(serviceId: Aci.randomForTesting(), phoneNumber: "+16505550101"),
        ]
    ...
    func testGetDisplayNamesWithProfileFullNames() {
        let addresses = [ZonaRosaServiceAddress.randomForTesting(), ZonaRosaServiceAddress.randomForTesting()]
    ...
    func testGetDisplayNamesWithPhoneNumbers() {
        let addresses = [
            ZonaRosaServiceAddress(phoneNumber: "+17035559900"),
            ZonaRosaServiceAddress(phoneNumber: "+17035559901"),
        ]
    ...
    func testGetDisplayNamesWithUserNames() {
        ...
        let addresses = [ZonaRosaServiceAddress(aliceAci), ZonaRosaServiceAddress(bobAci)]
    ...
    func testGetDisplayNamesMixed() throws {
        let aliceAci = Aci.randomForTesting()
        let aliceAddress = ZonaRosaServiceAddress(serviceId: aliceAci, phoneNumber: "+16505550100")
        ...
        let bobAddress = ZonaRosaServiceAddress.randomForTesting()
        (SSKEnvironment.shared.profileManagerRef as! OWSFakeProfileManager).fakeUserProfiles = [
            bobAddress: makeUserProfile(givenName: "Bob", familyName: "Bobson"),
        ]

        let carolAddress = ZonaRosaServiceAddress(phoneNumber: "+17035559900")

        let daveAci = Aci.randomForTesting()
        let daveAddress = ZonaRosaServiceAddress(daveAci)
        dbV2.write { transaction in
            mockUsernameLookupMananger.saveUsername("dave", forAci: daveAci, transaction: transaction)
        }

        let eveAddress = ZonaRosaServiceAddress.randomForTesting()

        let feliciaAci = Aci.randomForTesting()
        let feliciaAddress = ZonaRosaServiceAddress(serviceId: feliciaAci, phoneNumber: "+16505550109")
    ...
    func testCachedContactNameMixed() {
        ...
        let chuckAddress = ZonaRosaServiceAddress(serviceId: chuckAci, phoneNumber: "+16505550101")
