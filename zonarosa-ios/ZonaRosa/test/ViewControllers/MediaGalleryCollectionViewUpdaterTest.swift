...
class CollectionViewLogger: MediaGalleryCollectionViewUpdaterDelegate {
    ...
    func updaterDeleteSections(_ sections: ZonaRosa.MediaGallerySectionIndexSet) {
        log.append(.deleteSections(sections.indexSet))
    }

    func updaterDeleteItems(at indexPaths: [ZonaRosa.MediaGalleryIndexPath]) {
        log.append(.deleteItems(indexPaths.map { $0.indexPath }))
    }

    func updaterInsertSections(_ sections: ZonaRosa.MediaGallerySectionIndexSet) {
        log.append(.insertSections(sections.indexSet))
    }

    func updaterReloadItems(at indexPaths: [ZonaRosa.MediaGalleryIndexPath]) {
        log.append(.reloadItems(indexPaths.map { $0.indexPath }))
    }

    func updaterReloadSections(_ sections: ZonaRosa.MediaGallerySectionIndexSet) {
