...
final class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    private let flushQueue = DispatchQueue(label: "io.zonarosa.flush", qos: .utility)
    ...
    func application(
        ...
        DebugLogger.registerLibzonarosa()
    ...
    private func setUpMainAppEnvironment(
        ...
        let _currentCall = AtomicValue<ZonaRosaCall?>(nil, lock: .init())
    ...
    private func setAppIsReady(
        ...
        let remoteMegaphoneFetcher = RemoteMegaphoneFetcher(
            ...
            zonarosaService: SSKEnvironment.shared.zonarosaServiceRef,
        )
        cron.schedulePeriodically(
        ...
        ZonaRosaApp.shared.showLaunchInterface(
            launchInterface,
            appReadiness: appReadiness,
    ...
    private func showPreflightErrorUI(
        ...
        case .databaseCorrupted:
            title = OWSLocalizedString(
                ...
                comment: "Title for an action sheet explaining that ZonaRosa can't launch because the database is corrupted.",
            )
            message = OWSLocalizedString(
                "APP_LAUNCH_FAILURE_DATABASE_CORRUPTED_MESSAGE",
                comment: "Message for an action sheet explaining that ZonaRosa can't launch because the database is corrupted.",
            )
            actions = [
    ...
    private func presentLaunchFailureActionSheet(
        ...
        if DebugFlags.internalSettings {
            actionSheet.addAction(.init(title: "Export Database (internal)") { [unowned viewController] _ in
                ZonaRosaApp.shared.showExportDatabaseUI(from: viewController) {
                        ...
                        OWSActionSheets.showConfirmationAlert(
                            ...
                            proceedAction: { _ in
                                ModalActivityIndicatorViewController.present(fromViewController: viewController) { _ in
                                    ZonaRosaApp.shared.resetAppDataAndExit(keyFetcher: keyFetcher)
                                }
                            },
                        )
                    },
    ...
    private func refreshConnection(isAppActive: Bool, shouldRunCron: Bool) {
        ...
        if isAppActive {
            ...
            let isPastRegistration = ZonaRosaApp.shared.conversationSplitViewController != nil
    ...
    func application(
        ...
        case "INSendMessageIntent":
            ...
            appReadiness.runNowOrWhenAppDidBecomeReadySync {
                ...
                ZonaRosaApp.shared.presentConversationAndScrollToFirstUnreadMessage(
    ...
    func application(
        ...
        appReadiness.runNowOrWhenUIDidBecomeReadySync {
            ...
            ZonaRosaApp.shared.showNewConversationView()
    ...
    static func applicationShortcutItems(isRegistered: Bool) -> [UIApplicationShortcutItem] {
        ...
        return [.init(
            ...
            localizedTitle: OWSLocalizedString(
                ...
                comment: "On the iOS home screen, if you tap and hold the ZonaRosa icon, this shortcut will appear. Tapping it will let users send a new message. You may want to refer to similar behavior in other iOS apps, such as Messages, for equivalent strings.",
            ),
