...
/// thread recipient with all known ZonaRosa accounts.
public class ContactThreadNameCollisionFinder: NameCollisionFinder {
    ...
    public func findCollisions(transaction: DBReadTransaction) -> [NameCollision] {
        ...
        let candidateAddresses = { () -> [ZonaRosaServiceAddress] in
            var result = Set<ZonaRosaServiceAddress>()
            result.formUnion(SSKEnvironment.shared.profileManagerRef.allWhitelistedRegisteredAddresses(tx: transaction))
            // Include all ZonaRosaAccounts as well (even though most are redundant) to
            // ensure we check against blocked system contact names.
            result.formUnion(ZonaRosaAccount.anyFetchAll(transaction: transaction).map { $0.recipientAddress })
...
public class GroupMembershipNameCollisionFinder: NameCollisionFinder {
    ...
    let lock = UnfairLock()
    private var recentProfileUpdateMessages: [ZonaRosaServiceAddress: [TSInfoMessage]]?
    ...
    private func fetchRecentProfileUpdates(transaction: DBReadTransaction) -> [ZonaRosaServiceAddress: [TSInfoMessage]] {
        lock.withLock {
            ...
            // Build a map from (ZonaRosaServiceAddress) -> (List of recent profile update messages)
            let results: [ZonaRosaServiceAddress: [TSInfoMessage]] = finder
    ...
    func standardSort(readTx: DBReadTransaction) -> NameCollision {
        NameCollision(
            elements.sorted { lhs, rhs in
                ...
                // - ZonaRosaServiceAddress UUID otherwise, to ensure stable sorting
