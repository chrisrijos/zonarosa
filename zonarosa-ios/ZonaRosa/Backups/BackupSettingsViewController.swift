        ...
        let disabledSheet = HeroSheetViewController(
            ...
            primaryButton: HeroSheetViewController.Button(
                title: OWSLocalizedString(
                    ...
                    comment: "Title for a button that takes the users to ZonaRosa's iOS Settings page.",
                ),
                action: { sheet in
    ...
    var body: some View {
        ZonaRosaList {
            if viewModel.backupSubscriptionAlreadyRedeemed {
                ZonaRosaSection {
                    HStack(alignment: .center, spacing: 16) {
                        Image(.backupErrorBold)
                            ...
                            .foregroundStyle(Color.ZonaRosa.orange)
                        ...
                        .font(.subheadline)
                        .foregroundColor(Color.ZonaRosa.label)
                ...
                .listRowBackground(Color.ZonaRosa.quaternaryFill)
            }
            ...
            case .enabled:
                    ...
                    if viewModel.isBackgroundAppRefreshDisabled {
                        Label {
                            Text(OWSLocalizedString(
                                ...
                                comment: "Message describing that the Background App Refresh permission is disabled for ZonaRosa. \"Background App Refresh\" should be localized the same way it is in iOS Settings app permissions.",
                            ))
                            .appendLink(
                                OWSLocalizedString(
                                    "BACKUP_SETTINGS_BACKGROUND_APP_REFRESH_DISABLED_MESSAGE_UPDATE_NOW",
                                    comment: "Add-on to a message describing that the Background App Refresh permission is disabled for ZonaRosa. \"Background App Refresh\" should be localized the same way it is in iOS Settings app permissions.",
                                ),
                                useBold: true,
                                tint: .ZonaRosa.label,
                                action: {
                    ...
                    if let latestBackupExportProgressUpdate = viewModel.latestBackupExportProgressUpdate {
                        ...
                        VStack(alignment: .leading) {
                            Label {
                                ...
                                .appendLink(CommonStrings.learnMore, useBold: true, tint: .ZonaRosa.label) {
                                    CurrentAppContext().open(
                                        URL.Support.backups,
                                        completion: nil,
                                    )
                                ...
                                .font(.subheadline)
                                .foregroundStyle(Color.ZonaRosa.label)
                                .monospacedDigit()
                                .multilineTextAlignment(.leading)
                            } icon: {
                ...
                ZonaRosaSection {
                    Button {
                        ...
                        .foregroundStyle(Color.ZonaRosa.red)
                    }
                } footer: {
                    Text(OWSLocalizedString(
    ...
    var body: some View {
        VStack {
            ...
            Circle()
                ...
                .foregroundStyle(Color.ZonaRosa.yellow)
            Spacer()
        }
        ...
        VStack {
            ...
            case .determinate(let percentComplete):
                PulsingProgressBar(value: percentComplete)
                    .tint(.ZonaRosa.accent)
                    .clipShape(RoundedRectangle(cornerRadius: 3))
            case .indeterminate:
                LottieView(animation: .named("linear_indeterminate"))
                    .playing(loopMode: .loop)
                    .background {
                        Capsule().fill(Color.ZonaRosa.secondaryFill)
                    }
                    .clipShape(RoundedRectangle(cornerRadius: 3))
            }
        }
