...
extension ConversationViewController {
    ...
    private func didLongPressLink(dataItem: TextCheckingDataItem) {
        ...
        } else if ZonaRosaProxy.isValidProxyLink(dataItem.url) {
            actionSheet.addAction(ActionSheetAction(
                title: OWSLocalizedString("MESSAGE_ACTION_LINK_OPEN_PROXY", comment: "Label for button to open a zonarosa proxy."),
                style: .default,
                handler: { [weak self] _ in
    ...
    private func didLongPressPhoneNumber(dataItem: TextCheckingDataItem) {
        ...
        let blockedAddress = ZonaRosaServiceAddress(phoneNumber: phoneNumber)
    ...
    private func didLongPressEmail(dataItem: TextCheckingDataItem) {
        ...
        // TODO: We could show (Send ZonaRosa Message/ZonaRosa call) actions for this email address.
        //       Ideally we could detect whether this email address corresponds to a system contact
        //       which is a registered ZonaRosa user.
    ...
    private func didTapOrLongPressMention(_ mentionAci: Aci) {
        ...
        showMemberActionSheet(forAddress: ZonaRosaServiceAddress(mentionAci), withHapticFeedback: true)
    }
