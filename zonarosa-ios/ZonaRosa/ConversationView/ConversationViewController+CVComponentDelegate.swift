...
extension ConversationViewController: CVComponentDelegate {
    ...
    func handleUrl(_ url: URL) {
        ...
        if ZonaRosaProxy.isValidProxyLink(url) {
            didTapProxyLink(url: url)
            return
        }

        if ZonaRosaDotMePhoneNumberLink.isPossibleUrl(url) {
            cvc_didTapZonaRosaMeLink(url: url)
            return
        }
    ...
    public func cvc_didTapZonaRosaMeLink(url: URL) {
        ZonaRosaDotMePhoneNumberLink.openChat(url: url, fromViewController: self)
    }
    ...
    public func didTapUsernameLink(usernameLink: Usernames.UsernameLink) {
        Task {
            ...
            ZonaRosaApp.shared.presentConversationForAddress(
    ...
    public func didTapPreviouslyVerifiedIdentityChange(_ address: ZonaRosaServiceAddress) {
        AssertIsOnMainThread()

        showFingerprint(address: address)
    }

    public func showFingerprint(address: ZonaRosaServiceAddress) {
        AssertIsOnMainThread()
    ...
    public func didTapUnverifiedIdentityChange(_ address: ZonaRosaServiceAddress) {
        AssertIsOnMainThread()
    ...
    public func didTapSessionRefreshMessage(_ message: TSErrorMessage) {
        ...
        OWSActionSheets.showContactSupportActionSheet(
            ...
            emailFilter: .custom("ZonaRosa iOS Session Refresh"),
            fromViewController: self,
        )
    }
    ...
    public func didTapShowFingerprint(_ address: ZonaRosaServiceAddress) {
        AssertIsOnMainThread()
        ...
        alert.addAction(
            ActionSheetAction(
                title: OWSLocalizedString(
                    ...
                    comment: "Action to block contact in ZonaRosa for sheet shown when the user taps a missed call from a contact blocked in iOS settings.",
                ),
                style: .destructive,
            ) { [weak self] _ in
    ...
    public func didTapUpdateSystemContact(_ address: ZonaRosaServiceAddress, newNameComponents: PersonNameComponents) {
    ...
    public func didTapPhoneNumberChange(aci: Aci, phoneNumberOld: String, phoneNumberNew: String) {
        SUIEnvironment.shared.contactsViewHelperRef.checkEditAuthorization(
            performWhenAllowed: {
                ...
                let address = ZonaRosaServiceAddress(serviceId: aci, phoneNumber: phoneNumberNew)
                SUIEnvironment.shared.contactsViewHelperRef.presentSystemContactsFlow(
    ...
    public func didTapActivatePayments() {
        AssertIsOnMainThread()
        ZonaRosaApp.shared.showAppSettings(mode: .payments)
    }
