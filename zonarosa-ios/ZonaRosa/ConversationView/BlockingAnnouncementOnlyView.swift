...
class BlockingAnnouncementOnlyView: ConversationBottomPanelView {
    ...
    init(thread: TSThread, fromViewController: UIViewController, forceDarkMode: Bool = false) {
        ...
        attributedString.setAttributes([.foregroundColor: UIColor.ZonaRosa.link], forSubstring: adminsText)
    ...
    private func fetchGroupAdminAddresses(tx: DBReadTransaction) -> [ZonaRosaServiceAddress] {
        guard
        ...
        return contactManager.sortZonaRosaServiceAddresses(groupAdminAddresses, transaction: tx)
...
class MessageUserSubsetSheet: OWSTableSheetViewController {
    private let addresses: [ZonaRosaServiceAddress]
    private let forceDarkMode: Bool
    private let groupThread: TSGroupThread?

    init(addresses: [ZonaRosaServiceAddress], forceDarkMode: Bool, groupThread: TSGroupThread?) {
