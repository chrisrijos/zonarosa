        ...
        case .backgroundedDevice:
            progressView.renderError(
                text: OWSLocalizedString(
                    ...
                    comment: "An error indicating that the other device closed zonarosa mid-transfer and it could not complete",
                ),
            )
        case .cancel:
    ...
    init() {
        super.init(
            ...
            primaryButton: .init(title: OWSLocalizedString(
                ...
                comment: "Button for bottom sheet shown when device transfer completes on the receiving device. Tapping will terminate the ZonaRosa app and trigger a notification to relaunch.",
            )) { _ in
