    ...
    public enum Mode: Codable {
        ...
        public struct ChangeNumberState: Codable {
            ...
            public struct PendingPniState {
                ...
                public let localDevicePniSignedPreKeyRecord: Result<LibZonaRosaClient.SignedPreKeyRecord, DecodingError>
                public let localDevicePniPqLastResortPreKeyRecord: Result<LibZonaRosaClient.KyberPreKeyRecord, DecodingError>
...
extension RegistrationCoordinatorLoaderImpl.Mode.ChangeNumberState.PendingPniState: Codable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        if let modernValue = try container.decodeIfPresent(Data.self, forKey: .localDevicePniPqLastResortPreKeyRecordData) {
            self.localDevicePniPqLastResortPreKeyRecord = .success(try LibZonaRosaClient.KyberPreKeyRecord(bytes: modernValue))
        } else if
        ...
        {
            self.localDevicePniPqLastResortPreKeyRecord = .success(try LibZonaRosaClient.KyberPreKeyRecord(bytes: deprecatedValue.keyData))
        } else {
        ...
        if let modernValue = try container.decodeIfPresent(Data.self, forKey: .localDevicePniSignedPreKeyRecordData) {
            self.localDevicePniSignedPreKeyRecord = .success(try LibZonaRosaClient.SignedPreKeyRecord(bytes: modernValue))
        } else if
            BuildFlags.decodeDeprecatedPreKeys,
            let deprecatedValue = try container.decodeIfPresent(Data.self, forKey: .localDevicePniSignedPreKeyRecord)
        {
            guard let signedPreKeyRecord = try NSKeyedUnarchiver.unarchivedObject(ofClass: ZonaRosaServiceKit.SignedPreKeyRecord.self, from: deprecatedValue) else {
                throw DecodingError.dataCorruptedError(forKey: .localDevicePniSignedPreKeyRecord, in: container, debugDescription: "")
            }
            self.localDevicePniSignedPreKeyRecord = .success(try LibZonaRosaClient.SignedPreKeyRecord(
