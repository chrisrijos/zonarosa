...
class BankTransferMandateViewController: OWSTableViewController2 {
    ...
    private static let learnMoreURL = URL(string: "https://support.zonarosa.io/")!

    private func makeHeaderView() -> UIView {
        let bankIconView = UIImageView(image: UIImage(systemName: "building.columns"))
        bankIconView.contentMode = .scaleAspectFit
        bankIconView.tintColor = .ZonaRosa.label

        let bankIconCircle = CircleView()
        bankIconCircle.backgroundColor = .ZonaRosa.secondaryGroupedBackground
        ...
        let titleLabel = UILabel.title1Label(text: OWSLocalizedString(
            "BANK_MANDATE_TITLE",
            comment: "Users can donate to ZonaRosa with a bank account. We are required to show them a mandate with information about bank transfers. This is the title above that mandate.",
        ))
        ...
        subtitleTextView.attributedText = .composed(of: [
            OWSLocalizedString(
                ...
                comment: "Users can donate to ZonaRosa with a bank account. We are required to show them a mandate with information about bank transfers. This is a subtitle about the payment processor Stripe above that mandate.",
            ),
            " ",
            CommonStrings.learnMore.styled(with: .link(Self.learnMoreURL)),
        ]).styled(with: .color(.ZonaRosa.secondaryLabel), .font(.dynamicTypeSubheadline))
    ...
    private func updateBottomFooterButtonText() {
        ...
        if isScrolledCloseToBottom {
            title = OWSLocalizedString(
                "BANK_MANDATE_AGREE",
                comment: "Users can donate to ZonaRosa with a bank account. We are required to show them a mandate with information about bank transfers. This is a label for a button to agree to the mandate.",
            )
        } else {
            title = OWSLocalizedString(
                "BANK_MANDATE_READ_MORE",
                comment: "Users can donate to ZonaRosa with a bank account. We are required to show them a mandate with information about bank transfers. This is a label for a button that shows more of the mandate if it is not all visible.",
            )
        }
...
extension Stripe.PaymentMethod.Mandate {
    fileprivate static func accept() -> Self {
        .init(mode: .online(
            userAgent: OWSURLSession.userAgentHeaderValueZonaRosaIos,
            ipAddress: "0.0.0.0",
        ))
    }
}
