    ...
    private func updateTableContents() {
        ...
        SSKEnvironment.shared.databaseStorageRef.read { transaction in
            ...
            if let contactThread = thread as? TSContactThread {
                ...
                let zonarosaRecipient = recipientDatabaseTable.fetchRecipient(address: address, tx: transaction)

                section.add(.copyableItem(
                    label: "ACI",
                    value: zonarosaRecipient?.aci?.serviceIdString,
                ))

                section.add(.copyableItem(
                    label: "Phone Number",
                    value: zonarosaRecipient?.phoneNumber?.stringValue,
                ))

                section.add(.copyableItem(
                    label: "PNI",
                    value: zonarosaRecipient?.pni?.serviceIdString,
                ))

                section.add(.copyableItem(
                    label: "Discoverable Phone Number?",
                    value: zonarosaRecipient?.phoneNumber?.isDiscoverable == true ? "Yes" : "No",
                ))
