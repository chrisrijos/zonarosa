...
class ConversationSettingsViewController: OWSTableViewController2, BadgeCollectionDataSource, MemberLabelViewControllerPresenter {
    ...
    private(set) var groupMemberStateMap = [ZonaRosaServiceAddress: VerificationState]()
    private(set) var sortedGroupMembers = [ZonaRosaServiceAddress]()
    func updateGroupMembers(transaction tx: DBReadTransaction) {
        ...
        var allMembersSorted = [ZonaRosaServiceAddress]()
        var verificationStateMap = [ZonaRosaServiceAddress: VerificationState]()
        ...
        allMembersSorted = SSKEnvironment.shared.contactManagerImplRef.sortZonaRosaServiceAddresses(allMembers, transaction: tx)

        var membersToRender = [ZonaRosaServiceAddress]()
    ...
    func reloadThreadAndUpdateContent() {
        let didUpdate = SSKEnvironment.shared.databaseStorageRef.read { tx -> Bool in
            ...
            self.isSystemContact = {
                ...
                return SSKEnvironment.shared.contactManagerRef.fetchZonaRosaAccount(for: address, transaction: tx) != nil
            }()
    ...
    func didSelectGroupMember(_ memberAddress: ZonaRosaServiceAddress) {
    ...
    func presentCreateOrEditContactViewController(address: ZonaRosaServiceAddress, editImmediately: Bool) {
    ...
    private func presentAddToExistingContactFlow(address: ZonaRosaServiceAddress) {
        SUIEnvironment.shared.contactsViewHelperRef.presentSystemContactsFlow(
    ...
    func showReplaceAdminView(candidates: Set<ZonaRosaServiceAddress>) {
    ...
    private var replacementAdminCandidates: Set<ZonaRosaServiceAddress> {
        guard let groupThread = thread as? TSGroupThread else {
            owsFailDebug("Invalid thread.")
            return []
        }
    ...
    private func otherUsersProfileDidChange(notification: Notification) {
        ...
        guard
            let address = notification.userInfo?[UserProfileNotifications.profileAddressKey] as? ZonaRosaServiceAddress,
            address.isValid
        else {
