...
struct ProfileSheetSheetCoordinator {
    private let address: ZonaRosaServiceAddress
...
class MemberActionSheet: OWSTableSheetViewController {
    ...
    fileprivate static func fetchThreadViewModel(address: ZonaRosaServiceAddress) -> ThreadViewModel {
    ...
    private func viewSystemContactDetails(contactAddress: ZonaRosaServiceAddress) {
    ...
    private func showAddToSystemContactsActionSheet(contactAddress: ZonaRosaServiceAddress) {
        guard let viewController = fromViewController else { return }
        let contactsViewHelper = SUIEnvironment.shared.contactsViewHelperRef
