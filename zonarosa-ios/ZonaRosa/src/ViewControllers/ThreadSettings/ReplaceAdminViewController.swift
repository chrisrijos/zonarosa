...
class ReplaceAdminViewController: OWSTableViewController2 {
    ...
    private let candidates: Set<ZonaRosaServiceAddress>

    init(
        candidates: Set<ZonaRosaServiceAddress>,
    ...
    private func updateTableContents() {
        ...
        let sortedCandidates = SSKEnvironment.shared.databaseStorageRef.read { tx in
            SSKEnvironment.shared.contactManagerImplRef.sortZonaRosaServiceAddresses(self.candidates, transaction: tx)
        }
    ...
    private func candidateWasSelected(candidate: ZonaRosaServiceAddress) {
