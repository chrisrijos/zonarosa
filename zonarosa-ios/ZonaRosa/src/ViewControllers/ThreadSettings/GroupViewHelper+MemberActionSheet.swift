...
extension GroupViewHelper {
    ...
    func memberActionSheetCanMakeGroupAdmin(address: ZonaRosaServiceAddress) -> Bool {
        guard
    ...
    @MainActor
    func memberActionSheetMakeGroupAdminWasSelected(address: ZonaRosaServiceAddress) {
    ...
    func memberActionSheetCanRevokeGroupAdmin(address: ZonaRosaServiceAddress) -> Bool {
        guard
            let groupThread = thread as? TSGroupThread,
            groupThread.isGroupV2Thread
        else {
    ...
    func canRemoveFromGroup(address: ZonaRosaServiceAddress) -> Bool {
        guard
            let groupThread = thread as? TSGroupThread,
            groupThread.isGroupV2Thread
        else {
    ...
    @MainActor
    func presentRemoveFromGroupActionSheet(address: ZonaRosaServiceAddress) {
        let titleFormat = OWSLocalizedString(
