...
class AddToExistingContactFlow: SystemContactsFlow {
    ...
    init(address: ZonaRosaServiceAddress, nameComponents: PersonNameComponents? = nil) {
        self.address = address
        self.nameComponents = nameComponents
    }
}
...
extension ContactsViewHelper {
    ...
    fileprivate func contactViewController(for systemContactsFlow: SystemContactsFlow) -> CNContactViewController {
        ...
        let zonarosaAccount = SSKEnvironment.shared.databaseStorageRef.read { tx in
            return SSKEnvironment.shared.contactManagerRef.fetchZonaRosaAccount(for: address, transaction: tx)
        }
        ...
        if cnContact == nil, let cnContactId = zonarosaAccount?.cnContactId {
