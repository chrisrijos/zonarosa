...
class NicknameEditorViewController: OWSTableViewController2 {
    ...
    private let recipient: ZonaRosaRecipient
    private let initialNicknameRecord: NicknameRecord?

    static func create(for address: ZonaRosaServiceAddress, context: Context, tx: DBReadTransaction) -> NicknameEditorViewController? {
        guard
