...
struct ConversationHeaderBuilder {
    ...
    static func buildHeaderForContact(
        ...
        if isVerified {
            ...
            subtitle.append(ZonaRosaSymbol.safetyNumber.attributedString(for: .subheadline, clamped: true))
            subtitle.append(" ")
        ...
        if options.contains(.message) {
            buttons.append(buildIconButton(
                ...
                action: { [weak delegate] in
                    ...
                    ZonaRosaApp.shared.dismissAllModals(animated: true, completion: {
    ...
    static func threadAttributedString(
        ...
        if canTap {
            let chevron = ZonaRosaSymbol.chevronTrailing(for: threadName).attributedString(
    ...
    func threadAttributedString(renderLocalUserAsNoteToSelf: Bool, tx: DBReadTransaction) -> NSAttributedString {
        ...
        let isSystemContact =
            if let contactThread = self.thread as? TSContactThread {
                SSKEnvironment.shared.contactManagerRef.fetchZonaRosaAccount(
