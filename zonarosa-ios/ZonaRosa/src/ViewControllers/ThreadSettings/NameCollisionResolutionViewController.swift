...
class NameCollisionResolutionViewController: OWSTableViewController2 {
    ...
    private func unblock(address: ZonaRosaServiceAddress) {
    ...
    private func blockAddress(_ address: ZonaRosaServiceAddress) {
    ...
    private func removeFromGroup(_ address: ZonaRosaServiceAddress) {
    ...
    private func presentUpdateContactViewController(for address: ZonaRosaServiceAddress) {
...
extension NameCollisionResolutionViewController: ContactsViewHelperObserver {
    ...
    func shouldShowContactUpdateAction(for address: ZonaRosaServiceAddress) -> Bool {
        guard SSKEnvironment.shared.contactManagerImplRef.isEditingAllowed else {
            return false
        }
        return SSKEnvironment.shared.databaseStorageRef.read { transaction in
            return SSKEnvironment.shared.contactManagerRef.fetchZonaRosaAccount(for: address, transaction: transaction) != nil
        }
    }
