...
public class GroupMemberRequestsAndInvitesViewController: OWSTableViewController2 {
    ...
    private func addContentsForMemberRequests(contents: OWSTableContents) {
        ...
        let requestingMembersSorted = SSKEnvironment.shared.databaseStorageRef.read { tx in
            SSKEnvironment.shared.contactManagerImplRef.sortZonaRosaServiceAddresses(groupMembership.requestingMembers, transaction: tx)
        }
    ...
    private func buildMemberRequestButtons(address: ZonaRosaServiceAddress) -> ContactCellAccessoryView {
    ...
    private func approveMemberRequest(address: ZonaRosaServiceAddress) {
        showAcceptMemberRequestUI(address: address)
    }

    private func denyMemberRequest(address: ZonaRosaServiceAddress) {
    ...
    private func addContentsForPendingInvites(contents: OWSTableContents) {
        ...
        var membersInvitedByLocalUser = [ZonaRosaServiceAddress]()
        var membersInvitedByOtherUsers = [Aci: [ZonaRosaServiceAddress]]()
        ...
        membersInvitedByLocalUser = databaseStorage.read { tx in
            return contactManager.sortZonaRosaServiceAddresses(membersInvitedByLocalUser, transaction: tx)
        }
        ...
        if membersInvitedByOtherUsers.isEmpty {
            ...
            var inviterAddresses = membersInvitedByOtherUsers.keys.map(ZonaRosaServiceAddress.init(_:))
            inviterAddresses = databaseStorage.read { tx in
                return contactManager.sortZonaRosaServiceAddresses(inviterAddresses, transaction: tx)
            }
    ...
    private func showRevokePendingInviteFromLocalUserConfirmation(invitedAddress: ZonaRosaServiceAddress) {
    ...
    private func showRevokePendingInviteFromOtherUserConfirmation(
        invitedAddresses: [ZonaRosaServiceAddress],
        inviterAddress: ZonaRosaServiceAddress,
    ) {
    ...
    private func showMemberActionSheet(for address: ZonaRosaServiceAddress) {
    ...
    private func presentRequestApprovedToast(address: ZonaRosaServiceAddress) {
        let format = OWSLocalizedString(
    ...
    private func presentRequestDeniedToast(address: ZonaRosaServiceAddress) {
        let format = OWSLocalizedString(
...
private extension GroupMemberRequestsAndInvitesViewController {
    ...
    func revokePendingInvites(addresses: [ZonaRosaServiceAddress]) {
...
private extension GroupMemberRequestsAndInvitesViewController {
    ...
    func showAcceptMemberRequestUI(address: ZonaRosaServiceAddress) {
    ...
    func showDenyMemberRequestUI(address: ZonaRosaServiceAddress) {
    ...
    func acceptOrDenyMemberRequests(address: ZonaRosaServiceAddress, shouldAccept: Bool) {
