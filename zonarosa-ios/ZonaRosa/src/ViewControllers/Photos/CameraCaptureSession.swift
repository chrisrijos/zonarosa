...
class CameraCaptureSession: NSObject {
    ...
    private static let sessionQueue = DispatchQueue(label: "io.zonarosa.capture.camera")
...
private class VideoCapture: NSObject, AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureAudioDataOutputSampleBufferDelegate {
    ...
    private let videoCaptureQueue = DispatchQueue(label: "io.zonarosa.capture.video", qos: .userInteractive)
    private let audioCaptureQueue = DispatchQueue(label: "io.zonarosa.capture.audio", qos: .userInteractive)
    private let recordingQueue = DispatchQueue(label: "io.zonarosa.capture.recording", qos: .userInteractive)
    ...
    func beginRecording(aspectRatio: CGFloat, includeAudio: Bool) throws {
        ...
        // video specs from ZonaRosa-Android: 2Mbps video 192K audio, 720P 30 FPS
