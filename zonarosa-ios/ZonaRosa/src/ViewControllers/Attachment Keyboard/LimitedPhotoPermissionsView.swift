...
class LimitedPhotoPermissionsView: UIView {
    private let button: UIButton = {
        let selectMoreAction = UIAction(
            title: OWSLocalizedString(
                ...
                comment: "Button in a context menu from the 'manage' button in attachment panel that allows to select more photos/videos to give ZonaRosa access to",
            ),
        ...
        let settingsAction = UIAction(
            title: OWSLocalizedString(
                ...
                comment: "Button in a context menu from the 'manage' button in attachment panel that opens the iOS system settings for ZonaRosa to update access permissions",
            ),
        ...
        let button = UIButton(configuration: .smallSecondary(title: OWSLocalizedString(
            ...
            comment: "Button in chat attachment panel that allows to select photos/videos ZonaRosa has access to.",
        )))
    ...
    init() {
        ...
        let label = UILabel()
        label.textColor = UIColor.ZonaRosa.secondaryLabel
        label.font = .dynamicTypeSubheadlineClamped
        label.numberOfLines = 2
        label.setContentHuggingHorizontalLow()
        label.text = OWSLocalizedString(
            "ATTACHMENT_KEYBOARD_LIMITED_ACCESS",
            comment: "Text in chat attachment panel when ZonaRosa only has access to some photos/videos. This string is in a compact horizontal space, so it should be short if possible.",
        )
