...
extension PollDetailsViewController: PollDetailsViewModel.ActionsDelegate {
    ...
    func presentContactSheet(address: ZonaRosaServiceAddress) {
...
struct PollDetailsView: View {
                ...
                ForEach(poll.sortedOptions()) { option in
                    ZonaRosaSection {
                        ForEach(option.acis, id: \.self) { aci in
                            ContactRow(
                                address: ZonaRosaServiceAddress(aci),
                                onTap: { address in
                        ...
                        if option.acis.count == 0 {
                            ...
                            .font(.body)
                            .foregroundColor(Color.ZonaRosa.secondaryLabel)
                        }
                    } header: {
                        HStack {
                            Text(option.text)
    ...
    struct ContactRow: UIViewRepresentable {
        ...
        var onTap: (ZonaRosaServiceAddress) -> Void
        ...
        private func addressCell(address: ZonaRosaServiceAddress) -> ManualStackView? {
...
extension PollDetailsViewController: DatabaseChangeDelegate {
    func databaseChangesDidUpdate(databaseChanges: any ZonaRosaServiceKit.DatabaseChanges) {
