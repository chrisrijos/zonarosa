...
class DonationReceiptViewController: OWSTableViewController2 {
    ...
    let zonarosaLogoView: UIImageView = {
    ...
    private func updateZonaRosaLogoImage() {
        let zonarosaLogoImage = UIImage(named: "zonarosa-full-logo")
        if traitCollection.userInterfaceStyle == .dark {
            zonarosaLogoView.image = zonarosaLogoImage?.tintedImage(color: .ows_white)
        } else {
            zonarosaLogoView.image = zonarosaLogoImage
        }
    }
    ...
    private func amountSection() -> OWSTableSection {
        OWSTableSection(items: [
            OWSTableItem(customCellBlock: {
                ...
                amountLabel.textColor = .ZonaRosa.label
                amountLabel.font = .dynamicTypeLargeTitle1Clamped
                amountLabel.adjustsFontForContentSizeCategory = true

                let content = UIStackView(arrangedSubviews: [self.zonarosaLogoView, amountLabel])
    ...
    private class DonationReceiptImageActivityItemProvider: UIActivityItemProvider, @unchecked Sendable {
        ...
        private class func headerView() -> UIView {
            let zonarosaLogo = UIImage(named: "zonarosa-full-logo")
            let zonarosaLogoView = UIImageView(image: zonarosaLogo)

            zonarosaLogoView.autoSetDimensions(to: CGSize(width: 140, height: 40))
            ...
            let headerView = UIStackView(arrangedSubviews: [zonarosaLogoView, currentDateView])
