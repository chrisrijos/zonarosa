...
class AdvancedPrivacySettingsViewController: OWSTableViewController2 {
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(updateTableContents),
            name: .isZonaRosaProxyReadyDidChange,
            object: nil,
        )
    }
    ...
    private func updateTableContents() {
        ...
        if SSKEnvironment.shared.zonarosaServiceRef.hasCensoredPhoneNumber {
            isCensorshipCircumventionSwitchEnabled = true
            if SSKEnvironment.shared.zonarosaServiceRef.isCensorshipCircumventionManuallyDisabled {
            ...
            !SSKEnvironment.shared.zonarosaServiceRef.isCensorshipCircumventionActive,
            DependenciesBridge.shared.chatConnectionManager.unidentifiedConnectionState == .open
        {
            ...
            censorshipCircumventionSection.footerTitle = OWSLocalizedString(
                ...
                comment: "Table footer for the 'censorship circumvention' section shown when the app is connected to the ZonaRosa service.",
            )
        } else if !SSKEnvironment.shared.zonarosaServiceRef.isCensorshipCircumventionActive, !SSKEnvironment.shared.reachabilityManagerRef.isReachable {
            isCensorshipCircumventionSwitchEnabled = false
            censorshipCircumventionSection.footerTitle = OWSLocalizedString(
        ...
        censorshipCircumventionSection.add(.switch(
            ...
            isOn: { SSKEnvironment.shared.zonarosaServiceRef.isCensorshipCircumventionActive },
        ...
        if SSKEnvironment.shared.zonarosaServiceRef.isCensorshipCircumventionManuallyActivated {
        ...
        proxySection.add(.disclosureItem(
            ...
            accessoryText: ZonaRosaProxy.isEnabled ? CommonStrings.switchOn : CommonStrings.switchOff,
            actionBlock: { [weak self] in
        ...
        if DependenciesBridge.shared.tsAccountManager.registrationStateWithMaybeSneakyTransaction.isRegisteredPrimaryDevice {
            ...
            sealedSenderSection.footerAttributedTitle = NSAttributedString.composed(of: [
                ...
                CommonStrings.learnMore.styled(
                    with: .link(URL(string: "https://zonarosa.io/blog/sealed-sender/")!),
                ),
            ])
            .styled(with: defaultFooterTextStyle)
    ...
    private func didToggleEnableCensorshipCircumventionSwitch(_ sender: UISwitch) {
        SSKEnvironment.shared.zonarosaServiceRef.isCensorshipCircumventionManuallyDisabled = !sender.isOn
        SSKEnvironment.shared.zonarosaServiceRef.isCensorshipCircumventionManuallyActivated = sender.isOn
        updateTableContents()
    }

    private func ensureManualCensorshipCircumventionCountry() -> OWSCountryMetadata {
        let preferredCountryCode = SSKEnvironment.shared.zonarosaServiceRef.manualCensorshipCircumventionCountryCode ?? Locale.current.regionCode

        if
            let preferredCountryCode,
            let countryMetadata = OWSCountryMetadata.countryMetadata(countryCode: preferredCountryCode)
        {
            SSKEnvironment.shared.zonarosaServiceRef.manualCensorshipCircumventionCountryCode = preferredCountryCode
            return countryMetadata
        } else {
            let fallbackCountryCode = "US"
            SSKEnvironment.shared.zonarosaServiceRef.manualCensorshipCircumventionCountryCode = fallbackCountryCode
