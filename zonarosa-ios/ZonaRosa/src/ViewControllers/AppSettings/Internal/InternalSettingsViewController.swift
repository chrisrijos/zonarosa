...
class InternalSettingsViewController: OWSTableViewController2 {
    ...
    func updateTableContents() {
        ...
        if let buildDetails = Bundle.main.object(forInfoDictionaryKey: "BuildDetails") as? [String: AnyObject] {
            if let zonarosaCommit = (buildDetails["ZonaRosaCommit"] as? String)?.strippedOrNil?.prefix(12) {
                buildSection.add(.copyableItem(label: "Git Commit", value: String(zonarosaCommit)))
            }
        }
        ...
        otherSection.add(.copyableItem(label: "CC?", value: SSKEnvironment.shared.zonarosaServiceRef.isCensorshipCircumventionActive ? "Yes" : "No"))
