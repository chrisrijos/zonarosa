...
public final class MemberLabelCoordinator {
    ...
    private func buildGroupMemberLabelsWithoutLocalUser() -> [ZonaRosaServiceAddress: MemberLabelForRendering] {
        var memberLabels: [ZonaRosaServiceAddress: MemberLabelForRendering] = [:]
        ...
        let changeLabelBlock: () -> Void = {
            Task { @MainActor in
                let localUserBio = self.db.read(block: { tx -> String? in
                    return self.profileManager.userProfile(for: ZonaRosaServiceAddress(self.localIdentifiers.aci), tx: tx)?.bioForDisplay
                })
