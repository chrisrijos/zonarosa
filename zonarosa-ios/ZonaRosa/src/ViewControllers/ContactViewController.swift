...
class ContactViewController: OWSTableViewController2 {
    ...
    private enum ContactViewMode {
        case systemContactWithZonaRosa
        case systemContactWithoutZonaRosa
    ...
    private static func viewMode(for phoneNumberPartition: OWSContact.PhoneNumberPartition) -> ContactViewMode {
        return phoneNumberPartition.map(
            ifSendablePhoneNumbers: { _ in .systemContactWithZonaRosa },
            elseIfInvitablePhoneNumbers: { _ in .systemContactWithoutZonaRosa },
    ...
    private func showInviteToZonaRosa() -> Bool {
        switch viewMode {
        case .systemContactWithoutZonaRosa, .nonSystemContact:
            return true
        default:
            return false
        }
    ...
    private func updateContent() {
        ...
        // Message, Video, Audio buttons for ZonaRosa contacts as a horizontal stack of buttons
        if viewMode == .systemContactWithZonaRosa {
            ...
            if isRegistered {
                func addAction(title: String, action: ConversationViewAction) {
                    actionSheet.addAction(ActionSheetAction(
                        ...
                        style: .default,
                        handler: { _ in
                            let address = ZonaRosaServiceAddress(phoneNumber: phoneNumber)
                            ZonaRosaApp.shared.presentConversationForAddress(address, action: action, animated: true)
                        },
                    ))
                }
