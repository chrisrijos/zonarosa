...
class LongTextViewController: OWSViewController {
    ...
    private let textView: UITextView = {
        ...
        textView.textColor = .ZonaRosa.label
        textView.backgroundColor = .ZonaRosa.background
    ...
    private func loadContent() {
        ...
        if #available(iOS 17, *) {
            textColor = UIColor.ZonaRosa.label.resolvedColor(with: textView.traitCollection)
        } else {
        ...
        for item in linkItems {
            if item.range.contains(characterIndex) {
                ...
                case .mention(let mentionItem):
                    ...
                    let address = ZonaRosaServiceAddress(mentionItem.mentionAci)
