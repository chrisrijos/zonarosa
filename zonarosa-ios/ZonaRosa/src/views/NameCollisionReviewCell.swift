    ...
    func collisionCellModels(
        ...
        elements.map {
            return NameCollisionCellModel(
                ...
                isSystemContact: contactsManager.fetchZonaRosaAccount(for: $0.address, transaction: tx) != nil,
...
final class NameCollisionCell: UITableViewCell {
    ...
    let separatorView: UIView = {
        let hairline = UIView()
        hairline.backgroundColor = .ZonaRosa.opaqueSeparator
    ...
    func configure(model: NameCollisionCellModel, action: NameCollisionCell.Action?) {
        ...
        if model.isConnection {
            verticalStack.addArrangedSubview(ProfileDetailLabel.zonarosaConnectionLink(
                font: detailFont,
