...
class LinkPreviewFetchStateTest: XCTestCase {
    ...
    func testUpdateLoaded() async throws {
        ...
        let validURL = try XCTUnwrap(URL(string: "https://zonarosa.io"))
        ...
        for textValue in ["Check ou https://zonarosa.io", "Check out https://zonarosa.io"] {
            await linkPreviewFetchState.update(.init(text: textValue, ranges: .empty))?.value
        ...
        let invalidURL = try XCTUnwrap(URL(string: "https://zonarosa.io/not_found"))
        mockLinkPreviewFetcher.fetchLinkPreviewBlock = { fetchedURL in
            XCTAssertEqual(fetchedURL, invalidURL)
            throw OWSGenericError("Not found.")
        }
        for textValue in ["Check ou https://zonarosa.io/not_found", "Check out https://zonarosa.io/not_found"] {
            await linkPreviewFetchState.update(.init(text: textValue, ranges: .empty))?.value
    ...
    func testUpdateLoading() async throws {
        ...
        let task1 = linkPreviewFetchState.update(.init(text: "https://zonarosa.io is a grea", ranges: .empty))
        XCTAssert(linkPreviewFetchState.currentState.isLoading)

        // If there's a request in flight, we shouldn't send a new request.
        let task2 = linkPreviewFetchState.update(.init(text: "https://zonarosa.io is a great", ranges: .empty))
        XCTAssert(linkPreviewFetchState.currentState.isLoading)
    ...
    func testUpdateObsolete() async throws {
        ...
        let url1 = try XCTUnwrap(URL(string: "https://zonarosa.io/one"))
        let task1 = linkPreviewFetchState.update(.init(text: "https://zonarosa.io/one", ranges: .empty))
        XCTAssert(linkPreviewFetchState.currentState.isLoading)

        // If there's a request in flight & we change the URL, drop the original request.
        let url2 = try XCTUnwrap(URL(string: "https://zonarosa.io/two"))
        let task2 = linkPreviewFetchState.update(.init(text: "https://zonarosa.io/two", ranges: .empty))
        XCTAssert(linkPreviewFetchState.currentState.isLoading)
    ...
    func testUpdatePrependScheme() async throws {
        ...
        mockLinkPreviewFetcher.fetchLinkPreviewBlock = { fetchedURL in
            return OWSLinkPreviewDraft(url: fetchedURL, title: "ZonaRosa", isForwarded: false)
        }
        await linkPreviewFetchState.update(.init(text: "zonarosa.io", ranges: .empty), prependSchemeIfNeeded: false)?.value
        XCTAssert(linkPreviewFetchState.currentState.isNone)
        XCTAssertEqual(mockLinkPreviewFetcher.fetchedURLs, [])

        // If we should prepend a scheme, prepend "https://".
        await linkPreviewFetchState.update(.init(text: "zonarosa.io", ranges: .empty), prependSchemeIfNeeded: true)?.value
        XCTAssert(linkPreviewFetchState.currentState.isLoaded)
        XCTAssertEqual(mockLinkPreviewFetcher.fetchedURLs, [URL(string: "https://zonarosa.io")!])
        ...
        await linkPreviewFetchState.update(.init(text: "http://zonarosa.io", ranges: .empty), prependSchemeIfNeeded: true)?.value
        XCTAssert(linkPreviewFetchState.currentState.isNone)
        XCTAssertEqual(mockLinkPreviewFetcher.fetchedURLs, [])
    }
    ...
    func testOnStateChange() async throws {
        ...
        await linkPreviewFetchState.update(.init(text: "https://zonarosa.io", ranges: .empty))?.value
    ...
    func testDisable() async throws {
        ...
        let url = try XCTUnwrap(URL(string: "https://zonarosa.io"))
        await linkPreviewFetchState.update(.init(text: "https://zonarosa.io", ranges: .empty))?.value
        ...
        await linkPreviewFetchState.update(.init(text: "https://zonarosa.io/one", ranges: .empty))?.value
        XCTAssertNil(linkPreviewFetchState.linkPreviewDraftIfLoaded)
        XCTAssertEqual(mockLinkPreviewFetcher.fetchedURLs, [])

        // Ensure "enableIfEmpty" doesn't enable when the text isn't empty.
        await linkPreviewFetchState.update(.init(text: "https://zonarosa.io/one", ranges: .empty), enableIfEmpty: true)?.value
        XCTAssertNil(linkPreviewFetchState.linkPreviewDraftIfLoaded)
        XCTAssertEqual(mockLinkPreviewFetcher.fetchedURLs, [])
        ...
        let url2 = try XCTUnwrap(URL(string: "https://zonarosa.io/two"))
        await linkPreviewFetchState.update(.init(text: "https://zonarosa.io/two", ranges: .empty), enableIfEmpty: true)?.value
        XCTAssertEqual(linkPreviewFetchState.linkPreviewDraftIfLoaded?.url, url2)
    ...
    func testOnlyParseIfEnabled() async throws {
        ...
        do {
            ...
            await linkPreviewFetchState.update(.init(text: "https://zonarosa.io", ranges: .empty))?.value
            XCTAssertEqual(linkPreviewFetchState.currentUrl, try XCTUnwrap(URL(string: "https://zonarosa.io")))
        }
    }
