    ...
    static func usernameNotFoundError(_ username: String) -> SheetDisplayableError {
        return ActionSheetDisplayableError(
            localizedTitle: OWSLocalizedString(
                ...
                comment: "Title for an action sheet indicating that the given username is not associated with a registered ZonaRosa account.",
            ),
            localizedMessage: String(
                format: OWSLocalizedString(
                    "USERNAME_LOOKUP_NOT_FOUND_MESSAGE_FORMAT",
                    comment: "A message indicating that the given username is not associated with a registered ZonaRosa account. Embeds {{ a username }}.",
                ),
                username,
            ),
        )
