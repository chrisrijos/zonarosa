...
/// acceptable for ``SendableAttachment``/sending via ZonaRosa.
public struct NormalizedImage {
    ...
    private static func loadImage(dataSource: DataSourcePath, maxPixelSize: CGFloat) throws(ZonaRosaAttachmentError) -> CGImage {
    ...
    private static func saveImage(_ image: CGImage, containerType: ContainerType) throws(ZonaRosaAttachmentError) -> DataSourcePath {
    ...
    public static func forImage(
        ...
        guard let imageData = image.pngData() else {
            throw ZonaRosaAttachmentError.couldNotConvertImage
        }
    ...
    static func forDataSource(
        ...
        let originalMightBeValid = { () -> Bool in
            guard ZonaRosaAttachment.outputImageUTISet.contains(dataUTI) else {
                return false
            }
    ...
    ) throws(ZonaRosaAttachmentError) -> (dataSource: DataSourcePath, dataUTI: String) {
        let tier = imageQuality.startingTier
        return try autoreleasepool { () throws(ZonaRosaAttachmentError) -> (dataSource: DataSourcePath, dataUTI: String) in
            let containerType = ContainerType.png
    ...
    private func stripImage() throws -> DataSourcePath? {
        ...
        guard let fileExtension = MimeTypeUtil.fileExtensionForUtiType(dataUTI) else {
            throw ZonaRosaAttachmentError.couldNotRemoveMetadata
        }
    ...
    private func compressImageToQuality(_ imageQuality: ImageQualityLevel) throws -> FinalizedImage {
        ...
        throw ZonaRosaAttachmentError.fileSizeTooLarge
    }

    /// Compress the image to the specified tier.
    private func compressImageToTier(_ tier: ImageQualityTier) throws(ZonaRosaAttachmentError) -> FinalizedImage {
    ...
    private static func removeImageMetadata(fromData dataValue: Data, dataUti: String) throws(ZonaRosaAttachmentError) -> Data {
    ...
    /// - Throws: `ZonaRosaAttachmentError.couldNotRemoveMetadata` if the PNG parser fails.
    static func removeImageMetadata(fromPngData pngData: Data) throws(ZonaRosaAttachmentError) -> Data {
        do {
    ...
    private static func removeImageMetadata(fromNonPngData dataValue: Data) throws(ZonaRosaAttachmentError) -> Data {
