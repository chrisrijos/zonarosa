...
open class ScreenLockViewController: UIViewController {
    ...
    private lazy var imageViewLogo = UIImageView(image: UIImage(named: "zonarosa-logo-128-launch-screen"))
    private static var buttonHeight: CGFloat { 40 }
    private lazy var buttonUnlockUI = OWSFlatButton.button(
        title: OWSLocalizedString(
            "SCREEN_LOCK_UNLOCK_ZONAROSA",
            comment: "Label for button on lock screen that lets users unlock ZonaRosa.",
        ),
        font: OWSFlatButton.fontForHeight(ScreenLockViewController.buttonHeight),
        titleColor: UIColor.ZonaRosa.label,
        backgroundColor: UIColor.ZonaRosa.tertiaryFill,
        target: self,
