...
class MentionPicker: UIView {
    ...
    let style: Style
    let selectedAddressCallback: (ZonaRosaServiceAddress) -> Void

    init(
        mentionableAcis: [Aci],
        style: Style,
        selectedAddressCallback: @escaping (ZonaRosaServiceAddress) -> Void,
    ) {
        let databaseStorage = SSKEnvironment.shared.databaseStorageRef
        mentionableUsers = databaseStorage.read { transaction in
            let sortedAddresses = SSKEnvironment.shared.contactManagerImplRef.sortZonaRosaServiceAddresses(
                mentionableAcis.map({ ZonaRosaServiceAddress($0) }),
...
private class MentionableUserCell: UITableViewCell {
    ...
    override func updateConfiguration(using state: UICellConfigurationState) {
        ...
        if state.isSelected || state.isHighlighted {
            configuration.backgroundColor = .ZonaRosa.primaryFill
            if #available(iOS 26, *) {
