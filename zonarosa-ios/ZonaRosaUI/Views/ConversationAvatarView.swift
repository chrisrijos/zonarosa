...
public class ConversationAvatarView: UIView, CVView, PrimaryImageView {
    ...
    private static let serialQueue = ReverseDispatchQueue(
        label: "io.zonarosa.conversation-avatar.loading",
        qos: .userInitiated,
    ...
    private func ensureObservers() {
        ...
        if dataSource.isContactAvatar {
            ...
            NotificationCenter.default.addObserver(
                self,
                selector: #selector(handleZonaRosaAccountsChanged(notification:)),
                name: .OWSContactsManagerZonaRosaAccountsDidChange,
                object: nil,
            )
            NotificationCenter.default.addObserver(
                self,
    ...
    @objc
    private func handleZonaRosaAccountsChanged(notification: Notification) {
        AssertIsOnMainThread()

        // PERF: It would be nice if we could do this only if *this* user's ZonaRosaAccount changed,
    ...
    private func otherUsersProfileDidChange(notification: Notification) {
        AssertIsOnMainThread()
        guard let changedAddress = notification.userInfo?[UserProfileNotifications.profileAddressKey] as? ZonaRosaServiceAddress, changedAddress.isValid else {
    ...
    private func skipContactAvatarBlurDidChange(notification: Notification) {
        AssertIsOnMainThread()
        guard let address = notification.userInfo?[OWSContactsManager.skipContactAvatarBlurAddressKey] as? ZonaRosaServiceAddress, address.isValid else {
    ...
    private func handleUpdatedAddressNotification(address: ZonaRosaServiceAddress) {
        AssertIsOnMainThread()
        guard let dataSource = configuration.dataSource else { return }
...
public enum ConversationAvatarDataSource: Equatable, CustomStringConvertible {
    ...
    case address(ZonaRosaServiceAddress)
    ...
    var contactAddress: ZonaRosaServiceAddress? {
        switch self {
    ...
    fileprivate func fetchBadge(configuration: ConversationAvatarView.Configuration, transaction: DBReadTransaction?) -> UIImage? {
        ...
        let targetAddress: ZonaRosaServiceAddress
        switch self {
