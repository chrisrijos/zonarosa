...
public class RecipientPickerViewController: OWSViewController, OWSNavigationChildController {
    ...
    public enum SelectionMode {
        ...
        //
        // - If numbers aren't registered, don't offer to invite them to ZonaRosa. If
    ...
    // MARK: ZonaRosa Connections

    private var zonarosaConnections = [ComparableDisplayName]()
    private var zonarosaConnectionAddresses = Set<ZonaRosaServiceAddress>()
    ...
    override public func viewDidLoad() {
        ...
        // "No ZonaRosa Contacts"
        noZonaRosaContactsView.isHidden = !isNoContactsModeActive
        view.addSubview(noZonaRosaContactsView)
        noZonaRosaContactsView.autoPinWidthToSuperview()
        noZonaRosaContactsView.autoPinEdge(toSuperviewEdge: .top)
        noZonaRosaContactsView.autoPin(toBottomLayoutGuideOf: self, withInset: 0)
    ...
    private lazy var noZonaRosaContactsView = createNoZonaRosaContactsView()
    ...
    // MARK: - Fetching ZonaRosa Connections

    private func updateZonaRosaConnections() {
        SSKEnvironment.shared.databaseStorageRef.read { tx in
            let tsAccountManager = DependenciesBridge.shared.tsAccountManager

            // All ZonaRosa Connections that we believe are registered. In theory, this
            ...
            zonarosaConnections = SSKEnvironment.shared.contactManagerImplRef.sortedComparableNames(for: resolvedAddresses, tx: tx).filter { $0.displayName.hasKnownValue }
            zonarosaConnectionAddresses = Set(zonarosaConnections.lazy.map { $0.address })
        }
    }
    ...
    private func updateTableContents() {
        ...
        if !pickedRecipients.isEmpty, !isSearching {
            let sectionRecipients = pickedRecipients.filter { recipient in
                ...
                if zonarosaConnectionAddresses.contains(recipientAddress) {
                    return false
                }
                return true
            }
...
extension RecipientPickerViewController {
    ...
    private func createNoZonaRosaContactsView() -> UIView {
    ...
    /// no contacts on ZonaRosa." This method controls whether or not we show this
    ...
    /// ZonaRosa." row. The latter is shown at the very top of the recipient
    ...
    /// prevented ZonaRosa from accessing their contacts, we don't show the
    ...
    private func shouldNoContactsModeBeActive() -> Bool {
        ...
        case .authorized, .notAllowed:
            if !zonarosaConnections.isEmpty {
    ...
    private func noContactsTableItem() -> OWSTableItem {
        return OWSTableItem.softCenterLabel(
            withText: OWSLocalizedString(
                ...
                comment: "A label that indicates the user has no ZonaRosa contacts that they haven't blocked.",
            ),
        )
    }
...
extension RecipientPickerViewController {
    private func contactsSection() -> [OWSTableSection] {
        guard !zonarosaConnections.isEmpty else {
        ...
        guard shouldShowAlphabetSlider else {
            return [OWSTableSection(
                ...
                items: zonarosaConnections.map { item(forRecipient: PickedRecipient.for(address: $0.address)) },
            )]
        }

        var collatedZonaRosaConnections = collation.sectionTitles.map { _ in return [ComparableDisplayName]() }
        for zonarosaConnection in zonarosaConnections {
            let section = collation.section(
                for: CollatableComparableDisplayName(zonarosaConnection),
                collationStringSelector: #selector(CollatableComparableDisplayName.collationString),
            )
            guard section >= 0 else {
                continue
            }
            collatedZonaRosaConnections[section].append(zonarosaConnection)
        }

        let contactSections = collatedZonaRosaConnections.enumerated().map { index, zonarosaConnections in
    ...
    private func addressCell(for address: ZonaRosaServiceAddress, recipient: PickedRecipient, tableView: UITableView) -> UITableViewCell? {
        ...
        SSKEnvironment.shared.databaseStorageRef.read { transaction in
            ...
            if let delegate {
                ...
                let isSystemContact = SSKEnvironment.shared.contactManagerRef.fetchZonaRosaAccount(for: address, transaction: transaction) != nil
...
struct PhoneNumberFinder {
    ...
    enum LookupResult {
        ...
        case success(ZonaRosaRecipient)
    ...
    func lookUp(phoneNumber searchResult: SearchResult) async throws -> LookupResult {
        ...
        let zonarosaRecipients = try await contactDiscoveryManager.lookUp(phoneNumbers: [validE164ToLookUp], mode: .oneOffUserRequest)
        if let zonarosaRecipient = zonarosaRecipients.first {
            return .success(zonarosaRecipient)
        } else {
...
extension RecipientPickerViewController {
    ...
    private func handlePhoneNumberLookupResult(_ lookupResult: PhoneNumberFinder.LookupResult) {
        ...
        case (_, .success(let zonarosaRecipient)):
            // If the lookup was successful, select the recipient.
            tryToSelectRecipient(.for(address: zonarosaRecipient.address))

        case (.blocklist, .notFound(validE164: let validE164)):
            // If we're trying to block an unregistered user, allow it.
            tryToSelectRecipient(.for(address: ZonaRosaServiceAddress(phoneNumber: validE164)))
    ...
    public static func presentSMSInvitationSheet(
        ...
        let actionSheet = ActionSheetController(
            title: OWSLocalizedString(
                ...
                comment: "Alert title. Shown after selecting a phone number that isn't a ZonaRosa user.",
            ),
            message: String(
                format: OWSLocalizedString(
                    "RECIPIENT_PICKER_INVITE_MESSAGE",
                    comment: "Alert text. Shown after selecting a phone number that isn't a ZonaRosa user.",
                ),
        ...
        actionSheet.addAction(ActionSheetAction(
            title: OWSLocalizedString(
                ...
                comment: "Button. Shown after selecting a phone number that isn't a ZonaRosa user. Tapping the button will open a view that allows the user to send an SMS message to specified phone number.",
            ),
            style: .default,
...
extension RecipientPickerViewController: FindByPhoneNumberDelegate {
    ...
    public func findByPhoneNumber(
        ...
        didSelectAddress address: ZonaRosaServiceAddress,
    ) {
...
extension RecipientPickerViewController: FindByUsernameDelegate {
    func findByUsername(address: ZonaRosaServiceAddress) {
        owsAssertDebug(address.isValid)
        tryToSelectRecipient(.for(address: address))
    }
...
extension RecipientPickerViewController {
    ...
    private func findByUsername(_ username: String) {
        Task {
            ...
            tryToSelectRecipient(.for(address: ZonaRosaServiceAddress(aci)))
        }
    }
}
