...
// BaseMemberViewController is extended to conform to in the main ZonaRosa target.
...
open class BaseMemberViewController: RecipientPickerContainerViewController {
    ...
    private func updateMemberBar() {
        memberBar.setMembers(SSKEnvironment.shared.databaseStorageRef.read { tx in
            let members = self.recipientSet.orderedMembers.compactMap { pickedRecipient -> (PickedRecipient, ZonaRosaServiceAddress)? in
    ...
    public class func sortedMemberAddresses(
        ...
        return SSKEnvironment.shared.contactManagerRef.sortZonaRosaServiceAddresses(
