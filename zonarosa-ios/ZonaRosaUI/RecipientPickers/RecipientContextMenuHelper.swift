...
public protocol RecipientContextMenuHelperDelegate: AnyObject {
    func additionalActions(for address: ZonaRosaServiceAddress) -> [UIAction]
...
class RecipientContextMenuHelper {
    ...
    func actionProvider(address: ZonaRosaServiceAddress) -> UIContextMenuActionProvider {
        return { [weak self] _ in
            ...
            let localAddress: ZonaRosaServiceAddress? = self.databaseStorage.read { [weak self] tx in
    ...
    private func isSystemContact(e164: E164) -> Bool {
        return databaseStorage.read { tx in
            return contactsManager.fetchZonaRosaAccount(forPhoneNumber: e164.stringValue, transaction: tx) != nil
        }
    }
