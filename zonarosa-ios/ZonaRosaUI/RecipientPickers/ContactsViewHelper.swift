    ...
    private func setupNotificationObservations() {
        notificationObservers.append(contentsOf: [
            NotificationCenter.default.addObserver(
                forName: .OWSContactsManagerZonaRosaAccountsDidChange,
                object: nil,
                queue: nil,
    ...
    private static func presentContactAccessDeniedAlert(from viewController: UIViewController, access: Access) {
        ...
        case .edit:
            title = OWSLocalizedString(
                ...
                comment: "Alert title for when the user has just tried to edit a contacts after declining to give ZonaRosa contacts permissions",
            )
            message = OWSLocalizedString(
                "EDIT_CONTACT_WITHOUT_CONTACTS_PERMISSION_ALERT_BODY",
                comment: "Alert body for when the user has just tried to edit a contacts after declining to give ZonaRosa contacts permissions",
            )
        ...
        case .read(let readPurpose):
            ...
            case .invite:
                title = OWSLocalizedString(
                    ...
                    comment: "Alert title when contacts disabled while trying to invite contacts to zonarosa",
                )
                message = OWSLocalizedString(
                    "INVITE_FLOW_REQUIRES_CONTACT_ACCESS_BODY",
                    comment: "Alert body when contacts disabled while trying to invite contacts to zonarosa",
                )
            }
        }
