...
class DeleteSystemContactViewController: OWSTableViewController2 {
    ...
    private lazy var spinnerContainer: UIView = {
        ...
        view.backgroundColor = .ZonaRosa.backdrop
    ...
    private func updateTableContents() {
        ...
        let addressForProfileLookup = ZonaRosaServiceAddress(serviceId: serviceId, e164: e164)
        let (
            image,
            systemContactName,
    ...
    private func displayDeleteContactActionSheet(phoneNumber: String?, displayNameForToast: String) {
        let actionSheet = ActionSheetController(
            ...
            message: OWSLocalizedString(
                ...
                comment: "An explanation of what happens in ZonaRosa when you remove a system contact.",
            ),
        )
    ...
    private func handleContactDelete(displayNameForToast: String) {
        ...
        let zonarosaAccount = self.dependencies.databaseStorage.read { tx in
            return ZonaRosaAccountFinder().zonarosaAccount(
                for: self.e164,
                tx: tx,
            )
        }
        ...
        let preferredContactIdForDeletion = zonarosaAccount?.cnContactId
        ...
        Task {
            do {
                try await withCooperativeTimeout(seconds: 5) {
                    for await _ in NotificationCenter.default.notifications(named: .OWSContactsManagerZonaRosaAccountsDidChange).map({ _ in }) {
                        break
                    }
                }
            } catch {
            ...
            do {
                try await self.dependencies.databaseStorage.awaitableWrite { tx in
                    try self.dependencies.recipientHidingManager.addHiddenRecipient(
                        ZonaRosaServiceAddress(serviceId: self.serviceId, e164: self.e164),
