...
class NotificationService: UNNotificationServiceExtension {
    ...
    private func startProxyIfEnabled() async throws(CancellationError) {
        if ZonaRosaProxy.isEnabled {
            Logger.info("Waiting for zonarosa proxy to become ready for message fetch.")
            ZonaRosaProxy.startRelayServer()
            try await Preconditions([
                NotificationPrecondition(
                    notificationName: .isZonaRosaProxyReadyDidChange,
                    isSatisfied: { ZonaRosaProxy.isEnabledAndReady },
                ),
            ]).waitUntilSatisfied()
        }
    }
    ...
    private func fetchAndProcessMessages(logger: NSELogger) async -> UNNotificationContent {
        ...
        do {
            ...
            defer { ZonaRosaProxy.stopRelayServer() }
