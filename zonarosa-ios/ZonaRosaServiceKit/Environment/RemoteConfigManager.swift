        ...
        return Dictionary(
            uniqueKeysWithValues: self.valueFlags
                ...
                .compactMap { (key: String, value: String) -> (key: String, value: String)? in
                    ...
                    guard let range = key.range(of: "ios.libzonarosa.", options: [.anchored]) else {
                        return nil
                    }
...
public class RemoteConfigManagerImpl: RemoteConfigManager {
    ...
    private func _refresh() async throws {
        ...
        let serverEpochTimeMs = headers["x-zonarosa-timestamp"].flatMap(UInt64.init(_:))
        owsAssertDebug(serverEpochTimeMs != nil, "Must have X-ZonaRosa-Timestamp.")
