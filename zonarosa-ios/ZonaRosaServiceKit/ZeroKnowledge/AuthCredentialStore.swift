...
public class AuthCredentialStore {
    ...
    ) throws -> LibZonaRosaClient.CallLinkAuthCredential? {
        return try callLinkAuthCredentialStore.getData(
            Self.callLinkAuthCredentialKey(for: redemptionTime),
            transaction: tx,
        ).map {
            return try LibZonaRosaClient.CallLinkAuthCredential(contents: $0)
        }
    }

    func setCallLinkAuthCredential(
        _ credential: LibZonaRosaClient.CallLinkAuthCredential,
        for redemptionTime: UInt64,
    ...
    func setSVRBAuthCredential(
        _ credential: LibZonaRosaClient.Auth?,
        now: Date,
        tx: DBWriteTransaction,
    ) {
