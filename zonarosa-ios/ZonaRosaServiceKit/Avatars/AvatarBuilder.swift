...
public class AvatarBuilder {
    ...
    private func otherUsersProfileDidChange(notification: Notification) {
        AssertIsOnMainThread()
        if let address = notification.userInfo?[UserProfileNotifications.profileAddressKey] as? ZonaRosaServiceAddress {
    ...
    public func avatarImageWithSneakyTransaction(
        forAddress address: ZonaRosaServiceAddress,
        diameterPoints: UInt,
        localUserDisplayMode: LocalUserDisplayMode,
    ) -> UIImage? {
    ...
    public func defaultAvatarImage(
        ...
        address: ZonaRosaServiceAddress? = nil,
        diameterPoints: UInt,
        transaction: DBReadTransaction,
    ) -> UIImage? {
        let diameterPixels = CGFloat(diameterPoints).pointsAsPixels
    ...
    // Instead we add a layer of indirection to map known ZonaRosaServiceAddress to ephemeral UUID
    ...
    private let addressToAvatarIdentifierCache = LRUCache<ZonaRosaServiceAddress, String>(maxSize: 256, nseMaxSize: 0)

    public enum RequestType {
        case contactAddress(address: ZonaRosaServiceAddress, localUserDisplayMode: LocalUserDisplayMode)
    ...
    private enum AvatarContentType: Equatable {
        ...
        case cachedContact(address: ZonaRosaServiceAddress, cacheKey: String)
        case gradient(AvatarGradient)
