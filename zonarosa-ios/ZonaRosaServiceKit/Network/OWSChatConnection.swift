...
public class OWSChatConnection {
    ...
    public init(
        ...
        self.serialQueue = DispatchQueue(label: "io.zonarosa.chat-connection-\(type)")
        self.type = type
...
class OWSChatConnectionUsingLibZonaRosa<Connection: ChatConnection & Sendable>: OWSChatConnection, ConnectionEventsListener {
    fileprivate let libzonarosaNet: Net
    ...
    override fileprivate func appDidBecomeReady() {
        ...
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(zonarosaProxyConfigDidChange),
            name: .zonarosaProxyConfigDidChange,
            object: nil,
        )
    }
    ...
    @objc
    private func zonarosaProxyConfigDidChange(_ notification: NSNotification) {
        // The libzonarosa connection needs to be recreated whether the proxy is going up,
        // changing, or going down.
        Logger.info("\(logPrefix) zonarosa proxy config changed; cycling socket")
        cycleSocket()
    }
    ...
    override fileprivate func ensureWebsocketExists() {
        ...
        connection = .connecting(token: token, task: Task { [token] () -> Connection? in
            ...
            do {
                Logger.info("\(self.logPrefix): starting chat connect (zonarosaProxyEnabled: \(ZonaRosaProxy.isEnabled), zonarosaProxyReady: \(ZonaRosaProxy.isEnabledAndReady))")
            ...
            } catch ZonaRosaError.appExpired(_) {
                await appExpiry.setHasAppExpiredAtCurrentVersion(db: db)
            } catch ZonaRosaError.deviceDeregistered(_) {
    ...
    override fileprivate func makeRequestInternal(_ request: TSRequest, requestId: UInt64) async throws -> HTTPResponse {
        ...
        let libzonarosaRequest = ChatConnection.Request(method: httpMethod, pathAndQuery: "/\(requestUrl.relativeString)", headers: httpHeaders.headers, body: body, timeout: request.timeoutInterval)
        ...
        do {
            guard let chatService else {
                throw ZonaRosaError.chatServiceInactive("no connection to chat server")
            }

            connectionInfo = chatService.info()
            response = try await chatService.send(libzonarosaRequest)
        } catch {
            switch error {
            case ZonaRosaError.connectionTimeoutError(_), ZonaRosaError.requestTimeoutError:
            ...
            case ZonaRosaError.webSocketError(_), ZonaRosaError.possibleCaptiveNetwork(_), ZonaRosaError.connectionFailed(_), ZonaRosaError.chatServiceInactive:
                throw OWSHTTPError.networkFailure(.genericFailure)
            case ZonaRosaError.connectionInvalidated:
    ...
    func connectionWasInterrupted(_ service: Connection, error: Error?) {
        self.serialQueue.async { [self] in
            ...
            if let error {
                ...
                owsFailDebug("\(logPrefix) libzonarosa disconnected us without being asked")
            }
    ...
    func withLibzonarosaConnection<Output>(
...
class OWSUnauthConnectionUsingLibZonaRosa: OWSChatConnectionUsingLibZonaRosa<UnauthenticatedChatConnection> {
    init(libzonarosaNet: Net, appExpiry: AppExpiry, appReadiness: AppReadiness, db: any DB) {
        super.init(libzonarosaNet: libzonarosaNet, type: .unidentified, appExpiry: appExpiry, appReadiness: appReadiness, db: db)
    }
    ...
    override func connectChatService(token: NSObject) async throws -> UnauthenticatedChatConnection {
        return try await libzonarosaNet.connectUnauthenticatedChat(languages: Array(HttpHeaders.topPreferredLanguages()))
    }
}

class OWSAuthConnectionUsingLibZonaRosa: OWSChatConnectionUsingLibZonaRosa<AuthenticatedChatConnection>, ChatConnectionListener {
    ...
    init(
        ...
        super.init(libzonarosaNet: libzonarosaNet, type: .identified, appExpiry: appExpiry, appReadiness: appReadiness, db: db)
    }
    ...
    override fileprivate func connectChatService(token: NSObject) async throws -> AuthenticatedChatConnection {
        ...
        do {
            return try await libzonarosaNet.connectAuthenticatedChat(
                username: username ?? "",
                password: password ?? "",
            ...
            switch error {
            case ZonaRosaError.deviceDeregistered:
                serialQueue.async {
    ...
    ///
    /// This is on top of the websocket pings libzonarosa already uses to keep connections alive.
        ...
        return Task(priority: .low) { [logPrefix = self.logPrefix, weak chat] in
            while true {
                do {
                    ...
                    ZonaRosaError.chatServiceInactive(_)
                {
                    // No action necessary, we're done with this service.
                    return
                } catch ZonaRosaError.rateLimitedError(retryAfter: let delay, message: _) {
                    ...
                    // but continue to rely on libzonarosa reporting disconnects via delegate callback.
