...
extension ZonaRosaProxy {
    /// Establishes a connection to a ZonaRosa TLS Proxy and relays transmitted data via the provided `RelayClient`
    class ProxyClient {
        ...
        private lazy var queue = DispatchQueue(label: "io.zonarosa.proxy.client", attributes: .concurrent)
        ...
        func start() {
            ...
            guard let (proxyHost, proxyPort) = ZonaRosaProxy.host.flatMap({ Self.parseHost($0) }) else {
            ...
            // (This is only the "outer" layer of the proxy; the inner connection to the ZonaRosa servers
