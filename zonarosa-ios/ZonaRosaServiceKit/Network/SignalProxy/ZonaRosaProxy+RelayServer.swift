...
extension ZonaRosaProxy {
    /// An HTTP Proxy server that relays traffic to a ZonaRosa TLS Proxy
    class RelayServer {
        @Atomic private(set) var isStarted = false

        @Atomic private(set) var isReady = false {
            didSet {
                NotificationCenter.default.postOnMainThread(name: .isZonaRosaProxyReadyDidChange, object: nil)
            }
        }
        ...
        private let queue = DispatchQueue(label: "io.zonarosa.proxy.relay-server", attributes: .concurrent)

        func start() {
            guard !isStarted else { return }
            guard ZonaRosaProxy.isEnabled else { return }
