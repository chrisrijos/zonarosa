...
extension TSThread {
    ...
    public enum IntentContext {
        case senderAddress(ZonaRosaServiceAddress)
    ...
    private func _generateSendMessageIntent(
        ...
        let senderAddress: ZonaRosaServiceAddress
        let message: TSMessage?
        let recipientAddress: ZonaRosaServiceAddress?
        switch context {
    ...
    func generateIncomingCallIntent(
        ...
        if self.isGroupThread {
            ...
            return self.generateSendMessageIntent(
                context: .senderAddress(ZonaRosaServiceAddress(callerAci)),
                transaction: tx,
            )
        } else {
            ...
            return ResolvableDisplayNameBuilder(
                displayNameForAddress: ZonaRosaServiceAddress(callerAci),
                transformedBy: { displayName, tx in
                    let caller = Self.buildPerson(address: ZonaRosaServiceAddress(callerAci), displayName: displayName, tx: tx)
    ...
    private static func intentRecipientAvatarImage(recipient: ZonaRosaServiceAddress, transaction: DBReadTransaction) -> INImage? {
        // Generate avatar
        let image: INImage
        if
