...
private class IncomingContactSyncJobRunner: JobRunner {
    ...
    /// Clear ``ZonaRosaAccount``s that weren't part of a complete sync.
    ///
    /// Although "system contact" details (represented by a ``ZonaRosaAccount``)
    ...
    private func pruneContacts(exceptThoseReceivedFromCompleteSync phoneNumbers: [E164]) async throws {
        try await SSKEnvironment.shared.databaseStorageRef.awaitableWrite { transaction in
            ...
            // create a ZonaRosaAccount for your own address. (If you're a primary, this
            ...
            // Rather than collecting ZonaRosaAccount objects, collect their unique IDs.
            ...
            ZonaRosaAccount.anyEnumerate(transaction: transaction, batchingPreference: .batched(8)) { zonarosaAccount, _ in
                if let phoneNumber = E164(zonarosaAccount.recipientPhoneNumber), setOfPhoneNumbers.contains(phoneNumber) {
                    // This contact was received in this batch, so don't remove it.
                    return
                }
                uniqueIdsToRemove.append(zonarosaAccount.uniqueId)
            }
            Logger.info("Removing \(uniqueIdsToRemove.count) contacts during contact sync")
            for uniqueId in uniqueIdsToRemove {
                autoreleasepool {
                    guard let zonarosaAccount = ZonaRosaAccount.anyFetch(uniqueId: uniqueId, transaction: transaction) else {
                        return
                    }
                    zonarosaAccount.anyRemove(transaction: transaction)
                }
            }
            if !uniqueIdsToRemove.isEmpty {
                SSKEnvironment.shared.contactManagerImplRef.didUpdateZonaRosaAccounts(transaction: transaction)
            }
        }
    }
}
