...
/// We create ZonaRosaRecipient records for accounts we know about.
///
/// A ZonaRosaRecipient's stable identifier is an ACI. Once a ZonaRosaRecipient
...
public struct ZonaRosaRecipient: FetchableRecord, PersistableRecord, Codable {
    public static let databaseTableName = "model_ZonaRosaRecipient"
    ...
    /// Represents the ACI for this ZonaRosaRecipient.
    ///
    ...
    /// Represents the PNI for this ZonaRosaRecipient.
    ///
    ...
    public var address: ZonaRosaServiceAddress {
        // ZonaRosaRecipients store every identifier because they are the source of
    ...
    static func insertRecord(
        ...
        do {
            return try ZonaRosaRecipient.fetchOne(
                tx.database,
                sql: """
                INSERT INTO \(ZonaRosaRecipient.databaseTableName) (
                    \(zonarosaRecipientColumn: .recordType),
                    \(zonarosaRecipientColumn: .uniqueId),
                    \(zonarosaRecipientColumn: .aciString),
                    \(zonarosaRecipientColumn: .phoneNumber),
                    \(zonarosaRecipientColumn: .pni),
                    \(zonarosaRecipientColumn: .deviceIds),
                    \(zonarosaRecipientColumn: .unregisteredAtTimestamp),
                    \(zonarosaRecipientColumn: .status)
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?) RETURNING *
                """,
                arguments: [
                    SDSRecordType.zonarosaRecipient.rawValue,
                    UUID().uuidString,
    ...
    public init(from decoder: Decoder) throws {
        ...
        guard decodedRecordType == SDSRecordType.zonarosaRecipient.rawValue else {
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        try container.encode(SDSRecordType.zonarosaRecipient.rawValue, forKey: .recordType)
    ...
    public var addressComponentsDescription: String {
        ZonaRosaServiceAddress.addressComponentsDescription(uuidString: aciString, phoneNumber: phoneNumber?.stringValue)
    }
...
// MARK: - ZonaRosaRecipientManagerImpl

extension ZonaRosaRecipientManagerImpl {
    func setDeviceIds(
        _ deviceIds: Set<DeviceId>,
        for recipient: inout ZonaRosaRecipient,
...
public extension String.StringInterpolation {
    mutating func appendInterpolation(zonarosaRecipientColumn column: ZonaRosaRecipient.CodingKeys) {
        appendLiteral(column.rawValue)
    }

    mutating func appendInterpolation(zonarosaRecipientColumnFullyQualified column: ZonaRosaRecipient.CodingKeys) {
        appendLiteral("\(ZonaRosaRecipient.databaseTableName).\(column.rawValue)")
    }
}
