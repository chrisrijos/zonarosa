...
final class ContactDiscoveryTaskQueueImpl: ContactDiscoveryTaskQueue {
    ...
    func perform(for phoneNumbers: Set<String>, mode: ContactDiscoveryMode) async throws -> [ZonaRosaRecipient] {
        ...
        let discoveryResults = try await ContactDiscoveryV2Operation(
            ...
            mode: mode,
            udManager: udManager,
            connectionImpl: libzonarosaNet,
            remoteAttestation: ContactDiscoveryV2Operation<LibZonaRosaClient.Net>.Wrappers.RemoteAttestation(),
        ).perform()
    ...
    ) async throws -> [ZonaRosaRecipient] {
        var registeredRecipients = [ZonaRosaRecipient]()
