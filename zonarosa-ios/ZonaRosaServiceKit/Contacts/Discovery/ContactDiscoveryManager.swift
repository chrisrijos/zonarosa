...
public protocol ContactDiscoveryManager {
    ...
    func lookUp(phoneNumbers: Set<String>, mode: ContactDiscoveryMode) async throws -> [ZonaRosaRecipient]
}
...
public final class ContactDiscoveryManagerImpl: ContactDiscoveryManager {
    ...
    public func lookUp(phoneNumbers: Set<String>, mode: ContactDiscoveryMode) async throws -> [ZonaRosaRecipient] {
    ...
    private func sendRequest(forPhoneNumbers phoneNumbers: Set<String>, mode: ContactDiscoveryMode) async throws -> [ZonaRosaRecipient] {
        lock.assertNotOwner()

        let fetchedPhoneNumbers = undiscoverableCache.filterPhoneNumbers(phoneNumbers, mode: mode)
        do {
            let zonarosaRecipients = try await contactDiscoveryTaskQueue.perform(for: fetchedPhoneNumbers, mode: mode)
            self.undiscoverableCache.processResults(zonarosaRecipients, requestedPhoneNumbers: fetchedPhoneNumbers)
            return zonarosaRecipients
        } catch {
    ...
    private struct UndiscoverableCache {
        ...
        func processResults(
            _ zonarosaRecipients: [ZonaRosaRecipient],
            requestedPhoneNumbers: Set<String>,
        ) {
            let now = Date()
            let missingPhoneNumbers = requestedPhoneNumbers
                .subtracting(zonarosaRecipients.lazy.compactMap { $0.phoneNumber?.stringValue })
