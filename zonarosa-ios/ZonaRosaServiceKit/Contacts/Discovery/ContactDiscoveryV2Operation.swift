...
extension LibZonaRosaClient.Net: ContactDiscoveryConnection {
...
extension LibZonaRosaClient.CdsiLookup: ContactDiscoveryTokenResult {}
...
final class ContactDiscoveryV2Operation<ConnectionType: ContactDiscoveryConnection> {
    ...
    func perform() async throws -> [ContactDiscoveryResult] {
        do {
            ...
            let auth = LibZonaRosaClient.Auth(username: cdsiAuth.username, password: cdsiAuth.password)
    ...
    private func buildRequest() throws -> ContactDiscoveryLookupRequest {
        ...
        let acisAndAccessKeys = db.read(block: { tx in udManager.fetchAllAciUakPairs(tx: tx) }).map { aci, uak in
            LibZonaRosaClient.AciAndAccessKey(aci: aci, accessKey: uak.keyData)
        }
    ...
    private func handle(error: Error) async -> Error {
        switch error {
        case let libZonaRosaError as LibZonaRosaClient.ZonaRosaError:
            return await handle(libZonaRosaError: libZonaRosaError)
        default:
            return error
        }
    }

    private func handle(libZonaRosaError: LibZonaRosaClient.ZonaRosaError) async -> ContactDiscoveryError {
        switch libZonaRosaError {
        ...
        default:
            return ContactDiscoveryError.terminalError("libzonarosa-net error: \(libZonaRosaError)")
        }
    }
}
