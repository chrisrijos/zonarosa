...
public protocol RecipientMerger {
    ...
    func splitUnregisteredRecipientIfNeeded(
        localIdentifiers: LocalIdentifiers,
        unregisteredRecipient: inout ZonaRosaRecipient,
        tx: DBWriteTransaction,
    )
}
...
struct MergedRecipient {
    ...
    let oldRecipient: ZonaRosaRecipient?
    let newRecipient: ZonaRosaRecipient
}
...
class RecipientMergerImpl: RecipientMerger {
    ...
    static func buildObservers(
        ...
        return Observers(
            preThreadMerger: [
                zonarosaServiceAddressCache,
                AuthorMergeObserver(authorMergeHelper: authorMergeHelper),
                ZonaRosaAccountMergeObserver(),
            ...
            postThreadMerger: [
                // The group member MergeObserver depends on `ZonaRosaServiceAddressCache`,
    ...
    private func _applyValidatedMergeFromStorageService(
        ...
        let aciPhoneNumberRecipient: ZonaRosaRecipient? = {
            guard let aci = serviceIds.aci, let phoneNumber else {
                return nil
            }
            // Explicit cast to guarantee this method doesn't return an Optional.
            return mergeAlways(aci: aci, phoneNumber: phoneNumber, isLocalRecipient: false, tx: tx) as ZonaRosaRecipient
        }()
        let phoneNumberPniRecipient: ZonaRosaRecipient? = {
            guard let phoneNumber, let pni = serviceIds.pni else {
                return nil
            }
            // Explicit cast to guarantee this method doesn't return an Optional.
            return mergeAlways(phoneNumber: phoneNumber, pni: pni, isLocalRecipient: false, tx: tx) as ZonaRosaRecipient
        }()
    ...
    func splitUnregisteredRecipientIfNeeded(
        ...
        mergeAndNotify(
            ...
            var splitRecipient = failIfThrowsDatabaseError { () throws(GRDB.DatabaseError) in
                return try ZonaRosaRecipient.insertRecord(unregisteredAtTimestamp: Date.ows_millisecondTimestamp(), tx: tx)
            }
    ...
    ///
    /// There may be a ``ZonaRosaRecipient`` for one or more of the provided
    ...
    /// the ACI of a ``ZonaRosaRecipient`` from one ACI to another; instead we
    /// create a new ``ZonaRosaRecipient``. (However, the ACI *may* change from a
    ...
    private func mergeAlways(
        ...
        // remainder of this method is to ensure there's a `ZonaRosaRecipient` such
            ...
            return (
                mergedRecipient: mergeResult.mergedRecipient ?? {
                    var mergedRecipient = failIfThrowsDatabaseError { () throws(GRDB.DatabaseError) in
                        return try ZonaRosaRecipient.insertRecord(tx: tx)
                    }
                    mergedRecipient.aci = aci
                    mergedRecipient.phoneNumber = ZonaRosaRecipient.PhoneNumber(
    ...
    private func _mergeHighTrust(
        aci: Aci,
        phoneNumber: E164,
        aciRecipient: ZonaRosaRecipient?,
        phoneNumberRecipient: ZonaRosaRecipient?,
        tx: DBWriteTransaction,
    ) -> (mergedRecipient: ZonaRosaRecipient?, otherUpdatedRecipients: [ZonaRosaRecipient]) {
        if var aciRecipient {
        ...
        if var phoneNumberRecipient {
            if phoneNumberRecipient.aciString != nil {
                ...
                // a new ZonaRosaRecipient. We clear the phone number here since it will
                // belong to the new ZonaRosaRecipient.
                phoneNumberRecipient.phoneNumber = nil
                phoneNumberRecipient.pni = nil
    ...
    private func _mergeAlways(
        ...
        pni: Pni,
        phoneNumberRecipient: ZonaRosaRecipient?,
        pniRecipient: ZonaRosaRecipient?,
        tx: DBWriteTransaction,
    ) -> (mergedRecipient: ZonaRosaRecipient?, otherUpdatedRecipients: [ZonaRosaRecipient]) {
    ...
    private func _mergeAlwaysFromStorageService(
        ...
        if var aciRecipient {
            ...
            var updatedPniRecipient: ZonaRosaRecipient?
            if canAssignPni {
                if let pniRecipient {
    ...
    private func mergeAndNotify(
        existingRecipients: [ZonaRosaRecipient],
        mightReplaceNonnilPhoneNumber: Bool,
        insertSessionSwitchoverIfNeeded: Bool,
        isLocalMerge: Bool,
        tx: DBWriteTransaction,
        applyMerge: (DBWriteTransaction) -> (mergedRecipient: ZonaRosaRecipient, otherUpdatedRecipients: [ZonaRosaRecipient]),
    ) -> ZonaRosaRecipient {
    ...
    private enum SessionEvent {
        case sessionSwitchover(ZonaRosaRecipient, phoneNumber: String?)
        case safetyNumberChange(ZonaRosaRecipient, wasIdentityVerified: Bool)
    }

    private func prepareSessionEventsToInsert(
        oldRecipients: [ZonaRosaRecipient],
        affectedRecipients: [ZonaRosaRecipient],
        mergedRecipient: ZonaRosaRecipient,
        tx: DBWriteTransaction,
    ) -> [SessionEvent] {
    ...
    private func prepareSessionSwitchoverEvent(
        recipientPair: MergePair<ZonaRosaRecipient>,
        tx: DBWriteTransaction,
    ) -> SessionEvent {
    ...
    /// numbers to ACIs.
    private func sessionIdentifier(for recipient: ZonaRosaRecipient) -> some Equatable {
...
// MARK: - ZonaRosaServiceAddressCache

extension ZonaRosaServiceAddressCache: RecipientMergeObserver {
    func willBreakAssociation(for recipient: ZonaRosaRecipient, mightReplaceNonnilPhoneNumber: Bool, tx: DBWriteTransaction) {}
