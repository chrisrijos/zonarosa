...
public protocol PhoneNumberVisibilityFetcher {
    ...
    func isPhoneNumberVisible(for recipient: ZonaRosaRecipient, tx: DBReadTransaction) -> Bool

    /// Pre-fetches state about visible phone numbers.
    ///
    /// This is useful during `ZonaRosaServiceAddressCache`'s `warmCaches` method
...
public final class PhoneNumberVisibilityFetcherImpl: PhoneNumberVisibilityFetcher {
    ...
    public func isPhoneNumberVisible(for recipient: ZonaRosaRecipient, tx: DBReadTransaction) -> Bool {
        return _isPhoneNumberVisible(
            ...
            isSystemContact: {
                contactsManager.fetchZonaRosaAccount(forPhoneNumber: $0, transaction: tx) != nil
            },
        )
    }
    ...
    public func fetchAll(tx: DBReadTransaction) throws -> BulkPhoneNumberVisibilityFetcher {
        return BulkPhoneNumberVisibilityFetcher(
            ...
            phoneNumbersWithSystemContacts: Set(
                try ZonaRosaAccountFinder().fetchPhoneNumbers(tx: tx),
            ),
        )
    }
}
...
public final class BulkPhoneNumberVisibilityFetcher {
    ...
    func isPhoneNumberVisible(for recipient: ZonaRosaRecipient) -> Bool {
        return _isPhoneNumberVisible(
            for: recipient,
            localAci: { localAci },
