...
extension Notification.Name {
    public static let OWSContactsManagerZonaRosaAccountsDidChange = Notification.Name("OWSContactsManagerZonaRosaAccountsDidChangeNotification")
...
public class OWSContactsManager: NSObject, ContactsManagerProtocol {
    ...
    private let addressesAllowingAvatarDownloadCache = AtomicSet<ZonaRosaServiceAddress>(lock: .init())
    private let groupIdsAllowingAvatarDownloadCache = AtomicSet<Data>(lock: .init())
    private let addressesNotNeedingLowTrustWarningCache = AtomicSet<ZonaRosaServiceAddress>(lock: .init())
    private let groupIdsNotNeedingLowTrustWarningCache = AtomicSet<Data>(lock: .init())

    private let intersectionQueue = DispatchQueue(label: "io.zonarosa.contacts.intersection")
    ...
    private let avatarAddressesBeingDownloaded = AtomicSet<ZonaRosaServiceAddress>(lock: .init())
    private let avatarGroupIdsBeingDownloaded = AtomicSet<Data>(lock: .init())
    public let avatarAddressesToShowDownloadingSpinner = AtomicSet<ZonaRosaServiceAddress>(lock: .init())
    ...
    /// contacts haven't changed, and will clear out any stale cached ZonaRosaAccounts
...
extension OWSContactsManager: ContactManager {
    ...
    private func isInWhitelistedGroupsWithLocalUser(
        otherAddress: ZonaRosaServiceAddress,
    ...
    public func shouldBlurContactAvatar(address: ZonaRosaServiceAddress, tx: DBReadTransaction) -> Bool {
    ...
    public func shouldBlockAvatarDownload(address: ZonaRosaServiceAddress, tx: DBReadTransaction) -> Bool {
    ...
    private func doNotBlurContactAvatar(address: ZonaRosaServiceAddress, transaction tx: DBWriteTransaction) {
    ...
    private func postAvatarBlurDidChangeNotification(for address: ZonaRosaServiceAddress) {
    ...
    public func avatarImage(forAddress address: ZonaRosaServiceAddress?, transaction: DBReadTransaction) -> UIImage? {
    ...
    public func avatarImageData(forAddress address: ZonaRosaServiceAddress?, transaction: DBReadTransaction) -> Data? {
    ...
    private func profileAvatarImageData(for address: ZonaRosaServiceAddress, tx: DBReadTransaction) -> Data? {
        return SSKEnvironment.shared.profileManagerImplRef.userProfile(for: address, tx: tx)?.loadAvatarData()
    }

    private func systemContactOrSyncedImageData(for address: ZonaRosaServiceAddress, tx: DBReadTransaction) -> Data? {
        ...
        guard
            ...
            let zonarosaAccount = self.fetchZonaRosaAccount(forPhoneNumber: phoneNumber, transaction: tx),
            let cnContactId = zonarosaAccount.cnContactId,
    ...
    private func discoverableRecipient(for canonicalPhoneNumber: CanonicalPhoneNumber, tx: DBReadTransaction) -> ZonaRosaRecipient? {
    ...
    private func buildZonaRosaAccounts(
        for fetchedSystemContacts: FetchedSystemContacts,
        transaction: DBReadTransaction,
    ) -> [ZonaRosaAccount] {
        var discoverableRecipients = [CanonicalPhoneNumber: (ZonaRosaRecipient, ServiceId)]()
        var discoverablePhoneNumberCounts = [String: Int]()
        for (phoneNumber, contactRef) in fetchedSystemContacts.phoneNumberToContactRef {
            guard let zonarosaRecipient = discoverableRecipient(for: phoneNumber, tx: transaction) else {
                // Not discoverable.
                continue
            }
            guard let serviceId = zonarosaRecipient.aci ?? zonarosaRecipient.pni else {
                owsFailDebug("Can't be discoverable without an ACI or PNI.")
                continue
            }
            discoverableRecipients[phoneNumber] = (zonarosaRecipient, serviceId)
            discoverablePhoneNumberCounts[contactRef.cnContactId, default: 0] += 1
        }
        var zonarosaAccounts = [ZonaRosaAccount]()
        for (phoneNumber, contactRef) in fetchedSystemContacts.phoneNumberToContactRef {
            guard let (zonarosaRecipient, serviceId) = discoverableRecipients[phoneNumber] else {
                continue
            }
            ...
            let zonarosaAccount = ZonaRosaAccount(
                recipientPhoneNumber: zonarosaRecipient.phoneNumber?.stringValue,
            ...
            zonarosaAccounts.append(zonarosaAccount)
        }
        return zonarosaAccounts
    }

    private func buildZonaRosaAccountsAndUpdatePersistedState(for fetchedSystemContacts: FetchedSystemContacts) {
        assertOnQueue(intersectionQueue)

        let (oldZonaRosaAccounts, newZonaRosaAccounts) = SSKEnvironment.shared.databaseStorageRef.read { transaction in
            let oldZonaRosaAccounts = ZonaRosaAccount.anyFetchAll(transaction: transaction)
            let newZonaRosaAccounts = buildZonaRosaAccounts(for: fetchedSystemContacts, transaction: transaction)
            return (oldZonaRosaAccounts, newZonaRosaAccounts)
        }
        let oldZonaRosaAccountsMap: [String?: ZonaRosaAccount] = Dictionary(
            oldZonaRosaAccounts.lazy.map { ($0.recipientPhoneNumber, $0) },
            uniquingKeysWith: { _, new in new },
        )
        var newZonaRosaAccountsMap = [String: ZonaRosaAccount]()

        var zonarosaAccountChanges: [(remove: ZonaRosaAccount?, insert: ZonaRosaAccount?)] = []
        for newZonaRosaAccount in newZonaRosaAccounts {
            guard let phoneNumber = newZonaRosaAccount.recipientPhoneNumber else {
            ...
            if newZonaRosaAccountsMap[phoneNumber] != nil {
                Logger.warn("Ignoring redundant zonarosa account")
                continue
            }

            let oldZonaRosaAccountToKeep: ZonaRosaAccount?
            let oldZonaRosaAccount = oldZonaRosaAccountsMap[phoneNumber]
            switch oldZonaRosaAccount {
            case .none:
                oldZonaRosaAccountToKeep = nil

            case .some(let oldZonaRosaAccount) where oldZonaRosaAccount.hasSameContent(newZonaRosaAccount) && !oldZonaRosaAccount.hasDeprecatedRepresentation:
                // Same content, no need to update.
                oldZonaRosaAccountToKeep = oldZonaRosaAccount

            case .some:
                oldZonaRosaAccountToKeep = nil
            }

            if let oldZonaRosaAccount = oldZonaRosaAccountToKeep {
                newZonaRosaAccountsMap[phoneNumber] = oldZonaRosaAccount
            } else {
                newZonaRosaAccountsMap[phoneNumber] = newZonaRosaAccount
                zonarosaAccountChanges.append((oldZonaRosaAccount, newZonaRosaAccount))
            }
        }

        // Clean up orphans.
        for zonarosaAccount in oldZonaRosaAccounts {
            if let phoneNumber = zonarosaAccount.recipientPhoneNumber, newZonaRosaAccountsMap[phoneNumber]?.uniqueId == zonarosaAccount.uniqueId {
                // Don't clean up ZonaRosaAccounts that aren't changing.
                continue
            }
            // Clean up instances that have been replaced by another instance or are no
            // longer in the system contacts.
            zonarosaAccountChanges.append((zonarosaAccount, nil))
        }

        // Update cached ZonaRosaAccounts on disk
        SSKEnvironment.shared.databaseStorageRef.write { tx in
            for (zonarosaAccountToRemove, zonarosaAccountToInsert) in zonarosaAccountChanges {
                let oldZonaRosaAccount = zonarosaAccountToRemove.flatMap {
                    ZonaRosaAccount.anyFetch(uniqueId: $0.uniqueId, transaction: tx)
                }
                let newZonaRosaAccount = zonarosaAccountToInsert

                oldZonaRosaAccount?.anyRemove(transaction: tx)
                newZonaRosaAccount?.anyInsert(transaction: tx)

                updatePhoneNumberVisibilityIfNeeded(
                    oldZonaRosaAccount: oldZonaRosaAccount,
                    newZonaRosaAccount: newZonaRosaAccount,
                    tx: tx,
                )
            }

            if !zonarosaAccountChanges.isEmpty {
                Logger.info("Updated \(zonarosaAccountChanges.count) ZonaRosaAccounts; now have \(newZonaRosaAccountsMap.count) total")
            }
            ...
            for phoneNumber in newZonaRosaAccountsMap.keys {
        ...
        // Once we've persisted new ZonaRosaAccount state, we should let
        // StorageService know.
        updateStorageServiceForSystemContactsFetch(
            allZonaRosaAccountsBeforeFetch: oldZonaRosaAccountsMap,
            allZonaRosaAccountsAfterFetch: newZonaRosaAccountsMap,
        )

        let didChangeAnyZonaRosaAccount = !zonarosaAccountChanges.isEmpty
        DispatchQueue.main.async {
            // Post a notification if something changed or this is the first load since launch.
            let shouldNotify = didChangeAnyZonaRosaAccount || !self.hasLoadedSystemContacts
            self.hasLoadedSystemContacts = true
            self.didUpdateZonaRosaAccounts(shouldNotify: shouldNotify)
        }
    }

    /// Updates StorageService records for any ZonaRosa contacts associated with
    ...
    private func updateStorageServiceForSystemContactsFetch(
        allZonaRosaAccountsBeforeFetch: [String?: ZonaRosaAccount],
        allZonaRosaAccountsAfterFetch: [String: ZonaRosaAccount],
    ) {
        let tsAccountManager = DependenciesBridge.shared.tsAccountManager
        ...
        var allZonaRosaAccountsBeforeFetch = allZonaRosaAccountsBeforeFetch
        for (phoneNumber, newZonaRosaAccount) in allZonaRosaAccountsAfterFetch {
            let oldZonaRosaAccount = allZonaRosaAccountsBeforeFetch.removeValue(forKey: phoneNumber)
            if let oldZonaRosaAccount, newZonaRosaAccount.hasSameName(oldZonaRosaAccount) {
        ...
        phoneNumbersToUpdateInStorageService.append(
            contentsOf: allZonaRosaAccountsBeforeFetch.keys.lazy.compactMap { $0 },
        )
    ...
    private func updatePhoneNumberVisibilityIfNeeded(
        oldZonaRosaAccount: ZonaRosaAccount?,
        newZonaRosaAccount: ZonaRosaAccount?,
        tx: DBWriteTransaction,
    ) {
        let aciToUpdate = ZonaRosaAccount.aciForPhoneNumberVisibilityUpdate(
            oldAccount: oldZonaRosaAccount,
            newAccount: newZonaRosaAccount,
        )
        ...
        SSKEnvironment.shared.zonarosaServiceAddressCacheRef.updateRecipient(recipient, tx: tx)
    }

    public func didUpdateZonaRosaAccounts(transaction: DBWriteTransaction) {
        transaction.addFinalizationBlock(key: "OWSContactsManager.didUpdateZonaRosaAccounts") { _ in
            self.didUpdateZonaRosaAccounts(shouldNotify: true)
        }
    }

    private func didUpdateZonaRosaAccounts(shouldNotify: Bool) {
        if shouldNotify {
            NotificationCenter.default.postOnMainThread(name: .OWSContactsManagerZonaRosaAccountsDidChange, object: nil)
        }
    }
    ...
    private func intersectContacts(
        ...
        let (intersectionMode, zonarosaRecipientPhoneNumbers) = SSKEnvironment.shared.databaseStorageRef.read { tx in
            let intersectionMode = fetchIntersectionMode(isUserRequested: isUserRequested, tx: tx)
            let zonarosaRecipientPhoneNumbers = recipientDatabaseTable.fetchAllPhoneNumbers(tx: tx)
            return (intersectionMode, zonarosaRecipientPhoneNumbers)
        }

        var phoneNumbersToIntersect = Set(zonarosaRecipientPhoneNumbers.keys)
        ...
        intersectionPromise.done(on: intersectionQueue) { intersectedRecipients in
            ...
            self.buildZonaRosaAccountsAndUpdatePersistedState(for: fetchedSystemContacts)
            ...
            try SSKEnvironment.shared.databaseStorageRef.write { tx in
                self.postJoinNotificationsIfNeeded(
                    ...
                    phoneNumberRegistrationStatus: zonarosaRecipientPhoneNumbers,
    ...
    private func postJoinNotificationsIfNeeded(
        ...
        intersectedRecipients: some Sequence<ZonaRosaRecipient>,
        tx: DBWriteTransaction,
    ) {
        ...
        for zonarosaRecipient in intersectedRecipients {
            guard let phoneNumber = zonarosaRecipient.phoneNumber, phoneNumber.isDiscoverable else {
            ...
            NewAccountDiscovery.postNotification(for: zonarosaRecipient, tx: tx)
        }
    }
    ...
    private func intersectContacts(_ phoneNumbers: Set<String>) async throws -> [ZonaRosaRecipient] {
    ...
    func fetchProfile(forUnknownAddress address: ZonaRosaServiceAddress) {
    ...
    private func displayNamesRefinery(
        for addresses: [ZonaRosaServiceAddress],
        transaction: DBReadTransaction,
    ) -> Refinery<ZonaRosaServiceAddress, DisplayName> {
        let tx = transaction
        return .init(addresses).refine { addresses -> [DisplayName?] in
            ...
            return addresses.lazy.map { address -> DisplayName in
                let zonarosaRecipient = recipientDatabaseTable.fetchRecipient(
                    address: address,
                    tx: transaction,
                )
                if let zonarosaRecipient, !zonarosaRecipient.isRegistered {
    ...
    public func displayNamesByAddress(
        for addresses: [ZonaRosaServiceAddress],
        transaction: DBReadTransaction,
    ) -> [ZonaRosaServiceAddress: DisplayName] {
    ...
    private func systemContactNames(for addresses: some Sequence<ZonaRosaServiceAddress>, tx: DBReadTransaction) -> [DisplayName.SystemContactName?] {
    ...
    public func fetchZonaRosaAccounts(
        for phoneNumbers: [String],
        transaction: DBReadTransaction,
    ) -> [ZonaRosaAccount?] {
        return ZonaRosaAccountFinder().zonarosaAccounts(for: phoneNumbers, tx: transaction)
    }

    public func shortestDisplayName(
        forGroupMember groupMember: ZonaRosaServiceAddress,
...
extension ContactManager {
    ...
    public func sortZonaRosaServiceAddresses(
        _ addresses: some Sequence<ZonaRosaServiceAddress>,
        transaction: DBReadTransaction,
    ) -> [ZonaRosaServiceAddress] {
        return sortedComparableNames(for: addresses, tx: transaction).map { $0.address }
    }

    public func sortedComparableNames(
        for addresses: some Sequence<ZonaRosaServiceAddress>,
        tx: DBReadTransaction,
