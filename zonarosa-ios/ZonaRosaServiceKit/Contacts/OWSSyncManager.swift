...
public class OWSSyncManager {
    ...
    private func addObservers() {
        NotificationCenter.default.addObserver(self, selector: #selector(zonarosaAccountsDidChange(_:)), name: .OWSContactsManagerZonaRosaAccountsDidChange, object: nil)
    ...
    @objc
    private func zonarosaAccountsDidChange(_ notification: AnyObject) {
...
extension OWSSyncManager: SyncManagerProtocol, SyncManagerProtocolSwift {
    ...
    public func processIncomingMessageRequestResponseSyncMessage(
        ...
        guard
            let thread = { () -> TSThread? in
                ...
                {
                    return TSContactThread.getWithContactAddress(ZonaRosaServiceAddress(threadAci), transaction: transaction)
                }
                return nil
            }()
        else {
    ...
    public func syncAllContacts() async throws {
        ...
        try await syncContacts(mode: .allZonaRosaAccounts)
    }

    fileprivate func syncAllContactsIfNecessary() async throws {
        owsAssertDebug(CurrentAppContext().isMainApp)
        try await syncContacts(mode: .allZonaRosaAccountsIfChanged)
    }

    public func syncAllContactsIfFullSyncRequested() async throws {
        owsAssertDebug(CurrentAppContext().isMainApp)
        try await syncContacts(mode: .allZonaRosaAccountsIfFullSyncRequested)
    }

    private enum ContactSyncMode {
        case allZonaRosaAccounts
        case allZonaRosaAccountsIfChanged
        case allZonaRosaAccountsIfFullSyncRequested
    }
    ...
    private func _syncContacts(mode: ContactSyncMode) async throws {
        ...
        let opportunistic = mode == .allZonaRosaAccountsIfChanged
        ...
        // we should send that request since we've been given a strong zonarosa that
    ...
    private func buildContactSyncMessage(
        ...
        if mode == .allZonaRosaAccountsIfFullSyncRequested, fullSyncRequestId == nil {
            return nil
        }
