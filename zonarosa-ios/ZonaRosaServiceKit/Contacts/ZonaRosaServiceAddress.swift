...
@objc
public class ZonaRosaServiceAddress: NSObject, NSCopying, NSSecureCoding, Codable {
    ...
    /// to match what's currently in the cache (ie what's on ZonaRosaRecipient).
    public func withNormalizedPhoneNumber(cache: ZonaRosaServiceAddressCache? = nil) -> ZonaRosaServiceAddress {
        ...
        return ZonaRosaServiceAddress(
            ...
            cache: cache ?? SSKEnvironment.shared.zonarosaServiceAddressCacheRef,
        )
    }
    ...
    public func withNormalizedPhoneNumberAndServiceId(cache: ZonaRosaServiceAddressCache? = nil) -> ZonaRosaServiceAddress {
        return withNormalizedPhoneNumber(cache: cache).withNormalizedServiceId(cache: cache)
    }

    private func withNormalizedServiceId(cache: ZonaRosaServiceAddressCache?) -> ZonaRosaServiceAddress {
        let identifiers = cachedAddress.identifiers.get()
    ...
    /// cases where other ZonaRosaServiceAddresses can't be resolved to ACIs.
    ///
    ...
    public static func legacyAddress(serviceId: ServiceId?, phoneNumber: String?) -> ZonaRosaServiceAddress {
        return ZonaRosaServiceAddress(
            serviceId: serviceId,
            legacyPhoneNumber: phoneNumber,
            cache: SSKEnvironment.shared.zonarosaServiceAddressCacheRef,
        )
    }

    public static func legacyAddress(aciString: String?, phoneNumber: String?) -> ZonaRosaServiceAddress {
        return ZonaRosaServiceAddress(
    ...
    @objc
    public static func legacyAddress(serviceIdString: String?, phoneNumber: String?) -> ZonaRosaServiceAddress {
        return ZonaRosaServiceAddress(
    ...
    public convenience init(serviceIdString: String?, allowPni: Bool, phoneNumber: String?) {
        self.init(
            serviceId: serviceIdString.flatMap {
                ...
                owsFailDebug("Unexpectedly initialized ZonaRosaServiceAddress with invalid serviceIdString.")
                return nil
            },
            phoneNumber: phoneNumber,
        )
    ...
    private init(
        ...
        if let phoneNumber, phoneNumber.isEmpty {
            owsFailDebug("Unexpectedly initialized zonarosa service address with invalid phone number")
        }
    ...
    override public func isEqual(_ object: Any?) -> Bool {
        guard let otherAddress = object as? ZonaRosaServiceAddress else {
            return false
        }
    ...
    @objc
    public func isEqualToAddress(_ otherAddress: ZonaRosaServiceAddress?) -> Bool {
...
public class ZonaRosaServiceAddressCache: NSObject {
    ...
    /// Updates the cache to reflect `zonarosaRecipient`.
    ///
    /// This method doesn't require a write transaction to function, but there's
    /// an assumption throughout the application that ZonaRosaServiceAddresses
    ...
    public func updateRecipient(_ zonarosaRecipient: ZonaRosaRecipient, tx: DBWriteTransaction) {
        updateRecipient(
            zonarosaRecipient,
            isPhoneNumberVisible: phoneNumberVisibilityFetcher.isPhoneNumberVisible(for: zonarosaRecipient, tx: tx),
        )
    }

    func updateRecipient(_ zonarosaRecipient: ZonaRosaRecipient, isPhoneNumberVisible: Bool) {
        updateRecipient(
            aci: zonarosaRecipient.aci,
            phoneNumber: zonarosaRecipient.phoneNumber?.stringValue,
            pni: zonarosaRecipient.pni,
    ...
    func updateRecipient(aci: Aci?, phoneNumber: String?, pni: Pni?, isPhoneNumberVisible: Bool) {
        state.update { cacheState in
            ...
            for newOrUpdatedServiceId in newPotentiallyVisibleServiceIds {
                ...
                // future ZonaRosaServiceAddress instances will be able to find it.
    ...
    /// invariant that a ServiceId for a ZonaRosaServiceAddress remains stable.
...
extension ZonaRosaServiceAddress {
    static func randomForTesting() -> ZonaRosaServiceAddress { ZonaRosaServiceAddress(Aci.randomForTesting()) }

    static func isolatedRandomForTesting() -> ZonaRosaServiceAddress {
...
extension ZonaRosaServiceAddressCache {
    func makeAddress(serviceId: ServiceId?, phoneNumber: E164?) -> ZonaRosaServiceAddress {
        ZonaRosaServiceAddress(
            serviceId: serviceId,
