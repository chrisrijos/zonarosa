...
public class GRDBSchemaMigrator {
    ...
    private enum MigrationId: String, CaseIterable {
        ...
        case zonarosaAccount_add_contactAvatars
        case zonarosaAccount_add_contactAvatars_indices
        case jobRecords_add_attachmentId
        case createMediaGalleryItems
        case createReaction
        case dedupeZonaRosaRecipients
        ...
        case addUnregisteredAtTimestampToZonaRosaRecipient
        ...
        case expandZonaRosaAccountContactFields
...
// if we ever re-use an existing identifier.
        ...
        case zonarosaAccount_add_contactAvatar
        case zonarosaAccount_add_contactAvatarData
        case zonarosaAccount_add_contactAvatarPngData
    ...
    private static func registerSchemaMigrations(migrator: DatabaseMigratorWrapper) {
        ...
        migrator.registerMigration(.createInitialSchema) { tx in
            ...
            CREATE
                TABLE
                    IF NOT EXISTS "model_ZonaRosaRecipient" (
                        "id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL
                        ,"recordType" INTEGER NOT NULL
                        ,"uniqueId" TEXT NOT NULL UNIQUE
                            ON CONFLICT FAIL
            ...
            CREATE
                INDEX "index_model_ZonaRosaRecipient_on_uniqueId"
                    ON "model_ZonaRosaRecipient"("uniqueId"
            )
            ;

            CREATE
                TABLE
                    IF NOT EXISTS "model_ZonaRosaAccount" (
                        "id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL
                        ,"recordType" INTEGER NOT NULL
                        ,"uniqueId" TEXT NOT NULL UNIQUE
                            ON CONFLICT FAIL
            ...
            CREATE
                INDEX "index_model_ZonaRosaAccount_on_uniqueId"
                    ON "model_ZonaRosaAccount"("uniqueId"
            )
            ;
            ...
            CREATE
                INDEX "index_zonarosa_accounts_on_recipientPhoneNumber"
                    ON "model_ZonaRosaAccount"("recipientPhoneNumber"
            )
            ;

            CREATE
                INDEX "index_zonarosa_accounts_on_recipientUUID"
                    ON "model_ZonaRosaAccount"("recipientUUID"
            )
            ;

            CREATE
                INDEX "index_zonarosa_recipients_on_recipientPhoneNumber"
                    ON "model_ZonaRosaRecipient"("recipientPhoneNumber"
            )
            ;

            CREATE
                INDEX "index_zonarosa_recipients_on_recipientUUID"
                    ON "model_ZonaRosaRecipient"("recipientUUID"
            )
            ;
            ...
            CREATE
                VIRTUAL TABLE
                    "zonarosa_grdb_fts"
                        ...
                        ,ftsIndexableContent
                        ,tokenize = 'unicode61'
                    ) /* zonarosa_grdb_fts(collection,uniqueId,ftsIndexableContent) */
            ;
            """
        ...
        migrator.registerMigration(.zonarosaAccount_add_contactAvatars) { transaction in
            let sql = """
            DROP TABLE "model_ZonaRosaAccount";
            CREATE
                TABLE
                    IF NOT EXISTS "model_ZonaRosaAccount" (
                        "id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL
        ...
        migrator.registerMigration(.zonarosaAccount_add_contactAvatars_indices) { transaction in
            let sql = """
            CREATE
                INDEX IF NOT EXISTS "index_model_ZonaRosaAccount_on_uniqueId"
                    ON "model_ZonaRosaAccount"("uniqueId"
            )
            ;

            CREATE
                INDEX IF NOT EXISTS "index_zonarosa_accounts_on_recipientPhoneNumber"
                    ON "model_ZonaRosaAccount"("recipientPhoneNumber"
            )
            ;

            CREATE
                INDEX IF NOT EXISTS "index_zonarosa_accounts_on_recipientUUID"
                    ON "model_ZonaRosaAccount"("recipientUUID"
            )
            ;
            """
        ...
        migrator.registerMigration(.dedupeZonaRosaRecipients) { transaction in
            try dedupeZonaRosaRecipients(tx: transaction)

            try transaction.database.drop(index: "index_zonarosa_recipients_on_recipientPhoneNumber")
            try transaction.database.drop(index: "index_zonarosa_recipients_on_recipientUUID")

            try transaction.database.create(
                index: "index_zonarosa_recipients_on_recipientPhoneNumber",
                on: "model_ZonaRosaRecipient",
        ...
        migrator.registerMigration(.createIndexableFTSTable) { transaction in
            try Bench(title: MigrationId.createIndexableFTSTable.rawValue, logInProduction: true) {
                ...
                try transaction.database.execute(sql: "INSERT INTO indexable_text (collection, uniqueId, ftsIndexableContent) SELECT collection, uniqueId, ftsIndexableContent FROM zonarosa_grdb_fts")
                try transaction.database.drop(table: "zonarosa_grdb_fts")
            }
            return .success(())
        }
        ...
        migrator.registerMigration(.addUnregisteredAtTimestampToZonaRosaRecipient) { transaction in
            try transaction.database.alter(table: "model_ZonaRosaRecipient") { table in
        ...
        migrator.registerMigration(.addHiddenRecipientsTable) { transaction in
            try transaction.database.create(table: HiddenRecipient.databaseTableName) { table in
                ...
                table.foreignKey(
                    ["recipientId"],
                    references: "model_ZonaRosaRecipient",
                    columns: ["id"],
                    onDelete: .cascade,
                )
            }
        ...
        migrator.registerMigration(.addRecipientPniColumn) { transaction in
            ...
            try transaction.database.create(
                index: "index_zonarosa_recipients_on_pni",
                on: "model_ZonaRosaRecipient",
                columns: ["pni"],
                options: [.unique],
            )
        ...
        migrator.registerMigration(.addPhoneNumberSharingAndDiscoverability) { tx in
            try tx.database.alter(table: "model_ZonaRosaRecipient") { table in
        ...
        migrator.registerMigration(.removeRedundantPhoneNumbers2) { tx in
            ...
            try removeLocalProfileZonaRosaRecipient(in: tx.database)
        ...
        migrator.registerMigration(.addSearchableName) { tx in
            ...
            try tx.database.create(table: "SearchableName") { table in
                ...
                table.column("zonarosaAccountId", .integer).unique()
                table.column("userProfileId", .integer).unique()
                table.column("zonarosaRecipientId", .integer).unique()
                ...
                table.foreignKey(["zonarosaAccountId"], references: "model_ZonaRosaAccount", columns: ["id"], onDelete: .cascade, onUpdate: .cascade)
                table.foreignKey(["userProfileId"], references: "model_OWSUserProfile", columns: ["id"], onDelete: .cascade, onUpdate: .cascade)
                table.foreignKey(["zonarosaRecipientId"], references: "model_ZonaRosaRecipient", columns: ["id"], onDelete: .cascade, onUpdate: .cascade)
        ...
        migrator.registerMigration(.addNicknamesTable) { tx in
            try tx.database.create(table: "NicknameRecord") { table in
                table.column("recipientRowID", .integer).primaryKey().notNull()
                    .references("model_ZonaRosaRecipient", column: "id", onDelete: .cascade)
        ...
        migrator.registerMigration(.expandZonaRosaAccountContactFields) { tx in
            ...
            try tx.database.alter(table: "model_ZonaRosaAccount") { table in
        ...
        migrator.registerMigration(.cleanUpUniqueIndexes) { tx in
            ...
            DROP INDEX IF EXISTS "index_model_ZonaRosaAccount_on_uniqueId";
            DROP INDEX IF EXISTS "index_model_ZonaRosaRecipient_on_uniqueId";
        ...
        migrator.registerMigration(.removeVoIPToken) { tx in
            try tx.database.execute(sql: """
            DELETE FROM "keyvalue" WHERE "collection" = 'ZonaRosaPreferences' AND "key" = 'LastRecordedVoipToken'
            """)
            return .success(())
        }
        ...
        migrator.registerMigration(.addGroupSendEndorsement) { tx in
            ...
            try tx.database.create(table: "IndividualGroupSendEndorsement") { table in
                ...
                table.column("recipientId", .integer).notNull()
                    .references("model_ZonaRosaRecipient", column: "id", onDelete: .cascade, onUpdate: .cascade)
                table.column("endorsement", .blob).notNull()
            }
            try tx.database.create(
    ...
    private static func registerDataMigrations(migrator: DatabaseMigratorWrapper) {
        ...
        migrator.registerMigration(.dataMigration_turnScreenSecurityOnForExistingUsers) { transaction in
            ...
            // available in ZonaRosaMessaging.Preferences.
            let preferencesKeyValueStore = KeyValueStore(collection: "ZonaRosaPreferences")
        ...
        migrator.registerMigration(.dataMigration_populateGroupMember) { transaction in
            TSThread.anyEnumerate(
                ...
                groupThread.groupMembership.fullMembers.forEach { address in
                    ...
                    // so defer to the address from the zonarosa recipient (if one exists)
        ...
        migrator.registerMigration(.dataMigration_scheduleStorageServiceUpdateForSystemContacts) { transaction in
            ...
            // contact, if present. Consequently, for all ZonaRosa contacts with
            ...
            var accountsToRemove: Set<ZonaRosaAccount> = []

            ZonaRosaAccount.anyEnumerate(transaction: transaction) { account, _ in
    ...
    static func removeLocalProfileZonaRosaRecipient(in db: Database) throws {
        try db.execute(sql: """
        DELETE FROM "model_ZonaRosaRecipient" WHERE "recipientPhoneNumber" = 'kLocalProfileUniqueId'
        """)
    }
    ...
    static func removeRedundantPhoneNumbers(
        ...
        SET
            "\(phoneNumberColumn)" = NULL,
            "\(serviceIdColumn)" = "zonarosaRecipientAciString"
        FROM (
            SELECT
                "recipientUUID" AS "zonarosaRecipientAciString",
                "recipientPhoneNumber" AS "zonarosaRecipientPhoneNumber"
            FROM "model_ZonaRosaRecipient"
        )
        WHERE
            "\(serviceIdColumn)" IS NULL
            AND "\(phoneNumberColumn)" = "zonarosaRecipientPhoneNumber"
            AND "zonarosaRecipientAciString" IS NOT NULL;
        """)
    }
    ...
    private static func isPhoneNumberVisible(phoneNumber: String, aciString: String, tx: DBWriteTransaction) throws -> Bool {
        let isSystemContact = try Int.fetchOne(
            tx.database,
            sql: "SELECT 1 FROM model_ZonaRosaAccount WHERE recipientPhoneNumber IS ?",
    ...
    private static func fetchOrCreateRecipient(_ version: RecipientVersion, aciString: String, tx: DBWriteTransaction) throws -> ZonaRosaRecipient.RowId {
        let db = tx.database
        let existingRecipientId = try Int64.fetchOne(db, sql: "SELECT id FROM model_ZonaRosaRecipient WHERE recipientUUID IS ?", arguments: [aciString])
    ...
    private static func fetchOrCreateRecipient(_ version: RecipientVersion, phoneNumber: String, tx: DBWriteTransaction) throws -> ZonaRosaRecipient.RowId {
        let db = tx.database
        let existingRecipientId = try Int64.fetchOne(db, sql: "SELECT id FROM model_ZonaRosaRecipient WHERE recipientPhoneNumber IS ?", arguments: [phoneNumber])
        if let existingRecipientId {
            return existingRecipientId
        }
    ...
    private static func fetchOrCreateRecipient(_ version: RecipientVersion, pniString: String, tx: DBWriteTransaction) throws -> ZonaRosaRecipient.RowId {
        let db = tx.database
        let existingRecipientId = try Int64.fetchOne(db, sql: "SELECT id FROM model_ZonaRosaRecipient WHERE pni IS ?", arguments: [pniString])
        if let existingRecipientId {
            return existingRecipientId
        }
    ...
    private static func fetchOrCreateRecipient(_ version: RecipientVersion, address: FrozenZonaRosaServiceAddress, tx: DBWriteTransaction) throws -> ZonaRosaRecipient.RowId? {
    ...
    private static func createRecipient(_ version: RecipientVersion, aciString: String?, phoneNumber: String?, pniString: String?, tx: DBWriteTransaction) throws -> ZonaRosaRecipient.RowId {
        ...
        try tx.database.execute(
            sql: """
            INSERT INTO "model_ZonaRosaRecipient" ("recordType", "uniqueId", "devices", "recipientPhoneNumber", "recipientUUID", "pni") VALUES (31, ?, ?, ?, ?, ?)
            """,
            arguments: [
                UUID().uuidString,
                emptyDevices,
    ...
    private static func fetchRecipientAciString(recipientId: ZonaRosaRecipient.RowId, tx: DBWriteTransaction) throws -> String? {
        return try String.fetchOne(tx.database, sql: "SELECT recipientUUID FROM model_ZonaRosaRecipient WHERE id = ?", arguments: [recipientId])
    }

    private static func fetchRecipientUniqueId(recipientId: ZonaRosaRecipient.RowId, tx: DBWriteTransaction) throws -> String? {
        return try String.fetchOne(tx.database, sql: "SELECT uniqueId FROM model_ZonaRosaRecipient WHERE id = ?", arguments: [recipientId])
    }
    ...
    static func decodeBlockedGroupIds(dataValue: Data) throws -> [Data] {
        ...
        coder.setClass(TSBlockedGroupModel.self, forClassName: "ZonaRosaServiceKit.TSGroupModelV2")
    ...
    private static func decodeGroupIdFromGroupModelData(
        ...
        coder.setClass(TSGroupModelForMigrations.self, forClassName: "ZonaRosaServiceKit.TSGroupModelV2")
    ...
    static func createStoryRecipients(tx: DBWriteTransaction) throws {
        try tx.database.create(table: "StoryRecipient", options: [.withoutRowID]) { table in
            ...
            table.foreignKey(["recipientId"], references: "model_ZonaRosaRecipient", columns: ["id"], onDelete: .cascade, onUpdate: .cascade)
        }
    ...
    private static func migrateStoryRecipients(tx: DBWriteTransaction) throws {
        ...
        for storyThread in storyThreads {
            ...
            let addresses: [FrozenZonaRosaServiceAddress]
            do {
                addresses = try decodeZonaRosaServiceAddresses(dataValue: addressesData)
            } catch {
    ...
    /// A ZonaRosaServiceAddress without global magic; useful in migrations.
    @objc(FrozenZonaRosaServiceAddress)
    class FrozenZonaRosaServiceAddress: NSObject, NSSecureCoding {
    ...
    static func decodeZonaRosaServiceAddresses(dataValue: Data) throws -> [FrozenZonaRosaServiceAddress] {
        let coder = try NSKeyedUnarchiver(forReadingFrom: dataValue)
        coder.requiresSecureCoding = true
        coder.setClass(FrozenZonaRosaServiceAddress.self, forClassName: "ZonaRosaServiceKit.ZonaRosaServiceAddress")
        let decodedValue = try coder.decodeTopLevelObject(
            of: [NSArray.self, FrozenZonaRosaServiceAddress.self],
            forKey: NSKeyedArchiveRootObjectKey,
        )
        guard let result = decodedValue as? [FrozenZonaRosaServiceAddress] else {
    ...
    static func migrateRecipientDeviceIds(tx: DBWriteTransaction) throws {
        let rowIds = try Int64.fetchAll(tx.database, sql: "SELECT id FROM model_ZonaRosaRecipient")
        for rowId in rowIds {
            let oldEncodedValue = try Data.fetchOne(tx.database, sql: "SELECT devices FROM model_ZonaRosaRecipient WHERE id = ?", arguments: [rowId])
            ...
            try tx.database.execute(sql: "UPDATE model_ZonaRosaRecipient SET devices = ? WHERE id = ?", arguments: [newEncodedValue, rowId])
        }
    }
    ...
    static func createPreKey(tx: DBWriteTransaction) throws {
        ...
        // For fetching keys when asked by LibZonaRosa.
        try tx.database.create(
            index: "PreKey_Unique",
            on: "PreKey",
    ...
    static func migratePreKeys(tx: DBWriteTransaction) throws {
        ...
        try migratePreKeys(
            ...
            guard let decodedValue = try NSKeyedUnarchiver.unarchivedObject(ofClass: ZonaRosaServiceKit.PreKeyRecord.self, from: encodedValue) else {
            ...
            return (
                ...
                serializedRecord: try LibZonaRosaClient.PreKeyRecord(
                    id: keyId,
        ...
        try migratePreKeys(
            ...
            guard let decodedValue = try NSKeyedUnarchiver.unarchivedObject(ofClass: ZonaRosaServiceKit.SignedPreKeyRecord.self, from: encodedValue) else {
                throw OWSGenericError("missing decoded value")
            }
            let keyId = UInt32(bitPattern: decodedValue.id)
            return (
                ...
                isOneTime: false,
                replacedAt: decodedValue.replacedAt,
                serializedRecord: try LibZonaRosaClient.SignedPreKeyRecord(
                    id: keyId,
        ...
        try migratePreKeys(
            ...
            let decodedValue = try JSONDecoder().decode(ZonaRosaServiceKit.KyberPreKeyRecord.self, from: encodedValue)
            return (
                keyId: decodedValue.libZonaRosaRecord.id,
                isOneTime: !decodedValue.isLastResort,
                replacedAt: decodedValue.replacedAt,
                serializedRecord: decodedValue.libZonaRosaRecord.serialize(),
            )
        }
    }
    ...
    static func migrateSessions(
        ...
        for key in keys { try autoreleasepool {
            ...
            let recipientId = try Int64.fetchOne(
                tx.database,
                sql: "SELECT id FROM model_ZonaRosaRecipient WHERE uniqueId = ?",
                arguments: [key],
            )
            guard let recipientId else {
                // If we can't find the ZonaRosaRecipient, these sessions aren't reachable,
    ...
    static func addRecipientStatus(tx: DBWriteTransaction) throws {
        try tx.database.alter(table: "model_ZonaRosaRecipient") {
            $0.add(column: "status", .integer).notNull().defaults(to: 0)
        }
        // For fetching whitelisted recipients.
        try tx.database.create(index: "Recipient_Status", on: "model_ZonaRosaRecipient", columns: ["status"])
    }
    ...
    static func migrateRecipientWhitelist(tx: DBWriteTransaction) throws {
        ...
        var recipientIds = Set<ZonaRosaRecipient.RowId>()
        ...
        for recipientId in recipientIds {
            try tx.database.execute(sql: "UPDATE model_ZonaRosaRecipient SET status = 1 WHERE id = ?", arguments: [recipientId])
        }
    ...
    static func createKeyTransparencyTable(tx: DBWriteTransaction) throws {
        try tx.database.create(table: "KeyTransparency") { table in
            table.column("aci", .blob).primaryKey().notNull()
            table.column("libzonarosaBlob", .blob).notNull()
        }
    }
}
...
extension GRDBSchemaMigrator {
    static func dedupeZonaRosaRecipients(tx: DBWriteTransaction) throws {
        struct Recipient: FetchableRecord, Decodable {
            var id: Int64
            var aciString: String?
            var phoneNumber: String?
        }
        let fetchAllRecipients = "SELECT id, recipientUUID aciString, recipientPhoneNumber phoneNumber FROM model_ZonaRosaRecipient"
        ...
        for (_, rowIds) in recipientsByAciString {
            for duplicateRowId in rowIds.sorted().dropFirst() {
                try tx.database.execute(sql: "DELETE FROM model_ZonaRosaRecipient WHERE id = ?", arguments: [duplicateRowId])
            }
        }
        ...
        for (_, recipients) in recipientsByPhoneNumber {
            for recipient in recipients.sorted(by: { $0.id < $1.id }).dropFirst() {
                ...
                if recipient.aciString != nil {
                    mutationSql = "UPDATE model_ZonaRosaRecipient SET recipientPhoneNumber = NULL WHERE id = ?"
                } else {
                    mutationSql = "DELETE FROM model_ZonaRosaRecipient WHERE id = ?"
                }
