...
/// Wraps the stores for 1:1 sessions that use the ZonaRosa Protocol (Double Ratchet + X3DH).

public struct ZonaRosaProtocolStore {
...
public struct ZonaRosaProtocolStoreManager {
    let aciProtocolStore: ZonaRosaProtocolStore
    let pniProtocolStore: ZonaRosaProtocolStore
    let preKeyStore: PreKeyStore
    let sessionStore: SessionStore

    public func zonarosaProtocolStore(for identity: OWSIdentity) -> ZonaRosaProtocolStore {
        switch identity {
        case .aci:
    ...
    public func removeAllKeys(tx: DBWriteTransaction) {
        for zonarosaProtocolStore in [aciProtocolStore, pniProtocolStore] {
            zonarosaProtocolStore.preKeyStore.removeMetadata(tx: tx)
            zonarosaProtocolStore.signedPreKeyStore.removeMetadata(tx: tx)
            zonarosaProtocolStore.kyberPreKeyStore.removeMetadata(tx: tx)
        }
