...
public class OldSenderKeyStore {
    ...
    func readyRecipients(
        ...
        let sentKeyInfo = { () -> [ZonaRosaServiceAddress: SKDMSendInfo]? in
        ...
        for intendedRecipient in intendedRecipients {
            let priorSendRecipientState: SenderKeySentToRecipient = (
                sentKeyInfo?[ZonaRosaServiceAddress(intendedRecipient)]?.keyRecipient
            ...
            do {
                ...
                if case ZonaRosaError.invalidState = error {
    ...
    public func senderKeyDistributionMessage(
        ...
        return try LibZonaRosaClient.SenderKeyDistributionMessage(
            from: localAddress,
...
// MARK: - <LibZonaRosaClient.SenderKeyStore>

extension OldSenderKeyStore: LibZonaRosaClient.SenderKeyStore {
...
private struct KeyMetadata {
    ...
    private(set) var sentKeyInfo: [ZonaRosaServiceAddress: SKDMSendInfo]
    var currentRecipients: Set<ZonaRosaServiceAddress> { Set(sentKeyInfo.keys) }
    ...
    mutating func resetDeliveryRecord(for serviceId: ServiceId) {
        sentKeyInfo[ZonaRosaServiceAddress(serviceId)] = nil
    }
    ...
    mutating func recordSentSenderKey(_ sentSenderKey: SentSenderKey) {
        ...
        sentKeyInfo[ZonaRosaServiceAddress(sentSenderKey.recipient)] = sendInfo
    }
}
...
extension KeyMetadata: Codable {
    ...
    init(from decoder: Decoder) throws {
        ...
        // - V2: Record a mapping of ZonaRosaServiceAddress -> SenderKeySentToRecipient. This allowed us to
        //       track additional info about the recipient of a key like registrationId
        // - V3: Record a mapping of ZonaRosaServiceAddress -> SKDMSendInfo. This allows us to
        ...
        if let sendInfo = try container.decodeIfPresent([ZonaRosaServiceAddress: SKDMSendInfo].self, forKey: .sentKeyInfo) {
            sentKeyInfo = sendInfo
        } else if let keyRecipients = try legacyValues.decodeIfPresent([ZonaRosaServiceAddress: SenderKeySentToRecipient].self, forKey: .keyRecipients) {
