...
struct SessionRecord: Codable, FetchableRecord, PersistableRecord {
    ...
    let id: Int64
    var recipientId: ZonaRosaRecipient.RowId
...
struct SessionStore {
    func hasSessionRecords(
        forRecipientId recipientId: ZonaRosaRecipient.RowId,
    ...
    func mergeRecipientId(
        ...
        into targetRecipientId: ZonaRosaRecipient.RowId,
        localIdentity: OWSIdentity,
    ...
    func fetchSession(
        ...
        deviceId: DeviceId,
        tx: DBReadTransaction,
    ) -> LibZonaRosaClient.SessionRecord? {
        let sessionRecords = fetchSessionRecords(
            forRecipientId: recipientId,
            localIdentity: localIdentity,
            deviceId: deviceId,
        ...
        do {
            return try (sessionRecords.first?.serializedRecord).map(LibZonaRosaClient.SessionRecord.init(bytes:))
        } catch {
    ...
    fileprivate func _archiveSessions(
        ...
        for var sessionRecord in sessionRecords {
            ...
            let libZonaRosaSessionRecord: LibZonaRosaClient.SessionRecord
            do {
                libZonaRosaSessionRecord = try LibZonaRosaClient.SessionRecord(bytes: serializedRecord)
            } catch {
                owsFailDebug("couldn't decode session to archive it: \(error)")
                continue
            }
            libZonaRosaSessionRecord.archiveCurrentState()
            sessionRecord.serializedRecord = libZonaRosaSessionRecord.serialize()
            failIfThrows { try sessionRecord.update(tx.database) }
        }
    }
...
public class SessionManagerForIdentity: LibZonaRosaClient.SessionStore {
    ...
    func archiveSessions(forAddress address: ZonaRosaServiceAddress, tx: DBWriteTransaction) {
    ...
    private func _archiveSessions(
        recipientIdResult: Result<ZonaRosaRecipient.RowId, RecipientIdError>?,
        deviceId: DeviceId?,
        tx: DBWriteTransaction,
    ) {
    ...
    func loadSession(
        ...
        deviceId: DeviceId,
        tx: DBReadTransaction,
    ) throws -> LibZonaRosaClient.SessionRecord? {
        switch self.recipientIdFinder.recipientId(for: serviceId, tx: tx) {
        case .none:
            return nil
        case .some(.success(let recipientId)):
    ...
    public func loadSession(
        for address: LibZonaRosaClient.ProtocolAddress,
        context: any LibZonaRosaClient.StoreContext,
    ) throws -> LibZonaRosaClient.SessionRecord? {
    ...
    public func storeSession(
        _ record: LibZonaRosaClient.SessionRecord,
        for address: LibZonaRosaClient.ProtocolAddress,
        context: any LibZonaRosaClient.StoreContext,
    ) throws {
