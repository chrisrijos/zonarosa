...
public class OWSMessageDecrypter {
    ...
    private func trySendReactiveProfileKey(to sourceAci: Aci, tx transaction: DBWriteTransaction) {
        ...
        let contactThread = TSContactThread.getOrCreateThread(
            withContactAddress: ZonaRosaServiceAddress(sourceAci),
            transaction: transaction,
        )
    ...
    private func processError(
        ...
        if case ZonaRosaError.duplicatedMessage = error {
        ...
        let sourceAddress = ZonaRosaServiceAddress(sourceAci)
        ...
        default: // another ZonaRosaError, or another kind of Error altogether
            break
        }
    ...
    private func buildResendRequestDecryptionError(
        ...
        guard [.zonarosa, .senderKey, .preKey, .plaintext].contains(unsealedEnvelope.cipherType) else {
            return nil
        }
    ...
    func decryptIdentifiedEnvelope(
        ...
        do {
            ...
            let zonarosaProtocolStore = zonarosaProtocolStoreManager.zonarosaProtocolStore(for: validatedEnvelope.localIdentity)
            let preKeyStore = zonarosaProtocolStoreManager.preKeyStore.forIdentity(validatedEnvelope.localIdentity)

            let plaintext: Data
            switch cipherType {
            case .zonarosa:
                let message = try ZonaRosaMessage(bytes: encryptedData)
                plaintext = try zonarosaDecrypt(
                    message: message,
                    from: protocolAddress,
                    sessionStore: zonarosaProtocolStore.sessionStore,
                    identityStore: identityManager.libZonaRosaStore(for: localIdentity, tx: transaction),
                    context: transaction,
                )
            ...
            case .preKey:
                ...
                let message = try PreKeyZonaRosaMessage(bytes: encryptedData)
                plaintext = try zonarosaDecryptPreKey(
                    message: message,
                    from: protocolAddress,
                    sessionStore: zonarosaProtocolStore.sessionStore,
                    identityStore: identityManager.libZonaRosaStore(for: localIdentity, tx: transaction),
    ...
    private func sendReactiveProfileKeyIfNecessary(to sourceAci: Aci, tx transaction: DBWriteTransaction) {
        ...
        if SSKEnvironment.shared.blockingManagerRef.isAddressBlocked(ZonaRosaServiceAddress(sourceAci), transaction: transaction) {
                ...
                let needsReactiveProfileKeyMessage: Bool = db.read { transaction in
                    ...
                    if
                        profileManager.isUser(
                            inProfileWhitelist: ZonaRosaServiceAddress(sourceAci),
                            transaction: transaction,
                        )
                    {
                        return true
                    ...
                    TSGroupThread.enumerateGroupThreads(
                        with: ZonaRosaServiceAddress(sourceAci),
                        transaction: transaction,
                    ) { thread, stop in
    ...
    func decryptUnidentifiedSenderEnvelope(
        ...
        let identityManager = DependenciesBridge.shared.identityManager
        let zonarosaProtocolStoreManager = DependenciesBridge.shared.zonarosaProtocolStoreManager
        let zonarosaProtocolStore = zonarosaProtocolStoreManager.zonarosaProtocolStore(for: localIdentity)
        let preKeyStore = zonarosaProtocolStoreManager.preKeyStore.forIdentity(localIdentity)
    ...
    private func handleUnidentifiedSenderDecryptionError(
        ...
        case SMKSecretSessionCipherError.selfSentMessage:
            ...
            return error
        case is ZonaRosaError, PreKeyStore.Error.noPreKeyWithId:
            return processError(
                error,
                validatedEnvelope: validatedEnvelope,
                unsealedEnvelope: unsealedEnvelope,
    ...
    private func processDecryptedEnvelope(
        ...
        localIdentifiers: LocalIdentifiers,
        mergeRecipient: (DBWriteTransaction) -> ZonaRosaRecipient,
    ...
    private static func descriptionForEnvelopeType(_ envelope: SSKProtoEnvelope) -> String {
        ...
        case .unknown:
            ...
            return "Unknown"
        case .ciphertext:
            return "ZonaRosaEncryptedMessage"
        case .prekeyBundle:
