...
public protocol RecipientHidingManager {
    ...
    func hiddenRecipients(tx: DBReadTransaction) -> [ZonaRosaRecipient]

    /// Fetch the hidden-recipient state for the given `ZonaRosaRecipient`, if the
    /// `ZonaRosaRecipient` is currently hidden.
    func fetchHiddenRecipient(
        recipientId: ZonaRosaRecipient.RowId,
    ...
    /// Inserts hidden-recipient state for the given `ZonaRosaRecipient`.
    ///
    ...
    ///
    /// - Parameter recipient: A ``ZonaRosaRecipient``.
...
public extension RecipientHidingManager {
    ...
    /// Whether the given `ZonaRosaRecipient` is currently hidden.
...
/// A database record denoting a hidden ``ZonaRosaRecipient`` by their row ID.
...
public struct HiddenRecipient: Codable, FetchableRecord, PersistableRecord {
    ...
    public enum CodingKeys: String, CodingKey {
        case zonarosaRecipientRowId = "recipientId"
        case inKnownMessageRequestState
    }

    /// The hidden recipient's ``ZonaRosaRecipient.id``.
    let zonarosaRecipientRowId: Int64
...
public final class RecipientHidingManagerImpl: RecipientHidingManager {
    ...
    public func hiddenRecipients(tx: DBReadTransaction) -> [ZonaRosaRecipient] {
        do {
            let sql = """
                SELECT \(ZonaRosaRecipient.databaseTableName).*
                FROM \(ZonaRosaRecipient.databaseTableName)
                INNER JOIN \(HiddenRecipient.databaseTableName)
                    AS hiddenRecipient
                    ON hiddenRecipient.recipientId = \(zonarosaRecipientColumn: .id)
            """
            return try ZonaRosaRecipient.fetchAll(tx.database, sql: sql)
        } catch {
    ...
    public func addHiddenRecipient(
        ...
        let record = HiddenRecipient(
            zonarosaRecipientRowId: recipient.id,
...
private extension RecipientHidingManagerImpl {
    ...
    /// to the database.
    ///
    /// - Parameter recipient: The ``ZonaRosaRecipient`` who was just hidden.
    /// - Parameter wasLocallyInitiated: Whether the user initiated
    ///   the hide on this device (true) or a linked device (false).
    /// - Parameter tx: The transaction to use for database operations.
    func didSetAsHidden(
    ...
    /// from the database.
    ///
    /// - Parameter recipient: The ``ZonaRosaRecipient`` who was just unhidden.
    /// - Parameter wasLocallyInitiated: Whether the user initiated
    ///   the hide on this device (true) or a linked device (false).
    /// - Parameter tx: The transaction to use for database operations.
    ///
    /// Note: If a ``ZonaRosaRecipient`` is deleted, a cascade
    /// rule is in place that will also delete the corresponding
    /// `HiddenRecipient` entry. This method does not get hit in
    /// that case.
    func didSetAsUnhidden(recipient: inout ZonaRosaRecipient, wasLocallyInitiated: Bool, tx: DBWriteTransaction) {
        // Triggers UI updates of recipient lists.
        NotificationCenter.default.postOnMainThread(name: Self.hideListDidChange, object: nil)
...
public enum RecipientHidingError: Error, CustomStringConvertible {
    ...
    case invalidRecipientAddress(ZonaRosaServiceAddress)
...
public class RecipientHidingManagerObjcBridge: NSObject {
    ...
    @objc
    public static func isHiddenAddress(_ address: ZonaRosaServiceAddress, tx: DBReadTransaction) -> Bool {
