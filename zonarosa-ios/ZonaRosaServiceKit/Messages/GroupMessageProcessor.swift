...
public class GroupMessageProcessorManager {
    ...
    public static func discardMode(
        ...
        let isBlocked: Bool = (
            blockingManager.isAddressBlocked(ZonaRosaServiceAddress(sourceAci), transaction: tx)
        ...
        if shouldCheckGroupModel {
            ...
            guard groupModel.groupMembership.isFullMember(ZonaRosaServiceAddress(sourceAci)) else {
            ...
            if groupModel.isAnnouncementsOnly {
                guard groupModel.groupMembership.isFullMemberAndAdministrator(ZonaRosaServiceAddress(sourceAci)) else {
