...
public enum SealedSenderContentHint: Int, Codable, CustomStringConvertible {
    ...
    init(_ zonarosaClientHint: UnidentifiedSenderMessageContent.ContentHint) {
        switch zonarosaClientHint {
        case .default: self = .default
        case .resendable: self = .resendable
        case .implicit: self = .implicit
        default:
            owsFailDebug("Unspecified case \(zonarosaClientHint)")
            self = .default
        }
    }

    public var zonarosaClientHint: UnidentifiedSenderMessageContent.ContentHint {
        switch self {
...
public final class MessageReceiver {
    ...
    public func processEnvelope(
        ...
        do {
            ...
            case .unidentifiedSender, .identifiedSender:
                ...
                if SSKEnvironment.shared.blockingManagerRef.isAddressBlocked(ZonaRosaServiceAddress(sourceAci), transaction: tx) {
                    return
                }
            ...
            if let dataMessage = sent.message {
                ...
                if dataMessage.hasProfileKey {
                    if let groupId {
                        ...
                        let destinationAddress = ZonaRosaServiceAddress(
                            serviceId: serviceId,
    ...
    private func preprocessDataMessage(
        ...
        guard let groupContext = dataMessage.groupV2 else {
            let contactAddress = ZonaRosaServiceAddress(envelope.sourceAci)
    ...
    private func processFlaglessDataMessage(
        ...
        guard !dataMessage.hasRequiredProtocolVersion || dataMessage.requiredProtocolVersion <= SSKProtos.currentProtocolVersion else {
            ...
            OWSUnknownProtocolVersionMessage(
                ...
                sender: ZonaRosaServiceAddress(envelope.sourceAci),
        ...
        message.insertOrReplacePlaceholder(from: ZonaRosaServiceAddress(envelope.sourceAci), transaction: tx)
    ...
    private func handleIncomingEnvelope(
        ...
        do {
            let skdm = try LibZonaRosaClient.SenderKeyDistributionMessage(bytes: skdmData)
    ...
    private func handleIncomingEnvelope(
        ...
        if let groupId = typingMessage.groupID {
            ...
            let sourceAddress = ZonaRosaServiceAddress(envelope.sourceAci)
    ...
    func checkForUnknownLinkedDevice(in envelope: DecryptedIncomingEnvelope, tx: DBWriteTransaction) {
        ...
        // Check if the ZonaRosaRecipient (used for sending messages) knows about
        // this device.
        let recipientFetcher = DependenciesBridge.shared.recipientFetcher
        var recipient = recipientFetcher.fetchOrCreate(serviceId: aci, tx: tx)
        if !recipient.deviceIds.contains(deviceId) {
            let recipientManager = DependenciesBridge.shared.recipientManager
            Logger.info("Message received from unknown linked device; adding to local ZonaRosaRecipient: \(deviceId).")
    ...
    private func handleIncomingEndSessionEnvelope(
        ...
        let thread = TSContactThread.getOrCreateThread(
            withContactAddress: ZonaRosaServiceAddress(decryptedEnvelope.sourceAci),
            transaction: tx,
        )
