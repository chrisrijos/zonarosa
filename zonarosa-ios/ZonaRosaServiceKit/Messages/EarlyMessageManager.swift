...
public class EarlyMessageManager {
    ...
    private enum EarlyReceipt: Codable, Hashable {
        ...
        init(from decoder: Decoder) throws {
            ...
            let sender = try container.decodeIfPresent(ZonaRosaServiceAddress.self, forKey: .sender)
        ...
        case outgoingMessageRead(sender: ZonaRosaServiceAddress, deviceId: UInt32, timestamp: UInt64)
        case outgoingMessageDelivered(sender: ZonaRosaServiceAddress, deviceId: UInt32, timestamp: UInt64)
        case outgoingMessageViewed(sender: ZonaRosaServiceAddress, deviceId: UInt32, timestamp: UInt64)
        ...
        init(receiptType: SSKProtoReceiptMessageType, sender: ServiceId, senderDeviceId: DeviceId, timestamp: UInt64) {
            switch receiptType {
            case .delivery: self = .outgoingMessageDelivered(sender: ZonaRosaServiceAddress(sender), deviceId: senderDeviceId.uint32Value, timestamp: timestamp)
            case .read: self = .outgoingMessageRead(sender: ZonaRosaServiceAddress(sender), deviceId: senderDeviceId.uint32Value, timestamp: timestamp)
            case .viewed: self = .outgoingMessageViewed(sender: ZonaRosaServiceAddress(sender), deviceId: senderDeviceId.uint32Value, timestamp: timestamp)
            }
        }
    }
