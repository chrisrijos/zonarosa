...
struct ReceiptForLinkedDevice: Codable {
    ...
    init(senderAddress: ZonaRosaServiceAddress, messageUniqueId: String, messageIdTimestamp: UInt64, timestamp: UInt64) {
...
extension OWSReceiptManager {
    ...
    private func enqueueLinkedDeviceViewedReceipt(
        messageAuthorAddress: ZonaRosaServiceAddress,
        ...
        return processReceiptsForMessages(sentAt: sentTimestamps, tx: tx) { _, messages in
            if !messages.isEmpty {
                for message in messages {
                    message.update(
                        withDeliveredRecipient: ZonaRosaServiceAddress(recipientServiceId),
    ...
    func processReadReceipts(
        ...
        return processReceiptsForMessages(sentAt: sentTimestamps, tx: tx) { _, messages in
            if !messages.isEmpty {
                ...
                for message in messages {
                    message.update(
                        withReadRecipient: ZonaRosaServiceAddress(recipientAci),
                        deviceId: recipientDeviceId,
                        readTimestamp: readTimestamp,
                        tx: tx,
                    )
        ...
        return processReceiptsForMessages(sentAt: sentTimestamps, tx: tx) { sentTimestamp, messages in
            if !messages.isEmpty {
                if Self.areReadReceiptsEnabled(transaction: tx) {
                    for message in messages {
                        message.update(
                            withViewedRecipient: ZonaRosaServiceAddress(recipientAci),
                            deviceId: recipientDeviceId,
