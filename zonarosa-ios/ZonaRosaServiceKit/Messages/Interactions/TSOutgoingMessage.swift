...
extension TSOutgoingMessage {
    @objc
    open func recipientState(for address: ZonaRosaServiceAddress) -> TSOutgoingMessageRecipientState? {
        return recipientAddressStates?[address]
    }

    /// All recipients of this message.
    @objc
    public func recipientAddresses() -> [ZonaRosaServiceAddress] {
    ...
    @objc
    public func sendingRecipientAddresses() -> [ZonaRosaServiceAddress] {
    ...
    @objc
    public func sentRecipientAddresses() -> [ZonaRosaServiceAddress] {
        return filterRecipientAddresses { state in
            switch state.status {
    ...
    /// - Note: we only learn "read" status if read receipts are enabled.
    @objc
    public func deliveredRecipientAddresses() -> [ZonaRosaServiceAddress] {
        return filterRecipientAddresses { state in
            switch state.status {
    ...
    /// - Note: we only learn "read" status if read receipts are enabled.
    @objc
    open func readRecipientAddresses() -> [ZonaRosaServiceAddress] {
        return filterRecipientAddresses { state in
            switch state.status {
    ...
    /// viewed.
    @objc
    public func viewedRecipientAddresses() -> [ZonaRosaServiceAddress] {
        return filterRecipientAddresses { state in
            switch state.status {
    ...
    @objc
    public func failedRecipientAddresses(errorCode: Int) -> [ZonaRosaServiceAddress] {
        return filterRecipientAddresses { state in
    ...
    public func updateWithRecipientAddressStates(
        _ recipientAddressStates: [ZonaRosaServiceAddress: TSOutgoingMessageRecipientState]?,
    ...
    func updateWithSentRecipients(
        ...
        anyUpdateOutgoingMessage(transaction: tx) { outgoingMessage in
            for serviceId in serviceIds {
                let address = ZonaRosaServiceAddress(serviceId)
    ...
    func updateRecipientsFromNonLocalDevice(
        _ nonLocalRecipientStates: [ZonaRosaServiceAddress: TSOutgoingMessageRecipientState],
        isSentUpdate: Bool,
    ...
    func updateWithFailedRecipients(
        ...
        self.anyUpdateOutgoingMessage(transaction: tx) {
            for (serviceId, error) in recipientErrors {
                guard let recipientState = $0.recipientAddressStates?[ZonaRosaServiceAddress(serviceId)] else {
    ...
    private func maybeClearShouldSharePhoneNumber(
        for recipientAddress: ZonaRosaServiceAddress,
    ...
    public func update(
        withDeliveredRecipient recipientAddress: ZonaRosaServiceAddress,
        deviceId: DeviceId,
    ...
    public func update(
        withReadRecipient recipientAddress: ZonaRosaServiceAddress,
        deviceId: DeviceId,
    ...
    public func update(
        withViewedRecipient recipientAddress: ZonaRosaServiceAddress,
        deviceId: DeviceId,
    ...
    private func handleReceipt(
        from recipientAddress: ZonaRosaServiceAddress,
        deviceId: DeviceId,
    ...
    /// Used in MessageSender to zonarosa how a message should be encrypted before sending
    /// Currently only overridden by OutgoingResendRequest (this is asserted in the MessageSender implementation)
    @objc
    var encryptionStyle: EncryptionStyle { .zonarosa }

    func clearMessageSendLogEntry(forRecipient address: ZonaRosaServiceAddress, deviceId: DeviceId, tx: DBWriteTransaction) {
