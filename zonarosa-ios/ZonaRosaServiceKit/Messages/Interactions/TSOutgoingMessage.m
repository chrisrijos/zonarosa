                                   ...
                                   transaction:(DBReadTransaction *)transaction
{
    ...
    NSMutableSet<ZonaRosaServiceAddress *> *recipientAddresses = [NSMutableSet new];
    if ([self isKindOfClass:[OWSOutgoingSyncMessage class]]) {
        // Sync messages should only be sent to linked devices.
        ZonaRosaServiceAddress *localAddress = [TSAccountManagerObjcBridge localAciAddressWith:transaction];
        ...
        if (explicitRecipients.count > 0) {
            NSMutableSet<ZonaRosaServiceAddress *> *explicitRecipientAddresses = [[NSMutableSet alloc] init];
            for (AciObjC *recipientAci in explicitRecipients) {
                [explicitRecipientAddresses
                    addObject:[[ZonaRosaServiceAddress alloc] initWithServiceIdObjC:recipientAci]];
            }
        ...
        if (additionalRecipients.count > 0) {
            for (ServiceIdObjC *serviceId in additionalRecipients) {
                [recipientAddresses addObject:[[ZonaRosaServiceAddress alloc] initWithServiceIdObjC:serviceId]];
            }
        }
    }

    NSSet<ServiceIdObjC *> *skippedRecipientsSet = [NSSet setWithArray:skippedRecipients];
    NSMutableDictionary<ZonaRosaServiceAddress *, TSOutgoingMessageRecipientState *> *recipientAddressStates =
        [NSMutableDictionary new];
    for (ZonaRosaServiceAddress *recipientAddress in recipientAddresses) {
