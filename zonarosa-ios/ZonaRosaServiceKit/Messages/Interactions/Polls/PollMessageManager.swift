...
public class PollMessageManager {
    ...
    public func processIncomingPollVote(
        ...
        let zonarosaRecipient = recipientDatabaseTable.fetchRecipient(serviceId: voteAuthor, transaction: transaction)

        guard let voteAuthorId = zonarosaRecipient?.id else {
...
public struct BackupsPollData {
    public struct BackupsPollOption {
        public struct BackupsPollVote {
            let voteAuthorId: ZonaRosaRecipient.RowId
