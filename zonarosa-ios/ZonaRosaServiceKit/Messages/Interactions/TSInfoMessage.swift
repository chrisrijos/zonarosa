...
extension TSInfoMessage {
    ...
    func _infoMessagePreviewText(tx: DBReadTransaction) -> String {
            ...
            if let unregisteredAddress, unregisteredAddress.isValid {
                ...
                return String(
                    format: OWSLocalizedString(
                        ...
                        comment: "Format string for 'unregistered user' error. Embeds {{the unregistered user's name or zonarosa id}}.",
                    ),
                    recipientName,
                )
            } else {
        ...
        case .userJoinedZonaRosa:
            ...
            let format = OWSLocalizedString("INFO_MESSAGE_USER_JOINED_ZONAROSA_BODY_FORMAT", comment: "Shown in inbox and conversation when a user joins ZonaRosa, embeds the new user's {{contact name}}")
        ...
        case .phoneNumberChange:
            ...
            let address = ZonaRosaServiceAddress(aci.wrappedAciValue)
