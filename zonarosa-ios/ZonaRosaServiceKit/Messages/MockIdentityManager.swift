...
open class MockIdentityManager: OWSIdentityManager {
    ...
    var sessionSwitchoverMessages: [(ZonaRosaRecipient, phoneNumber: String?)]!
    open func insertSessionSwitchoverEvent(for recipient: ZonaRosaRecipient, phoneNumber: String?, tx: DBWriteTransaction) {
        sessionSwitchoverMessages.append((recipient, phoneNumber))
    }

    open func mergeRecipient(_ recipient: ZonaRosaRecipient, into targetRecipient: ZonaRosaRecipient, tx: DBWriteTransaction) {
    ...
    open func libZonaRosaStore(for identity: OWSIdentity, tx: DBReadTransaction) throws -> IdentityStore { fatalError() }
    open func groupContainsUnverifiedMember(_ groupUniqueID: String, tx: DBReadTransaction) -> Bool { fatalError() }
    open func recipientIdentity(for address: ZonaRosaServiceAddress, tx: DBReadTransaction) -> OWSRecipientIdentity? { fatalError() }
    ...
    open func identityKey(for address: ZonaRosaServiceAddress, tx: DBReadTransaction) -> Data? { fatalError() }
    ...
    open func untrustedIdentityForSending(to address: ZonaRosaServiceAddress, untrustedThreshold: Date?, tx: DBReadTransaction) -> OWSRecipientIdentity? { fatalError() }
    open func tryToSyncQueuedVerificationStates() { fatalError() }
    open func verificationState(for address: ZonaRosaServiceAddress, tx: DBReadTransaction) -> VerificationState { fatalError() }
    open func setVerificationState(_ verificationState: VerificationState, of identityKey: Data, for address: ZonaRosaServiceAddress, isUserInitiatedChange: Bool, tx: DBWriteTransaction) -> ChangeVerificationStateResult { fatalError() }
