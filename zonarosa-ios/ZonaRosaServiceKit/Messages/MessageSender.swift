...
public class MessageSender {
    ...
    // MARK: - Creating ZonaRosa Protocol Sessions

    private func validSession(for serviceId: ServiceId, deviceId: DeviceId, tx: DBReadTransaction) throws -> LibZonaRosaClient.SessionRecord? {
    ...
    private func makePreKeyRequest(
        ...
        deviceId: PreKeyDevice,
        sealedSenderParameters: SealedSenderParameters?,
    ) async throws -> ZonaRosaServiceKit.PreKeyBundle {
        ...
        guard let bundle = try? JSONDecoder().decode(ZonaRosaServiceKit.PreKeyBundle.self, from: responseData) else {
    ...
    private func _createSessions(
        for preKeyBundle: ZonaRosaServiceKit.PreKeyBundle,
        serviceId: ServiceId,
    ...
    private func _createSession(
        for deviceBundle: ZonaRosaServiceKit.PreKeyBundle.PreKeyDeviceBundle,
        serviceId: ServiceId,
        ...
        let bundle: LibZonaRosaClient.PreKeyBundle
        if let preKey = deviceBundle.preKey {
            bundle = try LibZonaRosaClient.PreKeyBundle(
        ...
        do {
            ...
            try processPreKeyBundle(
                bundle,
                for: protocolAddress,
                sessionStore: DependenciesBridge.shared.zonarosaProtocolStoreManager.zonarosaProtocolStore(for: .aci).sessionStore,
                identityStore: identityManager.libZonaRosaStore(for: .aci, tx: transaction),
                context: transaction,
            )
        } catch ZonaRosaError.untrustedIdentity(_), IdentityManagerError.identityKeyMismatchForOutgoingMessage {
            ...
            throw UntrustedIdentityError(serviceId: serviceId)
        } catch ZonaRosaError.invalidSignature(_) {
    ...
    private func willDefinitelyHaveUntrustedIdentityError(for serviceId: ServiceId) -> Bool {
        ...
        return SSKEnvironment.shared.databaseStorageRef.read { tx in
            return identityManager.untrustedIdentityForSending(
                to: ZonaRosaServiceAddress(serviceId),
    ...
    private func markSkippedRecipients(
        ...
        let skippedRecipients = Set(message.sendingRecipientAddresses())
            .subtracting(sendingRecipients.lazy.map { ZonaRosaServiceAddress($0) })
    ...
    private func unsentRecipients(
        ...
        if let groupThread = thread as? TSGroupThread {
            ...
            var recipientAddresses = Set<ZonaRosaServiceAddress>()
            ...
            if let localPni = localIdentifiers.pni {
                currentValidRecipients.remove(ZonaRosaServiceAddress(localPni))
            }
            ...
            if let localPni = localIdentifiers.pni {
                recipientAddresses.remove(ZonaRosaServiceAddress(localPni))
            }
    ...
    private static func partitionAddresses(_ addresses: [ZonaRosaServiceAddress]) -> ([ServiceId], [E164]) {
    ...
    private func handleSendFailure(
        ...
        for (serviceId, error) in allErrors {
            ...
            if !(thread is TSContactThread), error is MessageSenderNoSuchZonaRosaRecipientError {
        ...
        await SSKEnvironment.shared.databaseStorageRef.awaitableWrite { tx in
            if !skippedRecipients.isEmpty {
                message.updateWithSkippedRecipients(skippedRecipients.map { ZonaRosaServiceAddress($0) }, tx: tx)
            }
    ...
    private func normalizeRecipientStatesIfNeeded(
        ...
        let recipientStateMerger = RecipientStateMerger(
            ...
            zonarosaServiceAddressCache: SSKEnvironment.shared.zonarosaServiceAddressCacheRef,
        )
        ...
        do {
            ...
            if deviceMessages.isEmpty {
                if messageSend.isSelfSend {
                    ...
                    await SSKEnvironment.shared.databaseStorageRef.awaitableWrite { tx in
                        message.updateWithSkippedRecipients([ZonaRosaServiceAddress(messageSend.serviceId)], tx: tx)
                    }
                    return []
                }
    ...
    private func checkIfAccountExists(serviceId: ServiceId) async throws {
        if !(try await self.accountChecker.checkIfAccountExists(serviceId: serviceId)) {
            ...
            throw MessageSenderNoSuchZonaRosaRecipientError()
        }
    }
    ...
    ///   issue network requests to fetch pre keys to establish missing ZonaRosa
    ///   Protocol sessions. (If we don't establish a ZonaRosa Protocol session
    ...
    ///   it will either encrypt using already-available ZonaRosa Protocol
    ...
    func buildDeviceMessages(
        ...
        (deviceMessages, missingSessionPlaintextContent) = try await databaseStorage.awaitableWrite { tx -> ([DeviceMessage], [DeviceId: Data]) in
            ...
            for deviceId in deviceIds {
                ...
                } catch ZonaRosaError.sessionNotFound(_) {
    ...
    private func buildDeviceMessage(
        ...
        do {
            ...
            let zonarosaProtocolStoreManager = DependenciesBridge.shared.zonarosaProtocolStoreManager
            let aciSessionStore = zonarosaProtocolStoreManager.zonarosaProtocolStore(for: .aci).sessionStore
            aciSessionStore.archiveSession(forServiceId: serviceId, deviceId: deviceId, tx: tx)
            throw OWSRetryableMessageSenderError()
        } catch ZonaRosaError.untrustedIdentity {
            Logger.warn("Found untrusted identity on outgoing message to \(serviceId). Wrapping error and throwing...")
            throw UntrustedIdentityError(serviceId: serviceId)
        } catch {
            switch error {
            case ZonaRosaError.sessionNotFound:
    ...
    private func _updateDevices(
        serviceId: ServiceId,
        recipient: inout ZonaRosaRecipient,
        devicesToAdd: [DeviceId],
        devicesToRemove: [DeviceId],
        tx: DBWriteTransaction,
    ) {
    ...
    private func encryptMessage(
        ...
        let identityManager = DependenciesBridge.shared.identityManager
        let zonarosaProtocolStoreManager = DependenciesBridge.shared.zonarosaProtocolStoreManager
        let zonarosaProtocolStore = zonarosaProtocolStoreManager.zonarosaProtocolStore(for: .aci)
        let preKeyStore = zonarosaProtocolStoreManager.preKeyStore.forIdentity(.aci)
        ...
        if let sealedSenderParameters {
            let secretCipher = SMKSecretSessionCipher(
                sessionStore: zonarosaProtocolStore.sessionStore,
                preKeyStore: preKeyStore,
                signedPreKeyStore: preKeyStore,
                kyberPreKeyStore: preKeyStore,
                identityStore: try identityManager.libZonaRosaStore(for: .aci, tx: transaction),
            ...
            serializedMessage = try secretCipher.encryptMessage(
                ...
                contentHint: sealedSenderParameters.contentHint.zonarosaClientHint,
            ...
            let result = try zonarosaEncrypt(
        ...
        let session = try zonarosaProtocolStore.sessionStore.loadSession(
            for: protocolAddress,
            context: transaction,
        )!
    ...
    private func wrapPlaintextMessage(
        ...
        guard let session = try validSession(for: serviceId, deviceId: deviceId, tx: transaction) else {
            throw ZonaRosaError.sessionNotFound("")
        }
        return DeviceMessage(
            type: messageType,
            destinationDeviceId: deviceId,
