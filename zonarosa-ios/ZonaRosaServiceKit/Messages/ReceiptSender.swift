    ...
    private func sendReceipts(
        ...
        repeat {
            let sendResult = await SSKEnvironment.shared.databaseStorageRef.awaitableWrite { tx -> (sendPromise: Promise<Void>, batchEndIndex: Int)? in
                ...
                if SSKEnvironment.shared.blockingManagerRef.isAddressBlocked(ZonaRosaServiceAddress(aci), transaction: tx) {
                    Logger.warn("Dropping receipts for blocked \(aci)")
                    return (.value(()), remainingTimestamps.endIndex)
                }

                let recipientHidingManager = DependenciesBridge.shared.recipientHidingManager
                if recipientHidingManager.isHiddenAddress(ZonaRosaServiceAddress(aci), tx: tx) {
                ...
                guard identityManager.untrustedIdentityForSending(to: ZonaRosaServiceAddress(aci), untrustedThreshold: nil, tx: tx) == nil else {
                ...
                let thread = TSContactThread.getOrCreateThread(withContactAddress: ZonaRosaServiceAddress(aci), transaction: tx)
            ...
            } catch let error as MessageSenderNoSuchZonaRosaRecipientError {
