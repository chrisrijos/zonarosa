...
public class SMKSecretSessionCipher: NSObject {
    private let currentSessionStore: LibZonaRosaClient.SessionStore
    private let currentPreKeyStore: LibZonaRosaClient.PreKeyStore
    ...
    private let currentSenderKeyStore: LibZonaRosaClient.SenderKeyStore

    // public SecretSessionCipher(ZonaRosaProtocolStore zonarosaProtocolStore) {
    init(
        sessionStore: LibZonaRosaClient.SessionStore,
        preKeyStore: LibZonaRosaClient.PreKeyStore,
        signedPreKeyStore: SignedPreKeyStore,
        kyberPreKeyStore: KyberPreKeyStore,
        identityStore: IdentityKeyStore,
        senderKeyStore: LibZonaRosaClient.SenderKeyStore,
    ) {
    ...
    // public Pair<ZonaRosaProtocolAddress, byte[]> decrypt(CertificateValidator validator, byte[] ciphertext, long timestamp)
    ...
    public func decryptMessage(
        ...
        do {
            ...
            // return new Pair<>(new ZonaRosaProtocolAddress(content.getSenderCertificate().getSender(),
    ...
    private func decrypt(messageContent: UnidentifiedSenderMessageContent, context: StoreContext) throws -> Data {
        ...
        // ZonaRosaProtocolAddress sender = new ZonaRosaProtocolAddress(message.getSenderCertificate().getSender(),
        ...
        // case CiphertextMessage.WHISPER_TYPE: return new SessionCipher(zonarosaProtocolStore, sender).decrypt(new
        // ZonaRosaMessage(message.getContent())); case CiphertextMessage.PREKEY_TYPE: return new
        // SessionCipher(zonarosaProtocolStore, sender).decrypt(new PreKeyZonaRosaMessage(message.getContent())); default: throw
        // new InvalidMessageException("Unknown type: " + message.getType());
        // }
        let plaintextData: Data
        switch messageContent.messageType {
        case .zonarosa:
            let cipherMessage = try ZonaRosaMessage(bytes: messageContent.contents)
            plaintextData = try zonarosaDecrypt(
        ...
        case .preKey:
            let cipherMessage = try PreKeyZonaRosaMessage(bytes: messageContent.contents)
            plaintextData = try zonarosaDecryptPreKey(
                message: cipherMessage,
                from: ProtocolAddress(from: sender),
                sessionStore: currentSessionStore,
                identityStore: currentIdentityStore,
