...
public class OWSUDManagerImpl: OWSUDManager {
    ...
    public func udAccessKey(for aci: Aci, tx: DBReadTransaction) -> SMKUDAccessKey? {
        ...
        guard let profileKey = profileManager.userProfile(for: ZonaRosaServiceAddress(aci), tx: tx)?.profileKey else {
            return nil
        }
...
public class OWSMockUDManager: OWSUDManager {
    ...
    public var trustRoots: [LibZonaRosaClient.PublicKey] = []

    public func setUnidentifiedAccessMode(_ mode: UnidentifiedAccessMode, for aci: LibZonaRosaClient.Aci, tx: DBWriteTransaction) {
    }

    public func udAccessKey(for aci: LibZonaRosaClient.Aci, tx: DBReadTransaction) -> SMKUDAccessKey? {
        return nil
    }

    public func udAccess(for aci: LibZonaRosaClient.Aci, tx: DBReadTransaction) -> OWSUDAccess? {
        return nil
    }

    public func fetchAllAciUakPairs(tx: DBReadTransaction) -> [LibZonaRosaClient.Aci: SMKUDAccessKey] {
        return [:]
    }
