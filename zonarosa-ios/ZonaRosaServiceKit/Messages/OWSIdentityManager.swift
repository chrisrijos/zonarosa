...
public protocol OWSIdentityManager {
    func libZonaRosaStore(for identity: OWSIdentity, tx: DBReadTransaction) throws -> IdentityStore
    ...
    func recipientIdentity(for address: ZonaRosaServiceAddress, tx: DBReadTransaction) -> OWSRecipientIdentity?
    ...
    func identityKey(for address: ZonaRosaServiceAddress, tx: DBReadTransaction) -> Data?
    ...
    func insertSessionSwitchoverEvent(for recipient: ZonaRosaRecipient, phoneNumber: String?, tx: DBWriteTransaction)
    func mergeRecipient(_ recipient: ZonaRosaRecipient, into targetRecipient: ZonaRosaRecipient, tx: DBWriteTransaction)

    func untrustedIdentityForSending(
        to address: ZonaRosaServiceAddress,
    ...
    func verificationState(for address: ZonaRosaServiceAddress, tx: DBReadTransaction) -> VerificationState
    func setVerificationState(
        _ verificationState: VerificationState,
        of identityKey: Data,
        for address: ZonaRosaServiceAddress,
...
public class IdentityStore: IdentityKeyStore {
    ...
    public func identity(for address: ProtocolAddress, context: StoreContext) throws -> LibZonaRosaClient.IdentityKey? {
...
public class OWSIdentityManagerImpl: OWSIdentityManager {
    ...
    private let pniProtocolStore: ZonaRosaProtocolStore
    ...
    init(
        ...
        pniProtocolStore: ZonaRosaProtocolStore,
    ...
    public func libZonaRosaStore(for identity: OWSIdentity, tx: DBReadTransaction) throws -> IdentityStore {
    ...
    public func recipientIdentity(for address: ZonaRosaServiceAddress, tx: DBReadTransaction) -> OWSRecipientIdentity? {
    ...
    public func identityKey(for address: ZonaRosaServiceAddress, tx: DBReadTransaction) -> Data? {
    ...
    private func _saveIdentityKey(_ identityKey: Data, for serviceId: ServiceId, recipient: ZonaRosaRecipient, tx: DBWriteTransaction) -> IdentityChange {
    ...
    public func insertIdentityChangeInfoMessage(
        ...
        let contactThread = TSContactThread.getOrCreateThread(
            withContactAddress: ZonaRosaServiceAddress(serviceId),
            transaction: tx,
        )
        let contactThreadMessage: TSErrorMessage = .nonblockingIdentityChange(
            thread: contactThread,
            address: ZonaRosaServiceAddress(serviceId),
            wasIdentityVerified: wasIdentityVerified,
        )
        contactThreadMessage.anyInsert(transaction: tx)

        for groupThread in TSGroupThread.groupThreads(with: ZonaRosaServiceAddress(serviceId), transaction: tx) {
    ...
    public func insertSessionSwitchoverEvent(
        for recipient: ZonaRosaRecipient,
        phoneNumber: String?,
        tx: DBWriteTransaction,
    ) {
    ...
    public func mergeRecipient(_ recipient: ZonaRosaRecipient, into targetRecipient: ZonaRosaRecipient, tx: DBWriteTransaction) {
    ...
    private func buildVerificationStateSyncMessage(
        ...
        let value: Any? = queuedVerificationStateSyncMessagesKeyValueStore.getObject(
            key,
            ofClasses: [NSNumber.self, NSString.self, ZonaRosaServiceAddress.self],
            transaction: tx,
        )
        ...
        case is ZonaRosaServiceAddress:
            recipientUniqueId = key
        case let stringValue as NSString:
            // Previously, we stored phone numbers in this KV store.
            let address = ZonaRosaServiceAddress.legacyAddress(serviceId: nil, phoneNumber: stringValue as String)
    ...
    public func verificationState(for address: ZonaRosaServiceAddress, tx: DBReadTransaction) -> VerificationState {
    ...
    public func setVerificationState(
        ...
        guard var recipient = recipientDatabaseTable.fetchRecipient(address: address, tx: tx) else {
            owsFailDebug("Missing ZonaRosaRecipient")
            return .error
        }
        ...
        default:
            if isUserInitiatedChange {
                ...
                case .verified:
                    ...
                    // to the profile whitelist so they become a "ZonaRosa
...
class OWSIdentityManagerObjCBridge: NSObject {
    @objc
    static func identityKey(forAddress address: ZonaRosaServiceAddress) -> Data? {
