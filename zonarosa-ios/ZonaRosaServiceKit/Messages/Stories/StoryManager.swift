...
public class StoryManager {
    ...
    public class func processIncomingStoryMessage(
        ...
        guard !SSKEnvironment.shared.blockingManagerRef.isAddressBlocked(ZonaRosaServiceAddress(author), transaction: transaction) else {
        ...
        if let masterKey = storyMessage.group?.masterKey {
            ...
            guard SSKEnvironment.shared.profileManagerRef.isUser(inProfileWhitelist: ZonaRosaServiceAddress(author), transaction: transaction) else {
        ...
        case .authorAci(let authorAci):
            ...
            _ = TSContactThread.getOrCreateThread(withContactAddress: ZonaRosaServiceAddress(authorAci), transaction: transaction)
...
extension StoryManager {
    ...
    public static func buildStoryHeaders() -> HttpHeaders {
        ["X-ZonaRosa-Receive-Stories": areStoriesEnabled ? "true" : "false"]
    }
    ...
    func threadUniqueId(transaction: DBReadTransaction) -> String? {
        ...
        case .authorAci(let authorAci):
            return TSContactThread.getWithContactAddress(
                ZonaRosaServiceAddress(authorAci),
                transaction: transaction,
            )?.uniqueId
