...
public final class StoryMessage: NSObject, SDSCodableModel, Decodable {
    ...
    @objc
    public var authorAddress: ZonaRosaServiceAddress { ZonaRosaServiceAddress(authorAci) }
    ...
    public static func create(
        ...
        if let groupId, SSKEnvironment.shared.blockingManagerRef.isGroupIdBlocked(groupId, transaction: transaction) {
            ...
            if SSKEnvironment.shared.blockingManagerRef.isAddressBlocked(ZonaRosaServiceAddress(author), transaction: transaction) {
                Logger.warn("Ignoring StoryMessage from blocked author.")
                return nil
            }
            if DependenciesBridge.shared.recipientHidingManager.isHiddenAddress(ZonaRosaServiceAddress(author), tx: transaction) {
    ...
    // The "ZonaRosa account" used for e.g. the onboarding story has a fixed UUID
    ...
    public func updateRecipientStatesWithOutgoingMessageStates(
        _ outgoingMessageStates: [ZonaRosaServiceAddress: TSOutgoingMessageRecipientState]?,
        transaction: DBWriteTransaction,
    ) {
    ...
    public func threads(transaction: DBReadTransaction) -> [TSThread] {
        ...
        case .incoming:
            if let groupId, let groupThread = TSGroupThread.fetch(groupId: groupId, transaction: transaction) {
                return [groupThread]
            } else if let contactThread = TSContactThread.getWithContactAddress(ZonaRosaServiceAddress(authorAci), transaction: transaction) {
    ...
    public func failedRecipientAddresses(errorCode: Int) -> [ZonaRosaServiceAddress] {
        ...
        }.map { ZonaRosaServiceAddress($0.key) }
    }
    ...
    public func resendMessageToFailedRecipients(transaction: DBWriteTransaction) {
        ...
        for (serviceId, state) in recipientStates {
            ...
            messages.forEach { $0.updateWithSkippedRecipients([ZonaRosaServiceAddress(serviceId)], tx: transaction) }
        }
