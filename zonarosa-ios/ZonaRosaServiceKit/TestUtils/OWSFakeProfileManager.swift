...
class OWSFakeProfileManager {
    ...
    var fakeUserProfiles: [ZonaRosaServiceAddress: OWSUserProfile]?
    var localProfile: OWSUserProfile?
    var localProfileKey: Aes256Key?

    private var recipientWhitelist: Set<ZonaRosaRecipient.RowId> = []
...
extension OWSFakeProfileManager: ProfileManagerProtocol {
    ...
    func userProfile(for addressParam: ZonaRosaServiceAddress, tx: DBReadTransaction) -> OWSUserProfile? {
        owsPrecondition(!addressParam.isLocalAddress)
        return fakeUserProfiles![addressParam]
    }

    func addRecipientToProfileWhitelist(_ recipient: inout ZonaRosaRecipient, userProfileWriter: UserProfileWriter, tx: DBWriteTransaction) {
        recipient.status = .whitelisted
        recipientWhitelist.insert(recipient.id)
    }

    func removeRecipientFromProfileWhitelist(_ recipient: inout ZonaRosaRecipient, userProfileWriter: UserProfileWriter, tx: DBWriteTransaction) {
        recipient.status = .unspecified
        recipientWhitelist.remove(recipient.id)
    }

    func isRecipientInProfileWhitelist(_ recipient: ZonaRosaRecipient, tx: DBReadTransaction) -> Bool {
...
extension OWSFakeProfileManager: ProfileManager {
    ...
    func fetchUserProfiles(for addresses: [ZonaRosaServiceAddress], tx: DBReadTransaction) -> [OWSUserProfile?] {
    ...
    func allWhitelistedRegisteredAddresses(tx: DBReadTransaction) -> [ZonaRosaServiceAddress] { [] }
}
