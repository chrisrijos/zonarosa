...
extension Pni {
    ...
    public static func parseFrom(ambiguousString: String?) -> Pni? {
        ...
        // Give LibZonaRosa the first pass at parsing a "PNI:"-prefixed value.
...
extension ProtocolAddress {
    ...
    public var deviceIdObj: DeviceId {
        // LibZonaRosa also enforces [1, 127], so this can't fail.
...
/// `ZonaRosaRecipient`, but existing types may store a ServiceId/E164 pair.
///
...
public struct NormalizedDatabaseRecordAddress {
    ...
    public init?(address: ZonaRosaServiceAddress?) {
