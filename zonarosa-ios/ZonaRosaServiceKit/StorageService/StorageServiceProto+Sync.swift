...
struct StorageServiceContact {
    ...
    init?(_ zonarosaRecipient: ZonaRosaRecipient) {
        let unregisteredAtTimestamp: UInt64?
        if zonarosaRecipient.isRegistered {
            unregisteredAtTimestamp = nil
        } else {
            unregisteredAtTimestamp = (
                zonarosaRecipient.unregisteredAtTimestamp ?? ZonaRosaRecipient.Constants.distantPastUnregisteredTimestamp,
            )
        }
        self.init(
            aci: zonarosaRecipient.aci,
            phoneNumber: E164.expectNilOrValid(stringValue: zonarosaRecipient.phoneNumber?.stringValue),
...
class StorageServiceContactRecordUpdater: StorageServiceRecordUpdater {
    ...
    func buildRecord(
        ...
        let anyAddress = ZonaRosaServiceAddress(contact.serviceIds.aciOrElsePni)
        ...
        let systemContact = { () -> ZonaRosaAccount? in
            guard let phoneNumber = contact.phoneNumber else {
                return nil
            }
            return contactsManager.fetchZonaRosaAccount(
    ...
    private func _mergeRecord(
        ...
        let anyAddress = ZonaRosaServiceAddress(serviceIds.aciOrElsePni)
    ...
    private func mergeSystemContactNames(
        ...
        // the merging logic will delete the ZonaRosaAccount.
        guard let phoneNumber = recipient.phoneNumber?.stringValue else {
            return false
        }

        let localAccount = contactsManager.fetchZonaRosaAccount(
        ...
        let newAccount: ZonaRosaAccount?
        ...
        if let systemFullName {
            ...
            // `ZonaRosaAccount`s are associated with the same system contact.
            // For example, Alice may have a work and mobile number, both of
            // of which are registered with ZonaRosa. This text could be (work)
            // or (mobile), to help disambiguate - otherwise, both ZonaRosa
            // accounts will present as just "Alice".
            let multipleAccountLabelText = ""

            newAccount = ZonaRosaAccount(
        ...
        default:
            ...
            // case and `didModifyZonaRosaAccount` will remain false.
            var didModifyZonaRosaAccount = false
            if let localAccount {
                localAccount.anyRemove(transaction: tx)
                didModifyZonaRosaAccount = true
            }
            if let newAccount {
                newAccount.anyInsert(transaction: tx)
                didModifyZonaRosaAccount = true
            }
            if didModifyZonaRosaAccount {
                contactsManager.didUpdateZonaRosaAccounts(transaction: tx)
            }
            ...
            if aciToUpdate != nil {
                ...
                zonarosaServiceAddressCache.updateRecipient(recipient, tx: tx)
            }
        }
...
class StorageServiceAccountRecordUpdater: StorageServiceRecordUpdater {
    ...
    func buildRecord(
        ...
        let backupLevel: LibZonaRosaClient.BackupLevel? = switch backupPlanManager.backupPlan(tx: transaction) {
    ...
    private func mergeBackupPlan(
        ...
        if let backupTierRawValue = record.backupTier {
            if
                ...
                let backupLevel = LibZonaRosaClient.BackupLevel(rawValue: backupTierUInt8)
            {
    ...
    private func processPinnedConversationsProto(
        ...
        for pinnedConversation in pinnedConversations {
            ...
            case .contact(let contact)?:
                let address = ZonaRosaServiceAddress.legacyAddress(
