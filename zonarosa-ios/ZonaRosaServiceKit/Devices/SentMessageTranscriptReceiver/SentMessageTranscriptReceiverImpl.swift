...
public class SentMessageTranscriptReceiverImpl: SentMessageTranscriptReceiver {
    ...
    private func process(
        ...
        let recipientStates: [ZonaRosaServiceAddress: TSOutgoingMessageRecipientState] = {
            switch messageParams.target {
    ...
    private func archiveSessions(for address: ZonaRosaServiceAddress, tx: DBWriteTransaction) {
        self.zonarosaProtocolStoreManager.zonarosaProtocolStore(for: .aci).sessionStore.archiveSessions(forAddress: address, tx: tx)
    }
}
