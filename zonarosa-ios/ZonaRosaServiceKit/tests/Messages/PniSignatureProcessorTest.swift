...
private class MockRecipientMerger: RecipientMerger {
    func applyMergeForLocalAccount(aci: Aci, phoneNumber: E164, pni: Pni?, tx: DBWriteTransaction) -> ZonaRosaRecipient {
        fatalError()
    }

    func applyMergeFromStorageService(localIdentifiers: LocalIdentifiers, isPrimaryDevice: Bool, serviceIds: AtLeastOneServiceId, phoneNumber: E164?, tx: DBWriteTransaction) -> ZonaRosaRecipient {
        fatalError()
    }

    func applyMergeFromContactSync(localIdentifiers: LocalIdentifiers, aci: Aci, phoneNumber: E164?, tx: DBWriteTransaction) -> ZonaRosaRecipient {
        fatalError()
    }

    func applyMergeFromContactDiscovery(localIdentifiers: LocalIdentifiers, phoneNumber: E164, pni: Pni, aci: Aci?, tx: DBWriteTransaction) -> ZonaRosaRecipient? {
        fatalError()
    }

    func applyMergeFromSealedSender(localIdentifiers: LocalIdentifiers, aci: Aci, phoneNumber: E164?, tx: DBWriteTransaction) -> ZonaRosaRecipient {
        fatalError()
    }
    ...
    func splitUnregisteredRecipientIfNeeded(localIdentifiers: LocalIdentifiers, unregisteredRecipient: inout ZonaRosaRecipient, tx: DBWriteTransaction) {
        fatalError()
    }
}
...
final class PniSignatureProcessorTest: XCTestCase {
    ...
    private var aci: Aci!
    private var aciRecipient: ZonaRosaRecipient!
    private var aciIdentityKeyPair: IdentityKeyPair!

    private var pni: Pni!
    private var phoneNumber: E164!
    private var pniRecipient: ZonaRosaRecipient!
    ...
    override func setUp() {
        ...
        (aciRecipient, pniRecipient) = mockDB.write { tx in
            return (
                try! ZonaRosaRecipient.insertRecord(aci: aci, tx: tx),
                try! ZonaRosaRecipient.insertRecord(phoneNumber: phoneNumber, pni: pni, tx: tx),
            )
        }
