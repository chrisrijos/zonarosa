...
class MessageDecryptionTest: SSKBaseTest {
    ...
    lazy var remoteClient: TestZonaRosaClient = FakeZonaRosaClient.generate(e164Identifier: remoteE164Identifier)

    private lazy var localClient = LocalZonaRosaClient()
    private lazy var localPniClient = LocalZonaRosaClient(identity: .pni)
    ...
    private func generateAndDecrypt(
        ...
        write { transaction in
            let localClient: TestZonaRosaClient
            ...
            var contentProto = ZonaRosaServiceProtos_Content()
    ...
    func testDecryptPreKeyPniWithAciDestinationUuid() {
        expectDecryptionFailure(
            ...
            if let error = error as? OWSError {
                ...
                if case ZonaRosaServiceKit.PreKeyStore.Error.noPreKeyWithId(_)? = underlyingError {
                    return true
                }
            }
            return false
