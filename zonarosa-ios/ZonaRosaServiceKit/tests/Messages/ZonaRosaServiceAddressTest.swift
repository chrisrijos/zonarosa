...
class ZonaRosaServiceAddressTest: XCTestCase {

    private lazy var cache = ZonaRosaServiceAddressCache()
    private lazy var mockDb = InMemoryDB()

    private func makeAddress(serviceId: ServiceId? = nil, phoneNumber: String? = nil) -> ZonaRosaServiceAddress {
    ...
    private func makeHighTrustAddress(aci: Aci? = nil, phoneNumber: String? = nil) -> ZonaRosaServiceAddress {
    ...
    func testInitializers() {
        ...
        var recipient = mockDb.write { tx in
            return try! ZonaRosaRecipient.insertRecord(aci: aci, phoneNumber: pn_a, tx: tx)
        }

        cache.updateRecipient(recipient, isPhoneNumberVisible: true)

        let address1 = ZonaRosaServiceAddress(
            serviceId: aci,
            phoneNumber: nil,
            cache: cache,
        )
        let address2 = ZonaRosaServiceAddress(
            serviceId: aci,
            phoneNumber: pn_a.stringValue,
            cache: cache,
        )
        let address3 = ZonaRosaServiceAddress(
            serviceId: aci,
    ...
    func testInitializerPerformance() {
        ...
        let recipient = mockDb.write { tx in
            return try! ZonaRosaRecipient.insertRecord(aci: serviceId, phoneNumber: phoneNumber, tx: tx)
        }
        cache.updateRecipient(recipient, isPhoneNumberVisible: true)

        var addresses = [ZonaRosaServiceAddress]()
        addresses.reserveCapacity(iterations)

        for _ in 0..<iterations {
            addresses.append(ZonaRosaServiceAddress(
                serviceId: serviceId,
    ...
    func testPotentiallyVisible() {
        ...
        var aciAddresses = [ZonaRosaServiceAddress]()
        var pniAddresses = [ZonaRosaServiceAddress]()

        func addBatch(isVisible: Bool) {
            let ambiguousAddress = ZonaRosaServiceAddress(
                serviceId: nil,
                phoneNumber: phoneNumber?.stringValue,
                cache: cache,
            )
            ...
            aciAddresses.append(ZonaRosaServiceAddress(
                serviceId: nil,
            ...
            pniAddresses.append(ZonaRosaServiceAddress(
                serviceId: pni,
                phoneNumber: nil,
                cache: cache,
            ))
        ...
        let recipient = mockDb.write { tx in
            return try! ZonaRosaRecipient.insertRecord(aci: aci, phoneNumber: phoneNumber, pni: pni, tx: tx)
        }
...
class ZonaRosaServiceAddress2Test: SSKBaseTest {
    func testPersistence() throws {
        struct TestCase {
            var originalAddress: ZonaRosaServiceAddress
        ...
        let testCases: [TestCase] = [
            TestCase(
                originalAddress: ZonaRosaServiceAddress(serviceId: aci, phoneNumber: phoneNumber),
                decodedServiceId: aci,
                decodedPhoneNumber: nil,
            ),
            TestCase(
                originalAddress: ZonaRosaServiceAddress(phoneNumber: phoneNumber),
                decodedServiceId: nil,
                decodedPhoneNumber: phoneNumber,
            ),
            TestCase(
                originalAddress: ZonaRosaServiceAddress(serviceId: pni, phoneNumber: nil),
        ...
        for testCase in testCases {
            do {
                ...
                let decodedValue = try JSONDecoder().decode(ZonaRosaServiceAddress.self, from: encodedValue)
            ...
            do {
                ...
                let decodedValue = try NSKeyedUnarchiver.unarchivedObject(
                    ofClass: ZonaRosaServiceAddress.self,
                    from: encodedValue,
                )!
                XCTAssertEqual(decodedValue.serviceId, testCase.decodedServiceId)
                XCTAssertEqual(decodedValue.phoneNumber, testCase.decodedPhoneNumber)
