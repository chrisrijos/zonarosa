...
class ZonaRosaAccountTest: XCTestCase {
    ...
    func testInMemoryDatabaseRoundTrip() {
        for (idx, (constant, _)) in ZonaRosaAccount.constants.enumerated() {
            inMemoryDB.insert(record: constant)

            guard let deserialized = inMemoryDB.fetchExactlyOne(modelType: ZonaRosaAccount.self) else {
    ...
    func testHardcodedJsonDataDecodes() {
        ...
        case .printStrings:
            ZonaRosaAccount.printHardcodedJsonDataForConstants()
        case .runTest:
            for (idx, (constant, jsonData)) in ZonaRosaAccount.constants.enumerated() {
                do {
                    let decoded = try JSONDecoder().decode(ZonaRosaAccount.self, from: jsonData)
    ...
    /// This test ensures that json from the era before `ZonaRosaAccount`
    /// was an `SDSCodableModel` can still be decoded.
    func testHardcodedJsonLegacyDataDecodes() {
        for (idx, (constant, jsonData)) in ZonaRosaAccount.legacyConstants.enumerated() {
            do {
                let decoded = try JSONDecoder().decode(ZonaRosaAccount.self, from: jsonData)
                try constant.validate(against: decoded)
            } catch let error where error is DecodingError {
    ...
    func testContactNameComponents() {
        ...
        let nameComponents = zonarosaAccount.contactNameComponents()!
...
extension ZonaRosaAccount: ValidatableModel {
    static let constants: [(ZonaRosaAccount, jsonData: Data)] = [
        (
            ZonaRosaAccount(
    ...
    /// Note: These strings were generated before migrating `ZonaRosaAccount` from
    ...
    static let legacyConstants: [(ZonaRosaAccount, jsonData: Data)] = [
        (
            ZonaRosaAccount(
                recipientPhoneNumber: "little",
                recipientServiceId: nil, // Was hardcoded to a non-ServiceId string
    ...
    func validate(against otherAccount: ZonaRosaAccount) throws {
