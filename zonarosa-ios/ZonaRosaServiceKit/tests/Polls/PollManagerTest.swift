...
struct PollManagerTest {
    ...
    private var recipient: ZonaRosaRecipient!
    ...
    init() throws {
        ...
        recipient = db.write { tx in
            return try! ZonaRosaRecipient.insertRecord(aci: pollAuthorAci, phoneNumber: pollAuthorPhoneNumber, pni: pollAuthorPni, tx: tx)
        }
    }
    ...
    private func insertZonaRosaRecipient(aci: Aci, pni: Pni, phoneNumber: E164) {
        db.write { tx in
            _ = try! ZonaRosaRecipient.insertRecord(aci: aci, phoneNumber: phoneNumber, pni: pni, tx: tx)
        }
    }
    ...
    func testOutgoingPollVote() throws {
        ...
        // Before voting, insert voters into ZonaRosa Recipient Table
        // which is referenced by id in the vote table.
    ...
    func testPendingThenSentVote_singleSelect() throws {
        ...
        let zonarosaRecipient = db.read { tx in
        ...
        var voteCount = try db.write { tx in
            try pollStore.applyPendingVote(
                ...
                localRecipientId: zonarosaRecipient!.id,
        ...
        _ = try db.write { tx in
            try pollStore.updatePollWithVotes(
                ...
                voteAuthorId: zonarosaRecipient!.id,
