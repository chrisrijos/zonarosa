...
class ZonaRosaRecipientTest: SSKBaseTest {
    ...
    func testHighTrustFullyQualified() {
        ...
        let addressToBeUpdated = ZonaRosaServiceAddress(phoneNumber: phoneNumber.stringValue)
    ...
    func testHighTrustPhoneNumberChange() {
        ...
        let oldAddress = ZonaRosaServiceAddress(serviceId: aci, phoneNumber: oldPhoneNumber.stringValue)

        // Do this because of ZonaRosaServiceAddressTest.test_hashStability2().
        _ = ZonaRosaServiceAddress(serviceId: aci, phoneNumber: newPhoneNumber.stringValue)
        ...
        write { transaction in
            ...
            let newPhoneNumberProfile = OWSUserProfile(
                address: .otherUser(ZonaRosaServiceAddress(phoneNumber: newPhoneNumber.stringValue)),
            )
            newPhoneNumberProfile.anyInsert(transaction: transaction)

            let oldAccount = ZonaRosaAccount(phoneNumber: oldAddress.phoneNumber!)
            ...
            let newAddress = ZonaRosaServiceAddress(serviceId: aci, phoneNumber: newPhoneNumber.stringValue)
            ...
            let newAccount = ZonaRosaAccount.anyFetch(
    ...
    func testHighTrustUUIDChange() throws {
        ...
        let phoneNumber = E164("+16505550101")!
        let oldAddress = ZonaRosaServiceAddress(serviceId: oldAci, phoneNumber: phoneNumber.stringValue)
        ...
        try write { transaction in
            ...
            let newAddress = ZonaRosaServiceAddress(serviceId: newAci, phoneNumber: phoneNumber.stringValue)
        ...
        return self.write { tx -> TSGroupThread in
            ...
            let groupThread = {
                return try! GroupManager.createGroupForTests(
                    members: addresses.map { ZonaRosaServiceAddress(serviceId: $0.aci, phoneNumber: $0.phoneNumber?.stringValue) },
                    transaction: tx,
                )
            }()
            ...
            for address in addresses {
                ...
                _ = TSContactThread.getOrCreateThread(
                    withContactAddress: ZonaRosaServiceAddress(serviceId: address.aci, phoneNumber: address.phoneNumber?.stringValue),
                    transaction: tx,
                )
            }
            return groupThread
    ...
    //
    // * There _is_ a ZonaRosaRecipient with (u1, p1).
    // * There _is no_ ZonaRosaRecipient with u2 or p2.
    // * There is a group g1.
    ...
    // * ZonaRosaRecipient.clearDBMappings(forPhoneNumber:) must clean up p2 before
    //   ZonaRosaRecipient.changePhoneNumber() is called on uuidInstance for u1.
    ...
    // * ZonaRosaRecipient.clearDBMappings(forUuid:) must clean up u2 before
    //   ZonaRosaRecipient.changePhoneNumber() is called on phoneNumberInstance for p1.
    ...
    @discardableResult
    private func mergeHighTrust(aci: Aci, phoneNumber: E164?, transaction tx: DBWriteTransaction) -> ZonaRosaRecipient {
    ...
    private func fetchRecipient(aci: Aci, transaction tx: DBReadTransaction) -> ZonaRosaRecipient? {
        return DependenciesBridge.shared.recipientDatabaseTable
            .fetchRecipient(serviceId: aci, transaction: tx)
    }

    private func fetchRecipient(phoneNumber: E164, transaction tx: DBReadTransaction) -> ZonaRosaRecipient? {
        return DependenciesBridge.shared.recipientDatabaseTable
            .fetchRecipient(phoneNumber: phoneNumber.stringValue, transaction: tx)
    }
}

final class ZonaRosaRecipient2Test: XCTestCase {
    ...
    func testDecodeStableRow() throws {
        ...
        try inMemoryDB.write { tx in
            try tx.database.execute(sql: """
                INSERT INTO "model_ZonaRosaRecipient" (
        ...
        inMemoryDB.read { tx in
            let zonarosaRecipients = try! ZonaRosaRecipient.fetchAll(tx.database)
            XCTAssertEqual(zonarosaRecipients.count, 2)

            XCTAssertEqual(zonarosaRecipients[0].id, 18)
            XCTAssertEqual(zonarosaRecipients[0].uniqueId, "00000000-0000-4000-8000-00000000000A")
            XCTAssertEqual(zonarosaRecipients[0].deviceIds, [1, 2, 5, 4, 6].map { DeviceId(validating: $0)! })
            XCTAssertEqual(zonarosaRecipients[0].phoneNumber?.stringValue, "+16505550100")
            XCTAssertEqual(zonarosaRecipients[0].phoneNumber?.isDiscoverable, false)
            XCTAssertEqual(zonarosaRecipients[0].aciString, "00000000-0000-4000-8000-000000000000")
            XCTAssertEqual(zonarosaRecipients[0].unregisteredAtTimestamp, nil)

            XCTAssertEqual(zonarosaRecipients[1].id, 21)
            XCTAssertEqual(zonarosaRecipients[1].uniqueId, "00000000-0000-4000-8000-00000000000B")
            XCTAssertEqual(zonarosaRecipients[1].deviceIds, [])
            XCTAssertEqual(zonarosaRecipients[1].phoneNumber?.stringValue, "+16505550101")
            XCTAssertEqual(zonarosaRecipients[1].phoneNumber?.isDiscoverable, false)
            XCTAssertEqual(zonarosaRecipients[1].aciString, "00000000-0000-4000-8000-000000000001")
            XCTAssertEqual(zonarosaRecipients[1].unregisteredAtTimestamp, 1683679214631)
        }
    }
    ...
    func testDecodePni() throws {
        ...
        inMemoryDB.read { tx in
            let zonarosaRecipients = try! ZonaRosaRecipient.fetchAll(tx.database)
            XCTAssertEqual(zonarosaRecipients.count, 1)

            XCTAssertEqual(zonarosaRecipients[0].id, 1)
            XCTAssertEqual(zonarosaRecipients[0].uniqueId, "00000000-0000-4000-8000-000000000000")
            XCTAssertEqual(zonarosaRecipients[0].deviceIds, [])
            XCTAssertEqual(zonarosaRecipients[0].pni, Pni.constantForTesting("PNI:10000000-2000-4000-8000-300000000004"))
        }
    }
    ...
    func testEncodePni() throws {
        ...
        try inMemoryDB.write { tx in
            _ = try ZonaRosaRecipient.insertRecord(pni: pni, tx: tx)
        }
        inMemoryDB.read { tx in
            let db = tx.database
            let rawPniValue = try! String.fetchOne(db, sql: #"SELECT "pni" FROM "model_ZonaRosaRecipient""#)!
    ...
    func testUnregisteredTimestamps() {
        ...
        let recipientManager = ZonaRosaRecipientManagerImpl(
    ...
    func testDeDupe() throws {
        ...
        try databaseQueue.write { db in
            ...
            try db.execute(
                sql: "INSERT INTO model_ZonaRosaRecipient (recipientUUID) VALUES (?)",
                arguments: [aci1.serviceIdUppercaseString],
            )
            try db.execute(
                sql: "INSERT INTO model_ZonaRosaRecipient (recipientUUID, recipientPhoneNumber) VALUES (?, ?)",
                arguments: [aci1.serviceIdUppercaseString, phoneNumber1],
            )
            try db.execute(
                sql: "INSERT INTO model_ZonaRosaRecipient (recipientPhoneNumber) VALUES (?)",
            ...
            do {
                ...
                try GRDBSchemaMigrator.dedupeZonaRosaRecipients(tx: tx)
            }
            let recipientIds = try Int64.fetchAll(db, sql: "SELECT id FROM model_ZonaRosaRecipient")
            XCTAssertEqual(recipientIds, [1, 4, 6, 7])
            let phoneNumbers = try (String?).fetchAll(db, sql: "SELECT recipientPhoneNumber FROM model_ZonaRosaRecipient ORDER BY id")
            XCTAssertEqual(phoneNumbers, [nil, phoneNumber2, nil, phoneNumber3])
            let aciStrings = try (String?).fetchAll(db, sql: "SELECT recipientUUID FROM model_ZonaRosaRecipient ORDER BY id")
