...
final class ContactDiscoveryManagerTest: XCTestCase {
    private class MockContactDiscoveryTaskQueue: ContactDiscoveryTaskQueue {
        var onPerform: ((Set<String>, ContactDiscoveryMode) async throws -> [ZonaRosaRecipient])?

        func perform(for phoneNumbers: Set<String>, mode: ContactDiscoveryMode) async throws -> [ZonaRosaRecipient] {
            return try await onPerform!(phoneNumbers, mode)
        }

        static func foundResponse(for phoneNumbers: Set<String>) -> [ZonaRosaRecipient] {
            let db = InMemoryDB()
            return db.write { tx in
                return phoneNumbers.map {
                    return try! ZonaRosaRecipient.insertRecord(
    ...
    func testQueueing() async throws {
        ...
        let initialRequest = CancellableContinuation<CheckedContinuation<[ZonaRosaRecipient], any Error>>()
