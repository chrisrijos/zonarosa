...
private class MockStorageServiceManager: StorageServiceManager {
    ...
    func recordPendingUpdates(updatedAddresses: [ZonaRosaServiceAddress]) {}
...
private class TestDependencies {
    ...
    let sessionStore: ZonaRosaServiceKit.SessionStore
    ...
    init(observers: [RecipientMergeObserver] = []) {
        ...
        sessionStore = ZonaRosaServiceKit.SessionStore()
...
class RecipientMergerTest: XCTestCase {
        ...
        for (idx, testCase) in testCases.enumerated() {
            ...
            func run(transaction: DBWriteTransaction) {
                for initialRecipient in testCase.initialState {
                    let recipient = try! ZonaRosaRecipient.insertRecord(
                ...
                var recipientTable = [ZonaRosaRecipient.RowId: ZonaRosaRecipient]()
                ...
                for finalRecipient in testCase.finalState.reversed() {
                    let zonarosaRecipient = recipientTable.removeValue(forKey: finalRecipient.rowId)
                    XCTAssertEqual(zonarosaRecipient?.aci, finalRecipient.aci, "\(idx)")
                    XCTAssertEqual(zonarosaRecipient?.phoneNumber?.stringValue, finalRecipient.phoneNumber?.stringValue, "\(idx)")
                }
    ...
    func testAciPhoneNumberPniMerges() throws {
        ...
        for testCase in testCases {
            ...
            let mergedRecipient = d.mockDB.write { tx in
                for initialState in testCase.initialState {
                    _ = try! ZonaRosaRecipient.insertRecord(
                        aci: initialState.aci,
                        phoneNumber: initialState.phoneNumber,
    ...
    func testSessionSwitchoverEvents() throws {
        ...
        for testCase in testCases {
            ...
            let recipients = d.mockDB.write { tx in
                let recipient1 = try! ZonaRosaRecipient.insertRecord(aci: nil, phoneNumber: phone1, pni: pni1, tx: tx)
                let recipient2 = try! ZonaRosaRecipient.insertRecord(aci: aci2, phoneNumber: phone2, pni: pni2, tx: tx)
            ...
            d.mockDB.write { tx in
                for recipientNumber in testCase.hasSession {
                    ...
                    let thread = TSContactThread(contactAddress: ZonaRosaServiceAddress(
    ...
    func testPniSignatureMerge() throws {
        ...
        let aciRecipient = d.mockDB.write { tx in
            let aciRecipient = try! ZonaRosaRecipient.insertRecord(aci: aci, tx: tx)
            d.sessionStore.upsertSession(
            ...
            let pniRecipient = try! ZonaRosaRecipient.insertRecord(phoneNumber: phoneNumber, pni: pni, tx: tx)
            d.sessionStore.upsertSession(
