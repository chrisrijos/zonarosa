...
class AttachmentUploadManagerMockHelper {
    ...
    var mockNetworkManager = AttachmentUploadManagerImpl.Mocks.NetworkManager(appReadiness: AppReadinessMock(), libzonarosaNet: nil)
    var mockServiceManager = OWSZonaRosaServiceMock()
    ...
    func setup(
        ...
        mockServiceManager.mockUrlSessionBuilder = { (info: ZonaRosaServiceInfo, endpoint: OWSURLSessionEndpoint, config: URLSessionConfiguration?) in
