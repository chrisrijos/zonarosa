...
class GRDBSchemaMigratorTest: XCTestCase {
    ...
    private func encodeGroupIdInGroupModel(groupId: Data) -> Data {
        ...
        coder.setClassName("ZonaRosaServiceKit.TSGroupModelV2", for: TSGroupModelWithOnlyGroupId.self)
    ...
    func testMigrateRemovePhoneNumbers() throws {
        ...
        try databaseQueue.write { db in
            try db.execute(sql: """
            CREATE TABLE "model_ZonaRosaRecipient" (
                "id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
                "recipientPhoneNumber" TEXT,
                "recipientUUID" TEXT
            );
            CREATE UNIQUE INDEX "RecipientAciIndex" ON "model_ZonaRosaRecipient" ("recipientUUID");
            CREATE UNIQUE INDEX "RecipientPhoneNumberIndex" ON "model_ZonaRosaRecipient" ("recipientPhoneNumber");
            ...
            """)
            try GRDBSchemaMigrator.removeLocalProfileZonaRosaRecipient(in: db)
    ...
    func testMigrateBlockedRecipients() throws {
        ...
        try databaseQueue.write { db in
            ...
            CREATE TABLE "model_ZonaRosaAccount" (
                "id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
                "recipientPhoneNumber" TEXT UNIQUE
            );

            INSERT INTO "model_ZonaRosaAccount" ("recipientPhoneNumber") VALUES
                ('+17635550103');
            """)
    ...
    func testMigrateBlockedGroups() throws {
        ...
        coder.setClassName("ZonaRosaServiceKit.TSGroupModelV2", for: TSGroupModelV2MigrateBlockedGroups.self)
    ...
    func testPopulateDefaultAvatarColorsTable() throws {
        ...
        try databaseQueue.write { db in
            ...
            INSERT INTO model_ZonaRosaRecipient VALUES
    ...
    @objc(SampleZonaRosaServiceAddress)
    private class SampleZonaRosaServiceAddress: NSObject, NSSecureCoding {
    ...
    private static func encodedAddresses(_ addresses: [SampleZonaRosaServiceAddress]) throws -> Data {
        let coder = NSKeyedArchiver(requiringSecureCoding: true)
        coder.setClassName("ZonaRosaServiceKit.ZonaRosaServiceAddress", for: SampleZonaRosaServiceAddress.self)
        coder.encode(addresses, forKey: NSKeyedArchiveRootObjectKey)
        return coder.encodedData
    }

    func testDecodeZonaRosaServiceAddresses() throws {
        let exampleAddresses: [SampleZonaRosaServiceAddress] = [
        ...
        let decodedAddresses = try GRDBSchemaMigrator.decodeZonaRosaServiceAddresses(dataValue: encodedAddresses)
    ...
    func testCreateStoryRecipients() throws {
        ...
        try databaseQueue.write { db in
            ...
            let zonarosaRecipients = try Row.fetchAll(
                db,
                sql: "SELECT * FROM model_ZonaRosaRecipient ORDER BY id",
            )
            XCTAssertEqual(zonarosaRecipients.count, 6)
            XCTAssertEqual(zonarosaRecipients[3]["recipientUUID"], "00000000-0000-4000-A000-000000000AAA")
            XCTAssertEqual(zonarosaRecipients[3]["recipientPhoneNumber"], nil as String?)
            XCTAssertEqual(zonarosaRecipients[3]["pni"], nil as String?)

            XCTAssertEqual(zonarosaRecipients[4]["recipientUUID"], nil as String?)
            XCTAssertEqual(zonarosaRecipients[4]["recipientPhoneNumber"], nil as String?)
            XCTAssertEqual(zonarosaRecipients[4]["pni"], "PNI:00000000-0000-4000-A000-000000000BBB")

            XCTAssertEqual(zonarosaRecipients[5]["recipientUUID"], nil as String?)
            XCTAssertEqual(zonarosaRecipients[5]["recipientPhoneNumber"], "+17635550142")
            XCTAssertEqual(zonarosaRecipients[5]["pni"], nil as String?)
        }
    }
    ...
    func testMigrateRecipientDevices() throws {
        ...
        try databaseQueue.write { db in
            ...
            for deviceIds in sampleData {
                try db.execute(sql: "INSERT INTO model_ZonaRosaRecipient (devices) VALUES (?)", arguments: [Self.encodedDeviceIds(deviceIds)])
            }
            do {
                let tx = DBWriteTransaction(database: db)
                defer { tx.finalizeTransaction() }
            ...
            XCTAssertEqual(zonarosaRecipients.count, 3)
            XCTAssertEqual([UInt8](zonarosaRecipients[0]["devices"] as Data), [])
            XCTAssertEqual([UInt8](zonarosaRecipients[1]["devices"] as Data), [1])
            XCTAssertEqual([UInt8](zonarosaRecipients[2]["devices"] as Data), [1, 2, 3])
        }
    }
    ...
    func testMigratePreKeys() throws {
        ...
        try databaseQueue.write { db in
            ...
            let preKey = ZonaRosaServiceKit.PreKeyRecord(
                id: 123,
            ...
            let signedPreKey = ZonaRosaServiceKit.SignedPreKeyRecord(
                id: 234,
            ...
            let kyberPreKeyRecord = try LibZonaRosaClient.KyberPreKeyRecord(
                id: 345,
            ...
            let kyberPreKey = ZonaRosaServiceKit.KyberPreKeyRecord(
                replacedAt: now,
                libZonaRosaRecord: kyberPreKeyRecord,
                isLastResort: false,
            )
    ...
    func testMigrateSessions() throws {
        ...
        try databaseQueue.write { db in
            ...
            try db.execute(sql: """
            CREATE TABLE model_ZonaRosaRecipient (id INTEGER PRIMARY KEY, uniqueId TEXT NOT NULL);
            """)
            ...
            try db.execute(
                sql: "INSERT INTO model_ZonaRosaRecipient (id, uniqueId) VALUES (?, ?)",
    ...
    func testMigrateWhitelist() throws {
        ...
        try databaseQueue.write { db in
            ...
            let recipients = try Row.fetchAll(db, sql: "SELECT * FROM model_ZonaRosaRecipient ORDER BY id")
