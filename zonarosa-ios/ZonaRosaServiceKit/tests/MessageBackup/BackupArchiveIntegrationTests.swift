...
class BackupArchiveIntegrationTests: XCTestCase {
    ...
    /// Describes what output to log if LibZonaRosa reports a test failure.
    private enum LibZonaRosaComparisonFailureLogOutput {
        /// Log the full backup JSONs returned by LibZonaRosa, for external
        /// analysis via `parse-libzonarosa-comparator-failure.py`.
        case fullLibZonaRosaJSON

        /// Log a minimal diff of the backup JSONs returned by LibZonaRosa, for
    ...
    /// runs. Toggle to `.fullLibZonaRosaJSON` if desired during local
    /// development, for more thorough inspection of the failure case.
    private let preferredFailureLogOutput: LibZonaRosaComparisonFailureLogOutput = .minimalDiff
    ...
    /// by LibZonaRosa. They should be equivalent; any disparity indicates that
    ...
    private func runRoundTripTest(
        testCaseFileUrl: URL,
        failureLogOutput: LibZonaRosaComparisonFailureLogOutput,
    ) async throws {
    ...
    private func compareViaLibzonarosa(
        ...
        if sharedTestCaseBackupString != exportedBackupString {
            ...
            case .fullLibZonaRosaJSON:
                ...
                Copy the JSON lines below and run `pbpaste | parse-libzonarosa-comparator-failure.py`.
...
#else
        throw XCTSkip("LibZonaRosaClient.ComparableBackup is only available in the simulator.")
#endif
    }
    ...
    private func initializeApp(dateProvider: DateProvider?) async {
        ...
        /// are bound to the ZonaRosaUI target.
        await MockSSKEnvironment.activate(
            ...
            testDependencies: AppSetup.TestDependencies(
                ...
                networkManager: CrashyMocks.MockNetworkManager(appReadiness: appReadiness, libzonarosaNet: nil),
...
#if targetEnvironment(simulator)
private extension LibZonaRosaClient.ComparableBackup {
