...
extension Stripe {
    /// Create a payment method entry with the ZonaRosa service for a subscription
    /// processed by Stripe.
    ///
    /// - Returns
    /// A Stripe secret used to authorize payment for the new subscription.
    public static func createZonaRosaPaymentMethodForSubscription(subscriberId: Data) async throws -> String {
    ...
    /// A Stripe secret retrieved from the ZonaRosa service, used to authorize
    /// payment for the new subscription. See ``createZonaRosaPaymentMethodForSubscription(withSubscriberId:)``.
    /// - Returns
    /// The new payment ID.
