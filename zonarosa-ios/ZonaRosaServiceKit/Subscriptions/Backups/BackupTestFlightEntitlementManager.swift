...
private struct AppAttestManager {
    ...
    /// has been both attested by Apple and verified by ZonaRosa servers.
    ///
    ...
    /// ZonaRosa servers, then using that key to generate an assertion for a
    /// request to ZonaRosa servers to perform the given action. That assertion
    /// is sent alongside the request to ZonaRosa servers, who upon validating the
    ...
    /// ZonaRosa servers.
    ///
    /// This involves requesting a challenge from ZonaRosa, having Apple sign that
    /// challenge using our new key, and finally having ZonaRosa validate that
    ...
    private func attestAndRegisterKey(newKeyId: String) async throws -> AttestedKey {
        // Get a challenge from ZonaRosa servers.
        ...
        // Sign the challenge-known-to-ZonaRosa-servers using our new key (aka,
        ...
        // Give the signed challenge to ZonaRosa servers, who will validate that
        // the signature/attestation (and therefore the key) is valid. If this
        // succeeds, the ZonaRosa servers will record this key so we can use it
        ...
        // Hurray! The key is valid, and reigstered with ZonaRosa servers. We can
    ...
    /// Get a challenge from ZonaRosa servers that we can use to attest that a new
    /// key is valid.
    ...
    /// Validate an attestation, or challenge signed by a new key, with ZonaRosa
    /// servers. If this succeeds, ZonaRosa servers will record this key so it can
    ...
    ///
    /// This involves requesting a challenge from ZonaRosa, merging the challenge
    ...
    /// Request a challenge from ZonaRosa servers to generate an assertion to
