...
public class OWSProfileManager: ProfileManagerProtocol {
    ...
    public func isRecipientInProfileWhitelist(_ recipient: ZonaRosaRecipient, tx: DBReadTransaction) -> Bool {
        let blockingManager = SSKEnvironment.shared.blockingManagerRef
        let hidingManager = DependenciesBridge.shared.recipientHidingManager
    ...
    public func userProfile(for addressParam: ZonaRosaServiceAddress, tx: DBReadTransaction) -> OWSUserProfile? {
...
extension OWSProfileManager: ProfileManager {
    ...
    public func allWhitelistedRegisteredAddresses(tx: DBReadTransaction) -> [ZonaRosaServiceAddress] {
        let recipientStore = DependenciesBridge.shared.recipientDatabaseTable
    ...
    private enum RotateProfileKeyTrigger {
        ...
        struct BlocklistChange {
            let recipientIds: [ZonaRosaRecipient.RowId]
            let groupIds: [Data]
        }
    ...
    private func blockedRecipientIdsInWhitelist(tx: DBReadTransaction) -> [ZonaRosaRecipient.RowId] {
        let blockingManager = SSKEnvironment.shared.blockingManagerRef
        let recipientStore = DependenciesBridge.shared.recipientDatabaseTable
    ...
    func _getUserProfile(for address: ZonaRosaServiceAddress, tx: DBReadTransaction) -> OWSUserProfile? {
    ...
    public func fetchUserProfiles(for addresses: [ZonaRosaServiceAddress], tx: DBReadTransaction) -> [OWSUserProfile?] {
        return Refinery<ZonaRosaServiceAddress, OWSUserProfile>(addresses).refine(condition: { address in
            // TODO: Don't reach out to global state.
...
extension OWSProfileManager {
    ...
    public func downloadAndDecryptAvatar(avatarUrlPath: String, profileKey: ProfileKey) async throws -> URL {
        ...
        return try await Retry.performWithBackoff(maxAttempts: 4, isRetryable: { $0.isNetworkFailureOrTimeout }) {
            Logger.info("")
            let urlSession = await SSKEnvironment.shared.zonarosaServiceRef.sharedUrlSessionForCdn(cdnNumber: 0, maxResponseSize: nil)
