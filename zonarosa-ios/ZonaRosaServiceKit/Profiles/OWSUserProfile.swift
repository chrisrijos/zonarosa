...
public final class OWSUserProfile: NSObject, SDSCodableModel, Decodable {
    ...
    public enum Address: Hashable {
        case localUser
        case otherUser(ZonaRosaServiceAddress)
    }
    ...
    public var internalAddress: Address {
        if phoneNumber == Constants.localProfilePhoneNumber {
            ...
            return .otherUser(ZonaRosaServiceAddress.legacyAddress(serviceIdString: serviceIdString, phoneNumber: phoneNumber))
        }
    }
    ...
    public func publicAddress(localIdentifiers: LocalIdentifiers) -> ZonaRosaServiceAddress {
    ...
    public static func internalAddress(for publicAddress: ZonaRosaServiceAddress, localIdentifiers: LocalIdentifiers) -> Address {
    ...
    private static func publicAddress(for internalAddress: Address, localIdentifiers: LocalIdentifiers) -> ZonaRosaServiceAddress {
    ...
    public class func getOrBuildUserProfile(
        ...
        case .otherUser(let serviceId):
            address = .otherUser(ZonaRosaServiceAddress(serviceId))
        case .legacyUserPhoneNumberFromBackupRestore(let phoneNumber):
            address = .otherUser(ZonaRosaServiceAddress(phoneNumber: phoneNumber.stringValue))
        }
    ...
    ///
    /// We should only have one UserProfile for each ZonaRosaRecipient. However,
...
extension OWSUserProfile {
    ...
    private func displayNameBeforeLearningProfileNameIfNecessary(
        ...
        let zonarosaServiceAddress: ZonaRosaServiceAddress
        switch internalAddress {
        case .localUser:
            return nil
        case .otherUser(let _address):
            zonarosaServiceAddress = _address
        }
        ...
        if
            let aci = zonarosaServiceAddress.serviceId as? Aci,
            let username = usernameLookupManager.fetchUsername(forAci: aci, transaction: tx)
        {
            ...
            let serviceId = zonarosaServiceAddress.serviceId,
            ...
            let phoneNumber = recipient.phoneNumber
        {
            ...
            /// exclusively in the ZonaRosa ecosystem (i.e., not including names
