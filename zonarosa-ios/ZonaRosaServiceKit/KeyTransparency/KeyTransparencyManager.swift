...
public final class KeyTransparencyManager {
            ...
            do {
                ...
                try await Retry.performWithBackoff(
                    ...
                    preferredBackoffBlock: { error -> TimeInterval? in
                        switch error {
                        case ZonaRosaError.rateLimitedError(let retryAfter, message: _):
                            return retryAfter
                        default:
                            return nil
                        }
                    ...
                    isRetryable: { error -> Bool in
                        switch error {
                        case ZonaRosaError.rateLimitedError,
                             ZonaRosaError.connectionFailed,
                             ZonaRosaError.ioError,
                             ZonaRosaError.webSocketError:
                            return true
                        default:
                            return false
                        }
    ...
    private func _performCheck(
        ...
        let libZonaRosaStore = KeyTransparencyStoreForLibZonaRosa(
            db: db,
            keyTransparencyStore: keyTransparencyStore,
        )
...
public struct KeyTransparencyStore {
    ...
    private enum KVStoreKeys {
        ...
        /// Keys to an opaque LibZonaRosaClient blob.
    ...
    public func getKeyTransparencyBlob(
        ...
        return failIfThrows {
            try KeyTransparencyRecord.fetchOne(tx.database, key: aci.rawUUID)?.libzonarosaBlob
        }
    }

    public func setKeyTransparencyBlob(
        _ libzonarosaBlob: Data,
        aci: Aci,
        tx: DBWriteTransaction,
    ) {
        failIfThrows {
            let record = KeyTransparencyRecord(
                aci: aci.rawUUID,
                libzonarosaBlob: libzonarosaBlob,
            )
...
// MARK: - LibZonaRosaClient.KeyTransparency.Store

/// An instance type conforming to `LibZonaRosaClient.KeyTransparency.Store`, used
/// exclusively when calling LibZonaRosa's KT APIs.
private struct KeyTransparencyStoreForLibZonaRosa: KeyTransparency.Store {
    let db: DB
...
private struct KeyTransparencyRecord: Codable, FetchableRecord, PersistableRecord {
    ...
    let aci: UUID
    let libzonarosaBlob: Data

    enum CodingKeys: String, CodingKey {
        case aci
        case libzonarosaBlob
    }
}
