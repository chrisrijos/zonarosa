...
const uint64_t kMaxAvatarSize = (kMaxEncryptedAvatarSize
    /* The length of the padding. See GroupSecretParams:encrypt_blob_with_padding in LibZonaRosa. */
    - sizeof(uint32_t)
    ...
    - 0
    /* Encryption tag & nonce. See GroupSecretParams:encrypt_blob in LibZonaRosa. */
    - 16
    - 12
    /* Reserved byte. See GroupSecretParams:encrypt_blob in LibZonaRosa. */
    - 1);
...
- (instancetype)initWithGroupId:(NSData *)groupId
                 ...
                 addedByAddress:(nullable ZonaRosaServiceAddress *)addedByAddress
{
    self = [super init];
    if (!self) {
        return self;
...
{
    ZonaRosaServiceAddress *addedByAddress = self.addedByAddress;
...
- (nullable instancetype)initWithCoder:(NSCoder *)coder
{
    ...
    self->_addedByAddress = [coder decodeObjectOfClass:[ZonaRosaServiceAddress class] forKey:@"addedByAddress"];
    ...
    self->_groupMembers =
        [coder decodeObjectOfClasses:[NSSet setWithArray:@[ [NSArray class], [ZonaRosaServiceAddress class] ]]
                              forKey:@"groupMembers"];
    ...
    if (_groupModelSchemaVersion < 1) {
        ...
        if (memberE164s) {
            NSMutableArray<ZonaRosaServiceAddress *> *memberAddresses = [NSMutableArray new];
            for (NSString *phoneNumber in memberE164s) {
                [memberAddresses addObject:[ZonaRosaServiceAddress legacyAddressWithServiceIdString:nil
