...
public class GroupMembership: NSObject, NSSecureCoding {
    ...
    fileprivate typealias MemberStateMap = [ZonaRosaServiceAddress: GroupMemberState]
    fileprivate typealias InvalidInviteMap = [Data: InvalidInviteModel]
    fileprivate typealias MemberLabelsMap = [Aci: MemberLabel]

    private typealias LegacyMemberStateMap = [ZonaRosaServiceAddress: LegacyMemberState]
    ...
    public required init?(coder: NSCoder) {
        ...
        if let memberStatesData = coder.decodeObject(of: NSData.self, forKey: Self.memberStatesKey) as Data? {
            ...
            let legacyMemberStateMap = coder.decodeDictionary(
                withKeyClass: ZonaRosaServiceAddress.self,
    ...
    @objc
    init(v1Members: [ZonaRosaServiceAddress]) {
        var builder = Builder()
    ...
    public convenience init(membersForTest: [ZonaRosaServiceAddress]) {
    ...
    public var allMembersOfAnyKind: Set<ZonaRosaServiceAddress> {
    ...
    public func isMemberOfAnyKind(_ address: ZonaRosaServiceAddress) -> Bool {
        return memberStates[address] != nil
    }

    public func isMemberOfAnyKind(_ serviceId: ServiceId) -> Bool {
        return isMemberOfAnyKind(ZonaRosaServiceAddress(serviceId))
    }

    // MARK: -

    public func role(for serviceId: ServiceId) -> TSGroupMemberRole? {
        return role(for: ZonaRosaServiceAddress(serviceId))
    }

    public func role(for address: ZonaRosaServiceAddress) -> TSGroupMemberRole? {
        guard let memberState = memberStates[address] else {
            return nil
        }
    ...
    public var fullMemberAdministrators: Set<ZonaRosaServiceAddress> {
        return Set(memberStates.lazy.filter { $0.value.isAdministrator && $0.value.isFullMember }.map { $0.key })
    }

    public var fullMembers: Set<ZonaRosaServiceAddress> {
        return Set(memberStates.lazy.filter { $0.value.isFullMember }.map { $0.key })
    }

    public func isFullMemberAndAdministrator(_ address: ZonaRosaServiceAddress) -> Bool {
        guard let memberState = memberStates[address] else {
            return false
        }
    ...
    public func isFullMemberAndAdministrator(_ serviceId: ServiceId) -> Bool {
        return isFullMemberAndAdministrator(ZonaRosaServiceAddress(serviceId))
    }

    @objc
    public func isFullMember(_ address: ZonaRosaServiceAddress) -> Bool {
        guard let memberState = memberStates[address] else {
            return false
        }
    ...
    public func isFullMember(_ serviceId: ServiceId) -> Bool {
        return isFullMember(ZonaRosaServiceAddress(serviceId))
    }

    /// This method should only be called for full members.
    public func didJoinFromInviteLink(forFullMember address: ZonaRosaServiceAddress) -> Bool {
        guard let memberState = memberStates[address] else {
    ...
    /// this method should only be called for full members.
    public func didJoinFromAcceptedJoinRequest(forFullMember address: ZonaRosaServiceAddress) -> Bool {
        guard let memberState = memberStates[address] else {
            owsFailDebug("Missing member: \(address)")
            return false
        }
    ...
    public var invitedMembers: Set<ZonaRosaServiceAddress> {
        return Set(memberStates.lazy.filter { $0.value.isInvited }.map { $0.key })
    }

    public func isInvitedMember(_ address: ZonaRosaServiceAddress) -> Bool {
        guard let memberState = memberStates[address] else {
            return false
        }
    ...
    public func isInvitedMember(_ serviceId: ServiceId) -> Bool {
        return isInvitedMember(ZonaRosaServiceAddress(serviceId))
    }

    /// This method should only be called on invited members.
    public func addedByAci(forInvitedMember address: ZonaRosaServiceAddress) -> Aci? {
        guard let memberState = memberStates[address] else {
            return nil
        }
        switch memberState {
    ...
    public func addedByAci(forInvitedMember serviceId: ServiceId) -> Aci? {
        return addedByAci(forInvitedMember: ZonaRosaServiceAddress(serviceId))
    }

    // MARK: -

    public var requestingMembers: Set<ZonaRosaServiceAddress> {
        return Set(memberStates.lazy.filter { $0.value.isRequesting }.map { $0.key })
    }

    public func isRequestingMember(_ address: ZonaRosaServiceAddress) -> Bool {
        guard let memberState = memberStates[address] else {
            return false
        }
    ...
    public func isRequestingMember(_ serviceId: ServiceId) -> Bool {
        return isRequestingMember(ZonaRosaServiceAddress(serviceId))
    }
    ...
    public func hasProfileKeyInGroup(serviceId: ServiceId) -> Bool {
        guard let memberState = memberStates[ZonaRosaServiceAddress(serviceId)] else {
            return false
        }
    ...
    public struct Builder {
        ...
        public mutating func remove(_ serviceId: ServiceId) {
            remove(ZonaRosaServiceAddress(serviceId))
        }

        public mutating func remove(_ address: ZonaRosaServiceAddress) {
            remove([address])
        }

        public mutating func remove(_ addresses: Set<ZonaRosaServiceAddress>) {
        ...
        public mutating func addFullMembers(
            _ addresses: Set<ZonaRosaServiceAddress>,
            role: TSGroupMemberRole,
            didJoinFromInviteLink: Bool = false,
            didJoinFromAcceptedJoinRequest: Bool = false,
        ) {
        ...
        public mutating func addInvitedMember(_ serviceId: ServiceId, role: TSGroupMemberRole, addedByAci: Aci) {
            addInvitedMember(ZonaRosaServiceAddress(serviceId), role: role, addedByAci: addedByAci)
        }
        ...
        public mutating func addRequestingMember(_ aci: Aci) {
            addRequestingMember(ZonaRosaServiceAddress(aci))
        }

        public mutating func addRequestingMember(_ address: ZonaRosaServiceAddress) {
            addRequestingMembers([address])
        }

        public mutating func addRequestingMembers(_ addresses: Set<ZonaRosaServiceAddress>) {
        ...
        public func hasMemberOfAnyKind(_ address: ZonaRosaServiceAddress) -> Bool {
        ...
        public func build() -> GroupMembership {
            owsAssertDebug(
                Set(bannedMembers.keys.lazy.map { ZonaRosaServiceAddress($0) })
...
@objc(_TtCC16ZonaRosaServiceKit15GroupMembership11MemberState)
