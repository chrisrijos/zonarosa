...
public class GroupManager: NSObject {
    ...
    public static func doesUserSupportGroupsV2(address: ZonaRosaServiceAddress) -> Bool {
    ...
    public static func localCreateNewGroup(
        seed: NewGroupSeed,
        members membersParam: [ZonaRosaServiceAddress],
    ...
    public static func createGroupForTests(
        members: [ZonaRosaServiceAddress],
            ...
            let remoteContactName: String? = {
                    ...
                    changeAuthor != localIdentifiers.aci
                {
                    return SSKEnvironment.shared.contactManagerRef.displayName(
                        for: ZonaRosaServiceAddress(changeAuthor),
                        tx: transaction,
                    ).resolvedValue()
                }
    ...
    public static func isPossibleGroupInviteLink(_ url: URL) -> Bool {
        ...
        if url.scheme == "https" {
            possibleHosts = ["zonarosa.group"]
        } else if url.scheme == "sgnl" {
            possibleHosts = ["zonarosa.group", "joingroup"]
        } else {
            return false
        }
        ...
        do {
                ...
                !newGroupModel.membership.hasProfileKeyInGroup(serviceId: localIdentifiers.aci)
            {
                ...
                for member in oldMembers {
                    let memberAddress = ZonaRosaServiceAddress(member)
    ...
    private static func autoWhitelistGroupIfNecessary(
        ...
        case .aci(let aci):
            shouldAddToWhitelist = SSKEnvironment.shared.profileManagerRef.isUser(inProfileWhitelist: ZonaRosaServiceAddress(aci), transaction: tx)
        case .localUser:
