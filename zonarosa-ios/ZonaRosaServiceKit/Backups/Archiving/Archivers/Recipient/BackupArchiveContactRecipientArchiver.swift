...
/// Archives ``ZonaRosaRecipient``s as ``BackupProto_Contact`` recipients.
public class BackupArchiveContactRecipientArchiver: BackupArchiveProtoStreamWriter {
    ...
    func archiveAllContactRecipients(
        ...
        func writeToStream(
            ...
            contactDbRowId: ZonaRosaRecipient.RowId?,
        ...
        /// First, we enumerate all `ZonaRosaRecipient`s, which are our "primary
        ...
        /// the recipient.
        let recipientBlock: (ZonaRosaRecipient, BackupArchive.Bencher.FrameBencher) -> Void = { recipient, frameBencher in
            guard
            ...
            /// Track the `ServiceId`s for this `ZonaRosaRecipient`, so we don't
            ...
            let username: String? = recipient.aci
                ...
                .flatMap { username in
                    // Pass through LibZonaRosa to validate the username
            ...
            let contact = self.buildContactRecipient(
                ...
                registration: { () -> BackupProto_Contact.OneOf_Registration in
                    if !recipient.isRegistered {
                            ...
                            BackupArchive.Timestamps.isValid(unregisteredTimestamp)
                        {
                            ...
                            notRegistered.unregisteredTimestamp = ZonaRosaRecipient.Constants.distantPastUnregisteredTimestamp
                        }
                ...
                identity: identity,
                zonarosaAccount: self.contactManager.fetchZonaRosaAccount(
                    recipient.address,
                    tx: context.tx,
                ),
        ...
        do {
            try context.bencher.wrapEnumeration(
                recipientStore.enumerateAllZonaRosaRecipients(tx:block:),
                tx: context.tx,
        ...
        /// After enumerating all `ZonaRosaRecipient`s, we enumerate
        /// `OWSUserProfile`s. It's possible that we'll have an `OWSUserProfile`
        /// for a user for whom we have no `ZonaRosaRecipient`; for example, a
        ...
        /// included in the Backup. However, if we had a `ZonaRosaRecipient` for
        ...
        /// both an `OWSUserProfile` and a `ZonaRosaRecipient` for this entry.
        /// That's fine, even good: ideally we want to move towards a 1:1
        /// relationship between `ZonaRosaRecipient` and other user-related models
        /// like `OWSUserProfile`. If, in the future, we have an enforced 1:1
        /// relationship between `ZonaRosaRecipient` and `OWSUserProfile`, we can
        /// remove this code.
        context.bencher.wrapEnumeration(
            ...
            autoreleasepool {
                ...
                let zonarosaServiceAddress: BackupArchive.InteropAddress
                ...
                case .localUser:
                    ...
                    return
                case .otherUser(let _zonarosaServiceAddress):
                    zonarosaServiceAddress = _zonarosaServiceAddress
                }
                ...
                let contact = self.buildContactRecipient(
                    ...
                    nicknameRecord: nil, // Only contacts with ZonaRosaRecipients can have nicknames.
                    isBlocked: false, // Only contacts with ZonaRosaRecipients can be blocked.
                    isWhitelisted: whitelistedAddresses.contains(zonarosaServiceAddress),
                    ...
                    // profile addresses without ZonaRosaRecipients.
                    identity: nil,
                    zonarosaAccount: nil,
                    defaultAvatarColor: self.avatarDefaultColorManager.defaultColor(
    ...
    /// It is possible to have a TSContactThread for which we have no ZonaRosaRecipient
    /// or OWSUserProfile. One way to create this is to tap "Call with ZonaRosa" from the system
    /// contacts app, for a number that is not registered on ZonaRosa.
    ...
    private func buildContactRecipient(
        ...
        if let zonarosaAccount {
            contact.systemGivenName = zonarosaAccount.givenName
            contact.systemFamilyName = zonarosaAccount.familyName
            contact.systemNickname = zonarosaAccount.nickname
        }
    ...
    func restoreContactRecipientProto(
        ...
        var recipient: ZonaRosaRecipient
        do throws(GRDB.DatabaseError) {
            recipient = try ZonaRosaRecipient.insertRecord(
        ...
        zonarosaServiceAddressCache.updateRecipient(recipient, tx: context.tx)
        ...
        if contactProto.hasIdentityKey {
            ...
            // with the ZonaRosaRecipient and don't need serviceId-based checks.
            let identity = OWSRecipientIdentity(
        ...
        if let systemFullName {
            let systemContact = ZonaRosaAccount(
            ...
            contactManager.insertZonaRosaAccount(systemContact, tx: context.tx)
        }
