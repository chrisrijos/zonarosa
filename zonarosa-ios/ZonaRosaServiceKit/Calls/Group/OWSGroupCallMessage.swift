...
extension OWSGroupCallMessage {
    ...
    private func participantName(for address: ZonaRosaServiceAddress, tx: DBReadTransaction) -> String {
    ...
    public func systemText(tx: DBReadTransaction) -> String {
        ...
        let joinedMemberAddresses = self.joinedMemberAcis.map { ZonaRosaServiceAddress($0.wrappedValue) }
        let addresses = SSKEnvironment.shared.contactManagerRef.sortZonaRosaServiceAddresses(joinedMemberAddresses, transaction: tx)

        var localAddresses = [ZonaRosaServiceAddress]()
        var creatorAddresses = [ZonaRosaServiceAddress]()
        var otherAddresses = [ZonaRosaServiceAddress]()
...
extension OWSGroupCallMessage: OWSPreviewText {
    public func previewText(transaction: DBReadTransaction) -> String {
        ...
        if let creatorAci, ZonaRosaServiceAddress(creatorAci.wrappedAciValue).isLocalAddress {
            return self.groupCallStartedByYou
        }
        if let creatorAci {
            let creatorDisplayName = self.participantName(for: ZonaRosaServiceAddress(creatorAci.wrappedAciValue), tx: transaction)
